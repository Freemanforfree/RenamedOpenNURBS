...
#error Do NOT include opennurbs_version.h in your code.  Use MYON::Version() instead.
#endif
...
//
// branch => 0
// Use MYON::VersionBranch() to get this value.
...
////////////////////////////////////////////////////////////////
//
// MYON_VERSION_NUMBER_FEBDAYS(year) is a macro whose value is
// the number of days in the month of February in a specified
// year.
//
// In almost every situation, it is best to used the function
// call MYON_DaysInMonthOfGregorianYear(year,2) to get this value.
// The MYON_VERSION_NUMBER_FEBDAYS macro is for rare and unusual
// situations where the C preprocessor needs this value.
//
#define MYON_VERSION_NUMBER_FEBDAYS(year) \
    (((year) % 400) == 0 ? 29 : \
        (((year) % 100) == 0 ? 28 : \
            (((year) % 4) == 0 ? 29 : \
                28)))
...
////////////////////////////////////////////////////////////////
//
// MYON_VERSION_NUMBER_DAYOFYEAR(year, month, day_of_month) is a macro
...
// MYON_DayOfGregorianYear(year,month,day_of_month) to get this value.
// The MYON_VERSION_NUMBER_DAYOFYEAR macro is for rare and unusual
// situations where the C preprocessor needs this value.
//
#define MYON_VERSION_NUMBER_DAYOFYEAR(year, month, day_of_month) \
    ( (day_of_month) \
      + ((month) >=  2 ? 31 : 0) \
      + ((month) >=  3 ? MYON_VERSION_NUMBER_FEBDAYS(year) : 0) \
      + ((month) >=  4 ? 31 : 0) \
      + ((month) >=  5 ? 30 : 0) \
      + ((month) >=  6 ? 31 : 0) \
      + ((month) >=  7 ? 30 : 0) \
...
#define MYON_VERSION_NUMBER_TIME(year, month, day_of_month) \
  ((((year)-2000)*367) + (MYON_VERSION_NUMBER_DAYOFYEAR(year,month,day_of_month)))
...
//    6: Mac WIP build
//
// MYON_VERSION_NUMBER_PLATFORM_ID(branch) =
// = 0: developer build
//   1: Windows build
//   2: Mac build
//
// NOTE WELL:
//   MYON_VERSION_NUMBER_PLATFORM_ID(branch) must be a value between 0 and 3.
#define MYON_VERSION_NUMBER_PLATFORM_ID(branch) \
   (((branch) > 0x0U) ? (0x02U - ((branch) % 0x02U)) : 0x0U)

////////////////////////////////////////////////////////////////
//
// MYON_VERSION_NUMBER_CTOR(major,minor,year,month,day_of_month,branch)
...
// In almost every situation, it is best to used the function call
// MYON_VersionNumberConstruct(major,minor,year,month,day_of_month,branch)
// to get this value.  The MYON_VERSION_NUMBER_CTOR macro is for
// rare and unusual situations where the C preprocessor needs
// this value.
//
#define MYON_VERSION_NUMBER_CTOR(major,minor,year,month,day_of_month,branch) \
  (0x80000000U \
  + ((((major)*0x080U + (minor)))*0x010000U \
  + ((MYON_VERSION_NUMBER_TIME(year,month,day_of_month))))*0x04U \
  + ((MYON_VERSION_NUMBER_PLATFORM_ID(branch))))
...
//
// Always use MYON::Version() when you need this value.
// The OPENNURBS_VERSION_NUMBER macro is for rare and unusual
// situations where the C preprocessor needs this value.
//
#define OPENNURBS_VERSION_NUMBER MYON_VERSION_NUMBER_CTOR( \
