...
static bool IsAtoZ(MYON__UINT32 ch)
{
  return ( ch < 0xD800 && iswalpha((wchar_t)ch)) ? true : false;
}

//static bool IsHexDigit(MYON__UINT32 ch)
//{
//  if ( '0' <= ch && ch <= '9')
//    return true;
//  if ( 'A' <= ch && ch <= 'F')
...
static bool IsDigit(MYON__UINT32 ch)
{
...
static const wchar_t* tagUniCharDec          = L"u";      // UNOCODE UTF-16 encoded value as a signed short (0x8...) will be -....
                                                          ...
                                                          // MYON_UnicodeCodePoint::Wastebasket U+1F5D1 (decimal 128465)
...
MYON_TextIterator::MYON_TextIterator(const MYON_wString& str)
  : m_text(str.Array())
  , m_length(str.Length())
{
...
MYON_TextIterator::MYON_TextIterator(const wchar_t* str, size_t length)
  : m_text(str)
  , m_length(length)
{
  Step(); //  Make first character in the string current
...
bool MYON_TextIterator::Step()
{
  ...
  if(m_next_text_ci < m_length)
  {
    ...
    MYON__UINT32 codepoint = 0;
    
    // Works on Windows, Apple, and any other platform.
    const int ci = MYON_DecodeWideChar((m_text+m_next_text_ci), (int)(m_length-m_next_text_ci), &m_ue, &codepoint);
...
bool MYON_TextIterator::PeekCodePoint(MYON__UINT32& unicode_code_point) const
{
  unicode_code_point = m_cur_codepoint;
  return (0 != unicode_code_point);
}

bool MYON_TextIterator::ReadCodePoint(MYON__UINT32& unicode_code_point)
{
  unicode_code_point = m_cur_codepoint;
  Step();
  return (0 != unicode_code_point);
...
static bool Internal_IsHexDigit(MYON__UINT32 x)
{
  return ((x >= '0' && x <= '9') || (x >= 'A' && x <= 'F') || (x >= 'a' && x <= 'f'));
}

static MYON__UINT32 Internal_IsHexValue(MYON__UINT32 x)
{
  if (x >= '0' && x <= '9')
    return (x - '0');
  if (x >= 'A' && x <= 'F')
...
bool MYON_TextIterator::AtBackslashTic() const
{
  return (
    MYON_UnicodeCodePoint::MYON_Backslash == m_cur_codepoint
    && m_next_text_ci < m_length
    && '\'' == m_text[m_next_text_ci]
    );
}

bool MYON_TextIterator::ReadCharValue(
  unsigned char& c
)
{
  for(;;)
  {
    MYON__UINT32 buffer[4];
    if (false == ReadCodePoint(buffer[0]))
      break;
    if (MYON_UnicodeCodePoint::MYON_Backslash != buffer[0])
      break;
    ...
    if (false == Internal_IsHexDigit(buffer[3]))
      break;
    MYON__UINT32 x = 16 * Internal_IsHexValue(buffer[2]) + Internal_IsHexValue(buffer[3]);
    if (x > 255)
      break;
    c = (unsigned char)x;
    return true;
...
MYON__UINT32 MYON_RtfParser::Internal_ParseMBCSString( const MYON__UINT32 windows_code_page )
{
  MYON__UINT32 count = 0;
  bool bParsed = false;
  MYON_String mbcs;
  ...
  {
    const int sWideChar_count0 = MYON_ConvertMSMBCPToWideChar(
      windows_code_page,
      sMBCS,
      sMBCS_count,
      sWideChar,
    ...
    if (sWideChar_count0 > 0)
    {
      ...
      error_status = 0;
      const int sWideChar_count1 = MYON_ConvertMSMBCPToWideChar(
        windows_code_page,
        sMBCS,
        sMBCS_count,
        sWideChar,
      ...
      if (sWideChar_count1 > 0 && 0 != sWideChar[0] && 0 == sWideChar[sWideChar_capacity - 1])
      {
        ...
        {
          struct MYON_UnicodeErrorParameters e = MYON_UnicodeErrorParameters::MaskErrors;
          MYON__UINT32 unicode_code_point = MYON_UnicodeCodePoint::MYON_ReplacementCharacter;
          delta_i = MYON_DecodeWideChar(sWideChar + i, sWideChar_count1 - i, &e, &unicode_code_point);
          if (delta_i <= 0)
          {
            bParsed = false;
            break;
  ...
  if (false == bParsed)
    m_builder.AppendCodePoint(MYON_UnicodeCodePoint::MYON_ReplacementCharacter);
...
MYON_TextBuilder::~MYON_TextBuilder()
{}
...
void MYON_TextBuilder::InitBuilder(const MYON_Font* default_font)
{
  m_in_run = 0;
  m_level = 0;
...
void MYON_TextBuilder::GroupBegin()
{
  m_level++;
}

void MYON_TextBuilder::GroupEnd()
{
...
void MYON_TextBuilder::RunBegin()
{
}

void MYON_TextBuilder::RunEnd()
{
}

bool MYON_TextBuilder::ReadingFontTable()
{
  return (m_level >= m_font_table_level);
}

bool MYON_TextBuilder::ReadingFontDefinition()
{
  return m_bReadingFontDefinition;
}

void MYON_TextBuilder::SetReadingFontDefinition(bool b)
{
  m_bReadingFontDefinition = b;
}

void MYON_TextBuilder::FinishFontDef()
{
  SetReadingFontDefinition(false);
}

void MYON_TextBuilder::FlushText(size_t count, MYON__UINT32* cp)
{
}

void MYON_TextBuilder::BeginFontTable()
{
  m_font_table_level = m_level;
}

void MYON_TextBuilder::BeginHeader()
{
}

// Sets m_default_font_index when \deffn is read.
void MYON_TextBuilder::DefaultFont(const wchar_t* value)
{
  MYON__INT32 nval = -1;
  const wchar_t* sp = MYON_wString::ToNumber(value, nval, &nval);
  if(nval >= 0 && sp > value)
    m_default_font_index = nval;
}

// Process a rtf \fn tag to set the current font to the nth font in the rtf font table
void MYON_TextBuilder::FontTag(const wchar_t* value)
{
  MYON__INT32 nval = -1;
  const wchar_t* sp = MYON_wString::ToNumber(value, nval, &nval);
  if(nval >= 0 && sp > value)
...
const MYON_wString MYON_TextBuilder::FaceNameFromMap(int nval)
{
...
unsigned int MYON_TextBuilder::CodePageFromMap(int nval)
{
  int count = m_facename_map.Count();
  for (int mi = 0; mi < count; mi++)
  {
...
unsigned int MYON_TextBuilder::CharSetFromMap(int nval)
{
  int count = m_facename_map.Count();
  for (int mi = 0; mi < count; mi++)
  {
...
void MYON_TextBuilder::FontSize(const wchar_t* value)
{
}

void MYON_TextBuilder::CharSet(const wchar_t* value)  // \fcharsetN
{
  unsigned int charset = 0;
  const wchar_t* sp = MYON_wString::ToNumber(value, charset, &charset);
  if (sp > value)
  {
...
void MYON_TextBuilder::CodePage(const wchar_t* value)  // \cpgN
{
  unsigned int codepage = 0;
  const wchar_t* sp = MYON_wString::ToNumber(value, codepage, &codepage);
  if (sp > value)
  {
    //if (m_font_table_level >= 0 && m_level >= m_font_table_level)
    if(ReadingFontDefinition())
...
void MYON_TextBuilder::Newline()
{
}

void MYON_TextBuilder::Paragraph()
{
}

void MYON_TextBuilder::ParagraphDefaults()
{
}

void MYON_TextBuilder::Section()
{
}

void MYON_TextBuilder::Tab()
{
}

void MYON_TextBuilder::Bold(const wchar_t* value)
{
}

void MYON_TextBuilder::Italic(const wchar_t* value)
{
}

void MYON_TextBuilder::UnderlineOn()
{
}

void MYON_TextBuilder::UnderlineOff()
{
}

void MYON_TextBuilder::Strikethrough(const wchar_t* value)
{
}

void MYON_TextBuilder::Superscript()
{
}

void MYON_TextBuilder::Subscript()
{
}

void MYON_TextBuilder::NoSuperSub()
{
}

void MYON_TextBuilder::BeginColorTable()
{
}

void MYON_TextBuilder::ColorRed(const wchar_t* value)
{
}

void MYON_TextBuilder::ColorGreen(const wchar_t* value)
{
}

void MYON_TextBuilder::ColorBlue(const wchar_t* value)
{
}

void MYON_TextBuilder::ColorForeground(const wchar_t* value)
{
}

void MYON_TextBuilder::ColorBackground(const wchar_t* value)
{
}

void MYON_TextBuilder::SetStackScale(const wchar_t* value)
{
}

void MYON_TextBuilder::StackFraction(const wchar_t* value)
{
}

void MYON_TextBuilder::StackEnd()
{
}

void MYON_TextBuilder::TextField(const wchar_t* name)
{
}

MYON__UINT32* MYON_TextBuilder::RunCodePoints(const MYON_TextRun& run)
{
  return run.m_codepoints;
}

void MYON_TextBuilder::UniCpCount(const wchar_t* value)
{
  ...
  //short count = 1, d = MYON_DBL_QNAN;
  //const wchar_t* sp = MYON_wString::ToNumber(value, d, &d);
  //if (sp > value)
  //  count = d;
}

void MYON_TextBuilder::UniDecimal(const wchar_t* value)
{
  ...
  for (;;)
  {
    ...
    sp = MYON_wString::ToNumber(value, signed_short, &signed_short);
    ...
    sp = MYON_wString::ToNumber(value, unsigned_short, &unsigned_short);
    ...
    if (0 == m_current_UTF16_buffer_count)
    {
      ...
      unsigned int u32 = 0;
      sp = MYON_wString::ToNumber(value, u32, &u32);
      if (nullptr != sp && sp > value && u32 > 0xffff && MYON_IsValidUnicodeCodePoint(u32) )
      {
  ...
  {
    bool bError = false;
    const MYON__UINT32 error_cp = MYON_UnicodeCodePoint::MYON_ReplacementCharacter;

    const MYON__UINT16 waiting_mark = MYON_TextBuilder::m_UFT16_waiting_mark;
    const MYON__UINT16 unused_mark  = MYON_TextBuilder::m_UFT16_unused_mark;
    ...
    if (false == bError)
    {
      ...
      e.m_error_mask = 16;
      e.m_error_status = 0;
      const int rc = MYON_DecodeUTF16(m_current_UTF16_buffer, m_current_UTF16_buffer_count, &e, &cp);
      bError = (m_current_UTF16_buffer_count != rc || false == MYON_IsValidUnicodeCodePoint(cp));
    }
...
void MYON_TextBuilder::UniEmbeddedDest(const wchar_t* value)
{
  // two embedded unicode destinations - footnote, etc in rtf document
}

void MYON_TextBuilder::UniDest(const wchar_t* value)
{
  // unicode destination
}

void MYON_TextBuilder::LQuote()
{
  m_current_codepoints.Append(0x2018);
}

void MYON_TextBuilder::RQuote()
{
  m_current_codepoints.Append(0x2019);
}

void MYON_TextBuilder::LDblQuote()
{
  m_current_codepoints.Append(0x201c);
}

void MYON_TextBuilder::RDblQuote()
{
  m_current_codepoints.Append(0x201d);
}

void MYON_TextBuilder::Bullet()
{
  m_current_codepoints.Append(0x2022);
}

void MYON_TextBuilder::EnDash()
{
  m_current_codepoints.Append(0x2013);
}

void MYON_TextBuilder::EmDash()
{
  m_current_codepoints.Append(0x2014);
}

bool MYON_TextBuilder::AppendCodePoint(MYON__UINT32 codept)
{
  m_current_codepoints.Append(codept);
  return true;
}

void MYON_TextBuilder::FormatChange()
{
...
MYON_TextRunBuilder::~MYON_TextRunBuilder()
{}
  ...
  , m_text(text)
{
  ...
  const MYON_Font& style_font = dimstyle->Font();  // this is always a managed font
  double stackscale = dimstyle->StackHeightScale();
  MYON_DimStyle::stack_format stackformat = dimstyle->StackFractionFormat();
...
void MYON_TextRunBuilder::InitBuilder(const MYON_Font* default_font)
{
  if (nullptr == default_font)
    default_font = &MYON_Font::Default;
  if (nullptr == default_font)
    return;
  ...
  m_runs = MYON_TextRunArray::EmptyArray;
...
void MYON_TextRunBuilder::AppendCurrentRun()
{
  //if(m_current_run.IsValid())
  {
    MYON_TextRun* run = MYON_TextRun::GetManagedTextRun(m_current_run);
    if (0 != run)
    {
...
bool MYON_TextRunBuilder::AppendCodePoint(MYON__UINT32 codept)
{
  if (m_current_codepoints.Count() == 0 && m_current_run.IsStacked() != MYON_TextRun::Stacked::kStacked)
  {
    const MYON_TextRun::Stacked stacked = m_current_run.IsStacked();

    // First codepoint in a run after format changes
    m_current_run.Init(this->CurrentFont(), m_current_props.Height(), m_current_props.StackScale(), m_current_props.Color(),
      m_current_props.IsBold(), m_current_props.IsItalic(), m_current_props.IsUnderlined(), m_current_props.IsStrikethrough());

    if (MYON_TextRun::Stacked::kTop == stacked || MYON_TextRun::Stacked::kBottom == stacked)
    {
...
void MYON_TextRunBuilder::FormatChange()
{
...
#ifndef MYON_TEXT_BRACKET_FRACTION
...
static void FindFractions(MYON_TextRun* run, MYON_SimpleArray<Fraction>& fractlist)
{
  if (nullptr == run)
    return;
  size_t cpcount = MYON_TextRun::CodepointCount(run->UnicodeString());
  if (0 != cpcount)
  {
    MYON_wString str;
    MYON_TextContext::ConvertCodepointsToString((int)cpcount, run->UnicodeString(), str);
    if (!str.IsEmpty())
    {
...
void MYON_TextRunBuilder::FinishCurrentRun()
{
  //int cplen = m_current_run.m_codepoint_idx.j - m_current_run.m_codepoint_idx.i;
  if (m_current_run.Type() == MYON_TextRun::RunType::kText ||
    m_current_run.Type() == MYON_TextRun::RunType::kField ||
    m_current_run.Type() == MYON_TextRun::RunType::kNewline ||
    m_current_run.Type() == MYON_TextRun::RunType::kParagraph)
  {
    ...
    if (nullptr == this->CurrentFont())
      this->SetCurrentFont(&MYON_Font::Default);

    const MYON_Font* pManagedFont = this->CurrentFont()->ManagedFont();
...
#ifndef MYON_TEXT_BRACKET_FRACTION
    MYON_SimpleArray<Fraction> fractlist;
    int fcount = 0;
    if (MYON_DimStyle::stack_format::None != m_current_props.StackFormat())
    {
    ...
    {
      int cpos = 0;
      int cpcount = (int)MYON_TextRun::CodepointCount(m_current_run.m_codepoints);
      MYON__UINT32* cpts = (MYON__UINT32*)onmalloc((1 + cpcount) * sizeof(MYON__UINT32));
      memcpy(cpts, m_current_run.m_codepoints, (1 + cpcount) * sizeof(MYON__UINT32));
      for (int i = 0; i < fcount; i++)
      {
        ...
        if (f.start >= cpos)
        {
          ...
          if (m_current_props.StackFormat() != MYON_DimStyle::stack_format::None)
            m_current_run.SetStacked(MYON_TextRun::Stacked::kStacked);
          else
            m_current_run.SetStacked(MYON_TextRun::Stacked::kNone);
          AppendCurrentRun();
          m_current_run.SetStacked(MYON_TextRun::Stacked::kNone);
          cpos = f.end + 1;
        }
...
static bool IsValidFontName(const MYON_wString&  name)
{
...
void MYON_TextRunBuilder::FinishFontDef()
{
  if (ReadingFontDefinition())
  {
    int cp32_count = m_current_codepoints.Count();
    ...
    size_t cpcount = MYON_TextRun::CodepointCount(RunCodePoints(m_current_run));
    if (0 != cpcount)
    {
      MYON_wString rtf_fontname_string;
      MYON_TextContext::ConvertCodepointsToString((int)cpcount, RunCodePoints(m_current_run), rtf_fontname_string);
      if (!rtf_fontname_string.IsEmpty())
      {
        ...
        {
          MYON_ERROR("Invalid font name found in rtf string");
          rtf_fontname_string = MYON_Font::Default.RichTextFontName();
          if (rtf_fontname_string.IsEmpty())
        ...
        // for all the places MYON_Font::FontFromRichTextProperties is called in
        //  src4/rhino/... 
        //  opennurbs_textrun.cpp
        //  opennurbs_textiterator.cpp
        // and replace every call to MYON_Font::FontFromRichTextProperties with a call to
        // your new function.
        // 
        // This needs to be the regular face of the rich text quartet.
        const MYON_Font* managed_font = MYON_Font::FontFromRichTextProperties(rtf_fontname_string,false,false,false,false);
        ...
        MYON_FaceNameKey& fn_key = m_facename_map.AppendNew();
    ...
    if (m_current_run.Type() == MYON_TextRun::RunType::kFontdef && m_level == m_font_table_level)
    {
...
void MYON_TextRunBuilder::GroupBegin()  // {
{
  int cp32_count = m_current_codepoints.Count();
  if (cp32_count > 0)
  {
...
void MYON_TextRunBuilder::GroupEnd()   // '}'
{
  int cp32_count = m_current_codepoints.Count();
  if (cp32_count > 0)
  {
...
void MYON_TextRunBuilder::RunBegin()  // like { with no pushing properties
{
  int cp32_count = m_current_codepoints.Count();
  if (cp32_count > 0)
  {
...
void MYON_TextRunBuilder::RunEnd()   // like '}' with no popping properties
{
  int cp32_count = m_current_codepoints.Count();
  if (cp32_count > 0)
  {
...
void MYON_TextRunBuilder::FlushText(size_t count, MYON__UINT32* cp)
{
  ...
  if(ReadingFontDefinition())
  {
    ...
    m_current_run.SetType(MYON_TextRun::RunType::kFontdef);

    MYON_wString str;
    MYON_TextContext::ConvertCodepointsToString((int)count, cp, str);
    if (!str.IsEmpty())
    {
      ...
      const MYON_Font* rtf_fonttbl_font = MYON_Font::FontFromRichTextProperties(str,false,false,false,false);
  ...
  else
  {
    ...
    m_current_run.SetType(MYON_TextRun::RunType::kText);
  }

#if defined(MYON_DEBUG) && defined(MYON_COMPILER_MSC)
  if (m_current_run.IsStacked() == MYON_TextRun::Stacked::kStacked)
    count = count; // good place for a breakpoint
#endif
}

void MYON_TextRunBuilder::BeginFontTable()
{
  m_font_table_level = m_level;
}

void MYON_TextRunBuilder::BeginHeader()
{
  m_current_run.SetType(MYON_TextRun::RunType::kHeader);
}

// Sets m_default_font_index when \deffn is read.
void MYON_TextRunBuilder::DefaultFont(const wchar_t* value)
{
  MYON__INT32 nval = -1;
  const wchar_t* sp = MYON_wString::ToNumber(value, nval, &nval);
  if (nval >= 0 && sp > value)
...
static const MYON_Font* Internal_UpdateManagedFont(
  const MYON_Font* current_managed_font,
  const MYON_wString rtf_font_name,
  bool rtf_bBold,
  bool rtf_bItalic,
...
)
{
  ...
  if (nullptr == current_managed_font)
    current_managed_font = &MYON_Font::Default;

  MYON_wString local_rtf_name(rtf_font_name);
  local_rtf_name.TrimLeftAndRight();
  if (local_rtf_name.IsEmpty())
    local_rtf_name = MYON_Font::RichTextFontName(current_managed_font, true);

  const MYON_Font* new_managed_font = nullptr;

  const bool bChangeFont = MYON_wString::EqualOrdinal(local_rtf_name, MYON_Font::RichTextFontName(current_managed_font, false), true)
  ...
  if (bChangeFont || bChangeBold || bChangeItalic)
  {
    ...
    // be done using MYON_Font::FontFromRichTextProperties().
    // Otherwise this code will not work right for missing fonts.
    new_managed_font = MYON_Font::FontFromRichTextProperties(
      local_rtf_name,
      rtf_bBold,
      rtf_bItalic,
      rtf_bUnderlined,
  ...
  else if (bChangeUnderlined || bChangeStrikethrough)
  {
    ...
    MYON_Font f(*current_managed_font);
...
// Process a rtf \fn tag to set the current font to the nth font in the rtf font table
void MYON_TextRunBuilder::FontTag(const wchar_t* value)
{
  MYON__INT32 nval = -1;
  const wchar_t* sp = MYON_wString::ToNumber(value, nval, &nval);
  if (nval >= 0 && sp > value)
  {
    ...
    else
    {
      // Not defining the font table. Rather, setting a font current
      // Set current font to font corresponding to font_index
      //const MYON_Font* pManagedFont = FindFontInMap(nval);

      const MYON_Font* pManagedFont = (nullptr == this->CurrentFont())
        ? &MYON_Font::Default 
        : this->CurrentFont()->ManagedFont()
        ;

      MYON_wString rft_font_name = FaceNameFromMap(nval);
...
void MYON_TextRunBuilder::FontSize(const wchar_t* value)
{
  // Even though there is supposed to be a way
  // to decode control font height to 3d text height,
  // This always uses the TextHeight from the MYON_TextContent object
  m_current_run.SetTextHeight(m_current_props.Height());


  //wchar_t* eptr = const_cast< wchar_t* >(value);
  //MYON__UINT32 fs = MYON_wString::ToNumber(value, &eptr, 10);
  //double d = 1.0;
  //if(eptr != value && fs > 0)
...
void MYON_TextRunBuilder::Newline()
{
  m_current_run.SetType(MYON_TextRun::RunType::kNewline);
}

void MYON_TextRunBuilder::Paragraph()
{
  m_current_run.SetType(MYON_TextRun::RunType::kParagraph);
}

void MYON_TextRunBuilder::ParagraphDefaults()
{
}

void MYON_TextRunBuilder::Section()
{
}

void MYON_TextRunBuilder::Tab()
{
  for(int i = 0; i < 8; i++) AppendCodePoint((MYON__UINT32)' ');
  m_in_run = true;
}

void MYON_TextRunBuilder::Bold(const wchar_t* value)
{
  bool on = true;
  if (nullptr != value)
  {
  ...
  const MYON_Font* current_font = this->CurrentFont();
  if (false == current_font->IsManagedFont() || on != current_font->IsBoldInQuartet())
  {
    const MYON_Font* managed_font = this->CurrentFont()->ManagedFamilyMemberWithRichTextProperties(
      on,
...
void MYON_TextRunBuilder::Italic(const wchar_t* value)
{
  bool on = true;
  if (nullptr != value)
  {
  ...
  {
    const MYON_Font* managed_font = current_font->ManagedFamilyMemberWithRichTextProperties(
...
void MYON_TextRunBuilder::UnderlineOn()
{
  if (nullptr == this->CurrentFont())
    this->SetCurrentFont(&MYON_Font::Default);
...
void MYON_TextRunBuilder::UnderlineOff()
{
  if (nullptr == this->CurrentFont())
    this->SetCurrentFont(&MYON_Font::Default);
...
void MYON_TextRunBuilder::Strikethrough(const wchar_t* value)
{
  bool on = true;
  if (nullptr != value)
  {
...
void MYON_TextRunBuilder::Superscript()
{
  m_current_run.SetStacked(MYON_TextRun::Stacked::kTop);
}

void MYON_TextRunBuilder::Subscript()
{
  m_current_run.SetStacked(MYON_TextRun::Stacked::kBottom);
}

void MYON_TextRunBuilder::NoSuperSub()
{
}

void MYON_TextRunBuilder::BeginColorTable()
{
}

void MYON_TextRunBuilder::ColorRed(const wchar_t* value)
{
}

void MYON_TextRunBuilder::ColorGreen(const wchar_t* value)
{
}

void MYON_TextRunBuilder::ColorBlue(const wchar_t* value)
{
}

void MYON_TextRunBuilder::ColorForeground(const wchar_t* value)
{
}

void MYON_TextRunBuilder::ColorBackground(const wchar_t* value)
{
}

void MYON_TextRunBuilder::SetStackScale(const wchar_t* value)
{
  double stackscale = MYON_TextRun::DefaultStackFractionHeight();
  MYON_wString::ToNumber(value, stackscale, &stackscale);
  m_current_run.SetStackFractionHeight(stackscale);
  m_current_props.SetStackScale(stackscale);
}

void MYON_TextRunBuilder::StackFraction(const wchar_t* value)
{
  m_current_run.SetType(MYON_TextRun::RunType::kText);
  m_current_run.SetStacked(MYON_TextRun::Stacked::kStacked);
  if (nullptr != m_current_run.m_stacked_text)
    delete m_current_run.m_stacked_text;
  m_current_run.m_stacked_text = new MYON_StackedText;
  if (nullptr != value)
...
void MYON_TextRunBuilder::StackEnd()
{
  m_current_run.SetStackedOff();
}

void MYON_TextRunBuilder::TextField(const wchar_t* name)
{
  m_current_run.SetType(MYON_TextRun::RunType::kField);
}

void MYON_TextRunBuilder::UniEmbeddedDest(const wchar_t* value)
{
  // two embedded unicode destinations - footnote, etc in rtf document
}

void MYON_TextRunBuilder::UniDest(const wchar_t* value)
{
  // unicode destination
}
...
MYON_RtfStringBuilder::~MYON_RtfStringBuilder()
{}
...
void MYON_RtfStringBuilder::InitStringBuilder(const MYON_DimStyle* dimstyle)
{
  if (nullptr == dimstyle)
    dimstyle = &MYON_DimStyle::Default;
  const MYON_Font& style_font = dimstyle->Font();
  MYON_FaceNameKey& fnkey = m_facename_map.AppendNew();
  fnkey.m_rtf_font_name = MYON_Font::RichTextFontName(&style_font, true);
...
void MYON_RtfStringBuilder::PushRun(TextRun& run)
{
  m_run_stack.Append(run);
}
MYON_RtfStringBuilder::TextRun MYON_RtfStringBuilder::PopRun()
{
...
bool MYON_RtfStringBuilder::InColorTable()
{
  return m_in_color_table;
}
void MYON_RtfStringBuilder::SetInColorTable(bool b)
{
  m_in_color_table = b;
}

void MYON_RtfStringBuilder::SetSkipColorTbl(bool b)
{
  m_skip_color_tbl = b;
}
void MYON_RtfStringBuilder::SetSkipBold(bool b)
{
  m_skip_bold = b;
}
void MYON_RtfStringBuilder::SetSkipItalic(bool b)
{
  m_skip_italic = b;
}
void MYON_RtfStringBuilder::SetSkipUnderline(bool b)
{
  m_skip_underline = b;
}
void MYON_RtfStringBuilder::SetSkipFacename(bool b)
{
  m_skip_facename = b;
}

bool MYON_RtfStringBuilder::SkipColorTbl()
{
  return m_skip_color_tbl;
}
bool MYON_RtfStringBuilder::SkipBold()
{
  return m_skip_bold;
}
bool MYON_RtfStringBuilder::SkipItalic()
{
  return m_skip_italic;
}
bool MYON_RtfStringBuilder::SkipUnderline()
{
  return m_skip_underline;
}
bool MYON_RtfStringBuilder::SkipFacename()
{
  return m_skip_facename;
}

void MYON_RtfStringBuilder::SetMakeBold(bool b)
{
  if(b)
    m_skip_bold = true;
  m_make_bold = b;
}
void MYON_RtfStringBuilder::SetMakeItalic(bool b)
{
  if(b)
    m_skip_italic = true;
  m_make_italic = b;
}
void MYON_RtfStringBuilder::SetMakeUnderline(bool b)
{
  if (b)
    m_skip_underline = true;
  m_make_underline = b;
}
void MYON_RtfStringBuilder::SetMakeFacename(bool b)
{
  if (b)
...
bool MYON_RtfStringBuilder::MakeBold()
{
  return m_make_bold;
}
bool MYON_RtfStringBuilder::MakeItalic()
{
  return m_make_italic;
}
bool MYON_RtfStringBuilder::MakeUnderline()
{
  return m_make_underline;
}

bool MYON_RtfStringBuilder::MakeFacename()
{
  return m_make_facename;
}

void MYON_RtfStringBuilder::SetDefaultFacename(const wchar_t* facename)
{
  m_default_facename = (nullptr == facename) ? L"" : facename;
}

void MYON_RtfStringBuilder::SetOverrideFacename(const wchar_t* facename)
{
  m_override_facename = (nullptr == facename) ? L"" : facename;
}

bool MYON_RtfStringBuilder::SkippingFacename()
{
  return (m_skip_facename && !m_make_facename);
}

bool MYON_RtfStringBuilder::SettingFacename()
{
...
void MYON_RtfStringBuilder::GroupBegin()
{
  // not skipping the color table or we're not in it
  if (!SkipColorTbl() || m_current_run.Type() != MYON_TextRun::RunType::kColortbl)
...
void MYON_RtfStringBuilder::GroupEnd()
{
  if (m_current_run.Type() != MYON_TextRun::RunType::kColortbl)
  {
    if (m_level >= 0)
    {
      m_current_run.AddControl(L"}");
      m_level--;
      if (m_current_run.Type() == MYON_TextRun::RunType::kFonttbl)
      {
        if (SkippingFacename())
        {
...
void MYON_RtfStringBuilder::RunBegin()
{
  // not skipping the color table or we're not in it
  if (!SkipColorTbl() || m_current_run.Type() != MYON_TextRun::RunType::kColortbl)
    m_string_out += m_current_run.TextString();
...
void MYON_RtfStringBuilder::RunEnd()
{
  if (m_current_run.Type() != MYON_TextRun::RunType::kColortbl)
  {
    if (m_level >= 0)
...
void MYON_RtfStringBuilder::BeginFontTable()
{
  m_font_table_level = m_level;
  m_current_run.SetType(MYON_TextRun::RunType::kFonttbl);
...
void MYON_RtfStringBuilder::BeginHeader()
{
  m_current_run.SetType(MYON_TextRun::RunType::kHeader);
...
// Sets m_default_font_index when \deffn is read.
void MYON_RtfStringBuilder::DefaultFont(const wchar_t* value)
{
...
const MYON_wString MYON_RtfStringBuilder::OutputString()
{
...
void MYON_RtfStringBuilder::FontTag(const wchar_t* value)
{
  if (SkippingFacename())
    return;
  ...
  if (nval >= 0 && sp > value) // got a number
  {
    if (ReadingFontTable())
    {
      if (m_current_run.Type() == MYON_TextRun::RunType::kFonttbl && m_level == m_font_table_level)
      {
        m_string_out += m_current_run.TextString();
        m_current_run.EmptyText();
        m_current_run.SetTerminated(true);
...
void MYON_RtfStringBuilder::FontSize(const wchar_t* value)
{
...
void MYON_RtfStringBuilder::Newline()
{
  m_current_run.AddControl(L"\\par");
}

void MYON_RtfStringBuilder::Paragraph()
{
  m_current_run.AddControl(L"\\par");
}

void MYON_RtfStringBuilder::ParagraphDefaults()
{
  m_current_run.AddControl(L"\\pard");
}

void MYON_RtfStringBuilder::Section()
{
}

void MYON_RtfStringBuilder::Tab()
{
  for (int i = 0; i < 8; i++) AppendCodePoint((MYON__UINT32)' ');
  m_in_run = true;
}

void MYON_RtfStringBuilder::Bold(const wchar_t* value)
{
  if (!SkipBold())
  {
...
void MYON_RtfStringBuilder::Italic(const wchar_t* value)
{
  if (!SkipItalic())
  {
...
void MYON_RtfStringBuilder::UnderlineOn()
{
  if (!SkipUnderline())
  {
...
void MYON_RtfStringBuilder::UnderlineOff()
{
  if (!SkipUnderline())
  {
    bool under = m_current_run.IsUnderlined();
...
void MYON_RtfStringBuilder::Strikethrough(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::Superscript()
{
}

void MYON_RtfStringBuilder::Subscript()
{
}

void MYON_RtfStringBuilder::NoSuperSub()
{
}

void MYON_RtfStringBuilder::BeginColorTable()
{
  m_current_run.SetType(MYON_TextRun::RunType::kColortbl);
  m_current_run.AddControl(L"\\colortbl");
  SetInColorTable(true);
}

void MYON_RtfStringBuilder::ColorRed(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::ColorGreen(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::ColorBlue(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::ColorForeground(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::ColorBackground(const wchar_t* value)
{
}

void MYON_RtfStringBuilder::TextField(const wchar_t* name)
{
  m_current_run.SetType(MYON_TextRun::RunType::kField);
}

void MYON_RtfStringBuilder::UniEmbeddedDest(const wchar_t* value)
{
  // two embedded unicode destinations - footnote, etc in rtf document
}

void MYON_RtfStringBuilder::UniDecimal(const wchar_t* value)
{
...
void MYON_RtfStringBuilder::UniDest(const wchar_t* value)
{
  // unicode destination
}

bool MYON_RtfStringBuilder::AppendCodePoint(MYON__UINT32 codept)
{
  if ((SettingFacename() || SkippingFacename()) && m_current_run.Type() == MYON_TextRun::RunType::kFontdef)
    return true;
  ...
  MYON_wString str;
  MYON_TextContext::ConvertCodepointsToString(1, &codept, str);
  if (codept == MYON_UnicodeCodePoint::MYON_Backslash || codept == L'{' || codept == L'}')
    m_current_run.AddText(L"\\");
...
MYON_RtfFirstChar::~MYON_RtfFirstChar()
{}
...
void MYON_RtfFirstChar::InitStringBuilder(const MYON_DimStyle* dimstyle)
{
  if (nullptr == dimstyle)
    dimstyle = &MYON_DimStyle::Default;
  const MYON_Font& style_font = dimstyle->Font();
...
void MYON_RtfFirstChar::PushRun(TextRun& run)
{
  m_run_stack.Append(run);
}
MYON_RtfFirstChar::TextRun MYON_RtfFirstChar::PopRun()
{
  if (m_run_stack.Count() > 0)
  {
    TextRun run = *m_run_stack.Last();
...
bool MYON_RtfFirstChar::InColorTable()
{
  return m_in_color_table;
}
void MYON_RtfFirstChar::SetInColorTable(bool b)
{
  m_in_color_table = b;
}


void MYON_RtfFirstChar::GroupBegin()
{
  PushRun(m_current_run);
  m_have_rtf = true;
  m_level++;
...
void MYON_RtfFirstChar::GroupEnd()
{
  if (m_current_run.Type() == MYON_TextRun::RunType::kFonttbl)
    m_font_table_level = 10000;
  ...
  if (m_current_run.Type() == MYON_TextRun::RunType::kFontdef)
  {
...
void MYON_RtfFirstChar::RunBegin()
{
  m_have_rtf = true;
}

void MYON_RtfFirstChar::RunEnd()
{
  if (m_current_run.Type() == MYON_TextRun::RunType::kFontdef)
  {
    // String is a font facename. Make a font with that facename
...
void MYON_RtfFirstChar::BeginFontTable()
{
  m_font_table_level = m_level;
  m_current_run.SetType(MYON_TextRun::RunType::kFonttbl);
}

void MYON_RtfFirstChar::BeginHeader()
{
  m_current_run.SetType(MYON_TextRun::RunType::kHeader);
}

void MYON_RtfFirstChar::BeginColorTable()
{
  m_current_run.SetType(MYON_TextRun::RunType::kColortbl);
  SetInColorTable(true);
}

void MYON_RtfFirstChar::TextField(const wchar_t* name)
{
  m_current_run.SetType(MYON_TextRun::RunType::kField);
}

bool MYON_RtfFirstChar::AppendCodePoint(MYON__UINT32 codept)
{
  if (!m_have_rtf)
    m_have_rtf = true;
...
void MYON_RtfFirstChar::FontTag(const wchar_t* value)
{
    if (ReadingFontTable())
      m_current_run.SetType(MYON_TextRun::RunType::kFontdef);
...
void MYON_RtfFirstChar::Bold(const wchar_t* value)
{
  bool bold = m_current_run.IsBold();
  bool on = true;
  if (nullptr != value)
...
void MYON_RtfFirstChar::Italic(const wchar_t* value)
{
  bool italic = m_current_run.IsItalic();
  bool on = true;
  if (nullptr != value)
...
void MYON_RtfFirstChar::UnderlineOn()
{
    bool under = m_current_run.IsUnderlined();
    bool on = true;
    if (on != under)
...
void MYON_RtfFirstChar::UnderlineOff()
{
  bool under = m_current_run.IsUnderlined();
  bool on = false;
  if (on != under)
...
void MYON_RtfFirstChar::Strikethrough(const wchar_t* value)
{
}
...
MYON_RtfParser::MYON_RtfParser(MYON_TextIterator& iter, MYON_TextBuilder& builder)
  : m_ti(iter)
  , m_builder(builder)
{
}
...
MYON_Color ParseColor(const wchar_t* value)
{
  MYON_Color color;
  return color;
}

bool MYON_RtfParser::ProcessTag(const wchar_t* name, const wchar_t* value, bool optional)
{
  MYON_wString tagname(name);
  if( tagname.IsEmpty() )
    return false;
...
else if (0 == MYON_wString::CompareOrdinal(name, tagField, bOrdinalIgnoreCase))
m_builder.TextField(value);

else if (0 == MYON_wString::CompareOrdinal(name, tagUniCpCount, bOrdinalIgnoreCase))
m_builder.UniCpCount(value);
else if (0 == MYON_wString::CompareOrdinal(name, tagUniCharDec, bOrdinalIgnoreCase))
m_builder.UniDecimal(value);
else if (0 == MYON_wString::CompareOrdinal(name, tagUniTwoDest, bOrdinalIgnoreCase))
m_builder.UniEmbeddedDest(value);
else if (0 == MYON_wString::CompareOrdinal(name, tagUniDest, bOrdinalIgnoreCase))
m_builder.UniDest(value);

else if (0 == MYON_wString::CompareOrdinal(name, taglquote, bOrdinalIgnoreCase))
m_builder.LQuote();
else if (0 == MYON_wString::CompareOrdinal(name, tagrquote, bOrdinalIgnoreCase))
m_builder.RQuote();
else if (0 == MYON_wString::CompareOrdinal(name, tagldblquote, bOrdinalIgnoreCase))
m_builder.LDblQuote();
else if (0 == MYON_wString::CompareOrdinal(name, tagrdblquote, bOrdinalIgnoreCase))
m_builder.RDblQuote();
else if (0 == MYON_wString::CompareOrdinal(name, tagbullet, bOrdinalIgnoreCase))
m_builder.Bullet();
else if (0 == MYON_wString::CompareOrdinal(name, tagendash, bOrdinalIgnoreCase))
m_builder.EnDash();
else if (0 == MYON_wString::CompareOrdinal(name, tagemdash, bOrdinalIgnoreCase))
m_builder.EmDash();
else
rc = false;
...
bool MYON_RtfParser::ReadOptionalTag()
{
  bool rc = false;
  ...
  while (!end_of_tag)
  {
    ...
    switch (cp)
    {
    case MYON_UnicodeCodePoint::MYON_Backslash:
      rc = ReadTag(true);
...
// Called after '\\' is read
// tags can contain up to 32 chars in the name optionally followed by a number
bool MYON_RtfParser::ReadTag(bool optional)
{
  ...
  while(!end_of_tag)
  {
    ...
    {
      end_of_tag = true;
      if(0 != MYON_wString::CompareOrdinal(name, tagUniCharDec, true))
...
bool MYON_RtfParser::FlushCurText(MYON_SimpleArray< MYON__UINT32 >& cp_array)
{
...
bool MYON_RtfParser::Parse()
{
  bool rc = true;
  bool end_of_string = false;
  bool optional_tag = false;
  MYON__UINT32 rtf_code_point;
  ...
  while(!end_of_string)
  {
    if (m_suspend_to_close <= 0 && m_ti.AtBackslashTic())
    {
      ...
      // 
      const MYON__UINT32 char_count = Internal_ParseMBCSString( m_builder.m_current_props.CodePage() );
      if (char_count > 0)
        continue;
    }
    ...
    case MYON_UnicodeCodePoint::MYON_Backslash:
      {
        if ( m_ti.ReadCodePoint(rtf_code_point) )
        {
          ...
          case '\'':
            ...
            MYON_ERROR("Bug in RTF parsing code.");
            break;

            // Symbol tags...
          case '~':  // non-break space
            if (!m_builder.AppendCodePoint((MYON__UINT32)' '))
              return true;
            break;
    ...
    case MYON_UnicodeCodePoint::MYON_NoBreakSpace:
    case MYON_UnicodeCodePoint::MYON_NarrowNoBreakSpace:
    case MYON_UnicodeCodePoint::MYON_ZeroWidthSpace:
      if (!m_builder.AppendCodePoint((MYON__UINT32)' '))
        return true;
      break;
    ...
    case MYON_UnicodeCodePoint::MYON_Tab:
      // '\tab' tag
...
unsigned int RtfComposer::GetFacenameKey(const MYON_Font* font, MYON_ClassArray< MYON_wString >& fonttable)
{
  if (nullptr == font)
    return 0;
  ...
  const MYON_wString rtf_facename = font->QuartetName(MYON_Font::NameLocale::English);

  if (rtf_facename.IsEmpty())
    return 0;
#if defined(MYON_RUNTIME_WIN)
  if (rtf_facename.Length() > 33)
    MYON_ERROR("rtf_facename is longer than Windows LogfontName\n");
#endif

  return RtfComposer::GetFacenameKey(rtf_facename.Array(), fonttable);
}

unsigned int RtfComposer::GetFacenameKey(const wchar_t* facename, MYON_ClassArray< MYON_wString >& fonttable)
{
  ...
  for(int i = 0; i < count; i++)
  {
    if (nullptr == fonttable[i] || 0 == fonttable[i][0])
      continue;
    if(MYON_wString::EqualOrdinal(facename, fonttable[i], true))
      return i;
  }
...
unsigned int RtfComposer::GetColorKey(MYON_Color color, MYON_SimpleArray< unsigned int >& colortable)
{
...
bool RtfComposer::FormatTextHeight(double height, MYON_wString& str)
{
...
static bool GetRunText(MYON_TextRun* run, MYON_wString& text_out, bool& foundunicodepoint)
{
  if (nullptr == run)
    return false;
  ...
  const MYON__UINT32* run_code_points = run->UnicodeString(); // null terminated
  if (nullptr != run_code_points && 0 != run_code_points[0])
  {
    for (int ci = 0; 0 != run_code_points[ci]; ci++)
    {
      const MYON__UINT32 code_point = run_code_points[ci];

      // works on Windows and Apple
      MYON__UINT16 utf16[2] = { 0 };
      const int utf16_count = MYON_EncodeUTF16(code_point, utf16);
      if (utf16_count < 0 || utf16_count > 2 || 0 == utf16[0])
        continue;

      if (code_point > 0x80 || 1 != utf16_count || code_point != (MYON__UINT32)utf16[0])
      {
        // When we write RTF, we do not specify what encodding is used for values in the range 0x80 - 0xFF.
        // 
        // The MYON_wString temp should to have UTF-16 encoding on Windows platforms
        ...
        //
        // See MYON_DecodeWindowsCodePage1252Value() for more details.
        //
        ...
        {
          MYON_wString n;
          const MYON__INT16 signed_rtf_utf16_value = (MYON__INT16)utf16[utf16i]; // will be negative when utf16[utf16i] >= 0x8000;
  ...
  bool bForceRtf)
{
  ...
  const MYON_Font& style_font = text->DefaultFont();
  const MYON_wString style_fontname = style_font.RichTextFontName();
  if (style_fontname.IsEmpty())
    return false;

  bool style_bold = style_font.IsBoldInQuartet();
  bool style_italic = (MYON_Font::Style::Italic == style_font.FontStyle());
  ...
  // First facename is from the MYON_TextContent (style_font)
  // Any after that are from runs
  MYON_ClassArray< MYON_wString > fonttable(8);
  ...
  MYON_SimpleArray< MYON_TextRun* > runholders;
  for (int ri = 0; ri < runcount; ri++)
  {
    MYON_TextRun* run = (*runs)[ri];
    if (nullptr != run)
    {
      if (
        MYON_TextRun::RunType::kText == run->Type() ||
        ...
        )
      {
        ...
        if (nullptr != run_font)
        {
          ...
          const MYON_wString run_fontname = run_font->RichTextFontName();
        ...
        const MYON__UINT32* run_codepoints = run->UnicodeString();
        ...
        MYON_TextRun::RunType::kParagraph == run->Type() ||
        MYON_TextRun::RunType::kNewline == run->Type()
        )
      {
        runholders.AppendNew() = run;
        const MYON_Font* run_font = run->Font();
  ...
  MYON_wString run_strings;
  MYON_wString temp;
  runcount = runholders.Count();
  for (int ri = 0; ri < runcount; ri++)
  {
    MYON_TextRun* run = runholders[ri];
    if (nullptr == run)
      continue;
    ...
    {
      if (make_rtf || (run->IsStacked() == MYON_TextRun::Stacked::kStacked && run->m_stacked_text != 0))
      {
        unsigned int run_font_key = MYON_UNSET_UINT_INDEX;
        if (make_rtf)
        {
          const MYON_Font* run_font = run->Font();
          if (nullptr == run_font)
          ...
          if (run->IsStacked() == MYON_TextRun::Stacked::kBottom)
          {
            run_strings += L"\\sub";
            addspace = true;
          }
          if (run->IsStacked() == MYON_TextRun::Stacked::kTop)
          {
            run_strings += L"\\super";
            addspace = true;
          }
        ...
        if (MYON_TextRun::RunType::kField != run->Type() && run->IsStacked() == MYON_TextRun::Stacked::kStacked && run->m_stacked_text != 0)
        {
        ...
        else if (MYON_TextRun::RunType::kField == run->Type())
        {
          //run_strings += L"%<";
        ...
        if (ri < runcount - 1)
        {
          MYON_TextRun* par_run = runholders[ri+1];
          if (nullptr != par_run && (MYON_TextRun::RunType::kNewline == par_run->Type() || MYON_TextRun::RunType::kParagraph == par_run->Type()))
          {
            const MYON_Font* par_run_font = par_run->Font();
    ...
    else if (MYON_TextRun::RunType::kNewline == run->Type() || MYON_TextRun::RunType::kParagraph == run->Type())
    {
      if (make_rtf)
      {
        temp = L"{\\par}";
        const MYON_Font* par_run_font = run->Font();
        if (nullptr != par_run_font)
        {
          unsigned int par_run_font_key = GetFacenameKey(par_run_font, fonttable);
...
static const MYON_wString Internal_PostScriptNameIfAvailable(const MYON_Font& managed_font)
{
  MYON_wString style_fontname = managed_font.PostScriptName();
  if (style_fontname.IsNotEmpty())
    return style_fontname;

  return MYON_wString::FormatToString(L"Postscript-name-not-available-%u", managed_font.ManagedFontSerialNumber());
}

const MYON_wString RtfComposer::ComposeAppleRTF(
  const MYON_TextContent* text)
{
  MYON_wString rtf_string;
  ...
  const MYON_Font& style_font = text->DefaultFont();
  const MYON_wString style_fontname = Internal_PostScriptNameIfAvailable(style_font);

  // First facename is from the MYON_TextContent
  // Any after that are from runs
  MYON_ClassArray< MYON_wString > fonttable(8);
  ...
  for (int ri = 0; ri < runcount; ri++)
  {
      ...
      )
    {
      ...
      const MYON_wString& run_fontname = Internal_PostScriptNameIfAvailable(*run_font);
      ...
      bool true_bool = true;
      if (run->IsStacked() == MYON_TextRun::Stacked::kStacked && run->m_stacked_text != 0)
      {
        run_strings += L"[[";
      ...
      if (ri < runcount - 2)
      {
        ...
        if (nullptr != par_run && (MYON_TextRun::RunType::kNewline == par_run->Type() || MYON_TextRun::RunType::kParagraph == par_run->Type()))
        {
          ...
          if (nullptr != par_run_font)
          {
            const MYON_wString& par_run_fontname = Internal_PostScriptNameIfAvailable(*par_run_font);
    ...
    else if (ri < runcount - 1 && multiline && (MYON_TextRun::RunType::kNewline == run->Type() || MYON_TextRun::RunType::kParagraph == run->Type()))
    {
      temp = L"{\\par}";
      const MYON_Font* run_font = run->Font();
      if (nullptr != run_font)
