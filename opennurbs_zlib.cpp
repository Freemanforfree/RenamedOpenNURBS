...
#if defined(MYON_COMPILER_MSC) && !defined(MYON_CMAKE_BUILD)
...
class MYON_CompressorImplementation
{
public:
  MYON_CompressorImplementation()
    : m_mode(MYON::archive_mode::unset_archive_mode)
  {
    ClearStream();
  }
  ...
  MYON::archive_mode m_mode; // MYON::archive_mode::read = read and inflate,  MYON::archive_mode::write = deflate and write
  enum 
  {
...
class MYON_CompressorImplementation& MYON_BinaryArchive::Compressor()
{
  if (nullptr == m_compressor)
    m_compressor = (class MYON_CompressorImplementation*)oncalloc(1, sizeof(*m_compressor));
  return *m_compressor;
}


bool MYON_BinaryArchive::WriteCompressedBuffer(
...
#pragma MYON_PRAGMA_WARNING_POP
  ...
  const unsigned int buffer_crc = MYON_CRC32( 0, sizeof__inbuffer, inbuffer );
...
bool MYON_BinaryArchive::ReadCompressedBufferSize( size_t* sizeof__outbuffer )
{
#pragma MYON_PRAGMA_WARNING_PUSH
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC(4996)
#pragma MYON_PRAGMA_WARNING_DISABLE_CLANG("-Wdeprecated-declarations")
...
bool MYON_BinaryArchive::ReadCompressedBuffer( // read and uncompress
  ...
  )
{
  ...
  if (rc ) 
  {
    buffer_crc1 = MYON_CRC32( 0, sizeof__outbuffer, outbuffer );
    if ( buffer_crc1 != buffer_crc0 ) 
    {
      MYON_ERROR("MYON_BinaryArchive::ReadCompressedBuffer() crc error");
      if ( bFailedCRC )
        *bFailedCRC = true;
    }
  }
...
size_t MYON_BinaryArchive::WriteDeflate( // returns number of bytes written
        ...
        )
{
  ...
  */
  ...
  class MYON_CompressorImplementation& m_zlib(Compressor());
  ...
  while( rc && counter > 0 ) 
  {
    ...
    if ( zrc < 0 ) 
    {
      ...
      MYON_ERROR("MYON_BinaryArchive::WriteDeflate - z_deflate failure");
      rc = false;
      break;
    }
...
bool MYON_BinaryArchive::ReadInflate(
        size_t sizeof___outbuffer,  // sizeof uncompressed data
        void* out___buffer          // buffer for uncompressed data
        )
{
  const size_t max_avail = 0x7FFFFFF0; // See max_avail comment in MYON_BinaryArchive::WriteInflate
  ...
  while ( rc && counter > 0 )
  {
    ...
    {
      // Something went haywire - bail out.
      MYON_ERROR("MYON_BinaryArchive::ReadInflate - z_inflate failure");
      rc = false;
      break;
    }
...
bool MYON_BinaryArchive::CompressionInit()
{
  ...
  if ( WriteMode() ) 
  {
    ...
    rc = (m_zlib.m_mode == MYON::archive_mode::write) ? true : false;
    if ( !rc ) {
      CompressionEnd();
      if ( Z_OK == deflateInit( &m_zlib.m_strm, Z_BEST_COMPRESSION ) ) {
        m_zlib.m_mode = MYON::archive_mode::write;
        rc = true;
      }
      else {
  ...
  else if ( ReadMode() ) 
  {
    ...
    rc = (m_zlib.m_mode == MYON::archive_mode::read) ? true : false;
    if ( !rc ) {
      CompressionEnd();
      if ( Z_OK == inflateInit( &m_zlib.m_strm ) ) {
        m_zlib.m_mode = MYON::archive_mode::read;
        rc = true;
      }
      else {
        memset(&m_zlib.m_strm,0,sizeof(m_zlib.m_strm));
...
void MYON_BinaryArchive::CompressionEnd()
{
  ...
  if (0 != m_compressor)
  {
    ...
    m_compressor->m_mode = MYON::archive_mode::unset_archive_mode;
  }
}



struct MYON_CompressedBufferHelper
{
...
MYON_CompressedBuffer::MYON_CompressedBuffer()
...
MYON_CompressedBuffer::MYON_CompressedBuffer(const MYON_CompressedBuffer& src)
                    : m_sizeof_uncompressed(0),
                      m_sizeof_compressed(0),
                      m_crc_uncompressed(0),
                      m_crc_compressed(0),
...
MYON_CompressedBuffer& MYON_CompressedBuffer::operator=(const MYON_CompressedBuffer& src)
{
  if ( this != &src )
  {
    Destroy();
...
bool MYON_CompressedBuffer::Write( MYON_BinaryArchive& binary_archive ) const
{
...
bool MYON_CompressedBuffer::Read( MYON_BinaryArchive& binary_archive )
{
  int major_version = 0;
  int minor_version = 0;
...
void MYON_CompressedBuffer::Destroy()
{
  if ( m_buffer_compressed )
...
bool MYON_CompressedBuffer::Compress(
        size_t sizeof__inbuffer,  // sizeof uncompressed input data
        const void* inbuffer,     // uncompressed input data
        int sizeof_element
        )
{
  ...
  MYON_CompressedBufferHelper helper;
  ...
  case 8:
    if ( 0 == (sizeof__inbuffer%sizeof_element) )
    {
      ...
      bToggleByteOrder = (MYON::endian::big_endian == MYON::Endian());
    }
    break;
  };
  ...
  if ( bToggleByteOrder ) 
  {
    MYON_BinaryFile::ToggleByteOrder( 
  ...
  if (rc)
  {
    m_crc_uncompressed = MYON_CRC32( 0, sizeof__inbuffer, inbuffer );
    m_crc_compressed   = MYON_CRC32( 0, m_sizeof_compressed, m_buffer_compressed );
  }

  return rc;
}

bool MYON_CompressedBuffer::Uncompress(
          void* outbuffer,
          int* bFailedCRC
          ) const
{
  ...
  MYON__UINT32 compressed_crc = MYON_CRC32( 0, m_sizeof_compressed, m_buffer_compressed );
  ...
  case 8:
    ...
    {
      if (MYON::endian::big_endian == MYON::Endian())
      {
        MYON_BinaryFile::ToggleByteOrder( 
  ...
  if (rc ) 
  {
    MYON__UINT32 uncompressed_crc = MYON_CRC32( 0, m_sizeof_uncompressed, outbuffer );
    if ( uncompressed_crc != m_crc_uncompressed ) 
    {
      MYON_ERROR("MYON_CompressedBuffer::Uncompress() crc error");
      if ( bFailedCRC )
        *bFailedCRC = true;
    }
  }
...
bool MYON_CompressedBuffer::WriteChar( 
...
size_t MYON_CompressedBuffer::DeflateHelper( // returns number of bytes written
        MYON_CompressedBufferHelper* helper,
        size_t sizeof___inbuffer,  // sizeof uncompressed input data ( > 0 )
        const void* in___buffer     // uncompressed input data ( != nullptr )
        )
{
  ...
  */
  ...
  MYON_CompressedBufferHelper& m_zlib = *helper;
  ...
  while( rc && counter > 0 ) 
  {
    ...
    {
      // Something went haywire - bail out.
      MYON_ERROR("MYON_CompressedBuffer::DeflateHelper - z_deflate failure");
      rc = false;
      break;
    }
...
bool MYON_CompressedBuffer::InflateHelper(
        MYON_CompressedBufferHelper* helper,
        size_t sizeof___outbuffer,  // sizeof uncompressed data
        void* out___buffer          // buffer for uncompressed data
        ) const
{
  const size_t max_avail = 0x7FFFFFF0; // See max_avail comment in MYON_CompressedBuffer::InflateHelper
  ...
  struct MYON_CompressedBufferHelper& m_zlib = *helper;
  ...
  while ( rc && counter > 0 )
  {
    ...
    {
      // Something went haywire - bail out.
      MYON_ERROR("MYON_CompressedBuffer::InflateHelper - z_inflate failure");
      rc = false;
      break;
    }
...
bool MYON_CompressedBuffer::CompressionInit( struct MYON_CompressedBufferHelper* helper ) const
{
  bool rc = false;
...
bool MYON_CompressedBuffer::CompressionEnd( struct MYON_CompressedBufferHelper* helper ) const
{
  bool rc = false;
