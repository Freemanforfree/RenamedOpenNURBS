...
MYON_OBJECT_IMPLEMENT( MYON_Viewport, MYON_Geometry, "D66E5CCF-EA39-11d3-BFE5-0010830122F0" );
...
{
  const double eps = 2.0*MYON_SQRT_EPSILON;
...
static
bool MYON__IsCameraFrameUnitVectorHelper( const MYON_3dVector& v )
{
  // looser standard than MYON_3dVector::IsUnitVector() so
  // going to/from floats in OpenGL and Direct3d doesn't
  // create "invalid" views.
  return (v.x != MYON_UNSET_VALUE && v.y != MYON_UNSET_VALUE && v.z != MYON_UNSET_VALUE && fabs(v.Length() - 1.0) <= 1.0e-6);
}

static
bool MYON__IsCameraFramePerpindicular( const MYON_3dVector& unit_vector0,const MYON_3dVector& unit_vector1 )
{
  return ( fabs(unit_vector0.x*unit_vector1.x + unit_vector0.y*unit_vector1.y + unit_vector0.z*unit_vector1.z) <= 1.0e-6 );
}

bool 
MYON_GetViewportRotationAngles( 
    const MYON_3dVector& X, // X,Y,Z must be a right handed orthonormal basis
    ...
    )
{
...

  {
    ...
    const double eps = 8.0*MYON_SQRT_EPSILON;
    double dx,dy,dz,d;
    dx = X*X;
    dy = Y*Y;
    dz = Z*Z;
...
void MYON_Viewport::SetPerspectiveClippingPlaneConstraints(
        unsigned int depth_buffer_bit_depth
        )
{
  double min_near_dist = 0.0;
  double min_near_over_far = 0.0;
  MYON_Viewport::GetPerspectiveClippingPlaneConstraints(m_CamLoc,depth_buffer_bit_depth,&min_near_dist,&min_near_over_far);
  SetPerspectiveMinNearDist(min_near_dist);
  SetPerspectiveMinNearOverFar(min_near_over_far);
}


void MYON_Viewport::SetPerspectiveMinNearOverFar(double min_near_over_far)
{
  if (    MYON_IsValid(min_near_over_far) 
       && min_near_over_far > MYON_ZERO_TOLERANCE
       && min_near_over_far < 1.0-MYON_ZERO_TOLERANCE
     )
  {
...
double MYON_Viewport::PerspectiveMinNearOverFar() const
{
  return m__MIN_NEAR_OVER_FAR;
}

void MYON_Viewport::SetPerspectiveMinNearDist(double min_near_dist)
{
  if ( MYON_IsValid(min_near_dist) && min_near_dist > MYON_ZERO_TOLERANCE )
  {
    m__MIN_NEAR_DIST = min_near_dist;
  }
}

double MYON_Viewport::PerspectiveMinNearDist() const
{
  return m__MIN_NEAR_DIST;
}

void MYON_Viewport::Initialize()
{
  *this = MYON_Viewport::DefaultTopViewYUp;
}



bool MYON_Viewport::Read( MYON_BinaryArchive& file )
{
  *this = MYON_Viewport::DefaultTopViewYUp;
  ...
  m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
  ...
  if (rc && major_version==1) 
  {
    ...
    if (rc) rc = file.ReadInt( &i );
    if (rc) m_projection = MYON::ViewProjection(i);
    ...
    if ( m_bValidCamera )
    {
      if ( !MYON_Viewport::IsValidCameraLocation(m_CamLoc) )
      {
        MYON_ERROR("MYON_Viewport.m_bValidCamera in file was true and it should be false.");
        m_bValidCamera = false;
      }
      if ( !MYON_Viewport::IsValidCameraUpOrDirection(m_CamUp) || !MYON_Viewport::IsValidCameraUpOrDirection(m_CamDir) )
      {
        MYON_ERROR("MYON_Viewport.m_bValidCamera in file was true and it should be false.");
        m_bValidCamera = false;
        m_bValidCameraFrame = false;
      ...
      if (!m_bValidCamera)
      {
        if (MYON::view_projection::perspective_view == m_projection)
          SetCamera(MYON_Viewport::DefaultPerspectiveViewZUp, true);
        else
          SetCamera(MYON_Viewport::DefaultTopViewYUp, true);
      }
    }
    ...
    if( m_bValidFrustum )
    {
      if (    !MYON_IsValid(m_frus_left) || !MYON_IsValid(m_frus_right)
           || !MYON_IsValid(m_frus_top)  || !MYON_IsValid(m_frus_bottom)
           || !MYON_IsValid(m_frus_near) || !MYON_IsValid(m_frus_far)
           || !(m_frus_left < m_frus_right)
           || !(m_frus_bottom < m_frus_top)
           || !(0.0 < m_frus_near)
           || !(m_frus_near < m_frus_far)
           || !(-MYON_NONSENSE_WORLD_COORDINATE_VALUE < m_frus_left)
           || !(m_frus_right < MYON_NONSENSE_WORLD_COORDINATE_VALUE)
           || !(-MYON_NONSENSE_WORLD_COORDINATE_VALUE < m_frus_bottom)
           || !(m_frus_top < MYON_NONSENSE_WORLD_COORDINATE_VALUE)
           || !(m_frus_far < MYON_NONSENSE_WORLD_COORDINATE_VALUE)
         )
      {
        MYON_ERROR("MYON_Viewport.m_bValidFrustum in file was true and it should be false.");
        m_bValidFrustum = false;
      }
    }
  }
...
bool MYON_Viewport::Write( MYON_BinaryArchive& file ) const
{
  // 10 Jan 2022 S. Baer
...
*/
bool MYON_Viewport::SetCamera(
  const MYON_Viewport& source_viewport,
  bool bBreakLocks
)
{
  if (bBreakLocks)
...
MYON_SHA1_Hash MYON_Viewport::ViewProjectionContentHash() const
{
...
MYON_Viewport* MYON_Viewport::ShallowCopy(MYON_Viewport* destination) const
{
  if (nullptr == destination)
    destination = new MYON_Viewport();
  else
  {
    destination->PurgeUserData();
    if (this == destination)
      return destination; // The caller is probably confused but this is what they should get.
    *destination = MYON_Viewport::DefaultTopViewYUp; // default ctor values
  }

#define MYON_INTERNAL_SHALLOW_FIELD_COPY(field) destination->field = this->field
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bValidCamera);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bValidFrustum);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bValidPort);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bValidCameraFrame);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_projection);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bLockCamUp);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bLockCamDir);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_bLockCamLoc);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frustum_symmetry_flags);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamLoc);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamDir);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamUp);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamX);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamY);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_CamZ);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_left);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_right);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_bottom);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_top);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_near);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_frus_far);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_left);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_right);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_bottom);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_top);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_near);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_port_far);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_target_point);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_viewport_id);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_clip_mods);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m_clip_mods_inverse);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m__MIN_NEAR_DIST);
  MYON_INTERNAL_SHALLOW_FIELD_COPY(m__MIN_NEAR_OVER_FAR);
#undef MYON_INTERNAL_SHALLOW_COPY
  return destination;
}

bool MYON_Viewport::IsValidCameraLocation(
  MYON_3dPoint candidate_point
)
{
  const double x
    = candidate_point.IsValid()
    ? candidate_point.MaximumCoordinate()
    : MYON_NONSENSE_WORLD_COORDINATE_VALUE;
  return (x < MYON_NONSENSE_WORLD_COORDINATE_VALUE && x >= 0.0);
}

bool MYON_Viewport::IsValidCameraUpOrDirection(
  MYON_3dVector candidate_vector
)
{
  const double x
    ...
    : 0.0;
  return (x < MYON_NONSENSE_WORLD_COORDINATE_VALUE && x > MYON_ZERO_TOLERANCE);
}

bool MYON_Viewport::IsValidCamera() const
{
  return ( m_bValidCamera );
}

bool MYON_Viewport::IsValidCameraFrame() const
{
  return ( m_bValidCameraFrame );
}

bool MYON_Viewport::IsValidFrustum() const
{
  return ( m_bValidFrustum );
}

bool MYON_Viewport::IsValid( MYON_TextLog* text_log ) const
{
  if ( !IsValidCamera() )
  {
    if ( 0 != text_log )
...
int MYON_Viewport::Dimension() const
{
  return 3;
}

bool MYON_Viewport::GetNearPlane( MYON_Plane& near_plane ) const
{
...
bool MYON_Viewport::GetNearPlaneEquation( 
  MYON_PlaneEquation& near_plane_equation 
  ) const
{
...
bool MYON_Viewport::GetFarPlane( MYON_Plane& far_plane ) const
{
  bool rc = IsValidFrustum() && IsValidCamera();
  if ( rc ) 
  {
...
bool MYON_Viewport::GetFarPlaneEquation( 
  MYON_PlaneEquation& far_plane_equation 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
bool MYON_Viewport::GetViewPlane( 
  double view_plane_depth,
  MYON_Plane& view_plane 
  ) const
{
  bool rc = IsValidFrustum() && IsValidCamera();
  if ( rc ) 
...
bool MYON_Viewport::GetViewPlaneEquation( 
  double view_plane_depth,
  MYON_PlaneEquation& view_plane_equation 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
bool MYON_Viewport::GetNearRect( 
       MYON_3dPoint& left_bottom,
       MYON_3dPoint& right_bottom,
       MYON_3dPoint& left_top,
       MYON_3dPoint& right_top
       ) const
{
  MYON_Plane near_plane;
...
bool MYON_Viewport::GetViewPlaneRect(
        double view_plane_depth,
        MYON_3dPoint& left_bottom,
        MYON_3dPoint& right_bottom,
        MYON_3dPoint& left_top,
...
bool MYON_Viewport::GetBBox( 
       double* boxmin,
       double* boxmax,
       bool bGrowBox
       ) const
{
  MYON_3dPoint corners[9];
  ...
  if (rc)
  {
    rc = MYON_GetPointListBoundingBox( 
            3, 0, 9, 
            3, &corners[0].x, 
...
bool MYON_Viewport::Transform( const MYON_Xform& xform )
{
  bool rc = IsValidCamera() && xform.IsValid();
  if (rc)
  {
    // save input settings
    const MYON_3dPoint c0 = m_CamLoc;
    const MYON_3dPoint u0 = m_CamUp;
    const MYON_3dPoint d0 = m_CamDir;
    const MYON_3dPoint x0 = m_CamX;
    const MYON_3dPoint y0 = m_CamY;
    const MYON_3dPoint z0 = m_CamZ;
	  const MYON_3dPoint t0 = m_target_point;
    const bool bValidCamera0 = m_bValidCamera;
    const bool bValidCameraFrame0 = m_bValidCameraFrame;

    // compute transformed settings
    MYON_3dPoint c = xform*c0;
    MYON_3dVector u = (xform*(c0 + u0)) - c;
    MYON_3dVector d = (xform*(c0 + d0)) - c;
    MYON_3dPoint t = t0.IsValid() ? (xform*t0) : MYON_3dPoint::UnsetPoint;
    ...
    else 
    {
      ...
      {
        d.Unitize();
        if ( fabs(d*u) <= MYON_ZERO_TOLERANCE )
          d = -m_CamZ;
      }
...
bool MYON_Viewport::SetCameraLocation( const MYON_3dPoint& p )
{
  if (m_bLockCamLoc && MYON_Viewport::IsValidCameraLocation(m_CamLoc))
  {
    return (p == m_CamLoc);
  }

  if (p == MYON_3dPoint::UnsetPoint)
  {
    m_CamLoc = MYON_3dPoint::UnsetPoint;
    m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
    m_bValidCamera = false;
  }
  else if (MYON_Viewport::IsValidCameraLocation(p))
  {
    m_CamLoc = p;
    m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
...
bool MYON_Viewport::SetCameraDirection( const MYON_3dVector& v )
{
  if ( m_bLockCamDir && MYON_Viewport::IsValidCameraUpOrDirection(m_CamDir) )
  {
    return (v == m_CamDir);
  }

  if (v == MYON_3dVector::UnsetVector)
  {
    m_CamDir = MYON_3dVector::UnsetVector;
    m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
    m_bValidCameraFrame = false;
    m_bValidCamera = false;
  }
...
bool MYON_Viewport::SetCameraUp( const MYON_3dVector& v )
{
  if ( m_bLockCamUp && MYON_Viewport::IsValidCameraUpOrDirection(m_CamUp) )
  {
    return (v == m_CamUp);
  }
  ...
  if (v == MYON_3dVector::UnsetVector)
  {
    m_CamUp = MYON_3dVector::UnsetVector;
    m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
    m_bValidCameraFrame = false;
    m_bValidCamera = false;
  }
...
{
  MYON_ERROR("MYON_Viewport::SetCameraFrame() failed.");
  return false;
}

bool MYON_Viewport::SetCameraFrame()
{
  m_bValidCamera = false;
  m_bValidCameraFrame = false;
  m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
  ...
  if ( !MYON_Viewport::IsValidCameraUpOrDirection(m_CamDir) || !MYON_Viewport::IsValidCameraUpOrDirection(m_CamUp) )
  ...
  CamX = MYON_CrossProduct( CamY, CamZ );
  if ( !CamX.IsValid() )
    return Internal_SetCameraFameFailed();
  if ( !CamX.Unitize() )
    return false; // happens when up and dir are temporaily parallel.
  ...
  if ( !MYON__IsCameraFrameUnitVectorHelper(CamX) )
    return Internal_SetCameraFameFailed();
  if ( !MYON__IsCameraFrameUnitVectorHelper(CamY) )
    return Internal_SetCameraFameFailed();
  if ( !MYON__IsCameraFrameUnitVectorHelper(CamZ) )
    return Internal_SetCameraFameFailed();
  if ( !MYON__IsCameraFramePerpindicular(CamX,CamY) )
    return Internal_SetCameraFameFailed();
  if ( !MYON__IsCameraFramePerpindicular(CamY,CamZ) )
    return Internal_SetCameraFameFailed();
  if ( !MYON__IsCameraFramePerpindicular(CamZ,CamX) )
    return Internal_SetCameraFameFailed();
  ...
  m_bValidCamera = MYON_Viewport::IsValidCameraLocation(m_CamLoc);

  return m_bValidCamera;
}

MYON_3dPoint MYON_Viewport::CameraLocation() const
{
  return m_CamLoc;
}

MYON_3dVector MYON_Viewport::CameraDirection() const
{
  return m_CamDir;
}

MYON_3dVector MYON_Viewport::CameraUp() const
{
  return m_CamUp;
}

bool MYON_Viewport::CameraLocationIsLocked() const
{
  return m_bLockCamLoc;
}

bool MYON_Viewport::CameraDirectionIsLocked() const
{
  return m_bLockCamDir;
}

bool MYON_Viewport::CameraUpIsLocked() const
{
  return m_bLockCamUp;
}

bool MYON_Viewport::FrustumIsLeftRightSymmetric() const
{
  return (0 != (0x02 & m_frustum_symmetry_flags));
}

bool MYON_Viewport::FrustumIsTopBottomSymmetric() const
{
  return (0 != (0x01 & m_frustum_symmetry_flags));
}

void MYON_Viewport::UnlockCamera()
{
  SetCameraLocationLock(false);
  SetCameraDirectionLock(false);
  SetCameraUpLock(false);
...
void MYON_Viewport::UnlockFrustumSymmetry()
{
  SetFrustumLeftRightSymmetry(false);
  SetFrustumTopBottomSymmetry(false);
}

void MYON_Viewport::SetCameraLocationLock( bool bLockCameraLocation )
{
  m_bLockCamLoc = bLockCameraLocation ? true : false;
}

void MYON_Viewport::SetCameraDirectionLock( bool bLockCameraDirection ) 
{
  m_bLockCamDir = bLockCameraDirection ? true : false;
}

void MYON_Viewport::SetCameraUpLock( bool bLockCameraUp )
{
  m_bLockCamUp = bLockCameraUp ? true : false;
}

void MYON_Viewport::SetFrustumLeftRightSymmetry( bool bForceLeftRightSymmetry )
{
...
void MYON_Viewport::SetFrustumTopBottomSymmetry( bool bForceTopBottomSymmetry )
{
...
bool MYON_Viewport::GetDollyCameraVector(
         ...
         MYON_3dVector& dolly_vector// dolly vector returned here
         ) const
{
  int port_left, port_right, port_bottom, port_top;
  MYON_Xform c2w;
  dolly_vector = MYON_3dVector::ZeroVector;
  bool rc = GetScreenPort( &port_left, &port_right, &port_bottom, &port_top );
  if ( rc )
    rc = GetXform( MYON::clip_cs, MYON::world_cs, c2w );
  if ( rc ) {
    ...
    if ( dx == 0.0 || dy == 0.0 || dz == 0.0 )
      ...
      MYON_3dPoint c0( (x0-port_left)/dx-1.0, (y0-port_bottom)/dy-1.0, z );
      MYON_3dPoint c1( (x1-port_left)/dx-1.0, (y1-port_bottom)/dy-1.0, z );
      MYON_3dPoint w0 = c2w*c0;
      MYON_3dPoint w1 = c2w*c1;
      dolly_vector = w0 - w1;
    }
  }
  return rc;
...
bool MYON_Viewport::DollyCamera( const MYON_3dVector& dolly )
{
  bool rc = false;
  if ( m_CamLoc.IsValid() && dolly.IsValid() )
  {
    const MYON_3dPoint loc =  m_CamLoc + dolly;
...
bool MYON_Viewport::DollyFrustum( double dollyDistance )
{
  bool rc = false;
...
bool MYON_Viewport::GetCameraFrame(
  class MYON_Plane& camera_frame
) const
{
  ...
  if (false == rc)
    camera_frame = MYON_Plane::NanPlane;
  return rc;
}
...
MYON_3dVector MYON_Viewport::CameraX() const
{
  return m_CamX;
}

MYON_3dVector MYON_Viewport::CameraY() const
{
  return m_CamY;
}

MYON_3dVector MYON_Viewport::CameraZ() const
{
  return m_CamZ;
}

bool MYON_Viewport::IsCameraFrameWorldPlan( 
      int* xindex,
      int* yindex,
      int* zindex
      )
...
bool MYON_Viewport::GetCameraExtents( 
    ...
    MYON_BoundingBox& cbox, // bounding box in camera coordinates
    ...
    ) const
{
  ...
  if ( count > 0 && stride >= 3 && points != nullptr ) {
    rc = false;
    if ( GetXform( MYON::world_cs, MYON::camera_cs, w2c ) ) {
      rc = true;
      for ( i = 0; i < count && rc; i++, points += stride ) {
        rc = cbox.Set( w2c*MYON_3dPoint(points), bGrowBox );
        bGrowBox = true;
      }
    }
  }
...
bool MYON_Viewport::GetCameraExtents( 
    // returns bounding box in camera coordinates - this is useful information
    // for setting view frustrums to include the point list
    const MYON_BoundingBox& wbox, // world coordinate bounding box
    MYON_BoundingBox& cbox, // bounding box in camera coordinates
    int bGrowBox         // set to true to grow existing box
    ) const
{
...
bool MYON_Viewport::GetCameraExtents( 
    // returns bounding box in camera coordinates - this is useful information
    // for setting view frustrums to include the point list
    MYON_3dPoint& worldSphereCenter,
...
static void UpdateTargetPointHelper( MYON_Viewport& vp, double target_distance )
{
  if ( !vp.IsValidCamera() || !vp.IsValidFrustum() )
    return;
  if ( !MYON_IsValid(target_distance) || target_distance <= 0.0 )
    return;

  MYON_3dPoint old_tp = vp.TargetPoint();
  ...
  double target_tol = 1.0e-5*(vp.FrustumWidth()+vp.FrustumHeight())
                    + MYON_ZERO_TOLERANCE;
  MYON_3dPoint new_tp = vp.CameraLocation() - target_distance*vp.CameraZ();
  if ( new_tp.IsValid() 
...
bool MYON_Viewport::ChangeToParallelProjection( bool bSymmetricFrustum )
{
  bool rc = (m_bValidCamera && m_bValidFrustum);
  ...
  if (    MYON::parallel_view == m_projection 
  ...
  ChangeToSymmetricFrustum(bSymmetricFrustum,bSymmetricFrustum,MYON_UNSET_VALUE);
  SetFrustumTopBottomSymmetry(bSymmetricFrustum);
  SetFrustumLeftRightSymmetry(bSymmetricFrustum);

  const MYON::view_projection old_projection = m_projection;
  double target_distance = TargetDistance(true);
  if ( !MYON_IsValid(target_distance) 
       || !m_bValidFrustum
       || !MYON_IsValid(m_frus_near)
       || m_frus_near <= 0.0
  ...
  if ( MYON::parallel_view != old_projection )
  {
    if ( !SetProjection(MYON::parallel_view) )
      rc = false;
  }

  if ( rc )
  {
    if ( MYON::perspective_view == old_projection )
    {
...
static bool ChangeFromParallelToPerspectiveHelper( MYON_Viewport& vp, double target_distance, double lens_length )
{
  // helper use by ChangeToPerspectiveProjection() and ChangeToTwoPointPerspectiveProjection()
  if ( MYON::perspective_view == vp.Projection() )
    return true;
  ...
  const MYON_3dPoint width_point = ( MYON_IsValid(target_distance) && target_distance > 0.0) 
  ...
  if ( MYON_IsValid(lens_length) && lens_length > 0.0 )
  {
       ...
       )
    {
      ...
      if ( d > frus_near )
      {
        ...
        if ( width > w && w > 0.0 )
        {
          ...
          MYON_3dPoint cam_loc0 = vp.CameraLocation();
          double dz = d*(width/w - 1.0);
          MYON_3dPoint cam_loc1 = cam_loc0 + dz*vp.CameraZ();
...
bool MYON_Viewport::ChangeToPerspectiveProjection( 
        ...
        )
{
  ...
  if (    MYON::perspective_view == m_projection 
       && (bSymmetricFrustum?true:false) == FrustumIsLeftRightSymmetric()
       && (bSymmetricFrustum?true:false) == FrustumIsTopBottomSymmetric()
     )
  {
    double current_lens_length = lens_length;
    if ( MYON_IsValid(lens_length) 
         && lens_length > 0.0 
...
static
bool GetTwoPointPerspectiveUpAndDirHelper( const MYON_3dVector& up,
                                           const MYON_3dVector& CamDir,
                                           const MYON_3dVector& CamY,
                                           const MYON_3dVector& CamZ,
                                           MYON_3dVector& new_up,
                                           MYON_3dVector& new_dir
                                           )
{
  // get up direction
  MYON_3dVector unit_up;
  MYON_3dVector unit_dir;
  ...
  if ( CamDir.IsValid() && !CamDir.IsTiny() )
  {
    new_dir = CamDir;
    unit_dir = new_dir;
    if ( unit_dir.Unitize() && MYON__IsCameraFramePerpindicular(unit_up,unit_dir) )
      return true;
...
bool MYON_Viewport::ChangeToTwoPointPerspectiveProjection( 
        double target_distance,
        MYON_3dVector up,
        double lens_length
        )
{
  ...
  if ( rc )
  {
    MYON_3dVector new_up = m_CamY;
    MYON_3dVector new_dir = -m_CamZ;
    MYON_3dPoint  new_loc = m_CamLoc;
    ...
    else
    {
      ...
      MYON_3dPoint center_point = FrustumCenterPoint(target_distance);
      ...
      MYON_3dVector saved_up = m_CamUp;
      MYON_3dVector saved_dir = m_CamDir;
...
bool MYON_Viewport::SetProjection( MYON::view_projection projection )
{
  ...
  bool rc = false;
  if ( projection == MYON::perspective_view ) 
  {
    rc = true;
    m_projection = MYON::perspective_view;
  }
  else 
  {
    rc = (projection == MYON::parallel_view);
    m_projection = MYON::parallel_view;
  }

  return rc;
}

MYON::view_projection MYON_Viewport::Projection() const
{
  return m_projection;
}

bool MYON_Viewport::IsParallelProjection() const
{
  return ( MYON::parallel_view == m_projection );
}

bool MYON_Viewport::IsPerspectiveProjection() const
{
  return ( MYON::perspective_view == m_projection );
}

bool MYON_Viewport::IsTwoPointPerspectiveProjection() const
{
      ...
      )
{
  bool rc = false;
  if (  
          MYON_IsValid(frus_left)
       && MYON_IsValid(frus_right)
       && MYON_IsValid(frus_top)
       && MYON_IsValid(frus_bottom)
       && MYON_IsValid(frus_near)
       && MYON_IsValid(frus_far)
       && frus_left < frus_right 
       && frus_bottom < frus_top 
       && 0.0 < frus_near 
       && frus_near < frus_far 
       && -MYON_NONSENSE_WORLD_COORDINATE_VALUE < frus_left
       && frus_right < MYON_NONSENSE_WORLD_COORDINATE_VALUE
       && -MYON_NONSENSE_WORLD_COORDINATE_VALUE < frus_bottom
       && frus_top < MYON_NONSENSE_WORLD_COORDINATE_VALUE
       && frus_far < MYON_NONSENSE_WORLD_COORDINATE_VALUE
     ) 
  {
    ...
    {
      MYON_ERROR("MYON_Viewport::SetFrustum - Beyond float precision perspective frus_near/frus_far values - will crash MS OpenGL");
    }
  ...
  else
  {
    ...
    MYON_ERROR("MYON_Viewport::SetFrustum - invalid input");
  }
  return rc;
}


bool MYON_Viewport::GetFrustum(
      double* frus_left,
      double* frus_right,
      double* frus_bottom,
      double* frus_top,
...
double MYON_Viewport::FrustumLeft()   const { return m_frus_left; }
double MYON_Viewport::FrustumRight()  const { return m_frus_right; }
double MYON_Viewport::FrustumBottom() const { return m_frus_bottom; }
double MYON_Viewport::FrustumTop()    const { return m_frus_top; }
double MYON_Viewport::FrustumNear()   const { return m_frus_near; }
double MYON_Viewport::FrustumFar()    const { return m_frus_far;  }
double MYON_Viewport::FrustumWidth()  const { return m_frus_right-m_frus_left; }
double MYON_Viewport::FrustumHeight() const { return m_frus_top-m_frus_bottom; }

double MYON_Viewport::FrustumMinimumDiameter() const 
{ 
...
double MYON_Viewport::FrustumMaximumDiameter() const
{ 
  double w = fabs(m_frus_right-m_frus_left); 
  double h = fabs(m_frus_top-m_frus_bottom);
  return (w<=h)?w:h;
...
bool MYON_Viewport::SetFrustumAspect( double frustum_aspect )
{
...
bool MYON_Viewport::GetFrustumAspect( double& frustum_aspect ) const
{
...
bool MYON_Viewport::GetFrustumCenter( double* frus_center ) const
{
...
bool MYON_Viewport::SetScreenPort( 
      int port_left, 
      int port_right,
      int port_bottom, 
      int port_top,
...
bool MYON_Viewport::GetScreenPort( 
      int* port_left,
      int* port_right,
      int* port_bottom,
      int* port_top,
...
int MYON_Viewport::ScreenPortWidth() const
{
  int width = m_port_right - m_port_left;
  return width >= 0 ? width : -width;
}

int MYON_Viewport::ScreenPortHeight() const
{
  int height = m_port_top - m_port_bottom;
  return height >= 0 ? height : -height;
}

MYON_2iSize MYON_Viewport::ScreenPortSize() const
{
  return MYON_2iSize(ScreenPortWidth(), ScreenPortHeight());
}

bool MYON_Viewport::GetScreenPortAspect(double& aspect) const
{
  const double width = m_port_right - m_port_left;
  const double height = m_port_top - m_port_bottom;
  aspect = ( m_bValidPort && MYON_IsValid(height) && MYON_IsValid(width) && height != 0.0 )
         ? fabs(width/height) 
         : 0.0;
  return (m_bValidPort && aspect != 0.0);
}

bool MYON_ViewportFromRhinoView(
        MYON::view_projection projection,
        const MYON_3dPoint& rhvp_target, // 3d point
...
*****************************************************************************/
{
  ...
  */
  ...
  R1.Rotation( rhvp_angle1, MYON_3dVector::ZAxis, MYON_3dPoint::Origin ); // so called "twist"
  R2.Rotation( rhvp_angle2, MYON_3dVector::XAxis, MYON_3dPoint::Origin ); // so called "elevation"
  R3.Rotation( rhvp_angle3, MYON_3dVector::ZAxis, MYON_3dPoint::Origin ); // so called "fudge factor"
  RhinoRot = R3 * R2 * R1;

  vp.SetCameraUp( RhinoRot*MYON_3dVector::YAxis );
  vp.SetCameraDirection( -(RhinoRot*MYON_3dVector::ZAxis) );
...
bool MYON_Viewport::GetCameraAngle( 
       double* angle,
       double* angle_h, 
       double* angle_w
       ) const
{
  ...
  if ( GetFrustum( &left, &right, &bot, &top, &near_dist, nullptr ) ) 
  {
    ...
    if ( near_dist > 0.0 && MYON_IsValid(near_dist) )
    {
      if ( angle )
...
bool MYON_Viewport::SetCameraAngle( double angle )
{
  bool rc = false;
  double r, d, aspect, half_w, half_h, near_dist, far_dist;
  if ( angle > 0.0  && angle < 0.5*MYON_PI*(1.0-MYON_SQRT_EPSILON) ) {
...
bool MYON_Viewport::GetCamera35mmLenseLength( double* lens_length ) const
{
  return GetCamera35mmLensLength( lens_length );
}

bool MYON_Viewport::GetCamera35mmLensLength( double* lens_length ) const
{
...
// This version of the function has "lens" misspelled.
bool MYON_Viewport::SetCamera35mmLenseLength( double lens_length )
{
  return SetCamera35mmLensLength( lens_length );
}

bool MYON_Viewport::SetCamera35mmLensLength( double lens_length )
{
  // 35 mm film has a height of 24 mm and a width of 36 mm
...
bool MYON_Viewport::GetXform( 
       MYON::coordinate_system srcCS,
       MYON::coordinate_system destCS,
       MYON_Xform& xform
       ) const
{
  bool rc = false;
  ...
  case MYON::world_cs:
    ...
    case MYON::clip_cs:
      rc = GetXform( MYON::world_cs,  MYON::camera_cs, x0 );
      if (rc)
        rc = GetXform( MYON::camera_cs, MYON::clip_cs,   x1 );
      if (rc)
        xform = x1*x0;
      break;

    case MYON::screen_cs:
      rc = GetXform( MYON::world_cs,  MYON::clip_cs,   x0 );
      if (rc)
        rc = GetXform( MYON::clip_cs,   MYON::screen_cs, x1 );
      if (rc)
        xform = x1*x0;
      break;
  ...
  case MYON::camera_cs:
    ...
    case MYON::screen_cs:
      rc = GetXform( MYON::camera_cs,  MYON::clip_cs,  x0 );
      if (rc)
        rc = GetXform( MYON::clip_cs,   MYON::screen_cs, x1 );
      if (rc)
        xform = x1*x0;
  ...
  case MYON::clip_cs:
    ...
    {
    case MYON::world_cs:
      rc = GetXform( MYON::clip_cs,   MYON::camera_cs, x0 );
      if (rc)
        rc = GetXform( MYON::camera_cs,  MYON::world_cs, x1 );
      if (rc)
        xform = x1*x0;
      break;
  ...
  case MYON::screen_cs:
    ...
    {
    case MYON::world_cs:
      rc = GetXform( MYON::screen_cs, MYON::camera_cs, x0 );
      if (rc)
        rc = GetXform( MYON::camera_cs, MYON::world_cs,  x1 );
      if (rc)
        xform = x1*x0;
      break;
    case MYON::camera_cs:
      rc = GetXform( MYON::screen_cs, MYON::clip_cs,   x0 );
      if (rc)
        rc = GetXform( MYON::clip_cs,   MYON::camera_cs, x1 );
      if (rc)
        xform = x1*x0;
      break;
    case MYON::clip_cs:
...
bool MYON_Viewport::GetFrustumLine( double screenx, double screeny, MYON_Line& world_line ) const
{
  MYON_Xform s2c, c2w;
  MYON_3dPoint c;
  MYON_Line line;
  ...
  rc = GetXform( MYON::screen_cs, MYON::clip_cs, s2c );
  if ( rc )
    rc = GetXform( MYON::clip_cs, MYON::world_cs, c2w );
  if (rc )
  {
...
bool MYON_Viewport::SetFrustumNearFar( 
       ...
       )
{
  ...
  if ( !MYON_IsValid(box_min[0]) || !MYON_IsValid(box_min[1]) || !MYON_IsValid(box_min[2]) )
    return false;
  if ( !MYON_IsValid(box_max[0]) || !MYON_IsValid(box_max[1]) || !MYON_IsValid(box_max[2]) )
    return false;
  ...
  if ( GetCameraFrame( camLoc, nullptr, nullptr, camZ ) ) {
    ...
    if ( !MYON_IsValid(f) || !MYON_IsValid(n) )
      return false;
    if ( f <= 0.0 )
       ...
       )
{
  ...
  if ( !center 
       || !MYON_IsValid(center[0]) 
       || !MYON_IsValid(center[1]) 
       || !MYON_IsValid(center[2])
       || !MYON_IsValid(radius)
     )
  {
    return false;
...
bool MYON_Viewport::SetFrustumNearFar( double n, double f )
{
  // This is a bare bones setter.  
  // Except for the 0 < n < f < MYON_NONSENSE_WORLD_COORDINATE_VALU
  // requirement, do not add checking here.
  //
  // Use the MYON_Viewport::SetFrustumNearFar( near_dist, 
  //                                         far_dist, 
  //                                         min_near_dist, 
  //                                         min_near_over_far,
  //                                         target_dist );
  ...
  bool rc = false;
  if ( MYON_IsValid(n) && MYON_IsValid(f) && n > 0.0 && f > n && f < MYON_NONSENSE_WORLD_COORDINATE_VALUE ) 
  {
    ...
    else 
    {
      ...
      {
        MYON_ERROR("MYON_Viewport::SetFrustum - bogus perspective m_frus_near/far values - will crash MS OpenGL");
      }
      m_frus_near = n;
      m_frus_far = f;
      m_projection_content_sha1 = MYON_SHA1_Hash::ZeroDigest;
...
bool MYON_Viewport::ChangeToSymmetricFrustum( 
    ...
    )
{
  ...
  if ( m_bValidCamera && (dx != 0.0 || dy != 0.0 ) )
  {
    if ( MYON::perspective_view == m_projection )
    {
      if ( m_frus_near > 0.0 )
      {
        if ( MYON_UNSET_VALUE == target_distance )
          target_distance = TargetDistance(true);
        if ( MYON_IsValid(target_distance) && target_distance > 0.0 )
        {
          double s = target_distance/m_frus_near;
    ...
    if ( dx != 0.0 || dy != 0.0 )
    {
      MYON_3dPoint cam_loc = m_CamLoc + dx*m_CamX + dy*m_CamY;
...
bool MYON_Viewport::GetWorldToScreenScale(
  MYON_3dPoint world_point,
  double* scale
  ) const
{
  double frustum_depth = MYON_UNSET_VALUE;
  ...
  if (IsPerspectiveProjection() && world_point.IsValid())
  {
    ...
    if (!(frustum_depth > 0.0))
      frustum_depth = MYON_UNSET_VALUE;
  }
  ...
  ) const
{
  ...
  if (IsPerspectiveProjection() && MYON_IsValid(frustum_depth) && frustum_depth > 0.0)
  {
    if (!(m_frus_near > 0.0))
      return false;
    s = frustum_depth / m_frus_near;
    if (!(s >= 0.0) && MYON_IS_FINITE(s))
      return false;
  }
...
bool MYON_Viewport::GetCoordinateSprite( 
                     int size, 
                     int scrx, int scry,
                     ...
                     double scr_coord[3][2] ) const
{
  ...
  MYON_3dPoint C, XP, YP, ZP, ScrC, ScrXP;
  MYON_3dVector X, Z, Scr[3];
  MYON_Xform w2s;
  if (!GetFrustumCenter( C ) )
    return false;
  if (!GetCameraFrame( nullptr, X, nullptr, Z )) 
    return false;
  if (!GetXform( MYON::world_cs, MYON::screen_cs, w2s )) 
    return false;
...
bool MYON_Viewport::ZoomToScreenRect( int x0, int y0, int x1, int y1 )
{
...
/*
bool MYON_Viewport::DollyToScreenRect( double view_plane_distance,
                                        int x0, int y0, int x1, int y1 )
{
...
bool MYON_Viewport::Extents( double angle, const MYON_BoundingBox& bbox )
{
  double radius;
  ...
  if ( !bbox.IsValid() || !IsValid() )
    return false;
  MYON_3dVector camX = CameraX();
  MYON_3dVector camY = CameraY();
  MYON_3dPoint center = bbox.Center();
  xmin=xmax=ymin=ymax=0.0;
  for (i=0;i<2;i++) for (j=0;j<2;j++) for (k=0;k<2;k++) {
    MYON_3dVector box_corner = bbox.Corner(i,j,k);
    x = camX*box_corner;
    y = camY*box_corner;
    if ( i==0&&j==0&&k==0) {
      xmin=xmax=x;
...
bool MYON_Viewport::Extents( double angle, const MYON_3dPoint& center, double radius )
{
  if ( !IsValid() )
    return false;
  ...
  if ( radius <= 0.0 || 
       angle  <= 0.0 || 
       angle  >= 0.5*MYON_PI*(1.0-MYON_SQRT_EPSILON) )
    return false;
...
void MYON_Viewport::Dump( MYON_TextLog& dump ) const
{
  dump.Print("MYON_Viewport\n");
  dump.PushIndent();

  dump.Print("Projection: ");
  switch(m_projection)
  {
  case MYON::parallel_view:
    dump.Print("parallel\n");
    break;
  case MYON::perspective_view:
...
bool MYON_Viewport::GetPointDepth(       
       MYON_3dPoint point,
       double* near_dist,
       double* far_dist,
       bool bGrowNearFar
...
{
  bool rc = false;
  if ( point.x != MYON_UNSET_VALUE )
  {
    double depth = (m_CamLoc - point)*m_CamZ;
    if ( 0 != near_dist && (*near_dist == MYON_UNSET_VALUE || !bGrowNearFar || *near_dist > depth) )
      *near_dist = depth;
    if ( 0 != far_dist && (*far_dist == MYON_UNSET_VALUE || !bGrowNearFar || *far_dist < depth) )
      *far_dist = depth;
    rc = true;
  }
  return rc;
...
int MYON_Viewport::InViewFrustum(
  bool bInfiniteFrustum,
  const MYON_BoundingBox& bbox,
  const MYON_Xform* bbox_xform
  ) const
{
  double near_dist = MYON_UNSET_VALUE;
  double far_dist = MYON_UNSET_VALUE;
...
bool MYON_Viewport::GetBoundingBoxDepth(
...
int MYON_Viewport::GetBoundingBoxDepth(       
       MYON_BoundingBox bbox,
       const MYON_Xform* bbox_xform,
       double* near_dist,
       double* far_dist,
       ...
       ) const
{
  ...
  const double tol = MYON_SQRT_EPSILON*(1.0 + m_CamLoc.MaximumCoordinate());
  C = (MYON_3dPoint*)Pbuffer;
  P = C+8;
  S = (MYON_PlaneEquation*)Sbuffer;
  L = (MYON_Line*)Lbuffer;
  unsigned int i, j, k, Pin, Pout, Pcount;
  bool rc;
  bool bTrimmed = false;
  const bool bPerspectiveProjection = (MYON::perspective_view == m_projection);
  ...
  for (;;)
  {
    ...
    if ( Pcount < 8 )
    {
      ...
      for ( i = 0; i < 4; i++ )
      {
        ...
        // the expensive MYON_Intersect() function.
        if (    Linout[i] == (k & Linout[i]) 
             && MYON_Intersect(bbox,L[i],tol,(MYON_Interval*)t) 
           )
        {
          if ( bPerspectiveProjection )
          {
    ...
    if ( 0 != near_dist && (!bGrowNearFar || !MYON_IsValid(*near_dist) || t[0] < *near_dist) )
      *near_dist = t[0];
    if ( 0 != far_dist && (!bGrowNearFar || !MYON_IsValid(*far_dist) || t[1] > *far_dist) )
      *far_dist = t[1];
...
static bool TrimLineHelper( MYON_PlaneEquation e, bool bFlipPlane, MYON_Line& line )
{
...
bool MYON_Viewport::GetBoundingBoxProjectionExtents(
    MYON_BoundingBox bbox,
    MYON_Interval& x_extents,
    MYON_Interval& y_extents
    ) const
{
  const MYON_Interval unit_interval(0.0,1.0);
  ...
  const MYON_3dPoint cam_loc = CameraLocation();
  if ( !cam_loc.IsValid() )
    return false;

  // far_rect[] points run image ccw 
  // (image lower left, lower right, upper right, upper left)
  MYON_3dPoint far_rect[4];
  if ( !GetFarRect(far_rect[0],far_rect[1],far_rect[3],far_rect[2]) )
    return false;

  MYON_3dPoint clipping_point(MYON_UNSET_VALUE,MYON_UNSET_VALUE,0.0);
  MYON_BoundingBox clipping_bbox;
  clipping_bbox.m_min.z = clipping_point.z;
  clipping_bbox.m_max.z = clipping_point.z;

  MYON_Line camera_ray(cam_loc,MYON_3dPoint::UnsetPoint);
  for ( int i = 0; i < 4; i++ )
  {
    camera_ray.to = far_rect[i];
    for ( int j = 0; j < 3; j++ )
    {
      MYON_PlaneEquation e(0==j ? 1.0 : 0.0, 1 == j ? 1.0 : 0.0, 2==j?1.0:0.0, 0.0);
      for ( int k = 0; k < 2; k++ )
      {
        MYON_3dPoint bbox_point = k ? bbox.m_max : bbox.m_min;
        e.d = 0.0;
        e.d = -bbox_point[j];
        double t = MYON_UNSET_VALUE;
        if ( MYON_Intersect(camera_ray,e,&t) )
        {
          if ( t > 0.0 )
          {
            MYON_3dPoint P = camera_ray.PointAt(t);
            P[j] = bbox_point[j];
            if ( bbox.IsPointIn(P) )
            {
  ...
  {
    MYON_Xform w2c;
    if ( !GetXform(MYON::world_cs,MYON::clip_cs,w2c) )
      return false;

    MYON_PlaneEquation vp_near_plane;
    if ( !GetNearPlaneEquation(vp_near_plane) )
      return false;

    MYON_PlaneEquation vp_far_plane;
    if ( !GetFarPlaneEquation(vp_far_plane) )
      return false;

    MYON_PlaneEquation frustum_sides[4];
    ...
    MYON_Line bbox_lines[12];
    bbox.GetEdges(bbox_lines);

    for ( int i = 0; i < 12; i++ )
    {
      MYON_Line line = bbox_lines[i];
      ...
      for ( int j = 0; j < 4; j++ )
      {
        ...
        {
          line.from = MYON_3dPoint::UnsetPoint;
          line.to = MYON_3dPoint::UnsetPoint;
          break;
        }
      }
...
bool MYON_Viewport::GetSphereDepth( 
       MYON_Sphere sphere,
       double* near_dist,
       double* far_dist,
       bool bGrowNearFar
...
{
  if (    !MYON_IsValid(near_dist)
       || !MYON_IsValid(far_dist)
       || near_dist > far_dist )
  {
    return false;
  }
  ...
  // input.
  const double tiny = MYON_ZERO_TOLERANCE;
  const double MIN_NEAR_DIST = ( MYON_IsValid(m__MIN_NEAR_DIST) &&  m__MIN_NEAR_DIST <= tiny )
                            ? m__MIN_NEAR_DIST
                            : MYON_Viewport::DefaultMinNearDist;
  const double MIN_NEAR_OVER_FAR = (    MYON_IsValid(m__MIN_NEAR_OVER_FAR)
                                     && m__MIN_NEAR_OVER_FAR > tiny 
                                     && m__MIN_NEAR_OVER_FAR < 1.0-tiny )
                                 ? m__MIN_NEAR_OVER_FAR
                                 : MYON_Viewport::DefaultMinNearOverFar;

  // 30 May Dale Lear
  //    Add checks for validity of min_near_dist and min_near_over_far
  if ( !MYON_IsValid(min_near_dist) || min_near_dist <= tiny )
  {
  ...
  if ( IsPerspectiveProjection() )
  {
    ...
    while ( near_dist < 1.0001*min_near_over_far*far_dist )
    {
      ...
      if ( MYON_IsValid(target_dist) && near_dist < target_dist && target_dist < far_dist )
      {
        // STEP 1
        ...
        if ( b > 0.0)
        {
          ...
          if ( s > 1.0 || s <= MYON_ZERO_TOLERANCE || !MYON_IsValid(s) )
          {
            if ( s > 1.00001 || s <= MYON_ZERO_TOLERANCE )
            {
              // should never happen
              MYON_ERROR("MYON_Viewport::SetFrustumNearFar arithmetic problem 1.");
            }
            s = 1.0; 
          }
          ...
          if ( m < 0.95 || m > 1.05 )
          {
            MYON_ERROR("MYON_Viewport::SetFrustumNearFar arithmetic problem 2.");
          }
#endif

          if ( n < near_dist || n >= target_dist)
          {
            MYON_ERROR("MYON_Viewport::SetFrustumNearFar arithmetic problem 3.");
          ...
          {
            MYON_ERROR("MYON_Viewport::SetFrustumNearFar arithmetic problem 4.");
      ...
      else if ( MYON_IsValid(target_dist) && fabs(far_dist-target_dist) > fabs(near_dist-target_dist) )
      {
        far_dist = near_dist/min_near_over_far;
      }
      else
  ...
  else
  {
    ...
    if ( near_dist < min_near_dist || near_dist < MIN_NEAR_DIST )
    {
      ...
      MYON_3dPoint new_loc = CameraLocation() + d*CameraZ();
      SetCameraLocation(new_loc);
      if ( m_bValidFrustum && fabs(m_frus_near) >= d*MYON_SQRT_EPSILON )
      {
        m_frus_near += d;
        m_frus_far += d;
      }
      ...
      {
        // could happen if d is < MYON_EPSILON*far_dist
       ...
       )
  {
    ...
    {
#if defined(MYON_DEBUG)
      const MYON_3dPoint debug_camloc0(m_CamLoc);
#endif
      if ( IsPerspectiveProjection() )
      {
      ...
      {
        MYON_WARNING("Relative depth bias changed camera location.");
      }
#endif
    }
  }
...
bool MYON_Viewport::GetFrustumLeftPlane( 
  MYON_Plane& left_plane 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
  {
    if ( IsPerspectiveProjection() )
    {
      MYON_2dVector v(m_frus_near,m_frus_left);
      rc = v.Unitize();
...
bool MYON_Viewport::GetFrustumLeftPlaneEquation( 
  MYON_PlaneEquation& left_plane_equation
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
bool MYON_Viewport::GetFrustumRightPlane( 
  MYON_Plane& right_plane 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
  {
    if ( IsPerspectiveProjection() )
    {
      MYON_2dVector v(m_frus_near,-m_frus_right);
      rc = v.Unitize();
...
bool MYON_Viewport::GetFrustumRightPlaneEquation( 
  MYON_PlaneEquation& right_plane_equation
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
bool MYON_Viewport::GetFrustumBottomPlane( 
  MYON_Plane& bottom_plane 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
  {
    if ( IsPerspectiveProjection() )
    {
      MYON_2dVector v(m_frus_near,m_frus_bottom);
      rc = v.Unitize();
...
bool MYON_Viewport::GetFrustumBottomPlaneEquation( 
  MYON_PlaneEquation& bottom_plane_equation
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
bool MYON_Viewport::GetFrustumTopPlane( 
  MYON_Plane& top_plane 
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
  {
    if ( IsPerspectiveProjection() )
    {
      MYON_2dVector v(m_frus_near,-m_frus_top);
      rc = v.Unitize();
...
bool MYON_Viewport::GetFrustumTopPlaneEquation( 
  MYON_PlaneEquation& top_plane_equation
  ) const
{
  bool rc = m_bValidCamera && m_bValidFrustum;
  if (rc)
...
void MYON_Viewport::GetViewScale(double* x, double* y) const
{
  GetViewScale(x, y, nullptr);
}

void MYON_Viewport::GetViewScale( double* x, double* y, double* z) const
{
  if (x) *x = 1.0;
  if (y) *y = 1.0;
  if (z) *z = 1.0;
     ...
     )
  {
    ...
    if (    fabs(sx) > MYON_ZERO_TOLERANCE
         && fabs(sy) > MYON_ZERO_TOLERANCE
         && fabs(sz) > MYON_ZERO_TOLERANCE
...
bool MYON_Viewport::SetViewScale( double x, double y )
{
  return SetViewScale(x, y, 1);
}

bool MYON_Viewport::SetViewScale( double x, double y, double z)
{
  // 22 May Dale Lear
  ...
  bool validInput = fabs(x) > MYON_ZERO_TOLERANCE && MYON_IsValid(x)
    && fabs(y) > MYON_ZERO_TOLERANCE && MYON_IsValid(y)
    && fabs(z) > MYON_ZERO_TOLERANCE && MYON_IsValid(z);
  if (!validInput)
    return false;

  bool allOnes = fabs(x - 1.0) < MYON_EPSILON 
              && fabs(y - 1.0) < MYON_EPSILON
              && fabs(z - 1.0) < MYON_EPSILON;
  if (allOnes)
    return SetClipModXform(MYON_Xform::IdentityTransformation);
...
double MYON_Viewport::ClipCoordDepthBias( double relative_depth_bias, double clip_z, double clip_w ) const
{
  double d;
...
bool MYON_Viewport::GetClipCoordDepthBiasXform( 
    double relative_depth_bias,
    MYON_Xform& clipbias 
    ) const
{
       ...
       )
  {
    if ( MYON::perspective_view == m_projection )
    {
      MYON_Xform clip2cam, cam_delta(MYON_Xform::IdentityTransformation), cam2clip;
    ...
    {
      clipbias = MYON_Xform::IdentityTransformation;
...
bool MYON_Viewport::SetClipModXform( MYON_Xform clip_mod_xform )
{
  bool rc = false;
  MYON_Xform clip_mod_inverse_xform = clip_mod_xform;
  rc = clip_mod_inverse_xform.Invert();
  if ( rc )
  {
    MYON_Xform id = clip_mod_inverse_xform*clip_mod_xform;
    double e;
    int i, j;
    for ( i = 0; i < 4 && rc; i++ ) for ( j = 0; j < 4 && rc; j++ )
    {
      e = ( i == j ) ? 1.0 : 0.0;
      if ( fabs(id.m_xform[i][j] - e) > MYON_SQRT_EPSILON )
      {
        rc = false;
      }
    }
...
bool MYON_Viewport::ClipModXformIsIdentity() const
{
  return m_clip_mods.IsIdentity();
}

MYON_Xform MYON_Viewport::ClipModXform() const
{
  return m_clip_mods;
}

MYON_Xform MYON_Viewport::ClipModInverseXform() const
{
  return m_clip_mods_inverse;
}

bool MYON_Viewport::SetTargetPoint( MYON_3dPoint target_point )
{
  bool rc = (target_point.IsValid() || (MYON_3dPoint::UnsetPoint == target_point));
  if (rc)
    m_target_point = target_point;
  return rc;
}

MYON_3dPoint MYON_Viewport::FrustumCenterPoint( double target_distance ) const
{
  double s,dx,dy,dz;
  MYON_3dPoint target_point = MYON_3dPoint::UnsetPoint;

  if (!m_bValidCamera || !m_bValidFrustum)
    return target_point;

  if ( MYON_UNSET_VALUE == target_distance && m_bValidFrustum 
  ...
  if ( m_bValidFrustum )
  {
    s  = (MYON::perspective_view == m_projection && m_frus_near > 0.0)
  ...
  // Done this way instead of using MYON_3dPoint/MYON_3dVector arithmetic so the
...
MYON_3dPoint MYON_Viewport::TargetPoint() const
{
  return m_target_point;
}


double MYON_Viewport::TargetDistance( bool bUseFrustumCenterFallback ) const
{
  double d = MYON_UNSET_VALUE;
  if ( m_bValidCamera )
  {
    ...
    if ( m_target_point.IsValid() )
    {
      ...
      if ( bUseFrustumCenterFallback && (!MYON_IsValid(d) || d <= 0.0) )
        d = MYON_UNSET_VALUE;
    }
...
bool MYON_Viewport::SetViewportId( const MYON_UUID& id)
{
  ...
  if ( !rc && m_viewport_id == MYON_nil_uuid )
  {  
    m_viewport_id = id;
    rc = true;
  }
...
void MYON_Viewport::ChangeViewportId(const MYON_UUID& viewport_id)
{
  m_viewport_id = viewport_id; // <- good place for a breakpoint
}


MYON_UUID  MYON_Viewport::ViewportId(void) const
{
...
{
  double a = ((const MYON_PgonPt*)pa)->m_negcotangle;
  double b = ((const MYON_PgonPt*)pb)->m_negcotangle;
  if ( a == b )
  {
    a = ((const MYON_PgonPt*)pa)->m_Q.LengthSquared();
    b = ((const MYON_PgonPt*)pb)->m_Q.LengthSquared();
  }
  return ((a>b) ? 1 : ((a==b) ? 0 : -1));
}

bool MYON_IntersectViewFrustumPlane( 
    ...
    )
{
  ...
  double v[8], v0, v1, s;
  MYON_PgonPt ppt, ppt_list[24];
  MYON_3dPoint F[8], P0, P1, P;
  MYON_2dVector D;
  const MYON_3dPoint  C = vp.CameraLocation();
  const MYON_3dVector X = vp.CameraX();
  const MYON_3dVector Y = vp.CameraY();
  const MYON_3dVector Z = -vp.CameraZ();
  ...
  const MYON_Plane plane(plane_equation);
  if ( !plane.IsValid() )
    return false;

  s = MYON::perspective_view == vp.Projection() 
    ? far_dist/near_dist
    : 1.0;
  ...
  ppt_list[0].m_negcotangle = -MYON_DBL_MAX; // -cot(0) = - infinity
  for ( i = 1; i < ppt_count; i++ )
  {
    ppt_list[i].m_Q.x -= ppt_list[0].m_Q.x;
    ppt_list[i].m_Q.y -= ppt_list[0].m_Q.y;
    ppt_list[i].m_negcotangle = (0.0 >= ppt_list[i].m_Q.y) ? -MYON_DBL_MAX : -ppt_list[i].m_Q.x/ppt_list[i].m_Q.y;
  }
  ppt_list[0].m_Q.x = 0.0;
  ppt_list[0].m_Q.y = 0.0;
  MYON_qsort(ppt_list+1,ppt_count-1,sizeof(ppt_list[0]),comparePptAngle);
...
void MYON_Viewport::GetPerspectiveClippingPlaneConstraints( 
  ...
  )
{
  ...
  if ( camera_location.IsValid() )
  {
    ...
    //
    d = camera_location.DistanceTo(MYON_3dPoint::Origin);
    if ( d >= 1.0e5 )
    {
