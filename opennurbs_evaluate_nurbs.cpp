...
double MYON_EvaluateBernsteinBasis(int degree, int i, double t)
/*****************************************************************************
...
*****************************************************************************/
{
  ...
  default:                      /* degree >= 5 */
     ...
     */
    if (degree < 9)
      return (t*MYON_EvaluateBernsteinBasis(degree-1,i-1,t)
              + (1-t)*MYON_EvaluateBernsteinBasis(degree-1,i,t));
    else
      return MYON_BinomialCoefficient(degree-i,i)*((degree==i)?1.0:pow(1.0-t,(double)(degree-i)))*((i)?pow(t,(double)i):1.0);
  }
}


void MYON_EvaluatedeCasteljau(int dim, int order, int side, int cv_stride, double* cv, double t)
/*****************************************************************************
...
bool MYON_IncreaseBezierDegree(
        int     dim, 
        bool    is_rat, 
        int     order, 
        int     cv_stride,
                  ...
                  )
{
  ...
  while (order < ord0)
    MYON_IncreaseBezierDegree(dim,true,order++,cv_stride,cv);
  return true;
}


bool MYON_RemoveBezierSingAt1(
                  int dim, 
                  int order, 
                  int cv_stride,
                  double* cv
...
REFERENCE:
  AUTHOR page ?
RELATED FUNCTIONS:
  MYON_EvaluatedeCasteljau
  MYON_EvQuotientRule
  MYON_EvNurb
  MYON_EvPolynomialPoint
  MYON_onvertBezierToPolynomial
  MYON_onvertPolynomialToBezier
  MYON_onvertNurbToBezier
*****************************************************************************/
{
  ...
  if (!(t0!=t1)) 
  {
    ...
    //  and not in release builds.  The #if defined(MYON_DEBUG) projection
    ...
    //  I'm enabling the test in all builds and addeing a call to MYON_ERROR().
    MYON_ERROR("Invalid domain");
    return false;
  }
  ...
  {
    if ( !MYON_RemoveBezierSingAt0(dim,order,cvdim,CV) )
    {
      if ( free_me )
        onfree(free_me);  
      return false;
  ...
  if (is_rat) {
    MYON_EvaluateQuotientRule( dim, der_count, cvdim, CV );
  }
  ...
  )
{
  ...
  //   values are large.
  x = 1.0-MYON_SQRT_EPSILON;
  if ( N[0] >= x )
  {
    if ( N[0] != 1.0 && N[0] <= 1.0 + MYON_SQRT_EPSILON )
    {
      r = 1;
      for ( j = 1; j <= d; j++ )
      {
  ...
  {
    if ( N[d] != 1.0 && N[d] <= 1.0 + MYON_SQRT_EPSILON )
    {
      r = 1;
      for ( j = 0; j < d; j++ )
      {
...
bool MYON_EvaluateNurbsBasisDerivatives(
  int order,
  const double* knot, 
  int der_count,
  double* N 
...
static
bool MYON_EvaluateNurbsNonRationalSpan( 
                  int dim,             // dimension
                  int order,           // order
                  ...
                  )
{
  ...
  if ( der_count >= order )
	...
	MYON_EvaluateNurbsBasis( order, knot, t, N );
	if ( der_count ) 
		MYON_EvaluateNurbsBasisDerivatives( order, knot, der_count, N );
...
static
bool MYON_EvaluateNurbsRationalSpan( 
                  int dim,             // dimension
                  int order,           // order
                  const double* knot,  // knot[] array of (2*order-2) doubles
                  int cv_stride,       // cv_stride >= (is_rat)?dim+1:dim
                  ...
                  )
{
  ...
  rc = MYON_EvaluateNurbsNonRationalSpan( dim+1, order, knot, 
          cv_stride, cv, der_count, t, hv_stride, hv );
  if (rc) 
  {
    rc = MYON_EvaluateQuotientRule(dim, der_count, hv_stride, hv);
    if ( rc )
    {
      // copy answer to v[]
      for ( i = 0; i <= der_count; i++ ) {
                  ...
                  )
{
  ...
  if ( knot[0] == knot[order-2] && knot[order-1] == knot[2*order-3] ) {
    ...
    rc = MYON_EvaluateBezier(dim, is_rat, order, cv_stride, cv,
    ...
    rc = (is_rat)
         ? MYON_EvaluateNurbsRationalSpan( 
              dim, order, knot, cv_stride, cv,
              der_count, t, v_stride, v )
         : MYON_EvaluateNurbsNonRationalSpan( 
              dim, order, knot, cv_stride, cv,
              der_count, t, v_stride, v );
  }
  return rc;
...
bool MYON_EvaluateNurbsSurfaceSpan(
        int dim,
        bool is_rat,
        ...
        )
{
  ...
  if ( der_count > 0 ) {
	...
	if ( is_rat ) {
		MYON_EvaluateQuotientRule2( dim, der_count, cvdim, P0 );
		Psize -= 8;
	}
	for ( i = 0; i < Pcount; i++) {
		memcpy( v, P0, Psize );
 ...
 */
{
  ...
  t1 = knots[degree];
  if (t0 == t1) {
    MYON_ERROR("MYON_EvaluateNurbsDeBoor(): knots[degree-1] == knots[degree]");
    return false;
  }
...
bool MYON_EvaluateNurbsBlossom(int cvdim,
                             int order, 
                             int cv_stride,
...

{
  ...
  if (knot[degree] - knot[degree-1] < MYON_EPSILON)
    return false;
...
void MYON_ConvertNurbSpanToBezier(int cvdim, int order, 
 ...
 */
{
  MYON_EvaluateNurbsDeBoor(cvdim,order,cvstride,cv,knot, 1, 0.0, t0);
  MYON_EvaluateNurbsDeBoor(cvdim,order,cvstride,cv,knot,-2,  t0, t1);
}
