...
MYON_FixedSizePool::MYON_FixedSizePool()
{
  ////const size_t sz = sizeof(*this);
  ////MYON_TextLog::Null.Print("L%d", sz); // supress compile errors.
...
MYON_FixedSizePool::MYON_FixedSizePool(MYON_FixedSizePool&& src)
...
MYON_FixedSizePool& MYON_FixedSizePool::operator=(MYON_FixedSizePool&& src)
{
  if (this != &src)
  {
    Destroy();
...
size_t MYON_FixedSizePool::SizeofElement() const
{
  return m_sizeof_element;
}

size_t MYON_FixedSizePool::SizeOfPool() const
{
...
size_t MYON_FixedSizePool::SizeOfUnusedElements() const
{
  return SizeOfPool() - SizeOfActiveElements();
}

size_t MYON_FixedSizePool::SizeOfActiveElements() const
{
  return m_sizeof_element * ((size_t)m_active_element_count);
}


size_t MYON_FixedSizePool::DefaultElementCapacityFromSizeOfElement(size_t sizeof_element)
{
  size_t block_element_capacity = 0;
  if (sizeof_element <= 0)
  {
    MYON_ERROR("sizeof_element must be > 0");
    return 0;
  }

  size_t page_size = MYON_MemoryPageSize();
  if (page_size < 512)
    page_size = 512;

  // The "overhead" is for the 2*sizeof(void*) MYON_FixedSizePool uses at
...
bool MYON_FixedSizePool::Create(
  size_t sizeof_element
)
{
  return MYON_FixedSizePool::CreateForExperts(sizeof_element, 0, 0);
}
        ...
        )
{
  if ( sizeof_element <= 0 )
  {
    MYON_ERROR( "MYON_FixedSizePool::Create - sizeof_element <= 0" );
    return false;
  }

  if ( m_sizeof_element != 0 || 0 != m_first_block )
  {
    MYON_ERROR( "MYON_FixedSizePool::Create - called on a pool that is in use." );
    return false;
  }
  ...
  if ( block_element_capacity <= 0 )
    block_element_capacity = MYON_FixedSizePool::DefaultElementCapacityFromSizeOfElement(m_sizeof_element);
...
bool MYON_FixedSizePool::CreateForExperts(
  size_t sizeof_element,
...
)
{
  ...
  if (sizeof_element <= 0)
  {
    MYON_ERROR("Invalid parameter: sizeof_element <= 0.");
    return false;
  }

  const size_t default_block_capacity = MYON_FixedSizePool::DefaultElementCapacityFromSizeOfElement(sizeof_element);
  if (default_block_capacity <= 0 || default_block_capacity* sizeof_element <= 0)
  {
    MYON_ERROR("Invalid parameter: sizeof_element is too large for a fixed size pool.");
    return false;
  }

  if (maximum_element_count_estimate < 0)
  {
    MYON_ERROR("Invalid parameter: block1_element_count < 0.");
    return false;
  }
  ...
  {
    MYON_ERROR("Invalid parameter: minimum_block2_element_capacity < 0.");
    return false;
  }
...
void MYON_FixedSizePool::ReturnAll()
{
  if ( 0 != m_first_block )
  {
    // initialize
...
void MYON_FixedSizePool::Destroy()
{
  void* p;
  void* next;
  next = m_first_block;
...
size_t MYON_FixedSizePool::ActiveElementCount() const
{
  return m_active_element_count;
}

size_t MYON_FixedSizePool::TotalElementCount() const
{
  return m_total_element_count;
}

void* MYON_FixedSizePool::AllocateDirtyElement()
{
  void* p;
  ...
  else
  {
    if ( 0 == m_al_block || 0 == m_al_count )
    {
      ...
      if ( 0 == next_block )
      {
        ...
        {
          MYON_ERROR("MYON_FixedSizePool::AllocateElement - you must call MYON_FixedSizePool::Create with a valid element size before using MYON_FixedSizePool");
          return nullptr;
        }
...
bool MYON_FixedSizePool::IsValid() const
{
  if (nullptr != m_first_block)
  {
    ...
    for (block = (const char*)m_first_block; 0 != block; block = next_block)
    {
      ...
      {
        MYON_ERROR("sizeof_block is not a multiple of m_sizeof_element");
        return false;
      }

      if (capacity != block_element_capacity)
      {
        MYON_ERROR("MYON_FixedSizePool::BlockElementCapacity error.");
        return false;
      }
      ...
      if (bBlockIsAlBlock)
      {
        ...
        {
          MYON_ERROR("sizeof_block_allocated is not a multiple of m_sizeof_element");
          return false;
        }
        if ( block_element_count > block_element_capacity )
        {
          MYON_ERROR("block_element_count > block_element_capacity");
          return false;
        }
        if ( block_element_count + m_al_count != block_element_capacity)
        {
          MYON_ERROR("block_element_count + m_al_count != block_element_capacity");
          return false;
        }
      }
      else
      ...
      {
        MYON_ERROR("m_total_element_count is not correct or some other serious problem.");
        return false;
      }

      if (count != block_element_count)
      {
        MYON_ERROR("MYON_FixedSizePool::BlockElementCount error.");
        return false;
      }
    }
    ...
    if (total_element_count != (size_t)m_total_element_count)
    {
      MYON_ERROR("m_total_element_count or m_al_element_array is not correct or some other serious problem.");
      return false;
    }
  }
  ...
  {
    MYON_ERROR("m_active_element_count > m_total_element_count");
    return false;
  }
...
void* MYON_FixedSizePool::AllocateElement()
{
...
void MYON_FixedSizePool::ReturnElement(void* p)
{
  if ( p )
  {
    if ( m_active_element_count <= 0 )
    {
      ...
      MYON_ERROR("MYON_FixedSizePool::ReturnElement - no active elements exist.");
    }
    else
    {
      m_active_element_count--;
...
void* MYON_FixedSizePool::ThreadSafeAllocateDirtyElement()
{
  void* p = nullptr;
  {
...
void* MYON_FixedSizePool::ThreadSafeAllocateElement()
{
  void* p = nullptr;
  {
    if ( m_sleep_lock.GetLock() )
...
void MYON_FixedSizePool::ThreadSafeReturnElement(void* p)
{
  if (nullptr != p)
  {
    if ( m_sleep_lock.GetLock() )
...
MYON_FixedSizePoolIterator::MYON_FixedSizePoolIterator()
  : m_fsp(0)
  , m_it_block(0)
  , m_it_element(0)
{}

MYON_FixedSizePoolIterator::MYON_FixedSizePoolIterator( const MYON_FixedSizePool& fsp )
  : m_fsp(&fsp)
  , m_it_block(0)
  , m_it_element(0)
{}

const class MYON_FixedSizePool* MYON_FixedSizePoolIterator::FixedSizePool()
{
  return m_fsp;
}

void MYON_FixedSizePoolIterator::Create(const MYON_FixedSizePool* fsp)
{
  m_fsp = fsp;
  m_it_block = 0;
  m_it_element = 0;
...
void MYON_FixedSizePoolIterator::Reset()
{
  m_it_block = 0;
  m_it_element = 0;
}

void* MYON_FixedSizePoolIterator::FirstElement()
{
...
void* MYON_FixedSizePoolIterator::NextElement()
{
  if ( m_it_element )
  {
...
void* MYON_FixedSizePoolIterator::CurrentElement() const
{
  return m_it_element;
}

void* MYON_FixedSizePoolIterator::FirstElement(size_t element_index)
{
  const char* block;
  const char* block_end;
  const char* next_block;
...
size_t MYON_FixedSizePool::BlockElementCapacity( const void* block ) const
{
...
size_t MYON_FixedSizePool::BlockElementCount( const void* block ) const
{
...
void* MYON_FixedSizePoolIterator::FirstBlock( size_t* block_element_count )
{
  if ( m_fsp && m_fsp->m_first_block && m_fsp->m_total_element_count > 0 )
  {
    m_it_block = m_fsp->m_first_block;
...
void* MYON_FixedSizePoolIterator::NextBlock( size_t* block_element_count )
{
  if ( 0 != m_it_block
...
void* MYON_FixedSizePool::Element(size_t element_index) const
{
...
size_t MYON_FixedSizePool::ElementIndex(const void* element_pointer) const
{
  if (nullptr != element_pointer)
  {
    ...
    for (block = (const char*)m_first_block; 0 != block; block = next_block)
    {
      ...
      if (ptr >= block && ptr < block_end)
      {
        ...
        // Caller is confused
        MYON_ERROR("element_pointer is offset into an fsp element.");
        return MYON_MAX_SIZE_T;
      }
      block_count = (block_end - block) / m_sizeof_element;
      ptr_index += block_count;
    }
    // Caller is confused
    MYON_ERROR("element_pointer is not in allocated fsp memory.");
    return MYON_MAX_SIZE_T;
  }
...
bool MYON_FixedSizePool::InPool(
  const void* p
) const
{
  if (nullptr != p)
...
void* MYON_FixedSizePool::ElementFromId(
  size_t id_offset,
  unsigned int id
  ) const
{
  ...
  {
    // caller is confused.
    MYON_ERROR("id_offset is too small.");
    return nullptr;
  }
  if (id_offset + sizeof(id) > m_sizeof_element)
  {
    // caller is confused.
    MYON_ERROR("id_offset is too large.");
    return nullptr;
  }
  ...
  for (block = (const char*)m_first_block; 0 != block; block = next_block)
  {
    ...
    return (void*)MYON_BinarySearchArrayForUnsingedInt(id, block, count, m_sizeof_element, id_offset );
  }

  return nullptr;
}

unsigned int MYON_FixedSizePool::MaximumElementId(
  size_t id_offset
) const
{
  const char* block;
...
bool MYON_FixedSizePool::ElementIdIsIncreasing(
  size_t id_offset
) const
{
  const char* block;
  ...
  {
    // caller is confused.
    MYON_ERROR("m_sizeof_element must be a multiple of sizeof(unsigned int).");
    return false;
  }
  if (id_offset < sizeof(void*) )
  {
...
unsigned int MYON_FixedSizePool::ResetElementId(
  size_t id_offset,
