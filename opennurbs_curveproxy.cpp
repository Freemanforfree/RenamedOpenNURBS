...
MYON_OBJECT_IMPLEMENT(MYON_CurveProxy,MYON_Curve,"4ED7D4D9-E947-11d3-BFE5-0010830122F0");
...
MYON_CurveProxy::~MYON_CurveProxy()
{
  m_real_curve = 0;
}

MYON_CurveProxy::MYON_CurveProxy( const MYON_CurveProxy& src ) 
  : MYON_Curve(src)
...
MYON_CurveProxy& MYON_CurveProxy::operator=( const MYON_CurveProxy& src )
{
  if ( this != &src ) 
  {
...
MYON_CurveProxy::MYON_CurveProxy( MYON_CurveProxy&& src) MYON_NOEXCEPT
...
MYON_CurveProxy& MYON_CurveProxy::operator=( MYON_CurveProxy&& src)
{
  if ( this != &src ) 
  {
...
MYON_CurveProxy::MYON_CurveProxy( const MYON_Curve* c ) 
...
MYON_CurveProxy::MYON_CurveProxy( const MYON_Curve* c, MYON_Interval domain ) 
             : m_real_curve(c), 
               m_bReversed(0),
...
unsigned int MYON_CurveProxy::SizeOf() const
{
...
MYON__UINT32 MYON_CurveProxy::DataCRC(MYON__UINT32 current_remainder) const
{
  if ( m_real_curve )
    current_remainder = m_real_curve->DataCRC(current_remainder);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bReversed),&m_bReversed);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_real_curve_domain),&m_real_curve_domain);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_this_domain),&m_this_domain);
  return current_remainder;
}

double MYON_CurveProxy::RealCurveParameter( double t ) const
{
...
double MYON_CurveProxy::ThisCurveParameter( double real_curve_parameter ) const
{
...
MYON_Interval MYON_CurveProxy::RealCurveInterval( const MYON_Interval* sub_domain ) const
{
  if ( !sub_domain )
    return m_real_curve_domain;
  MYON_Interval d = m_this_domain;
  d.Intersection(*sub_domain);
  double t0 = RealCurveParameter( d[m_bReversed?1:0] );
  double t1 = RealCurveParameter( d[m_bReversed?0:1] );
  return MYON_Interval(t0,t1);
}


bool MYON_CurveProxy::ProxyCurveIsReversed() const
{
  return m_bReversed;
}

void MYON_CurveProxy::SetProxyCurveIsReversed(bool bReversed)
{
  m_bReversed = bReversed;
}

void MYON_CurveProxy::SetProxyCurve( const MYON_Curve* real_curve )
{
  ...
  {
    DestroyCurveTree();
    m_real_curve_domain = MYON_Interval::EmptyInterval;
    m_this_domain = MYON_Interval::EmptyInterval;
    m_bReversed = false;
  }
}
...
const MYON_Curve* MYON_CurveProxy::ProxyCurve() const
{
  return m_real_curve;
}

bool MYON_CurveProxy::SetProxyCurveDomain( MYON_Interval proxy_curve_subdomain )
{
  DestroyCurveTree();
  ...
  if ( rc )
  {
    if ( m_real_curve )
    {
      MYON_Interval cdom = m_real_curve->Domain();
...
MYON_Interval MYON_CurveProxy::ProxyCurveDomain() const
{
  return m_real_curve_domain;
}

MYON_Curve* MYON_CurveProxy::DuplicateCurve() const
{
  // duplicate underlying curve
  MYON_Curve* dup_crv = 0;
...
bool MYON_CurveProxy::IsValid( MYON_TextLog* text_log ) const
{
  ...
  if ( rc && !m_real_curve_domain.IsIncreasing() )
  {
    rc = false;
    if ( text_log) 
      text_log->Print("MYON_CurveProxy.m_real_curve_domain is not increasing.\n");
  }
  ...
  if ( rc && !m_real_curve->Domain().Includes( m_real_curve_domain ) )
  {
    rc = false;
    if ( text_log) 
      text_log->Print("MYON_CurveProxy.m_real_curve_domain is not included m_real_curve->Domain().\n");
  }
  ...
  if ( rc && !m_this_domain.IsIncreasing() )
  {
    rc = false;
    if ( text_log) 
      text_log->Print("MYON_CurveProxy.m_this_domain is not increasing.\n");
  }
...
void
MYON_CurveProxy::Dump( MYON_TextLog& dump ) const
{
  dump.Print("MYON_CurveProxy uses %x on [%g,%g]\n",m_real_curve,m_real_curve_domain[0],m_real_curve_domain[1]);
}

bool 
MYON_CurveProxy::Write(
...
int 
MYON_CurveProxy::Dimension() const
{
  return ( m_real_curve ) ? m_real_curve->Dimension() : 0;
}

bool MYON_CurveProxy::GetBBox( // returns true if successful
...
bool
MYON_CurveProxy::Transform( const MYON_Xform& )
{
  return false; // cannot modify proxy objects
}

MYON_Interval MYON_CurveProxy::Domain() const
{
  return m_this_domain;
}

bool MYON_CurveProxy::SetDomain( double t0, double t1 )
{
  bool rc = false;
  if (t0 < t1)
  {
...
bool MYON_CurveProxy::SetDomain( MYON_Interval domain )
{
...
int MYON_CurveProxy::SpanCount() const
{
  if (!m_real_curve) return 0;
  int rsc = m_real_curve->SpanCount();
  MYON_Interval domain = m_real_curve->Domain();
...
bool MYON_CurveProxy::GetSpanVector( double* d ) const
{
...
int MYON_CurveProxy::Degree() const
{
  return m_real_curve ? m_real_curve->Degree() : 0;
}

bool 
MYON_CurveProxy::GetParameterTolerance(
...
bool
MYON_CurveProxy::IsLinear( // true if curve locus is a line segment
      ...
      ) const
{
  ...
  if ( 0 != m_real_curve )
  {
    ...
    {
      // The MYON_CurveProxy::DuplicateCurve() scope is critical
      // because there are situation where people derive a 
      // class from MYON_CurveProxy, and override the virtual
      // DuplicateCurve().  In this situation I rely on getting
      // the result returned by MYON_CurveProxy::DuplicateCurve().
      MYON_Curve* temp_curve = MYON_CurveProxy::DuplicateCurve();
      if ( 0 != temp_curve )
      {
      ...
      ) const
{
  ...
  if ( !m_real_curve )
    return 0;
  const MYON_Interval cdom = m_real_curve->Domain();
...
bool
MYON_CurveProxy::IsArc( // true if curve locus in an arc or circle
...
bool 
MYON_CurveProxy::IsClosed() const
{
  bool rc = false;
...
bool 
MYON_CurveProxy::IsPeriodic() const
{
  bool rc = false;
  if ( m_real_curve && m_real_curve->Domain() == m_real_curve_domain )
  {
...
bool MYON_CurveProxy::GetNextDiscontinuity( 
                MYON::continuity c,
                double t0,
                double t1,
                double* t,
  ...
  if ( 0 != m_real_curve )
  {
    ...
    if ( realcrv_rc ) 
    {
      ...
      if ( !(t0 < thiscrv_t && thiscrv_t < t1) && !(t1 < thiscrv_t && thiscrv_t < t0) )
      {
        ...
        double s2 = MYON_SQRT_EPSILON*s1 + (1.0 - MYON_SQRT_EPSILON)*s0;
...
bool MYON_CurveProxy::IsContinuous(
...
bool 
MYON_CurveProxy::Evaluate( // returns false if unable to evaluate
       ...
       ) const
{
  ...
  if( fabs( normt )<MYON_ZERO_TOLERANCE)
    side = ( std::abs(side) <= 1) ? 1 : 2;
  else if( fabs(1.0 - normt)<MYON_ZERO_TOLERANCE)
    side = ( std::abs(side) <= 1) ? -1 : -2;
  ...
  if ( 0 != side )
  {
    ...
    if (m_bReversed || m_real_curve_domain != m_this_domain )
    {
      ...
      //  MYON_TuneupEvaluationParameter() when it matters.
      if ( -1 == side )
        side = -2;
      else if ( 1 == side )
        side = 2;
  ...
  )
{
  ...
  {
    MYON_Interval trim_dom = m_this_domain;
    trim_dom.Intersection(domain);
    if ( trim_dom.IsIncreasing() )
    {
      MYON_Interval real_dom = RealCurveInterval( &trim_dom );
  ...
  ) const
{
  ...
  if ( m_this_domain.IsIncreasing() && m_real_curve_domain.IsIncreasing() && m_this_domain.Includes(t,true) )
  {
    ...
    {
      MYON_CurveProxy* left_proxy = 0;
      MYON_CurveProxy* right_proxy = 0;
      if ( left_side )
      {
        left_proxy = MYON_CurveProxy::Cast(left_side);
        if ( !left_proxy )
          return false;
      }
      if ( right_side )
      {
        right_proxy = MYON_CurveProxy::Cast(right_side);
        if ( !right_proxy )
          return false;
      ...
      MYON_Interval left_real_dom, right_real_dom;
      if ( bRev )
      {
      ...
      MYON_Interval left_this_dom(m_this_domain[0],t);
      MYON_Interval right_this_dom(t,m_this_domain[1]);
           ...
           )
      {
        ...
        const MYON_Curve* real_crv = m_real_curve;
        if ( real_crv )
        {
          MYON_Interval d = real_crv->Domain();
        ...
        if ( !left_proxy )
          left_proxy = new MYON_CurveProxy();
        if ( !right_proxy )
          right_proxy = new MYON_CurveProxy();
...
int MYON_CurveProxy::GetNurbForm( // returns 0: unable to create NURBS representation
                 ...
                 //            desired accuracy.
      MYON_NurbsCurve& nurbs,
      double tolerance,  // (>=0)
      const MYON_Interval* sub_domain  // OPTIONAL subdomain of MYON::ProxyCurve::Domain()
      ) const
{
  int rc = 0;
  if ( m_real_curve ) 
  {
    MYON_Interval scratch_domain = RealCurveInterval( sub_domain );
    ...
    if ( rc > 0 )
    {
      ...
      if ( nurbs.m_dim <= 3 && nurbs.m_dim >= 1 )
      {
        ...
        MYON_3dPoint P0 = PointAt(t0);
        MYON_3dPoint P1 = PointAt(t1);
        MYON_3dPoint N0 = nurbs.PointAtStart();
        MYON_3dPoint N1 = nurbs.PointAtEnd();
...
int 
MYON_CurveProxy::HasNurbForm( // returns 0: unable to create NURBS representation
                 //            with desired accuracy.
                 //         1: success - returned NURBS parameterization
                 //            matches the curve's to wthe desired accuracy
                 //         2: success - returned NURBS point locus matches
...
bool MYON_CurveProxy::GetCurveParameterFromNurbFormParameter(
      double nurbs_t,
      double* curve_t
      ) const
{
  ...
  if ( m_real_curve ) 
  {
    ...
    MYON_Curve* tmp_real_crv = 0;
    if ( m_real_curve_domain != m_real_curve->Domain() )
    {
      const MYON_ArcCurve* arc_curve = MYON_ArcCurve::Cast(m_real_curve);
      if ( 0 != arc_curve )
      {
...
bool MYON_CurveProxy::GetNurbFormParameterFromCurveParameter(
      double curve_t,
      double* nurbs_t
      ) const
{
