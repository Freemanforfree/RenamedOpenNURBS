...
#if !defined(MYON_CONVEX_POLY_INC_)
#define MYON_CONVEX_POLY_INC_


// A Simplex in 3d
class MYON_CLASS MYON_3dSimplex
{
public:
  MYON_3dSimplex();																							// An empty simplex
  explicit MYON_3dSimplex(const MYON_3dPoint& a);																	//  0-simplex in 3d
  MYON_3dSimplex(const MYON_3dPoint& a, const MYON_3dPoint& b);										//  1-simplex
  MYON_3dSimplex(const MYON_3dPoint& a, const MYON_3dPoint& b, const MYON_3dPoint& c);			//  2-simplex
  MYON_3dSimplex(const MYON_3dPoint& a, const MYON_3dPoint& b, const MYON_3dPoint& c, const MYON_3dPoint& d);		//  3-simplex

  MYON_3dSimplex(const MYON_3dSimplex& rhs) = default;
  MYON_3dSimplex& operator=(const MYON_3dSimplex& rhs) = default;
  ~MYON_3dSimplex() = default;
  ...
  */
  MYON_3dPoint Evaluate(const double* b) const;
  MYON_3dPoint Evaluate(const MYON_4dPoint& b) const;
  ...
  */
  bool GetClosestPoint(const MYON_3dPoint& P0, MYON_4dPoint& Bary, double maximum_distance = MYON_DBL_MAX) const;
  bool GetClosestPointToOrigin(MYON_4dPoint& Bary) const;
  ...
  */
  double Volume() const;
  double SignedVolume() const; // returns MYON_UNSET_VALUE if Count()<4 else the signed volume

  /*
    FaceNormal(noti) is the oriented face normal obtained by omitting vertex noti.
    FaceNormal returns MYON_UNSET_VALUE if Count()<3 or Count()==4 noti not 0,1,2 or 3.
    FaceUnitNormal returns MYON_UNSET_VALUE if Count()<3 or Count()==4 noti not 0,1,2 or 3 or if FaceNormal(noti)=Zero_Vector
  */
  MYON_3dVector FaceNormal(int noti = 0) const;
  MYON_3dVector FaceUnitNormal(int noti = 0) const;

  /*
  Edge vector from Vertex(e0) to Vertex(e1)
  */
  MYON_3dVector Edge(int e0, int e1)const;
  ...
  */
  ...
  bool AddVertex(const MYON_3dPoint&);

  /* Modify a vertex. i<Count() */
  bool SetVertex(int i, MYON_3dPoint P);

  // Returns a Vertex or a reference to one when 0<=i<Count()
  MYON_3dPoint& operator[](int);
  const MYON_3dPoint& operator[](int i) const;

  MYON_3dPoint Vertex(int i) const;
  MYON_3dPoint& Vertex(int i);
...
private:
  ...
  bool Closest3plex(MYON_4dPoint& Bary) const;
  bool Closest2plex(MYON_4dPoint& Bary) const;
  bool Closest1plex(MYON_4dPoint& Bary) const;
  static bool RoundBarycentricCoordinate(MYON_4dPoint& Bary);
};
...
ClosestPoint(MYON_ConvexPoly& A, MYON_ConvexPoly& B, ...)

*/
class MYON_CLASS MYON_ConvexPoly
{
public:
  /*
  ...
  */
   ...
   */
  virtual MYON_3dVector Vertex(int i) const = 0;

  /*
  Description:
    Let K be this MYON_ConvexPoly then for a non-zero vector W the support Support(W) are point in K defined by
       arg max     x * W
        x \in K
     ...
     a minimizer.
   */
  MYON_3dPoint Support(MYON_3dVector W, int i0 =0) const
  {
  ...
  */
  virtual int SupportIndex(MYON_3dVector W, int i0=0) const = 0;

 /*
 Description:
   Points in a Convex Polytope are parameterized , not necessaily uniquely,
   by an MYON_4dex of vertex indices and a 4d barycentric point B
  ...
  */
  MYON_3dPoint Evaluate(MYON_4dex dex, MYON_4dPoint B)const
  {
...
*/
  bool GetClosestPoint( MYON_3dPoint P0,
    MYON_4dex& dex, MYON_4dPoint& bary, 
    double maximum_distance = MYON_DBL_MAX) const;
  ...
  bool GetClosestPointSeeded(MYON_3dPoint P0,
    MYON_4dex& dex, MYON_4dPoint& Bary,
    double maximum_distance = MYON_DBL_MAX) const;
 ...
 */
  bool GetClosestPoint(const MYON_ConvexPoly& BHull,
...
// the points of this-Bhull singled out by Adex and Bdex must define a nondegenerate simplex
  bool GetClosestPointSeeded(const MYON_ConvexPoly& BHull,
    MYON_4dex& Adex, MYON_4dex& Bdex, MYON_4dPoint& bary,
    double maximum_distance = MYON_DBL_MAX) const;
  ...
  */
  ...
  /*
  Description:
    A point represented by a MYON_4dex D and a barycentric coordinate B
  ...
  */
  static bool  Standardize(MYON_4dex& D, MYON_4dPoint& B);

  /*
  Returns:
    true if d[i]<n for i=0..3  a valid MYON_4dex for a point in a MYON_ConvexPolyBase with Count()=n 
  */
  static bool IsValid4DexN(const MYON_4dex& D, int n)
  {
    for (int i = 0; i < 4; i++) {
  ...
  bool IsValid4Dex(const MYON_4dex& D) const { return IsValid4DexN(D, Count()); };

  virtual ~MYON_ConvexPoly() {};
};
...
class MYON_CLASS MYON_ConvexHullRef : public MYON_ConvexPoly
{
public:
  MYON_ConvexHullRef() { m_n = 0; m_is_rat = false; m_stride = 3; };
  MYON_ConvexHullRef(const MYON_3dVector* V0, int  count);		// a 3d point array
  MYON_ConvexHullRef(const MYON_3dPoint* V0, int  count);		// a 3d point array
  MYON_ConvexHullRef(const MYON_4dPoint* V0, int count);			// a array of homogeneous points
  MYON_ConvexHullRef(const double* v0, bool is_rat, int n);		// v0 is an array of 3dpoints or homo 4d points
  MYON_ConvexHullRef(const double* v0, bool is_rat, int n, int stride);		// v0 is an array of 3dpoints or homo 4d points

  void Initialize(const MYON_3dVector* V0, int  count);
	void Initialize(const MYON_4dPoint* V0, int  count);
	void Initialize(const double* V0, MYON::point_style style, int  count);  // style must be either not_rational or homogeneous_rational = 2,

  int Count() const override { return m_n; }
  MYON_3dVector Vertex(int j) const override;

  // Support map
  virtual int SupportIndex(MYON_3dVector W, int i0) const override;
  virtual double MaximumCoordinate() const override;

  virtual ~MYON_ConvexHullRef() override {};
private:
...
// 3d convex hull defined by an explicit collection of points called vertices.
// Note: vertices need not be extreme points     
class MYON_CLASS MYON_ConvexHullPoint2 : public MYON_ConvexPoly
{
public:
  MYON_ConvexHullPoint2() = default;
  MYON_ConvexHullPoint2(int init_capacity) : m_Vert(init_capacity) {};

  virtual int Count() const override { return m_Vert.Count(); }
  virtual MYON_3dVector Vertex(int j) const override { return m_Vert[j]; }
  ...
  virtual ~MYON_ConvexHullPoint2() override {};

  int AppendVertex(const MYON_3dPoint& P); // return index of new vertex.  must set Adjacent Indices.
	void Empty();
...
private:
  MYON_ConvexHullRef Ref;
  MYON_SimpleArray<MYON_3dVector> m_Vert;
};
...
*/
MYON_DECL
int MYON_ConvexHull2d(const MYON_SimpleArray<MYON_2dPoint>& Pnt, MYON_SimpleArray<MYON_2dPoint>& Hull, MYON_SimpleArray< int>* PntInd = nullptr);
