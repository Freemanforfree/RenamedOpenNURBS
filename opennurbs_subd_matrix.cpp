  ...
  )
{
  ...
  if (fabs(c1 - cos(2.0*(MYON_PI*k / ((double)N)))) > 1e-6)
  {
    MYON_ERROR("bogus c1");
  }

  if (fabs(c2 - cos(MYON_PI*k / ((double)N))) > 1e-6)
  {
    MYON_ERROR("bogus c2");
  }
  ...
  {
    MYON_ERROR("bogus lambda0");
  }

  if (fabs(lambda1 - eigenvalues[1]) > 1e-6)
  {
    MYON_ERROR("bogus lambda1");
  }
...
unsigned int MYON_SubDSectorType::GetAllEigenvalues(
  double* eigenvalues,
  ...
  )
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < eigenvalues_capacity; i++)
      eigenvalues[i] = MYON_UNSET_VALUE;
  }
  ...
  const MYON_SubDVertexTag vertex_tag = VertexTag();
  const unsigned int N = EdgeCount();

  if (false == MYON_SubD::IsValidSectorEdgeCount(vertex_tag, N))
    return MYON_SUBD_RETURN_ERROR(0);
  ...
  if (MYON_SubDVertexTag::Smooth == vertex_tag)
  {
    ...
    MYON_SortDoubleArrayDecreasing(eigenvalues + 1, R - 1);
  }
...
bool MYON_SubDMatrix::EvaluateCosAndSin(
  unsigned int j,
  unsigned int n,
  double* cos_theta,
  double* sin_theta
  )
{
  ...
  {
    if (cos_theta)
      *cos_theta = MYON_DBL_QNAN;
    if (sin_theta)
      *sin_theta = MYON_DBL_QNAN;
    return MYON_SUBD_RETURN_ERROR(false);
  }

#if defined(MYON_DEBUG)
#define MYON_DEBUG_TEST_SUBD_EVSINCOS
#endif

#if defined(MYON_DEBUG_TEST_SUBD_EVSINCOS)
  const double theta_dbg = (j*MYON_PI)/n;
  ...
  else
  {
    ...
    {
      const double theta = (j*MYON_PI) / n;
  ...
  {
    MYON_SubDIncrementErrorCount();
    MYON_ERROR("Bug in SinCosEv precision improvment code.");
  }
#endif
...
unsigned int MYON_SubDSectorType::GetSubdivisionMatrix(
  double** S,
  size_t S_capacity
  ) const
{
  ...
  if (this->IsSmoothSector() || this->IsDartSector())
  {
    ...
    */
    ...
    const double dart_factor = this->IsDartSector() ? 0.25*cos(2.0*MYON_PI/((double)N)) : 0.0;
  ...
  if ( this->IsCreaseSector() || this->IsCornerSector() )
  {
    ...
    // special handling in all MYON_SubD code.
...
double MYON_SubDSectorType::SubdominantEigenvalue() const
{
  const double rc_error = MYON_UNSET_VALUE;
  const unsigned int R = PointRingCount();
  if (R < 3)
    return MYON_SUBD_RETURN_ERROR(rc_error);
  ...
  case MYON_SubDVertexTag::Smooth:
    if (1 == (R % 2))
    {
      ...
      MYON_SubDMatrix::EvaluateCosAndSin(2, F, &cos_2pi_over_F, &lambda);
      MYON_SubDMatrix::EvaluateCosAndSin(1, F, &cos_pi_over_F, &lambda);
...
unsigned int MYON_SubDSectorType::SubdominantEigenvalueMulitiplicity() const
{
  if (false == IsValid() )
    return 0;
  ...
  else if (2 == m_sector_face_count)
  {
    ...
    if (MYON_SubDVertexTag::Smooth == m_vertex_tag)
    {
      // April 2019 Dale Lear - Catmull Clark Valence 2 Evaluation: Smooth
      // See comments in MYON_SubDSectorType::GetSurfaceEvaluationCoefficients()
      // for more details on how this case is handled.
    ...
    if (MYON_SubDVertexTag::Dart == m_vertex_tag)
    {
      // April 2019 Dale Lear - Catmull Clark Valence 2 Evaluation: Dart
      // See comments in MYON_SubDSectorType::GetSurfaceEvaluationCoefficients()
      // for more details on how this case is handled.
...
double MYON_SubDSectorType::GetSubdominantEigenvectors(
  double* E1,
  size_t E1_capacity,
  double* E2,
  size_t E2_capacity
) const
{
  ...
  case MYON_SubDVertexTag::Smooth:
    if (1 == (R % 2))
    {
      if (nullptr != E1)
      {
        ...
        cos0 = 1.0;
        sin0 = 0.0;
        MYON_SubDMatrix::EvaluateCosAndSin(2, F, &cos1, &sin1);
        ...
        for (;;)
        {
          ...
          cos0 = cos1;
          sin0 = sin1;
          MYON_SubDMatrix::EvaluateCosAndSin(i, F, &cos1, &sin1);
  ...
  case MYON_SubDVertexTag::Corner:
    if (0 == (R % 2))
    {
      ...
      const unsigned int M = MYON_SubDSectorType::MaximumCornerAngleIndex;
      ...
      for (;;)
      {
        cos0 = cos1;
        sin0 = sin1;
        MYON_SubDMatrix::EvaluateCosAndSin((i/2 + 1)*I,F*M,&cos1,&sin1);
        E1[i] = sin0;
        E2[i] = cos0;
        i++;
        E1[i] = sin0+sin1;
    ...
    if (0 == (R % 2))
    {
      ...
      else
      {
        ...
        for (;;)
        {
          cos0 = cos1;
          sin0 = sin1;
          MYON_SubDMatrix::EvaluateCosAndSin(i/2 + 1,F,&cos1,&sin1);
          E1[i] = sin0;
          E2[i] = cos0;
          i++;
          E1[i] = sin0+sin1;
...
static double MYON_SubDSectorType_LimitSurfaceNormalSign(
  unsigned int R,
  double sector_angle,
  const double* L1,
  const double* L2
  )
{
  ...
  MYON_2dVector T[2] = { MYON_2dVector::ZeroVector, MYON_2dVector::ZeroVector };
  double a = 0.0;
...
double MYON_SubDSectorType::SurfaceNormalSign() const
{
  const double rc_error = MYON_UNSET_VALUE;
  if (false == IsValid())
    return MYON_SUBD_RETURN_ERROR(rc_error);
  ...
  MYON_SimpleArray<double> buffer;
  ...
  case MYON_SubDVertexTag::Smooth:
  case MYON_SubDVertexTag::Dart:
    sector_angle = 2.0*MYON_PI;
    break;
  case MYON_SubDVertexTag::Crease:
    sector_angle = 0.5*MYON_PI;
    break;
  case MYON_SubDVertexTag::Corner:
  ...
  double z = MYON_SubDSectorType_LimitSurfaceNormalSign(R,sector_angle,L1,L2);
  return z;
}

bool MYON_SubDSectorType::SurfaceEvaluationCoefficientsAvailable() const
{
  if (IsValid())
  {
    // Available as of March 23, 2015
    //if (MYON_SubDVertexTag::Dart == m_vertex_tag && FaceCount() > 5)
    //{
    //  // temporary limit
    //  return false;
    //}
...
unsigned int MYON_SubDSectorType::GetSurfaceEvaluationCoefficients(
  double* LP,
  size_t LP_capacity,
  double* L1,
  size_t L1_capacity,
    ...
    if (1 == (R % 2))
    {
      ...
      if (F > 5)
      {
        if (false == Sbuffer.Create(R+2,R))
          return MYON_SUBD_RETURN_ERROR(0);
        if (R != MYON_SubDSectorType::GetSubdivisionMatrix(Sbuffer.m, R))
          return MYON_SUBD_RETURN_ERROR(0);
        S = Sbuffer.m;
        E1 = Sbuffer.m[R];
        E2 = Sbuffer.m[R+1];
        ...
        default:
          {
            ...
            if (lambda_multiplicity != MYON_GetEigenvectors(R, S, true, lambda_local, lambda_multiplicity, termination_tolerances, eigenvectors, eigenprecision, eigenpivots))
              return MYON_SUBD_RETURN_ERROR(0);
            if (E1[0] < 0.0)
            {
              for ( unsigned int i = 0; i < R; i++ )
  ...
  case MYON_SubDVertexTag::Corner:
    if (0 == (R % 2))
    {
      ...
      else
      {
        ...
        double s = (2 * angle_index <= MYON_SubDSectorType::MaximumCornerAngleIndex) ? 1.0 : -1.0;
  ...
  case MYON_SubDVertexTag::Crease:
    ...
    if (0 == (R % 2))
    {
      ...
      else
      {
        ...
        cos0 = 1.0; // cos(0)
        sin0 = 0.0; // sin(0)
        MYON_SubDMatrix::EvaluateCosAndSin(1,F,&cos1,&sin1); // cos1 = cos(pi/F), sin1 = sin(pi/F)
        z = (1.0 + cos1) / sin1;
...
bool MYON_SubDMatrix::EvaluateSubdivisionPoint(
...
bool MYON_SubDMatrix::EvaluateSurfacePoint(
  const double* point_ring,
  size_t point_ring_count,
  size_t point_ring_stride,
...
static bool Internal_GetAlterateTangent(
  const MYON_SubDMatrix& subd_matrix,
  unsigned int Ldex,
  size_t point_ring_count,
  size_t point_ring_stride,
  const double* point_ring,
...
)
{
  ...
  if (2 == Ldex)
  {
    if ( 4 == subd_matrix.m_R && MYON_SubDVertexTag::Crease == subd_matrix.m_sector_type.VertexTag() )
    {
      // valence 2 crease case when crease edges are colinear
      // F = face point, C = crease vertex point.
      // Default tangents:
      //   L[1] = MYON_3dPoint(point_ring +  point_ring_stride) - MYON_3dPoint(point_ring + 3 * point_ring_stride);
      const MYON_3dPoint C(point_ring);
      const MYON_3dPoint Q(point_ring + 2 * point_ring_stride);
      MYON_3dVector V(Q - C);
      if (V.IsNotZero())
      {
...
)
{
  ...
  if ( 4 == subd_matrix.m_R && MYON_SubDVertexTag::Crease == subd_matrix.m_sector_type.VertexTag() )
  {
    ...
    if (false == bHaveAlternateL[2])
    {
      ...
      {
        MYON_3dVector T1(L[1]);
        MYON_3dVector T2(L[2]);
        N = MYON_CrossProduct(T1, T2).UnitVector();
      }
    }
    if (N.IsZero())
    {
      N = MYON_CrossProduct(L[1], L[2]).UnitVector();
      if (N.IsZero())
      {
        const MYON_3dPoint C(point_ring);
        const MYON_3dPoint P0(point_ring + 1 * point_ring_stride); // end of crease leaving C
        const MYON_3dPoint Q(point_ring + 2 * point_ring_stride);
        const MYON_3dPoint P1(point_ring + 1 * point_ring_stride); // end of crease entering C
        const MYON_3dVector A = (P0 - P1).UnitVector();
        const MYON_3dVector B = (Q - C).UnitVector();
        N = MYON_CrossProduct(A, B).UnitVector();
      }
    }
    if (N.IsNotZero())
    {
  ...
  ) const
{
  ...
  {
    limit_point[0] = MYON_DBL_QNAN;
    limit_point[1] = MYON_DBL_QNAN;
    limit_point[2] = MYON_DBL_QNAN;
  }

  if (nullptr != limit_normal)
  {
    limit_normal[0] = MYON_DBL_QNAN;
    limit_normal[1] = MYON_DBL_QNAN;
    limit_normal[2] = MYON_DBL_QNAN;
  }

  if (nullptr != limit_tangent1)
  {
    limit_tangent1[0] = MYON_DBL_QNAN;
    limit_tangent1[1] = MYON_DBL_QNAN;
    limit_tangent1[2] = MYON_DBL_QNAN;
  }

  if (nullptr != limit_tangent2)
  {
    limit_tangent2[0] = MYON_DBL_QNAN;
    limit_tangent2[1] = MYON_DBL_QNAN;
    limit_tangent2[2] = MYON_DBL_QNAN;
  }
  ...
  MYON_3dVector N = MYON_CrossProduct(T1, T2);
  N.Unitize();
  if (N.IsZero())
  {
  ...
  if (nullptr != limit_tangent1)
  {
    ...
    if (0.0 == limit_tangent1[0] && 0.0 == limit_tangent1[1] && 0.0 == limit_tangent1[2])
    {
      ...
      {
        MYON_ERROR("limit_tangent1[0] = zero vector");
  ...
  if (nullptr != limit_tangent2)
  {
    ...
    if (0.0 == limit_tangent2[0] && 0.0 == limit_tangent2[1] && 0.0 == limit_tangent2[2])
    {
      if ( false == bUndefinedNormalIsPossible )
      {
        MYON_ERROR("limit_tangent2[0] = zero vector");
        bUndefinedNormalIsPossible = true; // one error is sufficient
      }
    }
  }
  ...
  if (nullptr != limit_normal)
  {
    ...
    if (0.0 == limit_normal[0] && 0.0 == limit_normal[1] && 0.0 == limit_normal[2])
    {
      if ( false == bUndefinedNormalIsPossible )
      {
        MYON_ERROR("limit_normal[0] = zero vector");
      }
    }
  }
...
class MYON_SubDMatrixHashElement
{
public:
  MYON_SubDMatrix m_sm;
  class MYON_SubDMatrixHashElement* m_next = nullptr;
};

static MYON_SubDMatrixHashElement* FindMatrixHashElement(
  const MYON_SubDSectorType& sector_type,
  MYON_SubDMatrixHashElement* element_list
  )
{
  for (/*empty init*/; nullptr != element_list; element_list = element_list->m_next)
  {
    if ( 0 == MYON_SubDSectorType::Compare(&sector_type,&element_list->m_sm.m_sector_type) )
      return element_list;
  }
  return nullptr;
}
...
static int CompareMatrixHashElement(
  const MYON_SubDMatrixHashElement* a,
  const MYON_SubDMatrixHashElement* b
  )
{
  if ( a == b )
    return 0;
  ...
  return MYON_SubDSectorType::Compare(&a->m_sm.m_sector_type,&b->m_sm.m_sector_type);
}

const MYON_SubDMatrix& MYON_SubDMatrix::FromCache(
  MYON_SubDSectorType sector_type
  )
{
  // The MYON_SubDMatrix cache is a global resource for the entire application.
  // This is a thread safe function that manages the MYON_SubDMatrix cache.
  static MYON_SleepLock lock;

  // The sector_type.Hash() is used to find elements in the cache.
  static MYON_SubDMatrixHashElement* hash_table[256] = { 0 };

  const unsigned int hash = sector_type.SectorTypeHash();
  if (0 == hash)
    return MYON_SUBD_RETURN_ERROR(MYON_SubDMatrix::Empty); 
  ...
  MYON_SubDMatrixHashElement* hash_element = FindMatrixHashElement(sector_type,hash_table[hash_index]);
  ...
  // Lock the cache
  bool bReturnLock = lock.GetLock(0,30*MYON_SleepLock::OneSecond);
  if (false == bReturnLock)
    bReturnLock = lock.GetLock(0, MYON_SleepLock::OneSecond);
  ...
  if (nullptr == hash_element)
  {
    ...
    unsigned int R = 0;
    MYON_SubDMatrixHashElement* new_hash_element = new(std::nothrow) MYON_SubDMatrixHashElement();
    if (nullptr != new_hash_element)
    {
      ...
      if (0 == R)
      {
        ...
        new_hash_element = new(std::nothrow) MYON_SubDMatrixHashElement();
        if (nullptr != new_hash_element)
      ...
      if (nullptr != new_hash_element)
      {
        ...
        else
        {
          ...
          MYON_SubDMatrixHashElement* prev_hash_element = nullptr;
...
unsigned int MYON_SubDMatrix::SetFromSectorType(
  MYON_SubDSectorType sector_type
  )
{
  m_sector_type = MYON_SubDSectorType::Empty;
  m_R = 0;
  m_S = nullptr;
  m_LP = nullptr;
  m_L1 = nullptr;
...
{
  if (!(d >= 0.0 && rc >= 0.0))
    MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
...
double MYON_SubDMatrix::TestMatrix() const
{
  if (nullptr == m_S || m_R < 3)
    return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
...
double MYON_SubDMatrix::TestEvaluation(
  MYON_SubDSectorType sector_type,
...
{
  MYON_SubDVertexTag vertex_tags[] = { 
    MYON_SubDVertexTag::Smooth
    ,MYON_SubDVertexTag::Crease
    ,MYON_SubDVertexTag::Corner
    ,MYON_SubDVertexTag::Dart
  };
  ...
  unsigned int corner_sector_angle_index0 = MYON_UNSET_UINT_INDEX-1;
  unsigned int corner_sector_angle_index1 = MYON_UNSET_UINT_INDEX;
  const double corner_sector_angle_radians 
    = (MYON_SubDVertexTag::Corner == sector_type.VertexTag())
    ? sector_type.CornerSectorAngleRadians()
    : MYON_SubDSectorType::UnsetCornerSectorAngle;


  MYON_SubDVertexTag vertex_tag = sector_type.VertexTag();
  ...
  if (MYON_SubDVertexTag::Unset != vertex_tag)
  {
    for (size_t vertex_tag_index = vertex_tag_index0; vertex_tag_index < vertex_tag_count; vertex_tag_index++)
    {
      if (vertex_tag == vertex_tags[vertex_tag_index])
      {
        ...
        if (MYON_SubDVertexTag::Corner == vertex_tag && MYON_SubDSectorType::IsValidCornerSectorAngleRadians(corner_sector_angle_radians) )
        {
          unsigned int angle_dex = sector_type.CornerSectorAngleIndex();
          if (angle_dex <= MYON_SubDSectorType::MaximumCornerAngleIndex)
          {
          ...
          {
            corner_sector_angle_index1 = MYON_UNSET_UINT_INDEX;
  ...
  for (size_t vertex_tag_index = vertex_tag_index0; vertex_tag_index < vertex_tag_count; vertex_tag_index++)
  {
    const MYON_SubDVertexTag vertex_tag_for_scope = vertex_tags[vertex_tag_index];
    const char* sVertexTagName = vertex_tag_names[vertex_tag_index];

    unsigned int Fmin = MYON_SubDSectorType::MinimumSectorFaceCount(vertex_tag_for_scope);
    ...
    if (MYON_SubDVertexTag::Corner == vertex_tag_for_scope && MYON_SubDSectorType::UnsetCornerSectorAngle == corner_sector_angle_radians)
    {
      angle_i0 = 2;
      angle_i1 = MYON_SubDSectorType::MaximumCornerAngleIndex/2 - 1;
    }
    ...
    for (unsigned int F = Fmin; F <= Fmax; F++)
    {
      for (unsigned int corner_sector_angle_index = angle_i0; corner_sector_angle_index < angle_i1; corner_sector_angle_index++)
      {
        ...
        if (MYON_SubDVertexTag::Corner == vertex_tag_for_scope && MYON_SubDSectorType::UnsetCornerSectorAngle == angle_radians)
          angle_radians = MYON_SubDSectorType::AngleRadiansFromCornerAngleIndex(corner_sector_angle_index);

        MYON_SubDSectorType test_sector_type = MYON_SubDSectorType::Create( vertex_tag_for_scope, F, angle_radians);

        if (false == test_sector_type.SurfaceEvaluationCoefficientsAvailable())
          continue;
        const unsigned int N = test_sector_type.EdgeCount();
        double d = MYON_SubDMatrix::FromCache(test_sector_type).TestEvaluation();
        if (d >= 0.0)
        {
          pass_count++;
          if (d > max_d)
        ...
        if (nullptr != text_log)
        {
          MYON_String test_description;
          if (MYON_SubDVertexTag::Corner == vertex_tag_for_scope)
            test_description.Format("%s, %s, %u faces, %u edges, angle = %u/%u 2pi", sSubDTypeName, sVertexTagName, F, N, corner_sector_angle_index, MYON_SubDSectorType::MaximumCornerAngleIndex);
          else
...
double MYON_SubDMatrix::TestComponentRing(
  ...
  ) const
{
  ...
  const MYON_SubDVertexTag vertex_tag = m_sector_type.VertexTag();
  const unsigned int face_edge_count = m_sector_type.FacetEdgeCount();
  const bool bSubdivideFaces = (R == F+N+1 && 4 == face_edge_count);

  const MYON_SubDVertex* center_vertex = component_ring[0].Vertex();
  if (nullptr == center_vertex)
    return MYON_SUBD_RETURN_ERROR(rc_error);

  if (vertex_tag != center_vertex->m_vertex_tag || center_vertex->IsStandard())
    return MYON_UNSET_VALUE; // not an error - need a round of subdivision before matrix can be used

  MYON_SimpleArray<MYON_3dPoint> point_ring_array;
  if ( R != MYON_SubD::GetSectorPointRing(false,component_ring_count,component_ring,point_ring_array) )
    return MYON_SUBD_RETURN_ERROR(rc_error);
  const MYON_3dPoint* vertexR = point_ring_array.Array();
  ...
  {
    const MYON_SubDEdge* edge = component_ring[i].Edge();
    if (nullptr == edge)
      return MYON_SUBD_RETURN_ERROR(rc_error);
    ...
    i++;
    const MYON_SubDFace* face = component_ring[i].Face();
    if (nullptr == face)
      return MYON_SUBD_RETURN_ERROR(rc_error);
...
{
  const double rc_error = MYON_UNSET_VALUE;
  limit_point = MYON_SubDSectorSurfacePoint::Unset;

  component_ring.SetCount(0);
  subd_points.SetCount(0);

  const MYON_SubDSectorType sector_type = MYON_SubDSectorType::Create(sit);

  const unsigned int component_ring_count = MYON_SubD::GetSectorComponentRing(sit,component_ring);
  if ( component_ring_count < 4 || component_ring_count != sector_type.ComponentRingCount())
    return MYON_SUBD_RETURN_ERROR(rc_error);

  const unsigned int N = MYON_SubD::ComponentRingEdgeCount(component_ring_count);
  const unsigned int F = MYON_SubD::ComponentRingFaceCount(component_ring_count);
  ...
  MYON_3dPoint* point_ring = subd_points.Reserve(R*(subd_recursion_count+1));
  const MYON_3dPoint* point_ring0 = point_ring;
  const MYON_3dPoint* point_ringEnd = point_ring + R*(subd_recursion_count+1);

  unsigned int rc = MYON_SubD::GetSectorPointRing(
  ...
  const MYON_SubDVertex* center_vertex = sit.CenterVertex();
  const MYON_SubDFace* sector_face = sit.CurrentFace();

  if (subd_recursion_count > 0)
  {
    MYON_SubD_FixedSizeHeap fsh[2];
    const MYON_SubDComponentPtr* element = component_ring.Array();
    MYON_SimpleArray< MYON_SubDComponentPtr > component_ring_buffer;
    const MYON_SubDVertex* v0 = nullptr;
    for (unsigned int recursion_level = 0; recursion_level < subd_recursion_count; recursion_level++)
    {
      const MYON_SubDVertex* v1 = MYON_SubD::SubdivideSector( v0, element, component_ring_count, fsh[recursion_level%2]);
      ...
      {        
        if (point_ring0[0].x == v1->m_P[0] && point_ring0[0].y == v1->m_P[1] && point_ring0[0].z == v1->m_P[2])
        {
          ...
          v0 = v1;
          v1 = MYON_SubD::SubdivideSector( v0, element, component_ring_count, fsh[recursion_level % 2]);
          if (nullptr == v1 || N != v1->m_edge_count || F != v1->m_face_count)
            return MYON_SUBD_RETURN_ERROR(rc_error);
        }
      }
      ...
      {
        const MYON_SubDEdge* e = v1->Edge(i);
        if ( nullptr == e )
          return MYON_SUBD_RETURN_ERROR(rc_error);
        const MYON_SubDVertex* v = e->m_vertex[1];
        if ( nullptr == v )
          return MYON_SUBD_RETURN_ERROR(rc_error);
        subd_points.AppendNew() = v->m_P;
        if (i < F)
        {
          const MYON_SubDFace* f = v1->Face(i);
          if ( nullptr == f )
            return MYON_SUBD_RETURN_ERROR(rc_error);
          v = f->Vertex(2);
          if ( nullptr == v )
  ...
  const MYON_SubDMatrix& SM = MYON_SubDMatrix::FromCache(sector_type);
  ...
  {
    const MYON_3dPoint* point_ring1 = point_ring0;
    for (const MYON_3dPoint* point_ring2 = point_ring1 + R; point_ring2 < point_ringEnd; point_ring1 += R, point_ring2 += R)
    {
      for (unsigned int i = 0; i < R; i++)
      {
        MYON_3dPoint P(MYON_3dPoint::Origin);
  ...
  // calculate limit point using MYON_SubDVertex evaluation code
  MYON_SubDSectorSurfacePoint center_vertex_limit_point = MYON_SubDSectorSurfacePoint::Unset; 
  ...
  const MYON_3dPoint center_vertex_P(center_vertex_limit_point.m_limitP);
  const MYON_3dPoint matrix_P(limit_point.m_limitP);
  ...
  const MYON_3dVector center_vertex_N(center_vertex_limit_point.m_limitN);
  const MYON_3dVector matrix_N(limit_point.m_limitN);
...
//#define MYON_SUBD_JORDON_DECOMP_CODE
#if defined (MYON_SUBD_JORDON_DECOMP_CODE)

// This code is not required for MYON_SubD evaulation.
// It was of theoretical interest early in the project.

class MYON_NumberPolisher
{
public:
  MYON_NumberPolisher(unsigned int valence, unsigned int R, const double* const * S)
    : m_N(valence)
    , m_R(R)
    , m_S(S)
  {
    ...
    ) const
  {
    ...
    return MYON_NumberPolisher::PolishValue(m_polish_01_base, m_polish_01_tol, &polish, 1, v);
  }
  ...
  {
    return MYON_NumberPolisher::PolishValue(m_polish_base, m_polish_tol, m_polish, m_polish_count, v);
  }
...
double MYON_NumberPolisher::PolishValue(
  const double polish_base,
  const double polish_tol,
  const double* polish,
  const size_t polish_count,
...
bool MYON_NumberPolisher::PolishEigenvector(
  double lambda,
  double* eigenvector,
  double* eigenprecision
  ) const
{
  ...
  double eprecision = MYON_EigenvectorPrecision(m_R, m_S, m_bTransposeS, lambda, eigenvector);

  if (bRationalPolishApplied)
  {
    double Eprecision = MYON_EigenvectorPrecision(m_R, m_S, m_bTransposeS, lambda, m_E);
...
bool MYON_NumberPolisher::PolishGeneralizedEigenvector(
  double lambda,
  ...
  ) const
{
  ...
  double eprecision = MYON_MatrixSolutionPrecision(m_R, m_S, m_bTransposeS, lambda, generalized_eigenvector, xeigenvector);

  if (bRationalPolishApplied)
  {
    double Eprecision = MYON_MatrixSolutionPrecision(m_R, m_S, m_bTransposeS, lambda, m_E, xeigenvector);
    if (Eprecision <= eprecision)
    {
      for (unsigned int j = 0; j < m_R; j++)
      {
  ...
  )
{
  ...
  // The call to MYON_GetEigenvectors is still made to insure that
  // the MYON_GetEigenvectors() code is robust enough to calculate this
  // "easy" eigenvector.
  ...
  unsigned int ec = MYON_GetEigenvectors(R, S, bTransposeS, lambda, lambda_muliplicity, nullptr, eigenvectors, eigenprecision, eigenpivots);
  ...
  if (e1accuracy > 1e-4)
  {
    // MYON_GetEigenvectors returned junk on the easiest case.
    // We cannot expect anything good to come out for the
    // other eigenvalues and S is probably corrupt.
    return MYON_SUBD_RETURN_ERROR(false);
  }

  double e1precision = MYON_EigenvectorPrecision(R,S,bTransposeS,lambda,e1);
...
static bool MYON_SolveGetGeneralizedEigenvector(
...
{
  if (N < 1 || nullptr == S || !MYON_IsValid(lambda) || nullptr == eigenvector)
    return false;
  ...
  MYON_SimpleArray< double > _B(N);
  double* B = _B.Array();

  // Xdex records column permutations
  MYON_SimpleArray< unsigned int > _Xdex(N);
  ...
  MYON_SimpleArray< double > savedB;
  savedB.Append(N, B);
  ...
  if (nullptr != generalized_eigenprecision)
    *generalized_eigenprecision = MYON_MatrixSolutionPrecision(N, S, bTransposeS, lambda, generalized_eigenvector, eigenvector);
  
  return true;
}

static unsigned int MYON_GetGeneralizedEigenvectors(
  const MYON_NumberPolisher& polish,
  const unsigned int N,
  ...
  )
{
  ...
  if (!(lambda == lambda && MYON_IsValid(lambda)))
    return MYON_SUBD_RETURN_ERROR(0);
  ...
  // Get eigenvectors
  double epivots[3] = { 0 };
  unsigned int ec = MYON_GetEigenvectors(N, M, bTransposeM, lambda, lambda_multiplicity, nullptr, eigenvectors, eigenprecision, epivots);
  ...
  if (ec > 0 && ec <= lambda_multiplicity)
  {
    ...
    if (2 == lambda_multiplicity && 1 == ec)
    {
      ...
      double gpivots[3] = { 0 };
      double gep = 0.0;
      bool rc = MYON_SolveGetGeneralizedEigenvector(
        N,
        M,
        bTransposeM,
        lambda,
...
*/
unsigned int MYON_SubDMatrix_GetJordonDecomposition(
  const MYON_SubDMatrix& subd_matrix,
  ...
  )
{
  ...
  MYON_SimpleArray< double > _super_diagonal;
  ...
  if (nullptr != eigenvectors)
  {
    const MYON_NumberPolisher polish(R/2, R, S);

    MYON_SimpleArray<double> _eprecision;
    if (nullptr == eigenprecision)
    ...
    while (eigenvector_count < R)
    {
      ...
      double epivots[3] = { 0 };
      unsigned int ec = MYON_GetGeneralizedEigenvectors(polish, R, S, bTransposeS, lambda, lambda_multiplicity, eigenvectors + eigenvector_count, eigenprecision + eigenvector_count, epivots);
