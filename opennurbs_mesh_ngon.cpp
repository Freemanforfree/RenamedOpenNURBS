...
MYON_MeshNgonBuffer::MYON_MeshNgonBuffer()
{
  memset(m_ngon_buffer,0,sizeof(m_ngon_buffer));
}

const class MYON_MeshNgon* MYON_MeshNgonBuffer::Ngon() const
{
  return (const class MYON_MeshNgon*)m_ngon_buffer;
}
  
const class MYON_MeshNgon* MYON_MeshNgonBuffer::CreateFromMeshFaceIndex(
  const class MYON_Mesh* mesh,
  unsigned int face_index
  )
{
  const class MYON_MeshFace* mesh_face = ( mesh && face_index < mesh->m_F.UnsignedCount() )
                                     ? &mesh->m_F[face_index] 
                                     : 0;
...
const class MYON_MeshNgon* MYON_MeshNgonBuffer::CreateFromMeshFace(
...
const class MYON_MeshNgon* MYON_MeshNgonBuffer::CreateFromTriangle(
  ...
  )
{
  if ( 0 != triangle_vertex_indices )
  {
    ...
    return MYON_MeshNgon::NgonFromMeshFace(*this,face_index,quad_vertex_indices);
  }
  return MYON_MeshNgon::NgonFromMeshFace(*this,face_index,0);
}

const class MYON_MeshNgon* MYON_MeshNgonBuffer::CreateFromQuad(
  const unsigned int quad_vertex_indices[4],
  unsigned int face_index
  )
{
...
MYON__UINT32 MYON_MeshNgon::CRC32() const
{
  MYON__UINT32 crc = 0;
  if ( m_Vcount > 0 && m_vi )
    crc = MYON_CRC32(crc,m_Vcount*sizeof(m_vi[0]),m_vi);  
  if ( m_Fcount > 0 && m_fi )
    crc = MYON_CRC32(crc,m_Fcount*sizeof(m_fi[0]),m_fi);
  return crc;
}

MYON_SHA1_Hash MYON_MeshNgon::ContentHash() const
{
  MYON_SHA1 sha1;
  if ( m_Vcount > 0 && m_vi )
...
unsigned int MYON_MeshNgon::BoundaryEdgeCount(const MYON_MeshFaceList & mesh_face_list) const
{
  ...
  MYON_SimpleArray<MYON_2udex> edge_buffer(edge_capacity);
  edge_buffer.SetCount(edge_capacity);
  MYON_2udex* edges = edge_buffer.Array();
  ...
  MYON_qsort(edges, edge_count, sizeof(edges[0]), Internal_compare_ngon_edges);
...
unsigned int MYON_MeshNgon::OuterBoundaryEdgeCount() const
{
  return (m_Vcount >= 3 && nullptr != m_vi) ? m_Vcount : 0;
}

int MYON_MeshNgon::Orientation(
  const MYON_Mesh* mesh,
  bool bPermitHoles
) const
{
  const MYON_MeshFaceList face_list(mesh);
  return Orientation(face_list, bPermitHoles);
}


static int MYON_Internal_CompareNgonEdge(
  const void* lhs,
  const void* rhs
)
{
...
static int MYON_Internal_MeshNgon_NotOriented()
{
...
) const
{
  if (m_Fcount < 1 || nullptr == m_fi)
    return MYON_Internal_MeshNgon_NotOriented();
  ...
  MYON_SimpleArray<MYON_3udex> buffer;
  buffer.Reserve(a_capacity);
  MYON_3udex* edge = buffer.Array();
  ...
  for (unsigned int nfi = 0; nfi < m_Fcount; nfi++)
  {
    ...
    if (fi >= mesh_Fcount)
      return MYON_Internal_MeshNgon_NotOriented(); // invalid ngon m_fi[] value.
    ...
    for (unsigned int fvi = 0; fvi < fvi_count; fvi++)
    {
      ...
      e.j = face_vi[fvi];
      if (e.i == e.j)
        return MYON_Internal_MeshNgon_NotOriented(); // invalid face
      edge[edge_count++] = e;
    }
  }
  ...
  for (unsigned int nvi = 0; nvi < m_Vcount; nvi++)
  {
    ...
    e.j = m_vi[nvi];
    if (e.i == e.j)
      return MYON_Internal_MeshNgon_NotOriented(); // ngon m_vi[] has duplicates
    edge[edge_count++] = e;
  }
  ...
  MYON_qsort(edge, edge_count, sizeof(edge[0]), MYON_Internal_CompareNgonEdge);
  e.i = 0;
  e.j = 0;
  e.k = 0;
  int orientation = 0;
  ...
  {
    if (0 != edge[ei].k )
      return MYON_Internal_MeshNgon_NotOriented(); // invalid ngon boundary.

    e.k = 0;
    if (0 == MYON_Internal_CompareNgonEdge(&e, &edge[ei]))
    {
    ...
    if (0 == e.k)
    {
      ...
      const int c = MYON_Internal_CompareNgonEdge(&edge[ei], &e);
      if (c > 0)
      {
        MYON_ERROR("bug in this code");
        return MYON_Internal_MeshNgon_NotOriented();
      }
      if (c < 0)
      {
    ...
    else
    {
      ...
      if (1 == orientation)
      {
        ...
        if (e.i != edge[ei].i || e.j != edge[ei].j)
          return MYON_Internal_MeshNgon_NotOriented(); // ngon boundary orientation is not consistent or face set is not oriented.
      }
      ...
      else
      {
        ...
        if (e.i == edge[ei].i && e.j == edge[ei].j)
          ...
          return MYON_Internal_MeshNgon_NotOriented(); // the ngon boundary edge e is not an edge of a face
      }
    }
    ei++;
  }
...
unsigned int MYON_MeshNgon::BoundaryEdgeCount(const MYON_Mesh * mesh) const
{
  const MYON_MeshFaceList face_list(mesh);
  return BoundaryEdgeCount(face_list);
}

unsigned MYON_Mesh::NgonBoundaryEdgeCount(unsigned int ngon_index) const
{
  const MYON_MeshNgon* ngon = Ngon(ngon_index);
  return (nullptr == ngon) ? 0 : ngon->BoundaryEdgeCount(this);
}

unsigned int MYON_MeshNgon::Capacity() const
{
  if ( (0 != m_vi || 0 != m_fi) )
  {
...
static char* ToStringHelper( const MYON_MeshNgon* ngon, char* buffer, size_t sizeof_buffer)
{
...
MYON_String MYON_MeshNgon::ToString() const
{
  // supports this == null
  char buffer[256];
  return ( 0 == ToStringHelper(this,buffer,sizeof(buffer)) ) 
         ? MYON_String::EmptyString
         : MYON_String(buffer);
}

MYON_wString MYON_MeshNgon::ToWideString() const
{
  // supports this == null
  char buffer[256];
  return ( 0 == ToStringHelper(this,buffer,sizeof(buffer)) ) 
...
void MYON_MeshNgon::AppendToString( 
  class MYON_String& s
  )const
{
  // supports this == null
...
unsigned int MYON_Mesh::NgonUnsignedCount() const
{
  return m_Ngon.UnsignedCount();
}

int MYON_Mesh::NgonCount() const
{
  return m_Ngon.Count();
}

const MYON_MeshNgon* const * MYON_Mesh::Ngons() const
{
  return m_Ngon.Array();
}

const MYON_MeshNgon* MYON_Mesh::Ngon(
...
bool MYON_Mesh::IsValidNewNgonInformation(
  unsigned int Vcount, 
  ...
  ) const
{
  ...
  for ( i = 0; i < Fcount; i++ )
  {
    if ( ngon_fi[i] >= meshFcount )
      return false;
    if ( ngon_map && MYON_UNSET_UINT_INDEX != ngon_map[ngon_fi[i]] )
      return false;
  }
...
void MYON_Mesh::RemoveNgonMap()
{
  m_NgonMap.Destroy();
}

const unsigned int* MYON_Mesh::NgonMap() const
{
...
unsigned int MYON_Mesh::NgonIndexFromFaceIndex(
  unsigned int face_index
  ) const
{
  ...
  if (face_index < meshFcount && ngonCount > 0)
  {
    ...
    else
    {
      ...
      {
        const MYON_MeshNgon* ngon = m_Ngon[ni];
...
const unsigned int* MYON_Mesh::NgonMap(
...
bool MYON_Mesh::CreateNgonMap(MYON_SimpleArray<unsigned int>& map) const
{
  ...
  for ( fi = 0; fi < meshFcount; fi++ )
    ngon_map[fi] = MYON_UNSET_UINT_INDEX;
    
  const MYON_MeshNgon*const* ngons = m_Ngon.Array();
  for ( ngon_index = 0; ngon_index < ngon_count; ngon_index++ )
  {
    ...
    for ( j = 0; j < ngon->m_Fcount; j++ )
    {
      ...
      if (fi < meshFcount)
      {
        if (MYON_UNSET_UINT_INDEX == ngon_map[fi])
          ngon_map[fi] = ngon_index;
        else
        {
          MYON_ERROR("mesh face referenced more than one time by an ngon.");
        }
      }
    }
  }
...
bool MYON_Mesh::CreateNgonMap(unsigned int* map) const
{
  MYON_SimpleArray<unsigned int> new_map {};
...
const unsigned int* MYON_Mesh::CreateNgonMap()
{
...
bool MYON_Mesh::InsertNgon(
  unsigned int ngon_index,
  const MYON_MeshNgon* ngon
  )
{
  if ( ngon_index >= MYON_UNSET_UINT_INDEX )
    return false;
...
static bool Internal_ListContainNgon(
  unsigned int count,
  const int* fi,
  MYON_SimpleArray< const MYON_MeshNgon* >& tested_ngons, 
  const MYON_MeshNgon* ngon
)
{
  ...
  {
    if (nullptr == MYON_BinarySearchIntArray((int)ngon->m_fi[k], fi, count))
...
unsigned int MYON_Mesh::AddNgons(
...
)
{
  ...
  MYON_SimpleArray<int> fi_list(ci_count);
  ...
  for (int i = 0; i < ci_count; ++i)
  {
    ...
    case MYON_COMPONENT_INDEX::TYPE::mesh_ngon:
      ...
      {
        const MYON_MeshNgon* ngon = Ngon(ci.m_index);
        if (nullptr == ngon)
          break;
        for (unsigned j = 0; j < ngon->m_Fcount; ++j)
        {
  ...
  fi_list.QuickSortAndRemoveDuplicates(MYON_CompareIncreasing<int>);
  ...
  MYON_SimpleArray<bool> face_available_buffer(face_count);
  ...
  //
  MYON_SimpleArray<int> ngon_faces(fi_count);
  MYON_SimpleArray<unsigned int> ngon_partition(64);

  const unsigned int* ngon_map = (ngon_count > 0) ? NgonMap(true) : nullptr;

  MYON_SimpleArray< const MYON_MeshNgon* > old_ngons_inside_new_ngon(32);
  ...
  for (int i = 0; i < fi_count; ++i)
  {
    ...
    for (;;)
    {
      ...
      for (/*empty init*/; i0 < i1; ++i0)
      {
        ...
        const MYON_MeshTopologyFace& topf = top.m_topf[fi];
        ...
        for (int fei = 0; fei < fe_count; ++fei)
        {
          ...
          if (ei < 0 || ei >= edge_count)
            continue;
          const MYON_MeshTopologyEdge& tope = top.m_tope[ei];
    ...
    MYON_SortIntArray(MYON::sort_algorithm::quick_sort, new_ngon_fi, new_ngon_F_count);
    ...
    if (false == bSkipNewNgon && nullptr != ngon_map)
    {
      ...
      for (unsigned j = 0; j < new_ngon_F_count; ++j)
      {
        ...
        if (ngon_dex >= ngon_ucount)
          continue;
        const MYON_MeshNgon* existing_ngon = Ngon(ngon_dex);
  ...
  if (nullptr != ngon_map)
  {
    ...
    MYON_SimpleArray<bool> delete_old_ngon(ngon_count);
...
int MYON_Mesh::AddNgon(const MYON_SimpleArray<unsigned int>& ngon_fi)
{
  return AddNgon(ngon_fi.UnsignedCount(), ngon_fi.Array());
}

int MYON_Mesh::AddNgon(const MYON_SimpleArray<unsigned int>& ngon_fi, bool bPermitHoles)
{
  return AddNgon(ngon_fi.UnsignedCount(), ngon_fi.Array(), bPermitHoles);
}

int MYON_Mesh::AddNgon(
  unsigned int Fcount,
  const unsigned int* ngon_fi
)
{
...
{
  MYON_MeshVertexFaceMap* vertexFaceMap = nullptr;
  return AddNgon_Expert(Fcount, ngon_fi, bPermitHoles, vertexFaceMap);
}

int MYON_Mesh::AddNgon_Expert(
  unsigned int Fcount,
  const unsigned int* ngon_fi,
  bool bPermitHoles,
...
)
{
  ...
  const MYON_3dPointListRef mesh_vertex_list(this);
  ...
  if (bPermitHoles)
    vi_count = MYON_MeshNgon::FindNgonBoundary(
      mesh_vertex_list,
      mesh_face_list,
      vertexFaceMap,
      Fcount,
    ...
    vi_count = MYON_MeshNgon::FindNgonOuterBoundary(
      mesh_vertex_list,
      mesh_face_list,
      vertexFaceMap,
      Fcount,
  ...
  )
{
  ...
  for(;;)
  {
    ...
    MYON_MeshNgon* ngon = m_NgonAllocator.AllocateNgon(Vcount,Fcount);
    if ( 0 == ngon )
      break;
...
bool MYON_Mesh::OrientNgons(bool bPermitHoles)
{
  bool rc = true;
  for (;;)
  {
    ...
    MYON_MeshNgon** ngons = m_Ngon.Array();
    if (0 == ngons)
      break;

    const MYON_MeshFaceList face_list = MYON_MeshFaceList(this);
    for (unsigned int ni = 0; ni < ngon_count; ni++)
    {
      MYON_MeshNgon* ngon = ngons[ni];
      if (nullptr == ngon)
        continue;
...
void MYON_MeshNgon::ReverseOuterBoundary()
{
  if (m_Vcount < 3 || nullptr == m_vi)
    return;
  unsigned int vi;
...
void MYON_Mesh::FlipNgonOrientation()
{
  const unsigned int ngon_count = m_Ngon.UnsignedCount();
  if ( 0 == ngon_count )
    return;
...
unsigned int MYON_Mesh::RemoveNgons(
      ...
      if ( ngon_map )
      {
        for ( i = 0; i < ngon->m_Fcount; i++ )
        {
          ...
          if ( face_index < meshFcount && ngon_map[face_index] == ngon_index )
            ngon_map[face_index] = MYON_UNSET_UINT_INDEX;
        }
      }
    }
...
MYON_3dPoint MYON_Mesh::NgonCenter(unsigned int ngon_index) const
{
  return NgonCenter(ngon_index < m_Ngon.UnsignedCount() ? m_Ngon[ngon_index] : 0);
}

MYON_3dPoint MYON_Mesh::NgonCenter(const MYON_MeshNgon* ngon) const
{
  MYON_3dPoint center(MYON_3dPoint::UnsetPoint);
  if ( 0 != ngon && ngon->m_Vcount > 0 && 0 != ngon->m_vi )
  {
    MYON_3dPointListRef mesh_vertex_list;
...
MYON_BoundingBox MYON_Mesh::NgonBoundaryBoundingBox(unsigned int ngon_index) const
{
  return NgonBoundaryBoundingBox(ngon_index < m_Ngon.UnsignedCount() ? m_Ngon[ngon_index] : 0);
}

MYON_BoundingBox MYON_Mesh::NgonBoundaryBoundingBox(const MYON_MeshNgon* ngon) const
{
...
unsigned int MYON_MeshNgon::GetOuterBoundaryPoints(
...
unsigned int MYON_Mesh::GetNgonBoundaryPoints(
  const MYON_MeshNgon* ngon,
  bool bAppendStartPoint,
  MYON_SimpleArray<MYON_3dPoint>& ngon_boundary_points
  ) const
...
void MYON_Mesh::RemoveEmptyNgons()
{
  MYON_MeshNgon* ngon;
  MYON_MeshNgon** ngons = m_Ngon.Array();
...
void MYON_Mesh::RemoveAllNgons()
{
  SetNgonCount(0);
}

MYON_MeshNgon* MYON_Mesh::AllocateNgon(
  unsigned int Vcount, 
  unsigned int Fcount
  )
{
...
unsigned int MYON_Mesh::AddNgon(
  MYON_MeshNgon* ngon
  )
{
  int ngon_index = MYON_UNSET_UINT_INDEX;
...
struct MYON_MeshNgonLink
{
  struct MYON_MeshNgonLink* next;
  struct MYON_MeshNgonLink* prev;
};

static size_t SizeofNgon(unsigned int capacity)
{
  return sizeof(MYON_MeshNgon) + (capacity+1)*sizeof(unsigned int);
}

MYON_MeshNgonAllocator::MYON_MeshNgonAllocator() MYON_NOEXCEPT
  : m_31(0)
  , m_63(0)
  , m_active(0)
{}
...
MYON_MeshNgon* MYON_MeshNgonAllocator::CopyNgon(
    const MYON_MeshNgon* ngon
    )
{
  MYON_MeshNgon* ngon_copy = AllocateNgon(ngon->m_vi ? ngon->m_Vcount : 0,ngon->m_fi ? ngon->m_Fcount : 0);
  if ( ngon_copy )
  {
...
MYON_MeshNgon* MYON_MeshNgonAllocator::AllocateNgon(
  unsigned int Vcount,
  unsigned int Fcount
  )
{
  struct MYON_MeshNgonLink* link;
  MYON_MeshNgon* ngon;
  unsigned int* a;
  unsigned int capacity;
  ...
  if ( capacity <= 7 )
  {
    ...
    ngon = (MYON_MeshNgon*)m_7.AllocateElement();
  }
  else if ( capacity <= 15 )
  {
    capacity = 15;
    ...
    ngon = (MYON_MeshNgon*)m_15.AllocateElement();
  }
  else 
  {
    for(;;)
    {
      ...
      if ( capacity <= 31 )
      {
        ...
        if ( 0 != m_31 )
        {
          link = (struct MYON_MeshNgonLink*)m_31;
          m_31 = link->next;
          break;
        }
      }
      else if ( capacity <= 63 )
      {
        ...
        if ( 0 != m_63 )
        {
          link = (struct MYON_MeshNgonLink*)m_63;
          m_63 = link->next;
          break;
        }
      }
      ...
      link = (struct MYON_MeshNgonLink*)onmalloc(sizeof(*link) + SizeofNgon(capacity));
      break;
    }
    if ( 0 == link )
      return 0;
    ngon = (MYON_MeshNgon*)(link+1);
    link->next = (struct MYON_MeshNgonLink*)m_active;
    if ( link->next )
      link->next->prev = link;
    link->prev = 0;
    m_active = link;
...
MYON_MeshNgon* MYON_MeshNgonAllocator::ReallocateNgon(
  MYON_MeshNgon* ngon,
  unsigned int Vcount,
  unsigned int Fcount
  )
{
  ...
  MYON_MeshNgon* ngon1 = ( Vcount + Fcount <= capacity ) ? ngon : AllocateNgon(Vcount,Fcount);
  if ( 0 == ngon1 )
    return 0;
...
bool MYON_MeshNgonAllocator::DeallocateNgon(
  MYON_MeshNgon* ngon
  )
{
  struct MYON_MeshNgonLink* link;
  ...
  {
    link = ((struct MYON_MeshNgonLink*)ngon)-1;
    ...
    case 31:
      link->prev = 0;
      link->next = (struct MYON_MeshNgonLink*)m_31;
      m_31 = link;     
      break;
    case 63:
      link->prev = 0;
      link->next = (struct MYON_MeshNgonLink*)m_63;
      m_63 = link;   
      break;
    default:
      // deallocate heap
...
void MYON_MeshNgonAllocator::DeallocateAllNgons()
{
  struct MYON_MeshNgonLink* link;
  struct MYON_MeshNgonLink* next;
  struct MYON_MeshNgonLink* links[3] = {(struct MYON_MeshNgonLink*)m_31,(struct MYON_MeshNgonLink*)m_63,(struct MYON_MeshNgonLink*)m_active};
  m_7.Destroy();
  m_15.Destroy();
  m_31 = 0;
  m_63 = 0;
...
MYON_MeshNgonAllocator::MYON_MeshNgonAllocator(MYON_MeshNgonAllocator&& src)
  : m_31(src.m_31)
  , m_63(src.m_63)
...
MYON_MeshNgonAllocator& MYON_MeshNgonAllocator::operator=(MYON_MeshNgonAllocator&& src)
{
  if ( this != &src )
  {
    DeallocateAllNgons();
...
#define MYON_NGON_BOZO_VACCINE

// Do not copy or move the definition of
// the MYON_NGON_MEMBLK structure.
struct MYON_NGON_MEMBLK
{
#if !defined(MYON_NGON_BOZO_VACCINE)
#error You are a bozo!  Read the comments.
#endif
  struct MYON_NGON_MEMBLK* next;
};


class MYON_V4V5_MeshNgon
{
public:
  ...
  // If the MYON_MeshNgon is returned by the MYON_V4V5_MeshNgonList::AddNgon()
  // function, then the memory for vi is managed by the MYON_V4V5_MeshNgonList
  // class.  
  int* vi;
  ...
  // If the MYON_MeshNgon is returned by the MYON_V4V5_MeshNgonList::AddNgon()
  // function, then the memory for fi is managed by the MYON_V4V5_MeshNgonList
  // class.  
  int* fi;
};

class MYON_V4V5_MeshNgonList
{
public:
  MYON_V4V5_MeshNgonList();
  ~MYON_V4V5_MeshNgonList();
  MYON_V4V5_MeshNgonList(const MYON_V4V5_MeshNgonList&);
  MYON_V4V5_MeshNgonList& operator=(const MYON_V4V5_MeshNgonList&);
  ...
  */
  ...
  class MYON_V4V5_MeshNgon* V4V5_AddNgon(int N);
  ...
  */
  MYON_V4V5_MeshNgon* V4V5_Ngon(int Ngon_index) const;
...
private:
  ...
  MYON_V4V5_MeshNgon* m_ngons;
  struct MYON_NGON_MEMBLK* m_memblk_list;
};

MYON_V4V5_MeshNgonList::MYON_V4V5_MeshNgonList()
{
  m_ngons_count = 0;
  m_ngons_capacity = 0;
  m_ngons = 0;
...
void MYON_V4V5_MeshNgonList::V4V5_Destroy()
{
  m_ngons_count = 0;
  m_ngons_capacity = 0;
  if ( 0 != m_ngons )
  ...
  struct MYON_NGON_MEMBLK* p = m_memblk_list;
  m_memblk_list = 0;
  while(p)
  {
    struct MYON_NGON_MEMBLK* next = p->next;
    onfree(p);
    p = next;
  }
}
...
MYON_V4V5_MeshNgonList::MYON_V4V5_MeshNgonList(const MYON_V4V5_MeshNgonList& src)
{
  m_ngons_count = 0;
  m_ngons_capacity = 0;
  m_ngons = 0;
...
MYON_V4V5_MeshNgonList& MYON_V4V5_MeshNgonList::operator=(const MYON_V4V5_MeshNgonList& src)
{
  if ( this != &src )
  {
    V4V5_Destroy();
    ...
    {
      const MYON_V4V5_MeshNgon& ngon = src.m_ngons[i];
...
bool MYON_V4V5_MeshNgonList::V4V5_ReserveNgonCapacity(int capacity)
{
  bool rc = true;
  if ( capacity > m_ngons_capacity )
  {
    m_ngons = (MYON_V4V5_MeshNgon*)onrealloc(m_ngons,capacity*sizeof(m_ngons[0]));
    if ( 0 == m_ngons )
    {
      m_ngons_capacity = 0;
      m_ngons_count = 0;
...
class MYON_V4V5_MeshNgon* MYON_V4V5_MeshNgonList::V4V5_AddNgon(int N)
{
  if ( N < 3 || N > 100000 )
    return 0;
  ...
  MYON_V4V5_MeshNgon& ngon = m_ngons[m_ngons_count++];

  ngon.N = N;
  struct MYON_NGON_MEMBLK* blk = (struct MYON_NGON_MEMBLK*)onmalloc(sizeof(*blk) + (2*N)*sizeof(int));
  if ( 0 == blk )
    return 0;
  ngon.vi = (int*)(blk + 1);
  ngon.fi = ngon.vi + N;
...
bool MYON_V4V5_MeshNgonList::V4V5_AddNgon(int N, const int* vi, const int* fi)
{
  if ( 0 == vi || 0 == fi )
    return false;
  class MYON_V4V5_MeshNgon* ngon = V4V5_AddNgon(N);
  if ( 0 == ngon )
    return false;
...
int MYON_V4V5_MeshNgonList::V4V5_NgonCount() const
{
  return m_ngons_count;
}

MYON_V4V5_MeshNgon* MYON_V4V5_MeshNgonList::V4V5_Ngon(int Ngon_index) const
{
  return (Ngon_index < 0 || Ngon_index >= m_ngons_count) ? 0 : m_ngons+Ngon_index;
}


class /* DO NOT EXPORT THIS CLASS */ MYON_V4V5_MeshNgonUserData : public MYON_UserData
{
#if !defined(MYON_NGON_BOZO_VACCINE)
#error You are a bozo!  Read the comments.
#endif
  MYON_OBJECT_DECLARE(MYON_V4V5_MeshNgonUserData);

public:
  MYON_V4V5_MeshNgonUserData();
  ~MYON_V4V5_MeshNgonUserData();
  MYON_V4V5_MeshNgonUserData(const MYON_V4V5_MeshNgonUserData&);
  MYON_V4V5_MeshNgonUserData& operator=(const MYON_V4V5_MeshNgonUserData&);

  // vitual MYON_UserData override
...
public:
  MYON_V4V5_MeshNgonList* m_ngon_list;
...
MYON_OBJECT_IMPLEMENT(MYON_V4V5_MeshNgonUserData,MYON_UserData,"31F55AA3-71FB-49f5-A975-757584D937FF");

MYON_V4V5_MeshNgonUserData::MYON_V4V5_MeshNgonUserData()
{
  m_userdata_uuid = MYON_CLASS_ID(MYON_V4V5_MeshNgonUserData);
...
MYON_V4V5_MeshNgonUserData::MYON_V4V5_MeshNgonUserData(const MYON_V4V5_MeshNgonUserData& src) 
: MYON_UserData(src)
, m_mesh_F_count(src.m_mesh_F_count)
, m_mesh_V_count(src.m_mesh_V_count)
{
  m_ngon_list = (0 != src.m_ngon_list) 
              ? new MYON_V4V5_MeshNgonList(*src.m_ngon_list)
              : 0;
}

MYON_V4V5_MeshNgonUserData& MYON_V4V5_MeshNgonUserData::operator=(const MYON_V4V5_MeshNgonUserData& src)
{
  if ( this != &src )
  {
    if (0 != m_ngon_list )
    ...
    {
       m_ngon_list = new MYON_V4V5_MeshNgonList(*src.m_ngon_list);
    }
    m_mesh_F_count = src.m_mesh_F_count;
    m_mesh_V_count = src.m_mesh_V_count;
  }
...
bool MYON_V4V5_MeshNgonUserData::IsValid( MYON_TextLog* text_log ) const
{
  return true;
}

unsigned int MYON_V4V5_MeshNgonList::V4V5_SizeOf() const
{
...
unsigned int MYON_V4V5_MeshNgonUserData::SizeOf() const
{
...
bool MYON_V4V5_MeshNgonUserData::Write(MYON_BinaryArchive& archive) const
{
  ...
  for (;;)
  {
    ...
    const MYON_V4V5_MeshNgon* ngon_array = (count > 0) ? m_ngon_list->V4V5_Ngon(0) : 0;
    if ( 0 == ngon_array )
      count = 0;
    ...
    {
      const class MYON_V4V5_MeshNgon& ngon = ngon_array[i];
...
bool MYON_V4V5_MeshNgonUserData::Read(MYON_BinaryArchive& archive)
{
  if ( 0 != m_ngon_list )
  {
    delete m_ngon_list;
  ...
  for (;;)
  {
    ...
    m_ngon_list = new MYON_V4V5_MeshNgonList();
    if ( 0 == m_ngon_list )
      break;
    ...
    for ( int i = 0; i < count; i++ )
    {
      ...
      if ( N <= 0 )
        continue;
      class MYON_V4V5_MeshNgon* ngon = m_ngon_list->V4V5_AddNgon(N);
      if ( 0 == ngon )
        break;
...
// vitual MYON_UserData override
bool MYON_V4V5_MeshNgonUserData::GetDescription( MYON_wString& description )
{
  description = L"Mesh N-gon list";
  return true;
}

bool MYON_V4V5_MeshNgonUserData::Archive() const
{
  return ( 0 != m_ngon_list && m_ngon_list->V4V5_NgonCount() > 0 );
}

static
bool MYON_ValidateNgon(
  const MYON_V4V5_MeshNgon* ngon,
  int mesh_V_count,
  int mesh_F_count
  )
{
...
static 
bool MYON_ValidateMeshNgonUserData(
  MYON_V4V5_MeshNgonUserData* ngud,
  const MYON_Mesh& mesh
  )
{
  int i;
  ...
  if ( 0 == ngud->m_mesh_V_count && 0 == ngud->m_mesh_F_count )
  {
    ...
    {
      if ( !MYON_ValidateNgon(ngud->m_ngon_list->V4V5_Ngon(i),mesh_V_count,mesh_F_count) )
        return false;
    }
...
const class MYON_V4V5_MeshNgonList* MYON_Mesh::V4V5_NgonList() const
{
  MYON_UserData* ud = GetUserData(MYON_CLASS_ID(MYON_V4V5_MeshNgonUserData));
  MYON_V4V5_MeshNgonUserData* ngud = MYON_V4V5_MeshNgonUserData::Cast(ud);
  
  if ( 0 != ngud && !MYON_ValidateMeshNgonUserData(ngud,*this) )
  {
    delete ngud;
    ngud = 0;
  }
...
class MYON_V4V5_MeshNgonList* MYON_Mesh::V4V5_ModifyNgonList()
{
  MYON_UserData* ud = GetUserData(MYON_CLASS_ID(MYON_V4V5_MeshNgonUserData));
  MYON_V4V5_MeshNgonUserData* ngud = MYON_V4V5_MeshNgonUserData::Cast(ud);
  if ( 0 == ngud )
  {
    ...
    ngud = new MYON_V4V5_MeshNgonUserData();
    ngud->m_mesh_F_count = m_F.Count();
    ngud->m_mesh_V_count = m_V.Count();
    AttachUserData(ngud);
  }
  else if ( 0 != ngud->m_ngon_list && !MYON_ValidateMeshNgonUserData(ngud,*this) )
  {
  ...
  {
    ngud->m_ngon_list = new MYON_V4V5_MeshNgonList();
    ngud->m_mesh_F_count = m_F.Count();
    ngud->m_mesh_V_count = m_V.Count();
  }
...
void MYON_Mesh::V4V5_DestroyNgonList()
{
  MYON_UserData* ud = GetUserData(MYON_CLASS_ID(MYON_V4V5_MeshNgonUserData));
  if ( 0 != ud )
  {
...
static void InitializeVertexFaceMap(
  MYON__UINT_PTR* vertex_face_map,
  unsigned int i0,
  unsigned int i1,
  unsigned int* a
  )
{
  if ( i1 > i0 )
  {
    MYON__UINT_PTR vertex_face_count;
    vertex_face_map += i0;
    //i1 -= i0;
    for ( i1 -= i0; i1 > 0; i1-- )
    {
      ...
      {
        a[0] = 0;
        vertex_face_map[0] = (MYON__UINT_PTR)a;
...
const MYON_MeshFaceList MYON_MeshFaceList::EmptyFaceList;

MYON_MeshFaceList::MYON_MeshFaceList(
  const MYON_Mesh* mesh
  )
{
  SetFromMesh(mesh);
...
unsigned int MYON_MeshFaceList::SetFromTriangleList(
...
unsigned int MYON_MeshFaceList::SetFromQuadList(
  size_t quad_count,
  size_t quad_stride,
...
unsigned int MYON_MeshFaceList::SetFromMesh(const MYON_Mesh* mesh)
{
  if ( 0 != mesh )
  {
...
unsigned int MYON_MeshFaceList::GetVertexIndexInterval(
...
struct MYON_MeshVertexFaceMap_BLK
{
  struct MYON_MeshVertexFaceMap_BLK* m_next;
  static const size_t m_target_block_size;
};

// Four pages less a bit for overhead
const size_t MYON_MeshVertexFaceMap_BLK::m_target_block_size(4096*4 - sizeof(MYON_MeshVertexFaceMap_BLK) - 32);


void* MYON_MeshVertexFaceMap::m_alloc(size_t sz)
{
  ...
  // struct MYON_MeshVertexFaceMap_BLK changes in order to 
  // make certain alignment constraints are met on all
  // "reasonable" CPUs.

  struct MYON_MeshVertexFaceMap_BLK* blk;

  if ( sz <= 0 )
    return 0;

  if (sz % sizeof(struct MYON_MeshVertexFaceMap_BLK)) 
    sz += sizeof(struct MYON_MeshVertexFaceMap_BLK);
  
  blk = new (std::nothrow) MYON_MeshVertexFaceMap_BLK[1 + sz/sizeof(struct MYON_MeshVertexFaceMap_BLK)];
  
  if ( 0 == blk )
    return 0;

  blk->m_next = (struct MYON_MeshVertexFaceMap_BLK*)m_p;
  m_p = blk;
  return (blk+1);
}

void MYON_MeshVertexFaceMap::Destroy()
{
  MYON_MeshVertexFaceMap_BLK* blk;
  MYON_MeshVertexFaceMap_BLK* next;

  blk = (MYON_MeshVertexFaceMap_BLK*)m_p;
...
void MYON_MeshVertexFaceMap::m_copy(const MYON_MeshVertexFaceMap& src)
{
  const size_t blk_sz = MYON_MeshVertexFaceMap_BLK::m_target_block_size;
  size_t sz, delta_sz;
...
MYON_MeshVertexFaceMap::~MYON_MeshVertexFaceMap()
{
  Destroy();
}

MYON_MeshVertexFaceMap::MYON_MeshVertexFaceMap(const MYON_MeshVertexFaceMap& src)
  : m_vertex_count(0)
  , m_face_count(0)
  , m_vertex_face_map(0)
  , m_p(0)
...
MYON_MeshVertexFaceMap& MYON_MeshVertexFaceMap::operator=(const MYON_MeshVertexFaceMap& src)
{
  if ( this != &src )
  {
    Destroy();
...
#if defined(MYON_HAS_RVALUEREF)
MYON_MeshVertexFaceMap::MYON_MeshVertexFaceMap( MYON_MeshVertexFaceMap&& src) MYON_NOEXCEPT
...
MYON_MeshVertexFaceMap& MYON_MeshVertexFaceMap::operator=( MYON_MeshVertexFaceMap&& src) MYON_NOEXCEPT
{
  if ( this != &src )
  {
    Destroy();
...
bool MYON_MeshVertexFaceMap::SetFromMesh(
  const MYON_Mesh* mesh,
  bool bMapInvalidFaces
  )
{
  MYON_MeshFaceList face_list;
...
bool MYON_MeshVertexFaceMap::SetFromFaceList(
  unsigned int vertex_count,
  const MYON_MeshFaceList& face_list,
  bool bMapInvalidFaces
  )
{
  size_t sz, delta_sz;
  unsigned int* a;
  MYON__UINT_PTR* vertex_face_map = 0;
  ...
  for(;;)
  {
    ...
    vertex_face_map = (MYON__UINT_PTR*)m_alloc(sz);
    ...
    if ( sz*sizeof(a[0]) <= 4*MYON_MeshVertexFaceMap_BLK::m_target_block_size )
    {
    ...
    else
    {
      ...
      for ( j = 0; j < vertex_count; j++ )
      {
        if ( vertex_face_map[j] > 0 )
        {
          ...
          if ( sz + delta_sz > MYON_MeshVertexFaceMap_BLK::m_target_block_size )
          {
            a = (unsigned int*)m_alloc(sz);
            if ( 0 == a )
              return 0;
...
unsigned int MYON_MeshVertexFaceMap::VertexCount() const
{
  return m_vertex_count;
}

unsigned int MYON_MeshVertexFaceMap::FaceCount() const
{
  return m_face_count;
}

unsigned int MYON_MeshVertexFaceMap::VertexFaceCount(
...
const unsigned int* MYON_MeshVertexFaceMap::VertexFaceList(
  unsigned int vertex_index
  ) const
{
  const unsigned int* vf_list = (vertex_index < m_vertex_count) ? m_vertex_face_map[vertex_index] : 0;
...
const unsigned int *const* MYON_MeshVertexFaceMap::VertexFaceMap() const
{
...
static bool FaceInPlane(
  ...
  const MYON_3dPointListRef& vertex_list,
...
{
  // The  P = (MYON_3dPoint*)buffer; technique is used to
  ...
  const MYON_3dVector A(P[2].x-P[0].x,P[2].y-P[0].y,P[2].z-P[0].z);
  const MYON_3dVector B(P[3].x-P[1].x,P[3].y-P[1].y,P[3].z-P[1].z);
  MYON_3dVector N(A.y*B.z - B.y*A.z, A.z*B.x - B.z*A.x, A.x*B.y - B.x*A.y );
  double d = N.Length();
  if ( !(d > 0.0) )
    return false;
  N.x /= d;
...
/*
Returns:
  index into the MYON_MeshNgon.m_fi[] array.
*/
static MYON__UINT_PTR DecodeNFS_NgonFaceIndex( MYON__UINT_PTR nfs )
{
  return (nfs/8);
}

static unsigned int DecodeNFS_EdgeIndex( MYON__UINT_PTR nfs )
{
...
Parameters:
  ngon_face_index - [in]
    MYON_MeshNgon.m_fi[] array index
  face_edge_index - [in]
   0 to 3
   the edge runs from 
     vertex index MYON_MeshFace.vi[face_edge_index] 
   to 
     vertex index MYON_MeshFace.vi[(face_edge_index+1)%4].
    
Returns:
  index into the MYON_MeshNgon.m_fi[] array.
*/
static MYON__UINT_PTR EncodeNFS_Neighbor(MYON__UINT_PTR ngon_face_index, MYON__UINT_PTR face_edge_index)
{
...
static MYON__UINT_PTR EncodeNFS_BoundaryIndex(MYON__UINT_PTR boundary_index)
{
  // 0 = (boundary_mark & 0x4)
  // 0 != boundary_mark
...
static bool IsNFS_Neighbor(MYON__UINT_PTR nfs)
{
  return ( 0 != (nfs&0x4) );
}
...
struct NgonNeighbors
{
  ...
  // DecodeNFS_NgonFaceIndex(m_NFS[side_index]) = index into the MYON_MeshNgon.m_fi[] array
  // DecodeNFS_EdgeIndex(m_NFS[side_index]) = 0,1,2 or 3 identifies the edge
  MYON__UINT_PTR m_NFS[4];
};
...
static unsigned int SetFaceNeighborMap(
  ...
  MYON_MeshVertexFaceMap* vertex_face_map_obj,
  unsigned int face_index_count,
  const unsigned int* face_index_list,
  ...
  )
{
  ...
  MYON_MeshVertexFaceMap vf_tmp;
  MYON_MeshVertexFaceMap* vf_map = &vf_tmp;
...
static int GetCoplanarConnectedFaces(
  ...
  MYON_PlaneEquation e,
  MYON_SimpleArray<unsigned int>& ngon_fi,
  MYON_SimpleArray<struct NgonNeighbors>& ngon_nbr_map
  )
{
  MYON__UINT_PTR face_side, nbr_face_side;
      ...
      for ( face_side = 0; face_side < 4; face_side++ )
      {
        ...
        for ( Adex = 1; Adex <= face_listA[0]; Adex++ )
        {
          ...
          if ( face_index == nbr_face_index || nbr_face_index <= starting_face_index)
            continue;
          if ( MYON_UNSET_UINT_INDEX != ngonMap[nbr_face_index] && ngonIndex != ngonMap[nbr_face_index])
            continue; // MYON_Mesh.m_F[nbr_face_index] belongs to another n-gon
          ...
          for ( nbr_face_side = 0; nbr_face_side < 4; nbr_face_side++ )
          {
            ...
            if ( MYON_UNSET_UINT_INDEX == ngonMap[nbr_face_index] )
            {
              // add MYON_Mesh.m_F[nbr_face_index] to this ngon
            ...
            {
              // If things are as expected, then MYON_Mesh.m_F[nbr_face_index]
              // face was been added to the ngon after the face we are working
              // on ( MYON_Mesh.m_F[ngon_fi[ngon_fi_dex0]] ).
...
Parameters:
  ...
  ngon_side - [out]
    ngon_side[i]/8 = MYON_MeshNgon.m_fi[] index
...
static unsigned int GetNgonBoundarySegments(
  ...
  MYON_SimpleArray<unsigned int>* ngon_vi,
  MYON_SimpleArray<unsigned int>* ngon_side
  )
{
...
class MYON_MeshNgon* MYON_MeshNgon::NgonFromMeshFace(
  ...
  )
{
  ...
  // was in use?
  if ( MYON_IsNullPtr(&ngon_buffer) || 0 == fvi )
    return 0;
  ...
  unsigned int Vcount = (fvi[3] < MYON_UNSET_UINT_INDEX && fvi[3] != fvi[2]) ? 4 : 3;

  MYON_MeshNgon* ngon = (MYON_MeshNgon*)(&ngon_buffer);
...
class MYON_MeshNgon** MYON_MeshNgon::NgonListFromMeshFace(
  class MYON_MeshNgonBuffer& ngon_buffer,
  unsigned int mesh_face_index,
  const unsigned int* fvi
  )
{
  MYON_MeshNgon* ngon = NgonFromMeshFace(ngon_buffer,mesh_face_index,fvi);
  if ( 0 == ngon )
    return 0;

  MYON_MeshNgon** ngon_list = (MYON_MeshNgon**)(ngon+1);
  ngon_list[0] = ngon;

  return ngon_list;
}

const MYON_MeshNgon* MYON_Mesh::NgonFromComponentIndex(
  class MYON_MeshNgonBuffer& ngon_buffer,
  MYON_COMPONENT_INDEX ci
  ) const
{
  const MYON_MeshNgon* ngon = 0;
  switch( ci.m_type )
  {
  case MYON_COMPONENT_INDEX::mesh_face:
    if ( ci.m_index >= 0 && ci.m_index < m_F.Count() )
    {
      ngon = MYON_MeshNgon::NgonFromMeshFace(ngon_buffer,(unsigned int)ci.m_index,(const unsigned int*)m_F[ci.m_index].vi);
    }
    break;
...
static unsigned int FindNgonBoundary_Helper(
  const MYON_3dPointListRef& mesh_vertex_list,
  const MYON_MeshFaceList& mesh_face_list,
  const unsigned int *const* vertex_face_map,
  MYON_MeshVertexFaceMap* vertex_face_map_obj,
...
)
{
  ...
  for (;;)
  {
    if (mesh_vertex_count <= 0 || MYON_UNSET_UINT_INDEX == mesh_vertex_count)
      break;
...
double Internal_NgonBoundaryArea(
  const MYON_Plane& ngon_plane,
  const MYON_3dPointListRef& vertex_list,
  const MYON_SimpleArray< unsigned int >& ngon_vi
)
{
  MYON_2dPoint p0, p1;
...
)
{
  ...
  for (;;)
  {
    ...
    {
      MYON_SimpleArray<unsigned int> tmp;
      // ngon has holes.
      // The boundary with the largest area is the outer boundary.
      MYON_3dVector A(mesh_vertex_list[ngon_vi[1]] - mesh_vertex_list[ngon_vi[0]]);
      MYON_3dVector B(mesh_vertex_list[ngon_vi[ngon_vi.Count()-1]] - mesh_vertex_list[ngon_vi[0]]);
      A.Unitize(); B.Unitize();
      const MYON_Plane ngon_plane(mesh_vertex_list[ngon_vi[0]], MYON_CrossProduct(A, B));

      double ngon_vi_area = Internal_NgonBoundaryArea(ngon_plane, mesh_vertex_list, ngon_vi);

      for (;;)
      {
        MYON_SimpleArray<unsigned int> ngon_vi1;
        ngon_boundary_index++;
...
unsigned int MYON_MeshNgon::FindNgonOuterBoundary(
  const MYON_3dPointListRef& mesh_vertex_list,
  const MYON_MeshFaceList& mesh_face_list,
  const unsigned int *const* vertex_face_map,
  size_t ngon_fi_count,
...
unsigned int MYON_MeshNgon::FindNgonBoundaries(
  const class MYON_3dPointListRef& mesh_vertex_list,
...
unsigned int MYON_MeshNgon::FindNgonBoundary(
  const MYON_3dPointListRef& mesh_vertex_list,
  const MYON_MeshFaceList& mesh_face_list,
  const unsigned int *const* vertex_face_map,
  size_t ngon_fi_count,
...
unsigned int MYON_MeshNgon::GetBoundarySides(
  const class MYON_MeshFaceList& mesh_face_list,
...
unsigned int MYON_Mesh::GetNgonOuterBoundary(
    unsigned int ngon_fi_count,
    const unsigned int* ngon_fi,
    MYON_SimpleArray<unsigned int>& ngon_vi
    ) const
{
  MYON_3dPointListRef mesh_vertex_list;
  MYON_MeshFaceList mesh_face_list;
  mesh_vertex_list.SetFromMesh(this);
  mesh_face_list.SetFromMesh(this);
  return MYON_MeshNgon::FindNgonOuterBoundary(mesh_vertex_list,mesh_face_list,(MYON_MeshVertexFaceMap*)nullptr,ngon_fi_count,ngon_fi,ngon_vi);
}

unsigned int MYON_MeshNgon::FindPlanarNgons(
  const MYON_3dPointListRef& vertex_list,
  const MYON_MeshFaceList& face_list,
  const unsigned int *const* vertex_face_map,
  double planar_tolerance,
  ...
  bool bAllowHoles,
  MYON_MeshNgonAllocator& NgonAllocator,
  ...
  )
{
  ...
  MYON_MeshVertexFaceMap ws_vfmap;
  ...
  for (;;)
  {
    ...
    if ( face_count != NgonMap.UnsignedCount() )
    {
      ...
      for ( face_index = 0; face_index < face_count; face_index++ )
        ngonMap[face_index] = MYON_UNSET_UINT_INDEX;
    }

    int ngonIndex = NgonsCount0;
    unsigned int ngonUnsetIndex;
    const unsigned int omitted_face_mark = MYON_UNSET_UINT_INDEX - 1;

    for ( face_index = 0; face_index < face_count; face_index++ )
    {
      if ( MYON_UNSET_UINT_INDEX != ngonMap[face_index] )
      ...
      {
        ngon = 0;
        ngonUnsetIndex = MYON_UNSET_UINT_INDEX;
        ...
        if ( ngon_vi.UnsignedCount() < ngon_boundary_edge_count )
        {
          // ngon has holes.
          // The boundary with the largest area is the outer boundary.
          const MYON_Plane ngon_plane(vertex_list[ngon_vi[0]], e.UnitNormal());
...
unsigned int MYON_Mesh::AddPlanarNgons(
  const unsigned int *const* vertex_face_map,
  double planar_tolerance,
  unsigned int minimum_ngon_vertex_count,
  unsigned int minimum_ngon_face_count,
  ...
  )
{
  ...
  const MYON_MeshFaceList face_list(this);

  unsigned int added_ngon_count = MYON_MeshNgon::FindPlanarNgons(
    vertex_list,
    face_list,
    vertex_face_map,
    planar_tolerance,
...
class /* DO NOT EXPORT THIS CLASS OR PUT IT IN A HEADER FILE */ MYON_MeshSeparateNgonInfo
{
  // This class is used to pass around information needed
  // by the functions used by MYON_Mesh::SeparateNgons()
  // to duplicated shared vertices.
public:
  MYON_MeshSeparateNgonInfo(
    MYON_Mesh* mesh,
...
public:
    ...
    )
  {
    ...
    if ( 0 == m_virefpool.SizeofElement() )
      m_virefpool.Create(sizeof(MYON_VertexIndexRef),0,0);
    MYON_VertexIndexRef* viref = (MYON_VertexIndexRef*)m_virefpool.AllocateElement();
    if ( 0 == viref )
      return false;
  ...
  unsigned int m_ni = MYON_UNSET_UINT_INDEX;
  unsigned int m_fi = MYON_UNSET_UINT_INDEX;
  unsigned int m_vi= MYON_UNSET_UINT_INDEX;
  unsigned int m_ngon_vmark = 0;
  const MYON_MeshNgon* m_ngon = nullptr;

private:
  MYON_SimpleArray<unsigned int> m_ngon_vmap_buffer;
  MYON_MeshVertexFaceMap m_local_vfmap;
  MYON_FixedSizePool m_virefpool;
};

void MYON_MeshSeparateNgonInfo::DuplicateSharedNgonVertices()
{
  unsigned int vertex_count = m_mesh->VertexUnsignedCount();
  MYON_FixedSizePoolIterator fit(m_virefpool);
  for ( MYON_VertexIndexRef* vr = (MYON_VertexIndexRef*)fit.FirstElement(); 
        0 != vr; 
        vr = (MYON_VertexIndexRef*)fit.NextElement() 
      )
  {
...
bool MYON_MeshSeparateNgonInfo::TestNgonVertex(
  const unsigned int* vertex_index_ptr
  )
{
  if ( 0 == vertex_index_ptr )
...
bool MYON_MeshSeparateNgonInfo::FindSharedNgonVertices(
  unsigned int ngon_index
  )
{
  m_ni = ngon_index;
  m_fi = MYON_UNSET_UINT_INDEX;
  m_vi = MYON_UNSET_UINT_INDEX;
  m_ngon_vmark = 0;
  ...
  )
{
  ...
  MYON_MeshSeparateNgonInfo X(this,vertex_face_map);
  if ( false == X.IsValid() )
    return false;
...
bool MYON_Mesh::SetNgonVertexNormals(
  unsigned int ngon_index0,
  unsigned int ngon_index1
  )
{
  ...
  MYON_3dPointListRef vertex_list(this);
  MYON_MeshFaceList face_list(this);
...
bool MYON_Mesh::RemoveNgonInteriorVertices(
  const unsigned int *const* vertex_face_map,
  unsigned int ngon_index0,
  unsigned int ngon_index1
  )
...
unsigned int MYON_MeshTopology::NgonIndexFromTopologyVertexIndex(
  unsigned int topvi,
  ...
  ) const
{
  if ( topvi >= m_topv.UnsignedCount() )
    return MYON_UNSET_UINT_INDEX;
  const MYON_MeshTopologyVertex& v = m_topv[topvi];
  if ( v.m_tope_count <= 2 || 0 == v.m_topei )
    return MYON_UNSET_UINT_INDEX;

  const MYON_Mesh* mesh = m_mesh;
  ...
  if ( MYON_UNSET_UINT_INDEX == ngon_index )
    return MYON_UNSET_UINT_INDEX;
...
unsigned int MYON_MeshTopology::NgonIndexFromTopologyEdgeIndex(
  unsigned int topei,
  const unsigned int* mesh_facedex_to_ngondex_map
  ) const
{
  if ( topei >= m_tope.UnsignedCount() )
    return MYON_UNSET_UINT_INDEX;
  const MYON_MeshTopologyEdge& e = m_tope[topei];
  ...
  const MYON_MeshNgon* ngon = 0;
  unsigned int ngon_index = MYON_UNSET_UINT_INDEX;
  unsigned int rc = MYON_UNSET_UINT_INDEX;
  for ( int tefi = 0; tefi < e.m_topf_count; tefi++ )
  {
    ...
    if ( 0 != ngon || MYON_UNSET_UINT_INDEX == mesh_facedex_to_ngondex_map[fi] )
    {
...
MYON_Plane MYON_Plane::FromPointList(
  ...
  )
{
  ...
  return MYON_Plane::FromPointList(vertex_list);
}
...
{
  return MYON_Plane::FromPointList(point_list.UnsignedCount(),point_list.Array());
}
...
{
  return MYON_Plane::FromPointList(point_list.PointCount(),0,point_list);
}
  ...
  )
{
  ...
  return MYON_Plane::FromPointList(
    point_index_count,
    point_index_stride,
    point_index_list,
    point_list
...
{
  unsigned int i;
  MYON_3dPoint Pi, Pi0, Pi1;

  //////////////////////////////////////////////////////////
  //
  // Do not tolerate garbage.
  //
  if ( point_index_count < 3 || point_index_count >= (size_t)MYON_UNSET_UINT_INDEX )
    return MYON_Plane::UnsetPlane;

  if ( 0 != point_index_list )
  {
    if ( point_index_stride < 1 || point_index_stride >= (size_t)MYON_UNSET_UINT_INDEX )
      return MYON_Plane::UnsetPlane;
  }
  ...
  //
  MYON_3dVector N(MYON_3dVector::ZeroVector);
  MYON_3dVector X(MYON_3dVector::UnsetVector);
  ...
  if (N == MYON_3dVector::ZeroVector)
  {
    // find far apart points
    unsigned int i0 = 0;
    unsigned int i1 = point_stride;
    MYON_Line L(point_list[point_index_list ? point_index_list[i0] : i0],point_list[point_index_list ? point_index_list[i1] : i1]);
    double d = L.Length();
    ...
    d = 0.0;
    unsigned int i2 = MYON_UNSET_UINT_INDEX;
    ...
    for ( i = 0; i < point_count*point_stride; i += point_stride )
    {
      ...
      if ( !L.ClosestPointTo(Pi,&t1) || !(t1 != MYON_UNSET_VALUE) )
        continue;    
      MYON_3dPoint Q = L.PointAt(t1);
    ...
    if ( MYON_UNSET_UINT_INDEX == i2 || i0 == i2 || i1 == i2 || !(d > 0.0) || !P.IsValid() )
      return MYON_Plane::UnsetPlane;
    Pi = point_list[point_index_list ? point_index_list[i2] : i2];
    N = MYON_CrossProduct(X,Pi-P);
  }
  ...
  if ( i < 3 )
  {
    MYON_3dVector a[3] = {MYON_3dVector::XAxis,MYON_3dVector::YAxis,MYON_3dVector::ZAxis};
    plane.zaxis = a[i];
    plane.xaxis = a[(i+1)%3];
    plane.yaxis = a[(i+2)%3];
  }
...
const MYON_MeshNgonIterator MYON_MeshNgonIterator::EmptyMeshNgonIterator;

MYON_MeshNgonIterator::MYON_MeshNgonIterator(
  const class MYON_Mesh* mesh
  )
{
  if ( 0 != mesh )
...
{
  if (    MYON_COMPONENT_INDEX::mesh_face == m_current_ngon_ci.m_type 
       || MYON_COMPONENT_INDEX::mesh_ngon == m_current_ngon_ci.m_type 
     )
  {
    if ( 0 != m_mesh )
      m_current_ngon = (MYON__UINT_PTR)(m_mesh->NgonFromComponentIndex(m_ngon_buffer,m_current_ngon_ci));
  }
}

MYON_MeshNgonIterator& MYON_MeshNgonIterator::operator=(
  const MYON_MeshNgonIterator& src
  )
{
  if ( this != &src )
...
void MYON_MeshNgonIterator::SetMesh(
  const class MYON_Mesh* mesh,
  const unsigned int* meshfdex_to_meshngondex_map
  )
{
  *this = MYON_MeshNgonIterator::EmptyMeshNgonIterator;
  m_mesh = mesh;
  ...
  if ( 0 != m_mesh )
  {
    ...
    m_current_ngon_ci.Set(MYON_COMPONENT_INDEX::invalid_type,1);
  }
}

const MYON_Mesh* MYON_MeshNgonIterator::Mesh() const
{
  return m_mesh;
}

const class MYON_MeshNgon* MYON_MeshNgonIterator::FirstNgon()
{
  m_current_ngon_ci.Set(MYON_COMPONENT_INDEX::invalid_type,1);
  m_current_ngon = 0;
  m_iterator_index = 0;
...
const class MYON_MeshNgon* MYON_MeshNgonIterator::NextNgon()
{
  unsigned int fi, ni;
  ...
  case MYON_COMPONENT_INDEX::invalid_type:
    ...
    if ( 1 != m_current_ngon_ci.m_index )
      break;
    m_current_ngon_ci.Set(MYON_COMPONENT_INDEX::mesh_ngon,-1);
    m_iterator_index = MYON_UNSET_UINT_INDEX;
  ...
  case MYON_COMPONENT_INDEX::mesh_ngon:
        ...
        )
    {
      ...
      if ( 0 == ngon )
        continue;
      if ( MYON_UNSET_UINT_INDEX == m_iterator_index )
        m_iterator_index = 0;
      else
        m_iterator_index++;
      m_current_ngon_ci.m_index = (int)ni;
      m_current_ngon = (MYON__UINT_PTR)ngon;
      return ngon;
    }
    m_current_ngon_ci.Set(MYON_COMPONENT_INDEX::mesh_face,-1);
  ...
  case MYON_COMPONENT_INDEX::mesh_face:
    ...
    {
      ni = m_facedex_to_ngondex_map ? m_facedex_to_ngondex_map[fi] : MYON_UNSET_UINT_INDEX;
      if ( ni < m_mesh_ngon_count )
        continue;
      ngon = MYON_MeshNgon::NgonFromMeshFace(m_ngon_buffer,fi,(const unsigned int*)m_mesh->m_F[fi].vi);
      if ( 0 == ngon )
        continue;
      if ( MYON_UNSET_UINT_INDEX == m_iterator_index )
        m_iterator_index = 0;
  ...
  default:
    m_current_ngon_ci.Set(MYON_COMPONENT_INDEX::invalid_type,0);
    m_current_ngon = 0;
  }
...
const class MYON_MeshNgon* MYON_MeshNgonIterator::CurrentNgon()
{
  const class MYON_MeshNgon* ngon = 0;
  if ( 0 != m_current_ngon )
  {
    ngon = (0 != m_mesh)
         ...
         : 0;
    if ( m_current_ngon != (MYON__UINT_PTR)ngon )
    {
      ngon = 0;
      m_current_ngon = 0;
    }
...
MYON_COMPONENT_INDEX MYON_MeshNgonIterator::CurrentNgonComponentIndex() const
{
  return m_current_ngon_ci;
}

bool MYON_MeshNgonIterator::CurrentNgonIsMeshFace() const
{
  return (    MYON_COMPONENT_INDEX::mesh_face == m_current_ngon_ci.m_type 
           && m_current_ngon == (MYON__UINT_PTR)&m_ngon_buffer 
         );
}

bool MYON_MeshNgonIterator::CurrentNgonIsMeshNgon() const
{
  return (   0 != m_current_ngon 
          && 0 != m_mesh 
          && MYON_COMPONENT_INDEX::mesh_ngon == m_current_ngon_ci.m_type
          && m_current_ngon == (MYON__UINT_PTR)m_mesh->Ngon(m_current_ngon_ci.m_index)
          );
}
  
void MYON_MeshNgonIterator::Reset()
{
  const MYON_Mesh* mesh = m_mesh;
...
unsigned int MYON_MeshNgonIterator::Count() const
{
  ...
  if (nullptr != m_mesh)
  {
    ...
    if (explicit_ngon_count > 0)
    {
      ...
      const MYON_MeshNgon* const * ngons = m_mesh->Ngons();
      if (nullptr != ngons)
      {
        for (unsigned int ni = 0; ni < explicit_ngon_count; ni++)
        {
          const MYON_MeshNgon* ngon = ngons[ni];
          if (nullptr == ngon)
            explicit_ngon_count--;
          ...
          {
            MYON_ERROR("Invalid ngon information on mesh");
            return 0;
          }
        }
      }
