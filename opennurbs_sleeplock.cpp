...
bool MYON_SleepLock::IsLocked() const
{
  // MYON_SleepLock::IsLocked() is not reliable. Do not use it. It will be deleted in the next SDK.
...
bool MYON_SleepLock::GetLockOrReturnFalse()
{
...
bool MYON_SleepLock::ReturnLock()
{
...
bool MYON_SleepLock::GetLock()
{
  return GetLock(MYON_SleepLock::DefaultWaitInterval,0);
}
...
{
  // int locked = MYON_IntSleepLock_Test(m_locked,0,1)
  ...
  if ( interval_wait_msecs <= 0 )
    interval_wait_msecs = MYON_SleepLock::DefaultWaitInterval;

  for(;;)
  {
    // Something else currently has this lock.

    // MYON_PointerSleepLock_SuspendThisThread() suspends the thread
    ...
    // locked = MYON_IntSleepLock_Test(m_locked,0,1)
    // performs the following test and assignment in
    // an atomic operation.
    //
    // locked = m_locked;
    ...
    if ( max_wait_msecs > 0 )
    {
      if ( interval_wait_msecs >= max_wait_msecs )
      {
        if (bStealLockAfterWaiting)
        {
          ...
          //
          MYON_WARNING("Stealing a resource lock.");
...
MYON_SleepLockGuard::MYON_SleepLockGuard(class MYON_SleepLock& sleep_lock)
  : m_sleep_lock(sleep_lock)
{
  m_bIsManagingLock = m_sleep_lock.GetLock();
}

MYON_SleepLockGuard::MYON_SleepLockGuard(class MYON_FixedSizePool& fsp)
  : m_sleep_lock(fsp.m_sleep_lock)
{
  m_bIsManagingLock = m_sleep_lock.GetLock();
}

MYON_SleepLockGuard::MYON_SleepLockGuard(
...
void MYON_SleepLockGuard::ReturnLock()
{
...
const bool MYON_SleepLockGuard::IsManagingLock() const
{
