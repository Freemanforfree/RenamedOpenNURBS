...
MYON_OBJECT_IMPLEMENT(MYON_NurbsCage,MYON_Geometry,"06936AFB-3D3C-41ac-BF70-C9319FA480A1");

MYON_OBJECT_IMPLEMENT(MYON_MorphControl,MYON_Geometry,"D379E6D8-7C31-4407-A913-E3B7040D034A");


bool MYON_NurbsCage::Read(MYON_BinaryArchive& archive)
{
  Destroy();
  ...
  if ( rc )
  {
    while(rc)
    {
      if ( major_version != 1 )
      {
        MYON_ERROR("MYON_NurbsCage::Read - old code unable to read new version of chunk");
        rc = false;
        break;
      }
      ...
      if (dim < 1 || dim > 10000)
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid dim");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid is_rat");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid order0");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid order1");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid order2");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid cv_count0");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid cv_count1");
        rc=false;
        break;
      }
      ...
      {
        MYON_ERROR("MYON_NurbsCage::Read - invalid cv_count2");
        rc=false;
        break;
      }
...
bool MYON_NurbsCage::Write(MYON_BinaryArchive& archive) const
{
...
MYON_NurbsCage::MYON_NurbsCage( int dim, bool is_rat, 
                                int order0,
                                int order1,
                                int order2,
                                int cv_count0,
...
MYON_NurbsCage::MYON_NurbsCage( const MYON_BoundingBox& bbox, 
...
bool MYON_NurbsCage::IsParallelogram(double tolerance) const
{
  int i,j,k;
  double r,s,t;
  double x,y,z,dist;
  MYON_Interval d[3];
  MYON_3dPoint P, X, Y, Z, Q, B;
  ...
  if (rc)
  {
    ...
    if ( tolerance < MYON_ZERO_TOLERANCE )
      tolerance = MYON_ZERO_TOLERANCE;

    for ( i = 0; i < m_cv_count[0]; i++ )
    {
      r = MYON_GrevilleAbcissa(m_order[0],m_knot[0]+i);
      x = d[0].NormalizedParameterAt(r);
      for ( j = 0; j < m_cv_count[1]; j++ )
      {
        s = MYON_GrevilleAbcissa(m_order[1],m_knot[1]+j);
        y = d[1].NormalizedParameterAt(s);
        for ( k = 0; k < m_cv_count[2]; k++ )
        {
          t = MYON_GrevilleAbcissa(m_order[2],m_knot[2]+k);
...
MYON_NurbsCage::MYON_NurbsCage( const MYON_3dPoint* box_corners, 
                               int order0, int order1, int order2,
                               int cv_count0, int cv_count1, int cv_count2
                               )
: m_dim(0),m_is_rat(0),m_cv_capacity(0),m_cv(0)
...
MYON_NurbsCage::MYON_NurbsCage( const MYON_BezierCage& src )
: m_dim(0),m_is_rat(0),m_cv_capacity(0),m_cv(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
...
int MYON_NurbsCage::KnotCount(int dir) const
{
  return (dir>=0 && dir<=2) ? MYON_KnotCount(m_order[dir],m_cv_count[dir]) : 0;
}

double MYON_NurbsCage::GrevilleAbcissa(
         int dir,    // dir
         int gindex  // index (0 <= index < CVCount(dir)
         ) const
{
  return (dir >= 0 && dir <= 2)
         ? MYON_GrevilleAbcissa( m_order[dir], m_knot[dir] + gindex )
         : MYON_UNSET_VALUE;
}


bool MYON_NurbsCage::MakeDeformable()
{
  return true;
}

bool MYON_NurbsCage::IsDeformable() const
{
  return true;
}

bool MYON_NurbsCage::GetTightBoundingBox( MYON_BoundingBox& tight_bbox,bool bGrowBox,const MYON_Xform* xform) const
{
...
int MYON_NurbsCage::CVCount() const
{
  return CVCount(0)*CVCount(1)*CVCount(2);
}

int MYON_NurbsCage::CVCount(int dir) const
{
  return (dir>=0&&dir<=2) ? m_cv_count[dir] : 0;
}

void MYON_NurbsCage::DestroyRuntimeCache(bool bDelete)
{
  MYON_Geometry::DestroyRuntimeCache(bDelete);
}

MYON::object_type MYON_NurbsCage::ObjectType() const
{
  return MYON::cage_object;
}

MYON_NurbsCage& MYON_NurbsCage::operator=( const MYON_BezierCage& src )
{
              ...
              src.m_order[0],src.m_order[1],src.m_order[2]) )
  {
    ...
    for ( i = 0; i < m_cv_count[0]; i++ )
    {
      for ( j = 0; j < m_cv_count[1]; j++ )
      {
        for ( k = 0; k < m_cv_count[2]; k++ )
        {
          SetCV(i,j,k,MYON::intrinsic_point_style,src.CV(i,j,k));
        }           
      }
    }
  }
...
unsigned int MYON_NurbsCage::SizeOf() const
{
...
MYON__UINT32 MYON_NurbsCage::DataCRC(MYON__UINT32 current_remainder) const
{
  ...
  current_remainder = MYON_CRC32(current_remainder,3*sizeof(m_order[0]),&m_order[0]);
  current_remainder = MYON_CRC32(current_remainder,3*sizeof(m_cv_count[0]),&m_cv_count[0]);
  ...
  current_remainder = MYON_CRC32(current_remainder,KnotCount(2)*sizeof(m_knot[2][0]),m_knot[2]);

  return current_remainder;
}

MYON_NurbsCage::MYON_NurbsCage(const MYON_NurbsCage& src)
                 : MYON_Geometry(src),
                   m_dim(0),m_is_rat(0),m_cv_capacity(0),m_cv(0)
{
  m_order[0] = 0;
...
MYON_NurbsCage& MYON_NurbsCage::operator=(const MYON_NurbsCage& src)
{
  if ( this != &src ) 
  {
...
void MYON_NurbsCage::Dump( MYON_TextLog& dump ) const
{
  dump.Print( "MYON_NurbsCage dim = %d is_rat = %d\n"
               "        order = (%d, %d, %d) \n",
               "        cv_count = (%d, %d, %d) \n",
               m_dim, m_is_rat, 
...
int MYON_NurbsCage::Dimension() const
{
  return m_dim;
}

bool MYON_NurbsCage::Create( const MYON_BoundingBox& bbox, 
                            int order0, int order1, int order2,
                            int cv_count0, int cv_count1, int cv_count2 
                            )
{
...
bool MYON_NurbsCage::Create( int dim, bool is_rat, 
                            int order0, int order1, int order2,
                            int cv_count0, int cv_count1, int cv_count2 
                            )
{
  ...
  if ( order0 < 2 || order1 < 2 || order2 < 2 )
  {
    ...
    MYON_ERROR("MYON_NurbsCage::Create - invalid orders");
    return false;
  }

  if ( cv_count0 < order0 || cv_count1 < order1 || cv_count2 < order2 )
  {
    MYON_ERROR("MYON_NurbsCage::Create - invalid cv counts");
    return false;
  }

  if ( dim < 1 )
  {
    MYON_ERROR("MYON_NurbsCage::Create - invalid dim");
    return false;
  }

  if ( is_rat != true && is_rat != false )
  {
    MYON_ERROR("MYON_NurbsCage::Create - invalid is_rat");
    return false;
  }
  ...
  ReserveKnotCapacity(0,MYON_KnotCount(m_order[0],m_cv_count[0]));
  ReserveKnotCapacity(1,MYON_KnotCount(m_order[1],m_cv_count[1]));
  ReserveKnotCapacity(2,MYON_KnotCount(m_order[2],m_cv_count[2]));
  
  MYON_MakeClampedUniformKnotVector(m_order[0],m_cv_count[0],m_knot[0],1.0);
  MYON_MakeClampedUniformKnotVector(m_order[1],m_cv_count[1],m_knot[1],1.0);
  MYON_MakeClampedUniformKnotVector(m_order[2],m_cv_count[2],m_knot[2],1.0);

  MYON_SetKnotVectorDomain( m_order[0], m_cv_count[0], m_knot[0], 0.0, 1.0);
  MYON_SetKnotVectorDomain( m_order[1], m_cv_count[1], m_knot[1], 0.0, 1.0);
  MYON_SetKnotVectorDomain( m_order[2], m_cv_count[2], m_knot[2], 0.0, 1.0);
  
  return IsValid() ? true : false;
}

bool MYON_NurbsCage::Create(
  ...
  )
{
  ...
  else
  {
    ...
    MYON_GetGrevilleAbcissae(m_order[0],m_cv_count[0],m_knot[0],false,g0);
    MYON_GetGrevilleAbcissae(m_order[1],m_cv_count[1],m_knot[1],false,g1);
    MYON_GetGrevilleAbcissae(m_order[2],m_cv_count[2],m_knot[2],false,g2);
...
void MYON_NurbsCage::EmergencyDestroy()
{
...
bool MYON_NurbsCage::GetBBox( // returns true if successful
       ...
       ) const
{
  ...
  if ( !rc )
  {
    MYON_ERROR("MYON_NurbsCage::GetBBox - invalid input");
  }
  else
  {
    ...
    {
      rc = MYON_GetPointListBoundingBox( m_dim, m_is_rat, m_cv_count[2], m_cv_stride[2],
                                      CV(i,j,0), 
...
bool MYON_NurbsCage::Transform( const MYON_Xform& xform )
{
...
MYON_Interval MYON_NurbsCage::Domain( 
      int dir
      ) const
{
  MYON_Interval d;
  if ( dir < 0 || dir > 2 || !MYON_GetKnotVectorDomain( m_order[dir], m_cv_count[dir], m_knot[dir], &d.m_t[0], &d.m_t[1] ) || !d.IsIncreasing() )
...
bool MYON_EvaluateNurbsCageSpan(
        int dim,
        bool is_rat,
        int order0, int order1, int order2,
        const double* knot0,
        ...
        )
{
  ...
  MYON_EvaluateNurbsBasis( order0, knot0, t0, N_0 );
  MYON_EvaluateNurbsBasis( order1, knot1, t1, N_1 );
  MYON_EvaluateNurbsBasis( order2, knot2, t2, N_2 );
  if ( der_count0 > 0 )
  {
		MYON_EvaluateNurbsBasisDerivatives( order0, knot0, der_count0, N_0 );
		MYON_EvaluateNurbsBasisDerivatives( order1, knot1, der_count1, N_1 );
		MYON_EvaluateNurbsBasisDerivatives( order2, knot2, der_count2, N_2 );
  }
  ...
  if ( der_count > 0 ) 
  {
	...
	if ( is_rat ) 
  {
		MYON_EvaluateQuotientRule3( dim, der_count, cvdim, P0 );
		Psize -= 8;
	}
...
bool MYON_NurbsCage::Evaluate( // returns false if unable to evaluate
...
{  
  ...
  const int span_index0 = MYON_NurbsSpanIndex(m_order[0],m_cv_count[0],m_knot[0],r,side0,hint0);
  const int span_index1 = MYON_NurbsSpanIndex(m_order[1],m_cv_count[1],m_knot[1],s,side1,hint1);
  const int span_index2 = MYON_NurbsSpanIndex(m_order[2],m_cv_count[2],m_knot[2],t,side2,hint2);

  bool rc = MYON_EvaluateNurbsCageSpan(m_dim,m_is_rat,
    m_order[0],m_order[1],m_order[2],
...
MYON_3dPoint MYON_NurbsCage::PointAt(
        double r, 
        double s, 
        double t
        ) const
...
MYON_3dPoint MYON_NurbsCage::PointAt( MYON_3dPoint rst ) const
{
  MYON_3dPoint pt;
  if ( m_dim <= 3 )
  {
...
MYON_NurbsSurface* MYON_NurbsCage::IsoSurface(
        int dir,
        double c,
        ...
        ) const
{
  ...
  if ( dir < 0 || dir > 2 )
  {
    MYON_ERROR("MYON_NurbsCage::IsoSurface - invalid dir parameter");
    return 0;
  }
  if ( m_order[dir] < 2 || m_cv_count[dir] < m_order[dir] || 0 == m_knot[dir] )
  {
    MYON_ERROR("MYON_NurbsCage::IsoSurface - invalid NURBS cage");
    return 0;
  }

  const int cage_cvdim = CVSize();

  int span_index = MYON_NurbsSpanIndex(m_order[dir],m_cv_count[dir],m_knot[dir],c,0,0);
  ...
  */
  ...
  MYON_NurbsSurface* iso_srf = srf ? srf : MYON_NurbsSurface::New();
...
bool MYON_NurbsCage::IsRational() const
{
  return m_is_rat ? true : false;
}

int MYON_NurbsCage::CVSize() const
{
  return ( m_is_rat && m_dim>0 ) ? m_dim+1 : m_dim;
}

int MYON_NurbsCage::Order( int dir ) const
{
  return (dir>=0&&dir<=2) ? m_order[dir] : 0;
}

int MYON_NurbsCage::Degree(int dir) const
{
  int order = Order(dir);
  return (order>=2) ? order-1 : 0;
}

bool MYON_NurbsCage::IsClosed(int dir) const
{
...
bool MYON_NurbsCage::IsPeriodic(int dir) const
{
...
double* MYON_NurbsCage::CV( int i, int j, int k ) const
{

#if defined(MYON_DEBUG)
  if ( 0 == m_cv )
  {
    MYON_ERROR("MYON_NurbsCage::CV - nullptr m_cv");
    return 0;
  }
  if ( i < 0 || i >= m_cv_count[0] || j< 0 || j >= m_cv_count[1] || k < 0 || k >= m_cv_count[2])
  {
    MYON_ERROR("MYON_NurbsCage::CV - (i,j,k) out of range");
    return 0;
  }
#endif
...
MYON::point_style MYON_NurbsCage::CVStyle() const
{
  return m_is_rat ? MYON::homogeneous_rational : MYON::not_rational;
}

double MYON_NurbsCage::Weight( int i, int j, int k ) const
{
  return (m_cv && m_is_rat) ? m_cv[i*m_cv_stride[0] + j*m_cv_stride[1] + k*m_cv_stride[2] + + m_dim] : 1.0;
}


bool MYON_NurbsCage::SetWeight( int i, int j, int k, double w )
{
  bool rc = false;
  if ( m_is_rat ) 
  {
...
bool MYON_NurbsCage::SetCV( int i, int j, int k, MYON::point_style style, const double* Point )
{
  bool rc = true;
  int n;
  double w;
...
bool MYON_NurbsCage::SetCV( int i, int j, int k, const MYON_3dPoint& point )
{
  bool rc = false;
  double* cv = CV(i,j,k);
  if ( cv ) {
...
bool MYON_NurbsCage::SetCV( int i, int j, int k, const MYON_4dPoint& point )
{
  bool rc = false;
  double* cv = CV(i,j,k);
  if ( cv ) {
...
bool MYON_NurbsCage::GetCV( int i, int j, int k, MYON::point_style style, double* Point ) const
{
  const double* cv = CV(i,j,k);
  if ( !cv )
    return false;
...
bool MYON_NurbsCage::GetCV( int i, int j, int k, MYON_3dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i,j,k);
  if ( cv ) {
...
bool MYON_NurbsCage::GetCV( int i, int j, int k, MYON_4dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i,j,k);
  if ( cv ) {
      ...
      )
{
  ...
  {
    MYON_ERROR("MYON_NurbsCage::SetKnot - invalid input parameters");
    rc = false;
  }
  return rc;
}

double MYON_NurbsCage::Knot(
      int dir,
      int knot_index
      ) const
{
  ...
  {
    MYON_ERROR("MYON_NurbsCage::Knot - invalid input parameters");
    knot_value = MYON_UNSET_VALUE;
  }
  return knot_value;
}

bool MYON_NurbsCage::ZeroCVs()
{
...
bool MYON_NurbsCage::MakeRational()
{
  if ( !IsRational() ) 
  {
...
bool MYON_NurbsCage::MakeNonRational()
{
...
bool MYON_NurbsCage::ReserveCVCapacity(
...
bool MYON_NurbsCage::ReserveKnotCapacity(
  int dir,
  int knot_capacity
  )
{
...
bool MYON_NurbsCage::IsSingular(		 // true if surface side is collapsed to a point
       int //side														 // side of parameter space to test
																			// 0 = south, 1 = east, 2 = north, 3 = west, 4 = bottom, 5 =top
				) const
{
  MYON_ERROR("TODO: fill in MYON_NurbsCage::IsSingular\n");
  return false;
}

bool MYON_GetCageXform( const MYON_NurbsCage& cage, MYON_Xform& cage_xform )
{
  bool rc = false;
  cage_xform = MYON_Xform::IdentityTransformation;
  if ( cage.IsValid() )
  {
    MYON_3dPoint P000, P100, P010, P001;
    ...
    MYON_3dVector X0 = P100 - P000;
    MYON_3dVector Y0 = P010 - P000;
    MYON_3dVector Z0 = P001 - P000;

    double dx0 = X0.Length();
    double dy0 = Y0.Length();
    double dz0 = Z0.Length();

    MYON_Interval d0 = cage.Domain(0);
    MYON_Interval d1 = cage.Domain(1);
    MYON_Interval d2 = cage.Domain(2);
    ...
    const MYON_Xform x2(MYON_Xform::DiagonalTransformation( d0.Length()/dx0, d1.Length()/dy0, d2.Length()/dz0 ));
      
    const MYON_Xform x3(MYON_Xform::TranslationTransformation( d0[0],d1[0],d2[0]));
...
bool MYON_MorphControl::IsIdentity( const MYON_BoundingBox& bbox ) const
{
...
bool MYON_CageMorph::IsIdentity( const MYON_BoundingBox& bbox ) const
{
  return m_control ? m_control->IsIdentity(bbox) : true;
}

MYON_MorphControl::MYON_MorphControl() 
                : m_varient(0)
{
...
void MYON_MorphControl::Destroy()
{
  m_varient = 0;
  m_nurbs_cage0 = MYON_Xform::IdentityTransformation;
  m_nurbs_curve0.Destroy();
  m_nurbs_curve.Destroy();
  m_nurbs_curve_domain = MYON_Interval::EmptyInterval;
  m_nurbs_surface0.Destroy();
  m_nurbs_surface.Destroy();
  m_nurbs_surface_domain[0] = MYON_Interval::EmptyInterval;
  m_nurbs_surface_domain[1] = MYON_Interval::EmptyInterval;
...
void MYON_MorphControl::MemoryRelocate()
{
...
bool MYON_MorphControl::IsValid( MYON_TextLog* text_log ) const
{
  bool rc = false;
  switch(m_varient)
  {
...
void MYON_MorphControl::Dump( MYON_TextLog& text_log ) const
{
...
unsigned int MYON_MorphControl::SizeOf() const
{
  unsigned int sz = sizeof(*this) 
                  - 2*sizeof(MYON_NurbsCurve) 
                  - 2*sizeof(MYON_NurbsSurface) 
                  - sizeof(m_nurbs_cage);
...
MYON::object_type MYON_MorphControl::ObjectType() const
{
  return MYON::morph_control_object;
}

void MYON_MorphControl::DestroyRuntimeCache( bool bDelete )
{
...
int MYON_MorphControl::Dimension() const
{
  int dim = 0;
  switch(m_varient)
  {
...
bool MYON_MorphControl::GetBBox(
        double* boxmin,
        double* boxmax,
        bool bGrowBox
        ) const
...
bool MYON_MorphControl::GetTightBoundingBox( 
...
void MYON_MorphControl::ClearBoundingBox()
{
}

bool MYON_MorphControl::Transform( 
        const MYON_Xform& xform
        )
{
  // Call the base class so any user data is transformed.
...
bool MYON_MorphControl::HasBrepForm() const
{
  bool rc = false;
...
MYON_Brep* MYON_MorphControl::BrepForm( MYON_Brep* brep ) const
{
  switch(m_varient)
  {
  case 1:
...
bool MYON_MorphControl::IsRational() const
{
  bool rc = false;
  switch(m_varient)
  {
...
bool MYON_MorphControl::MakeRational()
{
  bool rc = false;
  switch(m_varient)
  {
...
bool MYON_MorphControl::MakeNonRational()
{
  bool rc = false;
  switch(m_varient)
  {
...
int MYON_MorphControl::CVCount() const
{
  int rc = 0;
  switch(m_varient)
  {
...
int MYON_MorphControl::CVCount(int dir) const
{
  int rc = 0;
  switch(m_varient)
  {
...
int MYON_MorphControl::Order(int dir) const
{
  int rc = 0;
  switch(m_varient)
  {
...
MYON_3dex MYON_MorphControl::MaxCVIndex() const
{
  MYON_3dex maxdex;
...
const double* MYON_MorphControl::Knot(int dir) const
{
  const double* knot = 0;
...
const double* MYON_MorphControl::CV(MYON_3dex ijk) const
{
  const double* cv = 0;
  switch(m_varient)
  {
...
double MYON_MorphControl::Weight(MYON_3dex ijk) const
{
  double w = 1.0;
...
bool MYON_MorphControl::GetCageMorph(MYON_CageMorph& cage_morph) const
{
...
bool MYON_MorphControl::Read( MYON_BinaryArchive& archive )
{
  Destroy();
  int major_version = 0;
  int minor_version = 0;
...
bool MYON_MorphControl::Write( MYON_BinaryArchive& archive ) const
{
...
bool MYON_MorphControl::AddControlLocalizer(
  ...
  )
{
  ...
  if (rc)
  {
    ...
    case 2:
      {
        MYON_Localizer& localizer = m_localizers.AppendNew();
        localizer.m_type = MYON_Localizer::distance_type;
    ...
    case 3:
      {
        MYON_Xform xform = m_nurbs_cage0;
        xform.Invert();
        MYON_Interval d[3];
        ...
        MYON_SimpleArray<MYON_Plane> planes(6);

        MYON_3dPoint C(d[0].ParameterAt(0.5),d[1].ParameterAt(0.5),d[2].ParameterAt(0.5));
        MYON_3dPoint P;
        MYON_3dVector N;
        int i;
        ...
        for ( i = 0; i < 3; i++ )
        {
          ...
          N[i] = -det;
          P[i] = d[i][0];
          MYON_Plane& plane0 = planes.AppendNew();
          plane0.CreateFromNormal(P,N);
          plane0.Transform(xform);

          P[i] = d[i][1];
          N[i] = det;
          MYON_Plane& plane1 = planes.AppendNew();
...
bool MYON_MorphControl::AddSphereLocalizer(
  MYON_3dPoint center,
  double support_distance, 
  double falloff_distance
  )
...
bool MYON_MorphControl::AddCylinderLocalizer(
  MYON_Line axis,
  double support_distance, 
  double falloff_distance
  )
...
bool MYON_MorphControl::AddBoxLocalizer(
...
bool MYON_MorphControl::AddPlaneLocalizer(
            const MYON_Plane& plane,
            double support_distance, 
            double falloff_distance
            )
{
  MYON_SimpleArray<MYON_Plane> planes(1);
  planes.Append(plane);
  return AddConvexPolygonLocalizer(planes,support_distance,falloff_distance);
}

bool MYON_MorphControl::AddConvexPolygonLocalizer(
  const MYON_SimpleArray<MYON_Plane>& planes,
  double support_distance, 
  double falloff_distance
  )
{
  ...
  if (rc)
  {
    ...
    for( i = 0; i < count && rc; i++)
    {
      const MYON_Plane& plane = planes[i];
      MYON_Localizer& localizer = m_localizers.AppendNew();
