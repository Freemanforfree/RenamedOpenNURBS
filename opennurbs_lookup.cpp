...
static bool IdIsNotNil(const MYON_UUID* id)
{
  // This is a fast static function to check for zero ids.
  // The caller always checks that id is not null.
  const MYON__UINT64* p = (const MYON__UINT64*)id;
  return (p[0] != 0 || p[1] != 0);
}

static bool IdIsNil(const MYON_UUID* id)
{
  // This is a fast static function to check for zero ids.
  // The caller always checks that id is not null.
  const MYON__UINT64* p = (const MYON__UINT64*)id;
...
static bool IdIsEqual(const MYON_UUID* a, const MYON_UUID* b)
{
  // This is a fast static function to check for zero ids.
  // The caller always checks that id is not null.
  const MYON__UINT64* p = (const MYON__UINT64*)a;
  const MYON__UINT64* q = (const MYON__UINT64*)b;
  return (p[0] == q[0] && p[1] == q[1]);
}

static bool IdIsNotEqual(const MYON_UUID* a, const MYON_UUID* b)
{
  // This is a fast static function to check for zero ids.
  // The caller always checks that id is not null.
  const MYON__UINT64* p = (const MYON__UINT64*)a;
...
static MYON__UINT32 IdCRC32(const MYON_UUID* id)
{
  return MYON_CRC32(0, sizeof(*id), id);
}
...
public:
  enum : unsigned int
  {
    // These numbers are chosen so the MYON_SerialNumberMap
  ...
  MYON_SN_BLOCK() = default;
  ~MYON_SN_BLOCK() = default;
  MYON_SN_BLOCK(const MYON_SN_BLOCK&) = default;
  MYON_SN_BLOCK& operator=(const MYON_SN_BLOCK&) = default;

  MYON__UINT32 m_count = 0;  // used elements in m_sn[]
  MYON__UINT32 m_purged = 0; // number of purged elements in m_sn[]
  MYON__UINT32 m_sorted = 1; // 0 = no, 1 = yes
  MYON__UINT32 m_reserved = 0;
  MYON__UINT64 m_sn0 = 0; // minimum sn in m_sn[]
  MYON__UINT64 m_sn1 = 0; // maximum sn in m_sn[]
  struct MYON_SerialNumberMap::SN_ELEMENT m_sn[MYON_SN_BLOCK::SN_BLOCK_CAPACITY];
  ...
  */
  ...
  bool IsValidBlock(
    MYON_TextLog* textlog,
    struct MYON_SerialNumberMap::SN_ELEMENT*const*const* hash_table,
    MYON__UINT32 hash_block_count,
    MYON__UINT64* active_id_count
    ) const;
  struct MYON_SerialNumberMap::SN_ELEMENT* BinarySearchBlockHelper(
    MYON__UINT64 sn
    );
  ...
  MYON__UINT64 ActiveElementEstimate(
    MYON__UINT64 sn0,
    MYON__UINT64 sn1
    ) const;
...
void MYON_SerialNumberMap::EmptyList()
{
  m_maxsn = 0;
  m_sn_count = 0;
  m_sn_purged = 0;
  ...
  if (m_snblk_list)
  {
    MYON__UINT64 i = m_snblk_list_capacity;
    while(i--)
    {
      if ( 0 != m_snblk_list[i] )
  ...
  if (nullptr != m_hash_table_blocks)
  {
    ...
    for (MYON__UINT32 i = 1; i < m_hash_block_count; i++)
...
MYON_SerialNumberMap::MYON_SerialNumberMap()
  : m_sn_block0(*(new MYON_SN_BLOCK()))
{}

MYON_SerialNumberMap::~MYON_SerialNumberMap()
{
  EmptyList();
  MYON_SN_BLOCK* p = &m_sn_block0;
  delete p;
}

void MYON_SN_BLOCK::EmptyBlock()
{
  m_count = 0;
  m_purged = 0;
  m_sorted = 1;
...
bool MYON_SN_BLOCK::NeedsToBeCulled() const
{
  const MYON__UINT32 purge_ratio 
    = (m_purged >= 16)
    ? 16
    : 2;
...
void MYON_SN_BLOCK::CullBlockHelper()
{
...
static void MYON_qsort_SN_ELEMENT( struct MYON_SerialNumberMap::SN_ELEMENT* base, size_t nel );
*/

#define MYON_SORT_TEMPLATE_COMPARE compare_SN_ELEMENT_sn
#define MYON_COMPILING_OPENNURBS_QSORT_FUNCTIONS
#define MYON_SORT_TEMPLATE_STATIC_FUNCTION
#define MYON_SORT_TEMPLATE_TYPE struct MYON_SerialNumberMap::SN_ELEMENT
#define MYON_QSORT_FNAME MYON_qsort_SN_ELEMENT

static int MYON_SORT_TEMPLATE_COMPARE(const struct MYON_SerialNumberMap::SN_ELEMENT * a, const struct MYON_SerialNumberMap::SN_ELEMENT * b)
{
  MYON__UINT64 asn, bsn;
...
void MYON_SN_BLOCK::SortBlockHelper()
{
...
#if 1
    // Quick sort
    MYON_qsort_SN_ELEMENT(m_sn, (size_t)m_count);
#else
    // Heap sort
    size_t i, j, k, i_end;
...
static bool MYON_SerialNumberMap_IsNotValidBlock()
{
  return MYON_IsNotValid();
}

bool MYON_SN_BLOCK::IsValidBlock(
  MYON_TextLog* textlog, 
  struct MYON_SerialNumberMap::SN_ELEMENT*const*const* hash_table,
  MYON__UINT32 hash_block_count,
  MYON__UINT64* active_id_count
  ) const
{
  ...
  if ( m_count > MYON_SN_BLOCK::SN_BLOCK_CAPACITY )
  {
    if (textlog)
      textlog->Print("MYON_SN_BLOCK m_count = %u (should be >=0 and <%u).\n",
                      m_count,MYON_SN_BLOCK::SN_BLOCK_CAPACITY);
    return MYON_SerialNumberMap_IsNotValidBlock();
  }

  if ( m_purged > m_count )
  {
    if (textlog)
      textlog->Print("MYON_SN_BLOCK m_purged = %u (should be >0 and <=%u).\n",
                      m_purged,m_count);
    return MYON_SerialNumberMap_IsNotValidBlock();
  }
  ...
  {
    if (textlog)
      textlog->Print("MYON_SN_BLOCK m_count = %u but m_sorted is not 1.\n",m_count);
    return MYON_SerialNumberMap_IsNotValidBlock();
  }
  ...
  if ( 0 == m_count )
  {
    ...
    {
      if (textlog)
        textlog->Print("MYON_SN_BLOCK m_count = 0 but m_sn0 != 0.\n");
      return MYON_SerialNumberMap_IsNotValidBlock();
    }
    if ( 0 != m_sn1 )
    {
      if (textlog)
        textlog->Print("MYON_SN_BLOCK m_count = 0 but m_sn1 != 0.\n");
      return MYON_SerialNumberMap_IsNotValidBlock();
    }
    return true;
  }
  ...
  {
    if (textlog)
      textlog->Print("MYON_SN_BLOCK m_sn1 < m_sn0.\n");
    return MYON_SerialNumberMap_IsNotValidBlock();
  }
  ...
  for (i = 0; i < m_count; i++)
  {
    ...
    if (0 == m_sn[i].m_sn_active)
    {
      ...
      {
        if (textlog)
          textlog->Print("MYON_SN_BLOCK m_sn[%d].m_sn_active = 0 but m_id_active != 0.\n",i);
        return MYON_SerialNumberMap_IsNotValidBlock();
      }
    }
    else if ( 0 != m_sn[i].m_id_active )
    {
      ...
      {
        const MYON__UINT32 id_crc32 = IdCRC32(&m_sn[i].m_id);
        if (id_crc32 != m_sn[i].m_id_crc32)
        {
          if (textlog)
            textlog->Print("MYON_SN_BLOCK m_sn[%d].m_id_active != 0 and m_sn[i].m_id_crc32 != IdCRC32(&m_sn[i].m_id).\n", i);
          return MYON_SerialNumberMap_IsNotValidBlock();
        }
        if (nullptr != hash_table)
        {
          const MYON_SerialNumberMap::SN_ELEMENT* e = nullptr;
          for (e = hash_table[id_crc32 % hash_block_count][(id_crc32/MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY) % MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY]; 0 != e; e = e->m_next)
          {
            if (e == &m_sn[i])
            {
          ...
          if (nullptr == e)
          {
            ...
            // it should be.
            if (textlog)
              textlog->Print("MYON_SN_BLOCK m_sn[%d].m_id_active != 0 but the element is not in m_hash_table[].\n", i);
            return MYON_SerialNumberMap_IsNotValidBlock();
          }
        }
      }
      ...
      {
        if (textlog)
          textlog->Print("MYON_SN_BLOCK m_sn[%d].m_id_active != 0 but m_id = 0.\n",i);
        return MYON_SerialNumberMap_IsNotValidBlock();
      }
    }
    ...
    {
      if (textlog)
        textlog->Print("MYON_SN_BLOCK m_sn[%d] < m_sn0.\n",i);
      return MYON_SerialNumberMap_IsNotValidBlock();
    }
    if ( sn > m_sn1 )
    {
      if (textlog)
        textlog->Print("MYON_SN_BLOCK m_sn[%d] > m_sn1.\n",i);
      return MYON_SerialNumberMap_IsNotValidBlock();
    }
    ...
    if ( m_sorted )
    {
      ...
      {
        if (textlog)
          textlog->Print("MYON_SN_BLOCK m_sn[%d] > m_sn[%d].\n",i,i-1);
        return MYON_SerialNumberMap_IsNotValidBlock();
      }
      sn0 = sn;
    }
  ...
  {
    if (textlog)
      textlog->Print("MYON_SN_BLOCK m_purged = %u (should be %u)\n",m_purged,pc);
    return MYON_SerialNumberMap_IsNotValidBlock();
  }
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SN_BLOCK::BinarySearchBlockHelper(MYON__UINT64 sn)
{
  ...
  MYON__UINT64 i, j;
  struct MYON_SerialNumberMap::SN_ELEMENT* e;
  MYON__UINT64 midsn;
...
void MYON_SerialNumberMap::UpdateMaxSNHelper()
{
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::FindElementHelper(MYON__UINT64 sn)
{
  class MYON_SN_BLOCK** eblk_array;
  class MYON_SN_BLOCK* eblk;
  size_t i, j;
  ...
  while (i > 0 )
  {
    ...
    // entire MYON_SerialNumberMap tends to
    // be tidy.
...
MYON__UINT64 MYON_SerialNumberMap::ActiveSerialNumberCount() const
{
  return m_sn_count - m_sn_purged;
}

MYON__UINT64 MYON_SerialNumberMap::ActiveIdCount() const
{
  return m_active_id_count;
}

MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::FirstElement() const
{
     ...
     )
  {
    ...
    if ( m_sn_block0.m_purged > 0 )
    {
      ...
      // memory location for specific elements will be changed.
      // This will make the hash table invalid.
      const_cast<MYON_SerialNumberMap*>(this)->Internal_HashTableInvalidate();

      const_cast<MYON_SerialNumberMap*>(this)->m_sn_count -= m_sn_block0.m_purged;
      const_cast<MYON_SerialNumberMap*>(this)->m_sn_purged -= m_sn_block0.m_purged;
      const_cast<MYON_SerialNumberMap*>(this)->m_sn_block0.CullBlockHelper();
    }
    if ( !m_sn_block0.m_sorted )
    {
      ...
      const_cast<MYON_SerialNumberMap*>(this)->m_sn_block0.SortBlockHelper();      
    }
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::LastElement() const
{
  ...
  i = (MYON__UINT64)m_snblk_list_count;
  while(i--)
  {
    if ( m_snblk_list[i]->m_count > m_snblk_list[i]->m_purged )
    {
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::FindSerialNumber(MYON__UINT64 sn) const
{
  struct SN_ELEMENT* e = const_cast<MYON_SerialNumberMap*>(this)->FindElementHelper(sn);
  return ( (e && e->m_sn_active) ? e : 0);
}

struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::FindId(MYON_UUID id) const
{
...
MYON__UINT64 MYON_SerialNumberMap::GetElements(
  MYON__UINT64 sn0,
  MYON__UINT64 sn1, 
  MYON__UINT64 max_count,
  MYON_SimpleArray<SN_ELEMENT>& elements
  ) const
{
  MYON__UINT64 i,j,k,c;
  ...
  if ( sn0+3 <= sn1 )
  {
    ...
    while ( sn0 <= sn1 )
    {
      ei = const_cast<MYON_SerialNumberMap*>(this)->FindElementHelper(sn0++);
     ...
     )
  {
    if ( !m_sn_block0.m_sorted )
    {
      if ( m_sn_block0.m_purged > 0 )
      {
        ...
        const_cast<MYON_SerialNumberMap*>(this)->m_sn_purged -= m_sn_block0.m_purged;
        const_cast<MYON_SerialNumberMap*>(this)->m_sn_block0.CullBlockHelper();
        const_cast<MYON_SerialNumberMap*>(this)->UpdateMaxSNHelper();
      }
      if ( m_sn_block0.m_count > 0 )
      {
        // memory location for specific elements will be changed.
...
MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::Internal_HashTableRemoveElement(
  struct SN_ELEMENT* e,
  ...
  )
{
  ...
  {
    MYON_ERROR("MYON_SerialNumberMap - m_active_id_count corruption");
    m_inactive_id = MYON_nil_uuid;
  }
  ...
  if ( m_bHashTableIsValid && bRemoveFromHashBlock )
  {
    ...
    const MYON__UINT32 hash_i = MYON_SerialNumberMap::Internal_HashTableBlockRowIndex(e->m_id_crc32);
    ...
    if (nullptr == h)
    {
      MYON_ERROR("id not found in hash table.");
    }
  }
...
struct MYON_SerialNumberMap::SN_ELEMENT* 
MYON_SerialNumberMap::RemoveSerialNumberAndId(MYON__UINT64 sn)
{
...
struct MYON_SerialNumberMap::SN_ELEMENT* 
MYON_SerialNumberMap::RemoveId(MYON__UINT64 sn, MYON_UUID id)
{
  if ( m_active_id_count > 0 && IdIsNotNil(&id) && IdIsNotEqual(&id,&m_inactive_id))
  {
    struct SN_ELEMENT* e;
    ...
    const MYON__UINT32 id_crc32 = IdCRC32(&id);
    struct SN_ELEMENT** hash_table_block = Internal_HashTableBlock(id_crc32);
    const MYON__UINT32 hash_i = MYON_SerialNumberMap::Internal_HashTableBlockRowIndex(id_crc32);
    struct SN_ELEMENT* prev = nullptr;
...
int MYON_SN_BLOCK::CompareMaxSN(const void* a, const void* b)
{
  const MYON__UINT64 sna = (*((const MYON_SN_BLOCK*const*)a))->m_sn1;
  const MYON__UINT64 snb = (*((const MYON_SN_BLOCK*const*)b))->m_sn1;
  if ( sna < snb )
  {
    return (0 == sna) ? 1 : -1;
  }
...
MYON__UINT64 MYON_SN_BLOCK::ActiveElementEstimate(MYON__UINT64 sn0, MYON__UINT64 sn1) const
{
  MYON__UINT64 c = m_count-m_purged;
  if ( c > 0 )
  {
    if ( sn0 < m_sn0 )
      sn0 = m_sn0;
...
static bool MYON_SerialNumberMap_IsNotValid()
{
  return MYON_IsNotValid();
}

bool MYON_SerialNumberMap::IsValid(
   bool bBuildHashTable,
   MYON_TextLog* textlog
   ) const
{
  MYON__UINT64 i, c, pc, aic;
  ...
  {
    c = 0;
    for (MYON__UINT32 hash_block_index = 0; hash_block_index < m_hash_block_count; hash_block_index++)
    {
      const SN_ELEMENT*const* hash_table_block = m_hash_table_blocks[hash_block_index];
      for (MYON__UINT32 hash_i = 0; hash_i < MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY; hash_i++)
      {
        for (const SN_ELEMENT* e = hash_table_block[hash_i]; nullptr != e && c <= m_active_id_count; e = e->m_next)
        {
          if (
            ...
            || hash_i != MYON_SerialNumberMap::Internal_HashTableBlockRowIndex(e->m_id_crc32)
            )
          {
            if (textlog)
...
MYON__UINT64 MYON_SerialNumberMap::GarbageCollectMoveHelper(MYON_SN_BLOCK* dst,MYON_SN_BLOCK* src)
{
  ...
  if ( src && dst )
  {
    n = MYON_SN_BLOCK::SN_BLOCK_CAPACITY - dst->m_count;
    if ( src->m_count < n )
      n = src->m_count;
    if ( n > 0 )
    {
...
void MYON_SerialNumberMap::GarbageCollectHelper()
{
  MYON__UINT64 i,m;
  MYON__UINT32 j,k,n;

  // This is a helper function.  The caller
  // should check that MYON_SN_BLOCK::SN_BLOCK_CAPACITY == m_sn_block0.m_count
  // before calling it.
  ...
  if ( m_sn_block0.m_purged > 0 )
  {
    ...
    if ( 0 == m_snblk_list_count )
      m_maxsn = m_sn_block0.m_sn1; 
    if ( m_sn_block0.m_count < 7*(MYON_SN_BLOCK::SN_BLOCK_CAPACITY/8) )
      return;
  }
    ...
    if ( 0 == m_snblk_list[i]->m_count )
    {
      ...
      for(m = i+1; m < m_snblk_list_count; m++ )
      {
        if ( m_snblk_list[m]->m_count > 0 )
        {
          ...
          MYON_qsort(m_snblk_list+i,(size_t)(m_snblk_list_count-i),sizeof(*m_snblk_list),MYON_SN_BLOCK::CompareMaxSN);
          break;
        }
      }
     ...
     )
  {
    ...
    // the largest.
    MYON__UINT32 snarray_count = 0;
    struct SN_ELEMENT* snarray = (struct SN_ELEMENT*)onmalloc(2*MYON_SN_BLOCK::SN_BLOCK_CAPACITY*sizeof(snarray[0]));
    for ( i = 0; i < m_snblk_list_count && m_sn_block0.m_count > 0; i++ )
    {
      ...
      MYON_SN_BLOCK* blk = m_snblk_list[i];
      const MYON__UINT64 sn1 = (i < m_snblk_list_count-1) 
      ...
      while(j < blk->m_count && k < m_sn_block0.m_count )
      {
        ...
        else
        {
          ...
          MYON_ERROR("Bogus information - should never get here");
          break;
        }
      }
      n = blk->m_count-j;
      ...
      n = (snarray_count > MYON_SN_BLOCK::SN_BLOCK_CAPACITY) 
        ? MYON_SN_BLOCK::SN_BLOCK_CAPACITY 
        : snarray_count;
  ...
  for ( i = 0; i < m_snblk_list_count; i++ )
  {
    for ( m = i+1; m < m_snblk_list_count; m++ )
    {
      if ( m_snblk_list[i]->m_count >= MYON_SN_BLOCK::SN_BLOCK_CAPACITY )
        break;
  ...
  if ( m_sn_block0.m_count > MYON_SN_BLOCK::SN_BLOCK_CAPACITY/4 )
  {
    if ( m_snblk_list_count > 0 )
    {
      GarbageCollectMoveHelper(m_snblk_list[m_snblk_list_count-1],&m_sn_block0);
    }
    if ( m_sn_block0.m_count > MYON_SN_BLOCK::SN_BLOCK_CAPACITY/2 )
    {
      ...
      if ( m_snblk_list_count == m_snblk_list_capacity )
      {
        ...
        m_snblk_list = (MYON_SN_BLOCK**)((0 == m_snblk_list)
                     ? onmalloc((size_t)m)
      ...
      if ( 0 == m_snblk_list[m_snblk_list_count] )
      {
        ...
        m_snblk_list[m_snblk_list_count] = (MYON_SN_BLOCK*)onmalloc(sizeof(*(m_snblk_list[m_snblk_list_count])));
      }
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::AddSerialNumber(MYON__UINT64 sn)
{
  if ( sn <= 0 )
    return 0;
  struct SN_ELEMENT* e = FindElementHelper(sn);
  ...
  {
    if ( MYON_SN_BLOCK::SN_BLOCK_CAPACITY == m_sn_block0.m_count )
    {
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::Internal_HashTableFindId(
  MYON_UUID id,
  MYON__UINT32 id_crc32,
  ...
  ) const
{
  ...
  if (false == m_bHashTableIsValid)
  {
    for (MYON__UINT32 i = 0; i < 8 && i < m_sn_block0.m_count; i++)
    {
      if ( IdIsEqual(&id,&m_sn_block0.m_sn[i].m_id) )
      {
        MYON_SerialNumberMap::SN_ELEMENT* e = const_cast<MYON_SerialNumberMap::SN_ELEMENT*>(&m_sn_block0.m_sn[i]);
        if ( 0 != e->m_id_active )
          return e;
      }
    }
  ...
  for (
    MYON_SerialNumberMap::SN_ELEMENT* e = m_hash_table_blocks[id_crc32 % m_hash_block_count][(id_crc32 / MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY) % MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY];
    nullptr != e;
    e = e->m_next
    )
  {
...
struct MYON_SerialNumberMap::SN_ELEMENT* MYON_SerialNumberMap::AddSerialNumberAndId(MYON__UINT64 sn,MYON_UUID id)
{
...
void MYON_SerialNumberMap::Internal_HashTableInvalidate()
{
...
bool MYON_SerialNumberMap::Internal_HashTableRemoveSerialNumberBlock(const MYON_SN_BLOCK* blk)
{
  ...
  if ( m_bHashTableIsValid && m_active_id_count > 128*blk->m_count )
  {
    ...
    for (e = blk->m_sn, i = blk->m_count; i--; e++)
    {
      ...
      MYON__UINT32 hash_i = Internal_HashTableBlockRowIndex(e->m_id_crc32);
      prev = 0;
...
MYON__UINT64 MYON_SerialNumberMap::Internal_HashTableAddSerialNumberBlock( MYON_SN_BLOCK* blk) const
{
  MYON__UINT32 active_id_count = 0; // 4 bytes is enough for a single block
  ...
  return (MYON__UINT64)active_id_count; // entire map may contain many blocks and more than 2^32 entries.
}


MYON__UINT32 MYON_SerialNumberMap::Internal_HashTableBlockIndex(
  MYON__UINT32 id_crc32
  ) const
{
...
MYON__UINT32 MYON_SerialNumberMap::Internal_HashTableBlockRowIndex(
  MYON__UINT32 id_crc32
  )
{
  return (id_crc32/MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY) % MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY;
}

MYON_SerialNumberMap::SN_ELEMENT** MYON_SerialNumberMap::Internal_HashTableBlock(
  MYON__UINT32 id_crc32
  ) const
{
...
void MYON_SerialNumberMap::Internal_HashTableGrow() const
{
  if (m_active_id_count >= m_hash_capacity && m_hash_block_count < MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY)
  {
    // Add capacity to the hash table.

    MYON__UINT64 sz = (2U*m_hash_block_count);
    const MYON__UINT32 target_linked_list_length = 4U;
    while (sz*((MYON__UINT64)(target_linked_list_length*MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY)) <= m_active_id_count)
      sz++;
    const MYON__UINT64 max_hash_block_count = (MYON__UINT64)MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY;
    MYON__UINT32 hash_block_count = (MYON__UINT32)((sz < max_hash_block_count) ? sz : max_hash_block_count);
    if ( 0 == hash_block_count )
      hash_block_count = 1;
    if (m_hash_block_count < hash_block_count )
    {
      const size_t sizeof_hash_table_block = MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY*sizeof(m_hash_table_blocks[0][0]);
      if (0 == m_hash_block_count)
      {
        m_hash_table_blocks = (SN_ELEMENT***)onmalloc(2 * sizeof_hash_table_block);
        m_hash_table_blocks[0] = (SN_ELEMENT**)(m_hash_table_blocks + MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY);
      ...
      m_hash_capacity = target_linked_list_length*MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY;
...
void MYON_SerialNumberMap::Internal_HashTableInitialize() const
{
  ...
  {
    for (MYON__UINT32 i = 0; i < m_hash_block_count; i++)
      memset(m_hash_table_blocks[i], 0, MYON_SN_BLOCK::ID_HASH_BLOCK_CAPACITY*sizeof(m_hash_table_blocks[0][0]));

    // m_bHashTableIsValid must be true
    // before calling Internal_HashTableAddSerialNumberBlock()
    m_bHashTableIsValid = true;

    MYON__UINT64 active_id_count = 0;
    for (MYON__UINT64 snblk_i = 0; snblk_i < m_snblk_list_count; snblk_i++)
    ...
    active_id_count += Internal_HashTableAddSerialNumberBlock(const_cast<MYON_SN_BLOCK*>(&m_sn_block0));

    if (active_id_count != m_active_id_count)
    {
      MYON_ERROR("m_active_id_count was corrupt and had to be fixed.");
      const_cast<MYON_SerialNumberMap*>(this)->m_active_id_count = active_id_count;
    }

  }
}

void MYON_SerialNumberMap::Internal_HashTableBuild() const
{
  // This function is called when code requires
  // m_hash_table[] to be valid so an id can be looked up.
...
void MYON_SerialNumberMap::SN_ELEMENT::Dump(MYON_TextLog& text_log) const
{
  text_log.Print("m_id = ");
  text_log.Print(m_id);
  text_log.Print("\nm_sn = %d\n",m_sn);
...
void MYON_SN_BLOCK::Dump(MYON_TextLog& text_log) const
{
...
void MYON_SerialNumberMap::Dump(MYON_TextLog& text_log) const
{
