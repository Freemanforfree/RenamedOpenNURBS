...
static void Internal_PointRingSharpSubdivision(
  const MYON_SubDVertexTag center_vertex_tag,
  double sector_coefficient,
  const MYON_SubDEdgePtr* edges,
  size_t edges_stride,
  const unsigned int N,
  const unsigned int F,
  MYON_SubDEdgeSharpness* edge_sharpness,
  double* point_ring,
...
)
{
  ...
  if (N < 2)
  {
    MYON_SUBD_ERROR("Invalid input. N must be at least 2.");
    return;
  }
  if (MYON_SubDVertexTag::Smooth == center_vertex_tag || MYON_SubDVertexTag::Dart == center_vertex_tag)
  {
    // These checks insure the pointer arithmetic below never dereferences an invalid
    // pointer due to invalid input.
    if (N != F)
    {
      MYON_SUBD_ERROR("Invalid input. At least on of the center vertex tag, N, or F is wrong.");
      return;
    }
  }
  else if(MYON_SubDVertexTag::Crease == center_vertex_tag)
  {
    // These checks insure the pointer arithmetic below never dereferences an invalid
    // pointer due to invalid input.
    if (N != F + 1)
    ...
    if (MYON_SubDEdgeTag::Crease != MYON_SUBD_EDGE_POINTER(edges->m_ptr)->m_edge_tag)
    {
      MYON_SUBD_ERROR("Invalid input. In a crease sector the first edge must be a crease.");
      return;
    }
    if (MYON_SubDEdgeTag::Crease != MYON_SUBD_EDGE_POINTER((edges + ((N - 1)* edges_stride) )->m_ptr)->m_edge_tag)
    {
      MYON_SUBD_ERROR("Invalid input. In a crease sector the last edge must be a crease.");
      return;
    }
  }
  else
  {
    MYON_SUBD_ERROR("Invalid input. Center vertex tag must be smooth, dart, or crease.");
    return;
  }

  const unsigned expected_vertex_C_count
    = (MYON_SubDVertexTag::Crease == center_vertex_tag)
    ? 2U
    : (MYON_SubDVertexTag::Dart == center_vertex_tag ? 1U : 0U);

  if (MYON_SubDVertexTag::Smooth == center_vertex_tag)
    sector_coefficient = 0.5;
  else if (false == (sector_coefficient > 0.0 && sector_coefficient < 1.0))
  {
    // Input edges passed to  MYON_SubD::GetQuadSectorPointRing()
    // had incorrect sector coefficients. Using 1/2 will be good enough
    // to get some sort of answer.
    MYON_SUBD_ERROR("An input edges to MYON_SubD::GetQuadSectorPointRing() had incorrect sector coeffient value.");
    sector_coefficient = 0.5;
  }

  const unsigned R = 1U + N + F;
  MYON_SimpleArray<MYON_3dPoint> R1_buffer;
  MYON_3dPoint* R1 = R1_buffer.Reserve(R);
  ...
  for (;;)
  {
    ...
    MYON_3dPoint vertex_A[2] = {}; // end points of the 1st two crease and/or sharp edges
    MYON_3dPoint vertex_C[2] = {}; // end points of the crease edges (at most two per sector)
    ...
    for (unsigned i = 0; i < N; ++i)
    {
      if (MYON_SubDEdgeTag::Crease == MYON_SUBD_EDGE_POINTER(edges->m_ptr)->m_edge_tag)
      {
        ...
        if (vertex_C_count >= 2)
        {
          MYON_SUBD_ERROR("Invalid input. A sector has at most 2 creased edges.");
          return;
        }
        vertex_C[vertex_C_count] = MYON_3dPoint(A0);
        ++vertex_C_count;
        if (vertex_A_count < 2)
          vertex_A[vertex_A_count] = MYON_3dPoint(A0);
        ++vertex_A_count;
    ...
    {
      MYON_SUBD_ERROR("Invalid input. Sector tag and number of crease edges are incompatible.");
      return;
    }
...
unsigned int MYON_SubD::GetQuadSectorPointRing(
  bool bFirstPass,
  bool bSecondPass,
  ...
  )
{
  ...
  const MYON_SubDVertex* vertex0 = center_vertex;
  ...
  MYON_SubDEdgeSharpness* edge_sharpness = nullptr;
  double maxs = 0.0;
  double sector_coefficient = (vertex0->IsDartOrCreaseOrCorner()) ? 0.0 : 0.5;

  edges = (1 == element_stride) ? vertex0->m_edges : (const MYON_SubDEdgePtr*)(component_ring + 1);
  for (unsigned i = 0; i < N; ++i, edges += element_stride)
  {
    const MYON_SubDEdge* edge = MYON_SUBD_EDGE_POINTER(edges->m_ptr);
    if (nullptr == edge)
      return MYON_SUBD_RETURN_ERROR(0);
    const MYON_SubDEdgeTag etag = edge->m_edge_tag;
    if (MYON_SubDEdgeTag::Smooth == etag || MYON_SubDEdgeTag::SmoothX == etag)
    {
      if (0.0 == sector_coefficient)
      {
        ...
        sector_coefficient = edge->m_sector_coefficient[MYON_SUBD_EDGE_DIRECTION(edges->m_ptr)];
      }
      MYON_SubDEdgeSharpness s = edge->Sharpness();
      if ( s.IsNotZero() )
      {
        if (nullptr == edge_sharpness)
          edge_sharpness = (MYON_SubDEdgeSharpness*)calloc(N, sizeof(edge_sharpness[0]));
        if (1 == MYON_SUBD_EDGE_DIRECTION(edges->m_ptr))
          s = s.Reversed();
        edge_sharpness[i] = s;
        if (maxs < s[0])
          maxs = s[0];
  ...
  for (unsigned int pass = (bFirstPass ? 0U : 1U); pass < (bSecondPass ? 2U : 1U); pass++)
  {
    if (1 == pass && nullptr != edge_sharpness)
    {
      ...
      else
      {
        ...
        {
          const MYON_SubDEdgeSharpness s1 = edge_sharpness[k].Subdivided(0);
          edge_sharpness[k] = s1;
          if (maxs < s1[0])
            maxs = s1[0];
          if (maxs < s1[1])
    ...
    else
    {
      if ( false == bSecondPass)
        return MYON_SUBD_RETURN_ERROR(0); // subdivision not permitted
    ...
    const MYON_SubDEdgePtr* edges0 = (1 == element_stride) ? vertex0->m_edges : (const MYON_SubDEdgePtr*)(component_ring + 1);
    edges = edges0;
    const MYON_SubDFacePtr* faces = (1 == element_stride) ? ((const MYON_SubDFacePtr * )vertex0->m_faces) : (const MYON_SubDFacePtr*)(component_ring + 2);
    for ( unsigned int i = 0; i < N; ++i, edges += element_stride, faces += element_stride )
    {
      // Get edge point
      MYON__UINT_PTR eptr = edges->m_ptr;
      const MYON_SubDEdge* edge = MYON_SUBD_EDGE_POINTER(eptr);
      if (nullptr == edge)
        return MYON_SUBD_RETURN_ERROR(0);
      eptr = 1 - MYON_SUBD_EDGE_DIRECTION(eptr);
      const MYON_SubDVertex* vertex1 = edge->m_vertex[eptr];
      if ( nullptr == vertex1)
        return MYON_SUBD_RETURN_ERROR(0);
      ...
      if (0 == pass)
      {
        ...
        if (MYON_SubDVertexTag::Smooth == vertex1->m_vertex_tag 
      ...
      const MYON_SubDFace* face = MYON_SUBD_FACE_POINTER(faces->m_ptr);
      if (0 == pass)
      {
        ...
        if ( nullptr == edge)
          return MYON_SUBD_RETURN_ERROR(0);
        eptr = MYON_SUBD_EDGE_DIRECTION(eptr);
        const MYON_SubDVertex* vertexQ = edge->m_vertex[eptr];
        if ( nullptr == vertexQ)
          return MYON_SUBD_RETURN_ERROR(0);
        Q = vertexQ->m_P;
      }
...
unsigned int MYON_SubD::ComponentRingEdgeCount(size_t component_ring_count)
{
  const unsigned int N 
...
unsigned int MYON_SubD::ComponentRingFaceCount(size_t component_ring_count)
{
...
bool MYON_SubD::ComponentRingIsValid(
  const MYON_SubDComponentPtr* component_ring,
  size_t component_ring_count
  )
{
  ...
  const MYON_SubDVertex* vertex = component_ring[0].Vertex();
  if ( nullptr == vertex )
    return MYON_SUBD_RETURN_ERROR(false);
  ...
  const MYON_SubDEdgeTag edge0_tag 
    = (F+1 == N || (F == N && MYON_SubDVertexTag::Dart == vertex->m_vertex_tag))
    ? MYON_SubDEdgeTag::Crease
    : MYON_SubDEdgeTag::Smooth;
  const MYON_SubDEdgeTag edge1_tag 
    = (F+1 == N)
    ? MYON_SubDEdgeTag::Crease
    : MYON_SubDEdgeTag::Smooth;
  ...
  {
    const MYON_SubDEdge* edge = component_ring[component_ring_index].Edge();
    if ( nullptr == edge)
      return MYON_SUBD_RETURN_ERROR(false);
    ...
    if (0 == i)
    {
      ...
      {
        if (  MYON_SubDEdgeTag::Smooth != edge0_tag || MYON_SubDEdgeTag::SmoothX != edge->m_edge_tag )
          return MYON_SUBD_RETURN_ERROR(false);
      }
    }
    else if (i+1 == N)
    {
      ...
      {
        if (  MYON_SubDEdgeTag::Smooth != edge1_tag || MYON_SubDEdgeTag::SmoothX != edge->m_edge_tag )
          return MYON_SUBD_RETURN_ERROR(false);
      }
      if ( MYON_SubDEdgeTag::Crease == edge1_tag)
        continue;
    }
    else
    {
    ...
    const MYON_SubDFace* face = component_ring[component_ring_index].Face();
    if ( nullptr == face)
      return MYON_SUBD_RETURN_ERROR(false);
  }
...
unsigned int MYON_SubD::GetSectorPointRing(
  ...
  )
{
  ...
  MYON_3dPoint* point_ring_array = point_ring.Reserve(point_ring_capacity);
...
unsigned int MYON_SubD::GetSectorSubdivsionPointRing(
  const MYON_SubDComponentPtr* component_ring,
  size_t component_ring_count,
...
unsigned int MYON_SubD::GetSectorSubdivisionPointRing(
  const MYON_SubDComponentPtr* component_ring,
  size_t component_ring_count,
  ...
  )
{
  ...
  MYON_3dPoint* subd_point_ring_array = subd_point_ring.Reserve(subd_point_ring_capacity);
  ...
  )
{
  ...
  MYON_SubDComponentPtr stack_component_ring[41];
  unsigned int component_ring_capacity = sizeof(stack_component_ring) / sizeof(stack_component_ring[0]);
  MYON_SubDComponentPtr* component_ring = stack_component_ring;
  if (component_ring_capacity < point_ring_capacity && component_ring_capacity < center_vertex_element_count)
  {
    ...
    component_ring = new(std::nothrow) MYON_SubDComponentPtr[component_ring_capacity];
    if ( nullptr == component_ring)
      return MYON_SUBD_RETURN_ERROR(0);
  }

  unsigned int point_ring_count = 0;
  unsigned int component_ring_count = MYON_SubD::GetSectorComponentRing(sit, component_ring,component_ring_capacity);
  if (component_ring_count > 0)
  {
    const bool bFirstPass = true;
    const bool bSecondPass = bSubdivideIfNeeded;
    point_ring_count = MYON_SubD::GetQuadSectorPointRing( bFirstPass, bSecondPass, nullptr, component_ring, component_ring_count, point_ring, point_ring_stride);
  }
...
const MYON_SubDVertex* MYON_SubD::SubdivideSector(
  const MYON_SubDVertex* center_vertex,
  const MYON_SubDComponentPtr* component_ring,
  size_t component_ring_count,
...
{
  const unsigned int N = (nullptr != center_vertex) ? center_vertex->m_edge_count : MYON_SubD::ComponentRingEdgeCount(component_ring_count);
  const unsigned int F = (nullptr != center_vertex) ? center_vertex->m_face_count : MYON_SubD::ComponentRingFaceCount(component_ring_count);
  if ( N < 2 )
  ...
  const MYON_SubDEdgePtr* edges;
  const MYON_SubDFacePtr* faces;

  if (nullptr == center_vertex)
  {
    if (nullptr == component_ring || component_ring_count < 4)
      return MYON_SUBD_RETURN_ERROR(nullptr);
    edges = (const MYON_SubDEdgePtr*)(component_ring + 1);
    faces = (const MYON_SubDFacePtr*)(component_ring + 2);
    element_stride = 2;
  ...
  else
  {
    ...
    faces = (const MYON_SubDFacePtr*)(center_vertex->m_faces);
    element_stride = 1;
  }
  ...
  const MYON_SubDEdgeTag edge0_tag = (F+1 == N) ? MYON_SubDEdgeTag::Crease : MYON_SubDEdgeTag::Smooth;

  //const unsigned int face_edge_count = 4;
  const unsigned int K = 3;
  
  const MYON_SubDEdge* edge0 = edges->Edge();
  if ( nullptr == edge0)
    return MYON_SUBD_RETURN_ERROR(nullptr);
  edges += element_stride;

  if (MYON_SubDEdgeTag::Smooth == edge0_tag)
  {
  ...
  const MYON_SubDFace* face0 = faces->Face();
  if ( nullptr == face0)
    return MYON_SUBD_RETURN_ERROR(nullptr);
  edges += element_stride;
  ...
  MYON_SubDVertex* v1[4] = {};
  MYON_SubDEdgePtr e1[4] = {};
  MYON_SubDEdgePtr f1epts[4] = {};
  ...
  if ( nullptr == v1[0])
    return MYON_SUBD_RETURN_ERROR(nullptr);
  //v1[0]->m_vertex_edge_order = MYON_SubD::VertexEdgeOrder::radial;

  MYON_SubDVertex* vertex1 = fsh.AllocateVertex(edge0);
  if ( nullptr == vertex1)
    return MYON_SUBD_RETURN_ERROR(nullptr);

  // at_crease weight is used when the cooresponding vertex is a crease.
  // Otherwise, fsh.AllocateEdge() ignores at_crease_weight.
  MYON_SubDEdgeTag edge1_tag = (MYON_SubDEdgeTag::SmoothX == edge0_tag) ? MYON_SubDEdgeTag::Smooth : edge0_tag;
  const double at_crease_weight 
    = MYON_SubDEdgeTag::Crease == edge1_tag
    ? MYON_SubDSectorType::CreaseSectorCoefficient(5-K)
    : MYON_SubDSectorType::IgnoredSectorCoefficient;
  MYON_SubDEdgePtr edge1 = fsh.AllocateEdge(v1[0], MYON_SubDSectorType::CopyEdgeSectorCoefficient(edge0,vertex0,MYON_UNSET_VALUE), vertex1, MYON_SubDSectorType::IgnoredSectorCoefficient );
  if (edge1.IsNull())
    return MYON_SUBD_RETURN_ERROR(nullptr);
  ...
  e1[0] = edge1;
  f1epts[0] = e1[0];
  edge1_tag = MYON_SubDEdgeTag::Smooth;
  ...
  for (unsigned int i = 1; i < N; i++, edges += element_stride, faces += element_stride)
  {
    ...
    if (i + 1 == N)
    {
      ...
      if ( edge1_tag != edge0->m_edge_tag)
        return MYON_SUBD_RETURN_ERROR(nullptr);
      if (MYON_SubDEdgeTag::Smooth == edge1_tag)
      {
        v1[K] = vertex1;
        e1[K] = edge1;
      }
    ...
    if (nullptr == v1[K])
    {
      ...
      if (nullptr == v1[K])
        return MYON_SUBD_RETURN_ERROR(nullptr);
      e1[K] = fsh.AllocateEdge(v1[0], MYON_SubDSectorType::CopyEdgeSectorCoefficient(edge0, vertex0, MYON_UNSET_VALUE), v1[K], MYON_SubDSectorType::IgnoredSectorCoefficient);
      if (e1[K].IsNull())
        return MYON_SUBD_RETURN_ERROR(nullptr);
    ...
    e1[1] = fsh.AllocateEdge(v1[1], at_crease_weight, v1[2], MYON_SubDSectorType::IgnoredSectorCoefficient);
    e1[2] = fsh.AllocateEdge(v1[2], MYON_SubDSectorType::IgnoredSectorCoefficient, v1[3], at_crease_weight);
    f1epts[1] = e1[1];
    f1epts[2] = e1[2];
    ...
    v1[K] = nullptr;
    e1[K] = MYON_SubDEdgePtr::Null;
  }

  return MYON_SUBD_RETURN_ERROR(nullptr);
}

unsigned int MYON_SubD::GetSectorComponentRing(
  const class MYON_SubDSectorIterator& sit,
  ...
  )
{
  ...
  const MYON_SubDVertex* vertex = sit.CenterVertex();
  if ( nullptr == vertex || vertex->m_edge_count < 2 || vertex->m_face_count < 1)
    return MYON_SUBD_RETURN_ERROR(0);

  const MYON_SubDVertexTag center_vertex_tag = vertex->m_vertex_tag;

  MYON_SubDSectorIterator localsit(sit);
  const bool bCreases = (nullptr != localsit.IncrementToCrease(-1));
  
  MYON_SubDEdgePtr edgeptr = localsit.CurrentEdgePtr(0);
  MYON_SubDFacePtr faceptr = localsit.CurrentFacePtr();

  const MYON_SubDEdge* edge0 = edgeptr.Edge();
  if ( nullptr == edge0 )
    return MYON_SUBD_RETURN_ERROR(0);
  const MYON_SubDFace* face0 = faceptr.Face();
  if ( nullptr == face0 )
    return MYON_SUBD_RETURN_ERROR(0);
  const MYON_SubDVertex* ring_vertex0 = localsit.CurrentEdgeRingVertex(0);
  if ( nullptr == ring_vertex0 || vertex == ring_vertex0)
    return MYON_SUBD_RETURN_ERROR(0);

  if (bCreases && MYON_SubDEdgeTag::Crease != edge0->m_edge_tag)
    return MYON_SUBD_RETURN_ERROR(0);

  unsigned int component_ring_count = 0;
  component_ring[component_ring_count++] = MYON_SubDComponentPtr::Create(vertex);
  component_ring[component_ring_count++] = MYON_SubDComponentPtr::Create(edgeptr);
  component_ring[component_ring_count++] = MYON_SubDComponentPtr::Create(faceptr);
  const unsigned int N = vertex->m_edge_count; // for () used to prevent infinite recursion when vertex is not valid
  for (unsigned int i = 0; i < N; i++)
  {
    const MYON_SubDFace* face = localsit.NextFace(MYON_SubDSectorIterator::StopAt::AnyCrease);

    edgeptr = localsit.CurrentEdgePtr(0);
    const MYON_SubDEdge* edge = edgeptr.Edge();
    if ( nullptr == edge)
      return MYON_SUBD_RETURN_ERROR(0);
    const MYON_SubDVertex* ring_vertex = localsit.CurrentEdgeRingVertex(0);
    ...
    if (face == face0 || edge == edge0 || ring_vertex == ring_vertex0)
    {
      ...
      if (edge == edge0 && ring_vertex == ring_vertex0)
      {
        if (MYON_SubDVertexTag::Smooth == center_vertex_tag)
        {
          if (face == face0 && MYON_SubDEdgeTag::Smooth == edge0->m_edge_tag)
            return component_ring_count; // back to start smooth case.
        }
        if (MYON_SubDVertexTag::Dart == center_vertex_tag)
        {
          if (nullptr == face && MYON_SubDEdgeTag::Crease == edge0->m_edge_tag)
            return component_ring_count; // back to start dart case.
        }
        if (MYON_SubDVertexTag::Corner == center_vertex_tag)
        {
      ...
      return MYON_SUBD_RETURN_ERROR(false); // damaged topology information
    }
    ...
    if (nullptr == face)
    {
      if (bCreases && MYON_SubDEdgeTag::Crease == edge->m_edge_tag)
...
unsigned int MYON_SubD::GetSectorComponentRing(
  const class MYON_SubDSectorIterator& sit,
  MYON_SimpleArray<MYON_SubDComponentPtr>& elements
  )
{
  elements.SetCount(0);
  const MYON_SubDVertex* vertex = sit.CenterVertex();
  ...
  unsigned int component_ring_count = MYON_SubD::GetSectorComponentRing(sit, elements.Reserve(component_ring_capacity), component_ring_capacity);
