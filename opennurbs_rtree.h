...
// in MYON_RTreeNode::m_branch[].
// must have MYON_RTree_MAX_NODE_COUNT > MYON_RTree_MIN_NODE_COUNT
#define MYON_RTree_MIN_NODE_COUNT 2
#define MYON_RTree_MAX_NODE_COUNT 6

/*
In a test of a sphere mesh with mesh: 8385 vertices, 8192 polygons
and MYON_RTree_MAX_NODE_COUNT = 3, 4, 5, and 6, the memory use was 
most efficient with MYON_RTree_MAX_NODE_COUNT=6

Memory Usage MAX_NODE_COUNT = 3
  MYON_RTree: 1212 KB (1241136) (352 wasted)
  MYON_RTree: 7036 nodes, 5881 unused branches (321 KB) 0.835844 per node

Memory Usage MAX_NODE_COUNT = 4
  MYON_RTree: 1152 KB (1179720) (5568 wasted)
  MYON_RTree: 5051 nodes, 6962 unused branches (380 KB) 1.37834 per node

Memory Usage MAX_NODE_COUNT = 5
  MYON_RTree: 1040 KB (1065504) (11808 wasted)
  MYON_RTree: 3655 nodes, 6429 unused branches (351 KB) 1.75896 per node

Memory Usage MAX_NODE_COUNT = 6
  MYON_RTree:  995 KB (1019592) (3440 wasted)
  MYON_RTree: 2951 nodes, 6564 unused branches (358 KB) 2.22433 per node
*/

// This struct is used instead of MYON_BoundingBox to avoid calling
// constructors.
struct MYON_RTreeBBox
{
  double m_min[3];
  double m_max[3];
};

struct MYON_RTreeSphere
{
  double m_point[3];
  double m_radius;
};

struct MYON_RTreeCapsule
{
  double m_point[2][3];
  double m_radius;
  double m_domain[2];
...
struct MYON_RTreeBranch
{
  MYON_RTreeBBox m_rect;

  // If MYON_RTreeNode.m_level > 0, then m_child points to a child node.
  // If MYON_RTreeNode.m_level == 0, then m_id identifies the leaf element.
  union
  {
    struct MYON_RTreeNode* m_child;
    MYON__INT_PTR m_id;
  };
};

struct MYON_RTreeLeaf
{
  MYON_RTreeBBox m_rect;
  MYON__INT_PTR m_id;
};

// The MYON_RTreeNode is used at root, branch and leaf nodes.
// When m_level > 0, the node is a branch.
// When m_level = 0, the node is a leaf.
struct MYON_RTreeNode
{
  ...
  // 0 <= m_count <= MYON_RTree_MAX_NODE_COUNT
  // m_count must be a signed int to insure signed compares work correctly
  int m_count; 
  MYON_RTreeBranch m_branch[MYON_RTree_MAX_NODE_COUNT];
};

// Passes data about the polyline being intersected
struct MYON_RTreePolylineContext
{
  unsigned int m_polyline_pointindex;
};

struct MYON_RTreeSearchResult
{
  int m_capacity;   // m_id[] array capacity (search terminates when m_count == m_capacity)
  int m_count;      // number of elements in m_id[]
  MYON__INT_PTR* m_id; // m_id[] = array of search results.
};

class MYON_CLASS MYON_RTreeMemPool
{
public:
  static const MYON_RTreeMemPool Empty;

  MYON_RTreeMemPool() = default;

  MYON_RTreeMemPool( size_t leaf_count );
  ~MYON_RTreeMemPool();

  MYON_RTreeNode* AllocNode();
  void FreeNode(MYON_RTreeNode* node);

  struct MYON_RTreeListNode* AllocListNode();
  void FreeListNode(struct MYON_RTreeListNode* list_node);
...
private:
  ...
  // linked list of unused MYON_RTreeNode 
  struct Blk* m_nodes = nullptr;
  // linked list of unused MYON_RTreeListNode
...
// MYON_RTreeIterator
//
//   The MYON_RTreeIterator class can be used to iterate each leaf 
//   in an MYON_RTree.
//
class MYON_CLASS MYON_RTreeIterator
{
public:
  /*
  Description:
  ...
  Remark:
    Any calls to MYON_RTree::Insert() or MYON_RTree::Remove() that modify
  ...
  */
  MYON_RTreeIterator();
  MYON_RTreeIterator(const class MYON_RTree& a_rtree);
  ...
  Parameters:
    a_rtree - [in]
      R-tree to iterate
  Example:
    See the comment for MYON_RTreeIterator::First().
  Returns:
  ...
  Remarks:
    ...
    iterator.    
  */
  bool Initialize(const class MYON_RTree& a_rtree);
  ...
  Remarks:
    ...
    iterator.    
  */
  bool Initialize(const struct MYON_RTreeNode* a_node);
  ...
  */
  const MYON_RTreeBranch* Value() const;
  ...
  Example:
    Iterate every leaf in an R-tree.
          ...
          MYON_RTree rtree;
          ...
          MYON_RtreeIterator rit(rtree);
          const MYON_RTreeBranch* rtree_leaf;
  ...
  */
  ...
  Example:
    See the comment for MYON_RTreeIterator::Last()
  Returns:
    True if a call to Value() will return a non-null pointer.
    False if there is not a previous leaf and all susequent calls to
    Value() will return null.
  See Also:
    MYON_RTreeIterator::Next();
  */
  bool Prev();
...
private:
  ...
  struct StackElement
  {
    const struct MYON_RTreeNode* m_node;
  ...
  const MYON_RTreeNode* m_root;       // root of tree being iterated
};


class MYON_CLASS MYON_RTree
{
public:
  static const MYON_RTree Empty;

  MYON_RTree( size_t leaf_count = 0 );
  ~MYON_RTree();
  ...
  Returns:
    True if successful.
  */
  bool CreateMeshFaceTree( const class MYON_Mesh* mesh );
  ...
  Remarks:
    Calling Insert() or Remove() invalidates any MYON_RTreeIterator
  ...
  */
  ...
  Parameters:
    a_rect - [in/out]
      The version of search that has MYON_RTreeBBox* a_rect as the first
    ...
    a_sphere - [in/out]
      The version of search that has MYON_RTreeSphere* a_sphere as the first
    ...
    a_capsule - [in/out]
      The version of search that has MYON_RTreeSphere* a_capsule as the first
  ...
  */
  ...
  bool Search(const double a_min[3], const double a_max[3],
    bool MYON_CALLBACK_CDECL resultCallback(void* a_context, MYON__INT_PTR a_id), void* a_context
    ) const;

	bool Search(const double a_min[3], const double a_max[3],
    MYON_RTreeSearchResult& a_result 
    ) const;
  ...
  Remarks:
    ...
    MYON_RTree::Search(double tolerance, ... results )
    member functions.
  */
  static bool Search( 
  ...
  */
  const MYON_RTreeNode* Root() const;
...
private:
  void SplitNode(MYON_RTreeNode*, MYON_RTreeBranch*, MYON_RTreeNode**);
  bool AddBranch(MYON_RTreeBranch*, MYON_RTreeNode*, MYON_RTreeNode**);
  bool InsertRectRec(MYON_RTreeBBox*, MYON__INT_PTR, MYON_RTreeNode*, MYON_RTreeNode**, int);
  bool InsertRect(MYON_RTreeBBox*, MYON__INT_PTR, MYON_RTreeNode**, int);
  void LoadNodes(MYON_RTreeNode*, MYON_RTreeNode*, struct MYON_RTreePartitionVars*);
  bool RemoveRect(MYON_RTreeBBox*, MYON__INT_PTR, MYON_RTreeNode**);
  bool RemoveRectRec(MYON_RTreeBBox*, MYON__INT_PTR, MYON_RTreeNode*, struct MYON_RTreeListNode**);
  void ReInsert(MYON_RTreeNode*, struct MYON_RTreeListNode**);
  void RemoveAllRec(MYON_RTreeNode*);
  MYON_RTreeNode* m_root = nullptr;
  size_t m_reserved = 0;
  MYON_RTreeMemPool m_mem_pool;
};
