...
enum : unsigned char
{
  MYON_SubDComponentArchiveAnonymousChunkMark = 254U,
  MYON_SubDComponentArchiveAdditionEndMark = 255U
};

static bool Internal_ReadComponentAdditionSize(MYON_BinaryArchive& archive, unsigned char valid_sz, unsigned char* sz)
{
  ...
  if (false == archive.ReadChar(sz))
    return MYON_SUBD_RETURN_ERROR(false);
  if ( 0 != *sz && valid_sz != *sz && MYON_SubDComponentArchiveAdditionEndMark != *sz )
    return MYON_SUBD_RETURN_ERROR(false);
  return true;
}

static bool Internal_WriteComponentAdditionSize(bool bHaveAddition, MYON_BinaryArchive& archive, unsigned char sz)
{
  if (archive.Archive3dmVersion() < 70)
  {
    return MYON_SUBD_RETURN_ERROR(false);
...
static bool Internal_FinishReadingComponentAdditions(MYON_BinaryArchive& archive)
{
  if (archive.Archive3dmVersion() < 70)
  {
    return MYON_SUBD_RETURN_ERROR(false);
  ...
  for (;;)
  {
    if (MYON_SubDComponentArchiveAdditionEndMark == sz)
      return true;
    if (MYON_SubDComponentArchiveAnonymousChunkMark == sz)
    {
...
static bool Internal_FinishWritingComponentAdditions(MYON_BinaryArchive& archive)
{
  if (archive.Archive3dmVersion() < 70)
    return MYON_SUBD_RETURN_ERROR(false);
  const unsigned char sz = MYON_SubDComponentArchiveAdditionEndMark;
...
static bool Internal_WriteArchiveIdAndFlags(
  ...
  MYON__UINT_PTR ptr_flags,
  MYON_BinaryArchive& archive
)
{
  if (!archive.WriteInt(archive_id))
    return MYON_SUBD_RETURN_ERROR(false);
  unsigned char flags = (unsigned char)MYON_SUBD_COMPONENT_FLAGS(ptr_flags);
...
)
{
  ...
  element_ptr *= (MYON_SUBD_COMPONENT_FLAGS_MASK + 1);
  element_ptr += (flags & MYON_SUBD_COMPONENT_FLAGS_MASK);
  return true;
}

static bool Internal_WritesSymmetrySetNext(
  const MYON_SubDComponentBase& c,
  MYON_BinaryArchive& archive
)
{
  const MYON_SubDComponentPtr symmetry_set_next = MYON_SubDArchiveIdMap::SymmetrySetNextForExperts(c);
  const MYON_SubDComponentBase* next_c = symmetry_set_next.ComponentBase();
...
{
  return Internal_ReadArchiveIdAndFlagsIntoComponentPtr(archive, MYON_SubDArchiveIdMap::SymmetrySetNextForExperts(c).m_ptr);
}

static bool WriteBase(
  const MYON_SubDComponentBase* base,
  MYON_BinaryArchive& archive
  )
{
  for (;;)
...
static bool Internal_ReadSavedLimitPointList(
  MYON_BinaryArchive& archive,
  unsigned int vertex_face_count,
  MYON_SimpleArray< MYON_SubDSectorSurfacePoint > limit_points
  )
{
  ...
  for (;;)
  {
    ...
    {
      MYON_SubDSectorSurfacePoint limit_point = MYON_SubDSectorSurfacePoint::Unset;
      ...
      if (!Internal_ReadDouble3(archive,limit_point.m_limitN))
        break;
      MYON_SubDFacePtr fptr = MYON_SubDFacePtr::Null;
...
static bool Internal_WriteVertexList(
  ...
  const MYON_SubDVertex*const* vertex,
  MYON_BinaryArchive& archive
  )
{
  for (;;)
  {
    MYON_SubDArchiveIdMap::ValidateArrayCounts(vertex_count,vertex_count,vertex,0,nullptr);
    ...
    const MYON__UINT_PTR ptr_flags = 0; // for future use
    unsigned short i = 0;
  ...
  )
{
  for (;;)
  {
    ...
    {
      MYON_ERROR("Archive vertex count != expected vertex count.");
      if ( archive_vertex_count < vertex_count)
        vertex_count = archive_vertex_count;
    }

    MYON_SubDArchiveIdMap::ValidateArrayCounts(vertex_count,vertex_capacity,vertex,0,nullptr);

    unsigned short i = 0;
    for (i = 0; i < vertex_count; i++)
    {
      MYON__UINT_PTR vptr = 0;
      if (!Internal_ReadArchiveIdAndFlagsIntoComponentPtr(archive,vptr))
        break;
      vertex[i] = (MYON_SubDVertex*)vptr;
    }
    if ( i < vertex_count )
      break;
...
static bool Internal_WriteEdgePtrList(
  ...
  const MYON_SubDEdgePtr* edgeN,
  unsigned short edgeX_capacity,
  const MYON_SubDEdgePtr* edgeX,
  MYON_BinaryArchive& archive
  )
{
  for (;;)
  {
    MYON_SubDArchiveIdMap::ValidateArrayCounts(edge_count,edgeN_capacity,edgeN,edgeX_capacity,edgeX);
    ...
    const MYON_SubDEdgePtr* eptr = edgeN;
    unsigned short i = 0;
  ...
  )
{
  for (;;)
  {
    ...
    {
      MYON_ERROR("Archive edge count != expected edge count.");
      if ( archive_edge_count < edge_count)
...
static bool Internal_WriteFacePtrList(
  ...
  const MYON_SubDFacePtr* faceN,
  unsigned short faceX_capacity,
  const MYON_SubDFacePtr* faceX,
  MYON_BinaryArchive& archive
  )
{
  for (;;)
  {
    MYON_SubDArchiveIdMap::ValidateArrayCounts(face_count,faceN_capacity,faceN,faceX_capacity,faceX);
    ...
    const MYON_SubDFacePtr* fptr = faceN;
    unsigned short i = 0;
    for (i = 0; i < face_count; i++, fptr++)
    {
      if ( i == faceN_capacity)
        fptr = faceX;
      const MYON_SubDFace* face = MYON_SUBD_FACE_POINTER(fptr->m_ptr);
  ...
  )
{
  for (;;)
  {
    ...
    {
      MYON_ERROR("Archive face count != expected face count.");
      if ( archive_face_count < face_count)
...
MYON_SubDComponentPtr& MYON_SubDArchiveIdMap::SymmetrySetNextForExperts(const MYON_SubDComponentBase& c)
{
  return const_cast<MYON_SubDComponentPtr&>(c.m_symmetry_set_next);
}

bool MYON_SubDVertex::Write(
  MYON_BinaryArchive& archive
  ) const
{
  for (;;)
  {
    ...
    if (!Internal_WriteEdgePtrList(m_edge_count,m_edge_capacity,m_edges,0,nullptr, archive))
      break;
    if (!Internal_WriteFacePtrList(m_face_count,m_face_capacity,(const MYON_SubDFacePtr*)m_faces,0,nullptr, archive))
      break;
  ...
  )
{
  ...
  for (;;)
  {
    MYON_SubDimple* subdimple = const_cast<MYON_SubDimple*>(subd.SubDimple());
    if ( nullptr == subdimple)
      break;

    MYON_SubDComponentBase base = MYON_SubDComponentBase::Unset;
    ...
    MYON_SubDVertex* v = subdimple->AllocateVertex(
      base.m_id, // serialization must preserve MYON_SubDVertex.m_id
      MYON_SubD::VertexTagFromUnsigned(vertex_tag),
    ...
    v->MYON_SubDComponentBase::operator=(base);

    if (!Internal_ReadEdgePtrList(archive,edge_count,v->m_edge_capacity,v->m_edges,0,nullptr))
      break;
    v->m_edge_count = edge_count;

    if (!Internal_ReadFacePtrList(archive,face_count,v->m_face_capacity,(MYON_SubDFacePtr*)v->m_faces,0,nullptr))
      break;
    v->m_face_count = face_count;

    for (unsigned int i = 0; i < limit_points.UnsignedCount(); i++)
    {
      MYON_SubDSectorSurfacePoint limit_point = limit_points[i];
      limit_point.m_next_sector_limit_point = (const MYON_SubDSectorSurfacePoint*)1U; // skips checks
  ...
  )
{
  ...
  for (;;)
  {
    ...
    MYON_SubDVertex* v[2] = { 0 };
    unsigned short vertex_count = 2;
    if (!Internal_ReadVertexList(archive, vertex_count, 2, v))
      break;

    MYON_SubDEdge* e = subdimple->AllocateEdge(
      base.m_id, // serialization must preserve MYON_SubDEdge.m_id
      MYON_SubD::EdgeTagFromUnsigned(edge_tag),
      base.SubdivisionLevel(),
      face_count
      );
    ...
    e->MYON_SubDComponentBase::operator=(base);
    ...
    if (archive.Archive3dmVersion() >= 80)
    {
      ...
      if (false == archive.ReadDouble(&sharpness1))
        break;
      if (e->IsSmooth())
        e->SetSharpnessForExperts(MYON_SubDEdgeSharpness::FromInterval(sharpness0,sharpness1));
    }
  ...
  ) const
{
  for (;;)
  {
    ...
    if (bWritePackRect)
    {
      const unsigned char obsolete_per_face_texture_coordinate_type = MYON_SubD::ObsoleteTextureDomainTypeFromTextureCoordinateType(MYON_SubDTextureCoordinateType::Packed);
      ...
      if (!archive.WriteChar(packing_rot_dex))
        break;
      const MYON_2dPoint pack_rect_origin = PackRectOrigin();
      if (!archive.WriteDouble(2, &pack_rect_origin.x))
        break;
      const MYON_2dVector pack_rect_size = PackRectSize();
    ...
    const bool bWriteMaterialChannelIndex = (material_channel_index > 0 && material_channel_index <= MYON_Material::MaximumMaterialChannelIndex);
    ...
    const MYON_Color per_face_color = PerFaceColor();
    const bool bWritePerFaceColor = (MYON_Color::UnsetColor != per_face_color);
    ...
    if (bTexturePoints)
    {
      ...
      // So, texture points are saved in 10 point chunks ( 10*sizeof(MYON_3dPoint) = 240 <= 243.
      ...
      const MYON_3dPoint* a = this->m_texture_points;
      const unsigned char sizeof_ten_points = (unsigned char)(10 * sizeof(MYON_3dPoint)); // sizeof_ten_points = 240 <= 243
      ...
      {
        const unsigned char sizeof_left_over_points = (unsigned char)(left_over_points_count * sizeof(MYON_3dPoint)); // sizeof_left_over_points < 240
...
)
{
  ...
  for (;;)
  {
    ...
    MYON_SubDFace* f = subdimple->AllocateFace(
      base.m_id, // serialzation must preserve MYON_SubDFace.m_id
      base.SubdivisionLevel(),
      edge_count
    );
    ...
    f->MYON_SubDComponentBase::operator=(base);
    ...
    if (0 != sz)
    {
      ...
      MYON_2dPoint pack_rect_origin(MYON_2dPoint::Origin);
      if (!archive.ReadDouble(2, &pack_rect_origin.x))
        break;
      MYON_2dVector pack_rect_delta(MYON_2dVector::ZeroVector);
      if (!archive.ReadDouble(2, &pack_rect_delta.x))
        break;

      if (MYON_SubDFace::IsValidPackRect(pack_rect_origin, pack_rect_delta, packing_rot) )
    ...
    if (0 != sz)
    {
      ...
      MYON_Color per_face_color = MYON_Color::UnsetColor;
    ...
    if (0 != sz)
    {
      ...
      MYON_3dPoint* tp = f->m_texture_points;
      ...
      if (false == bContinue)
        break;
      if ( nullptr != tp)
        f->m_texture_status_bits |= MYON_SubDFace::TextureStatusBits::TexturePointsSet;
    }
...
unsigned int MYON_SubDLevel::SetArchiveId(
  ...
  ) const
{
  ...
  const MYON_SubDComponentPtr::Type component_type[3] = {
    MYON_SubDComponentPtr::Type::Vertex,
    MYON_SubDComponentPtr::Type::Edge,
    MYON_SubDComponentPtr::Type::Face
  };

  const MYON_SubDComponentBaseLink* first_link[3] = {
    (const MYON_SubDComponentBaseLink*)m_vertex[0],
    (const MYON_SubDComponentBaseLink*)m_edge[0],
    (const MYON_SubDComponentBaseLink*)m_face[0]
  };
  ...
  for (unsigned int listdex = 0; listdex < 3; listdex++)
  {
    ...
    for (const MYON_SubDComponentBaseLink* clink = first_link[listdex]; nullptr != clink; clink = clink->m_next)
    {
      ++linked_list_count;
      ...
      prev_id = 0;
      for (const MYON_SubDComponentBase* c = cidit.FirstComponent(); nullptr != c; c = cidit.NextComponent())
      {
        if (prev_id >= c->m_id)
        {
          // This is a serious error!
          // Contine because this allows us to save something do the disk in these bad cases.
          MYON_SUBD_ERROR("The m_id values of the active components in the fixed size pool are corrupt.");
        }
        else
        {
          prev_id = c->m_id;
      ...
      if (cidit_level_count != linked_list_count)
      {
        // This is a serious error!
        // Contine because this allows us to save something do the disk in these bad cases.
        MYON_SUBD_ERROR("The m_level values of the active components in the fixed size pool are corrupt.");
      }
      break;
    }
  }
...
void MYON_SubDLevel::ClearArchiveId() const
{
  ...
  ) const
{
  ...
  for (;;)
  {
    ...
    MYON_BoundingBox bbox = m_aggregates.m_bDirtyBoundingBox ? MYON_BoundingBox::EmptyBoundingBox : m_aggregates.m_controlnet_bbox;
    ...
    MYON_SubDLevelComponentIdIterator idit;

    // must iterate vertices in order of increasing id
    idit.Initialize(bLevelLinkedListIncreasingId[0], MYON_SubDComponentPtr::Type::Vertex, subdimple, *this);
    ...
    idit.Initialize(bLevelLinkedListIncreasingId[1], MYON_SubDComponentPtr::Type::Edge, subdimple, *this);
    ...
    idit.Initialize(bLevelLinkedListIncreasingId[2], MYON_SubDComponentPtr::Type::Face, subdimple, *this);
    ...
    unsigned char c = 0;
    if (archive.Save3dmRenderMesh(MYON::object_type::subd_object) || archive.Save3dmAnalysisMesh(MYON::object_type::subd_object))
    {
      // no reason to save the m_control_net_mesh
      if (false == m_surface_mesh.IsEmpty())
      {
        c = 0;
        // c = 1; TODO change to c = 1 when MYON_SubDMesh::Write()/Read() actually work
      }
    }
  ...
  )
{
  ...
  for (;;)
  {
    ...
    MYON_BoundingBox controlnet_bbox;
    ...
    {
      MYON_SubDVertex* v = nullptr;
      if ( false == MYON_SubDVertex::Read(archive, subd, v) )
        break;
      if ( nullptr == v )
        break;
    ...
    for (archive_id = element_list.m_archive_id_partition[1]; archive_id < element_list.m_archive_id_partition[2]; archive_id++ )
    {
      ...
      if ( false == MYON_SubDEdge::Read(archive, subd, e) )
        break;
      if ( nullptr == e )
        break;
      ...
      if (
        archive.ArchiveOpenNURBSVersion() <= MYON_VersionNumberConstruct(7, 16, 2099, 12, 31, 6)
        && (e->m_edge_tag == MYON_SubDEdgeTag::Smooth || e->m_edge_tag == MYON_SubDEdgeTag::SmoothX)
        )
      {
        static const MYON_String format{
        ...
        {
          MYON_SubDVertex* vp{ const_cast<MYON_SubDVertex*>(e->m_vertex[evi]) };
          if (!element_list.ConvertArchiveIdToRuntimeVertexPtr(1, 1, &vp)) continue;
          if (vp->m_vertex_tag == MYON_SubDVertexTag::Corner)
          {
            MYON_String msg{};
    ...
    {
      MYON_SubDFace* f = nullptr;
      if ( false == MYON_SubDFace::Read(archive, subd, f) )
        break;
      if ( nullptr == f )
        break;
...
bool MYON_SubDimple::Write(
  MYON_BinaryArchive& archive
  ) const
{
  const_cast< MYON_SubDHeap* >(&m_heap)->ClearArchiveId();
  ...
  for (;;)
  {
    ...
    if (!archive.WriteBoundingBox(MYON_BoundingBox::EmptyBoundingBox))
      break;
    ...
    const unsigned char obsolete_texture_domain_type = MYON_SubD::ObsoleteTextureDomainTypeFromTextureCoordinateType(TextureCoordinateType());
    ...
    const MYON__UINT64 gsn = GeometryContentSerialNumber();
  ...
  )
{
  ...
  for (;;)
  {
    ...
    MYON_BoundingBox bbox_unsued_after_May_2015;
    if (!archive.ReadBoundingBox(bbox_unsued_after_May_2015))
      break;

    MYON_SubDArchiveIdMap element_list;
    ...
    if (minor_version >= 1)
    {
      ...
      if (false == archive.ReadChar(&obsolete_texture_domain_type))
        break;      
      m_texture_coordinate_type = MYON_SubD::TextureCoordinateTypeFromObsoleteTextureDomainType(obsolete_texture_domain_type);
      ...
      if (minor_version >= 2)
      {
        ...
        {
          // 
          MYON__UINT64 legacy_gsn_at_save_time = 0;
  ...
  {
    MYON_SUBD_ERROR("m_heap.IsValid() is false.");
  ...
  if (archive.ArchiveOpenNURBSVersion() < 2382394661)
  {
    ...
    const MYON_MappingTag file_tag = this->TextureMappingTag(true);
    MYON_MappingTag new_tag = file_tag;
    
    const MYON_SubDTextureCoordinateType file_type = this->TextureCoordinateType();
    MYON_SubDTextureCoordinateType new_type = file_type;

    if (MYON_TextureMapping::TYPE::srfp_mapping == file_tag.m_mapping_type)
    {
      new_tag = MYON_MappingTag::SurfaceParameterMapping;
      if (MYON_SubDTextureCoordinateType::FromMapping == file_type || MYON_SubDTextureCoordinateType::Unset == file_type)
        new_type = MYON_SubDTextureCoordinateType::Packed;
    }
    else
    {
      const bool bTagIsSet
        = MYON_TextureMapping::TYPE::srfp_mapping != file_tag.m_mapping_type
        && MYON_TextureMapping::TYPE::no_mapping != file_tag.m_mapping_type
        && file_tag.IsSet()
        ;
      if (MYON_SubDTextureCoordinateType::Unset == file_type)
      {
        if (bTagIsSet)
          new_type = MYON_SubDTextureCoordinateType::FromMapping;
        else
          new_tag = MYON_MappingTag::Unset;
      }
      else if (MYON_SubDTextureCoordinateType::FromMapping == file_type)
      {
        if (false == bTagIsSet)
        {
          new_tag = MYON_MappingTag::Unset;
    ...
    if (0 != MYON_MappingTag::CompareAll(file_tag, new_tag))
...
MYON_OBJECT_IMPLEMENT(MYON_SubDMeshProxyUserData,MYON_UserData,"2868B9CD-28AE-4EA7-8073-BD390B3E97C8");

const bool MYON_SubDMeshProxyUserData::Internal_MeshHasFaces(const MYON_Mesh* mesh)
{
  for (;;)
  {
    if (nullptr == mesh)
...
const MYON_SHA1_Hash MYON_SubDMeshProxyUserData::Internal_FaceSHA1(const MYON_Mesh* mesh)
{
  if (false == MYON_SubDMeshProxyUserData::Internal_MeshHasFaces(mesh))
...
const MYON_SHA1_Hash MYON_SubDMeshProxyUserData::Internal_VertexSHA1(const MYON_Mesh* mesh)
{
  if (false == MYON_SubDMeshProxyUserData::Internal_MeshHasFaces(mesh))
    return MYON_SHA1_Hash::EmptyContentHash;

  MYON_SHA1 sha1;
  const MYON_3fPoint* v = mesh->m_V.Array();
  sha1.AccumulateBytes(v, mesh->m_V.UnsignedCount() * sizeof(*v));
  return sha1.Hash();
}


void MYON_SubDMeshProxyUserData::Internal_CopyFrom(const MYON_SubDMeshProxyUserData& src)
{
  if ( src.IsValid() )
  {
    m_subd = new MYON_SubD(*src.m_subd);
...
void MYON_SubDMeshProxyUserData::Internal_Destroy()
{
  if (nullptr != m_subd)
  {
    delete m_subd;
  ...
  m_mesh_face_array_sha1 = MYON_SHA1_Hash::EmptyContentHash;
  m_mesh_vertex_array_sha1 = MYON_SHA1_Hash::EmptyContentHash;
}


const MYON_SubDDisplayParameters MYON_SubDMeshProxyUserData::MeshProxyDisplayParameters()
{
  return MYON_SubDDisplayParameters::CreateFromDisplayDensity(4);

}

MYON_Mesh* MYON_SubDMeshProxyUserData::MeshProxyFromSubD(
  const MYON_SubD* subd
)
{
  MYON_Mesh* mesh = nullptr;
  MYON_SubD* subd_copy = nullptr;

  for (;;)
  {
    if (nullptr == subd)
      break;
    subd_copy = new MYON_SubD(*subd);
    if (nullptr == subd_copy)
      break;
    mesh = subd_copy->GetControlNetMesh(nullptr, MYON_SubDGetControlNetMeshPriority::Geometry);
    if (false == MYON_SubDMeshProxyUserData::Internal_MeshHasFaces(mesh))
      break;
    MYON_SubDMeshProxyUserData* ud = new MYON_SubDMeshProxyUserData();
    ud->m_subd = subd_copy;
    ud->m_mesh_face_count = mesh->FaceUnsignedCount();
    ud->m_mesh_vertex_count = mesh->VertexUnsignedCount();
    ud->m_mesh_face_array_sha1 = MYON_SubDMeshProxyUserData::Internal_FaceSHA1(mesh);
    ud->m_mesh_vertex_array_sha1 = MYON_SubDMeshProxyUserData::Internal_VertexSHA1(mesh);
...
MYON_SubD* MYON_SubDMeshProxyUserData::SubDFromMeshProxy(
  const MYON_Mesh* mesh
)
{
  MYON_SubD* subd = nullptr;
  MYON_SubDMeshProxyUserData* ud = nullptr;
  
  for (;;)
  {
    if (nullptr == mesh)
      break;
    const MYON_UUID udid = MYON_CLASS_ID(MYON_SubDMeshProxyUserData);
    ud = MYON_SubDMeshProxyUserData::Cast(mesh->GetUserData(udid));
    if (nullptr == ud)
      break;
    if (false == ud->IsValid())
      break;
...
bool MYON_SubDMeshProxyUserData::IsSubDMeshProxy(
  const MYON_Mesh* mesh
)
{
  return false;
...
MYON_SubDMeshProxyUserData::MYON_SubDMeshProxyUserData()
{
  m_userdata_uuid = MYON_CLASS_ID(MYON_SubDMeshProxyUserData);
  m_application_uuid = MYON_opennurbs6_id; // opennurbs.dll reads/writes this userdata
...
MYON_SubDMeshProxyUserData::MYON_SubDMeshProxyUserData(const MYON_SubDMeshProxyUserData& src)
  : MYON_UserData(src)
{
...
MYON_SubDMeshProxyUserData& MYON_SubDMeshProxyUserData::operator=(const MYON_SubDMeshProxyUserData& src)
{
  if (this != &src)
  {
    Internal_Destroy();
...
bool MYON_SubDMeshProxyUserData::Write(MYON_BinaryArchive& archive) const
{
...
bool MYON_SubDMeshProxyUserData::Read(MYON_BinaryArchive& archive)
{
  Internal_Destroy();
...
bool MYON_SubDMeshProxyUserData::ParentMeshValid() const
{
  for (;;)
  {
    if (!IsValid())
    ...
    const MYON_SHA1_Hash f_sha1 = MYON_SubDMeshProxyUserData::Internal_FaceSHA1(mesh);
    if (f_sha1 != m_mesh_face_array_sha1)
      break;
    
    const MYON_SHA1_Hash v_sha1 = MYON_SubDMeshProxyUserData::Internal_VertexSHA1(mesh);
...
bool MYON_SubDMeshProxyUserData::IsValid(
...
) const
{
  for (;;)
  {
    ...
    if (m_mesh_vertex_count <= 2 )
      break;
    if (MYON_SHA1_Hash::EmptyContentHash == m_mesh_face_array_sha1)
      break;
    if (MYON_SHA1_Hash::EmptyContentHash == m_mesh_vertex_array_sha1)
      break;
...
bool MYON_SubDMeshProxyUserData::GetDescription(MYON_wString& description)
{
  if (IsValid())
...
bool MYON_SubDMeshProxyUserData::WriteToArchive(
