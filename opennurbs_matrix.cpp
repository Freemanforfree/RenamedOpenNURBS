...
//    changed MYON_Matrix to use multiple allocations
//    for coefficient memory in large matrices.

// MYON_Matrix.m_cmem points to a struct DBLBLK
struct DBLBLK
{
  int count;
  double* a;
...
double const * const * MYON_Matrix::ThisM() const
{
...
double* MYON_Matrix::operator[](int i)
{
  return m[i];
}

const double* MYON_Matrix::operator[](int i) const
{
  return m[i];
}

MYON_Matrix::MYON_Matrix()
: m(0)
, m_row_count(0)
, m_col_count(0)
, m_Mmem(0)
...
MYON_Matrix::MYON_Matrix( int row_size, int col_size ) 
: m(0)
, m_row_count(0)
, m_col_count(0)
, m_Mmem(0)
...
MYON_Matrix::MYON_Matrix( int row0, int row1, int col0, int col1 ) 
: m(0)
, m_row_count(0)
, m_col_count(0)
, m_Mmem(0)
...
MYON_Matrix::MYON_Matrix( const MYON_Xform& x ) 
: m(0)
, m_row_count(0)
, m_col_count(0)
, m_Mmem(0)
...
MYON_Matrix::MYON_Matrix( const MYON_Matrix& src )
: m(0)
, m_row_count(0)
, m_col_count(0)
, m_Mmem(0)
...
MYON_Matrix::MYON_Matrix(MYON_Matrix&& src) MYON_NOEXCEPT
  : m(src.m)
...
MYON_Matrix& MYON_Matrix::operator=(MYON_Matrix&& src)
{
  if (this != &src)
  {
    m = src.m;
...
double** MYON_Matrix::Allocate(
  unsigned int row_count,
  unsigned int col_count
  )
{
  if (row_count < 1 || row_count >= MYON_UNSET_UINT_INDEX / 2)
    return nullptr;
  if (col_count < 1 || col_count >= MYON_UNSET_UINT_INDEX / 2)
    return nullptr;
  double** M;
...
void MYON_Matrix::Deallocate(double** M)
{
  if (nullptr != M)
  {
    delete[] M;
...
int MYON_Matrix::RowCount() const
{
  return m_row_count;
}

int MYON_Matrix::ColCount() const
{
  return m_col_count;
}

int MYON_Matrix::MinCount() const
{
  return (m_row_count <= m_col_count) ? m_row_count : m_col_count;
}

int MYON_Matrix::MaxCount() const
{
  return (m_row_count >= m_col_count) ? m_row_count : m_col_count;
}

unsigned int MYON_Matrix::UnsignedRowCount() const
{
  return (m_row_count>0) ? (unsigned int)m_row_count : 0;
}

unsigned int MYON_Matrix::UnsignedColCount() const
{
  return (m_col_count>0) ? (unsigned int)m_col_count : 0;
}

unsigned int MYON_Matrix::UnsignedMinCount() const
{
  const unsigned int rc[2] = { UnsignedRowCount(), UnsignedColCount() };
  return (rc[0] < rc[1]) ? rc[0] : rc[1];
}

unsigned int MYON_Matrix::UnsignedMaxCount() const
{
  const unsigned int rc[2] = { UnsignedRowCount(), UnsignedColCount() };
  return (rc[0] > rc[1]) ? rc[0] : rc[1];
}

bool MYON_Matrix::Create( int row_count, int col_count)
{
  bool b = false;
  Destroy();
...
bool MYON_Matrix::Create( // E.g., Create(1,5,1,7) creates a 5x7 sized matrix that with
...
void MYON_Matrix::EmergencyDestroy()
{
...
MYON_Matrix& MYON_Matrix::operator=(const MYON_Matrix& src)
{
  if ( this != &src ) 
  {
...
MYON_Matrix& MYON_Matrix::operator=(const MYON_Xform& src)
{
  m_row_offset = 0;
  m_col_offset = 0;
...
bool MYON_Matrix::SwapRows( int row0, int row1 )
{
  bool b = false;
  double** this_m = ThisM();
  row0 -= m_row_offset;
...
bool MYON_Matrix::SwapCols( int col0, int col1 )
{
  bool b = false;
  int i;
  double t;
...
void MYON_Matrix::RowScale( int dest_row, double s )
{
  double** this_m = ThisM();
  dest_row -= m_row_offset;
  MYON_ArrayScale( m_col_count, s, this_m[dest_row], this_m[dest_row] );
}

void MYON_Matrix::RowOp( int dest_row, double s, int src_row )
{
  double** this_m = ThisM();
  dest_row -= m_row_offset;
  src_row -= m_row_offset;
  MYON_Array_aA_plus_B( m_col_count, s, this_m[src_row], this_m[dest_row], this_m[dest_row] );
}

void MYON_Matrix::ColScale( int dest_col, double s )
{
  int i;
  double** this_m = ThisM();
...
void MYON_Matrix::ColOp( int dest_col, double s, int src_col )
{
  int i;
  double** this_m = ThisM();
  dest_col -= m_col_offset;
    ...
    )
{
  ...
  for ( k = 0; k < n; k++ ) {
    ...
    x = 1.0/this_m[k][k];
    this_m[k][k] = 1.0;
    MYON_ArrayScale( m_col_count - 1 - k, x, &this_m[k][k+1], &this_m[k][k+1] );
    ...
    for ( i = k+1; i < m_row_count; i++ ) {
      ...
      if ( fabs(x) > zero_tolerance ) {
        MYON_Array_aA_plus_B( m_col_count - 1 - k, x, &this_m[k][k+1], &this_m[i][k+1], &this_m[i][k+1] );
      }
    }
  }
    ...
    ) const
{
  ...
  if ( X != B )
    X[n] = B[n];
  for ( i = n-1; i >= 0; i-- ) {
    X[i] = B[i] - MYON_ArrayDotProduct( n-i, &this_m[i][i+1], &X[i+1] );
  }
...
void MYON_Matrix::SetDiagonal( double d)
{
...
void MYON_Matrix::SetDiagonal( const double* d )
{
  Zero();
  if (d) 
  {
...
void MYON_Matrix::SetDiagonal( int count, const double* d )
{
  Create(count,count);
  Zero();
  SetDiagonal(d);
...
void MYON_Matrix::SetDiagonal( const MYON_SimpleArray<double>& a )
{
  SetDiagonal( a.Count(), a.Array() );
}

bool MYON_Matrix::IsValid() const
{
...
int MYON_Matrix::IsSquare() const
{
  return ( m_row_count > 0 && m_col_count == m_row_count ) ? m_row_count : 0;
}

bool MYON_Matrix::IsRowOrthoganal() const
{
  double d0, d1, d;
  int i0, i1, j;
  double const*const* this_m = ThisM();
  ...
  for ( i0 = 0; i0 < m_row_count && rc; i0++ ) for ( i1 = i0+1; i1 < m_row_count && rc; i1++ ) {
    ...
    if ( d0 <=  MYON_EPSILON || d1 <=  MYON_EPSILON || fabs(d) >= d0*d1* MYON_SQRT_EPSILON )
      rc = false;
  }
  return rc;
}

bool MYON_Matrix::IsRowOrthoNormal() const
{
  double d;
  int i, j;
  ...
  if ( rc ) {
    ...
    for ( i = 0; i < m_row_count; i++ ) {
      ...
      if ( fabs(1.0-d) >=  MYON_SQRT_EPSILON )
        rc = false;
    }
  }
  return rc;
...
bool MYON_Matrix::IsColOrthoganal() const
{
  double d0, d1, d;
  int i, j0, j1;
  ...
  for ( j0 = 0; j0 < m_col_count && rc; j0++ ) for ( j1 = j0+1; j1 < m_col_count && rc; j1++ ) {
    ...
    if ( d0 <=  MYON_EPSILON || d1 <=  MYON_EPSILON || fabs(d) >  MYON_SQRT_EPSILON )
      rc = false;
  }
  return rc;
}

bool MYON_Matrix::IsColOrthoNormal() const
{
  double d;
  int i, j;
...
bool MYON_Matrix::Invert( double zero_tolerance )
{
  MYON_Workspace ws;
  ...
  MYON_Matrix I(m_col_count, m_row_count);
  ...
  for ( k = 0; k < n; k++ ) {
    ...
    x = 1.0/this_m[k][k];
    this_m[k][k] = 1.0;
    MYON_ArrayScale( m_col_count-k-1, x, &this_m[k][k+1], &this_m[k][k+1] );
    I.RowScale( k, x );
    ...
    for ( i = 0; i < n; i++ ) {
      if ( i != k ) {
        ...
        this_m[i][k] = 0.0;
        if ( fabs(x) > zero_tolerance ) {
          MYON_Array_aA_plus_B( m_col_count-k-1, x, &this_m[k][k+1], &this_m[i][k+1], &this_m[i][k+1] );
          I.RowOp( i, x, k );
        }
      }
    }
...
bool MYON_Matrix::Multiply( const MYON_Matrix& a, const MYON_Matrix& b )
{
...
bool MYON_Matrix::Add( const MYON_Matrix& a, const MYON_Matrix& b )
{
  int i, j;
...
bool MYON_Matrix::Scale( double s )
{
  bool rc = false;
...
int MYON_RowReduce( int row_count, 
                  int col_count,
                  double zero_pivot,
                  double** A, 
                  double** B, 
                 ...
                 )
{
  ...
  if ( zero_pivot <= 0.0 || !MYON_IsValid(zero_pivot) )
    zero_pivot = 0.0;
  ...
  for ( i = 0; i < M; i++ )
  {
    ...
    if ( 1.0 != a )
    {
      if ( p <= zero_pivot || !MYON_IsValid(a) )
      {
        break;
      }
      a = 1.0/a;
  ...
  )
{
  ...
  if (row_count < 1 || MYON_UNSET_UINT_INDEX == row_count)
    return MYON_UNSET_UINT_INDEX;

  if (col_count < 1 || MYON_UNSET_UINT_INDEX == col_count)
    return MYON_UNSET_UINT_INDEX;
  ...
  std::unique_ptr< MYON_Matrix > uA;
  if (nullptr == A)
  {
    if (nullptr == constA)
      return MYON_UNSET_UINT_INDEX;
    uA = std::unique_ptr< MYON_Matrix >(new MYON_Matrix(row_count, col_count));
    A = uA.get()->m;
  }
...
double MYON_EigenvectorPrecision(
...
double MYON_MatrixSolutionPrecision(
  const unsigned int N,
  const double*const* M,
  bool bTransposeM,
  double lambda,
...
unsigned int MYON_GetEigenvectors(
  const unsigned int N,
  const double*const* M,
  bool bTransposeM,
  double lambda,
...
{
  if (N < 1 || MYON_UNSET_UINT_INDEX == N)
    return MYON_UNSET_UINT_INDEX;
  ...
  MYON_Matrix Abuffer(N, N);
  double** A = Abuffer.m;

  MYON_Matrix Bbuffer(N, N);
  double** B = Bbuffer.m;
  ...
  for(unsigned int rank0 = N+1; rank0 > 0; /*empty increment*/)
  {
    ...
    if (!(zero_pivot_tolerance >= 0.0))
    {
      ...
      MYON_ERROR("invalid zero_pivot_tolerance value");
      break;
    }
    ...
    rank = MYON_RowReduce(N, N, zero_pivot_tolerance, nullptr, true, false, A, B, nullptr, pivots);
    ...
    if (!(pivots[1] > zero_pivot_tolerance))
    {
      ...
      // we will note get a smaller rank from MYON_RowReduce().
      break;
    }
    ...
    //
    // from the call to MYON_RowReduce and we have found
  ...
  MYON_SimpleArray< double > Bprecision(N - rank);
  for (unsigned int k = rank; k < N; k++)
    Bprecision.Append(MYON_EigenvectorPrecision(N, M, bTransposeM, lambda, B[k]));
  MYON_SimpleArray< unsigned int > _Bdex(Bprecision.UnsignedCount());
  unsigned int* Bdex = _Bdex.Array();
  MYON_Sort(MYON::sort_algorithm::quick_sort, Bdex, Bprecision.Array(), Bprecision.UnsignedCount(), sizeof(double), CompareDoubleIncreasing);
