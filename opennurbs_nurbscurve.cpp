...
MYON_OBJECT_IMPLEMENT(MYON_NurbsCurve,MYON_Curve,"4ED7D4DD-E947-11d3-BFE5-0010830122F0");

/*
Description:
  Helper to make a deep copy (duplicate memory) from src to dest
*/
void MYON_NurbsCurve::Internal_DeepCopyFrom( const MYON_NurbsCurve& src )
{
  if (this == &src)
  {
    MYON_ERROR("this and &src must be different.");
    return;
  }

  // erase tags
  m_knot_capacity_and_tags &= MYON_NurbsCurve::masks::knot_capacity;
  ...
  // copy tags
  const unsigned int src_tags = (src.m_knot_capacity_and_tags & MYON_NurbsCurve::masks::all_tags);
...
void MYON_NurbsCurve::Internal_ShallowCopyFrom( const MYON_NurbsCurve& src )
{
  if (this == &src)
  {
    MYON_ERROR("this and &src must be different.");
...
*/
void MYON_NurbsCurve::Internal_InitializeToZero()
{
  m_dim = 0;
  m_is_rat = 0;
  m_order = 0;
...
Description:
  Helper to make a deep copy (duplicate memory) from src to dest
*/
void MYON_NurbsCurve::Internal_Destroy()
{
...
MYON_NurbsCurve::MYON_NurbsCurve() MYON_NOEXCEPT
{
  MYON__SET__THIS__PTR(m_s_MYON_NurbsCurve_ptr);
  Internal_InitializeToZero();
}

MYON_NurbsCurve::MYON_NurbsCurve( const MYON_NurbsCurve& src )
  : MYON_Curve(src)
{
  MYON__SET__THIS__PTR(m_s_MYON_NurbsCurve_ptr);
  Internal_InitializeToZero();
...
MYON_NurbsCurve& MYON_NurbsCurve::operator=( const MYON_NurbsCurve& src )
{
  if ( this != &src ) 
  {
...
MYON_NurbsCurve::MYON_NurbsCurve( MYON_NurbsCurve&& src) MYON_NOEXCEPT
...
MYON_NurbsCurve& MYON_NurbsCurve::operator=( MYON_NurbsCurve&& src)
{
  if ( this != &src )
  {
    Internal_Destroy();
...
MYON_NurbsCurve* MYON_NurbsCurve::New()
{
  return new MYON_NurbsCurve();
}
...
{
  return new MYON_NurbsCurve(nurbs_curve);
}

MYON_NurbsCurve* MYON_NurbsCurve::New(
          const MYON_BezierCurve& bezier_curve 
          )
{
  return new MYON_NurbsCurve(bezier_curve);
}
...
{
  return new MYON_NurbsCurve(dimension,bIsRational,order,cv_count);
}

MYON_NurbsCurve::MYON_NurbsCurve( int dim, bool bIsRational, int order, int cv_count )
{
  MYON__SET__THIS__PTR(m_s_MYON_NurbsCurve_ptr);
  Initialize();
...
MYON_NurbsCurve::MYON_NurbsCurve(const MYON_BezierCurve& src)
{
  MYON__SET__THIS__PTR(m_s_MYON_NurbsCurve_ptr);
  Initialize();
  MYON_NurbsCurve::operator=(src);
}

MYON_NurbsCurve::~MYON_NurbsCurve()
{
  Destroy();
}

unsigned int MYON_NurbsCurve::SizeOf() const
{
...
MYON__UINT32 MYON_NurbsCurve::DataCRC(MYON__UINT32 current_remainder) const
{
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_dim),&m_dim);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_is_rat),&m_is_rat);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_order),&m_order);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_cv_count),&m_cv_count);
  if ( m_cv_count > 0 && m_cv_stride > 0 && m_cv )
  {
    ...
    {
      current_remainder = MYON_CRC32(current_remainder,sizeof_cv,cv);
      cv += m_cv_stride;
    }
  }
  current_remainder = MYON_CRC32(current_remainder,KnotCount()*sizeof(m_knot[0]),m_knot);
  return current_remainder;
}

int MYON_NurbsCurve::Dimension() const
{
  return m_dim;
}

bool MYON_NurbsCurve::IsRational() const
{
  return m_is_rat?true:false;
}

int MYON_NurbsCurve::CVSize() const
{
  return (m_dim>0) ? (m_is_rat?(m_dim+1):m_dim) : 0;
}

int MYON_NurbsCurve::Order( void ) const
{
  return m_order;
}

int MYON_NurbsCurve::CVCount( void ) const
{
  return m_cv_count;
}

int MYON_NurbsCurve::KnotCount( void ) const
{
  return MYON_KnotCount( m_order, m_cv_count );
}

double* MYON_NurbsCurve::CV( int i ) const
{
  return (m_cv) ? (m_cv + i*m_cv_stride) : nullptr;
}

MYON::point_style MYON_NurbsCurve::CVStyle() const
{
  return m_is_rat ? MYON::homogeneous_rational : MYON::not_rational;
}


double MYON_NurbsCurve::Weight( int i ) const
{
  return (m_cv && m_is_rat) ? m_cv[i*m_cv_stride+m_dim] : 1.0;
}


double MYON_NurbsCurve::Knot( int i ) const
{
  return (m_knot) ? m_knot[i] : 0.0;
}

int MYON_NurbsCurve::KnotMultiplicity( int i ) const
{
  return MYON_KnotMultiplicity(m_order,m_cv_count,m_knot,i);
}

const double* MYON_NurbsCurve::Knot() const
{
  return m_knot;
}

double MYON_NurbsCurve::SuperfluousKnot( int end ) const
{
  return(m_knot) ? MYON_SuperfluousKnot(m_order,m_cv_count,m_knot,end) : 0.0;
}


bool MYON_NurbsCurve::MakePeriodicUniformKnotVector( double delta )
{
	DestroyCurveTree();
  ReserveKnotCapacity( MYON_KnotCount( m_order, m_cv_count ) );
  return MYON_MakePeriodicUniformKnotVector( m_order, m_cv_count, m_knot, delta );
}


bool MYON_NurbsCurve::MakeClampedUniformKnotVector( double delta )
{
	DestroyCurveTree();
  ReserveKnotCapacity( MYON_KnotCount( m_order, m_cv_count ) );
  return MYON_MakeClampedUniformKnotVector( m_order, m_cv_count, m_knot, delta );
}

bool MYON_NurbsCurve::CreateClampedUniformNurbs( 
        int dimension,
        int order,
        int point_count,
        ...
        )
{
  ...
  if ( rc )
  {
    ...
    for (i = 0; i < point_count; i++)
      SetCV( i, MYON::intrinsic_point_style, point[i] );
  }
  if ( rc )
...
bool MYON_NurbsCurve::CreatePeriodicUniformNurbs( 
        int dimension,
        int order,
        int point_count,
        const MYON_3dPoint* point,
        ...
        )
{
  ...
  if ( rc )
  {
    ...
    for (i = 0; i < point_count; i++)
      SetCV( i, MYON::intrinsic_point_style, point[i] );
    for (i = 0; i <= order-2; i++)
      SetCV( m_cv_count-m_order+1+i, MYON::intrinsic_point_style, CV(i) );
  }
  if ( rc )
...
void MYON_NurbsCurve::Destroy()
{
  DestroyCurveTree();
  Internal_Destroy();
}

void MYON_NurbsCurve::EmergencyDestroy()
{
  Internal_InitializeToZero();
}


void MYON_NurbsCurve::Initialize()
{
  Internal_InitializeToZero();
}

MYON_NurbsCurve& MYON_NurbsCurve::operator=(const MYON_BezierCurve& src)
{
  int i;
...
void MYON_NurbsCurve::Dump( MYON_TextLog& dump ) const
{
  dump.Print( "MYON_NurbsCurve dim = %d is_rat = %d\n"
               "        order = %d cv_count = %d\n",
...
static bool MYON_NurbsCurveIsNotValid()
{
  return MYON_IsNotValid(); // <-- good place for a breakpoint
}

static bool MYON_ControlPointsAreNotValid()
{
  return MYON_IsNotValid(); // <-- good place for a breakpoint
}

static bool MYON_ControlPointsAreValid( int cv_size, int cv_count, int cv_stride, const double* cv, MYON_TextLog* text_log )
{
  int i, j;
  if ( 0 == cv  )
  {
    ...
    return MYON_ControlPointsAreNotValid();
  }
...
bool MYON_NurbsCurve::IsValid( MYON_TextLog* text_log ) const
{
  if ( m_dim <= 0 )
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_dim = %d (should be > 0).\n",m_dim);
    }
    return MYON_NurbsCurveIsNotValid();
  }
  ...
  if (m_order < 2 )
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_order = %d (should be >= 2).\n",m_order);
    }
    return MYON_NurbsCurveIsNotValid();
  }
  ...
  if (m_cv_count < m_order )
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_cv_count = %d (should be >= m_order=%d).\n",m_cv_count,m_order);
    }
    return MYON_NurbsCurveIsNotValid();
  }
  ...
  if (m_cv_stride < CVSize() )
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_cv_stride = %d (should be >= %d).\n",m_cv_stride,CVSize());
    }
  }
  ...
  if (m_cv == nullptr)
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_cv is nullptr.\n");
    }
    return MYON_NurbsCurveIsNotValid();
  }
  ...
  if (m_knot == nullptr)
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_knot is nullptr.\n");
    }
    return MYON_NurbsCurveIsNotValid();
  }

  if ( !MYON_IsValidKnotVector( m_order, m_cv_count, m_knot, text_log ) ) 
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_knot[] is not a valid knot vector.\n");
    }
    return MYON_NurbsCurveIsNotValid();
  }

  if ( !MYON_ControlPointsAreValid(CVSize(),m_cv_count,m_cv_stride,m_cv,text_log) )
  {
    if ( text_log )
    {
      text_log->Print("MYON_NurbsCurve.m_cv[] is not valid.\n");
    }
    return MYON_NurbsCurveIsNotValid();
  }
  ...
  if ( m_is_rat )
  {
    ...
    for ( i = 0; i < m_cv_count; i++, w += m_cv_stride )
    {
      ...
      if ( zcount >= m_order )
      {
        if ( text_log )
        {
          text_log->Print("MYON_NurbsCurve.m_cv has zero weights for CV[%d],...,CV[%d].\n",i-m_order+1,i);
        }
        return MYON_NurbsCurveIsNotValid(); // denominator is zero for entire span
      }
      ...
      if ( m_knot[i] == m_knot[i+m_order-2] ) 
      {
        if ( *w == 0.0 )
        {
          if ( text_log )
          {
            text_log->Print("MYON_NurbsCurve.m_cv has zero weights for CV[%d].\n",i);
          }
          return MYON_NurbsCurveIsNotValid();
        }
        ...
        else if ( *w * sign <= 0.0 ) 
        {
          if ( text_log )
          {
            text_log->Print("MYON_NurbsCurve.m_cv has a zero denominator in the parameter interval [%g,%g].\n",
                            m_knot[i-1],m_knot[i]);
          }
          return MYON_NurbsCurveIsNotValid();
        }
    ...
    if ( m_dim <= 3 && 2 == m_order && 2 == m_cv_count )
    {
      ...
      MYON_3dPoint P0 = PointAtStart();
      MYON_3dPoint P1 = PointAtEnd();
      if ( P0 == P1 )
      {
        if ( text_log )
        {
          text_log->Print("MYON_NurbsCurve is a line with no length.\n");
        }
        return MYON_NurbsCurveIsNotValid();
      }
    }
...
bool MYON_NurbsCurve::GetBBox( // returns true if successful
       double* boxmin,    // minimum
       double* boxmax,    // maximum
       bool bGrowBox  // true means grow box
       ) const
{
  return MYON_GetPointListBoundingBox( 
...
bool MYON_NurbsCurve::Transform( const MYON_Xform& xform )
{
  if ( !this->MYON_Curve::Transform(xform) )
    return false;
  if ( 0 == m_is_rat )
  {
  ...
  return MYON_TransformPointList( m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv, xform );
}

bool MYON_NurbsCurve::IsDeformable() const
{
  return true;
}

bool MYON_NurbsCurve::MakeDeformable()
{
  return true;
}

bool MYON_PolylineCurve::IsDeformable() const
{
  return true;
}

bool MYON_PolylineCurve::MakeDeformable()
{
  return true;
}


bool MYON_NurbsCurve::Write(
     ...
     )
{
  ...
  if (rc && major_version==1)
  {
    ...
    else if ( 0 != count && count != MYON_KnotCount(order,cv_count) )
      rc = false;
...
MYON_Interval MYON_NurbsCurve::Domain() const
{
  MYON_Interval d;
  if ( !MYON_GetKnotVectorDomain( m_order, m_cv_count, m_knot, &d.m_t[0], &d.m_t[1] ) )
    d = MYON_Interval::EmptyInterval;
  return d;
}

bool MYON_NurbsCurve::SetDomain( double t0, double t1 )
{
  bool rc = false;
...
static MYON_NurbsCurve* MoveSeamPeriodicKnot(const MYON_NurbsCurve& crv, int knot_index)

{
  ...
  MYON_NurbsCurve* pNC = MYON_NurbsCurve::New(crv);
  int curr = dg-1;
  ...
  for (int i=0; i<pNC->CVCount(); i++){
    ...
    pNC->SetCV(i, MYON::intrinsic_point_style, pcv);
    cv_id++;
  }
...
static MYON_NurbsCurve* MoveSeamPeriodic(const MYON_Curve& crv, double t)

{
  MYON_NurbsCurve nc;
  double s = t;
  const MYON_NurbsCurve*pNC = MYON_NurbsCurve::Cast(&crv);
  if (pNC)
    nc = *pNC;
  else {
  ...
  bool bInsert = true;
  if (d0<=d1){
    if (d0 < MYON_ZERO_TOLERANCE*fabs(k0)){
      knot_index--;
      bInsert = false;
    }
  }
  else {
    if (d1 < MYON_ZERO_TOLERANCE*fabs(k1))
      bInsert = false;
  }
...
bool MYON_NurbsCurve::ChangeClosedCurveSeam( double t )
{
  bool rc = IsClosed();
  if ( rc )
  {
    ...
    if ( old_dom.Includes(k,true) )
    {
      ...
      if (!bGotIt)
      {
        MYON_Curve* pleft = &left;
        MYON_Curve* pright = &right;
...
int MYON_NurbsCurve::SpanCount() const
{
  // number of smooth spans in curve
  return MYON_KnotVectorSpanCount( m_order, m_cv_count, m_knot );
}

bool MYON_NurbsCurve::GetSpanVector( // span "knots" 
       double* s // array of length SpanCount() + 1 
       ) const
{
  return MYON_GetKnotVectorSpanVector( m_order, m_cv_count, m_knot, s );
}

int MYON_NurbsCurve::Degree() const
{
      ...
      ) const
{
  ...
  // if CV coordinates are NaNs, infinities or MYON_UNSET_VALUE,
  // then "t" will end failing the MYON_IsValid() test.
  D.x = L.to.x - L.from.x; D.y = L.to.y - L.from.y; D.z = L.to.z - L.from.z; 
  t = D.x*D.x + D.y*D.y + D.z*D.z;
  if ( !MYON_IsValid(t) || !(t > 0.0) )
    return false;
  ...
  for ( i = 1; i < m_cv_count-1; i++ )
  {
    ...
    {
      if ( !MYON_PointsAreCoincident(3,0,&P.x,&Q.x) )
        return false;
    }
    else
    {
...
int MYON_NurbsCurve::IsPolyline(
      ...
      for ( i_local = 0; i_local < span_count; i_local++, k++ )
      {
        if ( k[m_order-2] < k[m_order-1] )
        {
          ...
          for ( j = 1; j < m_order-1; j++ )
          {
            ...
            //  MYON_RELATIVE_TOLERANCE because that makes sense.
            //const double rel_tol = 0.00001;
            const double rel_tol = MYON_RELATIVE_TOLERANCE;
            if ( fabs(P.x - Q.x) > (fabs(P.x) + fabs(Q.x))*rel_tol + MYON_ZERO_TOLERANCE )
              return 0;
            if ( fabs(P.y - Q.y) > (fabs(P.y) + fabs(Q.y))*rel_tol + MYON_ZERO_TOLERANCE )
              return 0;
            if ( fabs(P.z - Q.z) > (fabs(P.z) + fabs(Q.z))*rel_tol + MYON_ZERO_TOLERANCE )
              return 0;
          }
          rc++;
        }
      ...
      ) const
{
  ...
  // If the tolerance > MYON_ZERO_TOLERANCE a fuzzy fit to arc
  // arc test is performed.  If tolerance <= MYON_ZERO_TOLERANCE,
  // a stricter arc test is performed.
  const bool bLooseTest = (tolerance > MYON_ZERO_TOLERANCE);
       ...
       )
  {
    if ( bLooseTest || 0 == (knotcount % degree) )
    {
      ...
      if ( MYON_Curve::IsArc( plane, arc, tolerance ) )
        return true;
    }
  }
      ...
      ) const
{
  ...
  else if ( m_cv_count >= 3 )
  {
    ...
    for ( i = 1; i < m_cv_count; i += k )
    {
      ...
      {
        GetCV(j,B);
        d = MYON_CrossProduct( A-P, B-P ).Length();
        if ( d > maxd )
        {
          maxd = d;
          Q = A;
    ...
    {
      MYON_2dVector v( X*test_plane.xaxis, X*test_plane.yaxis );
      if ( v.Unitize() )
      {
        // Rotate test_plane's x,y axes so its x axis is parallel
        // to the curve's initial tangent.
        if ( fabs(v.y) <= MYON_SQRT_EPSILON )
        {
          v.x = (v.x >= 0.0) ? 1.0 : -1.0;
          v.y = 0.0;
        }
        else if ( fabs(v.x) <= MYON_SQRT_EPSILON )
        {
          v.y = (v.y >= 0.0) ? 1.0 : -1.0;
          v.x = 0.0;
        }
        X = test_plane.xaxis;
        MYON_3dVector Y = test_plane.yaxis;
...
bool
MYON_NurbsCurve::GetParameterTolerance( // returns tminus < tplus: parameters tminus <= s <= tplus
       ...
       ) const
{
  ...
  if ( d.IsIncreasing() ) {
    ...
    rc = MYON_GetParameterTolerance( d.m_t[0], d.m_t[1], t, tminus, tplus );
  }
  return rc;
}
...
bool 
MYON_NurbsCurve::Evaluate( // returns false if unable to evaluate
       ...
       ) const
{
  ...
  int span_index = MYON_NurbsSpanIndex(m_order,m_cv_count,m_knot,t,side,(hint)?*hint:0);
  ...
  if ( -2 == side || 2 == side )
  {
    ...
    double a = t;
    if ( MYON_TuneupEvaluationParameter( side, m_knot[span_index+m_order-2], m_knot[span_index+m_order-1], &a) )
    {
      // recalculate span_index
      t = a;
      span_index = MYON_NurbsSpanIndex(m_order,m_cv_count,m_knot,t,side,span_index);
    }
  }

  rc = MYON_EvaluateNurbsSpan(
...
bool 
MYON_NurbsCurve::IsClosed() const
{
...
bool 
MYON_NurbsCurve::IsPeriodic() const
{
  bool bIsPeriodic = MYON_IsKnotVectorPeriodic( m_order, m_cv_count, m_knot );
  if ( bIsPeriodic ) {
    int i0 = m_order-2;
    int i1 = m_cv_count-1;
    ...
    for ( /*empty*/; i0 >= 0; i0--, i1-- ) {
      if ( false == MYON_PointsAreCoincident( m_dim, m_is_rat, cv0, cv1 ) )
        return false;
      cv0 -= m_cv_stride;
      cv1 -= m_cv_stride;      
    }
...
bool MYON_IsCurvatureDiscontinuity( 
  const MYON_3dVector Km, 
  const MYON_3dVector Kp,
  ...
  )
{
  ...
  return MYON_IsCurvatureDiscontinuity( Km, Kp, 
    cos_angle_tolerance, 
    curvature_tolerance,
    zero_curvature,
    radius_tolerance,
...
bool MYON_IsG2CurvatureContinuous(
  const MYON_3dVector Km, 
  const MYON_3dVector Kp,
  double cos_angle_tolerance,
  double curvature_tolerance
  )
{
  ...
  //  get test results.
  double rel_tol = MYON_RELATIVE_CURVATURE_TOLERANCE; // disables using curvature_tolerance to flag unequal scalars

  double cos_Kangle_tolerance = cos_angle_tolerance;
  if ( cos_Kangle_tolerance > MYON_DEFAULT_ANGLE_TOLERANCE_COSINE )
    cos_Kangle_tolerance = MYON_DEFAULT_ANGLE_TOLERANCE_COSINE;
  ...
  return !MYON_IsCurvatureDiscontinuity(Km,Kp,
            ...
            curvature_tolerance,
            MYON_ZERO_CURVATURE_TOLERANCE,
            MYON_UNSET_VALUE, // no absolute delta radius tolerance
            rel_tol
            );
}

bool MYON_IsGsmoothCurvatureContinuous(
  const MYON_3dVector Km, 
  const MYON_3dVector Kp,
  double cos_angle_tolerance,
  double curvature_tolerance
  ...
  )
{
  ...
  // curvature_tolerance is MYON_UNSET_VALUE
...
bool 
MYON_NurbsCurve::SetWeight( int i, double w )
{
  DestroyCurveTree();

  bool rc = false;

  if (0 == m_is_rat && w > 0.0 && w < MYON_UNSET_POSITIVE_VALUE)
  {
    MakeRational();
  }
...
bool 
MYON_NurbsCurve::SetCV( int i, MYON::point_style style, const double* Point )
{
  bool rc = true;
  int k;
  double w;
...
bool 
MYON_NurbsCurve::SetCV( int i, const MYON_3dPoint& point )
{
  bool rc = false;
  double* cv = CV(i);
  if ( cv ) {
...
bool 
MYON_NurbsCurve::SetCV( int i, const MYON_4dPoint& point )
{
  bool rc = false;
  double* cv = CV(i);
  if ( cv ) {
...
bool 
MYON_NurbsCurve::GetCV( int i, MYON::point_style style, double* Point ) const
{
...
const MYON_4dPoint MYON_NurbsCurve::ControlPoint(
  int cv_index
) const
{
  MYON_4dPoint cv;
...
bool 
MYON_NurbsCurve::GetCV( int i, MYON_3dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i);
  if ( cv ) {
...
bool 
MYON_NurbsCurve::GetCV( int i, MYON_4dPoint& point ) const
{
  bool rc = false;
...
bool MYON_NurbsCurve::SetKnot( int knot_index, double k )
{
...
bool MYON_NurbsCurve::SetStartPoint(
...
bool MYON_NurbsCurve::SetEndPoint(
...
bool
MYON_NurbsCurve::Reverse()
{
  bool rc0 = MYON_ReverseKnotVector( m_order, m_cv_count, m_knot );
  bool rc1 = MYON_ReversePointList( m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv );
	DestroyCurveTree();
  return rc0 && rc1;
}

bool
MYON_NurbsCurve::SwapCoordinates( int i, int j )
{
	DestroyCurveTree();
  return  MYON_SwapPointListCoordinates( m_cv_count, m_cv_stride, m_cv, i, j );
}

double MYON_NurbsCurve::GrevilleAbcissa(
         int gindex  // index (0 <= index < CVCount(dir)
         ) const
{
  return MYON_GrevilleAbcissa( m_order, m_knot+gindex );
}

bool MYON_NurbsCurve::GetGrevilleAbcissae( // see MYON_GetGrevilleAbcissa() for details
         ...
         ) const
{
  ...
  // was returned.
  return MYON_GetGrevilleAbcissae( m_order, m_cv_count, m_knot, false, g );
}


bool MYON_NurbsCurve::ZeroCVs()
{
  bool rc = false;
  int i;
  if ( m_cv ) 
...
bool MYON_NurbsCurve::IsClamped( // determine if knot vector is clamped
      int end // (default= 2) end to check: 0 = start, 1 = end, 2 = start and end
      ) const
{
  return MYON_IsKnotVectorClamped( m_order, m_cv_count, m_knot, end );
}
  
bool MYON_NurbsCurve::IsNatural(int end) const
{
  bool rc = false;
  MYON_3dPoint CV0, CV2, P;
  MYON_3dVector D1, D2;
  const MYON_Interval domain(Domain());
  for (int pass = ((0==end||2==end) ? 0 : 1); pass < ((1==end||2==end)?2 : 1); ++pass)
  {
    MYON_BezierCurve span;
...
int MYON_NurbsCurve::CVCapacity() const
{
...
bool MYON_NurbsCurve::ReserveCVCapacity(int desired_capacity)
{
  if (nullptr != m_cv && 0 == m_cv_capacity)
  {
    // If m_cv_capacity == 0 and m_cv != nullptr, then the "expert" user
    // has hand built the MYON_NurbsCurve.m_cv array and is responsible
...
int MYON_NurbsCurve::KnotCapacity() const
{
  const int knot_capacity 
    = (nullptr == m_knot) 
    ? 0 
    : ((int)(m_knot_capacity_and_tags & MYON_NurbsCurve::masks::knot_capacity));
  return knot_capacity;
}

void MYON_NurbsCurve::UnmanageKnotForExperts(
  int& knot_capacity,
  double*& knot
)
{
  ...
  knot = m_knot;
  m_knot_capacity_and_tags &= MYON_NurbsCurve::masks::all_tags; // knot_capacity = 0;  
  m_knot = nullptr;
}

void MYON_NurbsCurve::ManageKnotForExperts(
  int knot_capacity,
  double* knot
)
{
  ...
  const unsigned int tags = (m_knot_capacity_and_tags & MYON_NurbsCurve::masks::all_tags);
  const unsigned int new_knot_capacity = (knot_capacity > 0) ? (((unsigned int)knot_capacity) & MYON_NurbsCurve::masks::knot_capacity) : 0U;
  m_knot_capacity_and_tags = (tags | new_knot_capacity);
  m_knot = knot;
}

bool MYON_NurbsCurve::ReserveKnotCapacity(int desired_capacity)
{
  ...
  if (nullptr != m_knot && 0 == current_knot_capacity)
  {
    ...
    // has hand built the MYON_NurbsCurve.m_knot array and is responsible
    // for making sure it's always big enough.
    return true;
  }
...
bool MYON_NurbsCurve::ConvertSpanToBezier( int span_index, MYON_BezierCurve& bez ) const
{
  bool rc = false;
    ...
    if ( rc ) {
      ...
      const double* knot = m_knot + span_index;
			if( knot[m_order-2] < knot[m_order-1] )
				MYON_ConvertNurbSpanToBezier( cvdim, bez.m_order, bez.m_cv_stride, bez.m_cv,
...
bool MYON_NurbsCurve::HasBezierSpans() const
{
  return MYON_KnotVectorHasBezierSpans( m_order, m_cv_count, m_knot );
}

bool MYON_NurbsCurve::MakePiecewiseBezier( bool bSetEndWeightsToOne )
{
  ...
  if ( !rc && IsValid() ) 
  {
    ...
    GetSpanVector( t );
    int cvdim = CVSize();
    MYON_BezierCurve* bez = new MYON_BezierCurve[span_count];
    int ki, spani, i;
    for ( ki = m_order-2, spani = 0; ki < m_cv_count-1 && spani < span_count; ki++ ) {
      if ( m_knot[ki] < m_knot[ki+1] ) {
        ...
        for ( i = 0; i < m_order; i++ )
          bez[spani].SetCV(  i, MYON::intrinsic_point_style, CV( i + ki - m_order + 2 ) );
        MYON_ConvertNurbSpanToBezier( cvdim, bez[spani].m_order, bez[spani].m_cv_stride, bez[spani].m_cv,
    ...
    for ( spani = 0; spani < span_count; spani++ ) {
      for ( i = 0; i < m_order; i++ ) {
        SetCV(  spani*(m_order-1) + i, MYON::intrinsic_point_style, bez[spani].CV( i ) );
      }
...
double MYON_NurbsCurve::ControlPolygonLength() const
{
  double length = 0.0;
  MYON_GetPolylineLength( m_dim, m_is_rat, m_cv_count, m_cv_stride, m_cv, &length );
  return length;
}


bool MYON_NurbsCurve::InsertKnot( double knot_value, int knot_multiplicity )
{
  bool rc = false;
  ...
  {
    MYON_ERROR("MYON_NurbsCurve::MYON_InsertKnot(): knot_multiplicity < 1 or knot_multiplicity > degree.");
    return false;
  }

  if( knot_value < t0 || knot_value > t1 )
  {
    MYON_ERROR("MYON_InsertKnot(): knot_value not in NURBS curve domain.");
    return false;
  }
  ...
  if ( knot_value == t0 ) 
  {
    ...
    {
      MYON_ERROR("MYON_InsertKnot(): knot_value = t0 and 1 < knot_multiplicity < degree.");
      rc = false;
    }
    return rc;
  }
  ...
  if ( knot_value == t1 ) 
  {
    ...
    {
      MYON_ERROR("MYON_InsertKnot(): knot_value = t1 and 1 < knot_multiplicity < degree.");
      rc = false;
    }
    return rc;
  }
  ...
  int span_index = MYON_NurbsSpanIndex( m_order, m_cv_count, m_knot, knot_value, 0, 0 );
  ...
  int new_knot_count = MYON_InsertKnot( knot_value, knot_multiplicity, 
  ...
  if ( bIsPeriodic && rc && !IsPeriodic() ) {
    ...
    if ( MYON_MakeKnotVectorPeriodic( m_order, m_cv_count, m_knot ) ) {
      int i0, i1;
      for ( i0 = 0, i1 = m_cv_count-degree; i0 < degree; i0++, i1++ ) {
        ...
        if (i0 > span_index)
          SetCV( i1, MYON::intrinsic_point_style, CV(i0) ); // cv[i1] = cv[i0]
        else
          SetCV( i0, MYON::intrinsic_point_style, CV(i1) ); // cv[i0] = cv[i1]
      }
    }
    else {
      ClampEnd(2);
...
bool MYON_NurbsCurve::MakeRational()
{
  if ( !IsRational() ) {
...
bool MYON_NurbsCurve::MakeNonRational()
{
  if ( IsRational() ) {
    const int dim = Dimension();
    const int cv_count = CVCount();
...

{
  ...
  for (i=0; i<new_degree; i++){
    ...
    if (!MYON_EvaluateNurbsBlossom(cvdim, old_order, 
...
static bool IncrementNurbDegree(MYON_NurbsCurve& N)
//for use only in MYON_NurbsCurve::IncreaseDegree().  No validation done.
...

{
  ...
  for (i=0; i<span_count; i++){
    ...
    siN = MYON_NextNurbsSpanIndex(N.Order(), N.CVCount(), N.m_knot, siN);
    siM = MYON_NextNurbsSpanIndex(M.Order(), M.CVCount(), M.m_knot, siM);
  }
...
bool MYON_NurbsCurve::IncreaseDegree( int desired_degree )
{
...
bool MYON_NurbsCurve::ChangeDimension( int desired_dimension )
{
  bool rc = false;
  int i, j;
...
bool MYON_NurbsCurve::Append( const MYON_NurbsCurve& c )
{
  bool rc = false;
       ...
       && Dimension() == c.Dimension() ) 
  {
    ...
    ReserveKnotCapacity( MYON_KnotCount(Order(),CVCount()+c.CVCount()) );
...
bool TweakSplitTrimParameter(double k0, double k1, double& t )
{
  ...
  if (k0 < t && t < k1)
  {
    ...
    // Changed this from MYON_SQRT_EPSILON to 4*MYON_SQRT_EPSILON because 
    ...
    //  I changed the test Lowell referes to above.  Lowell's 4.0*MYON_SQRT_EPSILON 
    //  is now 8.0*MYON_EPSILON in the ktol2 initialization value.
    ...
    const double ktol1 = (k1 - k0)*MYON_SQRT_EPSILON;
    const double ktol2 = (fabs(k0) + fabs(k1))*8.0*MYON_EPSILON;
...
bool MYON_NurbsCurve::Trim( const MYON_Interval& in )
{
  if ( !in.IsIncreasing() )
    return false;
  ...
  ki = MYON_NurbsSpanIndex( order, m_cv_count, m_knot, t, side, 0 );
  ...
  if ( TweakSplitTrimParameter(m_knot[ki+order-2], m_knot[ki+order-1], split_t ) )
    ki = MYON_NurbsSpanIndex( order, m_cv_count, m_knot, split_t, side, ki );

  if ( !MYON_EvaluateNurbsDeBoor( cv_dim, order, m_cv_stride, CV(ki),
                                m_knot + ki, side, 0.0, t ) ) 
  {
    MYON_ERROR("MYON_NurbsCurve::Trim() - right end de Boor algorithm failed.");
    return false;
  }
  // clamp right end knots
  m_cv_count = ki + order;
  for ( i0 = MYON_KnotCount( order, m_cv_count)-1; i0 >= m_cv_count-1; i0--)
    m_knot[i0] = t;
  ...
  if ( new_cv_count < m_cv_count ) {
    ...
    for ( i0 = 0, i1 = ki*m_cv_stride; i1 < i1_max; i0++, i1++ )
      m_cv[i0] = m_cv[i1];
    i1_max = MYON_KnotCount( order, m_cv_count );
...

{
  ...
  if (domain[0] < Domain()[0]){
    ClampEnd(0);
    MYON_EvaluateNurbsDeBoor(cvdim,Order(),m_cv_stride, CV(0),m_knot,1,0.0,domain[0]);
  ...
  if (domain[1] > Domain()[1]){
    ClampEnd(1);
    int i = CVCount() - Order();
    MYON_EvaluateNurbsDeBoor(cvdim,Order(),m_cv_stride, CV(i),m_knot + i,-1,0.0,domain[1]);
...
bool MYON_NurbsCurve::Split(
    double t,    // t = curve parameter to split curve at
    MYON_Curve*& left_crv, // left portion returned here (must be an MYON_NurbsCurve)
    MYON_Curve*& right_crv // right portion returned here (must be an MYON_NurbsCurve)
  ) const
{

  int i;
  bool rc = false;
  if ( left_crv && !MYON_NurbsCurve::Cast(left_crv) )
    return false;
  if ( right_crv && !MYON_NurbsCurve::Cast(right_crv) )
    return false;
  if ( IsValid() && t > m_knot[m_order-2] && t < m_knot[m_cv_count-1] ) 
  {
    MYON_NurbsCurve* left = (MYON_NurbsCurve*)left_crv;
    MYON_NurbsCurve* right = (MYON_NurbsCurve*)right_crv;
    if ( !left )
      left = new MYON_NurbsCurve();
    else if ( left == right )
      return false;

    if ( !right )
      right = new MYON_NurbsCurve();
    left->DestroyCurveTree();
    right->DestroyCurveTree();
    
    int span_index = MYON_NurbsSpanIndex(m_order,m_cv_count,m_knot,t,1,0);
    ...
    if (TweakSplitTrimParameter(m_knot[span_index + m_order - 2], m_knot[span_index + m_order - 1], split_t))
    {
      ...
      span_index = MYON_NurbsSpanIndex(m_order,m_cv_count,m_knot,split_t,1,span_index);
    }
    ...
    if ( span_index >= 0 && span_index <= m_cv_count - m_order ) 
    {
      ...
      {
        i = MYON_KnotCount( m_order, left_cv_count);
      ...
      {
        i = MYON_KnotCount(m_order, right_cv_count);
      ...
      //MYON_EvaluateNurbsDeBoor( cvdim, m_order, cvdim, left->CV(i), left->m_knot + i, -1, 0.0, t );
      MYON_EvaluateNurbsDeBoor( cvdim, m_order, cv_stride, left->CV(i), left->m_knot + i, -1, 0.0, split_t);
      for ( i = left->m_cv_count-1; i < MYON_KnotCount(left->m_order,left->m_cv_count); i++ )
        left->m_knot[i] = t;
      ...
      //23 April 2015 - Chuck - maintain stride
     // 31 Aug 2022 GBA RH69916 split at split_t not t. Splitting at t produces a gap that can't be right.
      //MYON_EvaluateNurbsDeBoor( cvdim, m_order, cvdim, right->m_cv, right->m_knot, +1, 0.0, t );
      MYON_EvaluateNurbsDeBoor( cvdim, m_order, cv_stride, right->m_cv, right->m_knot, +1, 0.0, split_t);
...
int MYON_NurbsCurve::GetNurbForm( MYON_NurbsCurve& curve, 
                                double tolerance,
...
int MYON_NurbsCurve::HasNurbForm() const
{
  return 1;
}


bool MYON_NurbsCurve::GetCurveParameterFromNurbFormParameter(
      double nurbs_t,
      double* curve_t
      ) const
{
...
bool MYON_NurbsCurve::GetNurbFormParameterFromCurveParameter(
      double curve_t,
      double* nurbs_t
      ) const
{
...
bool MYON_NurbsCurve::ClampEnd( 
        int end // 0 = clamp start, 1 = clamp end, 2 = clamp start and end 
        )
{
  // Curve tree doesn't change when you clamp // DestroyCurveTree();
  return MYON_ClampKnotVector( CVSize(), m_order, 
                             m_cv_count, m_cv_stride, m_cv, 
                             m_knot, end );
}


double MYON_NurbsCurve::GetCubicBezierApproximation(
  double max_deviation,
  class MYON_BezierCurve& bezierCurve
) const
{
  MYON_3dPoint bezCV[4];
...
{
  const double failed_rc = MYON_DBL_QNAN;
  ...
  MYON_3dPoint bez0CV[4] = { MYON_3dPoint::Origin, MYON_3dPoint::Origin, MYON_3dPoint::Origin, MYON_3dPoint::Origin };
  // bez1CV[] are cubic bezier control points set from end locations and derivatives.
  MYON_3dPoint bez1CV[4] = { MYON_3dPoint::Origin, MYON_3dPoint::Origin, MYON_3dPoint::Origin, MYON_3dPoint::Origin };
  ...
  const MYON_3dVector D[2]{ c * bez1CV[1], c * bez1CV[3] };
  bez1CV[1] = bez0CV[0] + D[0]/3.0;
  bez1CV[2] = bez0CV[3] - D[1]/3.0;
  bez1CV[3] = bez0CV[3];

  // bez0CV[] are cubic bezier control points set from end locations and interpolating 2 interior points.
  MYON_3dPoint P[2] = {MYON_3dPoint::Origin,MYON_3dPoint::Origin};
  int hint = 0;
...
double MYON_NurbsSurface::GetCubicBezierApproximation(
  double max_deviation,
  class MYON_BezierSurface& bezierSurface
) const
{
  MYON_3dPoint bezCV[4][4];
  const double deviation = GetCubicBezierApproximation(max_deviation, bezCV);
  if (deviation >= 0.0)
  {
...
) const
{
  ...
  const MYON_Interval domain[2] = { this->Domain(0),this->Domain(1) };
  
  const double c[4] = { domain[1][0],domain[0][1],domain[1][1],domain[0][0] };
  const double maximum_deviation = MYON_DBL_QNAN;
  double deviation = 0.0;
  for (int side = 0; side < 4; side++)
  {
    MYON_Curve* iso = this->IsoCurve((side%2), c[side]);
    if (nullptr == iso)
      return failed_rc;
    const MYON_NurbsCurve* nurbs_curve = MYON_NurbsCurve::Cast(iso);
    double d
      = (nullptr != nurbs_curve)
  ...
  bezCV[1][1] = MYON_3dPoint::Origin;
  bezCV[1][2] = MYON_3dPoint::Origin;
  bezCV[2][1] = MYON_3dPoint::Origin;
  bezCV[2][2] = MYON_3dPoint::Origin;

  MYON_BezierSurface bezS;
  bezS.m_dim = 3;
  bezS.m_is_rat = 0;
  bezS.m_order[0] = 4;
  bezS.m_order[1] = 4;
  ...
  MYON_3dPoint srfP[4] = { MYON_3dPoint::Origin,MYON_3dPoint::Origin,MYON_3dPoint::Origin,MYON_3dPoint::Origin };
  int hint[2] = {};
  for (int n = 0; n < 4; n++)
  {
    ...
    MYON_3dPoint K(0.0, 0.0, 0.0);
    MYON_3dPoint valQ(0, 0, 0);
    for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++)
    {
      if ((1 == i || 2 == i) && (1 == j || 2 == j))
        continue;
  ...
  const MYON_2dex cvdex[4]{MYON_2dex(1,1),MYON_2dex(2,1),MYON_2dex(1,2),MYON_2dex(2,2)};
  ...
  MYON_3dPoint bezP(MYON_3dPoint::Origin), nurbsP(MYON_3dPoint::Origin);
  hint[0] = 0;
  hint[1] = 0;
...
/*
static 
bool MYON_DuplicateKnots( int order, int cv_count, bool bRevKnot1,
                        double* knot0, 
                        double* knot1
                        )
{
...
static 
bool MYON_DuplicateContolPoints( int dim, bool is_rat, int cv_count, 
                               bool bRevCV1,
...
static bool MYON_IsDuplicateKnotVector( int order, int cv_count, 
               ...
               bool bIgnoreParameterization )
{
  ...
  if (rc)
  {
    const int knot_count = MYON_KnotCount( order, cv_count );
    int i;
    if ( bIgnoreParameterization )
    {
      const MYON_Interval dom(knot[order-2],knot[cv_count-1]);
      const MYON_Interval other_dom(other_knot[order-2],other_knot[cv_count-1]);
      double k, other_k;
      for ( i = 0; i < knot_count && rc; i++ )
      {
        k = dom.NormalizedParameterAt(knot[i]);
        other_k = dom.NormalizedParameterAt(other_knot[i]);
        rc = (fabs(k-other_k) <= MYON_ZERO_TOLERANCE);
      }
    }
    else
    {
...
static bool MYON_IsDuplicatePointList( int dim, bool is_rat,
                                     int count, 
                                     int stride, 
                                     const double* cv,
                                     int other_stride,
...
bool MYON_NurbsCurve::IsDuplicate( 
        const MYON_NurbsCurve& other, 
        bool bIgnoreParameterization,
        double tolerance 
        ) const
{
  ...
  {
    // compare knots
    rc = MYON_IsDuplicateKnotVector( m_order, m_cv_count, m_knot, other.m_knot, bIgnoreParameterization );

    // compare control points
    if (rc)
      rc = MYON_IsDuplicatePointList( m_dim, m_is_rat?1:0, m_cv_count,
                                    m_cv_stride, m_cv,
...
bool MYON_NurbsSurface::IsDuplicate( 
        const MYON_NurbsSurface& other, 
        bool bIgnoreParameterization,
        double tolerance 
        ) const
{
  ...
  {
    // compare knots
    rc = MYON_IsDuplicateKnotVector( m_order[0], m_cv_count[0], m_knot[0], other.m_knot[0], bIgnoreParameterization );
    if (rc)
      rc = MYON_IsDuplicateKnotVector( m_order[1], m_cv_count[1], m_knot[1], other.m_knot[1], bIgnoreParameterization );

    // compare control points
    int i;
    for ( i = 0; i < m_cv_count[0] && rc; i++ )
    {
      rc = MYON_IsDuplicatePointList( m_dim, m_is_rat?1:0, m_cv_count[1],
...
bool MYON_NurbsCurve::Reparameterize(double c)
{
  if ( !MYON_IsValid(c) || 0.0 == c )
    return false;
  ...
  return MYON_ReparameterizeRationalNurbsCurve(
           c,
...
bool MYON_ReparameterizeRationalNurbsCurve(
          double c, 
          int dim, 
          int order, 
          int cv_count,
          ...
          )
{
  ...
  if ( !MYON_IsValid(c) || !MYON_IsValid(c1) || 0.0 == c )
    return false;
...
bool MYON_NurbsCurve::ChangeEndWeights(double w0, double w1)
{
  if ( m_cv_count < m_order || m_order < 2 || 0 == m_cv )
    return false;

  if ( !MYON_IsValid(w0) || !MYON_IsValid(w1) || 0.0 == w0 || 0.0 == w1 )
    return false;
  ...
  return MYON_ChangeRationalNurbsCurveEndWeights(
          m_dim,m_order,
          m_cv_count,m_cv_stride,m_cv,
          m_knot,
          w0,w1);
...
bool MYON_ChangeRationalNurbsCurveEndWeights(
          int dim, 
          int order, 
          int cv_count,
          int cvstride, 
          ...
          )
{
  ...
  if ( !MYON_ClampKnotVector( dim+1, order, cv_count, cvstride, cv, knot, 2 ) )
    return false;

  v0 = cv[dim];
  v1 = cv[cvstride*(cv_count-1)+dim];
  if (!MYON_IsValid(v0) || !MYON_IsValid(v1) || v0 == 0.0 || v1 == 0.0)
    return false;
  if (v0 < 0.0 && v1 > 0.0)
    return false;
  if ( v0 > 0.0 && v1 < 0.0)
  ...
  r = w0/v0;
  s = w1/v1;
  if ( fabs(r-s) <= fabs(s)*MYON_SQRT_EPSILON )
  {
    // simply scale
    if ( r != s )
      s = 0.5*(r+s);
  ...
  if ( r != s )
  {
    v0 = cv[dim];
    v1 = cv[cvstride*(cv_count-1)+dim];
    if ( MYON_IsValid(v0) && MYON_IsValid(v1) && 0.0 != v0 )
    {
      // need to scale and reparameterize
      r = pow(w0/v0,1.0/((double)(order-1)));
      if ( !MYON_IsValid(r) )
        return false;
      if ( !MYON_ReparameterizeRationalNurbsCurve(r,dim,order,cv_count,cvstride,cv,knot) )
        return false;
    }
  }
...
double MYON_Fuzz( double x, double absolute_tolerance )
{
  double fuzz = fabs(x)*MYON_RELATIVE_TOLERANCE;
  return(fuzz > absolute_tolerance) ? fuzz : absolute_tolerance;
}

bool MYON_NurbsCurve::SpanIsSingular( 
  int span_index 
  ) const
{
  const int cv_size = CVSize();
  ...
  {
    MYON_ERROR("Invalid NURBS curve.");
    return false;
  }

  if ( span_index < 0 || span_index > m_cv_count-m_order )
  {
    MYON_ERROR("span_index parameter is out of range.");
    return false;
  }
  ...
  if ( knot[0] != knot[m_order-2] || knot[m_order-1] != knot[2*m_order-3] )
  {
    ...
    MYON_ConvertNurbSpanToBezier( cv_size, m_order, cv_size, p,
															  knot, knot[m_order-2], knot[m_order-1] 
                              );
    cv_stride = cv_size;
    cv = p;
  }
  const bool rc = MYON_PointsAreCoincident(m_dim,m_is_rat,m_order,cv_stride,cv);
  if ( 0 != p )
    onfree(p);
...
bool MYON_NurbsCurve::RemoveSpan(
  int span_index 
  )
{
  const int cv_size = CVSize();
  ...
  if ( m_cv_count == m_order )
  {
    MYON_ERROR("Cannot remove the only span from a Bezier NURBS curve.");
    return false;
  }
  ...
  else
  {
    ...
    if ( !(knot_delta > 0.0) )
    {
      ...
      {
        MYON_ERROR("span_index parameter identifies an empty span.");
        return false;
      }
    }
    ...
    if ( span_index0 >= 0 && k0 - i0 + 1 < m_order-1 )
    {
      ...
      MYON_ClampKnotVector( cv_size, m_order, m_order, cv_size, cv0, knot0_local, 1 );
    }
    ...
    if ( i1-k1 < m_order-2 )
      MYON_ClampKnotVector( cv_size, m_order, m_order, m_cv_stride, 
...
int MYON_NurbsCurve::RemoveSingularSpans()
{
  const int cv_size = CVSize();
  if (    m_order < 2 
       || m_cv_count < m_order
...
bool MYON_NurbsCurve::SubDFriendlyTag() const
{
  if (0 != (m_knot_capacity_and_tags & MYON_NurbsCurve::masks::subdfriendly_tag))
  {
    if (IsSubDFriendly(true))
      return true;
    // Something modified NURBS properties after the tag was set.
    const_cast<MYON_NurbsCurve*>(this)->SetSubDFriendlyTag(false);
  }
  return false;
}

void MYON_NurbsCurve::SetSubDFriendlyTag(
  bool bSubDFriendlyTag
)
{
  if (bSubDFriendlyTag && IsSubDFriendly(true))
    m_knot_capacity_and_tags |= MYON_NurbsCurve::masks::subdfriendly_tag;
  else
    m_knot_capacity_and_tags &= ~MYON_NurbsCurve::masks::subdfriendly_tag;
}
...
static bool Internal_IsSubDFriendlyEnd(
  ...
  const MYON_3dPoint& cv0,
  const MYON_3dPoint& cv1,
  const MYON_3dPoint& cv2
)
{
  MYON_3dPoint Q;
  if (0 == endex)
...
bool MYON_NurbsCurve::IsSubDFriendly(
  bool bPermitCreases
) const
{
  for (;;)
  {
    ...
    const MYON_SubD::SubDFriendlyKnotType knot_type = MYON_SubD::NurbsKnotType(m_order, m_cv_count, m_knot);
    if (MYON_SubD::SubDFriendlyKnotType::Unset == knot_type || MYON_SubD::SubDFriendlyKnotType::Unfriendly == knot_type)
      return false;
    if (MYON_SubD::SubDFriendlyKnotType::ClampedPiecewiseUniform == knot_type && false == bPermitCreases)
      return false;

    MYON_3dPoint CV[2] = { MYON_3dPoint::NanPoint,MYON_3dPoint::NanPoint };
    ...
    if ( bNaturalEnds && MYON_SubD::SubDFriendlyKnotType::ClampedPiecewiseUniform == knot_type )
    {
...
MYON_SubD::SubDFriendlyKnotType MYON_SubD::NurbsKnotType(
  int order,
  int cv_count,
  const double* knots
)
{
  return MYON_SubD::NurbsKnotType(order, cv_count, knots, nullptr);
}
  ...
  )
{
  ...
  for (;;)
  {
    ...
    const double deltatol = MYON_SQRT_EPSILON * delta;
    ...
    for (int knot_dex = 3; knot_dex < cv_count - 1; ++knot_dex)
    {
      ...
      return MYON_SubD::SubDFriendlyKnotType::Unfriendly;
    }
    ...
    return
      bClamped
      ? (bPiecewiseUniform ? MYON_SubD::SubDFriendlyKnotType::ClampedPiecewiseUniform : MYON_SubD::SubDFriendlyKnotType::ClampedUniform)
      : MYON_SubD::SubDFriendlyKnotType::UnclampedUniform;
  }
