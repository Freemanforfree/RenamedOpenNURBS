...
double MYON_TestMathFunction( int function_index, double x, double y )
{
  // This function is used to test the results of performing operations.
  //
  // module          function
  // opennurbs.dll   MYON_TestMathFunction
  // tl.dll          TL_TestMathFunction
  // rhino.exe       Rhino_TestMathFunction

  double z = MYON_UNSET_VALUE;
  int i;
...
bool MYON_PassesNanTest()
{

  bool bPassesAllNanTests = false;
  for (;;)
  {
    const double nan1 = MYON_DBL_QNAN;
    const double nan2 = MYON_DBL_QNAN;
    ...
    if (false == bPassesIEE754NanCompareTests)
    {
      ...
      MYON_ERROR("This compiler does not conform to the IEEE-754 nan compare specification. Some opennurbs code will fail.");
      break;
    }
    ...
    {
      // some opennurbs code will fail.
      MYON_ERROR("This compiler does not conform to the IEEE-754 nan arithmetic specification. Some opennurbs code will fail.");
      break;
    }
...
{
  if (!MYON_IsValid(angle_in_radians))
    return angle_in_radians;

  double d = angle_in_radians*MYON_RADIANS_TO_DEGREES;
  ...
  for (int i = 0; scale[i] > 0.0; i++)
  {
    ...
    if (f + 0.5 < ds)
      f += 1.0;
    if (fabs(f - ds) < MYON_EPSILON*scale[i])
    {
      d = f/scale[i];
      break;
    }
...
{
  return 
    (MYON_IsValid(angle_in_degrees))
    ? (angle_in_degrees*MYON_DEGREES_TO_RADIANS)
    : angle_in_degrees;
}


double MYON_ArrayDotProduct(int dim, const double* A, const double* B)
{
  double AoB;
...
double 
MYON_ArrayDotDifference( int dim, const double* A, const double* B, const double* C )
{
  // returns A o ( B - C )
...
double MYON_ArrayDistance(int dim, const double *A, const double *B)
{
  ...
  case 3:
    ...
    if (a >= b) {
      if (a >= c) {
        if      (a == 0.0)         len = 0.0;
        else if (a == b && a == c) len = a*MYON_SQRT3;
...
double MYON_ArrayDistanceSquared(int dim, const double* A, const double* B)
{
  // returns (A-B)o(A-B)
...
double MYON_ArrayMagnitude(int dim, const double* A)
{
  double a, b, c, len;
  switch(dim) {
  case 1:
...
double MYON_ArrayMagnitudeSquared(int dim, const double* A)
{
  double x, len_sq=0.0;
  while (dim--) {
    x = *A++;
...
void 
MYON_ArrayScale( int dim, double s, const double* A, double* sA )
{
  if ( dim > 0 ) {
    while ( dim-- )
      *sA++ = s * *A++;
...
void
MYON_Array_aA_plus_B( int dim, double a, const double* A, const double* B, double* aA_plus_B )
{
  if ( dim > 0 ) {
    while ( dim-- )
      *aA_plus_B++ = a * *A++ + *B++;
...
float 
MYON_ArrayDotProduct( int dim, const float* A, const float* B )
{
  float d = 0.0;
  if ( dim > 0 ) {
    while(dim--)
...
void
MYON_ArrayScale( int dim, float s, const float* A, float* sA )
{
  if ( dim > 0 ) {
    while ( dim-- )
      *sA++ = s * *A++;
...
void
MYON_Array_aA_plus_B( int dim, float a, const float* A, const float* B, float* aA_plus_B )
{
  if ( dim > 0 ) {
    while ( dim-- )
      *aA_plus_B++ = a * *A++ + *B++;
...
int 
MYON_DecomposeVector(
        const MYON_3dVector& V,
        const MYON_3dVector& A,
        const MYON_3dVector& B,
        double* x, double* y
        )
{
  int rank;
  ...
  rank =  MYON_Solve2x2(  AoA, AoB, AoB, BoB, AoV, BoV, x, y, &pr );
  return (rank==2)?true:false;
}


bool 
MYON_EvJacobian( double ds_o_ds, double ds_o_dt, double dt_o_dt,
                    double* det_addr )
 ...
 *
 * RELATED FUNCTIONS:
 *      MYON_EvBsplineBasis(), MYON_EvdeCasteljau(), MYON_EvBezier()
 */
{
  bool rc = false;
  double det, a, b;
  ...
  det = a - b;
  if (ds_o_ds <= dt_o_dt* MYON_EPSILON || dt_o_dt <= ds_o_ds* MYON_EPSILON) {
    /* one of the paritals is (numerically) zero with respect
     * to the other partial - value of det is unreliable
     */
    rc = false;
  }
  else if (fabs(det) <= ((a > b) ? a : b)* MYON_SQRT_EPSILON) {
...
bool 
MYON_EvNormalPartials(
        const MYON_3dVector& ds,
        const MYON_3dVector& dt,
        const MYON_3dVector& dss,
        const MYON_3dVector& dst,
        const MYON_3dVector& dtt,
        MYON_3dVector& ns,
        MYON_3dVector& nt
        )
{
  ...
  rc = MYON_EvJacobian( ds_o_ds, ds_o_dt, dt_o_dt, nullptr );
  if (!rc) 
  {
    // degenerate Jacobian and unit surface normal is not well defined
    ns = MYON_3dVector::ZeroVector;
    nt = MYON_3dVector::ZeroVector;
  }
  else 
  {
    ...
    // and second partials.
    MYON_3dVector V = MYON_CrossProduct(ds,dt);
    double len = V.Length();
...
bool 
MYON_Pullback3dVector( // use to pull 3d vector back to surface parameter space
      const MYON_3dVector& vector,   // 3d vector
      ...
      const MYON_3dVector&  ds,      // surface first partials
      const MYON_3dVector&  dt,
      const MYON_3dVector&  dss,     // surface 2nd partials
      const MYON_3dVector&  dst,     // (used only when dist != 0)
      const MYON_3dVector&  dtt, 
      MYON_2dVector&  pullback       // pullback
      )
{
  bool rc = false;
  ...
  if (distance != 0.0) {
    MYON_3dVector ns, nt;
    rc = MYON_EvNormalPartials(ds,dt,dss,dst,dtt,ns,nt);
    if ( rc ) {
      // adjust ds and dt to take account of offset distance
      rc = MYON_DecomposeVector( vector, ds + distance*ns, dt + distance*nt, &pullback.x, &pullback.y );
    }
  }
  else {
    rc = MYON_DecomposeVector( vector, ds, dt, &pullback.x, &pullback.y );
  }
  return rc;
}
        ...
        )
{
  ...
  if ( rc ) {
    if ( t < t0 )
      ...
      t = t1;
    double dt = (t1-t0)*8.0* MYON_SQRT_EPSILON + (fabs(t0) + fabs(t1))* MYON_EPSILON;
    if ( dt >= t1-t0 )
      dt = 0.5*(t1-t0);
...
bool
MYON_EvNormal(int limit_dir,
                const MYON_3dVector& Du, const MYON_3dVector& Dv, 
                const MYON_3dVector& Duu, const MYON_3dVector& Duv, const MYON_3dVector& Dvv, 
                MYON_3dVector& N)
{
  const double DuoDu = Du.LengthSquared();
  const double DuoDv = Du*Dv;
  const double DvoDv = Dv.LengthSquared();
  if ( MYON_EvJacobian(DuoDu,DuoDv,DvoDv,nullptr) ) {
    N = MYON_CrossProduct(Du,Dv);
  }
  else {
...
bool MYON_EvTangent( 
        const MYON_3dVector& D1, // first derivative
        const MYON_3dVector& D2, // second derivative
        MYON_3dVector& T         // Unit tangent returned here
        )
{
...
MYON_EvCurvature( 
        ...
        const MYON_3dVector& D2, // second derivative
        MYON_3dVector& T,       // Unit tangent returned here
        MYON_3dVector& K        // Curvature returned here
        )
{
...
bool MYON_EvSectionalCurvature( 
    const MYON_3dVector& S10, 
    const MYON_3dVector& S01,
    const MYON_3dVector& S20, 
    const MYON_3dVector& S11, 
    const MYON_3dVector& S02,
    const MYON_3dVector& planeNormal,
    MYON_3dVector& K 
    )
{
  MYON_3dVector M, D1, D2;
  double a, b, e, pr;
  int rank;
  ...
  //D1 = MYON_CrossProduct(M,sectionN);
  ...
  rank = MYON_Solve3x2( &S10.x, &S01.x, D1.x, D1.y, D1.z, &a, &b, &e, &pr );
  if ( rank < 2 )
  {
    K.x = 0.0;
    K.y = 0.0;
  ...
  //M1 = MYON_CrossProduct(a*S20 + b*S11, S01) + MYON_CrossProduct(S10, a*S11 + b*S02);
  ...
  //D2 = MYON_CrossProduct(M1,sectionN);
...
bool MYON_IsContinuous(
  MYON::continuity desired_continuity,
  MYON_3dPoint Pa, MYON_3dVector D1a, MYON_3dVector D2a,
  MYON_3dPoint Pb, MYON_3dVector D1b, MYON_3dVector D2b,
  ...
  )
{
  ...
  switch( MYON::ParametricContinuity((int)desired_continuity) )
  {
  ...
  case MYON::continuity::Gsmooth_continuous:
    ...
    if ( !(Pa-Pb).IsTiny(point_tolerance) || Ta*Tb < cos_angle_tolerance )
      return false;
    if ( MYON::continuity::Gsmooth_continuous == desired_continuity )
    {
      if ( !MYON_IsGsmoothCurvatureContinuous(Ka,Kb,cos_angle_tolerance,curvature_tolerance) )
        return false;
    }
    else
    {
      if ( !MYON_IsG2CurvatureContinuous(Ka,Kb,cos_angle_tolerance,curvature_tolerance) )
        return false;
    }
    break;
  }
...
int 
MYON_SearchMonotoneArray(const double* array, int length, double t)
/*****************************************************************************
...
EXAMPLE:
  ...
  i = MYON_GetdblArrayIndex(knot+order-2,cv_count-order+2,t);
...
double 
MYON_BinomialCoefficient(int i, int j)
{
...
static const double bc[((MAX_HALF_N-2)*(MAX_HALF_N-1))/2 + MAX_HALF_N - 2] =
 {15.0, 20.0, 28.0, 56.0, 70.0, 45.0, 120.0, 210.0, 252.0, 66.0,
  ...
  if (n%2)
    return MYON_BinomialCoefficient(i-1,j)+MYON_BinomialCoefficient(i,j-1);
...
MYON_DECL
double MYON_TrinomialCoefficient( 
          int i,
          int j,
          int k
          )
{
  return (MYON_BinomialCoefficient(i,j+k)*MYON_BinomialCoefficient(j,k));
}


bool 
MYON_IsValidPointList(
       int dim,
       bool is_rat,
       int count,
       int stride,
...
{
  if ( !MYON_IsValidPointList(dim,is_rat,count,stride,p) )
    return false;
  if ( is_rat )
    dim++;
  if ( count <= 1 )
        ...
        )
{
  bool rc = false;
  if ( !dir ) {
    rc = MYON_ReversePointGrid( dim, is_rat, point_count1, point_count0, point_stride1, point_stride0, p, 1 );
  }
  else {
    int i;
    for ( i = 0; i < point_count0; i++ ) {
      if ( !MYON_ReversePointList( dim, is_rat, point_count1, point_stride1, p + i*point_stride0 ) ) {
        rc = false;
        break;
      }
      else if (!i) {
...
bool 
MYON_SwapPointListCoordinates( int count, int stride, float* p,
                                   int i, int j )
{
  float t;
  int k;
  if ( !MYON_IsValidPointList(stride,0,count,stride,p) )
    return false;
...
bool 
MYON_SwapPointListCoordinates( int count, int stride, double* p,
                                             int i, int j )
{
  double t;
  int k;
                  ...
                  )
{
  ...
  if ( !MYON_IsValidPointList( dim, is_rat, count, stride, point ) )
    return false;
                  ...
                  )
{
  ...
  double* pt = point;
  for ( i = 0; i < point_count0; i++ ) {
    if ( !MYON_TransformPointList( dim, is_rat, point_count1, point_stride1, pt, xform ) ) {
      rc = false;
    }
    else if ( !i ) {
      rc = true;
                  ...
                  )
{
  ...
  if ( !MYON_IsValidPointList( dim, 0, count, stride, vector ) )
    return false;
    ...
    )
{
  ...
  if ( is_rat )
  {
    ...
    {
      if ( 0.0 == wa && 0.0 == wb )
        return MYON_PointsAreCoincident(dim,0,pointA,pointB);
      return false;
    }
    while(dim--)
    {
      ...
      if ( d <= MYON_ZERO_TOLERANCE )
        continue;
      if ( d <= (fabs(a)+fabs(b))*MYON_RELATIVE_TOLERANCE )
        continue;
      return false;
    }
  }
    ...
    )
{
  ...
  if ( point_stride < (is_rat?(dim+1):dim) )
    return false;
  if ( false == MYON_PointsAreCoincident( dim, is_rat, points, points + ((point_count-1)*point_stride) ) )
    return false;
  if ( point_count > 2 )
  {
    point_count--;
    while ( point_count-- )
    {
      if ( false == MYON_PointsAreCoincident(dim,is_rat,points,points + point_stride ) )
        return false;
      points += point_stride;
    }
  }
          ...
          )
{
  ...
  for ( i = 0; i < dim; i++ )
  {
    a = wA* *pointA++;
    b = wB* *pointB++;
    tol = (fabs(a) + fabs(b))* MYON_RELATIVE_TOLERANCE;
    if ( tol <  MYON_ZERO_TOLERANCE )
      tol =  MYON_ZERO_TOLERANCE;
    if ( a < b-tol )
      return -1;
    if ( b < a-tol )
      return 1;
  }
  if ( wA < wB- MYON_SQRT_EPSILON )
    return -1;
  if ( wB < wA- MYON_SQRT_EPSILON )
    return -1;
  return 0;
}
          ...
          )
{
  ...
  const bool bDoSecondCheck = ( 1 == is_rat && dim <= 3 && point_count > 0 
                         && MYON_IsValid(pointA[dim]) && 0.0 != pointA[dim]
                         && MYON_IsValid(pointB[dim]) && 0.0 != pointB[dim]
                         );
  ...
  {
    rc = MYON_ComparePoint( dim, is_rat, pointA, pointB );
      ...
      )
    {
      ...
      rc = ( 0 == MYON_ComparePoint( dim, 0, A, B ) ) ? 0 : rc1;
    }
...
{
  bool rc = false;
  if ( count >= 4 && 0 == MYON_ComparePoint( dim, is_rat, p, p+stride*(count-1) ) ) 
  {
    // a bunch of points piled on top of each other is not considered to be closed.
    for ( int i = 1; i < count-1; i++ ) {
      if ( MYON_ComparePoint( dim, is_rat, p, p+stride*i ) ) {
        rc = true;
        break;
      }
    }
       ...
       )
{
  ...
  if ( point_count0>0 && point_count1>0 && p != nullptr ) {
    ...
    rc = (0==MYON_ComparePointList( dim, is_rat, count, stride, p0, stride, p1 ))?true:false;
  }
  return rc;
}
 ...
 *
 *   When |b^2 - 4*a*c| <= b*b*MYON_EPSILON, the discriminant
...
)
{
  ...
  if (a == 0.0)
  {
    ...
    else
    {
      // quadratic equation
      double rr0, rr1;
      int qrc = MYON_SolveQuadraticEquation(b, c, d, &rr0, &rr1);
      switch (qrc)
      {
      case 0:
  ...
  else
  {
    ...
    if (R*R < Q*Q*Q)
    {
      ...
      *r2 = -2.0 * sqrt(Q) * cos((Theta + 2.0*MYON_PI) / 3.0) - b / 3.0;
      *r3 = -2.0 * sqrt(Q) * cos((Theta - 2.0*MYON_PI) / 3.0) - b / 3.0;
      // inline bubble sort 
...
int MYON_SolveTriDiagonal( int dim, int n, 
...
OUTPUT:
  MYON_SolveTriDiagonal()  0: success
                        -1: failure - illegal input
                        -2: failure - zero pivot encountered
                                      (can happen even when matrix is
                                       non-singular)

  X     if MYON_SolveTriDiagonal() returns 0, then X is the solution to
...
COMMENTS:
  If n <= 3, this function uses MYON_Solve2x2() or MYON_Solve3x3().  
  ...
  d array will not be used after the call to MYON_SolveTriDiagonal(), then
...
int
MYON_Solve2x2( double m00, double m01, double m10, double m11, double d0, double d1,
 ...
 *      address of double
 * OUTPUT:
 *   MYON_Solve2x2() returns rank (0,1,2)
 *
 *   If MYON_Solve2x2() is successful (return code 2), then
 ...
 *      rc = MYON_Solve2x2(D0[0],-D1[0],D0[1],-D1[1],P1[0]-P0[0],P1[1]-P0[1],
 *                       &x, &y,&pivot_ratio);
 *      switch(rc) {
...
int 
MYON_Solve3x2(const double col0[3], const double col1[3], 
...
double MYON_SolveNxN(bool bFullPivot, bool bNormalize, int n, double* M[], double B[], double X[])
{
  if ( n <= 0 || 0 == M || 0 == B || 0 == X )
    return 0.0;
...
int
MYON_Solve4x4(const double row0[4], const double row1[4], const double row2[4], const double row3[4],
...
int
MYON_Solve3x3(const double row0[3], const double row1[3], const double row2[3],
                double d0, double d1, double d2,
...
void
MYON_Sort(MYON::sort_algorithm method,
  int* index,
  const void* data,
  size_t count,
  ...
  )
{
  ...
  MYON_Sort(method, uindex, data, count, sizeof_element, compar);
}
  ...
  )
{
  ...
  idx = index;
  if ( MYON::sort_algorithm::quick_sort == method )
  {
    MYON_qsort(idx,count,sizeof(idx[0]),qicompar2,&context);
  }
  else
  {
    // heap sort
...
{
  unsigned int* uindex = (unsigned int*)index;
  MYON_Sort(method,uindex,data,count,sizeof_element, compar, p);
}
  ...
  )
{
  ...
  if ( MYON::sort_algorithm::quick_sort == method )
  {
    MYON_qsort(idx,count,sizeof(idx[0]),qicompar3,&context);
  }
  else
  {
    // heap sort
...
static void MYON_hsort_str(char **e, size_t nel)
{
  size_t
    i_end,k;
  char
...
const int* MYON_BinarySearchIntArray( int key, const int* base, size_t nel )
{
  if (nel > 0 && base )
  {
    size_t i;
...
const unsigned int* MYON_BinarySearchUnsignedIntArray( unsigned int key, const unsigned int* base, size_t nel )
{
  if (nel > 0 && base )
  {
    size_t i;
...
)
{
  ...
  if (count > 0 && nullptr != base && key_offset + sizeof(key) <= sizeof_element)
  {
    ...
    size_t i;
    MYON_2udex d = *((const MYON_2udex*)(a + (count - 1) * sizeof_element));
    if (key <= d)
    {
      while (count > 0)
      {
        i = count / 2;
        d = *((const MYON_2udex*)(a + i * sizeof_element));
        if (key < d)
        {
          count = i;
        }
...
const double* MYON_BinarySearchDoubleArray( double key, const double* base, size_t nel )
{
  if (nel > 0 && base )
  {
    size_t i;
...
int MYON_Compare2dex( const MYON_2dex* a, const MYON_2dex* b)
{
  int d;
  if ( 0 == (d = (a->i - b->i)) )
  {
...
int MYON_Compare2udex(const MYON_2udex* a, const MYON_2udex* b)
{
  if (a->i < b->i) return -1;
  if (b->i < a->i) return 1;
...
int MYON_Compare3dex( const MYON_3dex* a, const MYON_3dex* b)
{
  int d;
  if ( 0 == (d = (a->i - b->i)) )
  {
...
int MYON_Compare4dex( const MYON_4dex* a, const MYON_4dex* b)
{
  int d;
  if ( 0 == (d = (a->i - b->i)) )
  {
        ...
        )
{
  if ( nel > 1 )
  {
    switch ( method ) 
    {
    case MYON::sort_algorithm::heap_sort:
      MYON_hsort_str( e, nel );
      break;
    case MYON::sort_algorithm::quick_sort:
    default:
      MYON_qsort( e, nel, sizeof(*e), compar_string );
      break;
    }
  }
}
...
See Also:
  MYON_EvaluateQuotientRule2
  MYON_EvaluateQuotientRule3
*/

bool MYON_EvaluateQuotientRule( int dim, int der_count, int v_stride, double *v )
{
  /*
          ...
          f  (t) =  x   (t)  -  (w  (t)*f(t) + n*w    (t)*f  (t) + ... + n*w  (t)*f    (t))
                    ---------------------------------------------------------------------
                                             w(t)
                                                                        ...
                                                                        (i)   (j)
         (The missing summands look like  MYON_BinomialCoefficient(i,j)*w   * f    )
  */
      ...
      if (der_count>2) {
        ...
        for (n = 3; n <= der_count; n++) {
          ...
          for (i = 0; i < n; i++) {
            ...
            wt = -MYON_BinomialCoefficient(n-i,i) * *w;
            w -= v_stride;
...
Returns:
  True if input is valid; i.e., v[dim] != 0.
See Also:
  MYON_EvaluateQuotientRule
  MYON_EvaluateQuotientRule3
*/

bool MYON_EvaluateQuotientRule2( int dim, int der_count, int v_stride, double *v )
{
  double
...
bool MYON_EvaluateQuotientRule3( int dim, int der_count, int v_stride, double *v )
{
  double
        ...
        for ( n = 3; n <= der_count; n++ ) 
        {
          for ( i = n; i >= 0; i-- ) 
          {
            for ( j = n-i; j >= 0; j-- )
            {
              ...
              for ( ii = 0; ii <= i; ii++ ) 
              {
                wr = MYON_BinomialCoefficient(ii,i-ii);
                for ( jj = 0; jj <= j; jj++ ) 
                {
                  ws = wr*MYON_BinomialCoefficient(jj,j-jj);
                  for ( kk = (ii||jj)?0:1; kk <= k; kk++ ) 
                  {
                    ...
                    wt = -ws*MYON_BinomialCoefficient(kk,k-kk)*v[Fn];
                    q = n-q;
                    Fn = q-i+ii;
...
//#define MYON_TEST_EV_KAPPAS

#if defined(MYON_TEST_EV_KAPPAS)

void MYON_EPC_WARNING(const char* msg)
{
  MYON_Warning(__FILE__,__LINE__,msg);
}

#else

#define MYON_EPC_WARNING(msg) 

#endif

bool MYON_EvPrincipalCurvatures( 
        const MYON_3dVector& Ds,
        const MYON_3dVector& Dt,
        const MYON_3dVector& Dss,
        const MYON_3dVector& Dst,
        const MYON_3dVector& Dtt,
        const MYON_3dVector& N, // unit normal (use TL_EvNormal())
        ...
        MYON_3dVector& K1,      // kappa1 unit principal curvature direction
        MYON_3dVector& K2       // kappa2 unit principal curvature direction
        ...
        )
{
  ...
  const double n = N.x*Dtt.x + N.y*Dtt.y + N.z*Dtt.z;
	...
	return MYON_EvPrincipalCurvatures(  Ds, Dt, l, m, n, N, 
        ...
        )
{
  ...
  //return MYON_WRONG_EvPrincipalCurvatures( Ds, Dt, Dss, Dst, Dtt, N, gauss, mean, kappa1, kappa2, K1, K2 );
...

  {
    ...
    {
      MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() Det(shape op) != gaussian");
    }
    if ( fabs(tttt - trace) > 1.0e-4*fabs(trace) )
    {
      MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() Trace(shape op) != trace");
    }
    ...
    {
      MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() k1 is bogus");
    }
    if ( fabs(zzz2) > (fabs(trace)+fabs(det))*1e-10 )
    {
      MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() k2 is bogus");
    }
#endif
      ...
      if ( fabs(k1-k2) > 1.0e-6*(fabs(k1) + fabs(k2)) ) 
      {
        ...
        for ( ki = 0; ki < 2; ki++ ) 
        {
          ...
          if ( fabs(x_local) > 1.0e-8 )
          {
            if ( 0==ki )
            {
              MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() |det(shape op - [k1,k1])| > 1.0e-8");
            }
            else
            {
              MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() |det(shape op - [k2,k2])| > 1.0e-8");
            }
          }
#endif
          ...
          if ( fabs(x_local) > 1.0e-8 || fabs(y) > 1.0e-8 )
          {
            if ( 0==ki )
            {
              MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() (shape op k1 eigenvector is noisy).");
            }
            else
            {
              MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() (shape op k2 eigenvector is noisy).");
            }
          }
#endif
        ...
        bFixK1 = bFixK2 = false;
        {
          ...
          {
            MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() K1*N > 1.0e-4.");
            bFixK1 = true;
          }
          ...
          if ( fabs(x_local) >= 1.0e-4 )
          {
            MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() K2*N > 1.0e-4.");
            bFixK2 = true;
          }
        }
        ...
        if ( !bFixK1 && !bFixK2 ) 
        {
          ...
          if ( fabs(x_local) >= 1.0e-4 ) 
          {
#if defined(MYON_TEST_EV_KAPPAS)
            {
              ...
              if ( !bSecondTry )
              {
                MYON_EPC_WARNING("MYON_EvPrincipalCurvatures() K1*K2 > 0.1.");
                ...
                MYON_3dVector KKK1, KKK2;
                MYON_EvPrincipalCurvatures(Ds,Dt,l,m,n,N,
        ...
        if ( bFixK1 || bFixK2 ) 
        {
          ...
          {
            K1 = MYON_CrossProduct( K2, N );
            K1.Unitize();
          }
          else if ( bFixK2 ) 
          {
            K2 = MYON_CrossProduct( N, K1 );
            K2.Unitize();
          }
        }
      }
...
MYON_3dVector 
MYON_NormalCurvature( 
    const MYON_3dVector& S10, const MYON_3dVector& S01,
    const MYON_3dVector& S20, const MYON_3dVector& S11, const MYON_3dVector& S02,
    const MYON_3dVector& UnitNormal, const MYON_3dVector& UnitTangent )
/*****************************************************************************
...
*****************************************************************************/
{
  MYON_3dVector NormalCurvature, D2, T, K;
  double a, b, d, e, pr;
  int rc;

  a = b = 0.0;
  // solve T = a*S10 + b*S01
  rc = MYON_Solve3x2( S10, S01, UnitTangent.x, UnitTangent.y, UnitTangent.z, 
                    &a, &b, &e, &pr );
  if (rc < 2) 
  {
    NormalCurvature = MYON_3dVector::ZeroVector;
  }
  else 
  {
    ...
    MYON_EvCurvature( UnitTangent, D2, T, K );
...
#define SUM_SIZE 128
  ...
  if (is_rat) {
    w1 = p1[dim];
    if (w1 == 0.0) {
      MYON_ERROR("MYON_GetPolylineLength: Zero weight");
      return false;
    }
    w1 = 1.0/w1;
    for ( i = 1; i < count; i++ ) {
...
int MYON_Evaluator::EvaluateHessian(
...
bool MYON_Evaluator::FiniteDomain() const
{
...
MYON_Interval MYON_Evaluator::Domain(
  int parameter_index
  ) const
{
  return ((m_parameter_count==m_domain.Count() && m_parameter_count>0) 
          ? m_domain[parameter_index] 
          : MYON_Interval(-1.0e300,1.0e300)
          );
}

double MYON_Max(double a, double b)
{
  if (a >= b)
    return a;
  if (b > a)
...
float MYON_Max(float a, float b)
{
  if (a >= b)
    return a;
  if (b > a)
...
int MYON_Max(int a, int b)
{
  return (a<b)? b:a;
}

double MYON_Min(double a, double b)
{
  if (a <= b)
    return a;
  if (b < a)
...
float MYON_Min(float a, float b)
{
  if (a <= b)
    return a;
  if (b < a)
...
int MYON_Min(int a, int b)
{
  return (a <= b) ? a : b;
}

int MYON_Round(double x)
{
  ...
  if (!MYON_IsValid(x))
  {
    // NaN, infinite, MYON_UNSET_VALUE,
    MYON_ERROR("MYON_Round - invalid input");
    return 0;
  }

  MYON_ERROR("MYON_Round - integer overflow");
  return (x > 0.0) ? 2147483647 : -2147483647;
}

double MYON_LinearInterpolation(double t, double x, double y)
{
  double z;
  if (x == y && t == t)
  {
...
static 
bool MYON_SymTriDiag3x3EigenSolver(double A, double B, double C,
	double D, double E,
	double* e1, MYON_3dVector& E1,
	double* e2, MYON_3dVector& E2,
	double* e3, MYON_3dVector& E3);

static
bool MYON_TriDiagonalQLImplicit(double* d, double* e, int n, MYON_Matrix* pV);
...
True if successful.
*/
bool MYON_Sym3x3EigenSolver(double A, double B, double C,
...
)
{
	...
	double ee1, ee2, ee3;
	MYON_3dVector EE1, EE2, EE3;
	bool rc = MYON_SymTriDiag3x3EigenSolver(AA, BB, CC, DD, EE,
		&ee1, EE1,
		&ee2, EE2,
		&ee3, EE3);
...
)
{
	...
	MYON_Matrix V(3, 3);
	bool rc = MYON_TriDiagonalQLImplicit(d, e, 3, &V);
	if (rc)
	{
		if (e1) *e1 = d[0];
		E1 = MYON_3dVector(V[0][0], V[1][0], V[2][0]);
		if (e2) *e2 = d[1];
		E2 = MYON_3dVector(V[0][1], V[1][1], V[2][1]);
		if (e3) *e3 = d[2];
		E3 = MYON_3dVector(V[0][2], V[1][2], V[2][2]);
	}
	return rc;
}
...
{
	/*  Debug code
	MYON_SimpleArray<double> OrigD(n);
	MYON_SimpleArray<double> OrigE(n);
	MYON_SimpleArray<double> Test(n);
	...
	for (int l = 0; l<n; l++)
	{
		...
		do
		{
			for (m = l; m<n - 1; m++)
			{
				if (fabs(e[m]) < MYON_EPSILON*(fabs(d[m]) + fabs(d[m + 1])))
					break;
			}
			if (m != l)
			{
...
unsigned MYON_GreatestCommonDivisor(
  unsigned a,
  unsigned b
)
{
...
unsigned MYON_LeastCommonMultiple(
  unsigned a,
  unsigned b
)
{
  ...
  const unsigned gcd = MYON_GreatestCommonDivisor(a, b);
  a /= gcd;
  b /= gcd;

  // 0 is returned when the a*b*gcd overflows unsigned storage.
  return (a * b < MYON_UINT_MAX / gcd) ? (a * b * gcd) : 0U;
}
