...
void MYON_NumberFormatter::Fraction(double value_in, int& wholenumber, int& numerator, int& denominator, int precision)
{
  int sign = 1;
  if (0 > value_in)
  {
...
double MYON_NumberFormatter::RoundOff(double n, double r)
{
  if (0.0 == r)
    return n;
...
void MYON_NumberFormatter::SuppressZeros(
  MYON_wString& dist,
  MYON_DimStyle::suppress_zero sz)
{
  ...
  {
  default:
  case MYON_DimStyle::suppress_zero::None:
    break;
  case MYON_DimStyle::suppress_zero::SuppressLeading:
    if (1 < dist.Length() && L'0' == dist[0])
      dist = dist.Right(dist.Length() - 1);
    break;
  case MYON_DimStyle::suppress_zero::SuppressLeadingAndTrailing:
  case MYON_DimStyle::suppress_zero::SuppressTrailing:
  {
    ...
    if (MYON_DimStyle::suppress_zero::SuppressLeadingAndTrailing == sz 
       && 1 < dist.Length() && L'0' == dist[0])
      dist = dist.Right(dist.Length() - 1);
  }
    break;
  case MYON_DimStyle::suppress_zero::SuppressZeroFeet:
    if (dist.Left(3).EqualOrdinal(L"0\'-", true))
      dist = dist.Right(dist.Length() - 3);
    break;
  case MYON_DimStyle::suppress_zero::SuppressZeroFeetAndZeroInches:
    if (dist.Left(3).EqualOrdinal(L"0\'-", true))
      dist = dist.Right(dist.Length() - 3);
  case MYON_DimStyle::suppress_zero::SuppressZeroInches:
...
bool MYON_NumberFormatter::bFormatIsAccurate = false;


bool MYON_NumberFormatter::FormatNumber(
  double inputdistance,
  MYON_DimStyle::OBSOLETE_length_format output_lengthformat,
  double round_off,
  int precision,
  ...
  MYON_wString& output)
{
  ...
  if (!MYON_IsValid(inputdistance))
  {
    if (MYON_UNSET_VALUE == inputdistance)
      output = "unset number";
    else
  ...
  case(MYON_DimStyle::OBSOLETE_length_format::Decimal):
  {
    if (precision >= 1 && precision < 10)
    {
      ...
      // (Note: input * MYON_EPSILON didn't work)
    ...
    if (sFormat.Length() > max_string_length)
      sFormat = MYON_wString::FromNumber(distance);

    MYON_NumberFormatter::SuppressZeros(sFormat, zero_suppress);
    break;
  }
...
bool MYON_NumberFormatter::FormatLength(
  double inputdistance,
  ...
  MYON_wString& output)
{
  MYON_DimStyle::OBSOLETE_length_format output_lengthformat = MYON_DimStyle::OBSOLETE_length_format::Decimal;
  if (output_lengthdisplay == MYON_DimStyle::LengthDisplay::FeetAndInches)
    output_lengthformat = MYON_DimStyle::OBSOLETE_length_format::FeetInches;
  else if (output_lengthdisplay == MYON_DimStyle::LengthDisplay::InchesFractional)
    output_lengthformat = MYON_DimStyle::OBSOLETE_length_format::Fractional;

  return MYON_NumberFormatter::FormatNumber(
    inputdistance,
    output_lengthformat,
    round_off,
    precision,
...
bool MYON_NumberFormatter::FormatAngleStringDMS(double angle_radians, MYON_wString& formatted_string)
{
  return MYON_NumberFormatter::FormatAngleStringDMS(angle_radians, 2, formatted_string);
  //bool rc = false;
  ...
  //double angle_degrees = MYON_RADIANS_TO_DEGREES * angle_radians;
  //if (MYON_IsValid(angle_degrees))
  //{
  //  double d_seconds;
  ...
  //  degrees *= sign;
  //  formatted_string.Format(L"%d%lc %d\' %d\"", degrees, MYON_wString::DegreeSymbol, minutes, seconds);
  //  rc = true;
  //}
  //return rc;
}

bool MYON_NumberFormatter::FormatAngleStringDMS(double angle_radians, int resolution, MYON_wString& formatted_string)
{
  bool rc = false;
  formatted_string.Empty();
  int idegrees = 0;
  ...
  if (resolution == 0)
  {
    ...
    idegrees *= sign;
    rc = formatted_string.Format(L"%d%lc", idegrees, MYON_wString::DegreeSymbol);
  }
  else
  {
    ...
    if (resolution == 1)
    {
      ...
      rc = formatted_string.Format(L"%d%lc %d\'", idegrees, MYON_wString::DegreeSymbol, iminutes);
    }
    else
    {
      ...
      if (resolution == 2)
      {
        ...
        rc = formatted_string.Format(L"%d%lc %d\' %d\"", idegrees, MYON_wString::DegreeSymbol, iminutes, iseconds);
      }
      else 
      {
        if (seconds >= 60.0)
        ...
        rc = formatted_string.Format(fmt.Array(), idegrees, MYON_wString::DegreeSymbol, iminutes, seconds);
      }
    }
  }
  return rc;
...
bool MYON_NumberFormatter::FormatAngleStringDecimal(
  double angle,
  int resolution,
  double roundoff,
  MYON_DimStyle::suppress_zero zero_suppress,
  MYON_wString& formatted_string)
{
  ...
  return MYON_NumberFormatter::FormatNumber(
    angle,
    MYON_DimStyle::OBSOLETE_length_format::Decimal,
    roundoff,
    resolution,
    zero_suppress,
    false,
