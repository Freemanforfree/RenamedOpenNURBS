...
#if !defined(MYON_ARC_INC_)
#define MYON_ARC_INC_

/*
Description:
	An MYON_Arc is a subcurve of 3d circle. 
Details:
	...
	The parameterization of the MYON_Arc is inherited from the MYON_Circle it is derived from.
	In particular
			 t -> center + cos(t)*radius*xaxis + sin(t)*radius*yaxis	
	where xaxis and yaxis, (part of MYON_Circle::m_plane) form an othonormal frame of the plane 
	containing the circle.
*/
class MYON_CLASS MYON_Arc : public MYON_Circle
{
public:
  // Create a radius one arc with angle = 2*pi
  MYON_Arc() = default;
  ~MYON_Arc() = default;
  MYON_Arc(const MYON_Arc&) = default;
  MYON_Arc& operator=(const MYON_Arc&) = default;

  MYON_Arc& operator=( const MYON_Circle& );

  static const MYON_Arc UnitCircle; // unit circle in the xy plane
  ...
  */
  ...
  Parameters:
    ...
    angle_interval_in_radians - [in] increasing angle interval
       in radians with angle_interval_in_radians.Length() <= 2.0*MYON_PI.
  */
  MYON_Arc(
    const MYON_Circle& circle,
  ...
  */
  MYON_Arc(
    const MYON_2dPoint& start_point,
    const MYON_2dPoint& interior_point,
    const MYON_2dPoint& end_point
    );
  ...
  */
  MYON_Arc(
    const MYON_3dPoint& start_point,
    const MYON_3dPoint& interior_point,
    const MYON_3dPoint& end_point
    );
  ...
  */
  ...
  Parameters:
    ...
    angle_interval_in_radians - [in] increasing angle interval in radians
              with angle_interval_in_radians.Length() <= 2.0*MYON_PI
  Returns:
    true if input is valid and a valid arc is created.
  */
  bool Create(
  ...
  */
  bool Create(
    const MYON_2dPoint& start_point,
    const MYON_2dVector& dir_at_start,
    const MYON_2dPoint& end_point
    );
  ...
  */
  bool Create(
    const MYON_3dPoint& start_point,
    const MYON_3dVector& dir_at_start,
    const MYON_3dPoint& end_point
    );
  ...
  //   arc.
  void Dump( MYON_TextLog& dump ) const;

  // Description:
  //   Checks an arc to make sure it is valid.
	// Detail:
	//	 Radius>0 and 0<AngleRadians()<=2 MYON_PI
  // Returns:
  ...
  */
  ...
  MYON_Interval DomainRadians() const;

  // Returns:
  //   The arc's domain in degrees.
  MYON_Interval DomainDegrees() const;

  // Description:
  //   Set arc's subtended angle in radians.
  // Parameters:
  //   angle_in_radians - [in] 0 <= angle_in_radians <= 2.0*MYON_PI
  //
  ...
  Parameters:
    angle_in_radians - [in] increasing interval with 
                            ...
                            Length of the interval <= 2.0*MYON_PI.
  Returns:
    true if successful. 
  */
  bool SetAngleIntervalRadians(
    MYON_Interval angle_in_radians
    );

  // Description:
  //   Set arc's domain as a subdomain of the circle.
  // Parameters:
  //   domain_radian - [in]   0 < domain_radian[1] - domain_radian[0] <= 2.0 * MYON*PI
  //
  bool Trim(
    MYON_Interval domain_radian
    );
  ...
  MYON_3dPoint StartPoint() const;


  // Returns:
  //   Point at middle of the arc.
  MYON_3dPoint MidPoint() const;

  // Returns:
  //   Point at end of the arc.
  MYON_3dPoint EndPoint() const;
  ...
  */
  ...
  Remarks:
    ...
    start.
  */
  MYON_3dPoint SectorAreaCentroid() const;
  ...
  */
  ...
  Remarks:
    ...
    the line segment from the arc's end to the arc's start.
  */
  MYON_3dPoint SegmentAreaCentroid() const;
  ...
  Example:
          ...
          MYON_Arc arc = ...;
          double nurbs_t = 1.2345; // some number in interval (0,2.0*MYON_PI).
          double arc_t;
  ...
  Remarks:
    ...
    created by MYON_Arc::GetNurbForm.  At nurbs parameter values of 
    0.0, 0.5*MYON_PI, MYON_PI, 1.5*MYON_PI, and 2.0*MYON_PI, the nurbs
    parameter and radian parameter are the same.  At all other
    values the nurbs and radian parameter values are different.
  See Also:
    MYON_Arc::GetNurbFormParameterFromRadian
  */
  ...
  Parameters:
    arc_radians_parameter - [in] 0.0 to 2.0*MYON_PI
    nurbs_parameter - [out]
  Example:

          MYON_Arc arc = ...;
          double arc_t = 1.2345; // some number in interval (0,2.0*MYON_PI).
          double nurbs_t;
  ...
  Remarks:
    The NURBS curve parameter is with respect to the NURBS curve
    created by MYON_Arc::GetNurbForm.  At radian values of 
    0.0, 0.5*MYON_PI, MYON_PI, 1.5*MYON_PI, and 2.0*MYON_PI, the nurbs
    parameter and radian parameter are the same.  At all other
    values the nurbs and radian parameter values are different.
  See Also:
...
private:
  friend bool MYON_BinaryArchive::ReadArc( MYON_Arc& );
  friend bool MYON_BinaryArchive::WriteArc( const MYON_Arc& );

  // increasing interval with start and end angle in radians
  MYON_Interval m_angle = MYON_Interval::ZeroToTwoPi;
};
