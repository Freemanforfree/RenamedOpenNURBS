...
bool MYON_IntersectLineLine(
          const MYON_Line& lineA, 
          const MYON_Line& lineB, 
          double* a,
          double* b,
          double tolerance,
...
{
  bool rc = MYON_Intersect(lineA,lineB,a,b) ? true : false;
  if (rc)
  {
...
bool MYON_Intersect( const MYON_BoundingBox& bbox, 
                   const MYON_Line& line, 
                   double tol,
                   MYON_Interval* line_parameters)
{
  ...
  MYON_3dVector v = line.Direction();
  const int i = v.MaximumCoordinateIndex();

  // gaurd against MYON_UNSET_VALUE as input
  if ( !(tol >= 0.0) )
    tol = 0.0;
  ...
  MYON_Line chord(line.PointAt(t0),line.PointAt(t1));
...
bool MYON_Intersect( const MYON_Line& lineA, const MYON_Line& lineB, 
                ...
                )
{
  ...
  MYON_3dVector A = lineA.Direction();
  MYON_3dVector B = lineB.Direction();
  MYON_3dVector C = lineB[0] - lineA[0];
  
  MYON_Matrix M(2,2);
  M[0][0] =  MYON_DotProduct( A, A );
  M[1][1] =  MYON_DotProduct( B, B );
  M[0][1] = M[1][0] = -MYON_DotProduct( A, B );
  ...
  // It does not make much sense to use a tolerance lower than MYON_EPSILON on the
  // result of arithmetic on doubles. If M[1][1] is smaller than 1, we need to
  // bound M_zero_tol to MYON_EPSILON or MYON_Intersect will wrongly return true on
  ...
  if (fabs(M[1][1]) > 1.) {
    pr_tolerance = fabs(M[1][1]) * MYON_SQRT_EPSILON;
    M_zero_tol = fabs(M[1][1]) * MYON_EPSILON;
  } else {
    pr_tolerance = MYON_SQRT_EPSILON;
    M_zero_tol = MYON_EPSILON;
  }

  Y[0] =  MYON_DotProduct( A, C );
  Y[1] = -MYON_DotProduct( B, C );
    ...
    else
    {
      ...
      if ( rc ) 
      {
        ...
        if ( fabs(pivot) <= pr_tolerance ) 
        {
          ...
          MYON_3dPoint pA = lineA.PointAt(X[i]);
          MYON_3dPoint pB = lineB.PointAt(X[1-i]);
          double d = pA.DistanceTo(pB);
          if ( d > pr_tolerance && d > MYON_ZERO_TOLERANCE ) { 
            MYON_3dPoint qA = lineA.ClosestPointTo(pB);
            MYON_3dPoint qB = lineB.ClosestPointTo(pA);
  ...
  )
{
  ...
  else
  {
    d = 1.0/d;
    fd = fabs(d);
    if ( fd > 1.0 && (fabs(a) >= MYON_DBL_MAX/fd || fabs(b) >= MYON_DBL_MAX/fd ) )
    {
...
{
  return MYON_Intersect( line, plane.plane_equation, line_parameter );
}



bool MYON_Intersect( const MYON_Plane& R, const MYON_Plane& S,
                   MYON_Line& L )
{
  MYON_3dVector d = MYON_CrossProduct( S.zaxis, R.zaxis );
  MYON_3dPoint p = 0.5*(R.origin + S.origin);
  MYON_Plane T( p, d );
  bool rc = MYON_Intersect( R, S, T, L.from );
  L.to = L.from + d;
  return rc;
}


bool MYON_Intersect( const MYON_Plane& R, const MYON_Plane& S, const MYON_Plane& T,
                MYON_3dPoint& P )
{
  double pr = 0.0;
  const int rank = MYON_Solve3x3( 
        ...
        )
{
  ...
  double tol = sphere_radius*MYON_SQRT_EPSILON;
  if ( !(tol >= MYON_ZERO_TOLERANCE) )
    tol = MYON_ZERO_TOLERANCE;
  const MYON_3dPoint sphere_center = sphere.Center();
  MYON_3dPoint circle_center = plane.ClosestPointTo(sphere_center);
  double d = circle_center.DistanceTo(sphere_center);

  circle.radius = 0.0;

  if ( MYON_IsValid(sphere_radius) && MYON_IsValid(d) && d <= sphere_radius + tol )
  {
    if ( sphere_radius > 0.0 )
    {
      d /= sphere_radius;
      d = 1.0 - d*d;
      // The d > 4.0*MYON_EPSILON was picked by testing spheres with
      // radius = 1 and center = (0,0,0).  Do not make 4.0*MYON_EPSILON 
      // any smaller and please discuss changes with Dale Lear.
      circle.radius = (d > 4.0*MYON_EPSILON) ? sphere_radius*sqrt(d) : 0.0;
    }
    else
      circle.radius = 0.0;

    if ( circle.radius <= MYON_ZERO_TOLERANCE )
    {
      ...
      MYON_3dVector R = circle_center - sphere_center;
      double r0 = R.Length();
      if ( r0 > 0.0 )
      {
        R.Unitize();
        MYON_3dPoint C1 = sphere_center + sphere_radius*R;
...
int MYON_Intersect(
        ...
        const MYON_Sphere& sphere,
        MYON_3dPoint& A,
        MYON_3dPoint& B // intersection point(s) returned here
        )
{
  int rc = 0;
  const MYON_3dPoint sphere_center = sphere.plane.origin;
  const double sphere_radius = fabs(sphere.radius);
  double tol = sphere_radius*MYON_SQRT_EPSILON;
  if ( tol < MYON_ZERO_TOLERANCE )
    tol = MYON_ZERO_TOLERANCE;
  MYON_3dPoint line_center = line.ClosestPointTo(sphere_center);
...
int Intersect2dLineCircle(
        MYON_2dPoint line_from, // 2d line from point
        MYON_2dPoint line_to,
        double r,
        double tol,
        double* t0,
        double* t1
...
int MYON_Intersect(
        const MYON_Line& line, 
        const MYON_Cylinder& cylinder, // if cylinder.height[0]==cylinder.height[1],
                                     // then infinite cyl is used.  Otherwise
                                     // finite cyl is used.
        MYON_3dPoint& A, MYON_3dPoint& B // intersection point(s) returned here
        )
{
  bool bFiniteCyl = true;
  int rc = 0;
  const double cylinder_radius = fabs(cylinder.circle.radius);
  double tol = cylinder_radius*MYON_SQRT_EPSILON;
  if ( tol < MYON_ZERO_TOLERANCE )
    tol = MYON_ZERO_TOLERANCE;
  ...
  if ( !MYON_Intersect(line,axis,&line_t,&axis_t) ) {
    axis.ClosestPointTo( cylinder.circle.plane.origin, &axis_t );
    line.ClosestPointTo( cylinder.circle.plane.origin, &line_t );
  }
  MYON_3dPoint line_point = line.PointAt(line_t);
  MYON_3dPoint axis_point = axis.PointAt(axis_t);
  ...
  if ( d >= cylinder_radius-tol) {
    ...
    A = line_point;
    MYON_3dVector V = line_point - axis_point;
    if ( bFiniteCyl ) {
    ...
    if ( rc == 1 ) {
      // check for overlap
      MYON_3dPoint P = axis.ClosestPointTo(line.from);
    ...
    MYON_Xform xform;
    xform.Rotation( cylinder.circle.plane, MYON_xy_plane );
    MYON_Line L = line;
    L.Transform(xform);
    ...
    double t0, t1;
    int qerc = MYON_SolveQuadraticEquation(ax+ay, bx+by, cx+cy-cylinder_radius*cylinder_radius,
                                         &t0,&t1);
    if ( qerc == 2 ) {
      ...
      )
{
  ...
  // is x^2 + y^2 = R^2 and solve for line parameter(s).
  MYON_Xform xform;
  xform.ChangeBasis( circle.plane, MYON_xy_plane );
  xform.ChangeBasis( MYON_xy_plane, circle.plane );
  MYON_Line L = line;
  L.Transform(xform);
  double r = fabs(circle.radius);
  double tol = r*MYON_SQRT_EPSILON;
  if ( tol < MYON_ZERO_TOLERANCE )
    tol = MYON_ZERO_TOLERANCE;
  int xcnt;
  ...
  {
    xcnt = Intersect2dLineCircle( MYON_2dPoint(L.from), MYON_2dPoint(L.to), r, tol, line_t0, line_t1 );
    if ( xcnt == 3 )
      xcnt = 1;
  }
  ...
  MYON_3dPoint line_point1, line_point0 = line.PointAt(*line_t0);
                  ...
                  )
{
  ...
  MYON_Line xline;
	double a,b;
  bool rc = MYON_Intersect(plane, circle.Plane(), xline);
	if(rc)
  {
    rval = MYON_Intersect(xline, circle, &a, point0, &b, point1);
  }
  else
  {
                  ...
                  )
{
  ...
  MYON_Line xline;
	double a,b;
  bool rc = MYON_Intersect(plane, arc.Plane(), xline);
	if(rc)
  {
    rval = MYON_Intersect(xline, arc, &a, point0, &b, point1);
  }
  else
  {
...
int MYON_Intersect(
  ...
  MYON_3dPoint& P1)
{
  P0 = P1 = MYON_3dPoint::UnsetPoint;
  int xcnt = -1;

  const double costol = MYON_ZERO_TOLERANCE;
  double scale0 = C0.MaximumCoordinate();
  double abstol = C1.MaximumCoordinate();
  if (abstol < scale0)
    abstol = scale0;
  abstol *= MYON_RELATIVE_TOLERANCE;
  if (abstol < MYON_ZERO_TOLERANCE)
    abstol = MYON_ZERO_TOLERANCE;
  ...
  if (coplanar)
  {
    ...
    MYON_3dVector D = C[1]->Center() - C[0]->Center();
    double d = D.Length();

    if (d > abstol)
    {
      D.Unitize();
      MYON_3dVector Dperp = MYON_CrossProduct(D, C[0]->Normal());
  ...
  {
    MYON_Line  PxP;
    if (MYON_Intersect(C0.plane, C1.plane, PxP))
    {
      MYON_3dPoint CxL[2][2]; 

      double t0, t1;
      int x0 = MYON_Intersect( PxP, C0,  &t0, CxL[0][0], &t1, CxL[0][1] );
      int x1 = MYON_Intersect( PxP, C1,  &t0, CxL[1][0], &t1, CxL[1][1] );
      xcnt = 0;
      for (int i = 0; i < x0; i++)
      {
        int j;
        for (j = 0; j < x1; j++)
        {
          if(MYON_PointsAreCoincident(3,false,CxL[0][i], CxL[1][j]))
            break;
        }
        if (j < x1)
        {
  ...
  MYON_3dPoint& P1)
{
  ...
  MYON_3dPoint CCX[2];
  int cxcnt = MYON_Intersect(static_cast<const MYON_Circle&>(A0), static_cast<const MYON_Circle&>(A1), CCX[0], CCX[1]);
  if ( cxcnt < 3)
  {
  ...
  else if (cxcnt == 3)
  {
    ...
    // order arcs by size
    const  MYON_Arc* Size[] = { &A0, &A1 };     //Size[0]<=Size[1]
    ...
    MYON_Interval BigInterior = Size[1]->Domain();    // interior domain of big arc
    if (!BigInterior.Expand(-abstol / Size[1]->Radius()))         // circles are not degenerate
      BigInterior = MYON_Interval::Singleton(Size[1]->Domain().Mid());
    
    for (int ei = 0; ei < 2; ei++)
    {
      double t;
      MYON_3dPoint LittleEnd = ei ? Size[0]->EndPoint() : Size[0]->StartPoint();
      ...
      )
{
  ...
  int i, xcnt = MYON_Intersect( line, c, &t[0], p[0], &t[1], p[1] );
  if ( xcnt > 0 )
  {
    // make sure points are on the arc;
    MYON_Interval arc_domain = arc.DomainRadians();
    for ( i = 0; i < xcnt; i++ )
    {
    ...
    if ( xcnt == 2 )
    {
      ...
      if ( xcnt == 2 && t[0] == t[1] )
      {
        xcnt = 1;
        b[1] = 0;
        MYON_3dPoint q = line.PointAt(t[0]);
...
int MYON_Intersect( const MYON_Sphere& sphere0, 
                  const MYON_Sphere& sphere1, 
                  MYON_Circle& circle
                 )

{
  ...
  MYON_3dPoint C0 = sphere0.Center();
  MYON_3dPoint C1 = sphere1.Center();
  MYON_3dVector D = C1-C0;
  double d = D.Length();
  if (!D.Unitize()){
    if (fabs(r1-r0) > MYON_ZERO_TOLERANCE)
  ...
  if (d == fabs(r0-r1)){
    MYON_3dPoint P = (r0 > r1) ? C0 + r0*D : C0 - r0*D;
    circle.Create(P, 0.0);
    return 1;
  }
  ...
  MYON_3dPoint P = C0 + x*D;
  MYON_Plane plane(P, D);
