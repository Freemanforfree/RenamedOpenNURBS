...
MYON_DecodeBase64::MYON_DecodeBase64()
{
  Begin();
}
...
void MYON_DecodeBase64::SetError()
{
  // unrecoverable error
  MYON_ERROR("MYON_DecodeBase64::Decode - error");
  m_status = 1;
}

const bool MYON_DecodeBase64::Error() const
{
  return (1 == m_status);
}

void MYON_DecodeBase64::DecodeHelper1()
{
...
void MYON_DecodeBase64::DecodeHelper2()
{
...
const char* MYON_DecodeBase64::Decode(const char* base64str)
{
  union
  {
    MYON__INT32 i;
...
const char* MYON_DecodeBase64::Decode(const char* base64str, size_t base64str_count)
{
  char* sEnd;
  const char* p;
  char s[1025];
...
const wchar_t* MYON_DecodeBase64::Decode(const wchar_t* base64str)
{
  const wchar_t* p;
  wchar_t w;
  if ( 0 == base64str )
...
const wchar_t* MYON_DecodeBase64::Decode(const wchar_t* base64str, size_t base64str_count)
{
  char* sEnd;
  const char* p;
  char s[1025];
...
MYON_EncodeBase64::MYON_EncodeBase64()
{
  Begin();
}
...
void MYON_EncodeBase64::EncodeHelper1(const unsigned char* inbuf, char* outbuf )
{
...
void MYON_EncodeBase64::EncodeHelper2(const unsigned char* inbuf, char* outbuf )
{
...
void MYON_EncodeBase64::EncodeHelper3(const unsigned char* inbuf, char* outbuf )
{
...
void MYON_EncodeBase64::EncodeHelper57(const unsigned char* inbuf )
{
...
void MYON_EncodeBase64::Encode( const void* buffer, size_t sizeof_buffer )
{
...
MYON_Base64EncodeStream::MYON_Base64EncodeStream()
...
void MYON_Base64EncodeStream::ErrorHandler()
{
  // place holder for error handing
  MYON_ERROR("MYON_UncompressStream error");
}

class MYON_Base64EncodeImplementation
{
public:
...
bool MYON_Base64EncodeStream::Begin()
{
  if ( 0 != m_implementation )
  {
    onfree(m_implementation);
  ...
  MYON_Base64EncodeImplementation* imp = (MYON_Base64EncodeImplementation*)onmalloc(sizeof(*imp));
...
bool MYON_Base64EncodeStream::In(
    MYON__UINT64 in_buffer_size,
    ...
    )
{
  ...
  MYON_Base64EncodeImplementation* imp = (MYON_Base64EncodeImplementation*)m_implementation;
  if ( imp->m_in_buffer_size > 0 )
  {
    ...
    if ( in_buffer_size < sz )
    {
      ...
      sz = (MYON__UINT32)in_buffer_size; // cast is safe because in_buffer_size < sizeof(imp->m_in_buffer_size) <= 8192
    ...
    crc1 = MYON_CRC32(m_out_crc,76,imp->m_out_buffer);
    ...
    m_in_crc = MYON_CRC32(m_in_crc,57,imp->m_in_buffer);
    m_in_size += 57;
    m_out_crc = crc1;
    m_out_size += 76;
  }
  ...
  while(in_buffer_size >= 57 )
  {
    ...
    m_in_crc = MYON_CRC32(m_in_crc,57,in_buffer);
    m_in_size += 57;
    m_out_crc = crc1;
    m_out_size += 76;
  ...
  imp->m_in_buffer_size = (MYON__UINT32)in_buffer_size; // safe cast - sizeof_buffer < 57

  return true;
}

bool MYON_Base64EncodeStream::End()
{
  if ( 0 == m_implementation )
  {
    ErrorHandler();
  ...
  if ( imp->m_in_buffer_size > 0 )
  {
    ...
    MYON__UINT32 in_buffer_size = imp->m_in_buffer_size;
    MYON__UINT32 out_buffer_size = 0;
    ...
    MYON__UINT32 crc1 = MYON_CRC32(m_out_crc,out_buffer_size,imp->m_out_buffer);
    rc = ( 0 != m_out_callback_function )
    ...
    if ( rc )
    {
      m_in_crc = MYON_CRC32(m_in_crc,imp->m_in_buffer_size,imp->m_in_buffer);
...
bool MYON_Base64EncodeStream::Out( void*, MYON__UINT32, const char* )
{
  // default base 64 encoded stream handler does nothing.
  return true;
}

bool MYON_Base64EncodeStream::SetCallback( 
...
MYON_StreamCallbackFunction MYON_Base64EncodeStream::CallbackFunction() const
{
  return m_out_callback_function;
}

void* MYON_Base64EncodeStream::CallbackContext() const
{
  return m_out_callback_context;
}

MYON__UINT64 MYON_Base64EncodeStream::InSize() const
{
  return m_in_size;
}

MYON__UINT64 MYON_Base64EncodeStream::OutSize() const
{
  return m_out_size;
}

MYON__UINT32 MYON_Base64EncodeStream::InCRC() const
{
  return m_in_crc;
}

MYON__UINT32 MYON_Base64EncodeStream::OutCRC() const
{
  return m_out_crc;
}
...
static bool ReadEncodedByte(int& pos, const wchar_t* wsz, MYON__UINT8& byteOut)
{
  wchar_t w = wsz[pos];
  if (w >= 128)
    w = 128; // Will fail validation.

  byteOut = MYON__UINT8(w);
...
size_t MYON_Base64::Decode(const wchar_t* wszBase64in, void* pvBufferOut, size_t maxLength) // Static.
{
  auto* pBufferOut = static_cast<char*>(pvBufferOut);

  MYON__UINT32 uBytesWritten = 0;

  int iResultSize = 3;
  MYON__UINT8 a = 0, b = 0, c = 0, d = 0, aResult[3] = { 0 };
  ...
  while (ReadEncodedByte(pos, wszBase64in, a))
  {
    ...
    {
      const auto val = (MYON__UINT32)((DecodeTab[a] << 18) | (DecodeTab[b] << 12) | (DecodeTab[c] << 6) | DecodeTab[d]);

      aResult[0] = (MYON__UINT8)((val & 0xFF0000) >> 16);
      aResult[1] = (MYON__UINT8)((val & 0x00FF00) >> 8);
      aResult[2] = (MYON__UINT8)((val));
...
bool MYON_Base64::Encode(const void* pvBufferIn, size_t bufNumBytes, MYON_wString& sBase64Out, bool bAppend) // Static.
{
  ...
  while (p < pEnd)
  {
    ...
    a = MYON__UINT8((a & 0x03) << 4);
    if (p < pEnd)
    {
      b = *p++;
      out[1] = EncodeTab[a | (b >> 4)];

      b = MYON__UINT8((b & 0x0F) << 2);
      if (p < pEnd)
      {
        c = *p++;
