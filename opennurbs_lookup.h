...
/*
Description:
  MYON_SerialNumberMap provides a way to map set of unique 
  ...
  inactive id.
*/
class MYON_CLASS MYON_SerialNumberMap
{
public:
  MYON_SerialNumberMap();
  ~MYON_SerialNumberMap();

  struct MAP_VALUE
  {
    MYON__UINT32 m_u_type;
    MYON__UINT32 m_u32;
    union
    {
      MYON__UINT64 u64;
  ...
  struct SN_ELEMENT
  {
    ...
    MYON__UINT8 m_sn_active; // 1 = serial number is active
    MYON__UINT8 m_id_active; // 1 = id is active
    MYON__UINT8 m_reserved1;
    MYON__UINT8 m_reserved2;

    MYON__UINT32 m_id_crc32; // id hash = IdCRC(id)
    ...
    // User information:
    //
    //   MYON_SerialNumberMap does not use the m_value field.
  ...
  */
  MYON__UINT64 ActiveSerialNumberCount() const;
  ...
  */
  MYON__UINT64 ActiveIdCount() const;
  ...
  */
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* FindSerialNumber(MYON__UINT64 sn) const;
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* FindId(MYON_UUID) const;
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* AddSerialNumber(MYON__UINT64 sn);
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* AddSerialNumberAndId(MYON__UINT64 sn, MYON_UUID id);
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* RemoveSerialNumberAndId(MYON__UINT64 sn);
  ...
  Restrictions:
    ...
    crashes.
  */
  struct SN_ELEMENT* RemoveId(MYON__UINT64 sn, MYON_UUID id);
...
private:
  ...
  // no implementation
  MYON_SerialNumberMap(const MYON_SerialNumberMap&) = delete;
  MYON_SerialNumberMap& operator=(const MYON_SerialNumberMap&) = delete;

private:
  MYON__UINT64 m_maxsn = 0; // largest sn stored anywhere


  // Serial Number list counts
  MYON__UINT64 m_sn_count = 0;   // total number of elements                       
  MYON__UINT64 m_sn_purged = 0;  // total number of purged elements
  ...
  MYON__UINT64 m_snblk_list_capacity = 0; // capacity of m_blk_list[]
  MYON__UINT64 m_snblk_list_count = 0;    // used elements in m_snblk_list[]
  class MYON_SN_BLOCK** m_snblk_list = nullptr;

  // If FindElementHelper() returns a non-null pointer
  // to an element, then m_e_blk points to the MYON_SN_BLOCK
  ...
  class MYON_SN_BLOCK* m_e_blk = nullptr;

private:
  class MYON_SN_BLOCK& m_sn_block0;

private:
  struct SN_ELEMENT* FindElementHelper(MYON__UINT64 sn);
  void UpdateMaxSNHelper();
  void GarbageCollectHelper();
  MYON__UINT64 GarbageCollectMoveHelper(MYON_SN_BLOCK* dst,MYON_SN_BLOCK* src);
  ...
  mutable MYON__UINT8 m_bHashTableIsValid = 0;

  mutable MYON__UINT32 m_hash_block_count = 0; // number of blocks in m_hash_tableX[] 
  mutable MYON__UINT64 m_hash_capacity = 0; // == m_hash_block_count*ID_HASH_BLOCK_CAPACITY
  ...
  MYON__UINT64 m_active_id_count = 0; // number of active ids in the hash table
  MYON_UUID m_inactive_id = MYON_nil_uuid; // frequently an id is removed and
  ...
  */
  MYON__UINT64 Internal_HashTableAddSerialNumberBlock(
    class MYON_SN_BLOCK* blk
    ) const;
  ...
  MYON__UINT32 Internal_HashTableBlockIndex(
    MYON__UINT32 id_crc32
    ) const;

  static MYON__UINT32 Internal_HashTableBlockRowIndex(
    MYON__UINT32 id_crc32
    );
