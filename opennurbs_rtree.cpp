...
#define MYON_RTree_NODE_DIM 3

/// A link list of nodes for reinsertion after a delete operation
struct MYON_RTreeListNode
{
  struct MYON_RTreeListNode* m_next;      ///< Next in list
  struct MYON_RTreeNode* m_node;  ///< MYON_RTreeNode
};

/// Variables for finding a split partition
struct MYON_RTreePartitionVars
{
  int m_partition[MYON_RTree_MAX_NODE_COUNT+1];
  int m_total;
  int m_minFill;
  int m_taken[MYON_RTree_MAX_NODE_COUNT+1];
  int m_count[2];
  MYON_RTreeBBox m_cover[2];
  double m_area[2];

  MYON_RTreeBranch m_branchBuf[MYON_RTree_MAX_NODE_COUNT+1];
  int m_branchCount;
  MYON_RTreeBBox m_coverSplit;
  double m_coverSplitArea;
}; 

typedef bool (*MYON_RTreeSearchCallback)(void* a_context, MYON__INT_PTR a_id );

struct MYON_RTreeSearchResultCallback
{
  void* m_context;
  MYON_RTreeSearchCallback m_resultCallback;
};

static void ChoosePartition(MYON_RTreePartitionVars* a_parVars, int a_minFill);
static void CountRec(MYON_RTreeNode* a_node, int& a_count);
static void PickSeeds(MYON_RTreePartitionVars* a_parVars);
static void InitParVars(MYON_RTreePartitionVars* a_parVars, int a_maxRects, int a_minFill);
static void GetBranches(MYON_RTreeNode* a_node, MYON_RTreeBranch* a_branch, MYON_RTreePartitionVars* a_parVars);
static int PickBranch(MYON_RTreeBBox* a_rect, MYON_RTreeNode* a_node);
static void DisconnectBranch(MYON_RTreeNode* a_node, int a_index);
static MYON_RTreeBBox NodeCover(MYON_RTreeNode* a_node);
static void InitRect(MYON_RTreeBBox* a_rect);
static MYON_RTreeBBox CombineRectHelper(const MYON_RTreeBBox* a_rectA, const MYON_RTreeBBox* a_rectB);
static double CalcRectVolumeHelper(const MYON_RTreeBBox* a_rect);
static bool OverlapHelper(const MYON_RTreeBBox* a_rectA, const MYON_RTreeBBox* a_rectB);
static bool OverlapLineHelper(const MYON_Line* a_line, const MYON_RTreeBBox* a_rectB);
static bool OverlapInfiniteLineHelper(const MYON_Line* a_line, const MYON_RTreeBBox* a_rectB);
static double DistanceToCapsuleAxisHelper(const struct MYON_RTreeCapsule* a_capsule, const MYON_RTreeBBox* a_rect);
static void ClassifyHelper(int a_index, int a_group, struct MYON_RTreePartitionVars* a_parVars);
static bool SearchHelper(const MYON_RTreeNode* a_node, MYON_RTreeBBox* a_rect, MYON_RTreeSearchResultCallback& a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_RTreeSearchResult& a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_SimpleArray<MYON_RTreeLeaf> &a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_SimpleArray<int> &a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_SimpleArray<void*> &a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, struct MYON_RTreeSphere* a_sphere, MYON_RTreeSearchResultCallback& a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, struct MYON_RTreeCapsule* a_capsule, MYON_RTreeSearchResultCallback& a_result );
static bool SearchHelper(const MYON_RTreeNode* a_node, const class MYON_Line* a_line, MYON_RTreeSearchResultCallback& a_result);
static bool SearchInfiniteLineHelper(const MYON_RTreeNode* a_node, const class MYON_Line* a_line, MYON_RTreeSearchResultCallback& a_result);
...
{
  // Room to reserve for struct MYON_RTreeMemPool::Blk.
  // This number should be >= sizeof(struct MYON_RTreeMemPool::Blk)
...
static size_t MemPoolBlkSize( size_t leaf_count )
{
  ...
  // struct MYON_RTreeMemPool::Blk header used to keep track
  // of our allocations.
  ...
  size_t pagesize = MYON_MemoryPageSize();
  ...
  if ( leaf_count > 0 )
  {
    ...
    // just fine.
    if ( 5*leaf_count < 4*MYON_RTree_MAX_NODE_COUNT )
      node_count = 3;
    else if ( 5*leaf_count < 4*MYON_RTree_MAX_NODE_COUNT*MYON_RTree_MAX_NODE_COUNT )
      node_count = MYON_RTree_MAX_NODE_COUNT+1;
  }
  
  // Set "sz" to an multiple of pagesize that is big enough
  // to hold node_count nodes.
  const size_t sizeof_node = sizeof(MYON_RTreeNode);
  size_t sz = pagesize;
...
MYON_RTreeMemPool::MYON_RTreeMemPool( size_t leaf_count  )
: m_nodes(0)
, m_list_nodes(0)
, m_buffer(0)
, m_buffer_capacity(0)
...
MYON_RTreeMemPool::~MYON_RTreeMemPool()
{
  DeallocateAll();
}

void MYON_RTreeMemPool::GrowBuffer()
{
  if ( (0 == m_sizeof_blk) || (0 != m_blk_list && 0 == m_blk_list->m_next) )
  {
    // Setting m_sizeof_blk happens twice per MYON_RTreeMemPool.
    // The first time is typically at construction and not here.
    // The (0 == m_sizeof_blk) check is here to support cases 
    // where the MYON_RTreeMemPool class is initialized by a "memset(...,0)
  ...
  else
  {
    m_buffer = 0;
    m_buffer_capacity = 0;
    MYON_ERROR("MYON_RTreeMemPool::GrowBuffer - out of memory");
  }
}

MYON_RTreeNode* MYON_RTreeMemPool::AllocNode()
{
  MYON_RTreeNode* node = (MYON_RTreeNode*)m_nodes;
  if ( node )
  {
  ...
  else
  {
    ...
    if ( 0 == (node = (MYON_RTreeNode*)m_buffer) )
    {
      MYON_ERROR("MYON_RTreeMemPool::AllocNode() - out of memory");
      return 0;
    }
...
void MYON_RTreeMemPool::FreeNode(MYON_RTreeNode* node)
{
  if ( node )
  {
...
struct MYON_RTreeListNode* MYON_RTreeMemPool::AllocListNode()
{
  struct MYON_RTreeListNode* list_node = (struct MYON_RTreeListNode*)m_list_nodes;
  if ( list_node )
  {
  ...
  else
  {
    ...
    list_node = (struct MYON_RTreeListNode*)m_buffer;
    if ( list_node )
    {
...
void MYON_RTreeMemPool::FreeListNode(struct MYON_RTreeListNode* list_node)
{
  if ( list_node )
  {
...
size_t MYON_RTreeMemPool::SizeOf() const
{
  return m_sizeof_heap;
}

size_t MYON_RTreeMemPool::SizeOfUnusedBuffer() const
{
  const struct Blk* blk;
  size_t sz = m_buffer_capacity;
  for ( blk = m_nodes; blk; blk = blk->m_next )
  {
    sz += sizeof(struct MYON_RTreeNode);
  }
  for ( blk = m_list_nodes; blk; blk = blk->m_next )
  {
    sz += sizeof(struct MYON_RTreeListNode);
  }
  return sz;
}

void MYON_RTreeMemPool::DeallocateAll()
{
...
MYON_RTreeIterator::MYON_RTreeIterator()
{ 
  Initialize(0);
}

MYON_RTreeIterator::MYON_RTreeIterator(const class MYON_RTree& rtree)
{ 
  Initialize(rtree);
}
...
const MYON_RTreeBranch* MYON_RTreeIterator::Value() const
{
  return ( 0 != m_sp )
...
bool MYON_RTreeIterator::Initialize(const MYON_RTree& a_rtree)
{
  return Initialize(a_rtree.Root());
}

bool MYON_RTreeIterator::Initialize(const MYON_RTreeNode* a_node)
{ 
  m_sp = 0;
...
bool MYON_RTreeIterator::PushChildren(StackElement* sp, bool bFirstChild )
{ 
  StackElement* spmax = &m_stack[0] + MAX_STACK;
  const MYON_RTreeNode* node = sp->m_node;
  m_sp = 0;
  ...
  while( 0 != node && node->m_level >= 0 && node->m_count > 0 )
  {
    ...
    if( ++sp == spmax )
    {
      ...
      MYON_ERROR("MYON_RTreeIterator::PushFirstChild - stack overflow");
      return false;
    }
    sp->m_node = node;
...
MYON_RTree::MYON_RTree( size_t leaf_count )
: m_root(0)
, m_reserved(0)
...
MYON_RTree::~MYON_RTree()
{
  RemoveAll();
}

bool MYON_RTree::CreateMeshFaceTree( const MYON_Mesh* mesh )
{
  double fmin[3], fmax[3];
  MYON_3dPoint V;
  unsigned int fi, fcount;
  const int* fvi;
  const MYON_MeshFace* meshF;
  const MYON_3fPoint* meshfV;
  const MYON_3dPoint* meshdV;
...
bool MYON_SubDRTree::CreateSubDEmptyRTree(
...
)
{
  ...
  m_subd.ShareContentsFrom(const_cast<MYON_SubD&>(subd));
  this->RemoveAll();
  return true;
}

bool MYON_SubDRTree::CreateSubDVertexRTree(
  const MYON_SubD& subd,
...
)
{
  ...
  {
    const MYON_3dPoint P = v->Point(vertex_location);
...
bool MYON_SubDRTree::AddVertex(
  const MYON_SubDVertex* v,
  MYON_SubDComponentLocation vertex_location
)
{
  const MYON_3dPoint P = (nullptr != v) ? v->Point(vertex_location) : MYON_3dPoint::NanPoint;
  return P.IsValid() ? this->Insert(&P.x, &P.x, (void*)v) : false;
}



const MYON_SubDVertex* MYON_SubDRTree::FindVertexAtPoint(
  const MYON_3dPoint P,
  const double distance_tolerance
) const
{
  MYON_SubDRTreeVertexFinder vf;
  vf = MYON_SubDRTreeVertexFinder::Create(P);

  MYON_BoundingBox rbox;
  const MYON_3dVector rtol(distance_tolerance, distance_tolerance, distance_tolerance);

  rbox.m_min = vf.m_P - rtol;
  rbox.m_max = vf.m_P + rtol;
  // vtree.Search() can return true (found nearby) and false (found exact) because 
  // MYON_SubDRTreeVertexFinder::Callback() cancels the search when a vertex at the exact location
  // is found.
  this->Search(&rbox.m_min.x, &rbox.m_max.x, MYON_SubDRTreeVertexFinder::Callback, &vf);
  return vf.m_v;
}


const MYON_SubDVertex* MYON_SubDRTree::FindMarkedVertexAtPoint(
  const MYON_3dPoint P,
  const double distance_tolerance
) const
{
  MYON_SubDRTreeVertexFinder vf;
  vf = MYON_SubDRTreeVertexFinder::Create(P, false);
...
const MYON_SubDVertex* MYON_SubDRTree::FindUnmarkedVertexAtPoint(
  const MYON_3dPoint P,
  const double distance_tolerance
) const
{
  MYON_SubDRTreeVertexFinder vf;
  vf = MYON_SubDRTreeVertexFinder::Create(P, true);
...
const MYON_SubDVertex* MYON_SubDRTree::FindVertex(
  const class MYON_SubDRTreeVertexFinder& vertex_finder,
  const double distance_tolerance
) const
{
  MYON_SubDRTreeVertexFinder vf(vertex_finder);
  if (false == vf.m_P.IsValid())
    return nullptr;

  vf.m_distance = MYON_SubDRTreeVertexFinder::Unset.m_distance;
  vf.m_v = MYON_SubDRTreeVertexFinder::Unset.m_v;
...
void MYON_SubDRTree::Clear()
{
  RemoveAll(); // clear the rtree
  m_subd = MYON_SubD::Empty; // clear an references to a subdimple.
}

const MYON_SubD& MYON_SubDRTree::SubD() const
{
  return m_subd;
}

const MYON_SubDRTreeVertexFinder MYON_SubDRTreeVertexFinder::Create(const MYON_3dPoint P)
{
  MYON_SubDRTreeVertexFinder vf;
  vf.m_P = P;
  return vf;
...
const MYON_SubDRTreeVertexFinder MYON_SubDRTreeVertexFinder::Create(const MYON_3dPoint P, bool bMarkFilter)
{
  MYON_SubDRTreeVertexFinder vf = MYON_SubDRTreeVertexFinder::Create(P);
  vf.m_bMarkFilterEnabled = true;
  vf.m_bMarkFilter = bMarkFilter;
  return vf;
}

const MYON_SubDRTreeVertexFinder MYON_SubDRTreeVertexFinder::Create(
  const MYON_3dPoint P,
...
bool MYON_SubDRTreeVertexFinder::Callback(void* a_context, MYON__INT_PTR a_id)
{
  for (;;)
  {
    MYON_SubDRTreeVertexFinder* vf = (MYON_SubDRTreeVertexFinder*)a_context;
    const MYON_SubDVertex* v = (const MYON_SubDVertex*)a_id;
    if (nullptr == v )
      break;
    ...
    if (MYON_SubDRTreeVertexFinder::MarkBitsFilter::None != vf->m_mark_bits_filter)
    {
      const MYON__UINT8 v_mark_bits = v->MarkBits();
      switch(vf->m_mark_bits_filter)
      {
      case MYON_SubDRTreeVertexFinder::MarkBitsFilter::Equal:
      ...
      case MYON_SubDRTreeVertexFinder::MarkBitsFilter::NotEqual:
        if (vf->m_mark_bits == v_mark_bits)
        {
          // v is not eligable.
          // Returning true means continue searching for other vertices
...
bool MYON_RTree::Insert2d(const double a_min[2], const double a_max[2], int a_element_id)
{
...
bool MYON_RTree::Insert(const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM], int a_element_id)
{
  bool rc;
  MYON_RTreeBBox rect;
  ...
  else
  {
    ...
    rc = false;
    MYON_ERROR("MYON_RTree::Insert - invalid a_min[] or a_max[] input.");
  }
  return rc;
}

bool MYON_RTree::Insert2d(const double a_min[2], const double a_max[2], void* a_element_id)
{
  const double min3d[3] = {a_min[0],a_min[1],0.0};
  const double max3d[3] = {a_max[0],a_max[1],0.0};
  return Insert(min3d,max3d,a_element_id);
...
bool MYON_RTree::Insert(const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM], void* a_element_id)
{
  bool rc;
  MYON_RTreeBBox rect;
  memcpy(rect.m_min,a_min,sizeof(rect.m_min));
  ...
  {  
    ...
    // The MYON__INT_PTR cast is safe because MYON__INT_PTR == sizeof(void*)
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_PUSH
// Disable warning C4311: 'type cast' : pointer truncation from 'void *' to 'MYON__INT_PTR'
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4311 )
#endif
    InsertRect(&rect, (MYON__INT_PTR)a_element_id, &m_root, 0);
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
...
bool MYON_RTree::Remove2d(const double a_min[2], const double a_max[2], int a_dataId)
{
  const double min3d[3] = {a_min[0],a_min[1],0.0};
  const double max3d[3] = {a_max[0],a_max[1],0.0};
...
bool MYON_RTree::Remove(const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM], int a_dataId)
{
  bool rc = false;
  if ( 0 != m_root )
  {
    ...
    {
      // invalid bounding box - don't let this corrupt the tree
      MYON_ERROR("MYON_RTree::Remove - invalid a_min[] or a_max[] input.");
    }
  }
  return rc;
}

bool MYON_RTree::Remove2d(const double a_min[2], const double a_max[2],  void* a_dataId)
{
  const double min3d[3] = {a_min[0],a_min[1],0.0};
  const double max3d[3] = {a_max[0],a_max[1],0.0};
  return Remove(min3d,max3d,a_dataId);
...
bool MYON_RTree::Remove(const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM], void* a_dataId)
{
  bool rc = false;
  if ( 0 != m_root )
  {
    ...
    {  
      // RemoveRect() returns 0 on success
      // The MYON__INT_PTR cast is save because MYON__INT_PTR == sizeof(void*)
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_PUSH
// Disable warning C4311: 'type cast' : pointer truncation from 'void *' to 'MYON__INT_PTR'
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4311 )
#endif
      rc = (0 ==  RemoveRect(&rect, (MYON__INT_PTR)a_dataId, &m_root));
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_POP
#endif
...
bool MYON_RTree::Search2d(
  const double a_min[2], const double a_max[2], 
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_data),
  void* a_context
  ) const
{
  if ( 0 == m_root )
  ...
  MYON_RTreeSearchResultCallback result;
  result.m_context = a_context;
  result.m_resultCallback = a_resultCallback;
  return SearchHelper(m_root, &rect, result);
}

bool MYON_RTree::Search(
  const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM], 
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_data),
  void* a_context
  ) const
{
...
bool MYON_RTree::Search(
  const MYON_Line* a_line,
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_data),
  void* a_context
) const
{
  return MYON_RTree::Search(a_line, false, a_resultCallback, a_context);
}
...
class MYON_RTreeSearchPolylineResultCallback : public MYON_RTreeSearchResultCallback
{
public:
  MYON_Workspace* m_ws;
};

static bool SearchPolylinePart(const MYON_RTreeNode* a_node, const MYON_Polyline* polyline, int from, int plcount,
  MYON_RTreeSearchPolylineResultCallback& result)
{
  if (plcount > 2)
  {
    ...
    {
      const MYON_RTreeBranch* branch = a_node->m_branch;
      if (a_node->IsInternalNode())
      {
        innercount = (plcount + 1) / 2;
        auto bb = (MYON_BoundingBox*)result.m_ws->GetMemory(sizeof(MYON_BoundingBox) * 2);
        *bb = polyline->BoundingBox(from, innercount);
        *(bb + 1) = polyline->BoundingBox(from + innercount - 1, plcount - innercount + 1);

        for (i = 0; i < count; ++i)
        {
          if (OverlapHelper((MYON_RTreeBBox*)bb, &branch[i].m_rect))
          {
          ...
          if (OverlapHelper((MYON_RTreeBBox*)bb+1, &branch[i].m_rect))
          {
      ...
      else
      {
        ...
        for (i = 0; i < count; ++i)
        {
          ...
          {
            MYON_Line* line = (MYON_Line*)(&polyline->Array()[innercount + from]);
            if (OverlapLineHelper(line, &branch[i].m_rect))
            {
              if (result.m_context) ((MYON_RTreePolylineContext*)result.m_context)->m_polyline_pointindex = innercount + from;
  ...
  {
    MYON_Line* line = (MYON_Line*)(&polyline->Array()[from]);
    if (result.m_context) ((MYON_RTreePolylineContext*)result.m_context)->m_polyline_pointindex = from;
    return SearchHelper(a_node, line, result);
  }
  else if (plcount < 2)
  {
    MYON_ERROR("Unexpected plcount");
    return true;
  }
...
bool MYON_RTree::Search(
  const MYON_Polyline* polyline,
  bool MYON_CALLBACK_CDECL resultCallback(void* a_context, MYON__INT_PTR a_id),
  MYON_RTreePolylineContext* a_context
) const
{
...
bool MYON_RTree::Search( 
  struct MYON_RTreeSphere* a_sphere,
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_id),
  void* a_context
  ) const
{
...
bool MYON_RTree::Search( 
  struct MYON_RTreeCapsule* a_capsule,
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_id),
  void* a_context
  ) const
{
...
bool MYON_RTree::Search2d(const double a_min[2], const double a_max[2],
                          MYON_SimpleArray<MYON_RTreeLeaf>& a_result 
                          ) const
{
  if ( 0 == m_root )
    return false;
...
bool MYON_RTree::Search(const double a_min[MYON_RTree_NODE_DIM], const double a_max[MYON_RTree_NODE_DIM],
                          MYON_SimpleArray<MYON_RTreeLeaf>& a_result 
                          ) const
{
  if ( 0 == m_root )
...
bool MYON_RTree::Search2d(const double a_min[2], const double a_max[2],
                          MYON_SimpleArray<void*>& a_result 
                          ) const
{
  if ( 0 == m_root )
    return false;
...
bool MYON_RTree::Search2d(const double a_min[2], const double a_max[2],
                          MYON_RTreeSearchResult& a_result ) const
{
  if ( 0 == m_root )
    return false;
...
struct MYON_RTreePairSearchResult
{
  double m_tolerance;
  MYON_SimpleArray<MYON_2dex>* m_result;
};


static bool PairSearchOverlapHelper( const MYON_RTreeBBox* a_rectA, const MYON_RTreeBBox* a_rectB, double tolerance )
{
  double dx,dy,dz,d;
  const double* mn;
  const double* mx;
...
static void PairSearchHelper( const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
  ...
  while(branchB < branchBmax)
  {
    if ( PairSearchOverlapHelper( &a_branchA->m_rect, &branchB->m_rect, a_result->m_tolerance ) )
    {
      ...
      {
        MYON_2dex& r = a_result->m_result->AppendNew();
...
static void PairSearchHelper( const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static void PairSearchHelper( const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
bool MYON_RTree::Search( 
          const MYON_RTree& a_rtreeA,
          const MYON_RTree& a_rtreeB, 
          double tolerance,
          MYON_SimpleArray<MYON_2dex>& a_result
          )
{
  ...
  if ( 0 == a_rtreeB.m_root )
    return false;
  MYON_RTreePairSearchResult r;
  r.m_tolerance = (MYON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
  r.m_result = &a_result;
  PairSearchHelper(a_rtreeA.m_root,a_rtreeB.m_root,&r);
  return true;
}

typedef void (*MYON_RTreePairSearchCallback)(void*, MYON__INT_PTR, MYON__INT_PTR);

struct MYON_RTreePairSearchCallbackResult
{
  double m_tolerance;
  void* m_context;
  MYON_RTreePairSearchCallback m_resultCallback;
};

typedef bool (*MYON_RTreePairSearchCallbackBool)(void*, MYON__INT_PTR, MYON__INT_PTR);

struct MYON_RTreePairSearchCallbackResultBool
{
  double m_tolerance;
  void* m_context;
  MYON_RTreePairSearchCallbackBool m_resultCallbackBool;
};


typedef bool(*MYON_RTreePairSearchCallbackBoolTolerance)(void*, MYON__INT_PTR, MYON__INT_PTR, double*);

struct MYON_RTreePairSearchCallbackResultBoolTolerance
{
  double m_tolerance;
  void* m_context;
  MYON_RTreePairSearchCallbackBoolTolerance m_resultCallbackBoolTolerance;
};

static void PairSearchHelper( const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static bool PairSearchHelperBool( const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBool* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static bool PairSearchHelperBoolTolerance(const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static void PairSearchHelper( const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static bool PairSearchHelperBool( const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResultBool* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static bool PairSearchHelperBoolTolerance(const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static void PairSearchHelper( const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResult* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
static bool PairSearchHelperBool( const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBool* a_result )
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
static bool PairSearchHelperBoolTolerance(const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
bool MYON_RTree::Search( 
  ...
  const MYON_RTree& a_rtreeB, 
  double tolerance,
  void MYON_CALLBACK_CDECL resultCallback(void* a_context, MYON__INT_PTR a_idA, MYON__INT_PTR a_idB),
  void* a_context
  )
{
  if ( 0 == a_rtreeA.m_root )
  ...
  if ( 0 == a_rtreeB.m_root )
    return false;
  MYON_RTreePairSearchCallbackResult r;
  r.m_tolerance = (MYON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
...
bool MYON_RTree::Search( 
  ...
  const MYON_RTree& a_rtreeB, 
  double tolerance,
  bool MYON_CALLBACK_CDECL resultCallback(void* a_context, MYON__INT_PTR a_idA, MYON__INT_PTR a_idB),
  void* a_context
  )
{
  if ( 0 == a_rtreeA.m_root )
  ...
  if ( 0 == a_rtreeB.m_root )
    return false;
  MYON_RTreePairSearchCallbackResultBool r;
  r.m_tolerance = (MYON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
  r.m_context = a_context;
...
bool MYON_RTree::Search(
  ...
  const MYON_RTree& a_rtreeB,
  double tolerance,
  bool MYON_CALLBACK_CDECL resultCallback(void* a_context, MYON__INT_PTR a_idA, MYON__INT_PTR a_idB, double* tolerance),
  void* a_context
  )
{
  if (0 == a_rtreeA.m_root)
  ...
  if (0 == a_rtreeB.m_root)
    return false;
  MYON_RTreePairSearchCallbackResultBoolTolerance r;
  r.m_tolerance = (MYON_IsValid(tolerance) && tolerance > 0.0) ? tolerance : 0.0;
  r.m_context = a_context;
...
static void SingleTreeSearchHelper(const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static void SingleTreeSearchHelper(const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static void SingleTreeSearchHelper(const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
static void SingleTreeSearchHelper(const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static bool SingleTreeSearchHelperBool(const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBool* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static bool SingleTreeSearchHelperBoolTolerance(const MYON_RTreeBranch* a_branchA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchB, *branchBmax;
...
static void SingleTreeSearchHelper(const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static bool SingleTreeSearchHelperBool(const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResultBool* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static bool SingleTreeSearchHelperBoolTolerance(const MYON_RTreeNode* a_nodeA, const MYON_RTreeBranch* a_branchB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax;
...
static void SingleTreeSearchHelper(const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResult* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
static bool SingleTreeSearchHelperBool(const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBool* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
static bool SingleTreeSearchHelperBoolTolerance(const MYON_RTreeNode* a_nodeA, const MYON_RTreeNode* a_nodeB, MYON_RTreePairSearchCallbackResultBoolTolerance* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
  const MYON_RTreeBranch *branchA, *branchAmax, *branchB, *branchBmax;
...
const MYON_RTreeNode* MYON_RTree::Root() const
{
  return m_root;
}

MYON_BoundingBox MYON_RTree::BoundingBox() const
{
...
size_t MYON_RTree::SizeOf() const
{
...
// never used
static void NodeCountHelper( const MYON_RTreeNode* node, size_t& node_count, size_t& wasted_branch_count, size_t& leaf_count )
{
  if ( 0 == node )
    return;
  node_count++;
  wasted_branch_count += (MYON_RTree_MAX_NODE_COUNT - node->m_count);
  if ( node->m_level > 0 )
  {
...
void MYON_RTree::RemoveAll()
{
  m_root = 0;
  m_mem_pool.DeallocateAll();
}

void MYON_RTree::RemoveAllRec(MYON_RTreeNode* a_node)
{
...
static void InitRect(MYON_RTreeBBox* a_rect)
{
  for(int index = 0; index < MYON_RTree_NODE_DIM; ++index)
  {
...
bool MYON_RTree::InsertRectRec(MYON_RTreeBBox* a_rect, MYON__INT_PTR a_id, MYON_RTreeNode* a_node, MYON_RTreeNode** a_newNode, int a_level)
{
  int index;
  MYON_RTreeBranch branch;
  MYON_RTreeNode* otherNode;
  ...
  else if(a_node->m_level == a_level) // Have reached level for insertion. Add rect, split if necessary
  {
    ...
    // The (MYON_RTreeNode*) cast is safe because MYON__INT_PTR == sizeof(void*)
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_PUSH
// Disable warning C4312: 'type cast' : conversion from 'MYON__INT_PTR' to 'MYON_RTreeNode *' of greater size
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4312 )
#endif
    branch.m_child = (MYON_RTreeNode*)a_id;
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_POP
#endif
  ...
  MYON_ERROR("MYON_RTree::InsertRectRec - bug in algorithm");
  return false;
}
...
bool MYON_RTree::InsertRect(MYON_RTreeBBox* a_rect, MYON__INT_PTR a_id, MYON_RTreeNode** a_root, int a_level)
{
  MYON_RTreeNode* newRoot;
  MYON_RTreeNode* newNode;
  MYON_RTreeBranch branch;
...
#else
      for( int j = 0; j < MYON_RTree_NODE_DIM; j++ )
      {
...
bool MYON_RTree::AddBranch(MYON_RTreeBranch* a_branch, MYON_RTreeNode* a_node, MYON_RTreeNode** a_newNode)
{
  if(a_node->m_count < MYON_RTree_MAX_NODE_COUNT)  // Split won't be necessary
  {
...
static int PickBranch(MYON_RTreeBBox* a_rect, MYON_RTreeNode* a_node)
{
  ...
  for(int index=0; index < a_node->m_count; ++index)
  {
    MYON_RTreeBBox* curRect = &a_node->m_branch[index].m_rect;
...
MYON_RTreeBBox CombineRectHelper(const MYON_RTreeBBox* a_rectA, const MYON_RTreeBBox* a_rectB)
{
  MYON_RTreeBBox rect = *a_rectA;
...
void MYON_RTree::SplitNode(MYON_RTreeNode* a_node, MYON_RTreeBranch* a_branch, MYON_RTreeNode** a_newNode)
{
  MYON_RTreePartitionVars localVars;
  int level;
  ...
  // Find partition
  ChoosePartition(&localVars, MYON_RTree_MIN_NODE_COUNT);
...
double CalcRectVolumeHelper(const MYON_RTreeBBox* a_rect)
{
  double d, r;
  ...
  //return (unit_sphere_volume * radius^MYON_RTree_NODE_DIM);

  // Faster rectangle volume calculation, but can cause poor merges
  d = a_rect->m_max[0] - a_rect->m_min[0];
  for(int i = 1; i < MYON_RTree_NODE_DIM; ++i)
  {
...
static void GetBranches(MYON_RTreeNode* a_node, MYON_RTreeBranch* a_branch, MYON_RTreePartitionVars* a_parVars)
{
  ...
  for(int index=0; index < MYON_RTree_MAX_NODE_COUNT; ++index)
  {
    a_parVars->m_branchBuf[index] = a_node->m_branch[index];
  }
  a_parVars->m_branchBuf[MYON_RTree_MAX_NODE_COUNT] = *a_branch;
  a_parVars->m_branchCount = MYON_RTree_MAX_NODE_COUNT + 1;

  // Calculate rect containing all in the set
  a_parVars->m_coverSplit = a_parVars->m_branchBuf[0].m_rect;
  for(int index=1; index < MYON_RTree_MAX_NODE_COUNT+1; ++index)
  {
...
static void ChoosePartition(MYON_RTreePartitionVars* a_parVars, int a_minFill)
{
       ...
       && (a_parVars->m_count[1] < (a_parVars->m_total - a_parVars->m_minFill)))
  {
    ...
    for(int index=0; index<a_parVars->m_total; ++index)
    {
      ...
      {
        MYON_RTreeBBox* curRect = &a_parVars->m_branchBuf[index].m_rect;
        MYON_RTreeBBox rect0 = CombineRectHelper(curRect, &a_parVars->m_cover[0]);
        MYON_RTreeBBox rect1 = CombineRectHelper(curRect, &a_parVars->m_cover[1]);
...
void MYON_RTree::LoadNodes(MYON_RTreeNode* a_nodeA, MYON_RTreeNode* a_nodeB, MYON_RTreePartitionVars* a_parVars)
{
  for(int index=0; index < a_parVars->m_total; ++index)
  {
...
// Initialize a MYON_RTreePartitionVars structure.
...
static void PickSeeds(MYON_RTreePartitionVars* a_parVars)
{
  int seed0 = 0, seed1 = 1;
  double worst, waste;
  double area[MYON_RTree_MAX_NODE_COUNT+1];
  ...
  for(int indexA=0; indexA < a_parVars->m_total-1; ++indexA)
  {
    ...
    {
      MYON_RTreeBBox oneRect = CombineRectHelper(&a_parVars->m_branchBuf[indexA].m_rect, &a_parVars->m_branchBuf[indexB].m_rect);
...
void ClassifyHelper(int a_index, int a_group, MYON_RTreePartitionVars* a_parVars)
{
...
// Pass in a pointer to a MYON_RTreeBBox, the tid of the record, ptr to ptr to root node.
// Returns 1 if record not found, 0 if success.
// RemoveRect provides for eliminating the root.

bool MYON_RTree::RemoveRect(MYON_RTreeBBox* a_rect, MYON__INT_PTR a_id, MYON_RTreeNode** a_root)
{
  MYON_RTreeNode* tempNode;
  MYON_RTreeListNode* reInsertList = nullptr;
  ...
  if(!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList))
  {
    ...
    while(reInsertList)
    {
      ...
      MYON_RTreeListNode* remLNode = reInsertList;
...
bool MYON_RTree::RemoveRectRec(MYON_RTreeBBox* a_rect, MYON__INT_PTR a_id, MYON_RTreeNode* a_node, MYON_RTreeListNode** a_listNode)
{
  if(a_node->IsInternalNode())  // not a leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
      if(OverlapHelper(a_rect, &(a_node->m_branch[index].m_rect)))
      {
        ...
        {
          if(a_node->m_branch[index].m_child->m_count >= MYON_RTree_MIN_NODE_COUNT)
          {
...
bool OverlapHelper(const MYON_RTreeBBox* a_rectA, const MYON_RTreeBBox* a_rectB)
{
  const double* mn;
  const double* mx;
...
bool OverlapLineHelper(const MYON_Line* line, const MYON_RTreeBBox* rect)
{
  MYON_BoundingBox* bbox = (MYON_BoundingBox*)rect;
  return !bbox->IsDisjoint(*line, false);
}

// Decide whether a box and an infinite line overlap.
bool OverlapInfiniteLineHelper(const MYON_Line* line, const MYON_RTreeBBox* rect)
{
  MYON_BoundingBox* bbox = (MYON_BoundingBox*)rect;
...
//static bool OverlapHelper(const struct MYON_RTreeSphere* a_sphere, const MYON_RTreeBBox* a_rect)
//{
//  double d[3], t, r;
//  const double* mn;
//  const double* mx;
...
static double DistanceToCapsuleAxisHelper(const struct MYON_RTreeCapsule* a_capsule, const MYON_RTreeBBox* a_rect)
{
  ...
  if ( 0.0 == a_capsule->m_domain[0] && 1.0 == a_capsule->m_domain[1] )
    return ((const MYON_BoundingBox*)a_rect->m_min)->MinimumDistanceTo( *((const MYON_Line*)a_capsule->m_point[0]) );
  ...
  return ((const MYON_BoundingBox*)a_rect->m_min)->MinimumDistanceTo( *((const MYON_Line*)L[0]) );
}
...
void MYON_RTree::ReInsert(MYON_RTreeNode* a_node, MYON_RTreeListNode** a_listNode)
{
  MYON_RTreeListNode* newListNode;
...
static
bool OverlapBoundedPlaneXYZHelper( const double* a_bounded_plane, const MYON_RTreeBBox* a_rect )
{
...
static
bool SearchBoundedPlaneXYZHelper(const MYON_RTreeNode* a_node, const double* a_bounded_plane, MYON_RTreeSearchResultCallback& a_result ) 
{
  int i, count;
...
bool MYON_RTree::Search(
  const class MYON_PlaneEquation& a_plane_eqn,
  double a_min,
  double a_max,
  bool MYON_CALLBACK_CDECL a_resultCallback(void* a_context, MYON__INT_PTR a_id),
  void* a_context
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, MYON_RTreeBBox* a_rect, MYON_RTreeSearchResultCallback& a_result ) 
{
  // NOTE: 
  //  Some versions of MYON_RTree::Search shrink a_rect as the search progresses.
  int i, count;
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, struct MYON_RTreeSphere* a_sphere, MYON_RTreeSearchResultCallback& a_result ) 
{
  // NOTE: 
  //  Some versions of MYON_RTree::Search shrink a_sphere as the search progresses.
  int i, closest_i, count;
  const double* sphere_center;
  const MYON_RTreeBranch* branch;
  double r[MYON_RTree_MAX_NODE_COUNT], sphere_radius, closest_d;
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, struct MYON_RTreeCapsule* a_capsule, MYON_RTreeSearchResultCallback& a_result ) 
{
  // NOTE: 
  //  Some versions of MYON_RTree::Search shrink a_sphere as the search progresses.
  int i, count;
...
static
bool SearchInfiniteLineHelper(const MYON_RTreeNode* a_node, const MYON_Line* a_line, MYON_RTreeSearchResultCallback& a_result)
{
  // NOTE: 
   //  Some versions of MYON_RTree::Search shrink a_line as the search progresses.
  int i, count;
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_Line* a_line, MYON_RTreeSearchResultCallback& a_result)
{
  // NOTE: 
   //  Some versions of MYON_RTree::Search shrink a_rect as the search progresses.
  int i, count;
  ...
  if ( (count = a_node->m_count) > 0 )
  {
    ...
    else
    {
      ...
      for(i=0; i < count; ++i)
      {
        if(OverlapHelper(a_rect, &branch[i].m_rect))
        {
          MYON_RTreeLeaf& leaf = a_result.AppendNew();
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_SimpleArray<void*> &a_result)
{
  int i, count;
  ...
  if ( (count = a_node->m_count) > 0 )
  {
    ...
    else
    {
      ...
      for(i=0; i < count; ++i)
      {
        if(OverlapHelper(a_rect, &branch[i].m_rect))
        {
          // The (void*) cast is safe because branch[i].m_id is an MYON__INT_PTR
#if defined(MYON_COMPILER_MSC) && 4 == MYON_SIZEOF_POINTER
#pragma MYON_PRAGMA_WARNING_PUSH
// Disable warning C4312: 'type cast' : conversion from 'const MYON__INT_PTR' to 'void *' of greater size
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4312 )
#endif
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_SimpleArray<int> &a_result)
{
  int i, count;
...
static
bool SearchHelper(const MYON_RTreeNode* a_node, const MYON_RTreeBBox* a_rect, MYON_RTreeSearchResult& a_result)
{
  int i, count;
