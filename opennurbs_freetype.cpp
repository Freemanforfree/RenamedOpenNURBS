...
*/
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4263 4264 )
#endif
...
class MYON_FontFileBuffer
{
public:
  MYON_FontFileBuffer() = default;
  ...
  MYON_FontFileBuffer(const MYON_FontFileBuffer& src)
  {
    Internal_CopyFrom(src);
  }

  MYON_FontFileBuffer& operator=(const MYON_FontFileBuffer& src)
  {
    Internal_CopyFrom(src);
    return *this;
  }
...
private:
  void Internal_CopyFrom(const MYON_FontFileBuffer& src)
  {
...
class MYON_FreeTypeFace
{
public:
  MYON_FreeTypeFace() = default;
  ~MYON_FreeTypeFace();
  FT_Face m_face = nullptr;
  MYON_FontFileBuffer m_font_buffer;
private:
  MYON_FreeTypeFace(const MYON_FreeTypeFace&) = delete;
  MYON_FreeTypeFace& operator=(const MYON_FreeTypeFace&) = delete;
};

MYON_FreeTypeFace::~MYON_FreeTypeFace()
{
...
public:
  ...
  static MYON_FreeTypeFace* CreateFace(
  ...
  */
  ...
  static const MYON_wString FaceFlagsToString(FT_Long face_flags);

  static const MYON_wString StyleFlagsToString(FT_Long style_flags);

  static const MYON_wString EncodingTypeToString(
    FT_Encoding charmap_encoding
  );

  static const MYON_wString CharmapPlatformEncodingDescription(
...
private:
  ...
  static MYON_FreeTypeFace* Internal_CreateFaceFromWindowsFont(
    const LOGFONT* logfont
  );
#endif
#if defined (MYON_RUNTIME_COCOA_AVAILABLE)
  static MYON_FreeTypeFace* Internal_CreateFaceFromAppleFont(CTFontRef);
#endif
};

FT_MemoryRec_ MYON_FreeType::m_memory_rec;
FT_Library MYON_FreeType::m_library = nullptr;

static void* MYON_Internal_FT_Alloc_Func(
  FT_Memory  memory,
  long size)
{
...
void MYON_Internal_FT_Free_Func(
  FT_Memory  memory,
  void* block
)
{
...
void* MYON_InternalFT_Realloc_Func(
  FT_Memory  memory,
  long cur_size,
  long new_size,
  void* block
...
FT_Library MYON_FreeType::Library()
{
  if (nullptr == MYON_FreeType::m_library)
  {
#if 1
    ...
    // MYON_Font::ManagedFont(). These are created as needed
    // one per face and never deleted.
    memset(&MYON_FreeType::m_memory_rec, 0, sizeof(MYON_FreeType::m_memory_rec));
    MYON_FreeType::m_memory_rec.user = nullptr;
    MYON_FreeType::m_memory_rec.alloc = MYON_Internal_FT_Alloc_Func;
    MYON_FreeType::m_memory_rec.free = MYON_Internal_FT_Free_Func;
    MYON_FreeType::m_memory_rec.realloc = MYON_InternalFT_Realloc_Func;

    FT_Library library = nullptr;
    int rc = FT_New_Library(&MYON_FreeType::m_memory_rec, &library);
    ...
    if (0 != rc || nullptr == library)
    {
      MYON_ERROR("FreeType FT_Init_FreeType() failed.");
    }
    else
    {
      MYON_FreeType::m_library = library;
    }
  }
  return MYON_FreeType::m_library;
}

bool MYON_FreeType::IsDamagedCharMap(
  FT_CharMap cmap
)
{
  ...
  case FT_ENCODING_APPLE_ROMAN:
    rc = MYON_FreeType::UseUnicodeAsAppleRomanCharCode(cmap->face);
    break;
  default:
    break;
  }
...
#define MYON__FLAG_TO_STR(flag,i,v,s) do {if ( flag == (i & flag) ) {if (s.IsNotEmpty()) s += L", "; s += v; }} while(false)

const MYON_wString MYON_FreeType::FaceFlagsToString(FT_Long face_flags)
{
  MYON_wString s;
  MYON__FLAG_TO_STR(FT_FACE_FLAG_SCALABLE, face_flags, L"SCALABLE", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_FIXED_SIZES, face_flags, L"FIXED_SIZES", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_FIXED_WIDTH, face_flags, L"FIXED_WIDTH", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_SFNT, face_flags, L"SFNT", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_HORIZONTAL, face_flags, L"HORIZONTAL", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_VERTICAL, face_flags, L"VERTICAL", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_KERNING, face_flags, L"KERNING", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_FAST_GLYPHS, face_flags, L"FAST_GLYPHS", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_MULTIPLE_MASTERS, face_flags, L"MULTIPLE_MASTERS", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_GLYPH_NAMES, face_flags, L"GLYPH_NAMES", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_EXTERNAL_STREAM, face_flags, L"EXTERNAL_STREAM", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_HINTER, face_flags, L"FLAG_HINTER", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_CID_KEYED, face_flags, L"CID_KEYED", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_TRICKY, face_flags, L"FLAG_TRICKY", s);
  MYON__FLAG_TO_STR(FT_FACE_FLAG_COLOR, face_flags, L"FLAG_COLOR", s);
  return s;
}

const MYON_wString MYON_FreeType::StyleFlagsToString(FT_Long style_flags)
{
  MYON_wString s;
  MYON__FLAG_TO_STR(FT_STYLE_FLAG_BOLD, style_flags, L"BOLD", s);
  MYON__FLAG_TO_STR(FT_STYLE_FLAG_ITALIC, style_flags, L"ITALIC", s);
  return s;
}


bool MYON_FreeType::UseUnicodeAsAppleRomanCharCode(FT_Face face)
{
  ...
  // and the funciton MYON_MapUnicodeToAppleRoman() works correctly with all the
    ...
    )
  {
    ...
    if (MYON_String::EqualOrdinal(face->family_name, "CityBlueprint", false))
      return true;
    if (MYON_String::EqualOrdinal(face->family_name, "CountryBlueprint", false))
      return true;
    if (MYON_String::EqualOrdinal(face->family_name, "Romantic", false))
      return true;
    if (MYON_String::EqualOrdinal(face->family_name, "SansSerif", false))
      return true;
    if (MYON_String::EqualOrdinal(face->family_name, "Technic", false))
      return true;
  }
  return false;
}

const MYON_wString MYON_FreeType::EncodingTypeToString( FT_Encoding charmap_encoding )
{
  MYON_wString e;
...
const MYON_wString MYON_FreeType::CharmapPlatformEncodingDescription( const FT_CharMap cmap )
{
  // Reference
  // https://www.microsoft.com/typography/otspec/name.htm#enc3

  MYON_wString platform;
  MYON_wString encoding;
  ...
  const MYON_wString e = MYON_FreeType::EncodingTypeToString(cmap->encoding);

  MYON_wString s = MYON_wString::FormatToString(
    L"%ls %d-%d",
...
bool MYON_FontGlyph::TestFreeTypeFaceCharMaps(
...
) const
{
  ...
  const MYON__UINT32 unicode_code_point = CodePoint();
  ...
  FT_Face face = reinterpret_cast<FT_Face>(MYON_Font::FreeTypeFace(font));
  if (nullptr == face)
  {
    if (text_log)
  ...
  for (int charmap_index = 0; charmap_index < face->num_charmaps; charmap_index++)
  {
    ...
    const MYON_wString e = MYON_FreeType::CharmapPlatformEncodingDescription(charmap);
    ...
    case FT_ENCODING_APPLE_ROMAN:
      bBuggyMap = MYON_FreeType::UseUnicodeAsAppleRomanCharCode(face);
      if ( bBuggyMap )
      {
      ...
      else
      {
        ...
        char_code = MYON_MapUnicodeToMSSBCP(10000, unicode_code_point);
    ...
    if (text_log)
    {
      const wchar_t* damaged = ( bBuggyMap || MYON_FreeType::IsDamagedCharMap(charmap) )? L"DAMAGED " : L"";
      MYON_wString s = MYON_wString::FormatToString(
        L"%lscmap[%d] %ls",
        damaged,
        charmap_index,
      ...
      s += MYON_wString::FormatToString(L" U+%04X", unicode_code_point);
      if (false == bHaveCharCode)
        s += L" (no char code)";
      if (false == bUnicode || bBuggyMap || char_code != unicode_code_point)
        s += MYON_wString::FormatToString(L" -> char code 0x%X (%u)", char_code, char_code);
      ...
      else
      {
        ...
        if (glyph_index != gid)
        {
          s += MYON_wString::FormatToString(L"ERROR(expected glyph index %u)",glyph_index);
        }
      }
...
unsigned int MYON_FreeType::GlyphId(
    FT_Face face,
    MYON__UINT32 unicode_code_point
  )
{
  ...
  //
  //  1-0 Apple Roman (256 codes - see MYON_MapAppleRomanToUnicode())
  ...
  for (size_t pass = 0; pass < pass_count; pass++)
  {
    ...
    for ( int charmap_index = 0; charmap_index < face->num_charmaps; charmap_index++)
    {
      ...
      case FT_ENCODING_APPLE_ROMAN:
        ...
        if (MYON_FreeType::UseUnicodeAsAppleRomanCharCode(face))
        {
          // Buggy font.
        ...
        else
        {
          ...
          charcode = MYON_MapUnicodeToMSSBCP(10000, unicode_code_point);
          if (charcode > 0xFF)
...
static bool Internal_CreateFontBufferFromDirectWrite(
  ...
  MYON_FontFileBuffer& font_buffer
)
{
  ...
  if (numfiles > 1)
  {
    ...
    // with multiple files.
    MYON_WARNING("Multiple font files.");
    numfiles = 1;
  }
...
)
{
  ...
  if (MYON_Font::WindowsConstants::logfont_symbol_charset != cleanLogFont.lfCharSet)
    cleanLogFont.lfCharSet = MYON_Font::WindowsConstants::logfont_default_charset;
  cleanLogFont.lfOutPrecision = MYON_Font::WindowsConstants::logfont_out_precis;
  cleanLogFont.lfClipPrecision = 0;
  cleanLogFont.lfQuality = MYON_Font::WindowsConstants::logfont_quality;
  cleanLogFont.lfPitchAndFamily = MYON_Font::WindowsConstants::logfont_pitch_and_family;
...
)
{
  ...
  for (;;)
  {
    ...
    if (0 == hfont)
    {
      MYON_ERROR("CreateFontIndirect failed.");
      break;
    }
    ...
    if (nullptr == font_hdc)
    {
      MYON_ERROR("MYON_Font::CreateWindowsLogfontDeviceContext()(nullptr) failed.");
      break;
    }
    ...
    {
      MYON_ERROR("SelectObject(hdc, hfont) failed.");
      break;
    }
    ...
    {
      MYON_ERROR("GetFontData(...,nullptr,0) failed.");
      break;
    }
    ...
    if (nullptr == buffer)
    {
      MYON_ERROR("onmalloc(buffer_capacity) failed.");
      break;
    }
...
MYON_FreeTypeFace* MYON_FreeType::Internal_CreateFaceFromWindowsFont(
  const LOGFONT* logfont
)
{
  if (nullptr == logfont)
  ...
  FT_Library freetype_library = MYON_FreeType::Library();
  ...
  for (int pass = 1; pass <= 2; pass++)
  {
    ...
    MYON_FreeTypeFace* f = new MYON_FreeTypeFace();
...
MYON_FreeTypeFace* MYON_FreeType::Internal_CreateFaceFromAppleFont (CTFontRef fontRef)
{
  if (nullptr == fontRef)
    return nullptr;
  ...
  FT_Face ftFace;
  FT_Error err = FT_New_Face (MYON_FreeType::Library(), path, 0, &ftFace);    // get first face
  if (err)
    return nullptr;     // that didn't work

  const int numFaces = (int)ftFace->num_faces; // determine number of faces in the font file

  MYON_FreeTypeFace* rc = new MYON_FreeTypeFace();
  if (numFaces <= 1)
  {
    rc->m_face = ftFace;
  ...
  for (;;)
  {
    const MYON_wString appleFamilyName = MYON_Font::AppleCTFontFamilyName(fontRef);
    const MYON_wString appleStyleName = MYON_Font::AppleCTFontFaceName(fontRef);
    const MYON_wString ftFamilyName(ftFace->family_name);
    const MYON_wString ftStyleName(ftFace->style_name);

    if (
        MYON_wString::EqualOrdinal(appleFamilyName, ftFamilyName, true)
        && MYON_wString::EqualOrdinal(appleStyleName, ftStyleName, true)
        )
    {
      rc->m_face = ftFace;
      return rc;
    ...
    FT_Error err = FT_New_Face (MYON_FreeType::Library(), path, ++faceIndex, &ftFace);
  ...
  FT_New_Face (MYON_FreeType::Library(), path, 0, &ftFace);    // get first face

  rc->m_face = ftFace;
  return rc;
}

#endif    // MYON_RUNTIME_APPLE


MYON_FreeTypeFace* MYON_FreeType::CreateFace(
  const MYON_Font& font
)
{
  MYON_FreeTypeFace* f = nullptr;
  ...
  f = MYON_FreeType::Internal_CreateFaceFromWindowsFont(&logfont);

#elif defined (MYON_RUNTIME_COCOA_AVAILABLE)
  bool bIsSubstituteFont = false;
  f = MYON_FreeType::Internal_CreateFaceFromAppleFont(font.AppleCTFont(bIsSubstituteFont));
#endif

  // Create empty holder so this function doesn't repeatedly
  // try to load the freetype face.
  if (nullptr == f)
    f = new MYON_FreeTypeFace();

  return f;
}

MYON__UINT_PTR MYON_Font::FreeTypeFace(
  const MYON_Font* font
)
{
  if (nullptr == font)
  ...
  {
    managed_font->m_free_type_face = MYON_FreeType::CreateFace(*managed_font);
  }
  ...
  return (MYON__UINT_PTR)ft_face;
}

void MYON_Font::DestroyFreeTypeFace(
  const MYON_Font* font
)
{
...
const MYON__UINT_PTR MYON_FontGlyph::FreeTypeFace() const
{
  return
    (nullptr == m_managed_font)
    ? 0
    : MYON_Font::FreeTypeFace(m_managed_font);
}

unsigned int MYON_FreeTypeGetFontUnitsPerM(
  const MYON_Font* font
  )
{
  for(;;)
  {
    ...
    const MYON__UINT_PTR ft_face_as_uint = MYON_Font::FreeTypeFace(font);
...
void MYON_FreeTypeGetFontMetrics(
  const MYON_Font* font,
  MYON_FontMetrics& font_unit_font_metrics
)
{
  font_unit_font_metrics = MYON_FontMetrics::Unset;
  ...
  const unsigned int glyph_id
    ...
    : MYON_FreeType::GlyphId(face, glyph->CodePoint());
  if (0 == glyph_id)
    return 0;
    */
  ...
  const MYON_FontGlyph Iglyph(font, 'I');
  MYON_TextBox Ibox = MYON_TextBox::Unset;
  const int ascent_of_I
    = (0 != MYON_FreeTypeGetGlyphMetrics(&Iglyph, Ibox) && Ibox.IsSet() && Ibox.m_bbmax.j > 0)
    ? Ibox.m_bbmax.j
    : 0;

  const MYON_FontGlyph Hglyph(font, 'H');
  MYON_TextBox Hbox = MYON_TextBox::Unset;
  const int ascent_of_H
    = (0 != MYON_FreeTypeGetGlyphMetrics(&Hglyph, Hbox) && Hbox.IsSet() && Hbox.m_bbmax.j > 0)
    ? Hbox.m_bbmax.j
    : 0;
...
class MYON_FreeTypeOutlineAccumlator : public MYON_OutlineAccumulator
{
public:
  MYON_FreeTypeOutlineAccumlator() = default;
  ~MYON_FreeTypeOutlineAccumlator() = default;
...
private:
  MYON_FreeTypeOutlineAccumlator(const MYON_FreeTypeOutlineAccumlator&) = delete;
  MYON_FreeTypeOutlineAccumlator& operator=(const MYON_FreeTypeOutlineAccumlator&) = delete;
...
private:
  ...
  MYON_OutlineFigurePoint::Type m_end_figure_point_type = MYON_OutlineFigurePoint::Type::Unset;

  FT_Vector m_start_point;
  FT_Vector m_prev_point;

  static const MYON_2fPoint Internal_To2fPoint(const FT_Vector* v) { return MYON_2fPoint((float)(v->x), (float)(v->y)); }
...
const MYON_TextBox MYON_TextBox_CreateFromFreeTypeGlyphMetrics(
...
bool MYON_FreeTypeOutlineAccumlator::AddFreeTypeFiguresToOutline(
  FT_Face ft_face,
  FT_UInt font_glyph_id,
  MYON_Outline& outline
)
{
  ...
  for (;;)
  {
    ...
    if (false == MYON_FreeTypeLoadGlyph((MYON__UINT_PTR)ft_face, font_glyph_id, false))
      break;

    rc = true;

    const MYON_TextBox glyph_metrics_in_font_design_units = MYON_TextBox_CreateFromFreeTypeGlyphMetrics(&ft_face->glyph->metrics);
    ...
    m_end_figure_point_type
      = (MYON_OutlineFigure::Type::SingleStroke == outline.FigureType())
    ...
    ft_outline_funcs.move_to = MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineMoveToFunc;
    ft_outline_funcs.line_to = MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineLineToFunc;
    ft_outline_funcs.conic_to = MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineConicToFunc;
    ft_outline_funcs.cubic_to = MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineCubicToFunc;
    ft_outline_funcs.line_to_close_contour = MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineLineToCloseContourFunc;
...
int MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineMoveToFunc(
  const FT_Vector* to,
  void* user
)
{
  MYON_FreeTypeOutlineAccumlator* a = (MYON_FreeTypeOutlineAccumlator*)user;
  if (nullptr == a)
  ...
  a->BeginFigure(MYON_OutlineFigurePoint::Type::BeginFigureUnknown, Internal_To2fPoint(to));
  a->m_start_point = *to;
  a->m_prev_point = *to;
...
int MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineLineToFunc(
  const FT_Vector* to,
  void* user
)
{
...
int MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineLineToCloseContourFunc(
  const FT_Vector* to,
  void* user
)
{
...
int MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineConicToFunc(
  const FT_Vector* control,
  const FT_Vector* to,
  void* user
)
...
int MYON_FreeTypeOutlineAccumlator::Internal_FreeTypeOutlineCubicToFunc(
  const FT_Vector* control1,
  const FT_Vector* control2,
  const FT_Vector* to,
  void* user
...
bool MYON_FreeTypeLoadGlyph(
  MYON__UINT_PTR ft_face_ptr,
  ...
  for (int pass = bLoadRenderBitmap ? 1 : pass0; pass < 2; pass++)
  {
    ...
    if (0 == pass)
    {
      const FT_UInt ft_font_height
        = (0 == pass)
        ? ((FT_UInt)MYON_Font::Constants::AnnotationFontCellHeight)
      ...
      */
      ...
      MYON_Font::DeleteWindowsLogfontDeviceContext(font_hdc);
      const double logical_font_height = ((double)MYON_Font::Constants::AnnotationFontCellHeight); // units = lfu
...
unsigned int MYON_FreeTypeGetGlyphMetrics(
...
)
{
  ...
  ////#if defined(MYON_RUNTIME_APPLE) && defined(MYON_DEBUG)
  ////  const unsigned int apple_glyph_id = MYON_AppleNSFontGlyphIndex(font->AppleFont(), glyph->CodePoint());
  ////  if (glyph_id != apple_glyph_id)
  ////  {
  ////    // In an Aug 20, 2018 test, anytime glyph_id != apple_glyph_id, the freetype value
  ////    // was incorrect. AppleGothic is a good test case. When freetype and MYON_AppleNSFontGlyphIndex
  ...
  ////    MYON_ERROR("MYON_AppleNSFontGlyphIndex() failed.");
  ////    MYON_AppleNSFontGlyphIndex(font->AppleFont(), glyph->CodePoint());
  ////  }
  ////#endif
  ...
  if (false == MYON_FreeTypeLoadGlyph(ft_face_as_uint, glyph_index, bLoadRenderBitmap))
    return 0;
  ...
  glyph_metrics_in_font_design_units = MYON_TextBox_CreateFromFreeTypeGlyphMetrics(&face->glyph->metrics);

  return glyph_index;
}


bool MYON_FreeTypeGetGlyphOutline(
  const MYON_FontGlyph* glyph,
...
)
{
  ...
  return MYON_FreeTypeGetGlyphOutline(glyph, glyph_index, figure_type, outline);
}
...
)
{
  ...
  FT_Face ft_face = (FT_Face)(MYON_Font::FreeTypeFace(font));
  if (nullptr == ft_face)
    return false;

  const MYON_FontMetrics fm = font->FontUnitFontMetrics();

  MYON_FreeTypeOutlineAccumlator a;
...
void MYON_Font::DumpFreeTypeFace(
...
)
{
  ...
  s = MYON_FreeType::StyleFlagsToString(face->style_flags);
  ...
  s = MYON_FreeType::FaceFlagsToString(face->face_flags);
  ...
  if (nullptr != face->charmaps)
  {
    for (int i = 0; i < face->num_charmaps; i++)
    {
      ...
      const wchar_t* damaged
        = MYON_FreeType::IsDamagedCharMap(cmap)
        ? L"DAMAGED "
        : L"";
      s = MYON_wString::FormatToString( L"%lscmap[%d]", damaged, i );
      if (nullptr == cmap)
        s += L"nullptr";
      else
      {
        const MYON_wString s1 = MYON_FreeType::CharmapPlatformEncodingDescription(cmap);
        s += MYON_wString::FormatToString(L" %ls", static_cast<const wchar_t*>(s1));
      }
      text_log.Print("%ls\n", static_cast<const wchar_t*>(s));
    }
  }
...
) const
{
  ...
  if (nullptr == managed_font->m_free_type_face)
    return;
  if (this != managed_font && MYON_Font::CompareFontCharacteristicsForExperts(true, false, *this, *managed_font))
  {
    text_log.Print("FreeType managed font = <%u>\n", managed_font->RuntimeSerialNumber());
  }
  MYON_Font::DumpFreeTypeFace((MYON__UINT_PTR)(managed_font->m_free_type_face->m_face), text_log);
}
