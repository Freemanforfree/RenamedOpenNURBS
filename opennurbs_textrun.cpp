...
class MYON_TextRunPool : public MYON_FixedSizePool
{
public:
  static MYON_TextRunPool thePool;

private:
  MYON_TextRunPool()
  {
    MYON_FixedSizePool::Create(sizeof(MYON_TextRun), 0, 0);
  } 
  ~MYON_TextRunPool() = default;
  MYON_TextRunPool(const MYON_TextRunPool&) = delete;
  MYON_TextRunPool& operator=(const MYON_TextRunPool&) = delete;
};

MYON_TextRunPool MYON_TextRunPool::thePool;

MYON_TextRun::MYON_TextRun(bool bManagedTextRun)
: m_managed_status(bManagedTextRun ? 1 : 0)
{}

MYON_TextRun* MYON_TextRun::GetManagedTextRun()
{
  // .NET wrappers manage objects with test runs and .NET GC runs in its own thread,
  // so text run allocation/return using the global MYON_TextRunPool::thePool resource
  // must be thread safe.
  void* p = MYON_TextRunPool::thePool.ThreadSafeAllocateDirtyElement();
  MYON_TextRun* run 
    = (nullptr != p)
    ? new(p)MYON_TextRun(true) // always happens in practice
    : new MYON_TextRun(false); // could occur if a crashing working thread leaves the pool locked.
  return run;
}
...
{
  MYON_TextRun* run = MYON_TextRun::GetManagedTextRun();
  *run = src;
  return run;
}
...
/*
Description:
  Returns an MYON_TextRun to the pool for reuse.
*/
bool MYON_TextRun::ReturnManagedTextRun(
  MYON_TextRun* run
  )
{
  if ( nullptr == run )
  ...
  if (1 == run->m_managed_status)
  {
    if (0 == run->m_active_status)
    {
      ...
      // so text run allocation/return using the global MYON_TextRunPool::thePool resource
      // must be thread safe.
      MYON_TextRunPool::thePool.ThreadSafeReturnElement(run);
      return true;
    }
    MYON_ERROR("Attempt to return a managed run that is not active.");
    return false;
  }

  MYON_ERROR("Attempt to return a run that is not managed.");
  return false;
}

bool MYON_TextRun::IsManagedTextRun() const
{
  return (1 == m_managed_status);
}

bool MYON_TextRun::IsActiveManagedTextRun() const
{
  return (1 == m_managed_status && 0 == m_active_status);
}

void MYON_TextRunArray::InsertRun(int i, MYON_TextRun*& run)
{
  MYON_SimpleArray< MYON_TextRun* >::Insert(i, run);
  run = nullptr;
}

void MYON_TextRunArray::RemoveRun(int i)
{
  MYON_SimpleArray< MYON_TextRun* >::Remove(i);
}

void MYON_TextRunArray::AppendRun(MYON_TextRun*& run)
{
  MYON_TextRun*& new_run = MYON_SimpleArray< MYON_TextRun* >::AppendNew();
  new_run = run; // copy pointer, not data
  run = 0;       // null source pointer
}

void MYON_TextRunArray::Internal_Destroy()
{
  for (int i = 0; i < m_count; i++)
  {
    MYON_TextRun* text_run = m_a[i];
    if (nullptr != text_run)
    {
      m_a[i] = nullptr;
      if (text_run->IsManagedTextRun())
      {
        MYON_TextRun::ReturnManagedTextRun(text_run);
      }
      else
      {
        delete text_run;
...
MYON_TextRun*const* MYON_TextRunArray::Array() const
{
  return (m_count > 0) ? m_a : nullptr;
}


const MYON_TextRun* MYON_TextRunArray::operator[](int i) const
{
  return (i >= 0 && i < m_count) ? m_a[i] : nullptr;
}

MYON_TextRun* MYON_TextRunArray::operator[](int i)
{
  return (i >= 0 && i < m_count) ? m_a[i] : nullptr;
}

int MYON_TextRunArray::Count() const
{
  return m_count;
}

unsigned int MYON_TextRunArray::UnsignedCount() const
{
  return (unsigned int)m_count;
}


void MYON_TextRunArray::SetTextHeight(double height)
{
  for (int ri = 0; ri < Count(); ri++)
  {
    MYON_TextRun* run = m_a[ri];
    if (nullptr == run)
      continue;
...
bool MYON_TextRunArray::Get2dCorners(MYON_2dPoint corners[4]) const
{
  MYON_2dPoint minpt(0.0, 0.0), maxpt(0.0, 0.0);
  bool rc = false;
  for (int ri = 0; ri < Count(); ri++)
  {
    const MYON_TextRun* run = m_a[ri];
    if (nullptr == run)
      continue;
...
void MYON_TextRunArray::Internal_CopyFrom(const MYON_TextRunArray& src)
{
  SetCount(0);
  Reserve(src.m_count);
  for (int i = 0; i < src.m_count; i++)
  {
    const MYON_TextRun* src_text_run = src.m_a[i];
    if ( nullptr != src_text_run )
      AppendNew() = MYON_TextRun::GetManagedTextRun(*src_text_run);
  }
}


MYON_TextRunArray::~MYON_TextRunArray()
{
  Internal_Destroy();
}

MYON_TextRunArray& MYON_TextRunArray::operator=(const MYON_TextRunArray& src)
{
  if (this != &src)
  {
    Internal_Destroy();
...
MYON_TextRunArray::MYON_TextRunArray(const MYON_TextRunArray& src)
{
  Internal_CopyFrom(src);
}
...
MYON_StackedText::~MYON_StackedText()
{
  // NOTE WELL:
  //  https://mcneel.myjetbrains.com/youtrack/issue/RH-57616
  //  MYON_StackedText::Empty is a global const instance.
  ...
  {
    MYON_TextRun::ReturnManagedTextRun(m_top_run);
    m_top_run = nullptr;
  }
  if (nullptr != m_bottom_run)
  {
    MYON_TextRun::ReturnManagedTextRun(m_bottom_run);
...
MYON_StackedText::MYON_StackedText(const MYON_StackedText& src)
  : MYON_StackedText()
{
  if (this != &src)
    *this = src;
...
MYON_StackedText& MYON_StackedText::operator=(const MYON_StackedText& src)
{
  if (&src != this)
  {
    MYON_TextRun::ReturnManagedTextRun(m_top_run);
    ...
    {
      m_top_run = MYON_TextRun::GetManagedTextRun();
      *m_top_run = *src.m_top_run;
    }
    if (nullptr != src.m_bottom_run)
    {
      m_bottom_run = MYON_TextRun::GetManagedTextRun();
...
//}
//
//bool MYON_StackedText::WriteNested(
...
//}
//
//bool MYON_StackedText::ReadNested(
//  unsigned int nested_depth,
//  MYON_BinaryArchive& archive
//  )
//{
//  *this = MYON_StackedText::Empty;
//  if ( nested_depth > 8 )
//    return false;
//
//  int major_version = 0;
...
MYON_TextRun::~MYON_TextRun()
{
  Internal_Destroy();
}

MYON_TextRun::MYON_TextRun(const MYON_TextRun& src)
{
  Internal_CopyFrom(src);
}

void MYON_TextRun::Internal_Destroy()
{
...
void MYON_TextRun::Internal_CopyFrom(const MYON_TextRun& src)
{
  // "this" is clean.
  ...
  {
    m_stacked_text = new MYON_StackedText(*src.m_stacked_text);
...
MYON_TextRun& MYON_TextRun::operator=(const MYON_TextRun& src)
{
  if ( &src != this )
  {
    Internal_Destroy();
  ...
  bool deletedisplay)
{
  *this = MYON_TextRun::Empty;
    ...
    )
  {
    ...
    // your new function.
    managed_font = MYON_Font::FontFromRichTextProperties(
      MYON_Font::RichTextFontName(managed_font, true),
      bold,
      italic,
      underlined,
      strikethrough);
...
bool MYON_TextRun::IsText() const
{
  return Type() == RunType::kText;
}
bool MYON_TextRun::IsNewline() const
{
  return Type() == RunType::kNewline;
}
bool MYON_TextRun::IsColumn() const
{
...
bool MYON_TextRun::IsValid() const
{
  switch (Type())
  {
  case RunType::kField:
  case RunType::kText:
  {
    ...
    if ( IsStacked() != Stacked::kStacked &&
       (nullptr == m_codepoints ||
        0 == MYON_TextRun::CodepointCount(m_codepoints)) &&
...
void MYON_TextRun::Internal_ContentChanged() const
{
  m_text_run_hash = MYON_SHA1_Hash::ZeroDigest;
  m_text_run_display_hash = MYON_SHA1_Hash::ZeroDigest;
}

void MYON_TextRun::SetType(MYON_TextRun::RunType type)
{
  //if (type == MYON_TextRun::RunType::kField)
  //  type = type;
  if (m_run_type != type)
  {
...
MYON_TextRun::RunType MYON_TextRun::Type() const
{
  //if (m_run_type == MYON_TextRun::RunType::kField)
  //  int i = 2;
  return m_run_type;
}

MYON_TextRun::RunDirection MYON_TextRun::FlowDirection() const
{
  return m_direction;
}

void MYON_TextRun::SetDisplayString(const wchar_t* str)
{
  if (str != m_display_string.Array())
  {
    MYON_wString local(str);
...
const wchar_t* MYON_TextRun::DisplayString() const
{
...
const wchar_t* MYON_TextRun::TextString() const
{
  if (m_text_string.IsEmpty())
  {
    if (nullptr != m_codepoints)
    {
      ...
      size_t cplen = MYON_TextRun::CodepointCount(m_codepoints);
      if (0 < cplen) 
        MYON_TextContext::ConvertCodepointsToString((int)cplen, m_codepoints, m_text_string);
    }
  }
  return m_text_string.Array();
}

MYON_TextRun::Stacked MYON_TextRun::IsStacked() const
{
  return m_text_stacked;
}

void MYON_TextRun::SetStacked(Stacked stacked)
{
...
void MYON_TextRun::SetStackedOff()
{
  SetStacked(Stacked::kNone);
}

double MYON_TextRun::TextHeight() const
{
  return m_run_text_height;
}

void MYON_TextRun::SetTextHeight(double h)
{
  if(h > 0.0)
  {
...
MYON_Color MYON_TextRun::Color() const
{
  return m_color;
}

void MYON_TextRun::SetColor(MYON_Color color)
{
  if (m_color != color)
  {
    Internal_ContentChanged();
...
void MYON_TextRun::SetFont(const MYON_Font* font)
{
  const MYON_Font* managed_font
    = (nullptr != font)
    ? font->ManagedFont()
...
const MYON_Font* MYON_TextRun::Font() const
{
  return m_managed_font;
}

const MYON_BoundingBox& MYON_TextRun::BoundingBox() const
{
  return m_bbox;
};

void MYON_TextRun::SetBoundingBox(MYON_2dPoint pmin, MYON_2dPoint pmax)
{
...
const MYON_2dVector& MYON_TextRun::Offset() const
{
  return m_offset;
}

void MYON_TextRun::SetOffset(MYON_2dVector offset)
{
  if (!(m_offset == offset))
  {
    Internal_ContentChanged();
...
const MYON_2dVector& MYON_TextRun::Advance() const
{
  return m_advance;
}

void MYON_TextRun::SetAdvance(MYON_2dVector advance)
{
...
MYON_SHA1_Hash MYON_TextRun::TextRunContentHash() const
{
  return TextRunContentHash(true);
}


MYON_SHA1_Hash MYON_TextRun::TextRunContentHash(
  bool bEvaluateFields
) const
{
  if (MYON_SHA1_Hash::ZeroDigest == m_text_run_hash)
  {
    MYON_SHA1 sha1;
    
    const bool bIsStacked = (
      MYON_TextRun::Stacked::kStacked == IsStacked() 
...
void MYON_TextRun::SetStackFractionHeight(double stackscale)
{
  if (MYON_SQRT_EPSILON < stackscale && 10000.0 > stackscale)
  {
...
double MYON_TextRun::StackHeightFraction() const
{
  return m_stackscale;
}

//static 
double MYON_TextRun::DefaultStackFractionHeight()
{
  return 0.7;
}

void MYON_TextRun::Get2dCorners(MYON_2dPoint corners[4]) const
{
  const MYON_2dVector& offset = Offset();
...
double MYON_TextRun::HeightScale(const MYON_Font* font) const
{
  if(m_height_scale <= 0.0)
  {
    if (0 != font)
      const_cast< MYON_TextRun* >(this)->m_height_scale = font->FontMetrics().GlyphScale(m_run_text_height);
  }
...
// static
size_t MYON_TextRun::CodepointCount(const MYON__UINT32* cp)
{
  if (0 == cp)
    return 0;
  size_t i = 0;
...
const MYON__UINT32* MYON_TextRun::UnicodeString() const
{
  return m_codepoints;
}
...
void MYON_TextRun::SetUnicodeString(size_t count, const MYON__UINT32* cp)
{
  m_bbox.Destroy();
  m_text_string.Empty();
...
void MYON_TextRun::SetUnicodeString(MYON__UINT32*& dest, size_t count, const MYON__UINT32* cp)
{
  if (0 == count || 0 == cp)
  {
    if (0 != dest)
  ...
  MYON__UINT32* newcp = (MYON__UINT32*)onrealloc(dest, (count + 1)*sizeof(MYON__UINT32));
  if(0 == newcp)
  {
    onfree(dest);
    dest = nullptr;
  ...
  {
    dest = newcp;
    memcpy(dest, cp, count*sizeof(MYON__UINT32));
    dest[count] = 0; // add terminating zero
  }
}

int MYON_TextRun::WrapTextRun(
  ...
  MYON_TextRunArray& newruns// new runs made by wrapping
) const
{
  int new_count = 0;
  ...
  if (500 < call_count)
  {
    MYON_ERROR("WrapTextRun: Recursion too deep.");
    return 0;
  }
  if (0 > start_char_offset)
  {
    MYON_ERROR("WrapTextRun: String start offset < 0.");
    start_char_offset = 0;
  }

  if (0.0 > linewidth)
  {
    MYON_ERROR("WrapTextRun: Linewidtht < 0.");
    linewidth = 0.0;
  }
  ...
  MYON_FontGlyph::GetGlyphList(display_string, font, MYON_NextLine, glyph_list, text_box);
  int glyph_count = glyph_list.Count();

  const MYON_FontGlyph* Aglyph = font->CodePointGlyph((MYON__UINT32)L'A');
  if (nullptr == Aglyph)
    return 0;
  const MYON_TextBox Aglyph_box = Aglyph->GlyphBox();
  ...
  if (floor(Awidth) < 0.0)
  {
    MYON_ERROR("Font height scale * width of 'A' is less than 0\n");
    return 0;
  }
...
#pragma region Run Width
  ...
  else // Part of the run has already been picked off and added to the previous line
  {
    ...
    {
      const MYON_FontGlyph* gi = glyph_list[ci];

      if (nullptr != gi)
      {
        const MYON_TextBox glyph_box = gi->GlyphBox();
...
#pragma region Whole Run
  ...
  if (runwidth + linewidth <= wrapwidth || 2 > wcscount)
  {
    ...
    MYON_TextRun* newrun = MYON_TextRun::GetManagedTextRun();
    if (nullptr != newrun)
    {
      *newrun = *this;
      if (Type() == MYON_TextRun::RunType::kNewline ||
        Type() == MYON_TextRun::RunType::kParagraph ||
        Type() == MYON_TextRun::RunType::kSoftreturn)
        linewidth = 0.0;
      else
      {
        if (0 != start_char_offset)
...
#pragma endregion Whole Run
  ...
  for (int ci = start_char_offset; ci < (int)wcscount; ci++)
  {
    ...
    const MYON_FontGlyph* glyph = glyph_list[gi];
    if (nullptr != glyph)
    {
      const MYON_TextBox glyph_box = glyph->GlyphBox();
      ...
      if (linewidth + curwidth > wrapwidth) // reached wrapping width
      {
        ...
        if (0 < run_length)
        {
          MYON_TextRun* newrun = MYON_TextRun::GetManagedTextRun();  // make a new run
          if (nullptr != newrun)
          {
            *newrun = *this;
            ...
            newrun->SetDisplayString(temp_display_str);
            newrun->SetOffset(MYON_2dVector(0.0, y_offset + Offset().y));
            newruns.AppendRun(newrun);
          }
        }
        // add a soft return
        MYON_TextRun* lfrun = MYON_TextRun::GetManagedTextRun();
        if (nullptr != lfrun)
        {
          lfrun->SetFont(Font());
          lfrun->SetType(MYON_TextRun::RunType::kSoftreturn);
...
MYON_StackedText::StackStyle MYON_StackedText::StackStyleFromUnsigned(
  unsigned int stack_style_as_unsigned
)
{
  switch (stack_style_as_unsigned)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_StackedText::StackStyle::kUnset);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_StackedText::StackStyle::kHorizontalToScreen);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_StackedText::StackStyle::kSlanted);
  }
  MYON_ERROR("stack_style_as_unsigned parameter is not valid");
  return MYON_StackedText::StackStyle::kUnset;
}

MYON_TextRun::RunType MYON_TextRun::RunTypeFromUnsigned(
  unsigned int run_type_as_unsigned
  )
{
  switch (run_type_as_unsigned)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kNone);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kText);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kNewline);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kSoftreturn);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kParagraph);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kColumn);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kField);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kFieldValue);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kFontdef);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kHeader);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunType::kFonttbl);
  }
  MYON_ERROR("run_type_as_unsigned parameter is not valid");
  return MYON_TextRun::RunType::kNone;
}
...
MYON_TextRun::Stacked MYON_TextRun::StackedFromUnsigned(
  unsigned int stacked_as_unsigned
  )
{
  switch (stacked_as_unsigned)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::Stacked::kNone);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::Stacked::kStacked);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::Stacked::kTop);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::Stacked::kBottom);
  }
  MYON_ERROR("stacked_as_unsigned parameter is not valid");
  return MYON_TextRun::Stacked::kNone;
}
...
MYON_TextRun::RunDirection MYON_TextRun::RunDirectionFromUnsigned(
  unsigned int run_direction_as_unsigned
  )
{
  switch (run_direction_as_unsigned)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunDirection::kLtR);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_TextRun::RunDirection::kRtL);
  }
  MYON_ERROR("run_direction_as_unsigned parameter is not valid");
  return MYON_TextRun::RunDirection::kLtR;
}

//bool MYON_TextRun::Write(
//  MYON_BinaryArchive& archive
//  ) const
//{
//  return WriteNested(0,archive);
...
//
//    
//    const MYON__UINT32* cp1 = m_codepoints;
//    if (nullptr != cp1)
//    {
//      while ( 0 != *cp1 )
//        cp1++;
...
//      // m_stacked_text will increment nested_depth if it calls back to MYON_TextRun::WriteNested().
...
//    if (count > 0)
//    {
//      m_codepoints = (MYON__UINT32*)onmalloc( (count + 1)*sizeof(m_codepoints[0]));
//      if ( nullptr == m_codepoints )
//        break;
