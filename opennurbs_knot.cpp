...
double MYON_DomainTolerance( double a, double b )
{
  if ( a == b )
    return 0.0;
  double tol = (fabs(a)+fabs(b)+fabs(a-b))* MYON_SQRT_EPSILON;
  if ( tol <  MYON_EPSILON )
    tol =  MYON_EPSILON;
  return tol;
}
...
double MYON_KnotTolerance( int order, int cv_count, const double* knot, 
                                    int knot_index )
{
  ...
  tol = (a==0.0 && b==0.0) ? 0.0 : (a + b + fabs(knot[knot_index]))* MYON_SQRT_EPSILON;
  return tol;
}
...
double MYON_SpanTolerance( int order, int cv_count, const double* knot, int span_index )
{
  const int i0 = span_index+order-2;
  return MYON_DomainTolerance( knot[i0], knot[i0+1] );
}
...
int MYON_KnotCount( int order, int cv_count )
{
          ...
          )
{
  if ( 0 == knot )
  {
    ...
    {
      MYON_ERROR("nullptr knot[] passed to MYON_KnotVectorSpanCount.");
    }
    return 0;
  }
  int i, span_count = 0;
...
bool MYON_GetKnotVectorSpanVector(
          int order,          // order (>=2)
          int cv_count,       // cv count
          const double* knot, // knot[] array
          double* s           // s[] array
          )
{
  if ( 0 == knot || 0 == s )
  {
    if ( 0 != order || 0 != cv_count )
    {
      MYON_ERROR("nullptr knot[] or s[] passed to MYON_KnotVectorSpanCount.");
      return false;
    }
    return true;
  }
...
int MYON_NurbsSpanIndex(
          int order,          // (>=2)
          int cv_count,
          const double* knot, // knot[] array or length MYON_KnotCount(order,cv_count)
          ...
          )
{
  ...
  j = MYON_SearchMonotoneArray(knot,len,t);
  if (j < 0)
    j = 0;
  else if (j >= len-1)
    j = len-2;
...
int MYON_NextNurbsSpanIndex( int order, int cv_count, const double* knot, int span_index )
...
EXAMPLE:
  ...
  for (;;) {
    ...
    next_span_index =MYON_NextNurbsSpanIndex( order, cv_count, knot, span_index );
...
int MYON_GetSpanIndices(int order, 
                            int cv_count, 
                            const double* knot, 
                            int* span_indices)
...
bool MYON_IsKnotVectorPeriodic(
       int order,
       int cv_count,
       const double* knot
       )

{
  ...
  if ( order < 2 || cv_count < order || !knot ) {
    MYON_ERROR("MYON_IsKnotVectorPeriodic(): illegal input");
    return false;
  }
  ...
  tol = fabs(knot[order-1] - knot[order-3])* MYON_SQRT_EPSILON;
  if (tol < fabs(knot[cv_count-1] - knot[order-2])* MYON_SQRT_EPSILON) 
    tol = fabs(knot[cv_count-1] - knot[order-2])* MYON_SQRT_EPSILON;
          ...
          )
{
  ...
  if (rc)
  {
    ...
    rc = (0.0 != delta && delta > MYON_UNSET_VALUE && delta < MYON_UNSET_POSITIVE_VALUE);
    if (rc)
    {
      const int i0 = MYON_IsKnotVectorClamped(order, cv_count, knot, 0) ? order : 1;
      const int i1 = MYON_IsKnotVectorClamped(order, cv_count, knot, 1) ? cv_count : MYON_KnotCount(order, cv_count);
      double k0 = knot[i0 - 1];
      const double delta_tol = fabs(MYON_SQRT_EPSILON*delta);
...
bool MYON_KnotVectorHasBezierSpans(
          int order,          // order (>=2)
          int cv_count,       // cv count
          const double* knot  // knot[] array
          )
{
  int knot_count = MYON_KnotCount( order, cv_count );
  if ( knot_count < 2 )
    return false;
  int span_count = MYON_KnotVectorSpanCount( order, cv_count, knot );
  if ( span_count < 1 )
    return false;
  if ( order >= 2 && 
       cv_count >= order && 
...
MYON::knot_style MYON_KnotVectorStyle( 
       int order,
       int cv_count,
       const double* knot
       )
{
  MYON::knot_style s = MYON::unknown_knot_style;
  if ( order >= 2 && cv_count >= order && knot && knot[order-2] < knot[cv_count-1] ) {
    ...
    int i;
    if ( MYON_IsKnotVectorClamped( order, cv_count, knot ) ) {
      if ( order == cv_count ) {
        s = MYON::piecewise_bezier_knots;
      }
      else {
        s = MYON::clamped_end_knots;
...
bool MYON_SetKnotVectorDomain( int order, int cv_count, double* knot, double t0, double t1 )
{
  bool rc = false;
  if ( order < 2 || cv_count < order || 0 == knot || t0 >= t1 || !MYON_IsValid(t0) || !MYON_IsValid(t1) )
  {
    MYON_ERROR("MYON_SetKnotVectorDomain - invalid input");
  }
  else if (    knot[order-2] >= knot[cv_count-1] 
            || !MYON_IsValid(knot[order-2]) 
            || !MYON_IsValid(knot[cv_count-2]) )
  {
    MYON_ERROR("MYON_SetKnotVectorDomain - invalid input knot vector");
  }
  else
  {
    const MYON_Interval oldd(knot[order-2],knot[cv_count-1]);
    const MYON_Interval newd(t0,t1);
    if ( oldd != newd )
    {
      int i, knot_count = MYON_KnotCount(order,cv_count);
...
int MYON_CompareKnotVector( // returns 
                                      // -1: first < second
                                      //  0: first == second
                                      // +1: first > second
          int orderA,
...
{
  const int knot_count = MYON_KnotCount(orderA,cv_countA);
  int i;
  ...
  if ( !MYON_GetKnotVectorDomain( orderA, cv_countA, knotA, &a, &b ) )
    return -1;
  atol = MYON_DomainTolerance( a, b );
  if ( !MYON_GetKnotVectorDomain( orderA, cv_countA, knotA, &a, &b ) )
    return 1;
  btol = MYON_DomainTolerance( a, b );
  ...
  for ( i = 0; i < knot_count; i++ ) {
    ...
    if ( b < a-tol )
      return 1;
    atol = MYON_KnotTolerance( orderA, cv_countA, knotA, i );
    btol = MYON_KnotTolerance( orderB, cv_countB, knotB, i );
...
static bool MYON_KnotVectorIsNotValid(bool bSilentError)
{
  return bSilentError ? false : MYON_IsNotValid(); // <-- good place for a breakpoint
}

bool MYON_IsValidKnotVector( int order, int cv_count, const double* knot, MYON_TextLog* text_logx )
{
  ...
  if ( order < 2 )
  {
    ...
    return MYON_KnotVectorIsNotValid(bSilentError);
  }
  if ( cv_count < order )
  {
    if ( text_log )
...
bool MYON_MakeKnotVectorPeriodic(
          int order,          // order (>=2)
          ...
          )
{
  ...
  if ( order < 2 || cv_count < order || !knot ) {
    MYON_ERROR("MYON_MakePeriodicKnotVector(): illegal input");
    return false;
  }
  ...
  case 2:
    if ( cv_count < 4 ) {
      MYON_ERROR("MYON_MakePeriodicKnotVector(): illegal input degree=1, cv_count<4");
      return false;
    }
    else {
      return true;
  ...
  case 3:
    if ( cv_count < 4 ) {
      MYON_ERROR("MYON_MakePeriodicKnotVector(): illegal input degree=2, cv_count<5");
      return false;
    }
    break;
  default:
    if ( cv_count < 2*order-2 ) {
      MYON_ERROR("MYON_MakePeriodicKnotVector(): illegal input degree>=3, cv_count<2*degree");
      return false;
    }
    break;
  }
...
MYON_DECL
bool MYON_MakeClampedUniformKnotVector(
          int order,
          int cv_count,
          double* knot,
          double delta
          )
{
  ...
  if ( order >= 2 && cv_count >= order && knot != nullptr && delta > 0.0 )
  {
    ...
    for ( i = order-2, k = 0.0; i < cv_count; i++, k += delta )
      knot[i] = k;
    MYON_ClampKnotVector(order,cv_count,knot,2);
    rc = true;
  }
  return rc;
}
...
//       MYON_KnotCount(order,cv_count) doubles.  Output is
...
//   true if successful
MYON_DECL
bool MYON_MakePeriodicUniformKnotVector(
          int order,
          int cv_count,
          double* knot,
          double delta
...
double MYON_GrevilleAbcissa( // get Greville abcissa
          int order,          // order (>=2)
...

{
  ...
  else 
  {
    ...
    const double tol = (k1-k0)*MYON_SQRT_EPSILON;
    ...
    //g /= ((double)order);
    g /= const_ord;
    if ( fabs(2*k - (k0+k1)) <= tol && fabs(g-k) <= (fabs(g)*MYON_SQRT_EPSILON+tol) )
      g = k; // sets g to exact value when knot vector is uniform
  }
  return g;
}

bool MYON_GetGrevilleAbcissae( // get Greville abcissa from knots
          int order,          // order (>=2)
          ...
          )
{
  ...
  else 
  {
    ...
    if (bPeriodic)
    {
      ...
      {
        g[i] = MYON_GrevilleAbcissa(order, knot + i);
        if (g[i] >= t0)
        {
...
bool MYON_GetGrevilleKnotVector( // get knots from Greville abcissa
          int g_stride,     
  ...
  if ( bPeriodic ) {
    ...
    for ( j = 0; j < g_count+order; j++ ) {
      ...
      if ( half_degree ) {
        ...
        if ( fabs(k-p[j+half_degree]) <=  MYON_SQRT_EPSILON*(p[j+degree-1]-p[j]) )
          k = p[j+half_degree];
      }
      p[j] = k;
    }
    ...
    // step 3: determine where g[0] maximizes NURBS basis functions
    {
      ...
      for ( j = 0; j < 2*degree; j++ ) {
        ...
        if ( g[0] < p[j+degree-1] )
          break;
        MYON_EvaluateNurbsBasis( order, p+j, g[0], B );
        if ( B[0] > maxB ) {
          maxB  = B[0];
          maxBj = j;
        }
    ...
    rc = MYON_MakeKnotVectorPeriodic( order, cv_count, knot );
  }
  else {
    // clamped knots
    rc = true;
        ...
        )
{
 ...
 bool rc = false;
  ...
  if (cv && knot && order >= 2 && cv_count >= order && end>=0 && end<=2 ) {
    rc = true;
    if ( end == 0 || end == 2 ) { 
      if (MYON_EvaluateNurbsDeBoor(cv_dim, order, cv_stride, cv, knot, 1, 0.0, knot[order - 2]))
      {
        for (i = 0; i < order - 2; i++)
          knot[i] = knot[order - 2];
      }
    ...
    if ( end == 1 || end == 2 ) {
      ...
      knot += i0;
      cv += i0*cv_stride;
      if (MYON_EvaluateNurbsDeBoor(cv_dim, order, cv_stride, cv, knot, -1, 0.0, knot[order - 1]))
      {
        i0 = order - 1;
...
static bool MYON_InsertSingleKnot( int cv_dim, int order, 
                             int cv_stride, 
                             ...
                             )
{
  ...
  if ( order < 2 || !knot || knot_value < knot[order-2] || knot[order-1] <= knot_value ) {
    MYON_ERROR( "MYON_InsertSingleKnot() - illegal knot input" );
    return false;
  }

  if ( cv ) {
    if ( cv_dim < 1 || cv_stride < cv_dim  ) {
      MYON_ERROR( "MYON_InsertSingleKnot() - illegal cv input" );
      return false;
    }
  }
        ...
        )
{
  ...
  if ( order < 2 || cv_count < order || !knot ) 
  {
    MYON_ERROR("MYON_InsertKnot(): illegal input" );
    return 0;
  }
  ...
  {
    MYON_ERROR("MYON_InsertKnot(): requested knot_multiplicity > degree" );
    return 0;
  }

  // shift knot vector and cv array so that knot_value lies in first span
  int span_index = MYON_NurbsSpanIndex( order, cv_count, knot, knot_value, 1, hint?*hint:0 );
  knot += span_index;
  if ( cv )
    cv += (span_index*cv_stride);
  cv_count -= span_index;

  const double knot_tolerance = MYON_SpanTolerance( order, cv_count, knot, 0 );
  ...
  if ( span_index == 0 ) 
  {
    if ( knot_value < knot[order-1] ) 
    {
      ...
      {
        MYON_ERROR("MYON_InsertKnot(): requested knot_value at start of NURBS domain" );
        return 0;
      }
    }
  }
  if ( span_index == cv_count-order ) 
  {
    ...
    {
      MYON_ERROR("MYON_InsertKnot(): requested knot_value at end of NURBS domain" );
      return 0;
    }
  }
  ...
  if ( !new_knot ) {
    MYON_ERROR("MYON_InsertKnot(): out of memory");
    return 0;
  }
  double* new_cv = 0;
  ...
  rc = 0;
  while (m>0) {
    if ( !MYON_InsertSingleKnot(cv_dim,order,cv_dim,new_cv,new_knot,knot_value) )
      break;
    m--;
    if ( new_cv )
      new_cv += cv_stride; 
  ...
  if ( rc > 0 ) 
  {
    ...
    int i0 = MYON_KnotCount( order, cv_count ) - 1; // knot[i0] = last input knot
    int i1 = i0 + rc;
