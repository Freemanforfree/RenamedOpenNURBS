...
MYON_OBJECT_IMPLEMENT( MYON_RevSurface, MYON_Surface, "A16220D3-163B-11d4-8000-0010830122F0");


void MYON_RevSurface::DestroyRuntimeCache( bool bDelete )
{
...
MYON_RevSurface* MYON_RevSurface::New()
{
  return new MYON_RevSurface();
}

MYON_RevSurface* MYON_RevSurface::New( const MYON_RevSurface& rev_surface )
{
  return new MYON_RevSurface(rev_surface);
}

MYON_RevSurface::MYON_RevSurface() : m_curve(0), 
                                 m_axis( MYON_3dPoint::Origin, MYON_3dVector::ZAxis ), 
                                 m_angle( 0.0, 2.0*MYON_PI ),
                                 m_t( 0.0, 2.0*MYON_PI ),
                                 m_bTransposed(0)
{
  MYON__SET__THIS__PTR(m_s_MYON_RevSurface_ptr);
}

MYON_RevSurface::~MYON_RevSurface()
{
  Destroy();
}

void MYON_RevSurface::Destroy()
{
  DestroySurfaceTree();
  if ( m_curve)
  {
  ...
  m_axis.Create( MYON_3dPoint::Origin, MYON_3dVector::ZAxis );
  m_angle.Set(0.0,2.0*MYON_PI);
  m_t = m_angle;
  m_bTransposed = false;
  m_bbox.Destroy();
}

MYON_RevSurface::MYON_RevSurface( const MYON_RevSurface& src ) : MYON_Surface(src)
{
  MYON__SET__THIS__PTR(m_s_MYON_RevSurface_ptr);
...
unsigned int MYON_RevSurface::SizeOf() const
{
...
MYON__UINT32 MYON_RevSurface::DataCRC(MYON__UINT32 current_remainder) const
{
  if ( m_curve )
    current_remainder = m_curve->DataCRC(current_remainder);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_axis),&m_axis);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_angle),&m_angle);
...
MYON_RevSurface& MYON_RevSurface::operator=( const MYON_RevSurface& src )
{
  if ( this != &src ) 
  {
    Destroy();
...
bool MYON_RevSurface::SetAngleRadians(
  ...
  )
{
  ...
  if ( d >= 0.0 )
  {
    if ( d <= MYON_ZERO_TOLERANCE || d > 2.0*MYON_PI )
    {
      end_angle_radians = start_angle_radians + 2.0*MYON_PI;
    }
...
bool MYON_RevSurface::SetAngleDegrees (
  double start_angle_degrees,
  double end_angle_degrees
  )
{
  return SetAngleRadians( 
    start_angle_degrees*MYON_PI/180.0, 
    end_angle_degrees*MYON_PI/180.0 
    );
}


bool MYON_RevSurface::IsValid( MYON_TextLog* text_log ) const
{
  if ( !m_curve )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_curve is nullptr.\n");
    return false;
  }
  if ( !m_curve->IsValid(text_log) )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_curve is not valid.\n");
    return false;
  }
  ...
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_curve->Dimension()=%d (should be 3).\n",dim);
    return false;
  }
  if ( !m_axis.IsValid() )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_axis is not valid.\n");
    return false;
  }
  if ( !m_angle.IsIncreasing() )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_angle = (%g,%g) (should be an increasing interval)\n",
                       m_angle[0],m_angle[1]);
    return false;
  }
  double length = m_angle.Length();
  if ( length > 2.0*MYON_PI + MYON_ZERO_TOLERANCE )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_angle.Length() = %g (should be <= 2*pi radians).\n",length);
    return false;
  }
  if ( m_angle.Length() <= MYON_ZERO_TOLERANCE )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_angle.Length() = %g (should be > MYON_ZERO_TOLERANCE).\n",length);
    return false;
  }
  if ( !m_t.IsIncreasing() )
  {
    if ( text_log )
      text_log->Print( "MYON_RevSurface.m_t = (%g,%g) (should be an increasing interval)\n",
                       m_t[0],m_t[1]);
    return false;
  }
  return true;
...
void MYON_RevSurface::Dump( MYON_TextLog& dump ) const
{
  MYON_Object::Dump(dump); // print class id
  dump.PushIndent();
  if ( m_bTransposed )
...
bool MYON_RevSurface::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_RevSurface::Read( MYON_BinaryArchive& file )
{
  int major_version = 0;
  int minor_version = 0;
  char bHaveCurve = 0;
...
int MYON_RevSurface::Dimension() const
{
  return 3;
}

bool MYON_RevSurface::Transform( const MYON_Xform& xform )
{
  DestroyRuntimeCache();
  ...
  if ( !m_axis.Transform(xform) )
    rc = false;
  MYON_3dVector transZ = m_axis.Tangent();
  ...
  else
  {
    ...
    MYON_3dVector transX = xform*X;
    MYON_3dVector transY = xform*Y;
    MYON_3dVector transXxY = MYON_CrossProduct( transX, transY );
...
bool MYON_RevSurface::Evaluate( // returns false if unable to evaluate
       double s, // angle
       ...
       ) const
{
  ...
  if ( curve_dim == 2 || curve_dim == 3 ) 
  {
    ...
    if ( rc ) 
    {
      MYON_3dVector zaxis = m_axis.Tangent();
      ...
      {
        dst_i = v_stride*((i+1)*(i+2)/2 - 1);
        pt = MYON_3dPoint(v+dst_i); // pt = curve derivative in world coords
        v[dst_i++] = pt*xaxis;
        v[dst_i++] = pt*yaxis;
        v[dst_i]   = pt*zaxis;
      }
...
class MYON_RevolutionTensor : public MYON_TensorProduct
{
public:
  MYON_3dPoint  O;
  MYON_3dVector X;
...
int MYON_RevolutionTensor::DimensionA() const
{
  return 2;
}

int MYON_RevolutionTensor::DimensionB() const
{
  return 3;
}

int MYON_RevolutionTensor::DimensionC() const
{
  return 3;
}

bool MYON_RevolutionTensor::Evaluate( double a, const double* ArcPoint, double b, const double* ShapePoint, double* SrfPoint )
{
...
int MYON_RevSurface::GetNurbForm(class MYON_NurbsSurface& srf , double tolerance ) const
{
  int rc = 0;
  if ( 0 != m_curve ) 
  {
    MYON_NurbsCurve a, c;
    MYON_Arc arc;
    arc.plane.CreateFromNormal( MYON_3dPoint::Origin, MYON_3dVector::ZAxis );
    arc.radius = 1.0;
    ...
    if ( arc.GetNurbForm(a) ) 
    {
      ...
      if (rc) 
      {
        if ( 2 == c.m_dim )
        {
          ...
          // bug # 103845 work.
          MYON_WARNING("MYON_RevSurface.m_curve is 2-dimensional.");
          c.ChangeDimension(3);
        }
        if ( 3 != c.m_dim )
        {
          MYON_ERROR("MYON_RevSurface.m_curve is not valid.");
          return 0;
        }
        ...
        MYON_RevolutionTensor rho;
        rho.O = m_axis.from;
        rho.Z = m_axis.Direction();
        rho.Z.Unitize();
        rho.X.PerpendicularTo(rho.Z);
        rho.X.Unitize();
        rho.Y = MYON_CrossProduct(rho.Z,rho.X);
        rho.Y.Unitize();
        ...
        MYON_3dPoint C0 = c.PointAtStart();
        MYON_3dPoint C1 = c.PointAtEnd();
        MYON_3dPoint A0, A1;
        MYON_4dPoint CV;
        double t0 = MYON_UNSET_VALUE;
        double t1 = MYON_UNSET_VALUE;
        if (m_axis.ClosestPointTo(C0,&t0) && MYON_IsValid(t0) )
        {
          A0 = m_axis.PointAt(t0);
          if ( C0.DistanceTo(A0) <= MYON_ZERO_TOLERANCE )
          {
            // SouthPole
            int j = 0;
        ...
        if (m_axis.ClosestPointTo(C1,&t1) && MYON_IsValid(t1) )
        {
          A1 = m_axis.PointAt(t1);
          if ( C1.DistanceTo(A1) <= MYON_ZERO_TOLERANCE )
          {
            // NorthPole
...
int MYON_RevSurface::HasNurbForm() const
...
bool MYON_RevSurface::GetSurfaceParameterFromNurbFormParameter(
      double nurbs_s, double nurbs_t,
      double* surface_s, double* surface_t
      ) const
{
  // NOTE: overrides MYON_Surface virtual function
  bool rc = (0 != m_curve);
  ...
  *surface_t = nurbs_t;
  ...
  MYON_Circle circle( MYON_xy_plane, 1.0 );
  MYON_Arc arc( circle, m_angle );
  MYON_ArcCurve arc_curve(arc, m_t[0], m_t[1]);
...
bool MYON_RevSurface::GetNurbFormParameterFromSurfaceParameter(
      double surface_s, double surface_t,
      double* nurbs_s,  double* nurbs_t
      ) const
{
...
MYON_Arc MYON_RevSurface::IsoArc(
...
) const
{
  for (;;)
  {
    ...
    //   when m_angle[0] != 0.
    MYON_Circle circle;
    MYON_3dPoint P = m_curve->PointAt(curve_parameter);
    if (false == P.IsValid())
      break;
    ...
    if (!circle.plane.xaxis.Unitize())
    {
      ...
      MYON_3dPoint Q = m_axis.ClosestPointTo(P);
    ...
    circle.plane.yaxis = MYON_CrossProduct(circle.plane.zaxis, circle.plane.xaxis);
  ...
  MYON_Arc arc;
  arc.plane = MYON_Plane::NanPlane;
  arc.radius = MYON_DBL_QNAN;
  return arc;
}


MYON_Curve* MYON_RevSurface::IsoCurve( int dir, double c ) const
{
  ...
  if ( dir == 0 )
  {
    ...
    //   when m_angle[0] != 0.
    MYON_Circle circle;
    MYON_3dPoint P = m_curve->PointAt(c);
    circle.plane.origin = m_axis.ClosestPointTo(P);
    circle.plane.zaxis = m_axis.Tangent();
    circle.plane.xaxis = P - circle.plane.origin;
    circle.radius = circle.plane.xaxis.Length();
    ...
    circle.plane.UpdateEquation();
    MYON_Arc arc( circle, m_angle );
    crv = new MYON_ArcCurve(arc, m_t[0], m_t[1]);
  }
...
bool MYON_RevSurface::Trim( int dir, const MYON_Interval& domain )
{
  bool rc = false;
  if ( dir != 0 && dir != 1 )
    return false;
  ...
  if ( dir == 0 )
  {
    ...
    double d = a.Length();
    if ( fabs(d) > MYON_ZERO_TOLERANCE && fabs(d) <= 2.0*MYON_PI+MYON_ZERO_TOLERANCE )
    {
      m_angle = a;
      m_t = domain;
      rc = true;
      ...
      )
{
  ...
  if ( dir == 0 )
  {
    ...
    a[0] = m_angle.ParameterAt(t0);
    a[1] = m_angle.ParameterAt(t1);
    if (a.Length() > 2.0*MYON_PI+MYON_ZERO_TOLERANCE) a[1] = a[0]+2.0*MYON_PI;
    m_angle = a;
    m_t = dom;
    rc = true;
  }
       ...
       ) const
{
  ...
  MYON_RevSurface* srf_ws=MYON_RevSurface::Cast(west_or_south_side);
  MYON_RevSurface* srf_en=MYON_RevSurface::Cast(east_or_north_side);
  ...
  MYON_Interval left_angle, right_angle;
  ...
  if ( rc )
  {
    ...
    const MYON_BoundingBox input_bbox(m_bbox);

    if ( !srf_ws )
      west_or_south_side = srf_ws = new MYON_RevSurface();
    ...
    if ( !srf_en )
      east_or_north_side = srf_en = new MYON_RevSurface();
...
bool MYON_RevSurface::Transpose()
{
  m_bTransposed = m_bTransposed ? false : true;
  return true;
}

bool MYON_RevSurface::IsContinuous(
...
bool MYON_RevSurface::Reverse( int dir )
{
  bool rc = false;
  if ( m_bTransposed )
    dir = dir ? 0 : 1;
  if ( dir == 0 )
  {
    ...
    double a0 = m_angle[0];
    double a1 = m_angle[1];
    m_angle.Set( 2.0*MYON_PI - a1, 2.0*MYON_PI - a0 );
    m_t.Reverse();
    rc = true;
  }
  else if ( dir == 1 && m_curve )
...
bool MYON_RevSurface::GetNextDiscontinuity( 
                int dir,
                MYON::continuity c,
                double t0,
                double t1,
...
bool MYON_RevSurface::IsSingular( int side ) const // 0 = south, 1 = east, 2 = north, 3 = west
{
  bool rc = false;
  MYON_3dPoint P, Q;
  //double d, tol;
  ...
  if ( 0 == side || 2 == side )
  {
    ...
    //   Use MYON_PointsAreCoincident() for all IsSingular queries
    //
    //d = fabs(P.x - Q.x);
    //tol = MYON_ZERO_TOLERANCE + fabs(Q.x)*MYON_SQRT_EPSILON;
    //if ( d <= tol )
    //{
    //  d = fabs(P.y - Q.y);
    //  tol = MYON_ZERO_TOLERANCE + fabs(Q.y)*MYON_SQRT_EPSILON;
    //  if ( d <= tol )
    //  {
    //    d = fabs(P.z - Q.z);
    //    tol = MYON_ZERO_TOLERANCE + fabs(Q.z)*MYON_SQRT_EPSILON;
    //    if ( d <= tol )
    //      rc = true;
    //  }
    //}
    rc = MYON_PointsAreCoincident(3,0,&P.x,&Q.x);
...
bool MYON_RevSurface::IsPeriodic( int dir ) const // dir  0 = "s", 1 = "t"
{
  bool rc = false;
  if ( m_bTransposed )
    dir = dir ? 0 : 1;
  if ( dir == 0 )
  {
    if (m_angle.Length() >= 2.0*MYON_PI-MYON_ZERO_TOLERANCE )
      rc = true;
  }
  else if ( dir == 1 && m_curve )
  {
      ...
      ) const
{
  bool rc = false;
  if( IsValid()){
    MYON_Plane AxisNormal( m_curve->PointAtStart(), m_axis.Tangent() );
...
bool MYON_RevSurface::IsClosed( int dir ) const  // dir  0 = "s", 1 = "t"
{
  bool rc = false;
  if ( m_bTransposed )
    dir = dir ? 0 : 1;
...
bool MYON_RevSurface::GetParameterTolerance( // returns tminus < tplus: parameters tminus <= s <= tplus
         ...
         ) const
{
  ...
  {
    if ( m_t.IsIncreasing() )
      rc = MYON_GetParameterTolerance( m_t[0], m_t[1], t, tminus, tplus );
  }
  else if ( dir == 1 && m_curve )
  {
...
bool MYON_RevSurface::SetDomain( 
...
MYON_Interval MYON_RevSurface::Domain( int dir ) const
{
  MYON_Interval d;
  if ( m_bTransposed )
    dir = 1-dir;
...
bool MYON_RevSurface::GetSurfaceSize( 
    double* width, 
    double* height 
    ) const
{
  ...
  if ( m_curve )
  {
    ...
    MYON_Interval cdom = m_curve->Domain();
    int i, hint = 0;
    int imax = 64;
    double d = 1.0/((double)imax);
    MYON_3dPoint pt0 = MYON_3dPoint::UnsetPoint;
    MYON_3dPoint pt;
      ...
      for ( i = 0; i <= imax; i++ )
      {
        if ( m_curve->EvPoint( cdom.ParameterAt(i*d), pt, 0, &hint ) )
        {
          ...
          if ( r > radius_estimate )
            radius_estimate = r;
          if ( pt0 != MYON_3dPoint::UnsetPoint )
...
int MYON_RevSurface::SpanCount( int dir ) const
{
  int span_count = 0;
  if ( m_bTransposed )
    dir = 1-dir;
  ...
  {
	  double a = (0.5 + MYON_SQRT_EPSILON)*MYON_PI;
...
bool MYON_RevSurface::GetSpanVector( int dir, double* s ) const
{
  bool rc = false;
  if ( m_bTransposed )
    dir = 1-dir;
...
int MYON_RevSurface::Degree( int dir ) const
{
  int span_degree = 0;
  if ( m_bTransposed )
    dir = 1-dir;
...
void MYON_RevSurface::ClearBoundingBox()
{
  m_bbox.Destroy();
}

bool MYON_RevSurface::GetBBox(    // returns true if successful
       ...
       ) const
{
  ...
  if ( !rc )
  {
    MYON_BoundingBox bbox, cbox, abox;
    ...
    if (rc)
    {
      ...
      for ( i = 0; i < corners.Count(); i++ )
      {
        ...
        while( m_axis.ClosestPointTo(P,&t) ) // not a loop - used for flow control
        {
          ...
          if ( fabs(dot) > 0.0001 )
            break; 
          arc.plane.yaxis = MYON_CrossProduct(arc.plane.zaxis,arc.plane.xaxis);
      ...
      if ( bbox.IsValid() )
      {
        MYON_RevSurface* ptr = const_cast<MYON_RevSurface*>(this);
        ptr->m_bbox = bbox;
        rc = true;
      }
    }
...
bool MYON_RevSurface::IsSpherical(MYON_Sphere* sphere, double tolerance ) const
{
  bool rc = false;
  if ( m_curve )
  {
    MYON_Plane plane;
    MYON_Arc arc;
    MYON_3dPoint P = m_curve->PointAt( m_curve->Domain().Mid() );
    plane.origin = m_axis.from;
    plane.yaxis = m_axis.Tangent();
    plane.zaxis = MYON_CrossProduct( P-plane.origin, plane.yaxis );
    plane.zaxis.Unitize();
      ...
      if ( m_curve->IsArc( &plane, &arc, tolerance ) )
      {
        ...
        if ( P.DistanceTo(arc.Center()) <= tolerance )
        {
          ...
          if ( sphere )
          {
            ...
            sphere->plane.xaxis = MYON_CrossProduct( sphere->plane.zaxis, sphere->plane.yaxis );
...
bool MYON_Surface::IsSphere( MYON_Sphere* sphere, double tolerance ) const
{
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
  const MYON_RevSurface* rs = MYON_RevSurface::Cast(this);
  if (rs)
  {
    return rs->IsSpherical(sphere,tolerance) ? true : false;
  }

  MYON_Curve* crv = IsoCurve(0,Domain(1).Mid());
  if ( !crv )
    return false;

  MYON_Arc arc0;
  int bIsArc0 = crv->IsArc(0,&arc0,tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
  delete crv;
  crv = 0;
  if ( !bIsArc0 )
    return false;
  ...
  if ( !crv )
    return false;
  MYON_Arc arc1;
  int bIsArc1 = crv->IsArc(0,&arc1,tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
  delete crv;
  crv = 0;
  if ( !bIsArc1 )
    return false;
  ...
  double tol = 0.5*MYON_SQRT_EPSILON*(arc0.radius+arc1.radius);

  MYON_3dPoint P, S;
  double a, d;
  for ( a = 0.0; a < 1.0; a += 0.25 )
  {
    P = arc0.PointAt(a*2.0*MYON_PI);
    if ( bTestSphere0 )
    {
...
bool MYON_Surface::IsCylinder( MYON_Cylinder* cylinder, double tolerance ) const
{
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
  const MYON_RevSurface* rs = MYON_RevSurface::Cast(this);
  ...
  if ( !rc && !rs )
  {
    ...
    MYON_Line line;
    int bIsLine = 0;
    int bIsArc = crv->IsArc(0,&arc,tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
    if ( !bIsArc )
    {
      bIsLine = crv->IsLinear(tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
      if ( bIsLine )
      {
    ...
    if ( !crv )
      return false;
    if ( !bIsArc )
      bIsArc = crv->IsArc(0,&arc,tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
    else if ( !bIsLine )
    {
      bIsLine = crv->IsLinear(tolerance > MYON_ZERO_TOLERANCE ? tolerance : 0.0);
      if ( bIsLine )
    ...
    double tol = 0.5*MYON_SQRT_EPSILON*(arc.radius);
    if ( tol < tolerance )
      tol = tolerance;
...
bool MYON_Surface::IsCone( MYON_Cone* cone, double tolerance ) const
{
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
  ...
  if ( fabs(r0-r1) <= MYON_ZERO_TOLERANCE )
    return false;
  double h0 = arc.plane.plane_equation.ValueAt(line.from);
  double h1 = arc.plane.plane_equation.ValueAt(line.to);
  if ( fabs(h0-h1) <= MYON_ZERO_TOLERANCE )
    return false;
  double tol = 0.5*MYON_SQRT_EPSILON*(r0+r1);

  MYON_Cone cn;
  cn.height = (h0*r1 - r0*h1)/(r1-r0);
  if ( !MYON_IsValid(cn.height) || fabs(cn.height) <= MYON_ZERO_TOLERANCE )
    return false;
  cn.plane = arc.plane;
  ...
  GetSpanVector(0,s);
  GetSpanVector(1,t);
  tol = 0.5*MYON_SQRT_EPSILON*(r0+r1);
  if ( tol < tolerance )
    tol = tolerance;
  tol *= fabs(cn.height);
  for ( int i = 0; i < sc0; i++ )
...
bool MYON_Surface::IsTorus( MYON_Torus* torus, double tolerance ) const
{
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
...
static
bool MYON__IsCylConeHelper(
          const MYON_Line& axis, 
          ...
          )
{
  ...
  if ( !line.IsValid() )
    return false;
  if ( line.Length() <= MYON_ZERO_TOLERANCE )
    return false;
  ...
  if ( h <= MYON_ZERO_TOLERANCE )
    return false;

  double a0 = MYON_UNSET_VALUE;
  if ( !axis.ClosestPointTo(line.from,&a0) )
    return false;
  double a1 = MYON_UNSET_VALUE;
  if ( !axis.ClosestPointTo(line.to,&a1) )
    return false;
  if ( !MYON_IsValid(a0) || !MYON_IsValid(a1) )
    return false;


  MYON_3dPoint A0 = axis.PointAt(a0);
  MYON_3dPoint A1 = axis.PointAt(a1);
  plane.origin = A0;
  MYON_3dVector x0 = line.from - A0;
  MYON_3dVector x1 = line.to   - A1;
  r[0] = x0.Length();
  r[1] = x1.Length();
  if ( x0*x0 < 0.0 && r[0] > MYON_ZERO_TOLERANCE && r[1] > MYON_ZERO_TOLERANCE )
    return false;
  plane.xaxis = ( r[0] >= r[1] ) ? x0 : x1;
  if (fabs(plane.xaxis.Length()) <= MYON_ZERO_TOLERANCE)
    return false;
  ...
  x0 = line.Tangent();
  if ( fabs(x0*plane.yaxis) > MYON_ZERO_TOLERANCE )
    return false;

  return (curve->IsLinear( tolerance )?true:false);
}

bool MYON_RevSurface::IsCylindrical(
      MYON_Cylinder* cylinder,
      double tolerance
      ) const
{
  MYON_Cylinder c;
  ...
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
  if ( !MYON__IsCylConeHelper(m_axis,m_curve,tolerance,c.circle.plane,line,r,h) )
    return false;
  if ( fabs(r[0]-r[1]) > tolerance )
    return false;
  if ( fabs(line.Tangent()*c.circle.plane.xaxis) > MYON_ZERO_TOLERANCE )
    return false;
...
bool MYON_RevSurface::IsConical(
      MYON_Cone* cone,
      double tolerance
      ) const
{
  ...
  if ( !MYON_IsValid(tolerance) || tolerance <= 0.0 )
    tolerance = MYON_ZERO_TOLERANCE;
  if ( !MYON__IsCylConeHelper(m_axis,m_curve,tolerance,c.plane,line,r,h) )
    return false;
  double dr = r[0]-r[1];
  if ( fabs(dr) <= MYON_ZERO_TOLERANCE )
    return false;
  if ( 0.0 == r[0] )
  {
    c.radius = r[1];
