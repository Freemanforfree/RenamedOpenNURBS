...
#if !defined(MYON_SAFE_FRAME_INC_)
#define MYON_SAFE_FRAME_INC_

class MYON_CLASS MYON_SafeFrame final
{
public:
  MYON_SafeFrame();
  MYON_SafeFrame(MYON_XMLNode& model_node);
  MYON_SafeFrame(const MYON_SafeFrame&);
  ~MYON_SafeFrame();

  const MYON_SafeFrame& operator = (const MYON_SafeFrame& sf);

  bool operator == (const MYON_SafeFrame& sf);
  bool operator != (const MYON_SafeFrame& sf);
