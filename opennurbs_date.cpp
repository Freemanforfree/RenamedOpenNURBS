...
unsigned int MYON_DaysInGregorianYear(
  unsigned int year
  )
{
  return (year >= FIRST_GREGORIAN_YEAR) ? (MYON_IsGregorianLeapYear(year) ? 366 : 365) : 0;
}
...
{
  return MYON_IsGregorianLeapYear(year) 
        ? LeapYearDayCount()
        : CommonYearDayCount();
}

unsigned int MYON_DaysInMonthOfGregorianYear(
  unsigned int year,
  unsigned int month
  )
{
...
unsigned int MYON_DayOfGregorianYear(
  unsigned int year,
  unsigned int month,
  unsigned int date
  )
...
bool MYON_GetGregorianMonthAndDayOfMonth(
  unsigned int year,
