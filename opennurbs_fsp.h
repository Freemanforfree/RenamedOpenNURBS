...
class MYON_CLASS MYON_FixedSizePoolElement
{
private:
  // MYON_FixedSizePoolElement is never instantiated 
  MYON_FixedSizePoolElement() = delete;
  ~MYON_FixedSizePoolElement() = delete;
  MYON_FixedSizePoolElement(const MYON_FixedSizePoolElement&) = delete;
  MYON_FixedSizePoolElement operator=(const MYON_FixedSizePoolElement&) = delete;

public:
  // next element - intentionally not initialized because instantiation is not permitted.
  MYON_FixedSizePoolElement* m_next;
};

class MYON_CLASS MYON_FixedSizePool
{
public:
  MYON_FixedSizePool();
  ~MYON_FixedSizePool();
  ...
  MYON_FixedSizePool& operator=(MYON_FixedSizePool&&);
#endif
  ...
  Remarks:
    You must call Create() on an unused MYON_FixedSizePool or call Destroy()
  ...
  */
  ...
  Remarks:
    You must call Create() or CreateEx() on an unused MYON_FixedSizePool or call Destroy()
    before calling create.
  Returns:
    True if successful and the pool can be used.
  */
  ...
  Returns:
    Total operating system heap memory (in bytes) used by this MYON_FixedSizePool.
  Remarks:
  ...
  */
  ...
  Remarks:
    ...
    MYON_FixedMemoryPool uses the first sizeof(void*) bytes of the
  ...
  */
  ...
  Remarks:
    It is faster to use MYON_FixedSizePoolIterator.FirstElement() and
    MYON_FixedSizePoolIterator.NextElement() to iterate through the 
  ...
  */
  ...
  Parameters:
    element_pointer - [in]
  Returns:
    An index >= 0 and < MYON_MAX_SIZE_T if the element_pointer
    points to an element managed by the this fixed size pool.
    MYON_MAX_SIZE_T otherwise.
  Remarks:
    It is faster to use MYON_FixedSizePoolIterator.FirstElement() and
    MYON_FixedSizePoolIterator.NextElement() to iterate through the 
    entire list of elements.  This function is relatively
...
private:
  friend class MYON_FixedSizePoolIterator;
...
private:
  ...
  MYON_FixedSizePool(const MYON_FixedSizePool&) = delete;
  MYON_FixedSizePool& operator=(const MYON_FixedSizePool&) = delete;
};

class MYON_CLASS MYON_FixedSizePoolIterator
{
public:
  MYON_FixedSizePoolIterator();
  MYON_FixedSizePoolIterator( const class MYON_FixedSizePool& fsp );

  const class MYON_FixedSizePool* FixedSizePool();

  void Create(const MYON_FixedSizePool* fsp);
...
private:
  const class MYON_FixedSizePool* m_fsp;
  void* m_it_block;
  void* m_it_element;
};


template <class T> class MYON_SimpleFixedSizePool : private MYON_FixedSizePool
{
public:
  // construction ////////////////////////////////////////////////////////

  MYON_SimpleFixedSizePool();
  ~MYON_SimpleFixedSizePool();
...
private:
  // prohibit copy construction and operator=.
  MYON_SimpleFixedSizePool(const MYON_SimpleFixedSizePool<T>&);
  MYON_SimpleFixedSizePool<T>& operator=(const MYON_SimpleFixedSizePool<T>&);
};

template <class T> class MYON_SimpleFixedSizePoolIterator : private MYON_FixedSizePoolIterator
{
public:
  MYON_SimpleFixedSizePoolIterator( const class MYON_SimpleFixedSizePool<T>& fsp );
  MYON_SimpleFixedSizePoolIterator(const class MYON_SimpleFixedSizePoolIterator<T>&);
...
private:
  ...
  class MYON_SimpleFixedSizePoolIterator<T>& operator=(const class MYON_SimpleFixedSizePoolIterator<T>&);
};
