        ...
        )
{
  ...
  MYON_Workspace ws;
  FILE* fp = ws.OpenFile( sFileName, L"rb" ); // file automatically closed by ~MYON_Workspace()
  if ( !fp ) {
...
/*
Returns:
  True if .3dm file was successfully read into an MYONX_Model class.
*/
  ...
  )
{
  ...
  FILE* archive_fp = MYON::OpenFile( sFileName, L"rb");
  if ( !archive_fp ) 
  {
  ...
  )
{
  ...
  if ( directory_depth <= maximum_directory_depth )
  {
        ...
        )
    {
      ...
      MYON_wString full_path(file_it.CurrentItemFullPathName());
      if ( full_path.IsEmpty() )
        continue;
      
      if ( !MYON::IsOpenNURBSFile(full_path) )
        continue;
    ...
    if ( bFoundDirectory && directory_depth < maximum_directory_depth )
    {
          ...
          )
      {
        ...
        MYON_wString full_path(dir_it.CurrentItemFullPathName());
        if ( full_path.IsEmpty() )
          continue;
...
int main( int argc, const char *argv[] )
{
  ...
  for ( argi = 1; argi < argc; argi++ ) 
  {
         ...
         && arg[5] )
    {
      ...
      const MYON_wString sDumpFilename = MYON_FileSystemPath::ExpandUser(arg + 5);
      FILE* text_fp = MYON::OpenFile(sDumpFilename,L"w");
      if ( text_fp )
      {
        dump_fp = text_fp;
    ...
    MYON_wString ws_arg = MYON_FileSystemPath::ExpandUser(arg);
    const wchar_t* wchar_arg = ws_arg;

    if ( MYON::IsDirectory(wchar_arg) )
    {
