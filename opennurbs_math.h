...
#if !defined(MYON_MATH_INC_)
#define MYON_MATH_INC_
...
*/
class MYON_CLASS MYON_Sum
{
public:

  /*
  Description:
    Calls MYON_Sum::Begin(x)
  */
  void operator=(double x);

  /*
  Description:
    Calls MYON_Sum::Plus(x);
  */
  void operator+=(double x);
...
Description:
  ...
  and values.  MYON_Evaluator is used to pass functions to
  local solvers.
*/
class MYON_CLASS MYON_Evaluator
{
public:
  ...
  */
  ...
  MYON_SimpleArray<MYON_Interval> m_domain;
  ...
  MYON_SimpleArray<bool> m_bPeriodicParameter;

private:
  MYON_Evaluator(); // prohibit default constructor
  MYON_Evaluator& operator=(const MYON_Evaluator&); // prohibit operator= (can't copy const members)
};
...
/*
Returns:
  (x > 0.0 && x < MYON_UNSET_POSITIVE_VALUE);
*/
MYON_DECL
...
/*
class MYON_CLASS MYON_TimeLimit
{
  MYON_TimeLimit();
  MYON_TimeLimit(MYON__UINT64 time_limit_seconds);
  void SetTimeLimit(MYON__UINT64 time_limit_seconds);
  bool Continue() const;
  bool IsSet() const;
private:
  MYON__UINT64 m_time_limit[2];
};
*/

// The MYON_IS_FINITE and MYON_IS_VALID defines are much faster
// than calling MYON_IsValid(), but need to be used when
// the macro expansion works.

#if   defined(MYON_LITTLE_ENDIAN)

// works on little endian CPUs with IEEE doubles
#define MYON_IS_FINITE(x) (0x7FF0 != (*((unsigned short*)(&x) + 3) & 0x7FF0))
#define MYON_IS_INFINITE(x) (0x7FF0 == (*((unsigned short*)(&x) + 3) & 0x7FF0))

#elif defined(MYON_BIG_ENDIAN)

// works on big endian CPUs with IEEE doubles
#define MYON_IS_FINITE(x) (0x7FF0 != (*((unsigned short*)(&x)) & 0x7FF0))
#define MYON_IS_INFINITE(x) (0x7FF0 == (*((unsigned short*)(&x)) & 0x7FF0))
...
//#define MYON_IS_FINITE(x) (finite(x)?true:false)
#define MYON_IS_FINITE(x) (isfinite(x)?true:false)
#else
#define MYON_IS_FINITE(x) (_finite(x)?true:false)
#endif

#endif

#define MYON_IS_FINITE_FLOAT(x) ((x) <= 3.402823466e+38F && (x) >= -3.402823466e+38F)
#define MYON_IS_INFINITE_FLOAT(x) ((x) > 3.402823466e+38F || (x) < -3.402823466e+38F)

#define MYON_IS_VALID(x)  ((x) > MYON_UNSET_VALUE && (x) < MYON_UNSET_POSITIVE_VALUE)
#define MYON_IS_VALID_FLOAT(x)  ((x) > MYON_UNSET_FLOAT && (x) < MYON_UNSET_POSITIVE_FLOAT)
#define MYON_IS_UNSET_DOUBLE(x) (MYON_UNSET_VALUE == (x) || MYON_UNSET_POSITIVE_VALUE == (x))
#define MYON_IS_UNSET_FLOAT(x) (MYON_UNSET_FLOAT == (x) || MYON_UNSET_POSITIVE_FLOAT == (x))
#define MYON_IS_NAN(x) (!((x)==(x)))

MYON_DECL
float MYON_ArrayDotProduct( // returns AoB
...
MYON_DECL
double MYON_ArrayDotProduct( // returns AoB
          int,           // size of arrays (can be zero)
          const double*, // A[]
          const double*  // B[]
          );
...
MYON_DECL
double MYON_ArrayDotDifference( // returns A o ( B - C )
          int,           // size of arrays (can be zero)
          const double*, // A[]
          const double*, // B[]
          const double*  // C[]
...
MYON_DECL
double MYON_ArrayMagnitude( // returns sqrt(AoA)
          int,           // size of arrays (can be zero)
          const double*  // A[]
          );

MYON_DECL
double MYON_ArrayMagnitudeSquared( // returns AoA
          int,           // size of arrays (can be zero)
          const double*  // A[]
          );

MYON_DECL
double MYON_ArrayDistance( // returns sqrt((A-B)o(A-B))
          int,           // size of arrays (can be zero)
          const double*, // A[]
          const double*  // B[]
          );
...
MYON_DECL
double MYON_ArrayDistanceSquared( // returns (A-B)o(A-B)
          int,           // size of arrays (can be zero)
          const double*, // A[]
          const double*  // B[]
          );
...
MYON_DECL
int    MYON_SearchMonotoneArray( // find a value in an increasing array
          // returns  -1: t < array[0]
...
*/
MYON_DECL
double MYON_BinomialCoefficient( 
          int i,
          int j
          );
...
Remarks:
  The trinomial coefficient is computed using the formula
          ...
          i! j! k!      i! (j+k)!      j! k!
                      ...
                      = MYON_BinomialCoefficient(i,j+k)*MYON_BinomialCoefficient(j,k)
...
MYON_DECL
bool MYON_GetParameterTolerance(
...
MYON_DECL
bool MYON_SwapPointListCoordinates( 
        int, // count
        int, // stride
        float*,
...
MYON_DECL
bool MYON_SwapPointGridCoordinates(
        int, int, // point_count0, point_count1,
        int, int, // point_stride0, point_stride1,
        double*,
        int, int // coordinates to swap
...
/*
Description
  See MYON_PointsAreCoincident() for a description of when opennurbs
...
MYON_DECL
int MYON_ComparePointList( // returns 
                              // -1: first < second
                              //  0: first == second
                              // +1: first > second
          int,           // dim (>=0)
...
MYON_DECL
int MYON_SolveQuadraticEquation( // solve a*X^2 + b*X + c = 0
...
*/
MYON_DECL
int MYON_SolveTriDiagonal( // solve TriDiagMatrix( a,b,c )*X = d
...
Returns:
    ...
    If MYON_Solve3x3() is successful (returns 3), then
...
Returns:
    ...
    If MYON_Solve4x4() is successful (returns 4), then
    the solution is returned in 
...
bool MYON_EvNormal(
        ...
        const MYON_3dVector&, const MYON_3dVector&, // first partials (Du,Dv)
        const MYON_3dVector&, const MYON_3dVector&, const MYON_3dVector&, // optional second partials (Duu, Duv, Dvv)
        MYON_3dVector& // unit normal returned here
        );

// returns false if the returned tangent is zero
MYON_DECL
bool MYON_EvTangent(
        const MYON_3dVector&, // first derivative
        const MYON_3dVector&, // second derivative
        MYON_3dVector&        // Unit tangent returned here
        );

// returns false if first derivative is zero
MYON_DECL
bool MYON_EvCurvature(
        const MYON_3dVector&, // first derivative
        const MYON_3dVector&, // second derivative
        MYON_3dVector&,       // Unit tangent returned here
        MYON_3dVector&        // Curvature returned here
        );
...
bool MYON_EvPrincipalCurvatures( 
        ...
        const MYON_3dVector&, // Dst,
        const MYON_3dVector&, // Dtt,
        const MYON_3dVector&, // N,   // unit normal to surface (use MYON_EvNormal())
...
MYON_DECL
MYON_3dVector MYON_NormalCurvature( 
        const MYON_3dVector&, // surface 1rst partial (Ds)
        const MYON_3dVector&, // surface 1rst partial (Dt)
        const MYON_3dVector&, // surface 1rst partial (Dss)
        const MYON_3dVector&, // surface 1rst partial (Dst)
        const MYON_3dVector&, // surface 1rst partial (Dtt)
        const MYON_3dVector&, // surface unit normal
        const MYON_3dVector&  // unit tangent direction
        );
...
Parameters:
  ...
  zero_curvature - [in] (ignored if < 2^-110 = 7.7037197787136e-34)
    ...
    When in doubt, use MYON_ZERO_CURVATURE_TOLERANCE.
  radius_tolerance - [in]
  ...
  relative_tolerance - [in]
    ...
    When in doubt, use MYON_RELATIVE_CURVATURE_TOLERANCE, which
    is currently 0.05.
Returns:
...
Description:
  ...
  when the continuity parameter is MYON::continuity::G2_continuous.
Parameters:
  Km - [in]
...
Description:
  This function is used to test curvature continuity
  in IsContinuous and GetNextDiscontinuity functions
  when the continuity parameter is MYON::continuity::Gsmooth_continuous.
Parameters:
  Km - [in]
    Curve's vector curvature evaluated from below
  Kp - [in]
...
Parameters:
  c - [in] type of continuity to test for. Read MYON::continuity
...
MYON_DECL
const MYON_2dex* MYON_BinarySearch2dexArray( 
          int key_i, 
          const MYON_2dex* base, 
          size_t nel
          );

MYON_DECL
const MYON_2udex* MYON_BinarySearch2udexArray(
  unsigned int key_i,
  const MYON_2udex* base,
  size_t nel
);
...
bool MYON_Intersect( 
        const MYON_Plane&, 
        const MYON_Plane&, 
        MYON_Line& // intersection line is returned here
        );
...
bool MYON_Intersect( 
        ...
        const MYON_Plane&, 
        const MYON_Plane&,
        MYON_3dPoint& // intersection point is returned here
        );
...
int MYON_Intersect(
        const MYON_Plane&,
        const MYON_Sphere&,
        MYON_Circle&  // intersection circle is returned here
        );
...
int MYON_Intersect(                  
        const MYON_Line&, 
        const MYON_Sphere&,
        MYON_3dPoint&, // [out] first intersection point
        MYON_3dPoint&  // [out] second intersection point
        );
...
int MYON_Intersect( 
                  ...
                  double* line_t0,
                  MYON_3dPoint& circle_point0,
                  double* line_t1,
                  MYON_3dPoint& circle_point1
                  );
...
int MYON_Intersect( 
                  ...
                  const MYON_Arc& arc,
                  double* line_t0,
                  MYON_3dPoint& arc_point0,
                  double* line_t1,
                  MYON_3dPoint& arc_point1
                  );
...
MYON_DECL
int MYON_Intersect(
  const MYON_Circle& c0,
  const MYON_Circle& c1,
  MYON_3dPoint& point0,
  MYON_3dPoint& point1
);
...
*/
MYON_DECL
bool MYON_EvaluateQuotientRule( 
          int dim, 
          int der_count,
          int v_stride, 
          double *v 
...
*/
MYON_DECL
bool MYON_EvaluateQuotientRule2( 
          int dim, 
          int der_count, 
          int v_stride, 
          double *v 
...
*/
MYON_DECL
bool MYON_EvaluateQuotientRule3( 
          int dim, 
          int der_count, 
          int v_stride,
          double *v 
...
MYON_DECL double MYON_Max(double a, double b);

// If performance is important, then
// you are better off using ((b<a)?a:b)
MYON_DECL float MYON_Max(float a, float b);

// If performance is important, then
// you are better off using ((b<a)?a:b)
MYON_DECL int MYON_Max(int a, int b);

// If performance is important, then
// you are better off using ((a<b)?a:b)
MYON_DECL double MYON_Min(double a, double b);

// If performance is important, then
// you are better off using ((a<b)?a:b)
MYON_DECL float MYON_Min(float a, float b);

// If performance is important, then
// you are better off using ((a<b)?a:b)
MYON_DECL int MYON_Min(int a, int b);

// Do not call MYON_Round() in any opennurbs code, tl code
...
MYON_DECL int MYON_Round(double x);
...
Returns:
  (1.0-t)*x + t*y
*/
MYON_DECL double MYON_LinearInterpolation(
  double t, 
  double x, 
  double y
  );
...
*/
MYON_DECL bool MYON_GetConicEquationThrough6Points( 
        int stride, 
...
Returns:
  ...
  are not changed.
*/
MYON_DECL bool MYON_IsConicEquationAnEllipse( 
...
Returns:
  ...
  are not changed.
*/
MYON_DECL bool MYON_GetEllipseConicEquation( 
      double a, double b, 
      double x0, double y0, 
      double alpha,
      double conic[6]
...
*/
MYON_DECL double MYON_Length2d( double x, double y );
...
Returns:
 (x^2 + y^2)
*/
MYON_DECL double MYON_Length2dSquared(double x, double y);
...
*/
MYON_DECL double MYON_Length3d( double x, double y, double z );
...
Returns:
 (x^2 + y^2 + z^2)
*/
MYON_DECL double MYON_Length3dSquared(double x, double y, double z);
...
*/
MYON_DECL float MYON_FloatFloor(double x);
...
*/
MYON_DECL float MYON_FloatCeil(double x);
...
*/
MYON_DECL bool MYON_IsConvexPolyline(
  size_t point_dim,
  size_t point_count,
  const double* points,
  size_t point_stride,
...
/// </returns>
MYON_DECL unsigned MYON_GreatestCommonDivisor(
  unsigned a,
  unsigned b
);
...
/// <param name="b"></param>
/// <returns>
/// If a $gt; 0 and b &gt; and the least common multiple of a and b &lt;= MYON_UINT_MAX, then the
/// least common multiple of a and b is returned.
/// Otherwise 0 is returned.
/// </returns>
MYON_DECL unsigned MYON_LeastCommonMultiple(
  unsigned a,
  unsigned b
);
