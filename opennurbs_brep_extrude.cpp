...
static 
void MYON_BrepExtrudeHelper_ReserveSpace( 
          MYON_Brep& brep, 
...
static
MYON_SumSurface* MYON_BrepExtrudeHelper_MakeSumSrf( const MYON_Curve& path_curve,
                                                 const MYON_BrepEdge& base_edge, bool bRev )
{
  MYON_SumSurface* sum_srf = 0;
  // create side surface
  if ( base_edge.ProxyCurve() )
  {
    MYON_Curve* srf_path_curve = path_curve.DuplicateCurve();
    MYON_Curve* srf_base_curve = base_edge.DuplicateCurve();
    if ( !bRev )
      srf_base_curve->Reverse();
    MYON_3dPoint sum_basepoint = -MYON_3dVector(srf_path_curve->PointAtStart());
    sum_srf = new MYON_SumSurface();
...
static
MYON_NurbsSurface* MYON_BrepExtrudeHelper_MakeConeSrf( const MYON_3dPoint& apex_point,
                                                 const MYON_BrepEdge& edge, bool bRev )
{
  ...
  MYON_NurbsSurface* cone_srf = new MYON_NurbsSurface();
...
static
bool MYON_BrepExtrudeHelper_MakeSides(
          MYON_Brep& brep,
          int loop_index,
          const MYON_Curve& path_curve,
          bool bCap,
          MYON_SimpleArray<int>& side_face_index
          )
{
  ...
  MYON_BrepExtrudeHelper_ReserveSpace( brep, loop_trim_count, bCap?1:0 );
  ...
  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
    ...
    // get side surface for new face
    {
      ...
      {
        const MYON_BrepEdge& base_edge = brep.m_E[trim.m_ei];
        ...
        sum_srf = MYON_BrepExtrudeHelper_MakeSumSrf( path_curve, base_edge, trim.m_bRev3d );
      }
    }
    if ( !sum_srf )
      continue;
    ...
    {
      const MYON_BrepTrim& prev_west_trim = brep.m_T[ brep.m_L[ brep.m_F[prev_face_index].m_li[0]].m_ti[3] ];
    ...
    {
      const MYON_BrepTrim& first_face_east_trim = brep.m_T[first_face_east_trim_index];
      vid[3] = first_face_east_trim.m_vi[1];
      eid[3] = first_face_east_trim.m_ei;
      bRev3d[3] = (first_face_east_trim.m_bRev3d?false:true);
    }
    const MYON_BrepFace* side_face = brep.NewFace(sum_srf,vid,eid,bRev3d);
    if ( side_face )
    {
...
static
bool MYON_BrepExtrudeHelper_CheckPathCurve( const MYON_Curve& path_curve, MYON_3dVector& path_vector )
{
  MYON_Line path_line;
...
static 
bool MYON_BrepExtrudeHelper_MakeTopLoop( 
          MYON_Brep& brep, 
          MYON_BrepFace& top_face,
          int bottom_loop_index,
          const MYON_3dVector path_vector,
          ...
          )
{
  ...
  if ( bottom_loop_index < 0 || bottom_loop_index >= brep.m_L.Count() )
    return false;
  MYON_BrepLoop::TYPE loop_type = brep.m_L[bottom_loop_index].m_type;
  if ( loop_type != MYON_BrepLoop::inner )
    loop_type = MYON_BrepLoop::outer;
  MYON_BrepLoop& top_loop = brep.NewLoop( loop_type, top_face );
  const MYON_BrepLoop& bottom_loop = brep.m_L[bottom_loop_index];
  ...
  MYON_SimpleArray<int> top_vertex_index(loop_trim_count);
  MYON_SimpleArray<int> top_edge_index(loop_trim_count);
  MYON_SimpleArray<bool> top_trim_bRev3d(loop_trim_count);
  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
  ...
  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
    ...
    {
      const MYON_BrepFace& side_face = brep.m_F[side_face_index[lti]];
      const MYON_BrepLoop& side_loop = brep.m_L[side_face.m_li[0]];
      const MYON_BrepTrim& side_north_trim = brep.m_T[side_loop.m_ti[2]];
    ...
    else 
    {
           ...
           )
      {
        ...
        {
          const MYON_BrepTrim& bottom_trim_prev = brep.m_T[bottom_ti_prev];
          const MYON_BrepTrim& bottom_trim = brep.m_T[bottom_ti];
          const MYON_BrepTrim& bottom_trim_next = brep.m_T[bottom_ti_next];
          if (    MYON_BrepTrim::seam == bottom_trim_prev.m_type
               && MYON_BrepTrim::singular == bottom_trim.m_type
               && MYON_BrepTrim::seam == bottom_trim_next.m_type 
               ...
               )
          {
            ...
            {
              MYON_BrepVertex& top_vertex = brep.NewVertex(brep.m_V[vi].point+path_vector,0.0);
    ...
    while( bKeepChecking )
    {
      ...
      for ( lti = 0; lti < loop_trim_count; lti++ )
      {
        if ( top_vertex_index[lti] == -1 )
        {
          ...
          {
            if ( MYON_BrepTrim::singular != brep.m_T[bottom_loop.m_ti[i-1]].m_type )
              break;
  ...
  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
    ...
    if ( bottom_ti < 0 || bottom_ti >= brep.m_T.Count() )
      continue;
    const MYON_BrepTrim& bottom_trim = brep.m_T[bottom_ti];
    if ( MYON_BrepTrim::seam != bottom_trim.m_type )
      continue;
    ...
    const MYON_BrepEdge& bottom_edge = brep.m_E[bottom_trim.m_ei];
    MYON_Curve* top_c3 = bottom_edge.DuplicateCurve();
    if ( 0 == top_c3 )
      continue;
    // move new edge curve to top location
    top_c3->Translate(path_vector);
    MYON_3dPoint P0 = top_c3->PointAtStart();
    MYON_3dPoint P1 = top_c3->PointAtEnd();
    ...
    if ( bottom_trim.m_bRev3d )
    {
      ...
      MYON_3dPoint tmp_P = P0; P0 = P1; P1 = tmp_P;
    }
    if ( e_vi0 < 0 )
    {
    ...
    MYON_BrepEdge& top_edge = brep.NewEdge(brep.m_V[e_vi0],brep.m_V[e_vi1],top_c3i);
    ...
    for( mate_lti = lti+1; mate_lti < loop_trim_count; mate_lti++ )
    {
      ...
      if ( bottom_mate_ti < 0 || bottom_mate_ti >= brep.m_T.Count() )
        continue;
      const MYON_BrepTrim& bottom_mate_trim = brep.m_T[bottom_mate_ti];
      if ( bottom_mate_trim.m_type != MYON_BrepTrim::seam )
        continue;
  ...
  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
    const MYON_BrepTrim& bottom_trim = brep.m_T[ bottom_loop.m_ti[lti] ];
    MYON_Curve* top_c2 = bottom_trim.DuplicateCurve();
    int top_c2i = (0!=top_c2) ? brep.AddTrimCurve(top_c2) : bottom_trim.m_c2i;
    top_trim_index = -1;
    if ( bottom_trim.m_type == MYON_BrepTrim::singular && top_vertex_index[lti] >= 0 )
    {
      top_trim_index = brep.NewSingularTrim(brep.m_V[top_vertex_index[lti]], top_loop, bottom_trim.m_iso, top_c2i ).m_trim_index;
    }
    else if ( bottom_trim.m_type != MYON_BrepTrim::singular && top_edge_index[lti] >= 0 && top_edge_index[lti] < brep.m_E.Count() )
    {
      MYON_BrepEdge& top_edge = brep.m_E[top_edge_index[lti]];
      top_trim_index = brep.NewTrim( top_edge, top_trim_bRev3d[lti], top_loop, top_c2i ).m_trim_index;
    }
    else
    {
      MYON_ERROR("MYON_BrepExtrudeHelper_MakeTopLoop ran into capping trouble.");
      rc = false;
      break;
    }
    MYON_BrepTrim& top_trim = brep.m_T[top_trim_index];
...
static          
bool MYON_BrepExtrudeHelper_CheckLoop( const MYON_Brep& brep, int loop_index )
{
  bool rc = false;
  if ( loop_index >= 0 )
  {
    MYON_BrepLoop::TYPE loop_type = brep.m_L[loop_index].m_type;
    if ( loop_type == MYON_BrepLoop::inner || loop_type == MYON_BrepLoop::outer )
      rc = true;
  }
  return rc;
}
...
static
bool MYON_BrepExtrudeHelper_MakeCap(
          MYON_Brep& brep,
          int bottom_loop_index,
          const MYON_3dVector path_vector,
          ...
          )
{
  bool bCap = true;
  // make cap
  if ( !MYON_BrepExtrudeHelper_CheckLoop( brep, bottom_loop_index ) )
    return false;
  brep.m_F.Reserve(brep.m_F.Count() + 1);
  brep.m_L.Reserve(brep.m_L.Count() + 1);
  const MYON_BrepLoop& bottom_loop = brep.m_L[bottom_loop_index];
  const MYON_BrepFace& bottom_face = brep.m_F[bottom_loop.m_fi];
  const MYON_Surface* bottom_surface = bottom_face.SurfaceOf();
  MYON_Surface* top_surface = bottom_surface->Duplicate();
  top_surface->Translate( path_vector );
  int top_surface_index = brep.AddSurface( top_surface );
  MYON_BrepFace& top_face = brep.NewFace( top_surface_index );

  bCap = MYON_BrepExtrudeHelper_MakeTopLoop( brep, top_face, bottom_loop_index, path_vector, side_face_index );
  if ( bCap )
  {
    MYON_BrepLoop& top_loop = brep.m_L[brep.m_L.Count()-1];
    if ( bottom_loop.m_type == MYON_BrepLoop::inner )
    {
      // we capped an inner boundary
      // top_loop.m_type = MYON_BrepLoop::outer; // done in MYON_BrepExtrudeHelper_MakeTopLoop
      brep.FlipLoop(top_loop);
    }
    else if ( bottom_loop.m_type == MYON_BrepLoop::outer )
    {
          ...
          )
{
  ...
  {
    rc = MYON_BrepExtrudeLoop( brep, brep.m_F[face_index].m_li[0], path_curve, bCap );
  }
  else
  {
    MYON_3dVector path_vector;
    MYON_SimpleArray<int> side_face_index;
    MYON_SimpleArray<int> side_face_index_loop_mark;
    int li, fli;

    if ( !MYON_BrepExtrudeHelper_CheckPathCurve( path_curve, path_vector ) )
      return 0;
    ...
    for ( fli = 0; fli < face_loop_count && rc; fli++ )
    {
      ...
      if ( !MYON_BrepExtrudeHelper_CheckLoop( brep, li ) )
        continue;
      MYON_BrepLoop& loop = brep.m_L[li];
      if ( bCap && loop.m_type == MYON_BrepLoop::outer )
      {
      ...
      rc = MYON_BrepExtrudeHelper_MakeSides( brep, li, path_curve, bCap, side_face_index );
    }

    if ( bCap && rc && outer_loop_index >= 0 )
    {
      const int face_count1 = brep.m_F.Count();
      bCap = MYON_BrepExtrudeHelper_MakeCap( 
                  brep, 
                  outer_loop_index, 
                  path_vector, 
      ...
      if ( bCap && brep.m_F.Count() > face_count1)
      {
        ...
        MYON_BrepFace& cap_face = brep.m_F[brep.m_F.Count()-1];
        for ( fli = 0; fli < face_loop_count && rc; fli++ )
        {
          li = face.m_li[fli];
          ...
          if ( !MYON_BrepExtrudeHelper_CheckLoop( brep, li ) )
            continue;
          if ( MYON_BrepExtrudeHelper_MakeTopLoop( 
                      brep, 
                      cap_face, 
                      li, 
                      path_vector,
...
{
  MYON_SimpleArray<int> side_face_index; // index of new face above brep.m_L[loop_index].m_ti[lti]
  MYON_3dVector path_vector;
  ...
  if ( brep.m_L[loop_index].m_type != MYON_BrepLoop::outer && brep.m_L[loop_index].m_type != MYON_BrepLoop::inner )
    bCap = false;

  // make sides
  if ( !MYON_BrepExtrudeHelper_MakeSides( brep, loop_index, path_curve, bCap, side_face_index ) )
    return false;

  // make cap
  if ( bCap )
    bCap = MYON_BrepExtrudeHelper_MakeCap( brep, loop_index, path_vector, side_face_index.Array() );

  const MYON_BrepLoop& loop = brep.m_L[loop_index];
          ...
          )
{
  ...
  // make sides
  bool bRev = false;
  MYON_SumSurface* sum_srf = MYON_BrepExtrudeHelper_MakeSumSrf( 
          ...
          )
{
  ...
  const MYON_3dPoint PathStart = path_curve.PointAtStart();
  MYON_3dPoint P = path_curve.PointAtEnd();
  if ( !PathStart.IsValid() || !P.IsValid() )
    return false;
  const MYON_3dVector height = P - PathStart;
  if ( !height.IsValid() || height.Length() <= MYON_ZERO_TOLERANCE )
    return false;

  MYON_Xform tr(MYON_Xform::TranslationTransformation(height));
  ...
  {
    const MYON_BrepEdge& e = brep.m_E[ei];
    if ( 1 == e.m_ti.Count() )
    {
      side_count++;
      bSideEdge[ei] = true;
  ...
  if ( bCap )
  {
    ...
    {
      const MYON_BrepVertex& bottomv = brep.m_V[vi];
      MYON_BrepVertex& topv = brep.NewVertex(bottomv.point+height,bottomv.m_tolerance);
  ...
  else
  {
    for ( ei = 0; ei < ecount0; ei++ )
    {
      if ( bSideEdge[ei] )
      {
        const MYON_BrepEdge& bottome = brep.m_E[ei];
        ...
        {
          const MYON_BrepVertex& bottomv = brep.m_V[bottomvi0];
          MYON_BrepVertex& topv = brep.NewVertex(bottomv.point+height,bottomv.m_tolerance);
          topvimap[bottomvi0] = topv.m_vertex_index;
        }
        if ( !topvimap[bottomvi1] )
        {
          const MYON_BrepVertex& bottomv = brep.m_V[bottomvi1];
          MYON_BrepVertex& topv = brep.NewVertex(bottomv.point+height,bottomv.m_tolerance);
  ...
  if ( bOK ) for ( ei = 0; ei < ecount0; ei++ )
  {
    ...
    {
      const MYON_BrepEdge& bottome = brep.m_E[ei];
      MYON_BrepVertex& topv0 = brep.m_V[topvimap[bottome.m_vi[0]]];
      MYON_BrepVertex& topv1 = brep.m_V[topvimap[bottome.m_vi[1]]];
      MYON_Curve* c3 = bottome.DuplicateCurve();
      if ( !c3 )
      {
        bOK = false;
        break;
      ...
      MYON_BrepEdge& tope = brep.NewEdge(topv0,topv1,c3i,0,bottome.m_tolerance);
  ...
  {
    MYON_BrepVertex& bottomv = brep.m_V[vi];
    for ( int vei = 0; vei < bottomv.m_ei.Count(); vei++ )
    {
      if ( bSideEdge[bottomv.m_ei[vei]] && topvimap[vi] )
      {
        MYON_BrepVertex& topv = brep.m_V[topvimap[vi]];
        MYON_Curve* c3 = path_curve.DuplicateCurve();
        if ( !c3 )
        {
          bOK = false;
        }
        ...
        {
          MYON_3dVector D = bottomv.point - PathStart;
          c3->Translate(D);
          int c3i = brep.AddEdgeCurve(c3);
          const MYON_BrepEdge& e = brep.NewEdge(bottomv,topv,c3i,0,0.0);
  ...
  if ( bOK && bCap )
  {
    ...
    {
      const MYON_BrepFace& bottomf = brep.m_F[fi];
      MYON_Surface* srf = bottomf.DuplicateSurface();
      if ( !srf )
      {
        bOK = false;
        break;
      ...
      MYON_BrepFace& topf = brep.NewFace(si);
      ...
      {
        const MYON_BrepLoop& bottoml = brep.m_L[bottomf.m_li[fli]];
        MYON_BrepLoop& topl = brep.NewLoop(bottoml.m_type,topf);
        const int loop_trim_count = bottoml.m_ti.Count();
        topl.m_ti.Reserve(loop_trim_count);
        for ( int lti = 0; lti < loop_trim_count; lti++ )
        {
          const MYON_BrepTrim& bottomt = brep.m_T[bottoml.m_ti[lti]];
          MYON_NurbsCurve* c2 = MYON_NurbsCurve::New();
          ...
          MYON_BrepTrim* topt = 0;
          if ( bottomt.m_ei >= 0 )
          {
            MYON_BrepEdge& tope = brep.m_E[topeimap[bottomt.m_ei]];
            topt = &brep.NewTrim(tope,bottomt.m_bRev3d,topl,c2i);
          }
          else
          {
            // singular trim
            MYON_BrepVertex& topv = brep.m_V[topvimap[bottomt.m_vi[0]]];
  ...
  if( bOK ) for ( ei = 0; ei < ecount0; ei++ )
  {
    ...
    {
      MYON_BrepEdge& bottome = brep.m_E[ei];
      MYON_BrepEdge& tope = brep.m_E[topeimap[ei]];
      ...
      {
        MYON_BrepEdge& leftedge = brep.m_E[sideveimap[vid[0]]];
        MYON_BrepEdge& rightedge = brep.m_E[sideveimap[vid[1]]];
        MYON_Curve* cx = bottome.DuplicateCurve();
        if ( !cx )
        {
          bOK = false;
          break;
        }
        MYON_Curve* cy = leftedge.DuplicateCurve();
        if ( !cy )
        {
          delete cx;
          bOK = false;
        ...
        MYON_SumSurface* srf = new MYON_SumSurface();
        srf->m_curve[0] = cx;
        srf->m_curve[1] = cy;
        ...
        MYON_BrepFace* face = brep.NewFace(srf,vid,eid,bRev3d);
        if ( !face )
        {
          bOK = false;
          break;
        ...
        {
          const MYON_BrepTrim& trim0 = brep.m_T[bottome.m_ti[0]];
          const MYON_BrepTrim& trim1 = brep.m_T[bottome.m_ti[1]];
          const MYON_BrepLoop& loop0 = brep.m_L[trim0.m_li];
          const MYON_BrepLoop& loop1 = brep.m_L[trim1.m_li];
          ...
          )
{
  ...
  if ( !MYON_BrepExtrudeHelper_CheckPathCurve(path_curve,path_vector) )
    return false;
  MYON_Curve* c3 = path_curve.Duplicate();
  brep.m_V.Reserve( brep.m_V.Count() + 1 );
  MYON_BrepVertex& v0 = brep.m_V[vertex_index];
  MYON_BrepVertex& v1 = brep.NewVertex( v0.point + path_vector, 0.0 );
          ...
          )
{
  ...
  if ( brep.m_F[face_index].m_li.Count() == 1 )
  {
    rc = MYON_BrepConeLoop( brep, brep.m_F[face_index].m_li[0], apex_point );
  }
  else
  {
    int li, fli;
    ...
    if ( new_side_trim_count == 0 )
      return false;
    MYON_BrepExtrudeHelper_ReserveSpace( brep, new_side_trim_count, 0 );

    const MYON_BrepFace& face = brep.m_F[face_index];

    //MYON_BrepVertex& apex_vertex = 
    ...
    for ( fli = 0; fli < face_loop_count && rc; fli++ )
    {
      ...
      rc = MYON_BrepConeLoop( brep, li, apex_point );
    }
  }
          ...
          )
{
  ...
  MYON_BrepExtrudeHelper_ReserveSpace( brep, loop_trim_count, 0 );

  int prev_face_index = -1;
  int first_face_east_trim_index = -1;

  MYON_BrepVertex& apex_vertex = brep.NewVertex( apex_point, 0.0 );

  for ( lti = 0; lti < loop_trim_count; lti++ )
  {
    MYON_NurbsSurface* cone_srf = 0;
    ti = brep.m_L[loop_index].m_ti[lti];
    if ( ti < 0 || ti >= trim_count0 )
      continue;
    ...
    // get side surface for new face
    {
      ...
      if ( trim.m_ei >= 0 &&  trim.m_ei < edge_count0 )
      {
        ...
        eid[0] = base_edge.m_edge_index;
        bRev3d[0] = (trim.m_bRev3d?false:true);
        cone_srf = MYON_BrepExtrudeHelper_MakeConeSrf( apex_point, base_edge, bRev3d[0] );
      }
    }
    if ( !cone_srf )
      continue;
    ...
    const MYON_BrepFace* side_face = brep.NewFace(cone_srf,vid,eid,bRev3d);
    if ( side_face )
    {
      prev_face_index = side_face->m_face_index;
      if ( first_face_east_trim_index < 0 )
          ...
          )
{
  ...
  // make sides
  MYON_NurbsSurface* cone_srf = MYON_BrepExtrudeHelper_MakeConeSrf( 
