...
class MYON_CLASS MYON_Hash32TableItem
{
public:
  MYON_Hash32TableItem() = default;
  ~MYON_Hash32TableItem() = default;
  MYON_Hash32TableItem(const MYON_Hash32TableItem&) = default;
  MYON_Hash32TableItem& operator=(const MYON_Hash32TableItem&) = default;

public:
  MYON__UINT32 HashTableSerialNumber() const;

  static MYON__UINT32 Hash32FromSHA1Hash(
    const class MYON_SHA1_Hash& sha1_hash
    );

  static MYON__UINT32 Hash32FromId(
    const MYON_UUID& id
    );

  /*
  Returns:
    If this item has been added to an MYON_Hash32Table.AddItem(hash32,item pointer) then the 
    value of hash3d passed as the first argument to MYON_Hash32Table.AddItem(hash32,item pointer)
    is returned. This is the value the MYON_Hash32Table uses for this item.
  ...
  Remarks:
    ...
    {
      MYON_SubDSurfaceInterpolatortHash32TableItem& hitem = copied_items[i];
      hitem.ClearHashTableSerialNumberForExperts();
      m_htable.AddItem(hitem.HashTableItemHash(), &hitem);
    }
  */
  MYON__UINT32 HashTableItemHash() const;
...
private:
  friend class MYON_Hash32Table;
  mutable MYON_Hash32TableItem* m_internal_next = nullptr;
  mutable MYON__UINT32 m_internal_hash32 = 0;
  mutable MYON__UINT32 m_internal_hash_table_sn = 0;
};
...
*/
class MYON_CLASS MYON_Hash32Table
{
public:
  MYON_Hash32Table();
  ~MYON_Hash32Table();
...
private:
  MYON_Hash32Table(const MYON_Hash32Table&) = delete;
  MYON_Hash32Table& operator=(const MYON_Hash32Table&) = delete;
...
public:
  ...
  */
  ...
  Remarks:
    ...
    specified hash32 falue. Use MYON_Hash32TableItem.NextItemWithSameHash() to get
    the next item in the has table with the same hash value.
  */
  class MYON_Hash32TableItem* FirstItemWithHash(
    MYON__UINT32 hash32
    ) const;

  class MYON_Hash32TableItem* NextItemWithHash(
    const class MYON_Hash32TableItem* current_item
    ) const;
  ...
  */
  class MYON_Hash32TableItem* FirstTableItem(
    ) const;
  ...
  Remarks:
    This function is used for iterating throught every element in the hash table.
  */
  class MYON_Hash32TableItem* NextTableItem(
...
private:
  const MYON__UINT32 m_hash_table_sn;
  MYON__UINT32 m_reserved = 0;
  mutable MYON__UINT32 m_hash_table_capacity = 0;
  MYON__UINT32 m_item_count = 0;
  mutable class MYON_Hash32TableItem** m_hash_table = nullptr;
