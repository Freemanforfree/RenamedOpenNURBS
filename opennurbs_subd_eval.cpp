...
MYON_SubD* MYON_SubDSectorType::SectorRingSubD(
  double radius,
  ...
  ) const
{
  ...
  MYON_SubDVertexTag vertex_tag0;
  MYON_SubDVertexTag vertex_tag1;
  MYON_SubDEdgeTag edge_tag0;
  MYON_SubDEdgeTag edge_tag1;

  switch (vertex_tag)
  {
  case MYON_SubDVertexTag::Smooth:
    sector_angle_radians = 2.0*MYON_PI;
    vertex_tag0 = MYON_SubDVertexTag::Smooth;
    vertex_tag1 = MYON_SubDVertexTag::Smooth;
    edge_tag0 = MYON_SubDEdgeTag::Smooth;
    edge_tag1 = MYON_SubDEdgeTag::Smooth;
    break;

  case MYON_SubDVertexTag::Crease:
    if ( !(sector_angle_radians > 0.0 && sector_angle_radians < 2.0*MYON_PI) )
      sector_angle_radians = 0.5*MYON_PI;
    vertex_tag0 = MYON_SubDVertexTag::Crease;
    vertex_tag1 = MYON_SubDVertexTag::Crease;
    edge_tag0 = MYON_SubDEdgeTag::Crease;
    edge_tag1 = MYON_SubDEdgeTag::Crease;
    break;
  ...
  unsigned int sector_angle_index = MYON_SubDSectorType::CornerAngleIndexFromCornerAngleRadians(
    MYON_SubDSectorType::ClampCornerSectorAngleRadians(sector_angle_radians)
  );
  if (sector_angle_index <= MYON_SubDSectorType::MaximumCornerAngleIndex
    && fabs(MYON_SubDSectorType::AngleRadiansFromCornerAngleIndex(sector_angle_index) - sector_angle_radians) <= 1.0e-6
    )
  {
    sector_angle_radians = MYON_SubDSectorType::AngleRadiansFromCornerAngleIndex(sector_angle_index);
  }
  else
  {
    sector_angle_index = MYON_UNSET_UINT_INDEX;
  }
  
  const double smooth_edge_w0 = this->SectorCoefficient();

  MYON_SimpleArray< MYON_SubDVertex* > V(R);
  MYON_SimpleArray< MYON_SubDEdge* > E(N);

  MYON_3dPoint vertexP = MYON_3dPoint::Origin;

  for (unsigned int vi = 0; vi < R; vi++)
  {
    MYON_SubDVertexTag vertex_tag_vi;
    if ( 0 == vi )
      ...
      vertex_tag_vi = MYON_SubDVertexTag::Smooth; // interior edge or an outer face vertex

    if (radius > 0.0)
    {
      double cos_a, sin_a;
      if (sector_angle_index == MYON_UNSET_UINT_INDEX)
      {
      ...
      {
        MYON_SubDMatrix::EvaluateCosAndSin(2*sector_angle_index*vi, (R-1)*MYON_SubDSectorType::MaximumCornerAngleIndex,&cos_a,&sin_a);
      }
      const double r = (1 == (vi%2)) ? radius : (2.0*radius);
      vertexP.x = r*cos_a;
      vertexP.y = r*sin_a;
    }
    MYON_SubDVertex* vertex = subd->AddVertex( vertex_tag_vi, vertexP);
    V.Append(vertex);
  }
  //V[0]->m_vertex_edge_order = MYON_SubD::VertexEdgeOrder::radial;
  
  for (unsigned int vei = 0; vei < N; vei++)
  {
    MYON_SubDEdgeTag edge_tag_vei;
    if ( 0 == vei )
      edge_tag_vei = edge_tag0; // first edge
    else if ( vei+1 == N )
      edge_tag_vei = edge_tag1; // last edge
    else
      edge_tag_vei = MYON_SubDEdgeTag::Smooth; // interior edge

    double w0 = (MYON_SubDEdgeTag::Smooth == edge_tag_vei) ? smooth_edge_w0 : MYON_SubDSectorType::IgnoredSectorCoefficient;
    unsigned int ev1i = 1 + vei*ring_ei_delta;
    E.Append(
      subd->AddEdgeWithSectorCoefficients(
        edge_tag_vei,
        V[0], w0,
        V[ev1i], MYON_SubDSectorType::IgnoredSectorCoefficient)
        );
  }

  MYON_SubDVertex* f_vertex[4] = {};
  MYON_SubDEdge* f_edge[4] = {};
  MYON_SubDEdgePtr f_edgeptr[4] = {};

  f_vertex[0] = V[0];
  f_vertex[3] = const_cast<MYON_SubDVertex*>(E[0]->m_vertex[1]);
  f_edge[3] = E[0];
  for (unsigned int vfi = 0; vfi < F; vfi++)
  {
    f_edge[0] = f_edge[3];
    f_edge[3] = E[(vfi + 1) % N];
    f_vertex[1] = const_cast<MYON_SubDVertex*>(f_edge[0]->m_vertex[1]);
    f_vertex[3] = const_cast<MYON_SubDVertex*>(f_edge[3]->m_vertex[1]);

    f_edgeptr[0] = MYON_SubDEdgePtr::Create(f_edge[0], 0);
    f_edgeptr[3] = MYON_SubDEdgePtr::Create(f_edge[3], 1);
    f_vertex[2] = V[2 + 2 * vfi];
    f_edge[1] = subd->AddEdgeWithSectorCoefficients(MYON_SubDEdgeTag::Smooth, f_vertex[1], MYON_SubDSectorType::IgnoredSectorCoefficient, f_vertex[2], MYON_SubDSectorType::IgnoredSectorCoefficient);
    f_edge[2] = subd->AddEdgeWithSectorCoefficients(MYON_SubDEdgeTag::Smooth, f_vertex[2], MYON_SubDSectorType::IgnoredSectorCoefficient, f_vertex[3], MYON_SubDSectorType::IgnoredSectorCoefficient);
    f_edgeptr[1] = MYON_SubDEdgePtr::Create(f_edge[1], 0);
    f_edgeptr[2] = MYON_SubDEdgePtr::Create(f_edge[2], 0);
...
{
  MYON_3dPoint P = SP[SPi];
  double z = fabs(P[(Pi + 1) % 3]) + fabs(P[(Pi + 1) % 3])+ fabs(Q[(Pi + 1) % 3]) + fabs(Q[(Pi + 1) % 3]);
  if (!(0.0 == z))
  {
    // MYON_ERROR("point coordinate is not zero.");
    return MYON_SUBD_RETURN_ERROR(false);
  }
  if (fabs(P[Pi]) > 1.0)
  {
    // MYON_ERROR("point coordinate P[Pi] > 1.");
    return MYON_SUBD_RETURN_ERROR(false);
  }
  if (fabs(Q[Pi]) > 1.0)
  {
    // MYON_ERROR("point coordinate Q[Pi] > 1.");
    return MYON_SUBD_RETURN_ERROR(false);
  }
  ...
  )
{
  if ( component_ring_count < 4 || nullptr == component_ring)
    return MYON_SUBD_RETURN_ERROR(false);
  MYON_SubDVertex* vertex = component_ring[0].Vertex();
  if ( nullptr == vertex)
    return MYON_SUBD_RETURN_ERROR(false);
  vertex->ClearSavedSubdivisionPoints();
  for (unsigned int i = 1; i < component_ring_count; i++)
  {
    MYON_SubDEdge* edge = component_ring[i].Edge();
    if ( nullptr == edge)
      return MYON_SUBD_RETURN_ERROR(false);
    ...
    if (i >= component_ring_count)
      break;
    MYON_SubDFace* face = component_ring[i].Face();
    if ( nullptr == face)
      return MYON_SUBD_RETURN_ERROR(false);
...
double MYON_SubDMatrix::TestEvaluation() const
{
  ...
  unsigned int SP_low_precision_index = MYON_UNSET_UINT_INDEX;
  
  MYON_SimpleArray< MYON_3dPoint > _P(R);
  MYON_3dPoint* SP = _P.Array();

  MYON_SimpleArray< double > _Scol(R);
  double* Scol = _Scol.Array();

  MYON_SubD subd;
  if (&subd != m_sector_type.SectorRingSubD(0.0,0.0,&subd))
    return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
  const MYON_SubDVertex* vertex0 = subd.FirstVertex();
  if (nullptr == vertex0)
    return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
  ...
  MYON_SimpleArray<const MYON_SubDVertex*> vertex_ring_array(subd.VertexCount());
  for (const MYON_SubDVertex* vertex = vertex0; nullptr != vertex; vertex = vertex->m_next_vertex)
  {
    vertex_ring_array.Append(vertex);
  }
  if ( R != vertex_ring_array.UnsignedCount())
    return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
  const MYON_SubDVertex*const* vertex_ring = vertex_ring_array.Array();

  MYON_SimpleArray<MYON_SubDComponentPtr> component_ring_array;
  const unsigned int component_ring_count = MYON_SubD::GetSectorComponentRing(sit,component_ring_array);
  if ( component_ring_count < 4 || component_ring_count != m_sector_type.ComponentRingCount())
    return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
  const MYON_SubDComponentPtr* component_ring = component_ring_array.Array();

  MYON_SimpleArray< MYON_3dPoint > _ringP0;

  MYON_SimpleArray< MYON_3dPoint > _ringP1;

  for (unsigned int vi = 0; vi < R; vi++)
    Scol[vi] = MYON_DBL_QNAN;
  ...
  for (unsigned int vi = 0; vi < R; vi++)
  {
    ...
    for (unsigned int Pi = 0; Pi < 3; Pi++)
    {
      ...
      const_cast<MYON_SubDVertex*>(vertex_ring[vi])->m_P[Pi] = 1.0;

      if ( R != MYON_SubD::GetSectorPointRing(false,component_ring_count,component_ring,_ringP0))
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      const MYON_3dPoint* ringP0 = _ringP0.Array();
      ...
      for (unsigned int i = 0; i < R; i++)
      {
        ...
        // there is a bug in MYON_SubD::GetSectorPointRing
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      }

      if ( R != MYON_SubD::GetSectorSubdivisionPointRing(component_ring, component_ring_count,_ringP1))
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      const MYON_3dPoint* ringP1 = _ringP1.Array();

      for (unsigned int i = 0; i < R; i++)
      {
        SP[i] = MYON_3dPoint::Origin;
      ...
      if (!(SP[vi][Pi] > 0.0))
      {
        // MYON_ERROR("SP[vi][Pi] is not positive.");
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      }
      ...
      {
        MYON_ERROR("x,y,z vertex point precisions are not identical.");
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      }
      if (!(N_outer_point_precision[0] == N_outer_point_precision[Pi]))
      {
        MYON_ERROR("x,y,z outer point precisions are not identical.");
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      }
      if (!(N_Scol_precision[0] == N_Scol_precision[Pi]))
      {
        MYON_ERROR("x,y,z S column precisions are not identical.");
        return MYON_SUBD_RETURN_ERROR(MYON_UNSET_VALUE);
      }
      ...
      const_cast<MYON_SubDVertex*>(vertex_ring[vi])->m_P[Pi] = 0.0;
    }
  }
...
{
  limit_point.m_limitP[0] = MYON_DBL_QNAN;
  limit_point.m_limitP[1] = MYON_DBL_QNAN;
  limit_point.m_limitP[2] = MYON_DBL_QNAN;
  ...
  const unsigned int point_ring_count = MYON_SubD::GetSectorPointRing( true, sit, point_ring, point_ring_capacity, point_ring_stride);
  ...
  for (;;)
  {
    ...
    if (
      ...
      && MYON_SubDVertexTag::Crease == SM.m_sector_type.VertexTag()
      && R >= 5
      && *((const MYON_3dPoint*)(point_ring+ point_ring_stride)) == *((const MYON_3dPoint*)(point_ring + (R-1)* point_ring_stride))
      )
    {
  ...
  return rc
    ? true
    : MYON_SUBD_RETURN_ERROR(false);
}

static MYON_SubDSectorSurfacePoint* LimitPointPool(
  const MYON_SubDSectorSurfacePoint* pReturnToPool // If nullptr, then one is allocated
  )
{
  static MYON_FixedSizePool limit_point_fsp;
  if (0 == limit_point_fsp.SizeofElement())
  {
    if (nullptr != pReturnToPool)
      return MYON_SUBD_RETURN_ERROR(nullptr);
    static MYON_SleepLock initialize_lock;
    initialize_lock.GetLock();
    if (0 == limit_point_fsp.SizeofElement())
      limit_point_fsp.Create(sizeof(MYON_SubDSectorSurfacePoint), 0, 0);
  ...
  MYON_SubDSectorSurfacePoint* lp = (MYON_SubDSectorSurfacePoint*)limit_point_fsp.ThreadSafeAllocateDirtyElement();  
  if (nullptr == lp)
    return MYON_SUBD_RETURN_ERROR(nullptr);
  return lp;
}

bool MYON_SubDVertex::GetSurfacePoint(
  ...
  ) const
{
  bool rc = false;
  MYON_SubDSectorIterator sit;
  const MYON_SubDFace* limit_point_sector_face = nullptr;
  ...
  MYON_SubDSectorSurfacePoint saved_limit_point = limit_point;
  saved_limit_point.m_next_sector_limit_point = (MYON_SubDSectorSurfacePoint*)1; // causes unnecessary test to be skipped
  SetSavedSurfacePoint( bUndefinedNormalIsPossible, saved_limit_point);

  return rc;
}

bool MYON_SubDVertex::GetSavedSurfacePoint(
...
const MYON_3dPoint MYON_SubDVertex::SurfacePoint() const
{
  MYON_3dPoint limit_point(MYON_3dPoint::NanPoint);
  return GetSurfacePoint(&limit_point.x) ? limit_point : MYON_3dPoint::NanPoint;
}

const MYON_3dVector MYON_SubDVertex::SurfaceNormal(
  const MYON_SubDFace* sector_face,
...
) const
{
  for (;;)
  {
    ...
    {
      MYON_ERROR("No faces on this vertex.");
      break;
    }

    if (nullptr == sector_face && IsCreaseOrCorner())
    {
      const MYON_SubDComponentPtrPair crease_pair = this->CreasedEdgePair(false);
      const MYON_SubDEdge* e[2] = { crease_pair.First().Edge(), crease_pair.Second().Edge() };
      if (nullptr == e[0] || 1 != e[0]->m_face_count || nullptr == e[1] || 1 != e[1]->m_face_count)
      {
        MYON_ERROR("sector_face must be specified in this case.");
        break;
      }
    }
    ...
    MYON_SubDSectorSurfacePoint limit_point;
    bool rc = MYON_SubDVertex::GetSurfacePoint(
      sector_face,
    ...
    if (false == rc)
      break;
    const MYON_3dVector N(limit_point.m_limitN);
...
const MYON_SubDSectorSurfacePoint& MYON_SubDVertex::SectorSurfacePointForExperts() const
{
  return this->m_limit_point;
}

const MYON_Plane MYON_SubDVertex::VertexFrame(
...
) const
{
  ...
  const MYON_SubDFace* sector_face = Face(0);
  if (nullptr == sector_face)
    return MYON_Plane::NanPlane;

  MYON_Plane vertex_frame(MYON_Plane::NanPlane);
  if (MYON_SubDComponentLocation::ControlNet == subd_appearance)
  {
    MYON_3dVector V = MYON_3dVector::ZeroVector;
    for (int vei = 0; vei < m_edge_count; ++vei)
    {
      const MYON_SubDEdge* e = Edge(vei);
      if (nullptr == e)
        continue;
      const MYON_SubDVertex* v1 = e->OtherEndVertex(this);
      if (nullptr == v1)
        continue;
      const MYON_SubDFace* f = (1 == e->m_face_count) ? e->Face(0) : nullptr;
      if (nullptr == f)
        continue;
      sector_face = f;
    ...
    const MYON_3dVector X = (V - (V * vertex_frame.zaxis) * vertex_frame.zaxis).UnitVector();
    if (X.IsUnitVector())
    {
      vertex_frame.xaxis = X;
      vertex_frame.yaxis = MYON_CrossProduct(vertex_frame.zaxis, vertex_frame.xaxis).UnitVector();
    }
  }
  else
  {
    ...
    MYON_3dVector Y(MYON_CrossProduct(limit_point.m_limitN, limit_point.m_limitT1));
    Y.Unitize();

    // The normal is more important than the tangent direction. 
    vertex_frame.CreateFromNormal(MYON_3dPoint(limit_point.m_limitP), MYON_3dVector(limit_point.m_limitN));
    vertex_frame.yaxis = Y;
    vertex_frame.xaxis = MYON_CrossProduct(vertex_frame.yaxis, vertex_frame.zaxis);
    vertex_frame.xaxis.Unitize();
  }

  return vertex_frame.IsValid() ? vertex_frame : MYON_Plane::NanPlane;
}


const MYON_Plane MYON_SubDEdge::CenterFrame(
  MYON_SubDComponentLocation subd_appearance
) const
{
  ...
  // to get P and N.
  MYON_Plane edge_frame(MYON_Plane::NanPlane);
  MYON_3dPoint center_point(MYON_3dPoint::NanPoint);
  MYON_3dVector center_normal(MYON_3dVector::NanVector);
  bool rc = false;
  ...
  if (rc)
  {
    if (false == edge_frame.CreateFromNormal(center_point, center_normal))
      return MYON_Plane::NanPlane;
    const MYON_3dVector U = ControlNetDirection();
    MYON_2dVector v(U * edge_frame.xaxis, U * edge_frame.yaxis);
    if ( v.Unitize() )
    {
      if (fabs(v.y) > MYON_SQRT_EPSILON&& fabs(v.x) < (1.0 - MYON_SQRT_EPSILON))
      {
        const MYON_3dVector X = (v.x * edge_frame.xaxis + v.y * edge_frame.yaxis).UnitVector();
        if (X.IsUnitVector())
        {
          const MYON_3dVector Y = MYON_CrossProduct(edge_frame.zaxis, X).UnitVector();
          if (Y.UnitVector())
          {
            edge_frame.xaxis = X;
            edge_frame.yaxis = Y;
  ...
  return edge_frame.IsValid() ? edge_frame : MYON_Plane::NanPlane;
}



const MYON_3dPoint MYON_SubDVertex::Point(MYON_SubDComponentLocation point_location) const
{
  ...
  )
{
  ...
  const MYON_SubDFace* limit_point_sector_face = limit_point.m_sector_face;
  ...
  if (nullptr != limit_point_sector_face)
  {
    ...
    if (false == rc)      
      return MYON_SUBD_RETURN_ERROR(false); // sector_face is not referenced by this vertex
  ...
  const MYON_SubDFace* sector_face0 = sit.CurrentFace();
  for (const MYON_SubDFace* sector_face = sector_face0; nullptr != sector_face && sector_face_count <= vertex_face_count; sector_face = sit.NextFace(MYON_SubDSectorIterator::StopAt::AnyCrease))
  {
...
bool MYON_SubDVertex::SetSavedSurfacePoint(
  bool bUndefinedNormalIsPossible,
  MYON_SubDSectorSurfacePoint limit_point
  ) const
{
  const bool bSkipSectorCheck = (1U == (MYON__UINT_PTR)limit_point.m_next_sector_limit_point);
  limit_point.m_next_sector_limit_point = nullptr;
    ...
    )
  {
    if (nullptr == limit_point.m_sector_face 
      || MYON_UNSET_VALUE == m_limit_point.m_limitP[0]
    ...
    else
    {
      ...
      MYON_SubDSectorSurfacePoint* lp = LimitPointPool(nullptr);
      if ( nullptr == lp)
        return MYON_SUBD_RETURN_ERROR(false);
      ...
      *lp = limit_point;
      ...
      MYON_SubDMeshFragment::SealPoints(true, m_limit_point.m_limitP, lp->m_limitP);
      ...
      const MYON_SubDSectorSurfacePoint* p = &m_limit_point;
      while(nullptr != p->m_next_sector_limit_point)
      {
        p = p->m_next_sector_limit_point;
      }

      const_cast<MYON_SubDSectorSurfacePoint*>(p)->m_next_sector_limit_point = lp;
    }
...
void MYON_SubDVertex::ClearSavedSurfacePoints() const
{
  ...
  if (nullptr != m_limit_point.m_next_sector_limit_point)
  {
    ...
    const MYON_SubDSectorSurfacePoint* next_p = m_limit_point.m_next_sector_limit_point;
    m_limit_point.m_next_sector_limit_point = nullptr;
    for (const MYON_SubDSectorSurfacePoint* p = next_p; nullptr != p; p = next_p)
    {
      next_p = p->m_next_sector_limit_point;
      LimitPointPool(p);
    }
  }
  m_limit_point = MYON_SubDSectorSurfacePoint::Unset;
}

void MYON_SubDVertex::ClearSavedSubdivisionPoints() const
{
  // clear vertex specific limit point cache
  ClearSavedSurfacePoints();
  // clear general subdivision point cache
  MYON_SubDComponentBase::Internal_ClearSubdivisionPointAndSurfacePointFlags();
}

void MYON_SubDVertex::ClearSavedSubdivisionPoints(bool bClearNeighborhood) const
{
  ...
  for (unsigned short vei = 0; vei < m_edge_count; ++vei)
  {
    ...
    const MYON_SubDVertex* other_v = e->OtherEndVertex(this);
    if (nullptr != other_v)
...
bool MYON_SubDVertex::SurfacePointIsSet() const
{
...
void MYON_SubDEdge::ClearSavedSubdivisionPoints() const
{
  // considering using a global pool for the limit curve cache - not yet.
  MYON_SubDComponentBase::Internal_ClearSubdivisionPointAndSurfacePointFlags();
}

void MYON_SubDEdge::ClearSavedSubdivisionPoints(bool bClearNeighborhood) const
{
  ClearSavedSubdivisionPoints();
  if (false == bClearNeighborhood)
    return;
  ...
  const MYON_SubDFacePtr* fptr = this->m_face2;
...
const MYON_SubDMeshFragment * MYON_SubDFace::MeshFragments() const
{
  // NOTE:
  // Clearing the MYON_SubDComponentBase::SavedPointsFlags::SurfacePointBit bit
  // on m_saved_points_flags is used to mark mesh fragmants as dirty. 
  // They need to be regerated before being used.
  return (0 != MYON_SUBD_CACHE_LIMITLOC_FLAG(m_saved_points_flags)) ? m_mesh_fragments : nullptr;
}

void MYON_SubDFace::ClearSavedSubdivisionPoints() const
{
  // considering using a global pool for the limit fragment cache - not yet.
  MYON_SubDComponentBase::Internal_ClearSubdivisionPointAndSurfacePointFlags();
}

void MYON_SubDFace::ClearSavedSubdivisionPoints(bool bClearNeighborhood) const
{
  ClearSavedSubdivisionPoints();
  if (false == bClearNeighborhood)
    return;

  const MYON_SubDEdgePtr* eptr = this->m_edge4;
...
unsigned int MYON_SubDVertex::VertexId() const
{
  return m_id;
}

unsigned int MYON_SubDEdge::EdgeId() const
{
  return m_id;
}

unsigned int MYON_SubDFace::FaceId() const
{
  return m_id;
}

const MYON_3dPoint MYON_SubDFace::ControlNetCenterPoint() const
{
  if (m_edge_count < 3)
    return MYON_3dPoint::NanPoint;
  MYON_3dPoint center(0.0, 0.0, 0.0);
...
const MYON_3dVector MYON_SubDFace::ControlNetCenterNormal() const
{
  if (4 == m_edge_count)
  {
    // faster code for most common case
    const MYON_SubDVertex* v[4] = { m_edge4[0].RelativeVertex(0),m_edge4[1].RelativeVertex(0),m_edge4[2].RelativeVertex(0),m_edge4[3].RelativeVertex(0)};
    if (nullptr == v[0] || nullptr == v[1] || nullptr == v[2] || nullptr == v[3])
      return MYON_3dVector::NanVector;
    const MYON_3dVector A = MYON_3dPoint(v[2]->m_P) - MYON_3dPoint(v[0]->m_P);
    const MYON_3dVector B = MYON_3dPoint(v[3]->m_P) - MYON_3dPoint(v[1]->m_P);
    const MYON_3dVector N = MYON_CrossProduct(A, B);
    return N.UnitVector();
  }
  ...
  if (3 == m_edge_count)
  {
    ...
    const MYON_SubDVertex* v[3] = { m_edge4[0].RelativeVertex(0),m_edge4[1].RelativeVertex(0),m_edge4[2].RelativeVertex(0)};
    if (nullptr == v[0] || nullptr == v[1] || nullptr == v[2])
      return MYON_3dVector::NanVector;
    const MYON_3dPoint C0(v[0]->m_P);
    const MYON_3dVector A = MYON_3dPoint(v[1]->m_P) - C0;
    const MYON_3dVector B = MYON_3dPoint(v[2]->m_P) - C0;
    const MYON_3dVector N = MYON_CrossProduct(A, B);
    return N.UnitVector();
  }

  // less common cases
  const MYON_3dPoint C = ControlNetCenterPoint();
  if (false == (C.x == C.x))
    return MYON_3dVector::NanVector;
  ...
  // a non-planar face.
  MYON_3dVector A;
  MYON_3dVector B = ControlNetPoint(m_edge_count - 1) - C;
  MYON_3dVector N = MYON_3dVector::ZeroVector;
  for (unsigned short fvi = 0; fvi < m_edge_count; ++fvi)
  {
    A = B;
    B = ControlNetPoint(fvi) - C;
    N += MYON_CrossProduct(A, B); // do not untize this and then the code works for non-convex faces too.
  }

  return N.UnitVector();
}

bool MYON_SubDFace::IsConvex() const
{
  const MYON_3dVector N = ControlNetCenterNormal();
  if (N.IsNotZero())
  {
    if (3 == m_edge_count)
      return true;
    MYON_3dPoint P = ControlNetPoint(m_edge_count - 2);
    MYON_3dPoint Q = ControlNetPoint(m_edge_count - 1);
    MYON_3dVector A;
    MYON_3dVector B = Q - P;
    for (unsigned short fvi = 0; fvi < m_edge_count; ++fvi)
    {
      ...
      A = B;
      B = Q - P;
      const MYON_3dVector AxB = MYON_CrossProduct(A, B);
...
bool MYON_SubDFace::IsNotConvex() const
{
  const MYON_3dVector N = ControlNetCenterNormal();
  if (N.IsNotZero())
  {
...
bool MYON_SubDFace::IsPlanar(double planar_tolerance) const
{
  const MYON_3dPoint C = ControlNetCenterPoint();
  const MYON_3dVector N = ControlNetCenterNormal();
  if (C.IsValid() && N.IsNotZero())
  {
    ...
    if (false == (planar_tolerance >= 0.0))
      planar_tolerance = MYON_ZERO_TOLERANCE;
    const MYON_SubDEdgePtr* eptr = m_edge4;
    double d0 = 0.0;
    double d1 = 0.0;
    for (unsigned short fvi = 0; fvi < m_edge_count; ++fvi, ++eptr)
    {
      ...
      if (nullptr == v)
        return false;
      const double d = (MYON_3dPoint(v->m_P) - C)*N;
      if (false == (d == d))
        return false; // nan
      if (d < d0)
        d0 = d;
...
bool MYON_SubDFace::IsNotPlanar(double planar_tolerance) const
{
  const MYON_3dPoint C = ControlNetCenterPoint();
  const MYON_3dVector N = ControlNetCenterNormal();
  if (C.IsValid() && N.IsNotZero())
...
const MYON_Plane MYON_SubDFace::ControlNetCenterFrame() const
{
  const MYON_3dPoint C = ControlNetCenterPoint();
  const MYON_3dVector N = ControlNetCenterNormal();
  if (C.IsValid() && N.IsNotZero())
