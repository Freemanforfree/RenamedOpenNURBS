...
bool
MYON_BrepVertex::Write( MYON_BinaryArchive& file ) const
{
...
bool
MYON_BrepVertex::Read( MYON_BinaryArchive& file )
{
...
bool MYON_BrepEdge::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_BrepEdge::Read( MYON_BinaryArchive& file )
{
  int bReversed = false;
  ...
  MYON_Interval domain = proxy_domain;
  if (    file.Archive3dmVersion() >= 3 
...
bool MYON_BrepTrim::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_BrepTrim::Read( MYON_BinaryArchive& file )
{
  MYON_3dPoint P[2];
  int i;
  ...
  case MYON_Surface::x_iso:
    m_iso = MYON_Surface::x_iso;
    break;
  case MYON_Surface::y_iso:
    m_iso = MYON_Surface::y_iso;
    break;
  case MYON_Surface::W_iso:
    m_iso = MYON_Surface::W_iso;
    break;
  case MYON_Surface::S_iso:
    m_iso = MYON_Surface::S_iso;
    break;
  case MYON_Surface::E_iso:
    m_iso = MYON_Surface::E_iso;
    break;
  case MYON_Surface::N_iso:
    m_iso = MYON_Surface::N_iso;
    break;
  }
  ...
  if ( file.Archive3dmVersion() >= 3 && file.ArchiveOpenNURBSVersion() >= 200206180 )
  {
    ...
    MYON_Interval d = ProxyCurveDomain();
    if (rc )
    {
      rc = file.ReadInterval( d );
      if ( !rc )
...
bool MYON_BrepLoop::Write( MYON_BinaryArchive& file ) const
{
  int i;
...
bool MYON_BrepLoop::Read( MYON_BinaryArchive& file )
{
  int i;
...
bool MYON_BrepFace::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_BrepFace::Read( MYON_BinaryArchive& file )
{
  int i;
...
bool MYON_BrepVertexArray::Read( MYON_BinaryArchive& file )
{
  Empty();
    ...
    if (rc) {
      if ( major_version==1 ) {
        ...
        for ( i = 0; i < count && rc ; i++ ) {
          MYON_BrepVertex& vertex = AppendNew();
...
bool MYON_BrepVertexArray::Write( MYON_BinaryArchive& file ) const
{
  int i;
...
bool MYON_BrepEdgeArray::Read( MYON_BinaryArchive& file )
{
  Empty();
  MYON__UINT32 tcode = 0;
  MYON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
bool MYON_BrepEdgeArray::Write( MYON_BinaryArchive& file ) const
{
  int i;
  bool rc = file.BeginWrite3dmChunk( TCODE_ANONYMOUS_CHUNK, 0 );
  if (rc) {
...
bool MYON_BrepTrimArray::Read( MYON_BinaryArchive& file )
{
  Empty();
  MYON__UINT32 tcode = 0;
  MYON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
        ...
        for ( i = 0; i < count && rc ; i++ ) {
          ...
          if ( rc )
          {
            if ( trim.m_trim_index != m_count-1 )
            {
              ...
              MYON_ERROR("Invalid value of m_trim_index");
...
bool MYON_BrepTrimArray::Write( MYON_BinaryArchive& file ) const
{
  int i;
  bool rc = file.BeginWrite3dmChunk( TCODE_ANONYMOUS_CHUNK, 0 );
  if (rc) {
...
bool MYON_BrepLoopArray::Read( MYON_BinaryArchive& file )
{
  Empty();
  MYON__UINT32 tcode = 0;
  MYON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
bool MYON_BrepLoopArray::Write( MYON_BinaryArchive& file ) const
{
  int i;
  bool rc = file.BeginWrite3dmChunk( TCODE_ANONYMOUS_CHUNK, 0 );
  if (rc) {
...
bool MYON_BrepFaceArray::Read( MYON_BinaryArchive& file )
{
  Empty();
  MYON__UINT32 tcode = 0;
  MYON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
          ...
          if (rc && minor_version >= 2)
          {
            ...
            if (rc && bHavePerFaceColors)
            {
              for (i = 0; rc && i < count; i++)
              {
                ...
                if (rc && MYON_Color::UnsetColor != per_face_color)
...
bool MYON_BrepFaceArray::Write( MYON_BinaryArchive& file ) const
{
  int i;
  bool rc = file.BeginWrite3dmChunk( TCODE_ANONYMOUS_CHUNK, 0 );
  if (rc) 
  {
    ...
    if (rc && minor_version >= 2)
    {
      ...
      for (i = 0; rc && i < count; i++)
      {
        if (MYON_Color::UnsetColor != m_a[i].PerFaceColor())
        {
...
bool MYON_Brep::Write( MYON_BinaryArchive& file ) const
{
  const MYON_Brep* brep = this;
  MYON_Brep* v2brep = 0;
  ...
  if (rc)
  {
    ...
    if ( rc )
    {
      for ( fi = 0; rc && fi < face_count; fi++ ) {
        const auto mesh = file.Save3dmRenderMesh(MYON::object_type::brep_object) ? brep->m_F[fi].SharedMesh(MYON::render_mesh) : nullptr;
        b = mesh ? 1 : 0;
        rc = file.WriteChar(b);
        if (rc && mesh) {
    ...
    {
      for ( fi = 0; rc && fi < face_count; fi++ ) {
        const auto mesh = file.Save3dmAnalysisMesh(MYON::object_type::brep_object) ? brep->m_F[fi].SharedMesh(MYON::analysis_mesh) : nullptr;
        b = mesh ? 1 : 0;
        rc = file.WriteChar(b);
        if (rc && mesh) {
          rc = file.WriteObject(*mesh);
...
static
void ReadFillInMissingBoxes( MYON_Brep& brep )
{
...
static const MYON_BoundingBox Internal_BestBoundingBox(
  const MYON_BoundingBox archive_bbox,
  const MYON_BoundingBox new_bbox
)
{
...
bool MYON_Brep::Read( MYON_BinaryArchive& file )
{
  int i;
  int C2_count = 0;
  int C3_count = 0;
      ...
      if (rc) {
        for ( i = 0; i < m_E.Count(); i++ ) {
          ...
          if ( e.m_c3i >= 0 && e.m_c3i < C3_count )
          {
            ...
            MYON_Interval pdom = e.ProxyCurveDomain();
            MYON_Interval edom = e.Domain();
          ...
          for ( fi = 0; rc && fi < face_count; fi++ ) 
          {
            ...
            if (rc && b) 
            {
              ...
              if ( 0 != obj )
              {
                ...
                {
                  m_F[fi].SetMesh(MYON::render_mesh, pMesh);
                }
              }
            }
          }
            ...
            for ( fi = 0; rc && fi < face_count; fi++ ) 
            {
              ...
              if (rc && b) 
              {
                ...
                if (nullptr != obj)
                {
                  ...
                  {
                    m_F[fi].SetMesh(MYON::analysis_mesh, pMesh);
                  }
                }
              }
            }
  ...
  {
    const MYON_BoundingBox archive_brep_bbox = m_bbox;
    MYON_BoundingBox new_brep_bbox = MYON_BoundingBox::EmptyBoundingBox;
    const unsigned int face_count = m_F.UnsignedCount();
    for (unsigned fi = 0; fi < face_count; ++fi)
    {
      MYON_BrepFace& f = this->m_F[fi];
      const MYON_BoundingBox archive_face_bbox = f.m_bbox;
      MYON_BoundingBox new_face_bbox = f.InternalFaceBoundingBox(false, false);
...
bool MYON_Brep::ReadOld200( MYON_BinaryArchive& file, int minor_version )
{
  bool rc = true;
  ...
  for ( i = 0; rc && i < trim_count; i++ ) {
    MYON_PolyCurve* curve = new MYON_PolyCurve();
  ...
  for ( i = 0; rc && i < face_count; i++ ) {
    MYON_NurbsSurface* surface = new MYON_NurbsSurface();
    rc = surface->Read( file )?true:false;
    m_S.Append( surface );
  }

  MYON_SimpleArray<int> te_index(trim_count);
  MYON_SimpleArray<int> te_twin_index(trim_count);
  ...
  for ( fi = 0; rc && fi < face_count; fi++ ) 
  {
    ...
    for ( fbi = 0; rc && fbi < fbcnt; fbi++ ) {
      btype_flag = 0;
      MYON_BrepLoop::TYPE looptype = MYON_BrepLoop::unknown;
      if (rc) rc = file.ReadInt( &i ); // legacy loop index
      if (rc) rc = file.ReadInt( &btype_flag );
      switch (btype_flag)
      {
      case 0:
        looptype = MYON_BrepLoop::outer;
        break;
      case 1:
        looptype = MYON_BrepLoop::inner;
        break;
      case -1:
        looptype = MYON_BrepLoop::slit;
        break;
      default:
        looptype = MYON_BrepLoop::unknown;
        break;
      }
      ...
      if (btcnt < 1 )
        rc = false;
      MYON_BrepLoop& bnd = NewLoop(looptype,f);
      for ( bti = 0; rc && bti < btcnt; bti++ ) {
        MYON_BrepTrim& trim = NewTrim(false,bnd,m_T.Count());
        te_index.Append(trim.m_trim_index);
        if (rc) rc = file.ReadInt( &i ); // legacy trim index
        if ( trim.m_trim_index != i )
        {
          MYON_ERROR("MYON_Brep::ReadOld200 - trim.m_trim_index out of synch.");
          //rc = false;
          //break;
        }
        ...
        if ( trim.m_ei < 0 || trim.m_ei >= c3_count )
        {
          ...
          if (b)
          {
            MYON_ERROR("MYON_Brep::ReadOld201 - trim.m_ei out of range.");
            rc = false;
            break;
          }
        }
        if ( trim.m_trim_index >= 0 && trim.m_trim_index < c2_count )
          ...
          MYON_ERROR("MYON_Brep::ReadOld200 - trim.m_trim_index out of range.");
          rc = false;
          trim.m_c2i = -1;
          break;
        }
  ...
  if (rc) {
    ...
    for ( i = 0; i < trim_count; i++ ) {
      ...
      if ( twin_index >= 0 && twin_index < m_T.Count() )
        continue;
      MYON_BrepTrim& trim1 = m_T[trim_index];
      MYON_BrepTrim& trim2 = m_T[twin_index];
    ...
    for ( i = 0; i < m_T.Count(); i++ ) {
      MYON_BrepTrim& trim = m_T[i];
      MYON_Curve* tcurve = m_C2[trim.m_c2i];
  ...
  if (rc) {
    ...
    for ( i = 0; rc && i < face_count; i++ ) {
      ...
      if (b) {
        ...
        if (nullptr != obj)
        {
          ...
          if (pMesh)
          {
            f.SetMesh(MYON::render_mesh, pMesh);
          }
          else
          {
            delete pMesh;
    ...
    if ( !rc ) {
      ...
      {
        MYON_BrepFace& f = m_F[i];
        f.DestroyMesh(MYON::render_mesh);
      }
    }
    ...
    if (rc && minor_version >= 1) {
      ...
      for ( i = 0; rc && i < face_count; i++ ) {
        ...
        if (b) 
        {
          ...
          if (nullptr != obj)
          {
            ...
            if (pMesh)
            {
              f.SetMesh(MYON::analysis_mesh, pMesh);
            }
            else
            {
              delete pMesh;
      ...
      if ( !rc ) {
        ...
        for ( i = 0; i < face_count; i++ ) {
          MYON_BrepFace& f = m_F[i];
          f.DestroyMesh(MYON::analysis_mesh);
        }
      }
    }
    ...
    if (!rc ) {
      MYON_ERROR("MYON_Brep::ReadOld201() - trouble reading render/analysis meshes");
      rc = true;
    }
  }
...
bool MYON_Brep::ReadOld100( MYON_BinaryArchive& file )
{
...
bool MYON_Brep::ReadOld101( MYON_BinaryArchive& file )
{
  MYON_Object*  pO = nullptr;
  MYON_Curve*   pC = nullptr;
  MYON_Surface* pS = nullptr;
  int i, count;
...
MYON_Curve* MYON_Brep::Read100_BrepCurve( MYON_BinaryArchive& ) const
{
  // TODO - look at old Rhino I/O tookit code and read b-rep curves
  return nullptr;
}

MYON_Surface* MYON_Brep::Read100_BrepSurface( MYON_BinaryArchive& ) const
{
