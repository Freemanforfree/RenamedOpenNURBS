...
#if !defined(MYON_LINEAR_WORKFLOW_INC_)
#define MYON_LINEAR_WORKFLOW_INC_

class MYON_CLASS MYON_LinearWorkflow final
{
public:
  MYON_LinearWorkflow();
  MYON_LinearWorkflow(MYON_XMLNode& model_node);
  MYON_LinearWorkflow(const MYON_LinearWorkflow& lw);
  ~MYON_LinearWorkflow();

  const MYON_LinearWorkflow& operator = (const MYON_LinearWorkflow& lw);

  bool operator == (const MYON_LinearWorkflow& lw);
  bool operator != (const MYON_LinearWorkflow& lw);
  ...
  void ApplyPreProcessGamma(MYON_4fColor& col, bool for_texture) const;
