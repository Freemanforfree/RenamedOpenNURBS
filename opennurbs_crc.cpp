...
MYON__UINT16 MYON_CRC16( MYON__UINT16 current_remainder, size_t count, const void* p )
{
  ...
  // first_crc = MYON_CRC16( first_crc, size1, buffer1 );
  // ...
  // first_crc = MYON_CRC16( first_crc, sizeN, bufferN );
  //
  ...
  // second_crc = MYON_CRC16( second_crc, size1, buffer1 );
  // ...
  // second_crc = MYON_CRC16( second_crc, sizeN, bufferN );


  // if ( MYON_CRC16( second_crc, 2, &first_crc ) ) {
  //   printf( "The value of at least one byte has changed.\n" );
  // }

  static MYON__UINT16 MYON_CRC16_CCITT_TABLE[256] =
  ...
  if ( count > 0 && b ) 
  {
    ...
    // The TestHashSpeed command tests hashing speeds. Short story - use either MYON_CRC32 or MYON_SHA1. 

	  //while (count >= 8) 
    // {
    //   // while() loop unrolled for speed
		 // r1 = MYON_CRC16_CCITT_TABLE[(current_remainder & ((MYON__UINT16)0xff00))>>8];
...
MYON__UINT32 MYON_CRC32( MYON__UINT32 current_remainder, size_t count, const void* p )
{
  /*
  //////////////////////////////////////////////////////////////////////////////////////////
  //
  // MYON_CRC32() is a slightly altered version of zlib 1.3.3's crc32()
  // and the zlib "legal stuff" is reproduced below.
  //
  // MYON_CRC32() and crc32() compute the same values.  MYON_CRC32() was renamed
  // so it wouldn't clash with the other crc32()'s that are out there and the
  // argument order was switched to match that used by the legacy MYON_CRC16().
  //
  //////////////////////////////////////////////////////////////////////////////////////////
  ...
  */
  ...
  /*
  MYON_CRC32_ZLIB_TABLE[] is a table for a byte-wise 32-bit CRC calculation 
  using the generator polynomial:
  x^32+x^26+x^23+x^22+x^16+x^12+x^11+x^10+x^8+x^7+x^5+x^4+x^2+x+1.
  */
  static MYON__UINT32 MYON_CRC32_ZLIB_TABLE[256] = {
  ...
  if ( count > 0 && p ) 
  {
    ...
    ////    {
    ////      // while() loop unrolled for speed
    ////      current_remainder = MYON_CRC32_ZLIB_TABLE[((int)current_remainder ^ (*b++)) & 0xff] ^ (current_remainder >> 8);
    ////      current_remainder = MYON_CRC32_ZLIB_TABLE[((int)current_remainder ^ (*b++)) & 0xff] ^ (current_remainder >> 8);
    ////      current_remainder = MYON_CRC32_ZLIB_TABLE[((int)current_remainder ^ (*b++)) & 0xff] ^ (current_remainder >> 8);
    ////      current_remainder = MYON_CRC32_ZLIB_TABLE[((int)current_remainder ^ (*b++)) & 0xff] ^ (current_remainder >> 8);
    ////      current_remainder = MYON_CRC32_ZLIB_TABLE[((int)current_remainder ^ (*b++)) & 0xff] ^ (current_remainder >> 8);
...
{
  for (int i = 0; i < 4; ++i)
    elapsed_time_in_seconds[i] = MYON_DBL_QNAN;

  MYON_RandomNumberGenerator rng;
  MYON_SimpleArray<MYON__UINT32> buffer_array(1024);
  for (int i = 0; i < buffer_array.Capacity(); ++i)
    buffer_array.Append(rng.RandomNumber());

  const MYON__UINT32* buffer = buffer_array.Array();
  ...
  if (nullptr != crc16)
  {
    ...
    for (size_t count = 0; count < byte_count; count += sizeof_buffer)
      h16 = MYON_CRC16(h16, sizeof_buffer, buffer);
    *crc16 = h16;
    sw.Stop();
  ...
  if (nullptr != crc32)
  {
    ...
    MYON__UINT32 h32 = 0;
    for (size_t count = 0; count < byte_count; count += sizeof_buffer)
      h32 = MYON_CRC32(h32, sizeof_buffer, buffer);
    *crc32 = h32;
    sw.Stop();
...
{
  MYON__UINT16 crc16 = 0;
  MYON__UINT32 crc32 = 0;
  MYON_MD5_Hash md5_hash = MYON_MD5_Hash::ZeroDigest;
  MYON_SHA1_Hash sha1_hash = MYON_SHA1_Hash::ZeroDigest;
  double elapsed_time_in_seconds[4] = {};
