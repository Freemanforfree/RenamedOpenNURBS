...
class MYON_FontListImpl
{
public:
  MYON_FontListImpl() = default;
  ~MYON_FontListImpl() = default;

private:
  MYON_FontListImpl(const MYON_FontListImpl&) = delete;
  MYON_FontListImpl& operator=(const MYON_FontListImpl&) = delete;

public:
  // List of fonts sorted by FontCharacteristics hash
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_font_characteristics_hash;

  // List of fonts sorted by PostScript name 
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_postscript_name;

  // List of fonts sorted by Windows LOGFONT.lfFaceName name
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_windows_logfont_name;

  // List of fonts sorted by Family name, then FaceName
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_family_name;
  ...
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_english_postscript_name;
  ...
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_english_windows_logfont_name;
  ...
  // (Recently added fonts may be in m_unsorted[])
  mutable MYON_SimpleArray< const MYON_Font* > m_by_english_family_name;

  mutable MYON_SimpleArray< const MYON_Font* > m_by_quartet_name;
};


MYON_PANOSE1::FamilyKind MYON_PANOSE1::FamilyKindFromUnsigned(
  unsigned int unsigned_panose_family_kind
)
{
  switch (unsigned_panose_family_kind)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::Any);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::NoFit);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::LatinText);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::LatinScript);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::LatinDecorative);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_PANOSE1::FamilyKind::LatinSymbol);
  default:
    break;
  }
  MYON_ERROR("unsigned_pannos1_classification is not valid");
  return MYON_PANOSE1::FamilyKind::Any;
}
const wchar_t*  MYON_PANOSE1::FamilyKindToWideString(
...
)
{
  ...
  switch (family_kind)
  {
  case MYON_PANOSE1::FamilyKind::Any: 
    s = L"Any";
    break;
  case MYON_PANOSE1::FamilyKind::NoFit:
    s = L"No fit";
    break;
  case MYON_PANOSE1::FamilyKind::LatinText:
    s = L"Latin Text"; 
    break;
  case MYON_PANOSE1::FamilyKind::LatinScript:
    s = L"Latin Script"; 
    break;
  case MYON_PANOSE1::FamilyKind::LatinDecorative: 
    s = L"Latin Decorative";
    break;
  case MYON_PANOSE1::FamilyKind::LatinSymbol:
    s = L"Latin Symbol";
    break;
  default:
    s = L"";
...
bool MYON_PANOSE1::IsZero() const
{
  const MYON__UINT8* p = TenBytes();
  for (int i = 0; i < 10; i++)
  {
    if (0 != p[i])
      return false;
...
bool MYON_PANOSE1::IsZeroOrOne() const
{
  const MYON__UINT8* p = TenBytes();
  for (int i = 0; i < 10; i++)
  {
...
bool MYON_PANOSE1::IsSet() const
{
  return IsZeroOrOne() ? false : true;
}

MYON_PANOSE1::FamilyKind MYON_PANOSE1::PANOSE1FamilyKind() const
{
  return m_family_kind;
}

const MYON__UINT8* MYON_PANOSE1::TenBytes() const
{
  return (const MYON__UINT8*)(&m_family_kind);
}

void MYON_PANOSE1::SetTenBytes(const MYON__UINT8* panose1_ten_bytes)
{
  SetNineBytes(MYON_PANOSE1::FamilyKindFromUnsigned(panose1_ten_bytes[0]), panose1_ten_bytes + 1);
}

void MYON_PANOSE1::SetNineBytes(
  MYON_PANOSE1::FamilyKind family_kind,
  const MYON__UINT8* panose1_properties_bytes
)
{
  const MYON__UINT8 b0 = static_cast<MYON__UINT8>(family_kind);
  if ( b0 > 5)
  {
    *this = MYON_PANOSE1::Zero;
  }
  else
  {
...
) const
{
  ...
  {
    MYON_wString family = FamilyKindToWideString(m_family_kind);
    if (family.IsEmpty())
      family = MYON_wString::FormatToString(L"%u", (unsigned int)m_family_kind);
...
int MYON_FontMetrics::Ascent() const
{
  return m_ascent;
}

int MYON_FontMetrics::Descent() const
{
  return m_descent;
}

int MYON_FontMetrics::LineSpace() const
{
  return m_line_space;
}

int MYON_FontMetrics::UPM() const
{
  return m_UPM;
}

int MYON_FontMetrics::AscentOfI() const
{
  return AscentOfCapital();
}

int MYON_FontMetrics::AscentOfCapital() const
{
  return (int)m_ascent_of_capital;
}

int MYON_FontMetrics::AscentOfx() const
{
  return (int)m_ascent_of_x;
}

double MYON_FontMetrics::GlyphScale(double text_height) const
{
  // Please consult Dale Lear and the MYON_FontGlyph 
  ...
  return (text_height > 0.0
    && text_height < MYON_UNSET_POSITIVE_FLOAT 
    && y > 0.0
    && y < MYON_UNSET_POSITIVE_FLOAT
    )
    ? (text_height / y) 
    : 1.0;
}

int MYON_FontMetrics::StrikeoutThickness() const
{
  return m_strikeout_thickness;
}

int MYON_FontMetrics::StrikeoutPosition() const
{
  return m_strikeout_position;
}

int MYON_FontMetrics::UnderscoreThickness() const
{
  return m_underscore_thickness;
}

int MYON_FontMetrics::UnderscorePosition() const
{
  return m_underscore_position;
}

bool MYON_FontMetrics::AscentDescentAndUPMAreValid() const
{
...
bool MYON_FontMetrics::HeightsAreValid() const
{
  // basic validation
...
bool MYON_FontMetrics::IsSetAndValid() const
{
  return (m_ascent_of_capital > 0 && HeightsAreValid());
}

void MYON_FontMetrics::SetHeights(
  int ascent,
  int descent,
  int UPM,
  int line_space
...
{
  if (MYON_UNSET_INT_INDEX < descent && descent < ascent && ascent < -MYON_UNSET_INT_INDEX)
  {
    m_ascent = ascent;
    m_descent = descent;
  }
  ...
  m_UPM = (UPM > 0 && UPM < -MYON_UNSET_INT_INDEX) ? UPM : 0;
  m_line_space = (line_space > 0 && line_space < -MYON_UNSET_INT_INDEX)? line_space : 0;
  if (
    m_line_space > 0
    && m_ascent > 0
    && m_descent <= m_ascent
...
void MYON_FontMetrics::SetAscentOfI(
...
void MYON_FontMetrics::SetAscentOfCapital(
  int ascent_of_capital
)
{
...
void MYON_FontMetrics::SetAscentOfx(
  int ascent_of_x
)
{
...
void MYON_FontMetrics::SetStrikeout(
...
void MYON_FontMetrics::SetUnderscore(
...
const MYON_FontMetrics MYON_FontMetrics::Scale(
  const MYON_FontMetrics& font_metrics,
  double scale
)
{
  MYON_FontMetrics scaled_font_metrics = font_metrics;
  ...
  if (scale > 0.0 && 1.0 != scale)
  {
    ...
    if (font_metrics.m_line_space == Internal_ScaleInt(MYON_FontMetrics::DefaultLineFeedRatio,font_metrics.AscentOfCapital()))
      scaled_font_metrics.m_line_space = Internal_ScaleInt(MYON_FontMetrics::DefaultLineFeedRatio,scaled_font_metrics.AscentOfCapital());
    else
...
const MYON_FontMetrics MYON_FontMetrics::Normalize(
  const MYON_FontMetrics& font_metrics
)
{
  if (font_metrics.m_UPM == MYON_Font::AnnotationFontCellHeight)
    return font_metrics;
  ...
  const double scale
    = ((double)MYON_Font::AnnotationFontCellHeight / ((double)font_metrics.m_UPM));
  return MYON_FontMetrics::Scale(font_metrics, scale);
}

MYON_ManagedFonts::MYON_ManagedFonts(MYON__UINT_PTR zero)
  : m_default_font_ptr(zero)
  , m_managed_fonts(true)
  , m_installed_fonts(false)
{}

MYON_ManagedFonts::~MYON_ManagedFonts()
{

  MYON_SimpleArray<const MYON_Font*> managed_fonts(m_installed_fonts.m_by_index);
  ...
  {
    MYON_Font* managed_font = const_cast<MYON_Font*>(managed_fonts[i]);
    if (nullptr == managed_font)
      continue;

    if (m_default_font_ptr == (MYON__UINT_PTR)managed_font)
    {
      // MYON_Font::Default is constructed after MYON_ManagedFonts::List 
      // and destructed before the destruction of MYON_ManagedFonts::List.
      // This continue prevents calling delete on 
      continue;
    }
    if (1 == managed_font->m_runtime_serial_number)
    {
      // MYON_Font::Default.m_runtime_serial_number = 1 and it is the only instance of a font
      // with m_runtime_serial_number = 1.  
      // However, the managed_font pointer points to MYON_Font::Default, which was destroyed a few miliseconds ago.
      // See opennurbs_statics.cpp and observe that construction order is
      // ..., MYON_ManagedFonts::List, MYON_Font::Unset, MYON_Font::Default, ...
      // and destruction order is
      // ..., MYON_Font::Default, MYON_Font::Unset, MYON_ManagedFonts::List, ...
      MYON_ERROR("The m_default_font_ptr test above should have detected MYON_Font::Default.");
      continue;
    }

    // The reset needs to happen for all fonts, including MYON_Font::Default
    // Otherwise we get unpredictable crashes when closing because the
    // order in which MYON_Font::Default, MYON_GlyphMap::GlyphPool and MYON_ManagedFonts::List
    // is not predictable.
...
class MYON_FontGlyphCache
{
public:
  static MYON_FontGlyphCache* New();

  void SetFontMetrics(
    const MYON_FontMetrics& font_unit_metrics
  );

#if defined(MYON_OS_WINDOWS_GDI)
  // primary installed font used to get glyphs
  // When the current (Windows) device has a font that
  // matches the MYON_Font, this is matching installed font.
  struct IDWriteFont* m_dwrite_font = 0;
#endif

  // See MYON_Font.FontMetrics() documentation for a discussion
  ...
  MYON_FontMetrics m_font_unit_metrics;

  // Font metrics normalized so UPM = MYON_Font::Constants::AnnotationFontCellHeight
  MYON_FontMetrics m_normalized_metrics;

  // Array of glyphs with sizes and display info
  std::shared_ptr<MYON_GlyphMap> m_glyphmap;
};

MYON_FontGlyphCache* MYON_FontGlyphCache::New()
{
  MYON_FontGlyphCache* font_cache = new MYON_FontGlyphCache();
  font_cache->m_glyphmap = std::make_shared<MYON_GlyphMap>();
  return font_cache;
}

void MYON_FontGlyphCache::SetFontMetrics(
  const MYON_FontMetrics& font_unit_metrics
)
{
  ...
  m_normalized_to_font_unit_scale 
    ...
    ? (((double)font_unit_metrics.UPM()) / ((double)MYON_Font::Constants::AnnotationFontCellHeight))
    : 0.0;
  m_font_unit_to_normalized_scale
    = m_normalized_to_font_unit_scale > 0.0
    ? (((double)MYON_Font::Constants::AnnotationFontCellHeight) / ((double)font_unit_metrics.UPM()))
    : 0.0;

  m_normalized_metrics
    = (m_font_unit_to_normalized_scale > 0.0 && 1.0 != m_font_unit_to_normalized_scale)
    ? MYON_FontMetrics::Scale(m_font_unit_metrics, m_font_unit_to_normalized_scale)
    : m_font_unit_metrics;
}

static const MYON_Font* Internal_BinarySearchForManagedFontSerialNumber( unsigned int key, const MYON_Font*const* base, size_t nel )
{
...
const MYON_Font* MYON_ManagedFonts::GetFromSerialNumber(
  unsigned int managed_font_serial_number
  )
{
  if (0 == m_managed_fonts.Count())
  {
    // Put MYON_Font::Default as the first entry in this list.
    Internal_AddManagedFont(&MYON_Font::Default, nullptr);
  }
  if (managed_font_serial_number < 1)
  {
    MYON_ERROR("managed_font_serial_number parameter must be >= 1");
    return nullptr;
  }

  if ( managed_font_serial_number == MYON_Font::Default.RuntimeSerialNumber() )
    return &MYON_Font::Default;

  const MYON_Font* const * managed_fonts = m_managed_fonts.m_by_index.Array();
...
const MYON_Font* MYON_ManagedFonts::GetFromFontCharacteristics(
...
)
{
  ...
  const double point_size
    = MYON_Font::IsValidPointSize(font_characteristics.PointSize())
    ? font_characteristics.PointSize()
    : 0.0;

  const MYON_Font* set_font_characteristics = &font_characteristics;

  std::unique_ptr< MYON_Font > fup;
  if (false == font_characteristics.IsInstalledFont())
  {
    ...
    {
      fup = std::make_unique< MYON_Font >(font_characteristics);
      MYON_Font* temporary_font = fup.get();
      if (nullptr != temporary_font)
      {
        if (false == bHaveName)
        {
          *temporary_font = MYON_Font::Default;
        }
        else
        {
          temporary_font->SetUnsetProperties(MYON_Font::Default, true);
        }
  ...
  MYON_Font::NameLocale name_locale = MYON_Font::NameLocale::LocalizedFirst;
  ...
  for (;;)
  {
    ...
    if (bIsStrikethrough)
      break;
    if (0.0 != point_size && point_size != MYON_Font::Default.m_point_size)
      break;

    if (false == MYON_Font::EqualWeightStretchStyle(&MYON_Font::Default, set_font_characteristics, false))
      break; // there are multiple weight-stretch-style settings for the same family name and same LOGFONT.lfFaceName.

    MYON_wString name = set_font_characteristics->PostScriptName(name_locale);
    if (name.IsNotEmpty())
    {
      if (MYON_wString::EqualOrdinal(name, MYON_Font::Default.PostScriptName(name_locale), true))
      {
        return &MYON_Font::Default;
      }
    ...
    if (name.IsNotEmpty())
    {
      if (MYON_wString::EqualOrdinal(name, MYON_Font::Default.WindowsLogfontName(name_locale), true))
      {
        return &MYON_Font::Default;
      }
    ...
    if (name.IsNotEmpty())
    {
      if (MYON_wString::EqualOrdinal(name, MYON_Font::Default.FamilyName(name_locale), true))
      {
        name = set_font_characteristics->FaceName(name_locale);
        if (
          name.IsEmpty()
          || MYON_wString::EqualOrdinal(name, MYON_Font::Default.FaceName(name_locale), true)
          )
        {
          return &MYON_Font::Default;
        }
  ...
  const MYON_Font* managed_font = m_managed_fonts.FromFontProperties(
  ...
  {
    MYON_Font undecorated(*set_font_characteristics);
  ...
  unsigned int managed_font_wss_dev = MYON_Font::WeightStretchStyleDeviation(
    set_font_characteristics, 
    managed_font
  );

  // Lowell - Added additional check for matching underline and strikethrough settings RH-60947
  unsigned int managed_font_us_dev = MYON_Font::UnderlinedStrikethroughDeviation(
    set_font_characteristics, 
    managed_font
  );
  ...
  for(;;)
  {
    ...
    if (bIsUnderlined || bIsStrikethrough || point_size > 0.0)
    {
      ...
      fup = std::make_unique< MYON_Font >(*installed_font);
      MYON_Font* temporary_font = fup.get();
      if (nullptr != temporary_font)
      {
        temporary_font->SetUnderlined(bIsUnderlined);
    ...
    managed_font_wss_dev = MYON_Font::WeightStretchStyleDeviation(
    ...
    // RH - 60947
    managed_font_us_dev = MYON_Font::UnderlinedStrikethroughDeviation(
  ...
  const MYON_FontMetrics* font_metrics = nullptr;
  const MYON_FontGlyphCache* font_glyph_cache = set_font_characteristics->m_font_glyph_cache.get();
  if (nullptr != font_glyph_cache)
    font_metrics = &font_glyph_cache->m_font_unit_metrics;
  managed_font = Internal_AddManagedFont(
    new MYON_Font( MYON_Font::FontType::ManagedFont, *set_font_characteristics),
    font_metrics
  );
...
{
  const MYON_wString dirty(s);
  MYON_wString& clean = const_cast<MYON_wString&>(s);
  clean.Destroy();
  clean = static_cast<const wchar_t*>(dirty);
}

const MYON_Font* MYON_ManagedFonts::Internal_AddManagedFont(
  const MYON_Font* managed_font,
  const MYON_FontMetrics* managed_font_metrics_in_font_design_units
  )
{
  ...
  {
    // MYON_Font::Default is constructed after MYON_ManagedFonts::List and
    // destroyed before MYON_ManagedFonts::List. Therefore ~MYON_ManagedFonts()
    // has a pointer to MYON_Font::Default that must be ignored during
    // destruction.
    m_default_font_ptr = (MYON__UINT_PTR)managed_font;
  }

  const MYON_Font* installed_font = InstalledFonts().FromFontProperties(managed_font, true, true);
  if (nullptr != installed_font)
  {
    // This managed font matches an installed font.
    MYON_Font::Internal_SetManagedFontInstalledFont(managed_font, installed_font, false);
  }
  else
  {
    ...
    if (MYON_FontFaceQuartet::Member::Unset == managed_font->m_quartet_member)
      managed_font->m_quartet_member = MYON_FontFaceQuartet::MemberFromBoldAndItalic(bBoldInQuartet, bItalicInQuartet);

    const MYON_FontFaceQuartet default_quartet = MYON_Font::Default.InstalledFontQuartet(); // only look at installed quartet!
    installed_font = default_quartet.ClosestFace(bBoldInQuartet, bItalicInQuartet);
    if (nullptr == installed_font)
      installed_font = &MYON_Font::Default;
    MYON_Font::Internal_SetManagedFontInstalledFont(managed_font, installed_font, true);
  }

  MYON_FontGlyphCache* font_cache = managed_font->m_font_glyph_cache.get();
  if (nullptr == font_cache)
  {
    ...
    //
    font_cache = MYON_FontGlyphCache::New();
    managed_font->m_font_glyph_cache = std::shared_ptr<MYON_FontGlyphCache>(font_cache);
  }
  ...
  if (nullptr != font_cache)
  {
    if (false == font_cache->m_font_unit_metrics.HeightsAreValid())
    {
      ...
      {
        MYON_ManagedFonts::GetFontMetricsInFontDesignUnits(
          managed_font,
...
#define MYON_MANAGED_FONT_CHECK(falure_return_value) {if (0 != m_managed_font) (MYON_ERROR("Cannot modify managed fonts."); return (falure_return_value);}}

//
// END list of managed MYON_Fonts
//
//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
//
// BEGIN list of platform MYON_Fonts
//


#if defined(MYON_RUNTIME_WIN)

////static bool MYON_Internal_WindowsGetGlyphMetricsEx(
////  LOGFONT& logfont,
////  int logfont_height,
////  //MAT2 mat2,
...
////)
////{
////  font_unit_glyph_box = MYON_TextBox::Unset;
////
////  if (logfont_height <= 0)
////    logfont_height = MYON_Font::Constants::AnnotationFontCellHeight;
////
...
////  const int w_count = MYON_EncodeWideChar(unicode_code_point, w_capacity, w);
...
////  for (;;)
////  {
////    font_hdc = MYON_Font::CreateWindowsLogfontDeviceContext();
////    if (nullptr == font_hdc)
////      break;
////
////    LOGFONT lf = logfont;
...
////
////    MYON__UINT16 glyphindex[sizeof(w)/sizeof(w[0])] = { 0 };
...
class MYON_InternalLogFontAndTextMetric
{
public:
  MYON_InternalLogFontAndTextMetric() = default;
  ~MYON_InternalLogFontAndTextMetric() = default;
  MYON_InternalLogFontAndTextMetric(const MYON_InternalLogFontAndTextMetric&) = default;
  MYON_InternalLogFontAndTextMetric& operator=(const MYON_InternalLogFontAndTextMetric&) = default;

  static const MYON_InternalLogFontAndTextMetric Unset;

public:
  MYON_SHA1_Hash m_font_hash = MYON_SHA1_Hash::ZeroDigest;
  LOGFONT m_lfX;
  TEXTMETRIC m_tmX;
};

MYON_InternalLogFontAndTextMetric Internal_InitUnset()
{
  MYON_InternalLogFontAndTextMetric lftm;
  memset(&lftm, 0, sizeof(lftm));
  return lftm;
}

const MYON_InternalLogFontAndTextMetric MYON_InternalLogFontAndTextMetric::Unset(Internal_InitUnset());

////class MYON_Font_Windows_LFTM_UserData : public MYON_UserData
////{
////public:
////  static void Set(
////    const LOGFONT& lf,
...
////      return;
////    MYON_UserData* ud = font->GetUserData(MYON_Font_Windows_LFTM_UserData::Id);
////    MYON_Font_Windows_LFTM_UserData* lftm = MYON_Font_Windows_LFTM_UserData::Cast(ud);
////    if (nullptr == lftm)
////    {
////      if (nullptr != ud)
////        delete ud;
////      ud = nullptr;
////      lftm = new MYON_Font_Windows_LFTM_UserData();
////    }
...
////  }
////
////  static MYON_InternalLogFontAndTextMetric Get(
////    const MYON_Font* font
////  );
////
////  MYON_Font_Windows_LFTM_UserData()
////  {
////    m_userdata_uuid = MYON_Font_Windows_LFTM_UserData::Id;
////    m_userdata_copycount = 1;
////  }
////  ~MYON_Font_Windows_LFTM_UserData() = default;
////  MYON_Font_Windows_LFTM_UserData(const MYON_Font_Windows_LFTM_UserData&) = default;
////  MYON_Font_Windows_LFTM_UserData& operator=(const MYON_Font_Windows_LFTM_UserData&) = default;
////
////  MYON_OBJECT_DECLARE(MYON_Font_Windows_LFTM_UserData);
////
////private:
////// {452ADA1A-49F0-4FAF-8E14-73DABE21FC5C}
////  static const MYON_UUID Id;
////
////private:
////  MYON_SHA1_Hash m_font_hash = MYON_SHA1_Hash::ZeroDigest;
////  MYON_InternalLogFontAndTextMetric m_lftm = MYON_InternalLogFontAndTextMetric::Unset;
////};
////
////// {452ADA1A-49F0-4FAF-8E14-73DABE21FC5C}
////const MYON_UUID MYON_Font_Windows_LFTM_UserData::Id = { 0x452ada1a, 0x49f0, 0x4faf, { 0x8e, 0x14, 0x73, 0xda, 0xbe, 0x21, 0xfc, 0x5c } };
////MYON_OBJECT_IMPLEMENT(MYON_Font_Windows_LFTM_UserData, MYON_UserData, "452ADA1A-49F0-4FAF-8E14-73DABE21FC5C");
////
////void MYON_Font_Windows_LFTM_UserData::Set(
////  const LOGFONT& lf,
////  const TEXTMETRIC& tf,
////  MYON_Font* font
////)
...
////}
////
////MYON_InternalLogFontAndTextMetric MYON_Font_Windows_LFTM_UserData::Get(
////  const MYON_Font* font
////)
////{
////}
...
)
{
  for (;;)
  {
    ...
    ((MYON_SimpleArray<LOGFONT>*)lParam)->AppendNew() = *lf;
    break;
  }
...
)
{
  ...
  else
  {
    ...
    if (SYMBOL_CHARSET != lf.lfCharSet)
      lf.lfCharSet = DEFAULT_CHARSET;
    lf.lfOutPrecision = MYON_Font::WindowsConstants::logfont_out_precis;
    lf.lfClipPrecision = CLIP_DEFAULT_PRECIS;
    lf.lfQuality = MYON_Font::WindowsConstants::logfont_quality;
    lf.lfPitchAndFamily = MYON_Font::WindowsConstants::logfont_pitch_and_family;
  }
}
...
static int Internal_CompareLogfont(const LOGFONT* lhs, const LOGFONT* rhs)
{
  ...
  int rc = MYON_wString::CompareOrdinal(
    lhs->lfFaceName,
    MYON_wString::Length(lhs->lfFaceName,face_name_capacity), 
    rhs->lfFaceName, 
    MYON_wString::Length(rhs->lfFaceName,face_name_capacity), 
    true
  );
  if (0 != rc)
    return rc;
...
unsigned int MYON_Font::GetInstalledWindowsLogFonts(
...
)
{
  ...
  lf.lfCharSet = DEFAULT_CHARSET;
  MYON_SimpleArray<LOGFONT> font_family_list(1000);
  ::EnumFontFamiliesEx(
    font_hdc, 
    &lf,
  ...
  for (int i = 0; i < font_family_count; i++)
  {
    ...
    MYON_qsort((void*)(logfont_list.Array() + count0), count1 - count0, sizeof(LOGFONT), (int (*)(const void*,const void*))Internal_CompareLogfont);
    memset(&lf, 0, sizeof(lf));
    for (int j = count0; j < count1; j++)
    {
...
void MYON_ManagedFonts::Internal_GetWindowsInstalledFonts(
  MYON_SimpleArray<const MYON_Font*>& platform_font_list
)
{
  // As of May 2016, Rhino 6 requires WIndows 7 SP2 and DWrite works better.

  // MYON_Font::GetInstalledWindowsLogFonts() uses the 2 pass EnumFontFamiliesEx() technique.
  ...
  ////MYON_SimpleArray<LOGFONT> logfont_list;
  ////MYON_Font::GetInstalledWindowsLogFonts(logfont_list);
  ////for (int i = 0; i < logfont_list.Count(); i++)
  ////{
  ////  LOGFONT lf = logfont_list[i];
  ////  lf.lfHeight = 0;
  ////  MYON_Font* font = new MYON_Font();
  ...
  MYON_Font::GetInstalledWindowsDWriteFonts(
  ...
  MYON_SHA1_Hash prev_installed_font_hash = MYON_SHA1_Hash::ZeroDigest;
  ...
  for (int i = 0; i < dwrite_font_list.Count(); i++)
  {
    ...
    //    continue;
    //  if (MYON_Font::IsSingleStrokeFontName(dwrite_font_list[i].m_postscript_name))
    //    continue;
    //}
    ...
    for (;;)
    {
      MYON_Font* installed_font = new MYON_Font(MYON_Font::FontType::InstalledFont, dwrite_font_list[i]);
...
MYON_FontFaceQuartet::MYON_FontFaceQuartet(
  const wchar_t* quartet_name,
  const MYON_Font* regular,
  const MYON_Font* bold,
  const MYON_Font* italic,
  const MYON_Font* bold_italic
)
...
int MYON_FontFaceQuartet::CompareQuartetName(
...
)
{
  ...
  if (nullptr == rhs)
    return -1;
  return MYON_wString::CompareOrdinal(lhs->m_quartet_name, rhs->m_quartet_name, true);
}

MYON::RichTextStyle MYON::RichTextStyleFromUnsigned(unsigned int u)
{
  switch (u)
  {
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON::RichTextStyle::Unset);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON::RichTextStyle::Windows10SDK);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON::RichTextStyle::AppleOSXSDK);
  };
  MYON_ERROR("Invalid MYON::RichTextStyle value.");
  return MYON::RichTextStyle::Unset;
}

MYON::RichTextStyle MYON::RichTextStyleFromCurrentPlatform()
{
  return
#if defined(MYON_RUNTIME_WIN)
    MYON::RichTextStyle::Windows10SDK
#elif defined(MYON_RUNTIME_APPLE)
    MYON::RichTextStyle::AppleOSXSDK
#else
    MYON::RichTextStyle::Unset
#endif
    ;
}

const MYON_wString MYON_FontFaceQuartet::RichTextSample(
...
) const
{
  ...
  const MYON_wString regular_ps = (nullptr != m_regular) ? m_regular->PostScriptName() : MYON_wString::EmptyString;
  const MYON_wString bold_ps = (nullptr != m_bold) ? m_bold->PostScriptName() : MYON_wString::EmptyString;
  const MYON_wString italic_ps = (nullptr != m_italic) ? m_italic->PostScriptName() : MYON_wString::EmptyString;
  const MYON_wString bold_italic_ps = (nullptr != m_bold_italic) ? m_bold_italic->PostScriptName() : MYON_wString::EmptyString;

  const MYON_wString not_available(L"Not available.");
  const MYON_wString regular = regular_ps.IsNotEmpty() ? regular_ps : not_available;
  const MYON_wString bold = bold_ps.IsNotEmpty() ? bold_ps : not_available;
  const MYON_wString italic = italic_ps.IsNotEmpty() ? italic_ps : not_available;
  const MYON_wString bold_italic = bold_italic_ps.IsNotEmpty() ? bold_italic_ps : not_available;

  MYON_wString sample;
  switch (rich_text_style)
  {
  case MYON::RichTextStyle::Unset:
    break;

  case MYON::RichTextStyle::Windows10SDK:
    // font table uses Windows LOGFONT name and \b \i to select faces
    sample = MYON_wString::FormatToString(L"{\\rtf1\\deff0{\\fonttbl{\\f0 %ls;}}\\fs40", quartet_name);
    sample += MYON_wString::FormatToString(L"{\\f0 Windows 10 LOGFONT Quartet: %ls}{\\par}", quartet_name);
    if (nullptr != m_regular)
      sample += MYON_wString::FormatToString(L"{\\f0 Regular: %ls}{\\par}", static_cast<const wchar_t*>(regular_ps));
    if (nullptr != m_bold)
      sample += MYON_wString::FormatToString(L"{\\f0\\b Bold: %ls}{\\par}", static_cast<const wchar_t*>(bold_ps));
    if (nullptr != m_italic)
      sample += MYON_wString::FormatToString(L"{\\f0\\i Italic: %ls}{\\par}", static_cast<const wchar_t*>(italic_ps));
    if (nullptr != m_bold_italic)
      sample += MYON_wString::FormatToString(L"{\\f0\\b\\i Bold Italic: %ls}{\\par}", static_cast<const wchar_t*>(bold_italic_ps));
    sample += MYON_wString(L"\\par}");
    break;

  case MYON::RichTextStyle::AppleOSXSDK:
    ...
    {
      sample = MYON_wString::FormatToString(L"{\\rtf1\\deff0{\\fonttbl");
      int fdex = 0;
      if (regular_ps.IsNotEmpty())
        sample += MYON_wString::FormatToString(L"{\\f%d %ls;}", fdex++, static_cast<const wchar_t*>(regular_ps));
      if (bold_ps.IsNotEmpty())
        sample += MYON_wString::FormatToString(L"{\\f%d %ls;}", fdex++, static_cast<const wchar_t*>(bold_ps));
      if (italic_ps.IsNotEmpty())
        sample += MYON_wString::FormatToString(L"{\\f%d %ls;}", fdex++, static_cast<const wchar_t*>(italic_ps));
      if (bold_italic_ps.IsNotEmpty())
        sample += MYON_wString::FormatToString(L"{\\f%d %ls;}", fdex++, static_cast<const wchar_t*>(bold_italic_ps));
      sample += MYON_wString(L"}\\fs40");

      sample += MYON_wString::FormatToString(L"{\\f0 Apple OS X Fake Quartet: %ls}{\\par}", quartet_name);
      fdex = 0;
      if (nullptr != m_regular)
        sample += MYON_wString::FormatToString(L"{\\f%d Regular: %ls}{\\par}", fdex++, static_cast<const wchar_t*>(regular_ps));
      if (nullptr != m_bold)
        sample += MYON_wString::FormatToString(L"{\\f%d\\b Bold: %ls}{\\par}", fdex++, static_cast<const wchar_t*>(bold_ps));
      if (nullptr != m_italic)
        sample += MYON_wString::FormatToString(L"{\\f%d\\i Italic: %ls}{\\par}", fdex++, static_cast<const wchar_t*>(italic_ps));
      if (nullptr != m_bold_italic)
        sample += MYON_wString::FormatToString(L"{\\f%d\\b\\i Bold Italic: %ls}{\\par}", fdex++, static_cast<const wchar_t*>(bold_italic_ps));
      sample += MYON_wString(L"\\par}");
    }
    break;
...
bool MYON_FontFaceQuartet::HasRegularFace() const
{
  return (nullptr != RegularFace());
}

bool MYON_FontFaceQuartet::HasBoldFace() const
{
  return (nullptr != BoldFace());
}

bool MYON_FontFaceQuartet::HasItalicFace() const
{
  return (nullptr != ItalicFace());
}

bool MYON_FontFaceQuartet::HasBoldItalicFace() const
{
  return (nullptr != BoldItalicFace());
}

bool MYON_FontFaceQuartet::HasAllFaces() const
{
  return HasRegularFace() && HasBoldFace() && HasItalicFace() && HasBoldItalicFace();
}

bool MYON_FontFaceQuartet::IsEmpty() const
{
  return IsNotEmpty() ? false : true;
}


bool MYON_FontFaceQuartet::IsNotEmpty() const
{
  return (HasRegularFace() || HasBoldFace() || HasItalicFace() || HasBoldItalicFace());
}

const MYON_wString MYON_FontFaceQuartet::QuartetName() const
{
  return m_quartet_name;
}

const MYON_Font* MYON_FontFaceQuartet::RegularFace() const
{
  return m_regular;
}

const MYON_Font* MYON_FontFaceQuartet::BoldFace() const
{
  return m_bold;
}

const MYON_Font* MYON_FontFaceQuartet::ItalicFace() const
{
  return m_italic;
}

const MYON_Font* MYON_FontFaceQuartet::BoldItalicFace() const
{
  return m_bold_italic;
}

MYON_FontFaceQuartet::Member MYON_FontFaceQuartet::QuartetMember(
  const MYON_Font* font
) const
{
  if (nullptr == font || m_quartet_name.IsEmpty())
    return MYON_FontFaceQuartet::Member::Unset;
  
  if (
    false == m_quartet_name.EqualOrdinal(font->QuartetName(MYON_Font::NameLocale::Localized), true)
    && false == m_quartet_name.EqualOrdinal(font->QuartetName(MYON_Font::NameLocale::English), true)
    )
    return MYON_FontFaceQuartet::Member::Unset;
  
  MYON_FontFaceQuartet::Member m[2] = { MYON_FontFaceQuartet::Member ::Unset,MYON_FontFaceQuartet::Member::Unset};
  if ( font->IsItalicOrOblique() )
  {
    m[0] = MYON_FontFaceQuartet::Member::Italic;
    m[1] = MYON_FontFaceQuartet::Member::BoldItalic;
  }
  else 
  {
    m[0] = MYON_FontFaceQuartet::Member::Regular;
    m[1] = MYON_FontFaceQuartet::Member::Bold;
  }
  
  const MYON_Font* q[2] = { Face(m[0]),Face(m[1]) };
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    if (q[i]->FontWeight() != font->FontWeight())
      continue;
    s[0] = q[i]->WindowsLogfontName(MYON_Font::NameLocale::Localized);
    s[1] = q[i]->WindowsLogfontName(MYON_Font::NameLocale::English);
    s[2] = font->WindowsLogfontName(MYON_Font::NameLocale::Localized);
    s[3] = font->WindowsLogfontName(MYON_Font::NameLocale::English);
#else
    // style and PostScript name is the most reliable way to uniquely identify quartet members on Mac OS.
    // We've already matched style.
    s[0] = q[i]->PostScriptName(MYON_Font::NameLocale::Localized);
    s[1] = q[i]->PostScriptName(MYON_Font::NameLocale::English);
    s[2] = font->PostScriptName(MYON_Font::NameLocale::Localized);
    s[3] = font->PostScriptName(MYON_Font::NameLocale::English);
#endif
...
const MYON_Font* MYON_FontFaceQuartet::Face(
  MYON_FontFaceQuartet::Member member
) const
{
  switch (member)
  {
  case MYON_FontFaceQuartet::Member::Regular:
    return m_regular;
  case MYON_FontFaceQuartet::Member::Bold:
    return m_bold;
  case MYON_FontFaceQuartet::Member::Italic:
    return m_italic;
  case MYON_FontFaceQuartet::Member::BoldItalic:
    return m_bold_italic;
  case MYON_FontFaceQuartet::Member::Unset:
    break;
  }
  return nullptr;
}

const MYON_Font* MYON_FontFaceQuartet::ClosestFace(
  MYON_FontFaceQuartet::Member member
) const
{
...
unsigned int MYON_FontFaceQuartet::FaceCount() const
{
  unsigned count = 0;
  const MYON_Font* f[4] = { m_regular,m_bold,m_italic,m_bold_italic };
...
unsigned int MYON_FontFaceQuartet::NotInstalledFaceCount() const
{
  unsigned count = 0;
  const MYON_Font* f[4] = { m_regular,m_bold,m_italic,m_bold_italic };
  for (unsigned i = 0; i < 4U; ++i)
...
unsigned int MYON_FontFaceQuartet::SimulatedFaceCount() const
{
  unsigned count = 0;
  const MYON_Font* f[4] = { m_regular,m_bold,m_italic,m_bold_italic };
  for (unsigned i = 0; i < 4U; ++i)
...
{
  const MYON_Font* f = Face(bPreferedBold, bPreferedItalic);
  if (nullptr != f)
    return f;
...
const MYON_wString MYON_FontFaceQuartet::MemberToString(
  MYON_FontFaceQuartet::Member member
)
{
  switch (member)
  ...
  case MYON_FontFaceQuartet::Member::Unset:
    ...
    break;
  case MYON_FontFaceQuartet::Member::Regular:
    return MYON_wString(L"Regular");
    break;
  case MYON_FontFaceQuartet::Member::Bold:
    return MYON_wString(L"Bold");
    break;
  case MYON_FontFaceQuartet::Member::Italic:
    return MYON_wString(L"Italic");
    break;
  case MYON_FontFaceQuartet::Member::BoldItalic:
    return MYON_wString(L"Bold-Italic");
    break;
  }
  return MYON_wString::EmptyString;
}

MYON_FontFaceQuartet::Member MYON_FontFaceQuartet::MemberFromUnsigned(
  unsigned int member_as_unsigned
)
{
  switch (member_as_unsigned)
  {
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON_FontFaceQuartet::Member::Unset);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON_FontFaceQuartet::Member::Regular);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON_FontFaceQuartet::Member::Bold);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON_FontFaceQuartet::Member::Italic);
    MYON_ENUM_FROM_UNSIGNED_CASE(MYON_FontFaceQuartet::Member::BoldItalic);
  }
  return MYON_FontFaceQuartet::Member::Unset;
}

MYON_FontFaceQuartet::Member MYON_FontFaceQuartet::MemberFromBoldAndItalic(
  bool bMemberIsBold,
  ...
  )
{
  return
    bMemberIsBold
    ? (bMemberIsItalic ? MYON_FontFaceQuartet::Member::BoldItalic : MYON_FontFaceQuartet::Member::Bold)
    : (bMemberIsItalic ? MYON_FontFaceQuartet::Member::Italic : MYON_FontFaceQuartet::Member::Regular)
    ;
}

unsigned MYON_FontFaceQuartet::BoldItalicDeviation(
...
)
{
  ...
  if (MYON_FontFaceQuartet::Member::Unset == desired_member)
  {
    distance += 4;
    desired_member = MYON_FontFaceQuartet::Member::Regular;
  }

  if (MYON_FontFaceQuartet::Member::Unset == available_member)
  {
    distance += 4;
    available_member = MYON_FontFaceQuartet::Member::Regular;
  }

  const bool bDesiredBold = (MYON_FontFaceQuartet::Member::Bold == desired_member || MYON_FontFaceQuartet::Member::BoldItalic == desired_member);
  const bool bDesiredItalic = (MYON_FontFaceQuartet::Member::Italic == desired_member || MYON_FontFaceQuartet::Member::BoldItalic == desired_member);

  const bool bAvailableBold = (MYON_FontFaceQuartet::Member::Bold == available_member || MYON_FontFaceQuartet::Member::BoldItalic == available_member);
  const bool bAvailableItalic = (MYON_FontFaceQuartet::Member::Italic == available_member || MYON_FontFaceQuartet::Member::BoldItalic == available_member);
...
const MYON_wString MYON_Font::WidthWeightSlantDescription(MYON_Font::Stretch width, MYON_Font::Weight weight, MYON_Font::Style slant)
{
  MYON_wString wws;
  if (MYON_Font::Stretch::Unset != width)
  {
    if (wws.IsNotEmpty())
      wws += MYON_wString(L"-");
    wws += MYON_Font::StretchToWideString(width);
  }
  if (MYON_Font::Weight::Unset != weight)
  {
    if (wws.IsNotEmpty())
      wws += MYON_wString(L"-");
    wws += MYON_Font::WeightToWideString(weight);
  }
  if (MYON_Font::Style::Unset != slant)
  {
    if (wws.IsNotEmpty())
      wws += MYON_wString(L"-");
    wws += MYON_Font::StyleToWideString(slant);
  }
  return wws;
}

const MYON_wString MYON_Font::WidthWeightSlantDescription() const
{
  return MYON_Font::WidthWeightSlantDescription(this->FontStretch(), this->FontWeight(), this->FontStyle());
}

void MYON_FontFaceQuartet::Dump(MYON_TextLog& text_log) const
{
  MYON_wString quartet_name = this->QuartetName();
  ...
  const MYON_wString quartet_face[4] = {
    MYON_FontFaceQuartet::MemberToString(MYON_FontFaceQuartet::Member::Regular),
    MYON_FontFaceQuartet::MemberToString(MYON_FontFaceQuartet::Member::Bold),
    MYON_FontFaceQuartet::MemberToString(MYON_FontFaceQuartet::Member::Italic),
    MYON_FontFaceQuartet::MemberToString(MYON_FontFaceQuartet::Member::BoldItalic)
  };
  const MYON_Font* quartet_font[4] = {
    RegularFace(),
    BoldFace(),
    ItalicFace(),
    BoldItalicFace()
  ...
  MYON_wString quartet_decription(L"Quartet");
  if (quartet_name.IsNotEmpty())
  {
    quartet_decription += MYON_wString(L" ");
    quartet_decription += quartet_name;
  }
  quartet_decription += MYON_wString(L":");
  if (bQuartetNotAvailable)
  {
    quartet_decription += MYON_wString(L" <not available>");
  }
  else if (bQuartetNotInstalled)
  {
    quartet_decription += MYON_wString(L" (not installed)");
  }
  ...
  for (int i = 0; i < 4; i++)
  {
    MYON_wString description(quartet_face[i]);
    description += MYON_wString(':');
    const MYON_Font* font = quartet_font[i];
    if (nullptr == font)
      description += MYON_wString(L" <not available>");
    else
    {
      const MYON_wString family_name = font->FamilyName();
      if (family_name.IsNotEmpty())
      {
        description += MYON_wString(L" ");
        description += family_name;
        const MYON_wString face_name = font->FaceName();
      ...
      const MYON_wString postscript_name = font->PostScriptName();
      if (postscript_name.IsNotEmpty())
      {
        description += MYON_wString(L" (PostScript name=");
        description += postscript_name;
        description += MYON_wString(L")");
      }
#endif

      const MYON_wString wws = font->WidthWeightSlantDescription();
      if (wws.IsNotEmpty())
      {
        description += MYON_wString(L" ");
        description += wws;
      ...
      if (font->IsManagedSubstitutedFont())
        description += MYON_wString(L" (not installed)");
      else if (font->IsInstalledFont() && font->IsSimulated())
        description += MYON_wString(L" (simulated)");
    }
...
#define MYON_ManagedFonts_CompareFontPointer(lhs,rhs) \
if (lhs == rhs) return 0; if (nullptr == lhs) return 1; if (nullptr == rhs) return -1; \
const MYON_Font* lhs_font = *lhs; const MYON_Font* rhs_font = *rhs; \
if (lhs_font == rhs_font) return 0; if (nullptr == lhs_font) return 1; if (nullptr == rhs_font) return -1

int MYON_ManagedFonts::CompareFontPointer(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return 0;
}

int MYON_FontList::ComparePostScriptName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->PostScriptName(MYON_Font::NameLocale::LocalizedFirst), 
    rhs_font->PostScriptName(MYON_Font::NameLocale::LocalizedFirst),
    true
  );
}

int MYON_FontList::CompareFamilyName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->FamilyName(MYON_Font::NameLocale::LocalizedFirst), 
    rhs_font->FamilyName(MYON_Font::NameLocale::LocalizedFirst),
    true
  );
}

int MYON_FontList::CompareFamilyAndFaceName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  int rc = MYON_wString::CompareOrdinal(
    lhs_font->FamilyName(MYON_Font::NameLocale::LocalizedFirst), 
  ...
  if ( 0 == rc )
    rc = MYON_wString::CompareOrdinal(
    lhs_font->FaceName(MYON_Font::NameLocale::LocalizedFirst), 
    rhs_font->FaceName(MYON_Font::NameLocale::LocalizedFirst),
    true
  );
  return rc;
}

int MYON_FontList::CompareFamilyAndWindowsLogfontName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  int rc = MYON_wString::CompareOrdinal(
    lhs_font->FamilyName(MYON_Font::NameLocale::LocalizedFirst),
  ...
  if (0 == rc)
    rc = MYON_wString::CompareOrdinal(
      lhs_font->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst),
      rhs_font->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst),
      true
    );
  return rc;
}

int MYON_FontList::CompareWindowsLogfontName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst), 
...
int MYON_FontList::CompareEnglishPostScriptName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->PostScriptName(MYON_Font::NameLocale::English), 
    rhs_font->PostScriptName(MYON_Font::NameLocale::English),
    true
  );
}

int MYON_FontList::CompareEnglishFamilyName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->FamilyName(MYON_Font::NameLocale::English), 
    rhs_font->FamilyName(MYON_Font::NameLocale::English),
    true
  );
}

int MYON_FontList::CompareEnglishFamilyAndFaceName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  int rc = MYON_wString::CompareOrdinal(
    lhs_font->FamilyName(MYON_Font::NameLocale::English), 
  ...
  if ( 0 == rc )
    rc = MYON_wString::CompareOrdinal(
    lhs_font->FaceName(MYON_Font::NameLocale::English), 
    rhs_font->FaceName(MYON_Font::NameLocale::English),
    true
  );
  return rc;
}

int MYON_FontList::CompareEnglishWindowsLogfontName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
    lhs_font->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst), 
...
int MYON_FontList::CompareQuartetName(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_wString::CompareOrdinal(
...
int MYON_FontList::CompareWeightStretchStyle(
  MYON_Font const* const* lhs, 
  MYON_Font const* const* rhs
  )
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
...
int MYON_FontList::CompareStretch(
  MYON_Font const* const* lhs, 
  MYON_Font const* const* rhs
  )
{
...
int MYON_FontList::CompareUnderlinedStrikethroughPointSize(
  MYON_Font const* const* lhs, 
  MYON_Font const* const* rhs
  )
{
...
int MYON_FontList::CompareFontCharacteristicsHash(
  MYON_Font const* const* lhs,
  MYON_Font const* const* rhs
)
{
  MYON_ManagedFonts_CompareFontPointer(lhs, rhs);
  return MYON_SHA1_Hash::Compare(lhs_font->FontCharacteristicsHash(), rhs_font->FontCharacteristicsHash());
}

//
// END list of platform MYON_Fonts
//
//////////////////////////////////////////////////////////////////////////


bool MYON_Font::ModificationPermitted(
    const char* function_name,
    const char* file_name,
    int line_number
  ) const
{
  if (this == &MYON_Font::Default)
    MYON_ErrorEx(file_name, line_number, function_name, "MYON_Font::Default cannot be modified.");
  else if (this == &MYON_Font::Unset)
    MYON_ErrorEx(file_name, line_number, function_name, "MYON_Font::Unset cannot be modified.");
  else if (IsManagedFont())
    MYON_ErrorEx(file_name, line_number, function_name, "Managed fonts cannot be modified.");
  else
  {
...
//// V6 files 4F0F51FB-35D0-4865-9998-6D2C6A99721D is the class id for MYON_TextStyle
////MYON_OBJECT_IMPLEMENT( MYON_Font, MYON_Object, "4F0F51FB-35D0-4865-9998-6D2C6A99721D" );
//MYON_OBJECT_IMPLEMENT( MYON_Font, MYON_Object, "5F7476D1-798A-4953-B359-4A37699CD6F4" );

const MYON_Font* MYON_Font::ManagedFont() const
{
  return 
    IsManagedFont()
    ? this
    : MYON_Font::GetManagedFont(*this, true);
}

const MYON_FontGlyph* MYON_Font::CodePointGlyph(
  MYON__UINT32 unicode_codepoint
) const
{
  const MYON_Font* managed_font = ManagedFont();
  if (nullptr == managed_font)
    return nullptr;
...
const MYON_Font* MYON_Font::GetManagedFont(
  const MYON_Font& font_characteristics,
  bool bCreateIfNotFound
)
{
  ...
  return MYON_ManagedFonts::List.GetFromFontCharacteristics(font_characteristics,bCreateIfNotFound);
}

const MYON_Font* MYON_Font::GetManagedFontFromSerialNumber(
  unsigned int managed_font_runtime_serial_number
  )
{
  return MYON_ManagedFonts::List.GetFromSerialNumber(managed_font_runtime_serial_number);
}

unsigned int MYON_Font::GetManagedFontList(
  MYON_SimpleArray< const MYON_Font* >& managed_fonts
  )
{
  const MYON_SimpleArray< const MYON_Font* >& a = MYON_ManagedFonts::ManagedFonts().ByIndex();
  if (&a != &managed_fonts)
    managed_fonts = a;
  return managed_fonts.UnsignedCount();
}

unsigned int MYON_Font::GetInstalledFontList(
  MYON_SimpleArray< const MYON_Font* >& installed_fonts
  )
{
  const MYON_SimpleArray< const MYON_Font* >& a = MYON_ManagedFonts::InstalledFonts().ByFamilyName();
  if (&a != &installed_fonts)
    installed_fonts = a;
  return installed_fonts.UnsignedCount();
}

unsigned int MYON_Font::GetInstalledFontFamily(
...
)
{
  ...
  const MYON_FontList& installed_font_list = MYON_ManagedFonts::InstalledFonts();
...
static const MYON_wString Internal_NameOverlapCleanName(
  const wchar_t* s,
  bool bNoiseFilter
)
{
  ...
  MYON_wString clean_name;
  wchar_t buffer[128];
  int buffer_count = 0;
  for (wchar_t c = *(s = Internal_NameOverLapSkipNoise(s,bNoiseFilter)); 0 != c; c = *(s = Internal_NameOverLapSkipNoise(++s,bNoiseFilter)))
  {
    buffer[buffer_count++] = MYON_wString::MapCharacterOrdinal(MYON_StringMapOrdinalType::MinimumOrdinal, c);
    if (127 == buffer_count)
    {
...
)
{
  ...
  for (
    ...
    0 != *clean_s && *clean_s == MYON_wString::MapCharacterOrdinal(MYON_StringMapOrdinalType::LowerOrdinal,*s);
    s = Internal_NameOverLapSkipNoise(s,bNoiseFilter)
    )
  {
    overlap_count++;
...
static bool Internal_EqualLogfontName(
  const MYON_wString& clean_logfont_name,
  const MYON_Font* candidate_font
)
{
  ...
  if (clean_logfont_name.IsEmpty())
    return false;
  const MYON_wString installed_font_logfont_name = Internal_NameOverlapCleanName(candidate_font->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst),true);
  const bool bEqualLogfontName = MYON_wString::EqualOrdinal(clean_logfont_name, installed_font_logfont_name, true);
  return bEqualLogfontName;
}

static bool Internal_EqualFamilyName(
  const MYON_wString& clean_family_name,
  const MYON_Font* installed_font,
  MYON_wString& installed_font_family_name
)
{
  if (nullptr == installed_font)
    return false;
  installed_font_family_name = Internal_NameOverlapCleanName(installed_font->FamilyName(),true);
  const bool bEqualFamileName = MYON_wString::EqualOrdinal(clean_family_name, installed_font_family_name, true);
...
public:
  ...
  Internal_FontDelta(
    const MYON_Font& target_font,
    const MYON_wString& target_clean_logfont_name,
    const MYON_wString& target_clean_family_name,
    const MYON_Font* candidate_font
  )
    : m_candidate_font(candidate_font)
  {
    ...
    // are exact matches.
    const MYON_wString target_postscript_name = target_font.PostScriptName();
    const bool bEqualPostScriptName
      = target_postscript_name.IsNotEmpty()
      && MYON_wString::EqualOrdinal(target_postscript_name, m_candidate_font->PostScriptName(), true);

    const bool bEqualLogfontName
      = bEqualPostScriptName
      || (
         MYON_Font::Origin::WindowsFont == target_font.FontOrigin()
         && MYON_Font::Origin::WindowsFont == candidate_font->FontOrigin()
    ...
    MYON_wString candidate_clean_family_name;
    ...
    m_hash_delta 
      = (0 == m_style_delta || MYON_Font::Style::Upright != target_font.FontStyle())
      ? 4
      : 5;
  }
  ...
  const MYON_Font* m_candidate_font = nullptr;
...
const MYON_Font* MYON_Font::BestMatch(
  MYON_Font const*const* candidate_font_list,
...
) const
{
  ...
  const MYON_wString target_clean_logfont_name = Internal_NameOverlapCleanName(WindowsLogfontName(),true);
  const MYON_wString target_clean_family_name = Internal_NameOverlapCleanName(FamilyName(),true);
 
  Internal_FontDelta best_match;
  
  for (size_t i = 0; i < candidate_font_count; i++)
  {
    const MYON_Font* candidate_font = candidate_font_list[i];
    if (nullptr == candidate_font)
      continue;
...
unsigned int MYON_Font::WeightStretchStyleDeviation(
...
{
  if (MYON_Font::Weight::Unset == prefered_weight)
    prefered_weight = available_weight;
  if (MYON_Font::Stretch::Unset == prefered_stretch)
    prefered_stretch = available_stretch;
  if (MYON_Font::Style::Unset == prefered_style)
...
)
{
  ...
  return MYON_Font::WeightStretchStyleDeviation(
    prefered_weight, 
    prefered_stretch, 
    prefered_style,
...
)
{
  if (nullptr == prefered_weight_stretch_style)
    prefered_weight_stretch_style = &MYON_Font::Default;
  MYON_Font::Weight prefered_weight = prefered_weight_stretch_style->FontWeight();
  MYON_Font::Stretch prefered_stretch = prefered_weight_stretch_style->FontStretch();
  MYON_Font::Style prefered_style = prefered_weight_stretch_style->FontStyle();
  return MYON_Font::WeightStretchStyleDeviation(prefered_weight, prefered_stretch, prefered_style, available_font);
}

unsigned int MYON_Font::UnderlinedStrikethroughDeviation(
...
{
  return MYON_Font::UnderlinedStrikethroughDeviation(
    bPreferedUnderline,
...
unsigned int MYON_Font::RichTextPropertyDeviation(
...
)
{
  if (nullptr == available_font)
    return 0xFFFFFFFF;
  return MYON_Font::RichTextPropertyDeviation(
    bPreferedRtfBold,
    bPreferedItalic,
    bPreferedUnderline,
    bPreferedStrikethrough,
...
{
  const MYON_Font::Weight prefered_weight = bPreferedRtfBold ? MYON_Font::Weight::Bold : MYON_Font::Weight::Normal;
  const MYON_Font::Weight available_weight = bAvailableRtfBold ? MYON_Font::Weight::Bold : MYON_Font::Weight::Normal;

  const MYON_Font::Style prefered_style = bPreferedItalic ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;
  const MYON_Font::Style available_style = bPreferedItalic ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;

  const unsigned int wss_dev = MYON_Font::WeightStretchStyleDeviation(
    prefered_weight, MYON_Font::Stretch::Medium, prefered_style,
    available_weight, MYON_Font::Stretch::Medium, available_style
  );

  const unsigned int us_dev = MYON_Font::UnderlinedStrikethroughDeviation(
    bPreferedUnderline,
    bPreferedStrikethrough,
    bAvailableUnderline,
...
const MYON_Font* MYON_Font::Internal_BestMatchWeightStretchStyle(
  MYON_Font::Weight prefered_weight,
  MYON_Font::Stretch prefered_stretch,
  MYON_Font::Style prefered_style,
...
)
{
  ...
  const MYON_Font* best_font = nullptr;
  unsigned int best_delta = 0xFFFFFFFF;
  for (size_t i = 0; i < font_count; i++)
  {
    const MYON_Font* font = font_list[i];
    if (nullptr == font)
      continue;
    const unsigned int delta = MYON_Font::WeightStretchStyleDeviation(
      prefered_weight, 
      prefered_stretch, 
      prefered_style,
      font
...
const MYON_FontList& MYON_Font::ManagedFontList()
{
  return MYON_ManagedFonts::ManagedFonts();
}

const MYON_FontList& MYON_Font::InstalledFontList()
{
  return MYON_ManagedFonts::InstalledFonts();
}

const MYON_Font* MYON_Font::InstalledFontFromRichTextProperties(
  const wchar_t* rtf_font_name,
  bool bRtfBold,
  bool bRtfItalic
)
{
  MYON_wString local_rtf_font_name(rtf_font_name);
  local_rtf_font_name.TrimLeftAndRight();
  if (local_rtf_font_name.IsEmpty())
    local_rtf_font_name = MYON_Font::Default.RichTextFontName();
  ...
  const MYON_Font::Weight preferred_weight = bRtfBold ? MYON_Font::Weight::Semibold : MYON_Font::Weight::Medium;

  // The preferred_style is a bias used to pick between faces that have matching name properties.
  // An exact match is not required because bRequireStyleMatch = false;
  const MYON_Font::Style preferred_style = bRtfItalic ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;
  ...
  const MYON_Font* installed_font = MYON_Font::InstalledFontList().FromNames(
    ignore_postscript_name,
    rtf_font_name, // windows_logfont_name,
    ignore_family_name,
    ignore_prefered_face_name,
    preferred_weight, MYON_Font::Stretch::Unset, preferred_style,
  ...
  while (nullptr == installed_font)
  {
    ...
    installed_font = MYON_Font::InstalledFontList().FromNames(
    ...
    const wchar_t hyphen[2] = {MYON_wString::HyphenMinus,0};
    for (const wchar_t* separator = rtf_font_name + 1; 0 != *separator; ++separator)
    {
      if (MYON_wString::Space != *separator && MYON_wString::HyphenMinus != *separator)
        continue;
      MYON_wString family_name(rtf_font_name, (int)(separator - rtf_font_name));
      family_name.TrimLeftAndRight();
      family_name.TrimLeftAndRight(hyphen);
      family_name.TrimLeftAndRight();
      if (family_name.IsEmpty())
        continue;
      MYON_wString face_name(separator + 1);
  ...
  if (nullptr != installed_font)
  {
    const MYON_FontFaceQuartet q = installed_font->InstalledFontQuartet(); // only look at installed quartet!
    const MYON_Font* f = q.ClosestFace(bRtfBold, bRtfItalic);
...
static void Internal_SetFakeNamesFromExistingNames(
  MYON_wString loc_existing,
  MYON_wString en_existing,
  MYON_wString& loc_fake,
  MYON_wString& en_fake
)
{
...
const MYON_Font* MYON_Font::Internal_DecoratedFont(
  bool bUnderlined,
  bool bStrikethrough
) const
{
  ...
  MYON_Font decorated(*this);
  decorated.SetUnderlined(bUnderlined);
  decorated.SetStrikethrough(bStrikethrough);
  const MYON_Font* decorated_font = decorated.ManagedFont();
  if (nullptr != decorated_font && MYON_FontFaceQuartet::Member::Unset == decorated_font->m_quartet_member)
  {
...
const MYON_Font* MYON_Font::FontFromRichTextProperties(
...
)
{
  ...
  if (rich_text_font_name.IsEmpty())
    rich_text_font_name = MYON_Font::Default.RichTextFontName();

  const MYON_FontFaceQuartet::Member rich_text_quartet_face = MYON_FontFaceQuartet::MemberFromBoldAndItalic(bBoldQuartetMember, bItalicQuartetMember);

  MYON_FontFaceQuartet installed_quartet = MYON_ManagedFonts::InstalledFonts().QuartetFromQuartetName(rich_text_font_name);
  if ( installed_quartet.IsNotEmpty() )
  {
    ...
    const MYON_Font* installed_font = installed_quartet.ClosestFace(rich_text_quartet_face); // DO NOT CHANGE TO Face()
    if (nullptr != installed_font)
      return installed_font->Internal_DecoratedFont( bUnderlined, bStrikethrough);
  }

  const MYON_FontFaceQuartet managed_substitute_quartet = MYON_ManagedFonts::ManagedFonts().QuartetFromQuartetName(rich_text_font_name);
  if (managed_substitute_quartet.IsNotEmpty())
  {
    ...
    const MYON_Font* managed_quartet_face = managed_substitute_quartet.Face(rich_text_quartet_face); // DO NOT CHANGE TO ClosestFace()
  ...
  MYON_MemoryAllocationTracking disable_tracking(false);
  {
    ...
    const MYON_wString local_str(rich_text_font_name);
    rich_text_font_name = MYON_wString::EmptyString;
    rich_text_font_name = MYON_wString(static_cast<const wchar_t*>(local_str));
  }

  const MYON_Font* installed_font = nullptr;
  installed_font = MYON_Font::InstalledFontFromRichTextProperties(rich_text_font_name, bBoldQuartetMember, bItalicQuartetMember);
  if (nullptr == installed_font)
  {
#if defined(MYON_RUNTIME_APPLE)
    if (nullptr == installed_font)
      installed_font = MYON_Font::InstalledFontList().FromPostScriptName(rich_text_font_name);
#endif
    if (nullptr == installed_font)
      installed_font = MYON_Font::InstalledFontList().FromWindowsLogfontName(rich_text_font_name);
#if !defined(MYON_RUNTIME_APPLE)
    if (nullptr == installed_font)
      installed_font = MYON_Font::InstalledFontList().FromPostScriptName(rich_text_font_name);
#endif
    if (nullptr == installed_font)
      installed_font = MYON_Font::InstalledFontList().FromFamilyName(rich_text_font_name, MYON_FontFaceQuartet::MemberToString(MYON_FontFaceQuartet::Member::Regular) );
    if (nullptr != installed_font)
    {
      installed_quartet = installed_font->InstalledFontQuartet(); // only look at installed quartet!
      const MYON_Font* installed_rtfface = installed_quartet.ClosestFace(bBoldQuartetMember, bItalicQuartetMember);
  ...
  MYON_wString loc_quartet_name = rich_text_font_name;
  MYON_wString en_quartet_name = rich_text_font_name;

  MYON_wString loc_family_name = rich_text_font_name;
  MYON_wString en_family_name = rich_text_font_name;

  MYON_wString loc_windows_logfont_name = rich_text_font_name;
  MYON_wString en_windows_logfont_name = rich_text_font_name;

  MYON_Font::Weight fake_normal_weight = MYON_Font::Weight::Unset;
  MYON_Font::Weight fake_bold_weight = MYON_Font::Weight::Unset;

  if (managed_substitute_quartet.IsNotEmpty())
  {
    // get known names and guesses at weights from the existing parts of the quartet.

    const MYON_Font* normal_weight_font = managed_substitute_quartet.Face(MYON_FontFaceQuartet::Member::Regular);
    if ( nullptr == normal_weight_font)
      normal_weight_font = managed_substitute_quartet.Face(MYON_FontFaceQuartet::Member::Italic);

    const MYON_Font* bold_weight_font = managed_substitute_quartet.Face(MYON_FontFaceQuartet::Member::Bold);
    if (nullptr == bold_weight_font)
      bold_weight_font = managed_substitute_quartet.Face(MYON_FontFaceQuartet::Member::BoldItalic);

    const MYON_Font* names_font = (nullptr != normal_weight_font) ? normal_weight_font : bold_weight_font;

    if (nullptr != names_font)
    {
      Internal_SetFakeNamesFromExistingNames(
        names_font->QuartetName(MYON_Font::NameLocale::Localized),
        names_font->QuartetName(MYON_Font::NameLocale::English),
        loc_quartet_name,
        en_quartet_name
      );

      Internal_SetFakeNamesFromExistingNames(
        names_font->FamilyName(MYON_Font::NameLocale::Localized),
        names_font->FamilyName(MYON_Font::NameLocale::English),
        loc_family_name,
        en_family_name
      );

      Internal_SetFakeNamesFromExistingNames(
        names_font->WindowsLogfontName(MYON_Font::NameLocale::Localized),
        names_font->WindowsLogfontName(MYON_Font::NameLocale::English),
    ...
    const unsigned default_normal_unsigned = static_cast<unsigned>(MYON_Font::Weight::Normal);
    const unsigned default_bold_unsigned = static_cast<unsigned>(MYON_Font::Weight::Bold);
    const unsigned min_weight = static_cast<unsigned>(MYON_Font::Weight::Thin);
    const unsigned max_weight = static_cast<unsigned>(MYON_Font::Weight::Heavy);

    if (MYON_Font::Weight::Unset != fake_normal_weight && MYON_Font::Weight::Unset == fake_bold_weight)
    {
      // have to guess at a bold weight
      fake_bold_weight
        = normal_unsigned < default_bold_unsigned
        ? MYON_Font::Weight::Bold
        : ((normal_unsigned + 2 <= max_weight) ? MYON_Font::FontWeightFromUnsigned(normal_unsigned + 2) : MYON_Font::Weight::Heavy)
        ;
    }

    if (MYON_Font::Weight::Unset != fake_bold_weight && MYON_Font::Weight::Unset == fake_normal_weight)
    {
      // have to guess at a normal weight
      fake_normal_weight
        = bold_unsigned > default_normal_unsigned
        ? MYON_Font::Weight::Normal
        : ((min_weight + 2 <= bold_unsigned) ? MYON_Font::FontWeightFromUnsigned(bold_unsigned - 2) : MYON_Font::Weight::Thin)
        ;
    }
  }

  if (MYON_Font::Weight::Unset == fake_normal_weight)
    fake_normal_weight = MYON_Font::Weight::Normal;
  if (MYON_Font::Weight::Unset == fake_bold_weight)
    fake_bold_weight = MYON_Font::Weight::Bold;

  const double point_size = 0.0;
  const unsigned int logfont_charset = MYON_Font::Default.LogfontCharSet();
  ...
  fake_font.SetFontCharacteristics(
    ...
    bItalicQuartetMember ? MYON_Font::Style::Italic : MYON_Font::Style::Upright,
    MYON_Font::Stretch::Medium,
    false, // DO NOT PASS bUnderlined here
    false, // DO NOT PASS bStrikethrough here
    MYON_FontMetrics::DefaultLineFeedRatio,
    logfont_charset
  );
  ...
  fake_font.m_loc_face_name = MYON_FontFaceQuartet::MemberToString(rich_text_quartet_face);
  fake_font.m_en_face_name = fake_font.m_loc_face_name;

  // There is no reliable way to get the PostScript name 
  fake_font.m_loc_postscript_name = MYON_wString::EmptyString;
  fake_font.m_en_postscript_name = MYON_wString::EmptyString;
  ...
  const MYON_Font* existing_managed_font = MYON_Font::GetManagedFont(fake_font, false);

  if (nullptr != existing_managed_font)
  {
    Internal_SetFakeNamesFromExistingNames(
      existing_managed_font->QuartetName(MYON_Font::NameLocale::Localized),
      existing_managed_font->QuartetName(MYON_Font::NameLocale::English),
      loc_quartet_name,
      en_quartet_name
    );

    Internal_SetFakeNamesFromExistingNames(
      existing_managed_font->FamilyName(MYON_Font::NameLocale::Localized),
      existing_managed_font->FamilyName(MYON_Font::NameLocale::English),
      loc_family_name,
      en_family_name
    );

    Internal_SetFakeNamesFromExistingNames(
      existing_managed_font->WindowsLogfontName(MYON_Font::NameLocale::Localized),
      existing_managed_font->WindowsLogfontName(MYON_Font::NameLocale::English),
      loc_windows_logfont_name,
      en_windows_logfont_name
    );
  ...
  // they are needed.
  const MYON_Font* managed_fake_font = MYON_Font::GetManagedFont(fake_font, true);

  if (nullptr == managed_fake_font)
  {
    // should never happen
    return MYON_Font::Default.Internal_DecoratedFont(bUnderlined, bStrikethrough);
  }
  ...
  const MYON_Font* installed_substitute = managed_fake_font->SubstituteFont();
  ...
  {
    // We have better information to select the correct substitute than inside the MYON_Font::GetManagedFont(fake_font, true) call above.
    // Use this information to specify a better substitute font.
    installed_substitute = MYON_Font::Default.InstalledFontQuartet().ClosestFace(rich_text_quartet_face); // only look at installed quartet
    if (nullptr == installed_substitute)
      installed_substitute = &MYON_Font::Default;
    const bool bInstalledFontIsASubstitute = true;
    MYON_Font::Internal_SetManagedFontInstalledFont(
      managed_fake_font,
...
const MYON_wString MYON_Font::RichTextPropertiesToString(
  bool bRtfBold,
  bool bRtfItalic,
  bool bRtfUnderlined,
...
)
{
  return RichTextPropertiesToString(
    MYON_Font::IsBoldWeight(rtf_weight), 
    MYON_Font::Style::Italic == rtf_style,
    bRtfUnderlined,
    bRtfStrikethrough
  );
}
...
const MYON_Font* MYON_Font::ManagedFontFromRichTextProperties(
  const wchar_t* rtf_font_name,
  bool bRtfBold,
  bool bRtfItalic,
  bool bRftUnderlined,
...
{
  // MYON_Font::ManagedFontFromRichTextProperties() is deprecated.
  // MYON_Font::FontFromRichTextProperties() is a single point source for
  // converting rich text font properties into a managed font.
  return MYON_Font::FontFromRichTextProperties(rtf_font_name, bRtfBold, bRtfItalic, bRftUnderlined, bRftStrikethrough);
}

const MYON_2dex MYON_FontList::Internal_SearchSortedList(
  const MYON_Font* key,
...
)
{
  for (;;)
  {
    ...
    while (i1 < sorted_count && 0 == compare_func(&key, &(sorted_font_list[i1]) ))
      i1++;
    return MYON_2dex(i0, i1);
  }

  return MYON_2dex(MYON_UNSET_INT_INDEX,MYON_UNSET_INT_INDEX);

}

void MYON_FontList::Internal_EmptyLists()
{
...
static int Internal_CompareLogfontNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareWindowsLogfontName(lhs, rhs);
  if (0 != rc)
    return rc;

  rc = MYON_FontList::CompareWeightStretchStyle(lhs, rhs);
  if (0 != rc)
    return rc;

  rc = MYON_FontList::CompareUnderlinedStrikethroughPointSize(lhs, rhs);
  if (0 != rc)
    return rc;

  rc = MYON_FontList::CompareFontCharacteristicsHash(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_CompareFamilyNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareFamilyAndFaceName(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_ComparePostScriptNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::ComparePostScriptName(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_CompareEnglishLogfontNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareEnglishWindowsLogfontName(lhs, rhs);
  if (0 != rc)
    return rc;
  ...
  return MYON_FontList::CompareFontCharacteristicsHash(lhs, rhs);
}

static int Internal_CompareEnglishFamilyNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareEnglishFamilyAndFaceName(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_CompareEnglishPostScriptNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareEnglishPostScriptName(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_CompareQuartetNameEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareQuartetName(lhs, rhs);
  if (0 != rc)
    return rc;
...
static int Internal_CompareFontCharacteristicsHashEtc(MYON_Font const* const* lhs, MYON_Font const* const* rhs)
{
  int rc = MYON_FontList::CompareFontCharacteristicsHash(lhs, rhs);
  if (0 != rc)
    return rc;
...
void MYON_FontList::Internal_UpdateSortedLists() const
{
  const int unsorted_count = m_unsorted.Count();
  if (unsorted_count <= 0)
    return;

  MYON_SimpleArray< const MYON_Font* >* sorted_lists[8] =
  {
  ...
  MYON_FontPtrCompareFunc compare_funcs[8] =
  {
  ...
  {
    MYON_SimpleArray< const MYON_Font* >& sorted_list = *(sorted_lists[array_dex]);

    bool bNeedSort = false;
    sorted_list.Reserve(sorted_list.Count() + unsorted_count);
    for (int j = 0; j < unsorted_count; j++)
    {
      const MYON_Font* font = m_unsorted[j];
      if (nullptr == font)
        continue;
      if (0 == array_dex)
      {
      ...
      {
        const MYON_wString en = font->PostScriptName(MYON_Font::NameLocale::English);
        if (en.IsEmpty())
          continue;
        if (MYON_wString::EqualOrdinal(en, font->PostScriptName(MYON_Font::NameLocale::English), true))
          continue;
      }
      else if (4 == array_dex)
      {
        const MYON_wString en = font->WindowsLogfontName(MYON_Font::NameLocale::English);
        if (en.IsEmpty())
          continue;
        if (MYON_wString::EqualOrdinal(en, font->WindowsLogfontName(MYON_Font::NameLocale::English), true))
          continue;
      }
      else if (5 == array_dex)
      {
        const MYON_wString en = font->FamilyName(MYON_Font::NameLocale::English);
        if (en.IsEmpty())
          continue;
        if (MYON_wString::EqualOrdinal(en, font->FamilyName(MYON_Font::NameLocale::English), true))
          continue;
      }      
      else if (6 == array_dex)
      {
        const MYON_wString qname = font->QuartetName();
        if (qname.IsEmpty())
          continue;
      ...
      {
        const MYON_SHA1_Hash sha1 = font->FontCharacteristicsHash();
        if (sha1.IsZeroDigestOrEmptyContentHash())
          continue; // no valid font wil have either one of these hashes.
      }
      else
      {
        MYON_ERROR("When you add an array to MYON_FontListImpl, you must add a corresponding if clause here.");
      }
...
MYON_FontList::MYON_FontList()
  : m_sorted(*(new MYON_FontListImpl()))
{}
...
MYON_FontList::~MYON_FontList()
{
  MYON_FontListImpl* ptr = &m_sorted;
  if (nullptr != ptr)
  {
    delete ptr;
  }
...
unsigned int MYON_FontList::Count() const
{
  return m_by_index.UnsignedCount();
}

MYON_Font::NameLocale MYON_FontList::NameLocale() const
{
  return m_name_locale;
}

const MYON_Font* MYON_FontList::FromFontCharacteristicsHash(
  ...
  bool bReturnFirst) const
{
  ...
  const MYON_SimpleArray<const MYON_Font*>& by_hash = ByFontCharacteristicsHash();

  // MYON_FontList::CompareFontCharacteristicsHash() only looks at m_font_characteristics_hash
  // and m_font_characteristics_hash must not be zero content hash (handled above).
  const MYON_Font keyf;
  keyf.m_font_characteristics_hash = font_characteristics_hash;
  const MYON_Font* key = &keyf;

  const int i = by_hash.BinarySearch(&key, MYON_FontList::CompareFontCharacteristicsHash);
  const int count = by_hash.Count();
  if (i < 0 || i >= count)
    return nullptr;

  int i0 = i;
  while (i0 > 0 && 0 == MYON_FontList::CompareFontCharacteristicsHash(&key, &by_hash[i0 - 1]))
    --i0;

  int i1 = i;
  while (i1 + 1 < count && 0 == MYON_FontList::CompareFontCharacteristicsHash(&key, &by_hash[i1 + 1]))
    ++i1;
...
const MYON_Font* MYON_FontList::FromPostScriptName(
  const wchar_t* postscript_name
) const
{
  return FromPostScriptName(postscript_name, MYON_Font::Weight::Normal, MYON_Font::Stretch::Condensed, MYON_Font::Style::Upright, false, false);
}
...
const MYON_Font* MYON_FontList::FromWindowsLogfontName(
  const wchar_t* windows_logfont_name
) const
{
  return FromWindowsLogfontName(windows_logfont_name, MYON_Font::Weight::Normal, MYON_Font::Stretch::Condensed, MYON_Font::Style::Upright);
}
...
const MYON_Font* MYON_FontList::FromFamilyName(
  const wchar_t* family_name,
  const wchar_t* prefered_face_name
) const
{
  return FromFamilyName(family_name, prefered_face_name, MYON_Font::Weight::Normal, MYON_Font::Stretch::Medium, MYON_Font::Style::Upright);
}
...
const MYON_Font* MYON_FontList::FromRichTextProperties(
  const wchar_t* rtf_font_name,
  bool bRtfBold,
  bool bRtfItalic,
  bool bUnderlined,
...
) const
{
  ...
  const MYON_Font::Weight prefered_weight = bRtfBold ? MYON_Font::Weight::Bold : MYON_Font::Weight::Normal;
  const MYON_Font::Stretch prefered_stretch = MYON_Font::Stretch::Medium;
  const MYON_Font::Style prefered_style = bRtfItalic ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;
...
const MYON_Font* MYON_FontList::FromNames(
  const wchar_t* postscript_name,
  const wchar_t* windows_logfont_name,
  const wchar_t* family_name,
  const wchar_t* prefered_face_name,
...
const MYON_Font* MYON_FontList::Internal_FromNames(
  const wchar_t* postscript_name,
  const wchar_t* windows_logfont_name,
  const wchar_t* family_name,
  const wchar_t* prefered_face_name,
...
) const
{
  ...
  if (!(point_size > 0.0 && point_size < MYON_UNSET_POSITIVE_FLOAT))
    point_size = 0.0;

  if (false == m_bMatchUnderlineStrikethroughAndPointSize)
    bMatchUnderlineStrikethroughAndPointSize = false;
  
  const MYON_SimpleArray< const MYON_Font* > * sorted_lists[16] = {};

  MYON_FontPtrCompareFunc compare_funcs[16] = { 0 };
  ...
  if (bKeyHasFamilyAndFace)
  {
    ...
    compare_funcs[pass_count] = MYON_FontList::CompareFamilyAndFaceName;
    pass_count++;

    sorted_lists[pass_count] = &m_sorted.m_by_english_family_name;
    compare_funcs[pass_count] = MYON_FontList::CompareEnglishFamilyAndFaceName;
    pass_count++;
  }
  ...
  if (bKeyWindowsLogfontName)
  {
    ...
    compare_funcs[pass_count] = MYON_FontList::CompareWindowsLogfontName;
    pass_count++;

    sorted_lists[pass_count] = &m_sorted.m_by_english_windows_logfont_name;
    compare_funcs[pass_count] = MYON_FontList::CompareEnglishWindowsLogfontName;
    pass_count++;
  }
#endif
  ...
  if (bKeyHasPostScriptName)
  {
    ...
    compare_funcs[pass_count] = MYON_FontList::ComparePostScriptName;
    postscript_name_pass[0] = pass_count;
    pass_count++;

    sorted_lists[pass_count] = &m_sorted.m_by_english_postscript_name;
    compare_funcs[pass_count] = MYON_FontList::CompareEnglishPostScriptName;
  ...
  {
    sorted_lists[pass_count] = &m_sorted.m_by_family_name;
    compare_funcs[pass_count] = MYON_FontList::CompareFamilyName;
    pass_count++;

    sorted_lists[pass_count] = &m_sorted.m_by_english_family_name;
    compare_funcs[pass_count] = MYON_FontList::CompareEnglishFamilyName;
    pass_count++;
  }
  ...
  const MYON_Font* font = nullptr;
  unsigned int font_dev = 0xFFFFFFFF;

  const MYON_Font* pkey = &key;

  const MYON_Font* postscript_name_match_candidate = nullptr;

  for (int pass = 0; pass < pass_count; pass++)
  {
    MYON_FontPtrCompareFunc compare_func = compare_funcs[pass];
    if (nullptr == compare_func)
      continue;

    const MYON_Font* candidate = nullptr;
    ...
    for (int list_dex = 0; list_dex < 2; list_dex++)
    {
      ...
      const MYON_SimpleArray< const MYON_Font* >& sorted_list 
        = (1==list_dex)
        ? *(sorted_lists[pass])
        : m_unsorted;

      const MYON_2dex subset 
        = (1==list_dex)
        ? MYON_FontList::Internal_SearchSortedList(&key, compare_func, sorted_list)
        : MYON_2dex(0,m_unsorted.Count())
        ;
      ...
      for (int i = subset.i; i < subset.j; i++)
      {
        ...
        const bool bCandidateFamilyAndFaceMatch 
          ...
          && (MYON_Font::EqualFontFamilyAndFace(&key, candidate) || 0 == MYON_FontList::CompareEnglishFamilyAndFaceName(&pkey,&candidate));
        ...
        const bool bFontFamilyAndFaceMatch 
          = bKeyHasFamilyAndFace
          && (nullptr != font) 
          && (MYON_Font::EqualFontFamilyAndFace(&key, font) || 0 == MYON_FontList::CompareEnglishFamilyAndFaceName(&pkey,&candidate));

        if (bFontFamilyAndFaceMatch && false == bCandidateFamilyAndFaceMatch)
          continue;

        candidate_dev = MYON_Font::WeightStretchStyleDeviation(prefered_weight, prefered_stretch, prefered_style, candidate);
  ...
  if (nullptr != postscript_name_match_candidate && font != postscript_name_match_candidate)
  {
    ...
    {
      // MYON Apple platforms, the postscript name is the most reliable indentification
      if (false == MYON_wString::EqualOrdinal(font->PostScriptName(), postscript_name_match_candidate->PostScriptName(), true))
        font = postscript_name_match_candidate;
    }
#endif
  }
...
unsigned int MYON_FontList::FontListFromNames(
  const wchar_t* postscript_name,
  const wchar_t* windows_logfont_name,
  const wchar_t* family_name,
...
) const
{
  ...
  {
    MYON_SimpleArray< const MYON_Font* >* sorted_list = nullptr;
    MYON_2dex subset(-1, -1);
    switch (pass)
    {
    case 0:
      ...
      subset = Internal_SearchSortedList(&key, MYON_FontList::ComparePostScriptName, *sorted_list);
      break;
    ...
    case 1:
      ...
      subset = Internal_SearchSortedList(&key, MYON_FontList::CompareWindowsLogfontName, *sorted_list);
      break;
    ...
    case 2:
      ...
      subset = Internal_SearchSortedList(&key, MYON_FontList::CompareFamilyName, *sorted_list);
      break;
    }
    ...
    for (int i = subset.i; i < subset.j; i++)
    {
      const MYON_Font* font = (*sorted_list)[i];
      if (
        pass < 1
        && key.m_loc_windows_logfont_name.IsNotEmpty()
        && 0 != MYON_FontList::CompareWindowsLogfontName(&pkey, &font)
        )
        continue;
      if (key.m_loc_family_name.IsNotEmpty())
      {
        if (0 != MYON_FontList::CompareFamilyName(&pkey, &font))
          continue;
        if (key.m_loc_face_name.IsNotEmpty()
          && false == MYON_wString::EqualOrdinal(key.FaceName(m_name_locale), font->FaceName(m_name_locale), true)
          )
          continue;
      }
...
const MYON_Font* MYON_FontList::FromFontProperties(
...
const MYON_Font* MYON_FontList::FamilyMemberWithWeightStretchStyle(
  const wchar_t* family_name,
...
) const
{
  ...
  key.m_loc_family_name.TrimLeftAndRight();
  if (key.m_loc_family_name.IsEmpty())
    key.m_loc_family_name = MYON_Font::DefaultFamilyName();
  else
    key.m_en_family_name = key.m_loc_family_name;
  ...
  const MYON_2dex subdex = Internal_SearchSortedList(&key, MYON_FontList::CompareFamilyName, m_sorted.m_by_family_name);
  if (subdex.j <= 0)
    return nullptr;
  const MYON_Font* candidate = nullptr;
  unsigned int candidate_dev = 0xFFFFFFFF;
  ...
  {
    const MYON_Font* font = m_sorted.m_by_family_name[i];
    if (nullptr == font)
      continue;
    unsigned int font_dev = MYON_Font::WeightStretchStyleDeviation(&key, font);    
...
) const
{
  ...
  if (MYON_Font::Weight::Unset == desired_weight)
    desired_weight = font->FontWeight();
  if (MYON_Font::Stretch::Unset == desired_stretch)
    desired_stretch = font->FontStretch();
  if (MYON_Font::Style::Unset == desired_style)
    desired_style = font->FontStyle();

  MYON_wString family_name = font->FamilyName();
  if (family_name.IsEmpty())
  {
    const MYON_SimpleArray< const MYON_Font* > * sorted_lists[2] = { &m_sorted.m_by_windows_logfont_name, &m_sorted.m_by_postscript_name };
    MYON_FontPtrCompareFunc compare_funcs[2] = {MYON_FontList::CompareWindowsLogfontName,MYON_FontList::ComparePostScriptName};
    ...
    for (int k = 0; k < k1 && family_name.IsEmpty(); k++)
    {
      if (bNameIsEmpty[k])
        continue;
      const MYON_SimpleArray< const MYON_Font* >& sorted_list = *sorted_lists[k];
      const MYON_2dex subdex = Internal_SearchSortedList(font, compare_funcs[k], sorted_list);
      if (subdex.j <= 0)
        continue;
      for (int i = subdex.i; i < subdex.j && family_name.IsEmpty(); i++)
      {
        const MYON_Font* f = sorted_list[i];
        if (nullptr == f)
          continue;
  ...
  const MYON_Font* family_member = FamilyMemberWithWeightStretchStyle(family_name, desired_weight, desired_stretch, desired_style);
  return family_member;
}

const MYON_Font* MYON_Font::InstalledFamilyMemberWithWeightStretchStyle(
  MYON_Font::Weight desired_weight,
  MYON_Font::Stretch desired_stretch,
  MYON_Font::Style desired_style
) const
{
  return MYON_Font::InstalledFontList().FamilyMemberWithWeightStretchStyle(
    this,
    desired_weight,
    desired_stretch,
    desired_style
...
const MYON_Font* MYON_Font::ManagedFamilyMemberWithRichTextProperties(
  bool bBold,
  bool bItalic,
  bool bUnderlined,
  bool bStrikethrough
) const
{
  ...
  // 
  // 1. If there's an obvious bug in MYON_Font::ManagedFontFromRichTextProperties(), fix it.
  // 
  ...
  // MYON_ManagedFonts::Internal_SetFakeWindowsLogfontNames(). That code is Apple specific.
  // 
  ...
  return MYON_Font::FontFromRichTextProperties(this->RichTextFontName(), bBold, bItalic, bUnderlined, bStrikethrough);
}

const MYON_Font* MYON_Font::ManagedFamilyMemberWithWeightStretchStyle(
  MYON_Font::Weight desired_weight,
  MYON_Font::Stretch desired_stretch,
  MYON_Font::Style desired_style,
  bool bUnderlined, 
...
) const
{
  ...
  const MYON_Font* font = this;

  if ( bChangeWeight || bChangeStretch || bChangeStyle )
  {
    const MYON_Font* installed_font = InstalledFamilyMemberWithWeightStretchStyle(desired_weight, desired_stretch, desired_style);
    if (nullptr != installed_font)
    {
      font = installed_font;
      bChangeWeight = false;
  ...
  MYON_Font changed_font(*font);

  if (bChangeWeight && MYON_Font::Weight::Unset != desired_weight)
    changed_font.SetFontWeight(desired_weight);

  if (bChangeStretch && MYON_Font::Stretch::Unset != desired_stretch)
    changed_font.SetFontStretch(desired_stretch);

  if (bChangeStyle && MYON_Font::Style::Unset != desired_style)
...
const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByPostScriptName() const
{
  Internal_UpdateSortedLists();
  return m_sorted.m_by_postscript_name;
}

const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByWindowsLogfontName() const
{
  Internal_UpdateSortedLists();
  return m_sorted.m_by_windows_logfont_name;
}

const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByFamilyName() const
{
  Internal_UpdateSortedLists();
  return m_sorted.m_by_family_name;
}

const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByQuartetName() const
{
  Internal_UpdateSortedLists();
  return m_sorted.m_by_quartet_name;
}

const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByFontCharacteristicsHash() const
{
  Internal_UpdateSortedLists();
  return m_sorted.m_by_font_characteristics_hash;
}

const MYON_SimpleArray< const class MYON_Font* >& MYON_FontList::ByIndex() const
{
  return m_by_index;
}
...
// for an english  name
static MYON_wString MYON_FontList__EnNameToLocName(const MYON_wString& english_name)
{
  const MYON_SimpleArray<const MYON_Font*>& fonts = MYON_Font::InstalledFontList().ByIndex();
  for (int i = 0; i < fonts.Count(); i++)
  {
    const MYON_Font* font = fonts[i];
    if (nullptr == font)
      continue;
    MYON_wString en_qname = font->QuartetName(MYON_Font::NameLocale::English);
    if (MYON_wString::CompareOrdinal(english_name, en_qname, true) == 0)
    {
...
const MYON_FontFaceQuartet MYON_FontList::QuartetFromQuartetName(
  const wchar_t* quartet_name
) const
{
  for (;;)
  {
    MYON_FontFaceQuartet qname(quartet_name, nullptr, nullptr, nullptr, nullptr);
    if (qname.QuartetName().IsEmpty())
      break;

    const MYON_ClassArray<MYON_FontFaceQuartet>& quartet_list = MYON_FontList::QuartetList();
    const int quartet_list_count = quartet_list.Count();
    int i = quartet_list.BinarySearch(&qname, MYON_FontFaceQuartet::CompareQuartetName);
    ...
    if (failedOnFirstTry)
    {
      ...
      MYON_wString locName = MYON_FontList__EnNameToLocName(quartet_name);
      if (locName.IsEmpty())
      {
        break;
      }
      qname = MYON_FontFaceQuartet(locName, nullptr, nullptr, nullptr, nullptr);
      i = quartet_list.BinarySearch(&qname, MYON_FontFaceQuartet::CompareQuartetName);
    ...
    while (i > 0 && 0 == MYON_FontFaceQuartet::CompareQuartetName(&qname, &quartet_list[i - 1]))
      i--;
    return quartet_list[i];
  }
  return MYON_FontFaceQuartet::Empty;
}

const MYON_Font* MYON_FontList::FontFromQuartetProperties(
  const wchar_t* quartet_name,
  bool bBold,
  bool bItalic
) const
{
  const MYON_FontFaceQuartet qname(quartet_name, nullptr, nullptr, nullptr, nullptr);
  if (qname.QuartetName().IsEmpty())
    return nullptr;
  ...
  do
  {
    const MYON_FontFaceQuartet& q = quartet_list[i];
    if (0 != MYON_FontFaceQuartet::CompareQuartetName(&qname, &q))
      break;
    const MYON_Font* font = q.Face(bBold, bItalic);
    if (nullptr != font)
      return font;
    i++;
...
MYON_3udex Internal_StretchSlantWeightDex(unsigned max_stretch_dex, unsigned max_weight_dex, const MYON_Font* f)
{
  for (;;)
  {
    if (nullptr == f)
    ...
    if (stretch_dex < 1 || stretch_dex >= max_stretch_dex)
      break;
    const unsigned slant_dex = (MYON_Font::Style::Italic == f->FontStyle() || MYON_Font::Style::Oblique == f->FontStyle())
      ? 1U
      : 0U;
    const unsigned weight_dex = static_cast<unsigned char>(f->FontWeight());
    if (weight_dex < 1 || weight_dex >= max_weight_dex)
      break;
    return MYON_3udex(stretch_dex, slant_dex, weight_dex);
  }
  return MYON_3udex(MYON_UNSET_UINT_INDEX, MYON_UNSET_UINT_INDEX, MYON_UNSET_UINT_INDEX);
}

const MYON_ClassArray< MYON_FontFaceQuartet >& MYON_FontList::QuartetList() const
{
  // call ByQuartetName() first to insure m_quartet_list[] is set correctly.
  const MYON_SimpleArray<const MYON_Font*>& a = this->ByQuartetName();
  ...
  const MYON_Font* fonts_by_ssw[11][2][11] = {};
  ...
  const unsigned medium_stretch_dex = (unsigned)static_cast<unsigned char>(MYON_Font::Stretch::Medium);

  MYON_SimpleArray<const MYON_Font*> regular_faces(8);
  MYON_SimpleArray<const MYON_Font*> bold_faces(8);
  MYON_SimpleArray<const MYON_Font*> italic_faces(8);
  MYON_SimpleArray<const MYON_Font*> bolditalic_faces(8);
  ...
  for (unsigned i = 0; i < font_count; i = (i < next_i) ? next_i : (i+1))
  {
    ...
    const MYON_wString quartet_name = f->QuartetName();
    ...
    const MYON_Font* upright_faces[2] = {}; // room to save up to 2 upright faces
    const MYON_Font* slanted_faces[2] = {}; // room to save up to 2 slanted faces
    ...
    for ( unsigned pass = 0; pass < pass_count; ++pass) for ( unsigned j = i; j < next_i; ++j)
    {
      ...
      const MYON_FontFaceQuartet::Member fm = f->m_quartet_member;

      // use f's stretch-slant-weight to add it to the fonts_by_ssw[][][] array.
      const MYON_3udex ssw_dex = Internal_StretchSlantWeightDex(max_stretch_dex, max_weight_dex, f);
    ...
    const MYON_Font* quartet_faces[2][2] = {};
    ...
    {
      if (2 == upright_face_count && MYON_Font::CompareWeight(upright_faces[0]->FontWeight(), upright_faces[1]->FontWeight()) > 0)
      {
        f = upright_faces[0];
        upright_faces[0] = upright_faces[1];
        upright_faces[1] = f;
      }
      if (2 == slanted_face_count && MYON_Font::CompareWeight(slanted_faces[0]->FontWeight(), slanted_faces[1]->FontWeight()) > 0)
      {
        f = slanted_faces[0];
    ...
    if (false == bHaveQuartetFaces)
    {
      ...
      // from Windows LOGFONT information when we create installed MYON_Fonts from 
      ...
      // quartets.
      // 
      // 4. Missing fonts that are created in MYON_Font::FontFromRichTextProperties() have the quartet
      ...
      if (stretch_dex_range[0] < stretch_dex_range[1])
      {
        ...
        // Pick the one closest to MYON_Font::Stretch::Medium with the most faces        
      ...
      const unsigned normal_weight_dex = (unsigned)static_cast<unsigned char>(MYON_Font::Weight::Normal);
      const unsigned medium_weight_dex = (unsigned)static_cast<unsigned char>(MYON_Font::Weight::Medium);
      const unsigned bold_weight_dex = (unsigned)static_cast<unsigned char>(MYON_Font::Weight::Bold);
    ...
    // If MYON_Font.m_quartet_member is not set or set incorrectly, 
    ...
    const MYON_FontFaceQuartet::Member member[2][2] = {
      {MYON_FontFaceQuartet::Member::Regular,MYON_FontFaceQuartet::Member::Bold},
      {MYON_FontFaceQuartet::Member::Italic,MYON_FontFaceQuartet::Member::BoldItalic}
    };
    for (int ii = 0; ii < 2; ++ii) for (int jj = 0; jj < 2; ++jj)
    {
      ...
      if (nullptr != f)
      {
        const MYON_FontFaceQuartet::Member m = f->m_quartet_member;
        if (MYON_FontFaceQuartet::Member::Unset == m)
          f->m_quartet_member = member[ii][jj];
        else if (m != member[ii][jj])
        {
          if (MYON_Font::FontType::InstalledFont != f->m_font_type)
    ...
    for (unsigned j = i; j < next_i; ++j)
    {
      ...
      const MYON_FontFaceQuartet::Member fm = f->m_quartet_member;
      if (MYON_FontFaceQuartet::Member::Unset == fm)
        continue;
      if (MYON_Font::FontType::InstalledFont != f->m_font_type)
        continue;
      ...
      f->m_quartet_member = MYON_FontFaceQuartet::Member::Unset;
    }
    ...
    if (decorated_fonts_count > 0)
    {
      ...
      for (unsigned j = i; j < next_i; ++j)
      {
        ...
        if (nullptr != cleanf)
        {
          const MYON_FontFaceQuartet::Member fm = cleanf->m_quartet_member;
    ...
    for (int ii = 0; ii < 2; ++ii) for (int jj = 0; jj < 2; ++jj)
    {
      ...
      // 
      // This is a case where a managed font, like MYON_Font::Default, 
      ...
      const MYON_Font* installed_font = f->Internal_ManagedFontToInstalledFont();
      if (nullptr == installed_font)
        continue;
      ...
      // MYON_Font::Default is a good example. 
      // If there 
      MYON_FontFaceQuartet installed_font_quartet = installed_font->InstalledFontQuartet();
    ...
    const MYON_FontFaceQuartet q(quartet_name, quartet_faces[0][0], quartet_faces[0][1], quartet_faces[1][0], quartet_faces[1][1]);
    if (q.IsEmpty())
      continue;
...
unsigned int MYON_FontList::AddFont(
  const MYON_Font* font,
...
)
{
  ...
  if (
    MYON_Font::Weight::Unset == font->FontWeight()
    || MYON_Font::Stretch::Unset == font->FontStretch()
    || MYON_Font::Style::Unset == font->FontStyle()
    )
    return false;

  if (bCheckforDuplicates)
  {
    const MYON_Font* f = FromFontProperties(font, true, true);
    if (
      nullptr != f
      ...
      && MYON_wString::EqualOrdinal(f->PostScriptName(m_name_locale), font->PostScriptName(m_name_locale), true)
      && MYON_wString::EqualOrdinal(f->WindowsLogfontName(m_name_locale), font->WindowsLogfontName(m_name_locale), true)
      && MYON_wString::EqualOrdinal(f->FamilyName(m_name_locale), font->FamilyName(m_name_locale), true)
      && MYON_wString::EqualOrdinal(f->FaceName(m_name_locale), font->FaceName(m_name_locale), true)
      )
    {
      return false;
    }
  ...
  if (count > 1)
  {
    ...
    {
      // font = m_by_index[i+1]
      const MYON_Font* a = m_by_index[i];
...
const MYON_Font* MYON_Font::InstalledFont(
  bool bAllowBestMatch
) const
{
  return IsInstalledFont()
    ? this
    : MYON_ManagedFonts::InstalledFonts().FromFontProperties(this, !bAllowBestMatch, !bAllowBestMatch);
}

bool MYON_Font::IsManagedFont() const
{
  return ( 0 != m_runtime_serial_number );
}

bool MYON_Font::IsInstalledFont() const
{
  bool rc;
  switch (m_font_type)
  {
  case MYON_Font::FontType::InstalledFont:
    rc = true;
    break;
  case MYON_Font::FontType::ManagedFont:
...
bool MYON_Font::IsManagedInstalledFont() const
{
  const MYON__UINT_PTR bits = 3;
  return IsManagedFont() && (1 == (m_managed_installed_font_and_bits & bits));
}

bool MYON_Font::IsManagedSubstitutedFont() const
{
  const MYON__UINT_PTR bits = 3;
  return IsManagedFont() && (2 == (m_managed_installed_font_and_bits & bits));
}

void MYON_Font::Internal_SetManagedFontInstalledFont(
  const MYON_Font* managed_font,
  const MYON_Font* installed_font,
...
)
{
  if (nullptr != managed_font)
  {
    ...
    if (nullptr != installed_font)
    {
      const MYON__UINT_PTR bits = bInstalledFontIsASubstitute ? 2 : 1;
      const MYON__UINT_PTR ptr = (MYON__UINT_PTR)installed_font;
      x = ptr | bits;
    }
...
const MYON_Font* MYON_Font::SubstituteFont() const
{
...
const MYON_Font* MYON_Font::Internal_ManagedFontToInstalledFont() const
{
  const MYON__UINT_PTR bits = 3;
  MYON__UINT_PTR ptr = m_managed_installed_font_and_bits & (~bits);
  return ((const MYON_Font*)ptr);
}
...
{
  return MYON_Font::GetManagedFont(0.0, face_name);
}
  ...
  )
{
  ...
  return MYON_Font::GetManagedFont(
    point_size,
    face_name,
    bBold,
    bItalic
...
{
  return MYON_Font::GetManagedFont(0.0, face_name, bBold);
}
...
{
  const bool bItalic = false;
  return MYON_Font::GetManagedFont( point_size, face_name, bBold, bItalic );
}
...
{
  return MYON_Font::GetManagedFont(0.0, face_name, bBold, bItalic);
}
  ...
  )
{
  return MYON_Font::GetManagedFont(
    point_size,
    name,
    bBold ? MYON_Font::Weight::Bold : MYON_Font::Default.FontWeight(),
    bItalic ? MYON_Font::Style::Italic : MYON_Font::Default.FontStyle()
    );
}
...
{
  return MYON_Font::GetManagedFont(0.0, face_name, font_weight, font_style);
}
...
{
  // Using MYON_Font::InstalledFontFromRichTextProperties() fixes lots 
  ...
  const MYON_Font* installed_font = MYON_Font::InstalledFontFromRichTextProperties(
    name, 
    MYON_Font::IsBoldWeight(font_weight), 
    (MYON_Font::Style::Italic == font_style)
  );
  if (nullptr != installed_font)
  {
    if (point_size > 0.0 && point_size < MYON_Font::AnnotationFontApplePointSize)
    {
      MYON_Font f(*installed_font);
  ...
  const unsigned int logfont_charset = static_cast<unsigned int>(MYON_Font::WindowsLogfontCharSetFromFaceName(name));
  ...
  return MYON_Font::GetManagedFont(
    ...
    font_weight,
    font_style,
    MYON_Font::Default.m_font_stretch,
    MYON_Font::Default.m_font_bUnderlined,
    MYON_Font::Default.m_font_bStrikethrough,
    MYON_FontMetrics::DefaultLineFeedRatio,
    logfont_charset
    );
}
...
const MYON_Font* MYON_Font::GetManagedFontFromWindowsLogfont(
  int map_mode,
  HDC hdc,
...
const MYON_Font* MYON_Font::GetManagedFontFromFontDescription(
...
bool MYON_Font::IsNotAppleFontName(
  const wchar_t* font_description
  )
{
  if (nullptr == font_description || 0 == font_description[0])
    return true;
  if ( MYON_wString::EqualOrdinal(L"Default",font_description,true) )
    return true;
  // In RH-35535 Marlin reports that Arial is shipped with OS X.
  //if ( MYON_wString::EqualOrdinal(L"Arial",font_description,true) )
  //  return true;
  return false;
}

const MYON_Font* MYON_Font::GetManagedFontFromAppleFontName(
  const char* postscript_name
  )
{
  return MYON_Font::GetManagedFontFromPostScriptName(postscript_name);
}
...
const MYON_Font* MYON_Font::GetManagedFontFromPostScriptName(
  const char* postscript_name
  )
{
  const MYON_wString wide_string_postscript_name(postscript_name);
  return MYON_Font::GetManagedFontFromPostScriptName(wide_string_postscript_name);
}
...
{
  MYON_wString buffer(postscript_name);
  buffer.TrimLeftAndRight();
  postscript_name = buffer;
  if (nullptr == postscript_name || 0 == postscript_name[0])
    return &MYON_Font::Default;

  const MYON_Font* managed_font = MYON_Font::ManagedFontList().FromPostScriptName(postscript_name);
  if (nullptr != managed_font)
    return managed_font;

  const MYON_Font* installed_font = MYON_Font::InstalledFontList().FromPostScriptName(postscript_name);
  if (nullptr != installed_font)
    return installed_font->ManagedFont();

  // This font is not installed.
  MYON_Font font(MYON_Font::Unset);

  // prefered weight/stretch/style since this font is not installed
  font.SetFontWeight(MYON_Font::Weight::Normal);
  font.SetFontStretch(MYON_Font::Stretch::Medium);
  font.SetFontStyle(MYON_Font::Style::Upright);
...
int MYON_Font::WindowsLogfontWeightFromWeight(
  MYON_Font::Weight font_weight
  )
{
...
int MYON_Font::AppleWeightOfFontFromWeight(
  MYON_Font::Weight font_weight
  )
{
  return MYON_Font::WindowsLogfontWeightFromWeight(font_weight)/100;
}

double MYON_Font::AppleFontWeightTraitFromWeight(
  MYON_Font::Weight font_weight
)
{
  ...
  switch (font_weight)
  {
  case MYON_Font::Weight::Unset:
    apple_font_weight_trait = default_apple_font_weight_trait;
    break;
  case MYON_Font::Weight::Thin:
    apple_font_weight_trait = -0.4;
    break;
  case MYON_Font::Weight::Ultralight:
    apple_font_weight_trait = w;
    break;
  case MYON_Font::Weight::Light:
    apple_font_weight_trait = w;
    break;
  case MYON_Font::Weight::Normal:
    apple_font_weight_trait = 0.0;
    break;
  case MYON_Font::Weight::Medium:
    apple_font_weight_trait = w;
    break;
  case MYON_Font::Weight::Semibold:
    apple_font_weight_trait = w;
    break;
  case MYON_Font::Weight::Bold:
    apple_font_weight_trait = 0.4;
    break;
  case MYON_Font::Weight::Ultrabold:
    apple_font_weight_trait = w;
    break;
  case MYON_Font::Weight::Heavy:
    apple_font_weight_trait = w;
    break;
  default:
    apple_font_weight_trait = default_apple_font_weight_trait;
...
MYON_Font::Weight MYON_Font::WeightFromWindowsLogfontWeight(
  int windows_logfont_weight
  )
{
  if ( windows_logfont_weight <= 0 || windows_logfont_weight > 1000 )
    return MYON_Font::Weight::Normal;

  if ( windows_logfont_weight < 150 )
    return MYON_Font::Weight::Thin;

  if ( windows_logfont_weight >= 850 )
    return MYON_Font::Weight::Heavy;

  const MYON_Font::Weight weights[] = 
  {
  ...
  MYON_Font::Weight font_weight = MYON_Font::Weight::Normal;
  int delta = std::abs(static_cast<int>(MYON_Font::WindowsLogfontWeightFromWeight(font_weight)) - windows_logfont_weight);

  for (size_t i = 0; 0 != delta && i < weight_count; i++)
  {
    // look for a closer match
    int d = std::abs(static_cast<int>(MYON_Font::WindowsLogfontWeightFromWeight(weights[i])) - windows_logfont_weight);
    if (d < delta)
    {
...
MYON_Font::Weight MYON_Font::WeightFromAppleWeightOfFont(
  int apple_weight_of_font
  )
{
  return MYON_Font::WeightFromWindowsLogfontWeight(apple_weight_of_font*100);
}

MYON_Font::Weight MYON_Font::WeightFromAppleFontWeightTrait(
  double apple_font_weight_trait
)
{
  if (false == MYON_IsValid(apple_font_weight_trait))
    return MYON_Font::Weight::Unset;
  ...
  return MYON_Font::WeightFromWindowsLogfontWeight(windows_logfont_weight);
}

void MYON_Font::Internal_CopyFrom(
    const MYON_Font& src
    )
{
  if ( 0 == ((MYON__UINT_PTR)(&src)) )
  {
    MYON_ERROR("nullptr is target of MYON_Font copy ctor or operator=. Crash is imminent or already happened.");
    return;
  }
  
  const bool bThisIsManagedFont = (MYON_Font::FontType::ManagedFont == m_font_type);

  if ( bThisIsManagedFont )
  {
    if (0 == m_runtime_serial_number)
    {
      MYON_ERROR("Invalid parameters: true == bThisIsManagedFont and 0 == m_runtime_serial_number.");
      return;
    }
  }
  else
  {
    if (0 != m_runtime_serial_number && MYON_Font::FontType::ManagedFont != m_font_type)
    {
      MYON_ERROR("Invalid parameters: false == bThisIsManagedFont and 0 != m_runtime_serial_number.");
      return;
    }
  }
  
  if ( ((MYON__UINT_PTR)(&src)) <= MYON_PTR_SEMAPHORE_MAX )
  {
    // If 8 == ((MYON__UINT_PTR)(&src)), Initializing MYON_Font::Unset (and m_runtime_serial_number is 0 )
    // If 16 == ((MYON__UINT_PTR)(&src)), Initializing MYON_Font::Default (and m_runtime_serial_number is already set to 1)
    ...
    m_font_weight  = bDefaultFont ? MYON_Font::Weight::Normal  : MYON_Font::Weight::Unset;
    m_font_stretch = bDefaultFont ? MYON_Font::Stretch::Medium : MYON_Font::Stretch::Unset;
    m_font_style   = bDefaultFont ? MYON_Font::Style::Upright  : MYON_Font::Style::Unset;

    m_loc_family_name = (bDefaultFont ? MYON_wString(MYON_Font::DefaultFamilyName()) : MYON_wString::EmptyString);
    m_en_family_name  = (bDefaultFont ? MYON_wString(MYON_Font::DefaultFamilyName()) : MYON_wString::EmptyString);

    m_loc_face_name   = (bDefaultFont ? MYON_wString(MYON_Font::DefaultFaceName()) : MYON_wString::EmptyString);
    m_en_face_name    = (bDefaultFont ? MYON_wString(MYON_Font::DefaultFaceName()) : MYON_wString::EmptyString);

    m_loc_windows_logfont_name = (bDefaultFont ? MYON_wString(MYON_Font::DefaultWindowsLogfontName()) : MYON_wString::EmptyString);
    m_en_windows_logfont_name = (bDefaultFont ? MYON_wString(MYON_Font::DefaultWindowsLogfontName()) : MYON_wString::EmptyString);

    m_quartet_member = bDefaultFont ? MYON_FontFaceQuartet::Member::Regular  : MYON_FontFaceQuartet::Member::Unset;

    m_loc_postscript_name = (bDefaultFont ? MYON_wString(MYON_Font::DefaultPostScriptName()) : MYON_wString::EmptyString);
    m_en_postscript_name = (bDefaultFont ? MYON_wString(MYON_Font::DefaultPostScriptName()) : MYON_wString::EmptyString);
    ...
    m_font_origin
      ...
      ? 
#if defined(MYON_RUNTIME_WIN)
      MYON_Font::Origin::WindowsFont
#elif defined(MYON_RUNTIME_APPLE)
      MYON_Font::Origin::AppleFont
#else
      MYON_Font::Origin::Unknown
#endif
      : MYON_Font::Origin::Unset;
    
    const MYON_Font* installed_font
      = bDefaultFont
      ? MYON_Font::InstalledFontList().FromFontProperties(this, true, true)
      : nullptr;

    if (
      nullptr != installed_font
      && MYON_Font::EqualFontFamily(this, installed_font)
      && m_font_style == installed_font->FontStyle()
      )
    {
      if (
        MYON_Font::FontType::ManagedFont == this->m_font_type
        && this->m_runtime_serial_number > 0
        && 0 == this->m_managed_installed_font_and_bits)
      {
        // When 1 == m_runtime_serial_number, this font is MYON_Font::Default 
        ...
        MYON_Font::Internal_SetManagedFontInstalledFont(this, installed_font, false);
      }
      ...
      if (m_loc_windows_logfont_name.IsNotEmpty() || m_en_windows_logfont_name.IsNotEmpty())
        ...
        m_quartet_member = MYON_FontFaceQuartet::Member::Unset;
  ...
  else
  {
    ...
    bool bCopyCache = (0 == m_runtime_serial_number && MYON_Font::FontType::Unset == m_font_type);
    if (
      false == bCopyCache
      && MYON_Font::FontType::ManagedFont == m_font_type
      && MYON_Font::FontType::InstalledFont == src.m_font_type
  ...
  m_font_characteristics_hash = MYON_SHA1_Hash::ZeroDigest;
...
MYON_Font::MYON_Font()
{
  ...
  ////  MYON_TextLog::Null.PrintNewLine();
  /*
...
MYON_Font::MYON_Font(
  ...
  : m_runtime_serial_number(
    (MYON_Font::FontType::ManagedFont == font_type)
    ? (++MYON_Font::__runtime_serial_number_generator) // only managed fonts have non-zero m_runtime_serial_number
    : 0
  )
...
MYON_Font::MYON_Font(const MYON_Font& src)
  : m_runtime_serial_number(  (MYON_PTR_SEMAPHORE2 == ((MYON__UINT_PTR)&src)) ? 1 : 0)
  , m_font_type( (MYON_PTR_SEMAPHORE2 == ((MYON__UINT_PTR)&src)) ? MYON_Font::FontType::ManagedFont : MYON_Font::FontType::Unset)
{
  // (MYON_PTR_SEMAPHORE1 == ((MYON__UINT_PTR)&src)) means we are constructing MYON_Font::Unset.
  // (MYON_PTR_SEMAPHORE2 == ((MYON__UINT_PTR)&src)) means we are constructing MYON_Font::Default.
  // Otherwise "this" should be an ordinary and unmanaged font.
  // See opennurbs_statics.cpp and MYON_Font::CopyHelper() for more information.
  Internal_CopyFrom(src);
}

MYON_Font& MYON_Font::operator=(const MYON_Font& src)
{
  if (this != &src)
  {
    if (IsManagedFont() )
    {
      ...
      if ( false == MYON_Font::EqualFontCharacteristics(*this, src) )
      {
        MYON_ERROR("Attempt to modify a managed font");
      }
    }
    else
    {
...
bool MYON_Font::SetFontCharacteristics(
...
)
{
  ...
  {
    gdi_logfont_name = MYON_Font::Default.m_loc_windows_logfont_name;
  }
  return SetFontCharacteristics(
    point_size,
    gdi_logfont_name,
    (bBold ? MYON_Font::Weight::Bold :  MYON_Font::Weight::Normal),
    (bItalic ? MYON_Font::Style::Italic : MYON_Font::Style::Upright),
    MYON_Font::Default.m_font_stretch,
    bUnderlined,
    bStrikethrough,
    MYON_FontMetrics::DefaultLineFeedRatio,
    MYON_Font::WindowsLogfontCharSetFromFaceName(gdi_logfont_name)
    );
}

bool MYON_Font::SetFontCharacteristicsForExperts(
  double point_size,
...
)
{
  ...
  this->m_font_origin = MYON_Font::Origin::Unset;
  this->m_simulated = 0;
  this->m_font_characteristics_hash = MYON_SHA1_Hash::ZeroDigest;

  return this->IsValid();
}

bool MYON_Font::IsValidFaceName(
  const wchar_t* face_name
  )
{
  if ( nullptr == face_name || 0 == face_name[0] || MYON_wString::Space == face_name[0])
    return false;
  
  int i = 0;
  while (i < 32 && 0 != face_name[i])
  {
    if (face_name[i] < MYON_wString::Space )
      return false;
    switch (face_name[i])
    {
    case ';':
...
MYON_Font::Weight MYON_Font::FontWeightFromUnsigned(
  unsigned int unsigned_font_weight
  )
{
  switch (unsigned_font_weight)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Unset);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Thin);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Ultralight);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Light);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Normal);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Medium);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Semibold);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Bold);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Ultrabold);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Weight::Heavy);
  default:
    break;
  }
  MYON_ERROR("unsigned_font_weight is not valid");
  return MYON_Font::Weight::Unset;
}

bool MYON_Font::EqualWeightStretchStyle(
  const MYON_Font* lhs,
  const MYON_Font* rhs,
  bool bUnsetIsEqual
)
{
  return
    MYON_Font::EqualWeight(lhs, rhs, bUnsetIsEqual)
    && MYON_Font::EqualStretch(lhs, rhs, bUnsetIsEqual)
    && MYON_Font::EqualStyle(lhs, rhs, bUnsetIsEqual);
}

bool MYON_Font::EqualWeight(
  const MYON_Font* lhs,
  const MYON_Font* rhs,
  bool bUnsetIsEqual
)
{
  ...
  if (lhs->m_font_weight != rhs->m_font_weight)
  {
    if (false == bUnsetIsEqual)
      return false;
    if (MYON_Font::Weight::Unset != lhs->m_font_weight && MYON_Font::Weight::Unset != rhs->m_font_weight)
      return false;
  }
  return true;
}

bool MYON_Font::EqualStretch(
  const MYON_Font* lhs,
  const MYON_Font* rhs,
  bool bUnsetIsEqual
)
{
  ...
  if (lhs->m_font_stretch != rhs->m_font_stretch)
  {
    if (false == bUnsetIsEqual)
      return false;
    if (MYON_Font::Stretch::Unset != lhs->m_font_stretch && MYON_Font::Stretch::Unset != rhs->m_font_stretch)
      return false;
  }
  return true;
}

bool MYON_Font::EqualStyle(
  const MYON_Font* lhs,
  const MYON_Font* rhs,
  bool bUnsetIsEqual
)
{
  ...
  if (lhs->m_font_style != rhs->m_font_style)
  {
    if (false == bUnsetIsEqual)
      return false;
    if (MYON_Font::Style::Unset != lhs->m_font_style && MYON_Font::Style::Unset != rhs->m_font_style)
      return false;
  }
  return true;
}

int MYON_Font::CompareWeight(
...
MYON_Font::Style MYON_Font::FontStyleFromUnsigned(
  unsigned int unsigned_font_style
  )
{
  switch (unsigned_font_style)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE( MYON_Font::Style::Upright);
  MYON_ENUM_FROM_UNSIGNED_CASE( MYON_Font::Style::Italic);
  MYON_ENUM_FROM_UNSIGNED_CASE( MYON_Font::Style::Oblique);
  default:
    break;
  }
  MYON_ERROR("unsigned_font_style is not valid");
  return MYON_Font::Style::Upright;
}

bool MYON_Font::IsBoldWeight(
  MYON_Font::Weight weight
)
{
  return (static_cast<unsigned int>(weight) >= static_cast<unsigned int>(MYON_Font::Weight::Semibold));
}

const wchar_t* MYON_Font::WeightToWideString(
  MYON_Font::Weight font_weight
)
{
  const wchar_t* s;
...
const wchar_t* MYON_Font::StretchToWideString(
  MYON_Font::Stretch font_stretch
)
{
  const wchar_t* s;
  switch (font_stretch)
  {
  case MYON_Font::Stretch::Unset:
    s = L"Unsetstretch";
    break;
  case MYON_Font::Stretch::Ultracondensed:
    s = L"Ultracondensed";
    break;
  case MYON_Font::Stretch::Extracondensed:
    s = L"Extracondensed";
    break;
  case MYON_Font::Stretch::Condensed:
    s = L"Condensed";
    break;
  case MYON_Font::Stretch::Semicondensed:
    s = L"Semicondensed";
    break;
  case MYON_Font::Stretch::Medium:
    s = L"Medium";
    break;
  case MYON_Font::Stretch::Semiexpanded:
    s = L"Semiexpanded";
    break;
  case MYON_Font::Stretch::Expanded:
    s = L"Expanded";
    break;
  case MYON_Font::Stretch::Extraexpanded:
    s = L"Extraexpanded";
    break;
  case MYON_Font::Stretch::Ultraexpanded:
    s = L"Ultraexpanded";
    break;
  default:
    s = L"";
...
const wchar_t* MYON_Font::StyleToWideString(
  MYON_Font::Style font_style
)
{
  const wchar_t* s;
  switch (font_style)
  {
  case MYON_Font::Style::Unset:
    s = L"Unsetstyle";
    break;
  case MYON_Font::Style::Upright:
    s = L"Upright";
    break;
  case MYON_Font::Style::Italic:
    s = L"Italic";
    break;
  case MYON_Font::Style::Oblique:
    s = L"Oblique";
    break;
  default:
    s = L"";
...
MYON_Font::Stretch MYON_Font::FontStretchFromUnsigned(
  unsigned int unsigned_font_stretch
  )
{
  switch (unsigned_font_stretch)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Unset);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Ultracondensed);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Extracondensed);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Condensed);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Semicondensed);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Medium);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Semiexpanded);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Expanded);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Extraexpanded);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Stretch::Ultraexpanded);
  default:
    break;
  }
  MYON_ERROR("unsigned_font_stretch is not valid");
  return MYON_Font::Stretch::Unset;
}

unsigned int MYON_Font::FontCharacteristicsAsUnsigned() const
{
  return MYON_Font::Internal_FontCharacteristicsAsUnsigned(
    FontWeight(),
    FontStyle(),
    FontStretch(),
    m_font_bUnderlined,
...
unsigned int MYON_Font::Internal_FontCharacteristicsAsUnsigned(
  MYON_Font::Weight font_weight,
  MYON_Font::Style font_style,
  MYON_Font::Stretch font_stretch,
  bool bUnderlined,
...
void MYON_Font::Internal_GetFontCharacteristicsFromUnsigned(
...
)
{
  ...
  font_weight = (1U == u_one && u_font_weight > 0) ? MYON_Font::FontWeightFromUnsigned(u_font_weight) : MYON_Font::Default.FontWeight();
  font_style = (1U == u_one) ? MYON_Font::FontStyleFromUnsigned(u_font_style) : MYON_Font::Default.FontStyle();
  font_stretch = (1U == u_one) ? MYON_Font::FontStretchFromUnsigned(u_font_stretch) : MYON_Font::Default.FontStretch();
  bUnderlined = (1U == u_one) ? (1 == u_bUnderlined) : MYON_Font::Default.IsUnderlined();
  bStrikethrough = (1U == u_one) ? (1 == u_bStrikethrough) : MYON_Font::Default.IsStrikethrough();
}

bool MYON_Font::Internal_SetFontCharacteristicsFromUnsigned(
  unsigned int font_characteristics_as_unsigned
  )
{
  MYON_Font::Weight font_weight = MYON_Font::Weight::Normal;
  MYON_Font::Stretch font_stretch = MYON_Font::Stretch::Medium;
  MYON_Font::Style font_style = MYON_Font::Style::Upright;
  bool bUnderlined = false;
  bool bStrikethrough = false;

  MYON_Font::Internal_GetFontCharacteristicsFromUnsigned(
  ...
  const MYON_wString windows_logfont_name = WindowsLogfontName();
  ...
  ) const
{
  ...
  const MYON_wString windows_logfont_name = WindowsLogfontName();
  const MYON_wString windows_logfont_name_crc
    = bIgnoreNameOrdinalCase
    ? windows_logfont_name.MapStringOrdinal(MYON_StringMapOrdinalType::MinimumOrdinal)
    : windows_logfont_name;

  // SHould probably include PostScript name as well.

  MYON__UINT32 hash = MYON_CRC32(0, sizeof(u), &u );
  hash = MYON_CRC32(hash,windows_logfont_name_crc.Length()*sizeof(wchar_t),static_cast<const wchar_t*>(windows_logfont_name_crc));

#if defined(MYON_RUNTIME_WIN) 
  if ( m_point_size > 0.0 )
    hash = MYON_CRC32(hash, sizeof(m_point_size), &m_point_size);
#endif
...
{
  const unsigned char logfont_charset = MYON_Font::WindowsLogfontCharSetFromFaceName(gdi_logfont_name);

  double linefeed_ratio = MYON_FontMetrics::DefaultLineFeedRatio;
  ...
  )
{
  ...
  // and *this = MYON_Font::Unset will set this->m_face_name to the empty string.
  MYON_wString local_face_name(gdi_logfont_name);
  local_face_name.TrimLeftAndRight();
  gdi_logfont_name = static_cast<const wchar_t*>(local_face_name);

  *this = MYON_Font::Unset;

  if (false == MYON_Font::IsValidFaceName(gdi_logfont_name))
    return false;

  if (logfont_charset >= 256)
    return false;

  MYON_Font new_characteristics(MYON_Font::Unset);
  ...
  new_characteristics.m_font_weight = MYON_Font::FontWeightFromUnsigned(static_cast<unsigned char>(font_weight));
  new_characteristics.m_point_size 
    = (point_size > 0.0 && point_size < 2147483640.0)
    ? point_size
    : 0.0;
  new_characteristics.m_windows_logfont_weight = MYON_Font::WindowsLogfontWeightFromWeight(new_characteristics.m_font_weight);
  new_characteristics.m_apple_font_weight_trait = MYON_Font::AppleFontWeightTraitFromWeight(new_characteristics.m_font_weight);
  ...
  new_characteristics.m_loc_family_name = MYON_Font::FamilyNameFromDirtyName(gdi_logfont_name);
  new_characteristics.m_en_family_name = new_characteristics.m_loc_family_name;

  if (
    MYON_wString::EqualOrdinal(L"CityBlueprint", -1, gdi_logfont_name, -1, true)
    || MYON_wString::EqualOrdinal(L"CountryBlueprint", -1, gdi_logfont_name, -1, true)
    )
  {
    ...
    logfont_charset = MYON_Font::WindowsConstants::logfont_symbol_charset;
  }
  else if (MYON_Font::logfont_symbol_charset == logfont_charset)
  {
    // verify this is correct.
    logfont_charset = MYON_Font::WindowsLogfontCharSetFromFaceName(new_characteristics.WindowsLogfontName());
  }
  else
  {
    logfont_charset = MYON_Font::WindowsConstants::logfont_default_charset;
  }

  new_characteristics.m_logfont_charset 
    = (MYON_Font::WindowsConstants::logfont_symbol_charset == (unsigned char)logfont_charset)
    ? MYON_Font::WindowsConstants::logfont_symbol_charset
    : MYON_Font::WindowsConstants::logfont_default_charset;

  if ( 
    // 3 fast checks to avoid time consuming hash calculation
    false == MYON_wString::EqualOrdinal(WindowsLogfontName(),new_characteristics.WindowsLogfontName(),false)
    || false == MYON_wString::EqualOrdinal(PostScriptName(),new_characteristics.PostScriptName(),false)
    || false == MYON_wString::EqualOrdinal(FamilyName(),new_characteristics.FamilyName(),false)
    || false == MYON_wString::EqualOrdinal(FaceName(),new_characteristics.FaceName(),false)
    ...
    )
  {
    ...
    if (
      MYON_Font::Stretch::Medium == m_font_stretch
      && (MYON_Font::Weight::Normal == m_font_weight || MYON_Font::Weight::Bold == m_font_weight)
      && (MYON_Font::Style::Upright == m_font_style || MYON_Font::Style::Italic == m_font_style)
      )
    {
      const MYON_wString logfontName = WindowsLogfontName();
      MYON_wString postscript_suffix;
      bool bSetMakePostScriptName = false;
      if (
        MYON_wString::EqualOrdinal(L"Arial", logfontName, true)
        )
      {
      ...
      else if (MYON_wString::EqualOrdinal(L"Segoe Print", logfontName, true))
      {
        bSetMakePostScriptName = true;
      }
      else if (MYON_wString::EqualOrdinal(L"Segoe Script", logfontName, true))
      {
        bSetMakePostScriptName = true;
      }
      else if (MYON_wString::EqualOrdinal(L"Segoe UI", logfontName, true))
      {
        bSetMakePostScriptName = true;
      }
      else if (MYON_wString::EqualOrdinal(L"Microsoft YaHei", logfontName, true))
      {
        bSetMakePostScriptName = true;
      }
      else if (MYON_wString::EqualOrdinal(L"Microsoft YaHei UI", logfontName, true))
      {
        bSetMakePostScriptName = true;
      }
      ...
      for (;;)
      {
        ...
        MYON_wString postscript_name = logfontName;
        postscript_name.TrimLeftAndRight();
        postscript_name.Remove(MYON_wString::Space);
        if (postscript_name.IsEmpty())
          break;
        
        // add -<Weight><Style>
        MYON_wString postscript_face;
        if (MYON_Font::Weight::Bold == m_font_weight)
          postscript_face += L"Bold";
        if (MYON_Font::Style::Italic == m_font_style)
          postscript_face += L"Italic";
        if (postscript_face.IsNotEmpty())
        {
          postscript_name += MYON_wString::HyphenMinus;
...
bool MYON_Font::SetFromAppleFontName(
...
)
{
  ...
  = MYON_Font::IsValidPointSize(point_size) && point_size < MYON_Font::AnnotationFontCellHeight
    ? false
    : true;
  ...
  CTFontRef appleFont = MYON_Font::AppleCTFont(apple_font_name,point_size, bIsSubstituteFont);
  ...
  if (false == rc)
  {
    ...
    m_loc_family_name = MYON_Font::FamilyNameFromDirtyName(m_loc_postscript_name);
...
{
  return (lhs.IsNotEmpty() && MYON_wString::EqualOrdinal(lhs, rhs, true));
}

bool MYON_Font::EqualFontFamily(
  const MYON_Font* lhs,
  const MYON_Font* rhs
)
{
  ...
  if (Internal_EqualName(lhs->m_en_family_name, rhs->m_en_family_name))
    return true;
  if (Internal_EqualName(lhs->FamilyName(MYON_Font::NameLocale::LocalizedFirst), rhs->FamilyName(MYON_Font::NameLocale::LocalizedFirst)))
    return true;
  ...
  const bool rc
    ...
    || Internal_EqualName(lhs->PostScriptName(MYON_Font::NameLocale::LocalizedFirst),rhs->PostScriptName(MYON_Font::NameLocale::LocalizedFirst))
    || Internal_EqualName(lhs->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst),rhs->WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst))
    ;
  return rc;
}

bool MYON_Font::EqualFontFamilyAndFace(
  const MYON_Font* lhs,
  const MYON_Font* rhs
)
{
  ...
  if (rc)
  {
    rc = Internal_EqualName(lhs->m_en_face_name, rhs->m_en_face_name)
      || Internal_EqualName(lhs->FaceName(MYON_Font::NameLocale::LocalizedFirst), rhs->FaceName(MYON_Font::NameLocale::LocalizedFirst))
      ;
  }
...
const MYON_wString MYON_Font::Locale() const
{
  return m_locale_name;
}

const wchar_t* MYON_Font::AppleFontNameAsPointer() const
{
  return MYON_Font::Internal_GetName(MYON_Font::NameLocale::LocalizedFirst, m_loc_postscript_name, m_en_postscript_name).Array();
}

const MYON_wString& MYON_Font::AppleFontName() const
{
  return
...
const wchar_t* MYON_Font::PostScriptNameAsPointer() const
{
  return MYON_Font::Internal_GetName(MYON_Font::NameLocale::LocalizedFirst, m_loc_postscript_name, m_en_postscript_name).Array();
}

const MYON_wString MYON_Font::PostScriptName(MYON_Font::NameLocale name_locale) const
{
  return MYON_Font::Internal_GetName(name_locale, m_loc_postscript_name, m_en_postscript_name);
}

const MYON_wString MYON_Font::PostScriptName() const
{
  return MYON_Font::Internal_GetName(MYON_Font::NameLocale::LocalizedFirst, m_loc_postscript_name, m_en_postscript_name);
}

const MYON_wString MYON_Font::FamilyName(MYON_Font::NameLocale name_locale) const
{
  return MYON_Font::Internal_GetName(name_locale, m_loc_family_name, m_en_family_name);
}

const MYON_wString MYON_Font::FamilyName() const
{
  return FamilyName(MYON_Font::NameLocale::LocalizedFirst);
}

const MYON_wString MYON_Font::FaceName(
  MYON_Font::NameLocale name_locale
) const
{
  return MYON_Font::Internal_GetName(name_locale, m_loc_face_name, m_en_face_name);
}

const MYON_wString MYON_Font::FaceName() const
{
  return FaceName(MYON_Font::NameLocale::LocalizedFirst);
}

const MYON_wString MYON_Font::WindowsLogfontName(
  MYON_Font::NameLocale name_locale
) const
{
  return MYON_Font::Internal_GetName(name_locale, m_loc_windows_logfont_name, m_en_windows_logfont_name);
}

const MYON_wString MYON_Font::WindowsLogfontName() const
{
  return WindowsLogfontName(MYON_Font::NameLocale::LocalizedFirst);
}
...
public:
  ...
  //, m_postscript_name(postscript_name)
  {
    ...
    if (
      ...
      || (MYON_FontFaceQuartet::Member::Unset != quartet_member && m_fake_logfont_name.IsEmpty())
      || m_family_and_postcript_name_hash.IsZeroDigestOrEmptyContentHash()
      )
    {
      MYON_ERROR("Invalid input.");
      m_fake_logfont_name = MYON_wString::EmptyString;
      m_family_and_postcript_name_hash = MYON_SHA1_Hash::EmptyContentHash;
      m_quartet_member = MYON_FontFaceQuartet::Member::Unset;
    }
  }

  static const MYON_SHA1_Hash NameHash(
    const wchar_t* family_name,
    const wchar_t* postscript_name
  )
  {
    ...
    MYON_wString s1(family_name);
    s1.Remove(MYON_wString::Space);
    s1.Remove(MYON_wString::HyphenMinus);
    s1.TrimLeftAndRight();
    if (s1.IsEmpty())
      return MYON_SHA1_Hash::EmptyContentHash;
        
    MYON_wString s2(postscript_name);
    s2.Remove(MYON_wString::Space);
    s2.Remove(MYON_wString::HyphenMinus);
    s2.TrimLeftAndRight();
    if (s2.IsEmpty())
      return MYON_SHA1_Hash::EmptyContentHash;
    ...
    s1 += MYON_wString::HyphenMinus;
    s1 += s2;
       
    return s1.ContentHash(MYON_StringMapOrdinalType::MinimumOrdinal);
  }
    ...
    )
  {
    ...
    if (nullptr == rhs)
      return -1;
    return MYON_SHA1_Hash::Compare(lhs->m_family_and_postcript_name_hash, rhs->m_family_and_postcript_name_hash);
  }

  const MYON_SHA1_Hash QuartetFamilyAndPostscriptNameHash() const
  {
    return m_family_and_postcript_name_hash;
  }
  
  MYON_FontFaceQuartet::Member QuartetMember() const
  {
    return m_quartet_member;
  }

  const MYON_wString FakeWindowsLogfontName() const
  {
    return m_fake_logfont_name;
  }

private:
  MYON_SHA1_Hash m_family_and_postcript_name_hash = MYON_SHA1_Hash::EmptyContentHash;
  MYON_wString m_fake_logfont_name = MYON_wString::EmptyString;
  MYON_FontFaceQuartet::Member m_quartet_member = MYON_FontFaceQuartet::Member::Unset;

  // for debugging
  //const MYON_wString m_family_name = MYON_wString::EmptyString;
  //const MYON_wString m_postscript_name = MYON_wString::EmptyString;
};

const MYON_wString MYON_Font::FakeWindowsLogfontNameFromFamilyAndPostScriptNames(
...
)
{
  ...
  static Internal_FakeWindowsLogfontName fake_names[] = 
  {
    ...
    Internal_FakeWindowsLogfontName(L"American Typewriter", L"AmericanTypewriter-Light",          L"American Typewriter Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"American Typewriter", L"AmericanTypewriter-Semibold",       L"American Typewriter Semibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"American Typewriter", L"AmericanTypewriter-CondensedLight", L"American Typewriter Condensed Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"American Typewriter", L"AmericanTypewriter-Condensed",      L"American Typewriter Condensed", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"American Typewriter", L"AmericanTypewriter-CondensedBold",  L"American Typewriter Condensed", MYON_FontFaceQuartet::Member::Bold),

    Internal_FakeWindowsLogfontName(L"Apple Braille", L"AppleBraille-Outline6Dot",  L"Apple Braille Outline 6 Dot", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple Braille", L"AppleBraille-Outline8Dot",  L"Apple Braille Outline 8 Dot", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple Braille", L"AppleBraille-Pinpoint6Dot", L"Apple Braille Pinpoint 6 Dot", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple Braille", L"AppleBraille-Pinpoint8Dot", L"Apple Braille Pinpoint 8 Dot", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-Thin", L"Apple SD Gothic Neo Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-UltraLight", L"Apple SD Gothic Neo Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-Light", L"Apple SD Gothic Neo Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-Medium", L"Apple SD Gothic Neo Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-SemiBold", L"Apple SD Gothic Neo Semibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-ExtraBold", L"Apple SD Gothic Neo Extra Bold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Apple SD Gothic Neo", L"AppleSDGothicNeo-Heavy", L"Apple SD Gothic Neo Heavy", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Arial Hebrew", L"ArialHebrew-Light", L"Arial Hebrew Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Arial Hebrew Scholar", L"ArialHebrewScholar-Light", L"Arial Hebrew Scholar Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-Book",          L"Avenir Book", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-BookOblique",   L"Avenir Book", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-Light",         L"Avenir Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-LightOblique",  L"Avenir Light", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-Medium",        L"Avenir Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-MediumOblique", L"Avenir Medium", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-Heavy",         L"Avenir Heavy", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-HeavyOblique",  L"Avenir Heavy", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-Black",         L"Avenir Black", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir", L"Avenir-BlackOblique",  L"Avenir-Black", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-UltraLight", L"Avenir Next Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-UltraLightItalic", L"Avenir Next Ultralight", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-Medium", L"Avenir Next Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-MediumItalic", L"Avenir Next Medium", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-DemiBold", L"Avenir Next Demibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-DemiBoldItalic", L"Avenir Next Demibold", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-Heavy", L"Avenir Next Heavy", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next", L"AvenirNext-HeavyItalic", L"Avenir Next Heavy", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-UltraLight", L"Avenir Next Condensed Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-UltraLightItalic", L"Avenir Next Condensed Ultralight", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-Medium", L"Avenir Next Condensed Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-MediumItalic", L"Avenir Next Condensed Medium", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-DemiBold", L"Avenir Next Condensed Demibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-DemiBoldItalic", L"Avenir Next Condensed Demibold", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-Heavy", L"Avenir Next Condensed Heavy", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Avenir Next Condensed", L"AvenirNextCondensed-HeavyItalic", L"Avenir Next Condensed Heavy", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Baskerville", L"Baskerville-SemiBold", L"Baskerville Semibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Baskerville", L"Baskerville-SemiBoldItalic", L"Baskerville Semibold", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Chalkboard SE", L"ChalkboardSE-Light", L"Chalkboard SE Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Charter", L"Charter-Black", L"Charter Black", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Charter", L"Charter-BlackItalic", L"Charter Black", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Copperplate", L"Copperplate-Light", L"Copperplate Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Damascus", L"DamascusLight", L"Damascus Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Damascus", L"DamascusMedium", L"Damascus Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Damascus", L"DamascusSemiBold", L"Damascus Semibold", MYON_FontFaceQuartet::Member::Regular),

    // DINPro https://mcneel.myjetbrains.com/youtrack/issue/RH-54627
    // DO NOT try making a fake quartet that combines regular/bold.
    // The last parameter for all DINPro "fake quartets" must be MYON_FontFaceQuartet::Member::Regular.
    ...
    Internal_FakeWindowsLogfontName(L"DINPro", L"DINPro-Regular", L"DINPro Regular", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"DINPro", L"DINPro-Bold",    L"DINPro Bold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"DINPro", L"DINPro-Light",   L"DINPro Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"DINPro", L"DINPro-Medium",  L"DINPro Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"DINPro", L"DINPro-Black",   L"DINPro Black", MYON_FontFaceQuartet::Member::Regular),

    // Futura https://mcneel.myjetbrains.com/youtrack/issue/RH-56085
    // DO NOT try making a fake quartet that combines regular/bold.
    // The last parameter for all Futura "fake quartets" must be MYON_FontFaceQuartet::Member::Regular.
    // When text is created and then later edited the, the Mac RTF used by Eto and the "composed" RTF used by Rhino 
    // are not currently capable looking at the "fake quartet" information and preserving the font. So the Mac users get
    // to see 5 "fake quartets" for Futura. The "fix" is to return to the Rhino 5 Mac font UI.
    Internal_FakeWindowsLogfontName(L"Futura", L"Futura-Medium",             L"Futura Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Futura", L"Futura-Bold",               L"Futura Bold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Futura", L"Futura-MediumItalic",       L"Futura Medium Italic", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Futura", L"Futura-CondensedMedium",    L"Futura Condensed Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Futura", L"Futura-CondensedExtraBold", L"Futura Condensed ExtraBold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Gill Sans", L"GillSans-Light", L"Gill Sans Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Gill Sans", L"GillSans-LightItalic", L"Gill Sans Light", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Gill Sans", L"GillSans-SemiBold", L"Gill Sans Semibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Gill Sans", L"GillSans-SemiBoldItalic", L"Gill Sans Semibold", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Gill Sans", L"GillSans-UltraBold", L"Gill Sans Ultrabold", MYON_FontFaceQuartet::Member::Regular),
    ...
    //  used in Mac OS. 
    Internal_FakeWindowsLogfontName(L"Gill Sans Std", L"GillSansStd-Light", L"Gill Sans Std Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Helvetica", L"Helvetica-Light", L"Helvetica Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica", L"Helvetica-LightOblique", L"Helvetica Light", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-UltraLight",       L"Helvetica Neue Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-UltraLightItalic", L"Helvetica Neue Ultralight",  MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-Thin",             L"Helvetica Neue Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-ThinItalic",       L"Helvetica Neue Thin", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-Light",            L"Helvetica Neue Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-LightItalic",      L"Helvetica Neue Light", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-Medium",           L"Helvetica Neue Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-MediumItalic",     L"Helvetica Neue Medium", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-CondensedBold",    L"Helvetica Neue Condensed", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Helvetica Neue", L"HelveticaNeue-CondensedBlack",   L"Helvetica Neue Condensed", MYON_FontFaceQuartet::Member::Bold),

    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W3", L"Hiragino Sans W3", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W6", L"Hiragino Sans W6", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W0", L"Hiragino Sans W0", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W1", L"Hiragino Sans W1", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W2", L"Hiragino Sans W2", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W4", L"Hiragino Sans W4", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W5", L"Hiragino Sans W5", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W7", L"Hiragino Sans W7", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W8", L"Hiragino Sans W8", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Hiragino Sans", L"HiraginoSans-W9", L"Hiragino Sans W9", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Hoefler Text", L"HoeflerText-Ornaments", L"Hoefler Text Ornaments", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"ITF Devanagari", L"ITFDevanagari-Light", L"ITF Devanagari Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"ITF Devanagari", L"ITFDevanagari-Medium", L"ITF Devanagari Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"ITF Devanagari", L"ITFDevanagari-Demi", L"ITF Devanagari Demibold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"ITF Devanagari Marathi", L"ITFDevanagariMarathi-Light", L"ITF Devanagari Marathi Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"ITF Devanagari Marathi", L"ITFDevanagariMarathi-Medium", L"ITF Devanagari Marathi Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"ITF Devanagari Marathi", L"ITFDevanagariMarathi-Demi", L"ITF Devanagari Marathi Demibold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Kohinoor Bangla", L"KohinoorBangla-Light", L"Kohinoor Bangla Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Bangla", L"KohinoorBangla-Medium", L"Kohinoor Bangla Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Bangla", L"KohinoorBangla-Semibold", L"Kohinoor Bangla Semibold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Kohinoor Devanagari", L"KohinoorDevanagari-Light", L"Kohinoor Devanagari Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Devanagari", L"KohinoorDevanagari-Medium", L"Kohinoor Devanagari Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Devanagari", L"KohinoorDevanagari-Semibold", L"Kohinoor Devanagari Semibold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Kohinoor Telugu", L"KohinoorTelugu-Light", L"Kohinoor Telugu Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Telugu", L"KohinoorTelugu-Medium", L"Kohinoor Telugu Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Kohinoor Telugu", L"KohinoorTelugu-Semibold", L"Kohinoor Telugu Semibold", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Muna", L"MunaBlack", L"Muna Black", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Optima", L"Optima-ExtraBlack", L"Optima Black", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Papyrus", L"Papyrus-Condensed", L"Papyrus Condensed", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Phosphate", L"Phosphate-Inline", L"Phosphate Inline", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Phosphate", L"Phosphate-Solid", L"Phosphate Solid", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"PingFang HK", L"PingFangHK-Ultralight", L"PingFang HK Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang HK", L"PingFangHK-Thin", L"PingFang HK Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang HK", L"PingFangHK-Light", L"PingFang HK Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang HK", L"PingFangHK-Medium", L"PingFang HK Medium", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"PingFang SC", L"PingFangSC-Ultralight", L"PingFang SC Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang SC", L"PingFangSC-Thin", L"PingFang SC Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang SC", L"PingFangSC-Light", L"PingFang SC Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang SC", L"PingFangSC-Medium", L"PingFang SC Medium", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"PingFang TC", L"PingFangTC-Ultralight", L"PingFang TC Ultralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang TC", L"PingFangTC-Thin", L"PingFang TC Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang TC", L"PingFangTC-Light", L"PingFang TC Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"PingFang TC", L"PingFangTC-Medium", L"PingFang TC Medium", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Light", L"Skia Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Black", L"Skia Black", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Condensed", L"Skia Condensed", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Black-Condensed", L"Skia Condensed Black", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Light-Condensed", L"Skia Condensed Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Extended", L"Skia Extended", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Black-Extended", L"Skia Extended Black", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Skia", L"Skia-Regular_Light-Extended", L"Skia Extended Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Songti SC", L"STSongti-SC-Light", L"Songti SC Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Songti SC", L"STSongti-SC-Black", L"Songti SC Black", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Songti TC", L"STSongti-TC-Light", L"Songti TC Light", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Sukhumvit Set", L"SukhumvitSet-Light", L"Sukhumvit Set Light", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Sukhumvit Set", L"SukhumvitSet-Medium", L"Sukhumvit Set Medium", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Sukhumvit Set", L"SukhumvitSet-SemiBold", L"Sukhumvit Set Semibold", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Sukhumvit Set", L"SukhumvitSet-Thin", L"Sukhumvit Set Thin", MYON_FontFaceQuartet::Member::Regular),

    Internal_FakeWindowsLogfontName(L"Thonburi", L"Thonburi-Light", L"Thonburi Light", MYON_FontFaceQuartet::Member::Regular),

    // RH-68433
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Black",            L"Graphik Black",      MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-BlackItalic",      L"Graphik Black",      MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Bold",             L"Graphik Bold",       MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-BoldItalic",       L"Graphik Bold",       MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Extralight",       L"Graphik Extralight", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-ExtralightItalic", L"Graphik Extralight", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Light",            L"Graphik Light",      MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-LightItalic",      L"Graphik Light",      MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Medium",           L"Graphik Medium",     MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-MediumItalic",     L"Graphik Medium",     MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Regular",          L"Graphik Regular",    MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-RegularItalic",    L"Graphik Regular",    MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Semibold",         L"Graphik Semibold",   MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-SemiboldItalic",   L"Graphik Semibold",   MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Super",            L"Graphik Super",      MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-SuperItalic",      L"Graphik Super",      MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-Thin",             L"Graphik Thin",       MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Graphik", L"Graphik-ThinItalic",       L"Graphik Thin",       MYON_FontFaceQuartet::Member::Italic),

    // RH-68539
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-15UltTh",   L"NeueHaasGroteskDisp UltTh", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-16UltThIt", L"NeueHaasGroteskDisp UltTh", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-25Th",      L"NeueHaasGroteskDisp Pro Thin", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-26ThIt",    L"NeueHaasGroteskDisp Pro Thin", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-35XLt",     L"NeueHaasGroteskDisp Pro XLt", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-36XLtIt",   L"NeueHaasGroteskDisp Pro XLt", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-45Lt",      L"NeueHaasGroteskDisp Pro Lt", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-46LtIt",    L"NeueHaasGroteskDisp Pro Lt", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-55Rg",      L"NeueHaasGroteskDisp Pro", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-56It",      L"NeueHaasGroteskDisp Pro", MYON_FontFaceQuartet::Member::Italic),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-75Bd",      L"NeueHaasGroteskDisp Pro", MYON_FontFaceQuartet::Member::Bold),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-76BdIt",    L"NeueHaasGroteskDisp Pro", MYON_FontFaceQuartet::Member::BoldItalic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-65Md",      L"NeueHaasGroteskDisp Pro Md", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-66MdIt",    L"NeueHaasGroteskDisp Pro Md", MYON_FontFaceQuartet::Member::Italic),

    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-95Blk",     L"NeueHaasGroteskDisp Pro Blk", MYON_FontFaceQuartet::Member::Regular),
    Internal_FakeWindowsLogfontName(L"Neue Haas Grotesk Display Pro", L"NHaasGroteskDSPro-96BlkIt",   L"NeueHaasGroteskDisp Pro Blk", MYON_FontFaceQuartet::Member::Italic),
  };
  ...
  const Internal_FakeWindowsLogfontName key(family_name, postscript_name, L"", MYON_FontFaceQuartet::Member::Unset);
  ...
  if (
      fake_name
      && MYON_FontFaceQuartet::Member::Unset != fake_name->QuartetMember()
...
bool MYON_Font::TestInstalledFontList(MYON_TextLog& text_log)
{
  const class MYON_FontList& font_list = MYON_Font::InstalledFontList();
  const unsigned font_count = font_list.Count();
  if (font_count <= 0)
  {
    text_log.Print("ERROR: 0 = MYON_Font::InstalledFontList().Count()\n");
    return Internal_TestInstalledFontsFailure();
  }

  const MYON_SimpleArray< const class MYON_Font* >& by_hash = font_list.ByFontCharacteristicsHash();
  if (font_count != by_hash.UnsignedCount())
  {
    text_log.Print("ERROR: nullptr = MYON_Font::InstalledFontList()..FromFontCharacteristicsHash(MYON_Font::Default.FontCharacteristicsHash(),false)\n");
    return Internal_TestInstalledFontsFailure();
  }
  ...
  text_log.Print("Testing %u installed fonts:\n", font_count);
  {
    ...
    text_log.Print(L"FromFontCharacteristicsHash() tests ...");
    {
      ...
      {
        const MYON_Font* f = by_hash[i];
        const MYON_SHA1_Hash h = f->FontCharacteristicsHash();

        const MYON_Font* f1 = f = font_list.FromFontCharacteristicsHash(h, bReturnFirst);
        if (f != f1)
        {
          if (0 == error_count)
            text_log.PrintNewLine();
          ++error_count;
          text_log.Print("ERROR: nullptr = MYON_Font::InstalledFontList().FromFontCharacteristicsHash(by_hash[%u],false).\n", i);
          bPassedTest = false;
        }
      }
      if (error_count > 0)
    ...
    {
      const MYON_Font* f = font_list.FromFontCharacteristicsHash(MYON_Font::Default.FontCharacteristicsHash(), bReturnFirst);
      if (nullptr == f)
      {
        text_log.Print("ERROR: nullptr = MYON_Font::InstalledFontList()..FromFontCharacteristicsHash(MYON_Font::Default.FontCharacteristicsHash(),false)\n");
        bPassedTest = false;
  ...
  const MYON_ClassArray< MYON_FontFaceQuartet >& quartet_list = font_list.QuartetList();
  ...
  text_log.Print("Testing %u quartets:\n", quartet_count);
  {
    ...
    {
      const MYON_FontFaceQuartet& q = quartet_list[i];
      const MYON_wString qname = q.QuartetName();
      if (qname.IsEmpty())
      {
        ++error_count;
      ...
      const MYON_FontFaceQuartet q1 = font_list.QuartetFromQuartetName(qname);
      bool bQuartetFromQuartetNamePass = q.QuartetName() == q1.QuartetName();
      const MYON_Font* f[4] = { q.RegularFace(),q.BoldFace(),q.ItalicFace(),q.BoldItalicFace() };
      const MYON_Font* f1[4] = { q1.RegularFace(),q1.BoldFace(),q1.ItalicFace(),q1.BoldItalicFace() };
      const bool bExpectedIsBoldInQuartet[4] = { false,true,false,true };
      const bool bExpectedIsItalicInQuartet[4] = { false,false,true,true };
      const MYON_wString qface[4] = { MYON_wString(L"regular"), MYON_wString(L"bold"), MYON_wString(L"italic"), MYON_wString(L"bolditalic") };
      for (unsigned k = 0; k < 4; ++k)
      {
        ...
        if (nullptr != f[k])
        {
          MYON_wString qname1 = qname;
          qname1 += L" (";
          qname1 += qface[k];
          qname1 += L")";
          const MYON_SHA1_Hash h = f[k]->FontCharacteristicsHash();
          const MYON_Font* f2 = font_list.FromFontCharacteristicsHash(h, bReturnFirst);
          if (f2 != f[k])
          {
            ++error_count;
            text_log.Print("ERROR: nullptr = MYON_Font::InstalledFontList().FromFontCharacteristicsHash(%ls,false).\n", qname1.Array());
            bPassedTest = false;
          }
...
const MYON_wString MYON_Font::QuartetName(
  MYON_Font::NameLocale name_locale
) const
{
...
MYON_FontFaceQuartet::Member MYON_Font::QuartetFaceMember() const
{
...
const MYON_wString MYON_Font::QuartetName() const
{
  return MYON_Font::QuartetName(MYON_Font::NameLocale::LocalizedFirst);
}

const MYON_wString MYON_Font::QuartetDescription() const
{
  MYON_wString s = this->QuartetName();
  if (s.IsEmpty())
    return MYON_wString::EmptyString;
  if (s.IsNotEmpty())
  {
    const MYON_FontFaceQuartet::Member m = MYON_Font::QuartetFaceMember();
    if (MYON_FontFaceQuartet::Member::Unset != m)
    {
      s += L" (";
      s += MYON_FontFaceQuartet::MemberToString(m);
      s += L")";
    }
  }
  return s;
...
bool MYON_Font::IsBoldInQuartet() const
{
  if (MYON_FontFaceQuartet::Member::Unset != this->m_quartet_member)
  {
    return MYON_FontFaceQuartet::Member::Bold == this->m_quartet_member || MYON_FontFaceQuartet::Member::BoldItalic == this->m_quartet_member;
  }

  // ... sigh, start guessing from imperfect information
  const MYON_Font::Weight font_weight = this->FontWeight();

  MYON_FontFaceQuartet q = FontQuartet();
  ...
  if (q.IsEmpty())
  {
    ...
    return MYON_Font::IsBoldWeight(font_weight);
  }

  const bool bProbablyItalic = (MYON_Font::Style::Italic == m_font_style || MYON_Font::Style::Oblique == m_font_style);

  const MYON_Font* regular2[2] = { q.Face(false,bProbablyItalic), q.Face(false,!bProbablyItalic) };
  const MYON_Font* bold2[2] = { q.Face(true,bProbablyItalic), q.Face(true,!bProbablyItalic) };
  ...
  const MYON_SHA1_Hash this_hash = this->FontCharacteristicsHash();
  ...
  if (MYON_Font::Weight::Unset == this->FontWeight())
  ...
  if (nullptr != regular2[0] && MYON_Font::Weight::Unset != regular2[0]->FontWeight())
    regular_weight = static_cast<unsigned>(regular2[0]->FontWeight());
  else if (nullptr != bold2[0] && MYON_Font::Weight::Unset != bold2[0]->FontWeight())
    regular_weight = static_cast<unsigned>(bold2[0]->FontWeight())-1;
  else
    regular_weight = static_cast<unsigned>(MYON_Font::Weight::Normal);

  return static_cast<unsigned>(font_weight) > regular_weight;
}


bool MYON_Font::IsItalicInQuartet() const
{
  if (MYON_FontFaceQuartet::Member::Unset != this->m_quartet_member)
  {
    return MYON_FontFaceQuartet::Member::Italic == this->m_quartet_member || MYON_FontFaceQuartet::Member::BoldItalic == this->m_quartet_member;
  }

  const bool bProbablyItalic = (MYON_Font::Style::Italic == m_font_style || MYON_Font::Style::Oblique == m_font_style);

  const MYON_FontFaceQuartet q = InstalledFontQuartet(); // only look at installed quartets for italic query
  if (q.IsEmpty())
    return bProbablyItalic; // this font is not in a quartet

  const MYON_SHA1_Hash this_hash = this->FontCharacteristicsHash();

  const MYON_Font* upright2[2] = { q.RegularFace(), q.BoldFace() };
  ...
  const MYON_Font* italic2[2] = { q.ItalicFace(), q.BoldItalicFace() };
  ...
  const MYON_Font* upright = upright2[bBoldInQuartet ? 1 : 0];
  const MYON_Font* italic = italic2[bBoldInQuartet ? 1 : 0];
...
const MYON_FontFaceQuartet MYON_Font::FontQuartet() const
{
  const MYON_wString quartet_name[2] = { QuartetName(), this->RichTextFontName() };
  const MYON_SHA1_Hash quartet_name_hash[2] = { MYON_Font::FontNameHash(quartet_name[0],false), MYON_Font::FontNameHash(quartet_name[1],false) };
  ...
  {
    MYON_FontFaceQuartet installed_quartet = MYON_Font::InstalledFontList().QuartetFromQuartetName(quartet_name[i]);
    if (installed_quartet.IsNotEmpty())
  ...
  for (int i = imin; i <= imax; ++i)
  {
    MYON_FontFaceQuartet managed_quartet = MYON_Font::ManagedFontList().QuartetFromQuartetName(quartet_name[i]);
...
const MYON_FontFaceQuartet MYON_Font::InstalledFontQuartet() const
{
  return InstalledFontList().QuartetFromQuartetName(QuartetName());
}


const MYON_wString& MYON_Font::Internal_GetName(
  MYON_Font::NameLocale name_locale,
  const MYON_wString& localized_name,
  const MYON_wString& english_name
)
{
  if (MYON_Font::NameLocale::Localized == name_locale)
    return localized_name;
  if (MYON_Font::NameLocale::English == name_locale)
    return english_name;
  return localized_name.IsNotEmpty() ? localized_name : english_name;
}

const MYON_wString Internal_RichTextCleaner
(
  const wchar_t* dirty_name
)
{
  MYON_wString clean_name(dirty_name);
  clean_name.TrimLeftAndRight();
  clean_name.Remove(MYON_wString::Space);
  clean_name.Remove(MYON_wString::HyphenMinus);
  return clean_name;
}

const MYON_wString MYON_Font::RichTextFontName() const
{
  return MYON_Font::RichTextFontName(this, false);
}

const MYON_wString MYON_Font::RichTextFontName(
  const MYON_Font* font,
...
)
{
  ...
  // and bold-italic), the MYON_Font::FamilyName() generally works well both
  ...
  const MYON_Font* fonts[2]{ font, bDefaultIfEmpty ? &MYON_Font::Default : nullptr };
  MYON_wString rtf_font_name;
  for (int i = 0; i < 2; i++)
  {
    const MYON_Font* f = fonts[i];
    if (nullptr == f)
      continue;
    ...
    rtf_font_name = MYON_Font::FamilyNameFromDirtyName(f->PostScriptName());
    if (rtf_font_name.IsNotEmpty())
      break;
  ...
  {
    rtf_font_name = MYON_Font::DefaultFamilyName();
  }
...
a MYON_Font::FontNameHash() to the corresponding correct font Family name.
*/
...
public:
  ...
  (
    ...
    : m_dirty_name_hash(MYON_Font::FontNameHash(dirty_name,true))
  ...
  (
    ...
    : m_dirty_name_hash(MYON_Font::FontNameHash(clean_family_name,false))
    , m_family_name(clean_family_name)
  {}
...
public:
  ...
  {
    return MYON_SHA1_Hash::Compare(((const InternalHashToName*)lhs)->m_dirty_name_hash, ((const InternalHashToName*)rhs)->m_dirty_name_hash);
  }
  ...
  static size_t SortAndCullByHash(InternalHashToName* a, size_t a_count)
  {
    ...
    MYON_qsort(a, a_count, sizeof(a[0]), InternalHashToName::CompareHash);
...
public:
  MYON_SHA1_Hash m_dirty_name_hash = MYON_SHA1_Hash::ZeroDigest;
  const wchar_t* m_family_name = nullptr;
};

static MYON_OutlineFigure::Type Internal_FigureTypeFromHashedFontName(
...
)
{
  ...
  if ( nullptr == key)
    return MYON_OutlineFigure::Type::Unknown;
  ...
  if (nullptr != e)
    return MYON_OutlineFigure::Type::SingleStroke;
  ...
  if (nullptr != e)
    return MYON_OutlineFigure::Type::DoubleStroke;
  ...
  if (nullptr != e)
    return MYON_OutlineFigure::Type::Perimeter;

  // return Unset instead of Unknown in this internal tool
  return MYON_OutlineFigure::Type::Unset;
}
...
)
{
  ...
  MYON_OutlineFigure::Type outline_type = Internal_FigureTypeFromHashedFontName(key);
  if (MYON_OutlineFigure::Type::SingleStroke == outline_type)
    return true;
  if (MYON_OutlineFigure::Type::DoubleStroke == outline_type)
    return true;
...
const wchar_t* MYON_OutlineFigure::OrientationToWideString(
...
)
{
  ...
  switch (orientation)
  {
  case MYON_OutlineFigure::Orientation::Unset:
    s = L"Unset";
    break;
  case MYON_OutlineFigure::Orientation::CounterClockwise:
    s = L"CounterClockwise";
    break;
  case MYON_OutlineFigure::Orientation::Clockwise:
    s = L"Clockwise";
    break;
  case MYON_OutlineFigure::Orientation::NotOriented:
    s = L"NotOriented";
    break;
  case MYON_OutlineFigure::Orientation::Error:
    s = L"Error";
    break;
  default:
    s = L"<INVALID>";
...
MYON_OutlineFigure::Type MYON_OutlineFigure::FigureTypeFromFontName(
  const wchar_t* font_name
)
{
  if (nullptr == font_name || 0 == font_name[0])
    return MYON_OutlineFigure::Type::Unset; // better than unknown when name is empty

  const InternalHashToName key(font_name, nullptr);
  return Internal_FigureTypeFromHashedFontName(&key);
}

MYON_OutlineFigure::Type MYON_OutlineFigure::FigureTypeFromField10Description(
  const MYON_wString field_10_description
)
{
  MYON_wString description(field_10_description);

  // ignore white space and hyphens
  description.Remove(MYON_wString::Space);
  description.Remove(MYON_wString::Tab);
  description.Remove(MYON_wString::HyphenMinus);
  ...
  if (description_len > 0)
  {
    ...
    const MYON_wString s[2] = {
      MYON_wString(L"singlestroke"),
      MYON_wString(L"doublestroke")
    };
    const MYON_OutlineFigure::Type t[2] = {
      MYON_OutlineFigure::Type::SingleStroke,
      MYON_OutlineFigure::Type::DoubleStroke
    };
...
MYON_OutlineFigure::Type MYON_Font::OutlineFigureType() const
{
  if (m_outline_figure_type != MYON_OutlineFigure::Type::Unset)
    return m_outline_figure_type;

  const MYON_wString names[] =
  {
    FamilyName(),
    FamilyName(MYON_Font::NameLocale::English),
    PostScriptName(),
    PostScriptName(MYON_Font::NameLocale::English)
  };
  ...
  {
    const MYON_wString& name = names[i];
    if (name.IsEmpty())
      continue;
    ...
    MYON_OutlineFigure::Type figure_type = Internal_FigureTypeFromHashedFontName(&key[key_count]);
    key_count++;
    if (
      MYON_OutlineFigure::Type::Unset != figure_type
      && MYON_OutlineFigure::Type::Unknown != figure_type
      && MYON_OutlineFigure::Type::Mixed != figure_type
      )
      return figure_type;
  }
  return MYON_OutlineFigure::Type::Unknown; // unknown means don't try again
}


bool MYON_Font::IsSingleStrokeFont() const
{
  return MYON_OutlineFigure::Type::SingleStroke == OutlineFigureType();
}


bool MYON_Font::IsDoubleStrokeFont() const
{
  return MYON_OutlineFigure::Type::DoubleStroke == OutlineFigureType();
}

bool MYON_Font::IsSingleStrokeOrDoubleStrokeFont() const
{
  const MYON_OutlineFigure::Type figure_type = OutlineFigureType();
  return (
    MYON_OutlineFigure::Type::SingleStroke == figure_type
    || MYON_OutlineFigure::Type::DoubleStroke == figure_type
    );
}

const MYON_Font* MYON_Font::DefaultEngravingFont()
{
  ...
  /*
   Font description = "SLF-RHN Architect Regular"
      ...
      LOGFONT
  */
  static const MYON_Font* default_engraving_font = nullptr;

  if (nullptr == default_engraving_font)
  {
    default_engraving_font = MYON_Font::InstalledFontList().FromNames(
      ...
      MYON_Font::Weight::Normal,  // prefered_weight,
      MYON_Font::Stretch::Medium, // prefered_stretch,
      MYON_Font::Style::Upright,   // prefered_style,
...
bool MYON_Font::IsEngravingFont() const
{
  const MYON_wString names[] =
  {
    FamilyName(),
...
const MYON_wString MYON_Font::FamilyNameFromDirtyName(
  const wchar_t* dirty_name
)
{
  if (nullptr == dirty_name
    || 0 == dirty_name[0]
    || MYON_wString::HyphenMinus == dirty_name[0])
  {
    return MYON_wString::EmptyString;
  }

  // incredibly common query
  if (MYON_wString::EqualOrdinal(MYON_Font::DefaultFamilyName(), dirty_name, true))
  {
    return MYON_wString(MYON_Font::DefaultFamilyName());
  }

  static bool bInitialized = false;
  static MYON_SimpleArray<InternalHashToName> clean_names;
  static MYON_SimpleArray<InternalHashToName> dirty_names;
  ...
  if (false == bInitialized)
  {
    ...
    MYON_MemoryAllocationTracking disable_tracking(false);
    MYON_SimpleArray<const MYON_Font*> installed_fonts;
    MYON_Font::GetInstalledFontList(installed_fonts);
    ...
    for (size_t i = 0; i < installed_count; i++)
    {
      ...
      const MYON_Font* font = installed_fonts[(int)i];
#else
      const MYON_Font* font = installed_fonts[i];
#endif
      if (nullptr == font)
        continue;
      ...
      if (false == MYON_wString::EqualOrdinal(clean_family_name, prev_clean_family_name, true))
      {
      ...
      for (int j = 0; j < 2; j++)
      {
        ...
        if (nullptr == other_name || 0 == other_name[0])
          continue;
        if (MYON_wString::EqualOrdinal(clean_family_name, other_name, true))
        {
    ...
    {
      MYON_SimpleArray<InternalHashToName>& a
        = (0 == adex)
        ? clean_names
        : dirty_names;
      ...
      MYON_qsort(a.Array(), count0, sizeof_name, InternalHashToName::CompareHash);
      int count1 = 0;
      ...
      for (size_t i = 0; i < count0; i++)
      {
        ...
        if (candidate.m_dirty_name_hash == e.m_dirty_name_hash)
        {
          ...
          if (MYON_wString::EqualOrdinal(candidate.m_family_name, e.m_family_name, true))
            continue; // duplicate
  ...
  if (nullptr != e)
  {
    ...
    return MYON_wString(e->m_family_name);
  }
  ...
  MYON_wString best_guess(dirty_name);
  for (const wchar_t* c = dirty_name; 0 != *c; c++)
  {
    if (MYON_wString::HyphenMinus != *c)
      continue;
...
const MYON_String MYON_Font::CleanFontName(
  const wchar_t* dirty_font_name,
  MYON_StringMapOrdinalType map
)
{
  MYON_wString clean_font_name(dirty_font_name);
  wchar_t* s0 = clean_font_name.Array();
  wchar_t* s1 = s0;
  const bool bMap = (MYON_StringMapOrdinalType::Identity != map);
  for (wchar_t* s = s0, c = *s++; 0 != c; c = *s++)
  {
    if (
      MYON_wString::Space == c
      || MYON_wString::HyphenMinus == c
      || MYON_wString::Underscore == c
      )
    {
    ...
    if (bMap)
      c = MYON_wString::MapCharacterOrdinal(map, c);
    *s1++ = c;
  }
  *s1 = 0;
  clean_font_name.SetLength(s1 - s0);
  if (map != MYON_StringMapOrdinalType::Identity)
  {
...
const MYON_SHA1_Hash MYON_Font::FontNameHash(
  const wchar_t* font_name,
  bool bStopAtHyphen
)
{
  ...
  {
    if (MYON_wString::Space == c)
      continue;
    if (MYON_wString::Underscore == c)
      continue;
    if (MYON_wString::HyphenMinus == c)
    {
      if (bStopAtHyphen)
      {
        ...
        if (font_name == (font_name0 + 6) && MYON_wString::EqualOrdinal(L"Arial-Black", 11, font_name0, 11, true))
          continue;
        if (font_name == (font_name0 + 3) && MYON_wString::EqualOrdinal(L"MS-", 3, font_name0, 3, true))
          continue;
        if (font_name == (font_name0 + 4) && MYON_wString::EqualOrdinal(L"OCR-A", 5, font_name0, 5, true))
          continue;
        if (font_name == (font_name0 + 9) && MYON_wString::EqualOrdinal(L"AvenirLT-Roman", 14, font_name0, 14, true))
          continue;
        if (font_name == (font_name0 + 12) && MYON_wString::EqualOrdinal(L"MecSoftFont-1", 13, font_name0, 13, true))
          continue;
        if (font_name == (font_name0 + 13) && MYON_wString::EqualOrdinal(L"MecSoft_Font-1", 14, font_name0, 14, true))
          continue;
        ...
        if (font_name == (font_name0 + 4) && MYON_wString::EqualOrdinal(L"SLF-", 4, font_name0, 4, true))
          continue;
        if (font_name == (font_name0 + 8) && MYON_wString::EqualOrdinal(L"SLF-RHN-", 8, font_name0, 8, true))
          continue;
    ...
    MYON__UINT32 cp = MYON_DecodeUTF16SurrogatePair(c, *font_name, 0);
    if (0 != cp)
    {
    ...
    {
      cp = MYON_UnicodeMapCodePointOrdinal(MYON_StringMapOrdinalType::MinimumOrdinal, (MYON__UINT32)c);
    }
...
int MYON_Font::CompareFontName(const MYON_wString& lhs, const MYON_wString& rhs)
{
  return MYON_Font::CompareFontNameWideChar(lhs,rhs);
}

int MYON_Font::CompareFontNameWideChar(const wchar_t* lhs, const wchar_t* rhs)
{
  if (lhs == rhs)
    return 0;
  if (nullptr == lhs)
  ...
  if (nullptr == rhs)
    return -1;
  if (MYON_Font::FontNameHash(lhs, false) == MYON_Font::FontNameHash(rhs, false))
    return 0;
  return MYON_wString::CompareOrdinal(lhs, rhs, true);
}

int MYON_Font::CompareFontNamePointer(const MYON_wString* lhs, const MYON_wString* rhs)
{
  if (lhs == rhs)
    return 0;
  if (nullptr == lhs)
  ...
  if (nullptr == rhs)
    return -1;
  return MYON_Font::CompareFontName(*lhs, *rhs);
}


int MYON_Font::CompareFontNameToHyphen(const MYON_wString& lhs, const MYON_wString& rhs)
{
  return MYON_Font::CompareFontNameToHyphenWideChar(lhs, rhs);
}

int MYON_Font::CompareFontNameToHyphenWideChar(const wchar_t* lhs, const wchar_t* rhs)
{
  if (lhs == rhs)
    return 0;
  if (nullptr == lhs)
  ...
  if (nullptr == rhs)
    return -1;
  if (MYON_Font::FontNameHash(lhs, true) == MYON_Font::FontNameHash(rhs, true))
    return 0;
  return MYON_wString::CompareOrdinal(lhs, rhs, true);
}

int MYON_Font::CompareFontNameToHyphenPointer(const MYON_wString* lhs, const MYON_wString* rhs)
{
  if (lhs == rhs)
    return 0;
  if (nullptr == lhs)
  ...
  if (nullptr == rhs)
    return -1;
  return MYON_Font::CompareFontNameToHyphen(*lhs, *rhs);
}
  ...
  )
{
  if (IsAtoZ(s))
  {
    for (size_t i = 0; i < count; i++)
    {
      const int len = (int)MYON_wString::Length(token[i]);
      if (MYON_wString::EqualOrdinal(token[i], len, s, len, true))
      {
        s += len;
        return token_rc[i];
      }
...
static bool SkipSeparator(bool bSkipSpace, const wchar_t*& s)
{
  ...
  case '.':
    ...
    break;
  default:
    if (bSkipSpace && MYON_wString::Space == s[0])
      rc = true;
    break;
  }
  if (rc)
  ...
  )
{
  ...
  {
  case 1: // Thin
    font_weight = MYON_Font::Weight::Thin;
    rc = true;
    break;
  ...
  case 2: // Light
    ...
    {
    case 0:
      font_weight = MYON_Font::Weight::Light;
      rc = true;
    case 1: // Semi
      font_weight = MYON_Font::Weight::Light;
      rc = true;
    case 2: // Ultra
      font_weight = MYON_Font::Weight::Ultralight;
      rc = true;
      break;
    }
    break;
  ...
  case 3: // Normal
    font_weight = MYON_Font::Weight::Normal;
    rc = true;
    break;

  case 4: // Medium
    font_weight = MYON_Font::Weight::Medium;
    rc = true;
    break;
  ...
  case 5: // Bold
    ...
    {
    case 0:
      font_weight = MYON_Font::Weight::Bold;
      rc = true;
    case 1: // Semi
      font_weight = MYON_Font::Weight::Semibold;
      rc = true;
    case 2: // Ultra
      font_weight = MYON_Font::Weight::Ultrabold;
      rc = true;
      break;
    }
    break;
  ...
  case 6: // Heavy
    font_weight = MYON_Font::Weight::Heavy;
    rc = true;
    break;
  }
  ...
  )
{
  ...
  {
  case 1: // Upright
    font_style = MYON_Font::Style::Upright;
    rc = true;
    break;

  case 2: // Italic
    font_style = MYON_Font::Style::Italic;
    rc = true;
    break;

  case 3: // Oblique
    font_style = MYON_Font::Style::Oblique;
    rc = true;
    break;
  }
...
bool MYON_Font::SetFromPostScriptName(
...
)
{
  ...
  MYON_wString local_postscript_font_name(postscript_font_name);
  ...
  *this = MYON_Font::Unset;
...
#elif defined (MYON_RUNTIME_APPLE_CORE_TEXT_AVAILABLE)
  ...
  bool bIsSubstituteFont = false;
  CTFontRef apple_font = MYON_Font::AppleCTFont(postscript_font_name,0.0, bIsSubstituteFont);
...
bool MYON_Font::SetFromFontDescription(
  const wchar_t* font_description
)
{
  ...
  )
{
  ...
  MYON_wString local_postscript_name(postscript_name);
  local_postscript_name.TrimLeftAndRight();
  postscript_name = static_cast<const wchar_t*>(local_postscript_name);

  if (nullptr == font_description || font_description[0] <= MYON_wString::Space)
  {
  ...
  //  so that it's string buffer is not shared with other MYON_wStrings.
  //  wchar_t values in local_face_name are modifed via const_cast<> below.
  MYON_wString local_face_name = static_cast<const wchar_t*>(font_description);
  const wchar_t* characteristics = nullptr;
  int face_name_length = local_face_name.Length();
  for (int i = 0; nullptr != special_cases[i]; i++)
  {
    const int special_case_length = MYON_wString::Length(special_cases[i]);
    if (special_case_length > face_name_length)
      continue;
    
    if (special_case_length < face_name_length && local_face_name[special_case_length] > MYON_wString::Space )
      continue;
    
    if (false == MYON_wString::EqualOrdinal(special_cases[i], special_case_length, font_description, special_case_length, true))
      continue;
  ...
  MYON_Font::Weight font_weight = MYON_Font::Default.m_font_weight;
  ...
  MYON_Font::Style font_style = MYON_Font::Default.m_font_style;
  for (wchar_t* s0 = const_cast<wchar_t*>(characteristics); 0 != *s0; s0++)
  {
    const wchar_t* s1 = s0;
  ...
  MYON_Font::Stretch font_stretch = MYON_Font::Default.m_font_stretch;
  for (wchar_t* s0 = const_cast<wchar_t*>(characteristics); 0 != *s0; s0++)
  {
    const wchar_t* s1 = s0;
  ...
  {
    if (*s >= MYON_wString::Space)
      face_name_length++;
    else
    {
      while (*s > 0 && *s <= MYON_wString::Space)
        s++;
      break;
    }
  }
...
bool MYON_Font::IsValid(MYON_TextLog* text_log) const
{
  return (    
    (FamilyName().IsNotEmpty() || WindowsLogfontName().IsNotEmpty() || PostScriptName().IsNotEmpty())
    && MYON_Font::Weight::Unset != m_font_weight
    && MYON_Font::Style::Unset != m_font_style
    && MYON_Font::Stretch::Unset != m_font_stretch
    );
}

void MYON_Font::Dump(MYON_TextLog& dump) const
{
  ...
  {
    const MYON_wString description = Description();
    dump.Print(L"Font description = \"%ls\"\n", static_cast<const wchar_t*>( description ));
    dump.PushIndent();
    
    const MYON_wString family_name = FamilyName();
    dump.Print(L"Family name = \"%ls\"\n", static_cast<const wchar_t*>(family_name ));
    const MYON_wString en_family_name = FamilyName(MYON_Font::NameLocale::English);
    ...
    const MYON_wString quartet_description = this->QuartetDescription();
    ...
    const MYON_wString en_windows_logfont_name = WindowsLogfontName(MYON_Font::NameLocale::English);
    ...
    const MYON_wString rich_text_name = MYON_Font::RichTextFontName(this, false);
    dump.Print(L"Rich text name = \"%ls\"\n", static_cast<const wchar_t*>(rich_text_name));

    if (this->IsManagedSubstitutedFont())
    {
      const MYON_Font* substitute = this->SubstituteFont();
    ...
    {
    case MYON_Font::Origin::Unset: break;
    case MYON_Font::Origin::Unknown: s = L"Unknown"; break;
    case MYON_Font::Origin::WindowsFont: s = L"Windows Font"; break;
    case MYON_Font::Origin::AppleFont: s = L"Apple Font";  break;
    default: s = MYON_wString::FormatToString(L"%u", static_cast<unsigned int>(this->FontOrigin())); break;
    };
    ...
    s = MYON_wString::EmptyString;
    const MYON_OutlineFigure::Type outline_figure_type = this->OutlineFigureType();
    switch (outline_figure_type)
    {
    case MYON_OutlineFigure::Type::Unset: break;
    case MYON_OutlineFigure::Type::Unknown: break;
    case MYON_OutlineFigure::Type::SingleStroke: s = L"Engraving - single stroke"; break;
    case MYON_OutlineFigure::Type::DoubleStroke: s = L"Engraving - double stroke"; break;
    case MYON_OutlineFigure::Type::Perimeter: s = L"Perimeter"; break;
    case MYON_OutlineFigure::Type::NotPerimeter: s = L"Not a perimeter"; break;
    case MYON_OutlineFigure::Type::Mixed: s = L"Mixed"; break;
    default: s = MYON_wString::FormatToString(L"%u", static_cast<unsigned int>(outline_figure_type)); break;
    };
  ...
  if (MYON_Font::IsValidPointSize(m_point_size))
  {
  ...
  const MYON_FontFaceQuartet q = this->FontQuartet();
  const MYON_FontFaceQuartet::Member m = q.QuartetMember(this);
  switch (m)
  {
  case MYON_FontFaceQuartet::Member::Regular:
  ...
  s = MYON_Font::WeightToWideString(FontWeight());
  if( s.IsEmpty())
    s = MYON_wString::FormatToString(L"%u", static_cast<unsigned int>(this->FontWeight()));
  if (SimulatedWeight())
    s += L" (simulated)";
  dump.Print(L"Weight = %ls\n", static_cast<const wchar_t*>(s));

  s = MYON_Font::StretchToWideString(FontStretch());
  if( s.IsEmpty())
    s = MYON_wString::FormatToString(L"%u", static_cast<unsigned int>(this->FontStretch()));
  if (SimulatedStretch())
    s += L" (simulated)";
  dump.Print(L"Stretch = %ls\n", static_cast<const wchar_t*>(s));

  s = MYON_Font::StyleToWideString(FontStyle());
  if( s.IsEmpty())
    s = MYON_wString::FormatToString(L"%u", static_cast<unsigned int>(this->FontStyle()));
  if (SimulatedStyle())
    s += L" (simulated)";
  ...
  const MYON_OutlineFigure::Type font_figure_type = this->OutlineFigureType();
  if (MYON_OutlineFigure::Type::SingleStroke == font_figure_type)
    dump.Print(L"Engraving font = Single-stroke\n");
  else if (MYON_OutlineFigure::Type::DoubleStroke == font_figure_type)
    dump.Print(L"Engraving font = Double-stroke\n");

  if ( 
    MYON_PANOSE1::FamilyKind::Any != m_panose1.PANOSE1FamilyKind()
    && MYON_PANOSE1::FamilyKind::NoFit != m_panose1.PANOSE1FamilyKind()
    )
  {
  ...
  if (false == bTextHash)
  {
    const MYON_wString characteristics_hash = FontCharacteristicsHash().ToString(true);
    ...
    {
      if (this == &MYON_Font::Default)
        dump.Print(L"Managed font <%u> (MYON_Font::Default)\n", runtime_sn);
...
#if defined(MYON_OS_WINDOWS_GDI)
    {
      ...
      MYON_Font::DumpLogfont(&logfont, dump);
    }
#elif defined (MYON_RUNTIME_APPLE_CORE_TEXT_AVAILABLE)
    {
      bool bIsSubstituteFont = false;
      ...
      dump.PushIndent();
      MYON_Font::DumpCTFont(apple_font, dump);
      dump.PopIndent();
      if (nullptr != apple_font && MYON_Font::Origin::AppleFont == m_font_origin)
      {
...
bool MYON_FontMetrics::IsUnset() const
{
  return (
    0 == m_UPM
    && 0 == m_ascent
...
bool MYON_FontMetrics::IsSet() const
{
  return IsUnset() ? false : true;
}

void MYON_FontMetrics::Dump(class MYON_TextLog& text_log) const
{
...
static const MYON_String Internal_WindowsLogFontCharSet(
  BYTE charSet
)
{
  MYON_String s;
...
#endif
  default:
    s = MYON_String::FormatToString("%u", (unsigned char)charSet);
    break;
  }
  return s;
}
...
static const MYON_String Internal_WindowsLogFontFamily(
  BYTE pitchAndFamily
)
{
  MYON_String s;
  ...
  default:
    s = MYON_String::FormatToString("%02X", family);
    break;
  };
  return s;
}

static const MYON_String Internal_WindowsLogFontQuality(
...
#endif
  default:
    s = MYON_String::FormatToString("%u", (unsigned int)lfQuality);
    break;
  }
  return s;
}

static const MYON_String Internal_WindowsLogFontOutPrecision(
...
)
{
  ...
  default:
    s = MYON_String::FormatToString("%u", (unsigned int)lfOutPrecision);
    break;
  }
  return s;
}

static const MYON_String Internal_WindowsLogFontClipPrecision(
...
)
{
  ...
  default:
    s = MYON_String::FormatToString("%u", (unsigned int)lfClipPrecision);
    break;
  }
  return s;
}

void MYON_Font::DumpLogfont(
  const LOGFONT* logfont,
...
)
{
  ...
  text_log.PushIndent();
  MYON_wString sFaceName = logfont->lfFaceName;
  ...
  default: s = MYON_String::EmptyString; break;
  }
  if (s.IsNotEmpty())
    s += " = ";
  s += MYON_String::FormatToString("%d", logfont->lfWeight);
  ...
  default: s = MYON_String::FormatToString("0x%02x", pitch);
  };
  s += " | ";
  s += Internal_WindowsLogFontFamily(logfont->lfPitchAndFamily);
  if (s.IsNotEmpty())
    s += L" = ";
  s += MYON_wString::FormatToString(L"0x%02x", (unsigned int)logfont->lfPitchAndFamily);
  text_log.Print("lfPitchAndFamily = %s\n", static_cast<const char*>(s));
  text_log.PopIndent();
}

void MYON_Font::DumpTextMetric(
  const TEXTMETRIC* tm,
  MYON_TextLog& text_log
)
{
  ...
  if (s.IsNotEmpty())
    s += " = ";
  s += MYON_wString::FormatToString(L"0x%02X", (unsigned int)tm->tmPitchAndFamily);
...
const MYON_wString MYON_WindowsDWriteFontInformation::FamilyName() const
{
  return m_loc_family_name.IsNotEmpty() ? m_loc_family_name : m_en_family_name;
}

const MYON_wString MYON_WindowsDWriteFontInformation::FaceName() const
{
  return m_loc_face_name.IsNotEmpty() ? m_loc_face_name : m_en_face_name;
}

const MYON_wString MYON_WindowsDWriteFontInformation::PostScriptName() const
{
  return m_loc_postscript_name.IsNotEmpty() ? m_loc_postscript_name : m_en_postscript_name;
}

const MYON_wString MYON_WindowsDWriteFontInformation::WindowsLogfontName() const
{
  return m_loc_gdi_family_name.IsNotEmpty() ? m_loc_gdi_family_name : m_en_gdi_family_name;
}

int MYON_WindowsDWriteFontInformation::CompareFamilyName(
  ...
  const MYON_WindowsDWriteFontInformation* rhs)
{
  ...
  return MYON_wString::CompareOrdinal(lhs->m_loc_family_name, rhs->m_loc_family_name, true);
}

int MYON_WindowsDWriteFontInformation::CompareFamilyNameFaceNameWeightStretchStyle(
  const MYON_WindowsDWriteFontInformation* lhs, 
  const MYON_WindowsDWriteFontInformation* rhs)
{
  if (lhs == rhs) return 0;
  if (nullptr == lhs) return 1;
  if (nullptr == rhs) return -1;
  int rc = MYON_wString::CompareOrdinal(lhs->FamilyName(), rhs->FamilyName(), true);
  if (0 != rc)
    return rc;
  rc = MYON_wString::CompareOrdinal(lhs->FaceName(), rhs->FaceName(), true);
  if (0 != rc)
    return rc;
...
int MYON_WindowsDWriteFontInformation::ComparePostScriptName(
  const MYON_WindowsDWriteFontInformation* lhs, 
  const MYON_WindowsDWriteFontInformation* rhs)
{
  if (lhs == rhs) return 0;
  if (nullptr == lhs) return 1;
  if (nullptr == rhs) return -1;
  return MYON_wString::CompareOrdinal(lhs->PostScriptName(), rhs->PostScriptName(), true);
}


unsigned char MYON_Font::WindowsLogfontCharSetFromLogfont(
  const LOGFONT* logfont,
...
)
{
  for (;;)
  {
    ...
    if ( MYON_wString::EqualOrdinal(L"CityBlueprint", -1, logfont->lfFaceName, logfont_capacity, true))
       return MYON_Font::WindowsConstants::logfont_symbol_charset;
    if ( MYON_wString::EqualOrdinal(L"CountryBlueprint", -1, logfont->lfFaceName, logfont_capacity, true))
       return MYON_Font::WindowsConstants::logfont_symbol_charset;
    if (MYON_Font::WindowsConstants::logfont_symbol_charset != logfont->lfCharSet)
      break;
    if (bValidateSymbolFont
      && MYON_Font::WindowsConstants::logfont_symbol_charset != MYON_Font::WindowsLogfontCharSetFromFaceName(logfont->lfFaceName)
      )
      break;
    return MYON_Font::WindowsConstants::logfont_symbol_charset;
  }

  return MYON_Font::WindowsConstants::logfont_default_charset;
}
...
MYON_Font::MYON_Font(
  ...
  : m_runtime_serial_number((MYON_Font::FontType::ManagedFont == font_type)?++MYON_Font::__runtime_serial_number_generator:0)
  , m_font_type(font_type)
{
  const LOGFONT logfont = dwrite_font_information.m_gdi_interop_logfont;
  m_windows_logfont_weight = logfont.lfWeight;
  m_font_weight = MYON_Font::WeightFromWindowsLogfontWeight(m_windows_logfont_weight);
  m_apple_font_weight_trait = MYON_Font::AppleFontWeightTraitFromWeight(m_font_weight);
  m_point_size = 0.0;

  m_font_style = (0 != logfont.lfItalic) ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;

  m_font_stretch = MYON_Font::FontStretchFromDWriteStretch(dwrite_font_information.m_stretch,MYON_Font::Stretch::Medium);

  m_font_bUnderlined = false;
  m_font_bStrikethrough = false;

  m_logfont_charset = MYON_Font::WindowsLogfontCharSetFromLogfont(&logfont,false);
  ...
  if ( MYON_Font::FontType::ManagedFont == font_type ||  MYON_Font::FontType::InstalledFont == font_type )
  {
    MYON_FontGlyphCache* font_cache = MYON_FontGlyphCache::New();
#if defined(MYON_OS_WINDOWS_GDI)
    ...
    m_font_glyph_cache = std::shared_ptr<MYON_FontGlyphCache>(font_cache);
  }
}

#endif

bool MYON_Font::Write(
  ...
  ) const
{
  ...
  {
    MYON_WARNING("This font should probably be an MYON_TextStyle.");
    return WriteV5(
      RuntimeSerialNumber(),
      MYON_nil_uuid,
      file
  ...
  for (;;)
  {
    ...
    MYON_wString obsolete_font_description = PostScriptName();
...
//#define CREATE_MISSING_FONT_TEST_3DM_FILE
      ...
      const MYON_wString MISSING_FONT_TEST_valid_font_family_name(L"Courier New");
      if (MISSING_FONT_TEST_valid_font_family_name == FamilyName() && FaceName().IsNotEmpty())
      {
        // Write Missing Font names
        const MYON_wString missing_font_family_name(L"Missing Font Test");
        const MYON_wString missing_font_face_name(FaceName());
        const MYON_wString missing_font_logfont_name(missing_font_family_name);
        MYON_wString missing_font_postscript_name = missing_font_family_name + L"-" + missing_font_face_name;
        missing_font_postscript_name.Remove(MYON_wString::Space);
  ...
  ) const
{
  ...
  while(rc)
  {
    rc = file.Write3dmReferencedComponentIndex(MYON_ModelComponent::Type::TextStyle,V5_font_index);
    if(!rc) 
      break;
    ...
    MYON_wString description = PostScriptName();
...

    {
      ...
      const MYON_wString logfont_name = WindowsLogfontName();
    ...
    // 1.1 additions
    int windows_logfont_weight = MYON_Font::WindowsLogfontWeightFromWeight(m_font_weight);
    if (file.Archive3dmVersion() < 60)
    {
    ...
    if(!rc) break;
    rc = file.WriteInt(MYON_Font::Style::Italic == m_font_style);
    if(!rc) break;
    rc = file.WriteDouble(MYON_FontMetrics::DefaultLineFeedRatio);
    if(!rc) break;
  ...
  )
{
  ...
  *this = MYON_Font::Unset;
  ...
  {
    MYON_WARNING("Should probably be reading an MYON_TextStyle");
    int font_index = -1;
    MYON_UUID font_id = MYON_nil_uuid;
    return ReadV5(
      file,
      &font_index,
      &font_id
  ...
  for (;;)
  {
    ...
    MYON_wString windows_logfont_name;
    ...
    if (MYON::RuntimeEnvironment::Windows == file.ArchiveRuntimeEnvironment())
    {
      ...
      const unsigned int broken_apple_font_name_version = MYON_VersionNumberConstruct(6, 0, 2016, 8, 18, 0);
      ...
      {
        postscript_name = MYON_wString::EmptyString;
      }
    }
    ...
    MYON_wString obsolete_font_description;
    ...
    const bool bSetLogfontWeight
      ...
      (nullptr == installed_font)
#else
      (MYON::RuntimeEnvironment::Windows == file.ArchiveRuntimeEnvironment())
#endif
    ;
    ...
    const bool bSetAppleFontWeightTrait
      =
#if defined(MYON_RUNTIME_WIN)
      (MYON::RuntimeEnvironment::Apple == file.ArchiveRuntimeEnvironment())
#else
      (nullptr == installed_font)
#endif
    ;
    ...
    if (!file.ReadByte(1,&quartet_member_as_unsigned))
      break;
    this->m_quartet_member = MYON_FontFaceQuartet::MemberFromUnsigned(quartet_member_as_unsigned);
  ...
  )
{
  *this = MYON_Font::Unset;
  ...
  MYON_wString postscript_name;
  MYON_Font::Weight font_weight = MYON_Font::Weight::Normal;
  MYON_Font::Stretch font_stretch = MYON_Font::Stretch::Unset;
  MYON_Font::Style font_style = MYON_Font::Style::Upright;
  ...
  for (;;)
  {
    ...
    MYON_wString corrupt_information_font_description;
    if (!file.ReadString(corrupt_information_font_description))
      break;

    if ( false == MYON_Font::IsNotAppleFontName(corrupt_information_font_description)  )
    {
      ...
      else if ( file.ArchiveOpenNURBSVersion() > MYON_VersionNumberConstruct(6, 2, 2018, 2, 23, 0) )
      {
        postscript_name = corrupt_information_font_description;
      }
    }
    ...
    m_logfont_charset = MYON_Font::WindowsLogfontCharSetFromFaceName(WindowsLogfontName());
    if (minor_version >= 1)
    {
      ...
      {
        font_weight = MYON_Font::WeightFromWindowsLogfontWeight(logfont_weight);
      }
      ...
      double obsolete_linefeed_ratio = MYON_FontMetrics::DefaultLineFeedRatio;
      if (!file.ReadDouble(&obsolete_linefeed_ratio))
        break;

      if (minor_version >= 2)
      {
        MYON_UUID uuid = MYON_nil_uuid;
  ...
  else if (rc)
  {
    ...
    m_loc_postscript_name = postscript_name;
    m_en_postscript_name = m_loc_postscript_name;
    m_loc_family_name = MYON_Font::FamilyNameFromDirtyName(m_loc_windows_logfont_name);
    m_en_family_name = MYON_Font::FamilyNameFromDirtyName(m_en_windows_logfont_name);
  }
  else
  {
    *this = MYON_Font::Default;
...
unsigned int MYON_Font::RuntimeSerialNumber() const
{
  return m_runtime_serial_number;
}

unsigned int MYON_Font::ManagedFontSerialNumber() const
{
  if (0 != m_runtime_serial_number)
    return m_runtime_serial_number;
  const MYON_Font* mananged_font = this->ManagedFont();
  return (nullptr == mananged_font) ? 0 : mananged_font->RuntimeSerialNumber();
}

#if defined(MYON_OS_WINDOWS_GDI)
static
int CALLBACK MYON__IsSymbolCharSetFontFaceNameHelper(ENUMLOGFONTEX* lf, NEWTEXTMETRICEX* tm, DWORD font_type, LPARAM)
{
...
unsigned char MYON_Font::WindowsLogfontCharSetFromFaceName(
  const wchar_t* face_name
  )
{
  unsigned char logfont_charset = MYON_Font::WindowsConstants::logfont_default_charset;


  if( nullptr != face_name && face_name[0] > MYON_wString::Space )
  {
    // Problem fonts
    if (MYON_wString::EqualOrdinal(L"CityBlueprint", -1, face_name, -1, true))
      return MYON_Font::WindowsConstants::logfont_symbol_charset;

    if (MYON_wString::EqualOrdinal(L"CountryBlueprint", -1, face_name, -1, true))
      return MYON_Font::WindowsConstants::logfont_symbol_charset;
    ...
    if(nullptr != font_hdc)
    {
      ...
      logfont.lfCharSet = MYON_Font::WindowsConstants::logfont_symbol_charset;
      if (7 == ::EnumFontFamiliesEx(font_hdc, &logfont, (FONTENUMPROC)MYON__IsSymbolCharSetFontFaceNameHelper, 0, 0))
      {
...
const MYON_wString& MYON_Font::FontDescription() const
{
  return
    m_loc_postscript_name.IsNotEmpty()
    ? m_loc_postscript_name
...
bool MYON_Font::SetFontDescriptionForExperts(
...
const wchar_t* MYON_Font::FontDescriptionAsPointer() const
{
  return static_cast<const wchar_t*>(PostScriptName());
}

const MYON_wString MYON_Font::Description(
) const
{
  return Description(MYON_Font::NameLocale::LocalizedFirst, MYON_wString::HyphenMinus, MYON_wString::Space, true);
}
...
) const
{
  ...
  {
    description += MYON_wString(L"[Not on device] ");
  }

  MYON_wString family_name = FamilyName();
  family_name.TrimLeftAndRight();
  MYON_wString logfont_name = WindowsLogfontName();
  ...
  if (family_name.IsEmpty() && logfont_name.IsNotEmpty())
  {
    ...
    const MYON_wString wws = this->WidthWeightSlantDescription();
    if (wws.IsNotEmpty())
    {
      description += MYON_wString(L" (");
      description += wws;
  ...
  else if (family_name.IsNotEmpty() )
  {
    // family face
    description += family_name;
    MYON_wString face_name = FaceName();
    face_name.TrimLeftAndRight();
    if ( face_name.IsNotEmpty() )
    {
      description += MYON_wString(L" ");
  ...
  {
    MYON_wString postscript_name = PostScriptName();
    postscript_name.TrimLeftAndRight();
    if (postscript_name.IsNotEmpty())
      description += postscript_name;
  }
  ...
  if (description.IsNotEmpty())
  {
    ...
    if (bUnderlined && bStrikethrough)
      description += MYON_wString(L" (underlined,strikethrough)");
    else if (bUnderlined)
      description += MYON_wString(L" (underlined)");
    else if (bStrikethrough)
      description += MYON_wString(L" (strikethrough)");
  }

  return description;
}

double MYON_Font::LinefeedRatio() const
{
  return MYON_FontMetrics::DefaultLineFeedRatio;
}

bool MYON_Font::HasUnsetProperties(
    bool bCheckFamilyName,
    bool bCheckPostScriptName
) const
{
  if (MYON_Font::Style::Unset == m_font_style)
    return true;
  if (MYON_Font::Weight::Unset == m_font_weight)
    return true;
  if (MYON_Font::Stretch::Unset == m_font_stretch)
    return true;
...
unsigned int MYON_Font::SetUnsetProperties(
...
)
{
  ...
  if (MYON_Font::Weight::Unset == m_font_weight
    && MYON_Font::Weight::Unset != source.m_font_weight
    )
  {
  ...
  if (MYON_Font::Style::Unset == m_font_style
    && MYON_Font::Style::Unset == source.m_font_style
    )
  {
  ...
  if (MYON_Font::Stretch::Unset == m_font_stretch
    && MYON_Font::Stretch::Unset != source.m_font_stretch
    )
  {
  ...
  bool bSameFamily = MYON_Font::EqualFontFamily(this, &source);
...
bool MYON_Font::SetFontFaceName(
  const wchar_t* windows_logfont_name
)
{
...
bool MYON_Font::SetFamilyName(
  const wchar_t* family_name
)
{
  MYON_wString local_family_name = MYON_Font::FamilyNameFromDirtyName(family_name);
  local_family_name.TrimLeftAndRight();
  family_name = local_family_name;

  if ( MYON_wString::EqualOrdinal(family_name, FamilyName(), false) )
  {
    return true;
  }
...
bool MYON_Font::SetWindowsLogfontName(
  const wchar_t* windows_logfont_name
  )
{
  if (false == MYON_Font::IsValidFaceName(windows_logfont_name))
    return false;

  const MYON_Font::Weight font_weight = MYON_Font::Weight::Normal;
  const MYON_Font::Style font_style = MYON_Font::Style::Upright;
  const MYON_Font::Stretch font_stretch = MYON_Font::Stretch::Medium;

  const MYON_Font* installed_font =
    MYON_Font::InstalledFontList().FromNames(
      nullptr,
      windows_logfont_name,
      nullptr,
      nullptr,
  ...
  {
    const unsigned char logfont_charset = MYON_Font::WindowsLogfontCharSetFromFaceName(windows_logfont_name);

    const MYON_wString family_name = MYON_Font::FamilyNameFromDirtyName(windows_logfont_name);

    if (
      MYON_wString::EqualOrdinal(windows_logfont_name, m_loc_windows_logfont_name, false)
      ...
      && MYON_Font::Origin::Unknown == m_font_origin
      )
    {
      return true;
    }
    ...
    if (MYON_wString::EqualOrdinal(L"Arial", windows_logfont_name, true))
      m_loc_postscript_name = L"ArialMT";
    else if (MYON_wString::EqualOrdinal(L"Segoe UI", windows_logfont_name, true)
      || MYON_wString::EqualOrdinal(L"SegoeUI", windows_logfont_name, true))
    {
      m_loc_postscript_name = L"SegoeUI";
    }
    else if (MYON_wString::EqualOrdinal(L"Helvetica Neue", windows_logfont_name, true)
      || MYON_wString::EqualOrdinal(L"HelveticaNeue", windows_logfont_name, true))
      m_loc_postscript_name = L"HelveticaNeue";
    else if (MYON_wString::EqualOrdinal(L"Helvetica", windows_logfont_name, true))
      m_loc_postscript_name = L"Helvetica";
    else
      m_loc_postscript_name = MYON_wString::EmptyString;
    ...
    m_font_origin = MYON_Font::Origin::Unknown;
  }
...
const wchar_t* MYON_Font::FontFaceName() const
{
...
const wchar_t* MYON_Font::WindowsLogfontNameAsPointer() const
{
  return 
    m_loc_windows_logfont_name.IsNotEmpty()
    ? static_cast<const wchar_t*>(m_loc_windows_logfont_name)
...
MYON_Font::Weight MYON_Font::FontWeight() const
{
  return m_font_weight;
}

bool MYON_Font::IsLight() const
{
  const int font_weight = MYON_Font::WindowsLogfontWeightFromWeight(m_font_weight);
  const int normal_weight = MYON_Font::WindowsLogfontWeightFromWeight(MYON_Font::Weight::Normal);
  return (font_weight < normal_weight && m_font_weight != MYON_Font::Weight::Unset);
}

bool MYON_Font::IsNormalWeight() const
{
  return (MYON_Font::Weight::Normal == m_font_weight || MYON_Font::Weight::Medium == m_font_weight);
}

bool MYON_Font::IsBold() const
{
  return MYON_Font::IsBoldWeight(m_font_weight);
}

bool MYON_Font::IsItalic() const
{
  return (MYON_Font::Style::Italic == m_font_style);
}

bool MYON_Font::IsUpright() const
{
  return (MYON_Font::Style::Upright == m_font_style);
}

bool MYON_Font::IsItalicOrOblique() const
{
  return (MYON_Font::Style::Italic == m_font_style || MYON_Font::Style::Oblique == m_font_style);
}

bool MYON_Font::IsOblique()
{
  return (MYON_Font::Style::Oblique == m_font_style);
}

void MYON_Font::Internal_AfterModification(
)
{
  ...
  // It means the MYON_Font may or may not have valid settings in platform specific
  // fields. In particular the Apple font name is typically no longer valid.
  if (MYON_Font::Origin::Unset != m_font_origin && MYON_Font::Origin::Unknown != m_font_origin)
    m_font_origin = MYON_Font::Origin::Unknown;
  m_panose1 = MYON_PANOSE1::Zero;
  m_simulated = 0;
  m_font_glyph_cache.reset();
}
void MYON_Font::Internal_ClearAllNames()
{
  Internal_ClearName(true, true, true, true);
}

void MYON_Font::Internal_ClearName(
...
)
{
  if (bClearFamilyName)
  {
    m_loc_family_name = MYON_wString::EmptyString;
    m_en_family_name = MYON_wString::EmptyString;
  }
  if (bClearFaceName)
  {
    m_loc_face_name = MYON_wString::EmptyString;
    m_en_face_name = MYON_wString::EmptyString;
  }
  if (bClearPostScriptName)
  {
    m_loc_postscript_name = MYON_wString::EmptyString;
    m_en_postscript_name = MYON_wString::EmptyString;
  }
  if (bClearWindowsLogfontName)
  {
    m_loc_windows_logfont_name = MYON_wString::EmptyString;
    m_en_windows_logfont_name = MYON_wString::EmptyString;
    m_quartet_member = MYON_FontFaceQuartet::Member::Unset;
  }
}


bool MYON_Font::Internal_SetFontWeightTrio(
  MYON_Font::Weight font_weight,
  int windows_logfont_weight,
  double apple_font_weight_trait,
...
{
  if (MYON_Font::Weight::Unset == font_weight)
  {
    if (windows_logfont_weight > 0 && windows_logfont_weight <= 1000)
    {
      font_weight = MYON_Font::WeightFromWindowsLogfontWeight(windows_logfont_weight);
    }
    else if (-1.0 <= apple_font_weight_trait && apple_font_weight_trait <= 1.0)
    {
      font_weight = MYON_Font::WeightFromAppleFontWeightTrait(apple_font_weight_trait);
    }
  }

  font_weight = MYON_Font::FontWeightFromUnsigned((unsigned int)static_cast<unsigned char>(font_weight));
  if (MYON_Font::Weight::Unset == font_weight)
    return false;

  if (false == (1 <= windows_logfont_weight && windows_logfont_weight <= 1000))
    windows_logfont_weight = MYON_Font::WindowsLogfontWeightFromWeight(font_weight);

  if (false == (-1.0 <= apple_font_weight_trait && apple_font_weight_trait <= 1.0) )
    apple_font_weight_trait = MYON_Font::AppleFontWeightTraitFromWeight(font_weight);
...
bool MYON_Font::SetFontWeight(MYON_Font::Weight font_weight)
{
...
bool MYON_Font::SetWindowsLogfontWeight(
  int windows_logfont_weight
)
{
  const MYON_Font::Weight font_weight = MYON_Font::WeightFromWindowsLogfontWeight(windows_logfont_weight);
  return Internal_SetFontWeightTrio(
    font_weight, 
...
int MYON_Font::WindowsLogfontWeight() const
{
  return
    (100 <= m_windows_logfont_weight && m_windows_logfont_weight <= 1000)
    ? m_windows_logfont_weight
    : MYON_Font::WindowsLogfontWeightFromWeight(FontWeight());
}

bool MYON_Font::SetAppleWeightOfFont(
  int apple_weight_of_font
)
{
  const MYON_Font::Weight font_weight = MYON_Font::WeightFromAppleWeightOfFont(apple_weight_of_font);
  double apple_font_weight_trait = MYON_Font::AppleFontWeightTraitFromWeight(font_weight);
...
int MYON_Font::AppleWeightOfFont() const
{
  return WindowsLogfontWeight() / 100;
}

bool MYON_Font::SetAppleFontWeightTrait(
  double apple_font_weight_trait
)
{
  if (false == MYON_IsValid(apple_font_weight_trait))
  ...
  const MYON_Font::Weight font_weight = MYON_Font::WeightFromAppleFontWeightTrait(apple_font_weight_trait);
...
double MYON_Font::AppleFontWeightTrait() const
{
  ...
  return
    ...
    ? m_apple_font_weight_trait
    : MYON_Font::AppleFontWeightTraitFromWeight(this->FontWeight());
}

double MYON_Font::AppleFontWeightTraitEx() const
{
  return
    MYON_Font::Origin::AppleFont == FontOrigin()
...
double MYON_Font::AppleFontWidthTrait() const
{
  return
    MYON_Font::Origin::AppleFont == FontOrigin()
...
bool MYON_Font::IsValidPointSize(
  double point_size
)
{
...
double MYON_Font::PointSize() const
{
  return m_point_size;
}

bool MYON_Font::SetPointSize(
  double point_size
)
{
  double x 
    = MYON_Font::IsValidPointSize(point_size) 
    ? point_size
    : 0.0;
  if (!(m_point_size == x))
  {
...
MYON_Font::Style MYON_Font::FontStyle() const
{
  return m_font_style;
}

bool MYON_Font::SetFontStyle(
  MYON_Font::Style font_style
  )
{
...
MYON_Font::Stretch MYON_Font::FontStretch() const
{
  return m_font_stretch;
}

bool MYON_Font::SetFontStretch(
  MYON_Font::Stretch font_stretch
  )
{
...
bool MYON_Font::IsUnderlined() const
{
  return m_font_bUnderlined;
}

bool MYON_Font::SetUnderlined(bool bUnderlined)
{
...
bool MYON_Font::IsStrikethrough() const
{
  return m_font_bStrikethrough;
}

bool MYON_Font::SetStrikethrough(bool bStrikethrough)
{
...
bool MYON_Font::IsSymbolFont() const
{
  return (MYON_Font::WindowsConstants::logfont_symbol_charset == m_logfont_charset);
}

const MYON_PANOSE1 MYON_Font::PANOSE1() const
{
  return m_panose1;
}

void MYON_Font::SetPANOSE1(
  MYON_PANOSE1 panose1
)
{
  m_panose1 = panose1;
...
unsigned char MYON_Font::LogfontCharSet() const
{
  return m_logfont_charset;
}

bool MYON_Font::SetLogfontCharSet(unsigned char logfont_charset)
{
  if (MYON_Font::WindowsConstants::logfont_symbol_charset != logfont_charset)
    logfont_charset = MYON_Font::WindowsConstants::logfont_default_charset;
...
const MYON_FontMetrics& MYON_Font::FontMetrics() const
{
  class MYON_FontGlyphCache* font_cache = FontGlyphCache(true);
  if (nullptr != font_cache)
    return font_cache->m_normalized_metrics;
  return MYON_FontMetrics::Unset;
}

const MYON_FontMetrics& MYON_Font::FontUnitFontMetrics() const
{
  class MYON_FontGlyphCache* font_cache = FontGlyphCache(true);
  if (nullptr != font_cache)
...
double MYON_Font::FontUnitToNormalizedScale() const
{
  class MYON_FontGlyphCache* font_cache = FontGlyphCache(true);
  if (nullptr != font_cache)
...
double MYON_Font::NormalizedToFontUnitScale() const
{
  class MYON_FontGlyphCache* font_cache = FontGlyphCache(true);
  if (nullptr != font_cache)
...
int MYON_Font::HeightOfI() const
{
  return FontMetrics().AscentOfCapital();
}

int MYON_Font::HeightOfLinefeed() const
{
  return FontMetrics().LineSpace();
}

double MYON_Font::HeightScale(double text_height) const
{
  return FontMetrics().GlyphScale(text_height);
}

int MYON_Font::GetStrikeoutSize() const
{
  return FontMetrics().StrikeoutThickness();
}

int MYON_Font::GetStrikeoutPosition() const
{
  return FontMetrics().StrikeoutPosition();
}

int MYON_Font::GetUnderscoreSize() const
{
  return FontMetrics().UnderscoreThickness();
}

int MYON_Font::GetUnderscorePosition() const
{
  return FontMetrics().UnderscorePosition();
}

const class MYON_SHA1_Hash& MYON_Font::FontCharacteristicsHash() const
{
  if (m_font_characteristics_hash.IsZeroDigest())
  {
    ...
    MYON_SHA1_Hash string_hash;
    const MYON_wString windows_logfont_name = WindowsLogfontName();
    if ( windows_logfont_name.IsNotEmpty() )
    {
      string_hash = MYON_SHA1_Hash::StringHash(windows_logfont_name);
      sha1.AccumulateSubHash(string_hash);
    }
    sha1.AccumulateUnsigned8(static_cast<MYON__UINT8>(m_font_weight));
    sha1.AccumulateUnsigned8(static_cast<MYON__UINT8>(m_font_style));
    sha1.AccumulateUnsigned8(static_cast<MYON__UINT8>(m_font_stretch));
    sha1.AccumulateBool(m_font_bUnderlined);
    sha1.AccumulateBool(m_font_bStrikethrough);
    sha1.AccumulateUnsigned8(static_cast<MYON__UINT8>(m_logfont_charset));

    const MYON_wString family_name = FamilyName();
    if (family_name.IsNotEmpty())
    {
      string_hash = MYON_SHA1_Hash::StringHash(family_name);
      sha1.AccumulateSubHash(string_hash);
    }
    ...
    if (postscript_name.IsNotEmpty())
    {
      string_hash = MYON_SHA1_Hash::StringHash(postscript_name);
      sha1.AccumulateSubHash(string_hash);
    }
#endif
...
int MYON_Font::CompareFontCharacteristicsForExperts(
...
)
{
  ...
  for (;;)
  {
    const MYON_wString lhs_family_name = lhs.FamilyName();
    const MYON_wString rhs_family_name = rhs.FamilyName();
    ...
    {
      i = MYON_wString::CompareOrdinal(lhs_family_name, rhs_family_name, true);
      j = 0;
      if ( i != j )
        break;
    }

    const MYON_wString lhs_logfont_name = lhs.WindowsLogfontName();
    const MYON_wString rhs_logfont_name = rhs.WindowsLogfontName();
    ...
    {
      i = MYON_wString::CompareOrdinal(lhs_logfont_name, rhs_logfont_name, true);
      j = 0;
      if ( i != j )
        break;
    }

    const MYON_Font::Weight lhs_weight = lhs.m_font_weight;
    const MYON_Font::Weight rhs_weight = rhs.m_font_weight;
    const bool bCompareWeight =
      false == bIgnoreUnsetCharacteristics
      || (MYON_Font::Weight::Unset != lhs_weight && MYON_Font::Weight::Unset != rhs_weight);
    if (bCompareWeight)
    {
      i = MYON_Font::WindowsLogfontWeightFromWeight(lhs_weight);
      j = MYON_Font::WindowsLogfontWeightFromWeight(rhs_weight);
      if (i != j)
        break;
    }

    const MYON_Font::Style lhs_style = lhs.m_font_style;
    const MYON_Font::Style rhs_style = rhs.m_font_style;
    const bool bCompareStyle =
      false == bIgnoreUnsetCharacteristics
      || (MYON_Font::Style::Unset != lhs_style && MYON_Font::Style::Unset != rhs_style);
    if (bCompareStyle)
    {
    ...
    const MYON_Font::Stretch lhs_stretch = lhs.m_font_stretch;
    const MYON_Font::Stretch rhs_stretch = rhs.m_font_stretch;
    const bool bCompareStretch =
      false == bIgnoreUnsetCharacteristics
      || (MYON_Font::Stretch::Unset != lhs_stretch && MYON_Font::Stretch::Unset != rhs_stretch);
    if (bCompareStretch)
    {
    ...
    const double lhs_point_size = MYON_Font::IsValidPointSize(lhs.m_point_size) ? lhs.m_point_size : 0.0;
    const double rhs_point_size = MYON_Font::IsValidPointSize(rhs.m_point_size) ? rhs.m_point_size : 0.0;
    j = 0;
...
#if defined(MYON_RUNTIME_WIN) 
    i = (MYON_Font::WindowsConstants::logfont_symbol_charset == lhs.m_logfont_charset) ? 1 : 0;
    j = (MYON_Font::WindowsConstants::logfont_symbol_charset == rhs.m_logfont_charset) ? 1 : 0;
    if ( i != j )
      break;
#endif
...
#if defined(MYON_RUNTIME_APPLE)     
  const MYON_wString lhs_postscript_name = lhs.PostScriptName();
  const MYON_wString rhs_postscript_name = rhs.PostScriptName();
    const bool bComparePostScriptName =
      false == bIgnoreUnsetCharacteristics
      || (lhs_postscript_name.IsNotEmpty() && rhs_postscript_name.IsNotEmpty());
    if (bComparePostScriptName)
    {
      i = MYON_wString::CompareOrdinal(lhs_postscript_name, rhs_postscript_name, false);
      if (0 == i)
      {
...
int MYON_Font::CompareFontCharacteristics(
  const MYON_Font& lhs,
  const MYON_Font& rhs
)
{
  ...
  return MYON_Font::CompareFontCharacteristicsForExperts(
...
bool MYON_Font::EqualFontCharacteristics(
  const MYON_Font& a,
  const MYON_Font& b
  )
{
  return (0 == MYON_Font::CompareFontCharacteristics(a,b));
}


MYON_Font::Origin MYON_Font::FontOriginFromUnsigned(
  unsigned int unsigned_font_origin
)
{
  switch (unsigned_font_origin)
  {
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Origin::Unset);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Origin::Unknown);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Origin::WindowsFont);
  MYON_ENUM_FROM_UNSIGNED_CASE(MYON_Font::Origin::AppleFont);
  };

  MYON_ERROR("Invalid unsigned_font_origin value.");
  return MYON_Font::Origin::Unset;
}

MYON_Font::Origin MYON_Font::FontOrigin() const
{
  return m_font_origin;
}
  
void MYON_Font::SetFontOrigin(
...
bool MYON_Font::IsSimulated() const
{
  return 0 != m_simulated;
}

bool MYON_Font::SimulatedWeight() const
{
  return (0 != (m_simulated & 2));
}

bool MYON_Font::SimulatedStretch() const
{
  return (0 != (m_simulated & 4));
}

bool MYON_Font::SimulatedStyle() const
{
  return (0 != (m_simulated & 8));
}

void MYON_Font::SetSimulated(
...
static int MYON_InternalVerticalPixelScale(
  HDC hdc,
  bool bDCtoLP,
  int y0
)
...
bool MYON_Font::GetWindowsDeviceToLogicalHeightScales(
  HDC hdc,
...
)
{
  ...
  const int device_to_logical_1000x = MYON_InternalVerticalPixelScale(hdc, true, thousand);
  const int logical_to_device_1000x = MYON_InternalVerticalPixelScale(hdc, false, thousand);
  if (device_to_logical_1000x <= 0 && logical_to_device_1000x <= 0)
  {
    MYON_ERROR("DPtoLP(hdc, ...) and LPtoDP(hdc, ...) failed.");
    return false;
  }
  ...
  else
  {
    ...
    int y0 = 0;
    int y1 = 0;
    const int s1024 = MYON_InternalVerticalPixelScale(hdc, bDCtoLP, 1024);
    for (y0 = 1; y0 <= 100; y0++)
    {
      y1 = MYON_InternalVerticalPixelScale(hdc, bDCtoLP, y0);
...
HDC MYON_Font::CreateWindowsLogfontDeviceContext()
{
  return ::CreateCompatibleDC(nullptr);
}

void MYON_Font::DeleteWindowsLogfontDeviceContext(
  HDC hdc
)
{
  if ( nullptr != hdc )
...
bool MYON_Font::GetWindowsTextMetrics(
  int map_mode,
  HDC hdc,
  const LOGFONT& logfont,
...
)
{
  ...
  for (;;)
  {
    ...
    if (false == bValidMM)
    {
      if (MYON_Font::MAP_MODE_ZERO_ERROR_SUPPRESS != map_mode)
      {
        MYON_ERROR("GetMapMode(hdc) failed.");
      }
      break;
    }
    ...
    if (0 == ::GetTextMetrics(hdc, &tm))
    {
      if (MYON_Font::MAP_MODE_ZERO_ERROR_SUPPRESS != map_mode)
      {
        MYON_ERROR("GetTextMetrics(hdc,...) failed.");
      }
      break;
    }
...
int MYON_Font::WindowsLogfontCharacterHeight(
  int map_mode,
  HDC hdc,
  const LOGFONT& logfont
)
{
  ...
  for (;;)
  {
    TEXTMETRIC tm;
    memset(&tm, 0, sizeof(tm));
    bool bHaveTextMetric = MYON_Font::GetWindowsTextMetrics(
      map_mode,
      hdc,
      logfont,
      tm
    ...
    if (false == bHaveTextMetric)
    {
      if (MYON_Font::MAP_MODE_ZERO_ERROR_SUPPRESS != map_mode)
      {
        MYON_ERROR("MYON_Font::GetWindowsTextMetrics...) failed.");
      }
      break;
    }
    ...
    {
      MYON_ERROR("tm.tmInternalLeading >= cellHeight.");
      break;
    }
...
int MYON_Font::WindowsLogfontCellHeight(
  int map_mode,
  HDC hdc,
  const LOGFONT& logfont
)
{
  ...
  for (;;)
  {
    ...
    if (cellHeight <= 0)
    {
      MYON_ERROR("tm.tmAscent + tm.tmDescent < 0.");
      break;
    }
...
int MYON_Font::WindowsLogfontCharacterHeightFromPointSize(
  int map_mode,
  HDC hdc,
  double point_size
)
{
  ...
  for (;;)
  {
    ...
    {
      MYON_ERROR("Invalid point_size parameter.");
      break;
    }
    ...
    case MM_TEXT:
      ...
      if(nullptr != hdc)
      {
        ...
        {
          MYON_ERROR("GetDeviceCaps(hdc, LOGPIXELSY) failed.");
          break;
        }

        double device_to_logical_scale = 1.0;
        if (false == MYON_Font::GetWindowsDeviceToLogicalHeightScales(hdc, &device_to_logical_scale, nullptr) )
        {
          MYON_ERROR("MYON_Font::GetWindowsDeviceToLogicalHeightScales(hdc, ...) failed.");
          break;
        }
    ...
    case MM_ISOTROPIC:
      ...
      // the units isotropic).
      MYON_ERROR("GetMapMode(hdc) returned MM_ISOTROPIC mapping mode.");
      break;
    case MM_ANISOTROPIC:
      // Logical units are mapped to arbitrary units with arbitrarily scaled axes. 
      // Use the SetWindowExtEx and SetViewportExtEx functions to specify the units, 
      // orientation, and scaling required.
      MYON_ERROR("GetMapMode(hdc) returned MM_ANISOTROPIC mapping mode.");
      break;
    default:
      MYON_ERROR("GetMapMode(hdc) returned undocumented mapping mode.");
    }
...
double MYON_Font::PointSizeFromWindowsLogfontCharacterHeight(
  int map_mode,
  HDC hdc,
  ...
  for (;;)
  {
    ...
    case MM_TEXT:
      ...
      if(nullptr != hdc)
      {
        ...
        if (false == MYON_Font::GetWindowsDeviceToLogicalHeightScales(hdc, nullptr, &logical_to_device_scale))
        {
          MYON_ERROR("MYON_Font::GetWindowsDeviceToLogicalHeightScales(hdc, ...) failed.");
          break;
        }
    ...
    for (;;)
    {
      ...
      if (i1 >= 1.0)
      {
        ...
        {
          if (logfont_lfHeight == MYON_Font::WindowsLogfontCharacterHeightFromPointSize(map_mode, hdc, i0))
          {
            point_size = i0;
            break;
          }
        ...
        {
          if (logfont_lfHeight == MYON_Font::WindowsLogfontCharacterHeightFromPointSize(map_mode, hdc, i1))
          {
            point_size = i1;
            break;
          }
        ...
        {
          if (logfont_lfHeight == MYON_Font::WindowsLogfontCharacterHeightFromPointSize(map_mode, hdc, 0.5*(i0 + i1)))
          {
...
bool MYON_Font::SetFromWindowsLogFont(
  int map_mode,
  HDC hdc,
...
)
{
  ...
  *this = MYON_Font::Unset;
  ...
  logfont.lfCharSet = MYON_Font::WindowsLogfontCharSetFromLogfont(&logfont, true);
  logfont.lfOutPrecision = MYON_Font::WindowsConstants::logfont_out_precis;
  ...
  const int logfont_lfHeight
    = (0 != logfont.lfHeight)
    ? MYON_Font::WindowsLogfontCharacterHeight(map_mode, hdc, logfont)
    : 0;

  const double point_size
    = (logfont_lfHeight < 0)
    ? MYON_Font::PointSizeFromWindowsLogfontCharacterHeight(map_mode, hdc, logfont_lfHeight)
    : 0.0;

  logfont.lfHeight = MYON_Font::Constants::AnnotationFontCellHeight;

  // TODO - use DWrite gdi interop to get an IDWriteFont and use it to set the MYON_Font.
  
  const MYON_Font::Weight font_weight = MYON_Font::WeightFromWindowsLogfontWeight(logfont.lfWeight);
  const MYON_Font::Style font_style = (0 != logfont.lfItalic) ? MYON_Font::Style::Italic : MYON_Font::Style::Upright;
  const MYON_Font::Stretch font_stretch = MYON_Font::Stretch::Medium;
  const double linefeed_ratio = MYON_FontMetrics::DefaultLineFeedRatio;
  ...
  SetFontOrigin(MYON_Font::Origin::WindowsFont);

  return rc;
}

const LOGFONT MYON_Font::WindowsLogFont(
  int map_mode,
  HDC hdc
  ) const
{
  ...
  logfont.lfHeight 
    = (m_point_size > 0.0)
    ? MYON_Font::WindowsLogfontCharacterHeightFromPointSize(map_mode,hdc,m_point_size)
    : MYON_Font::Constants::AnnotationFontCellHeight;

  logfont.lfWidth = 0;
  logfont.lfEscapement = 0;
  logfont.lfOrientation = 0;
  
  const MYON_Font::Weight font_weight_from_windows_logfont_weight = MYON_Font::WeightFromWindowsLogfontWeight(m_windows_logfont_weight);
  logfont.lfWeight 
    = (m_windows_logfont_weight > 0 && m_windows_logfont_weight <= 1000 && font_weight_from_windows_logfont_weight == m_font_weight)
    ? m_windows_logfont_weight
    : MYON_Font::WindowsLogfontWeightFromWeight(m_font_weight);

  logfont.lfItalic = (MYON_Font::Style::Italic == m_font_style) ? 1 : 0;
...
const MAT2 MYON_Font::WindowsFontMat2() const
{
  MAT2 mat2;
  memset(&mat2, 0, sizeof(mat2));
...
void MYON_Font::DestroyFontGlyphCache()
{
  m_font_glyph_cache.reset();
}

class MYON_FontGlyphCache* MYON_Font::FontGlyphCache(
  bool bCreateIfMissing
  ) const
{  
  MYON_FontGlyphCache* font_cache = m_font_glyph_cache.get();
  if(nullptr == font_cache)
  {
    // get the static MYON_Font that matches this one
    const MYON_Font* managed_font = this->ManagedFont();
    if(nullptr != managed_font)
    {
      // The first call to MYON_Font::GetManagedFont() adds
      // &MYON_Font::Default and calculates its font metric
...
// static
void MYON_Font::GetRunBounds(
  const MYON_Font& font,
  const wchar_t* text,
  double font_height_pixels,
  MYON::TextHorizontalAlignment horizontal_alignment,
  MYON::TextVerticalAlignment vertical_alignment,
  MYON_2dPoint& bounds_min,
  MYON_2dPoint& bounds_max, 
  int& line_count
)
{
  line_count = 0;
  bounds_min = MYON_2dPoint::Origin;
  bounds_max = MYON_2dPoint::Origin;

  MYON_wString run = text;
  int textlength = run.Length();
  if(textlength < 1)
    return;

  const MYON_Font* managed_font = font.ManagedFont();
  if (nullptr == managed_font)
    managed_font = &MYON_Font::Default;

  const MYON_FontMetrics& fm = managed_font->FontMetrics();

  const int height_of_I = fm.AscentOfCapital();
  const int height_of_LF = fm.LineSpace();

  MYON_TextBox text_box;
  line_count = MYON_FontGlyph::GetGlyphListBoundingBox(text, managed_font, text_box);
  ...
  const MYON_2dPoint first_line_basepoint = MYON_2dPoint::Origin;
  const MYON_2dPoint last_line_basepoint(first_line_basepoint.x, -(line_count - 1)*height_of_LF);

  MYON_2dVector offset = MYON_2dVector::ZeroVector;
  ...
  const double scale = font_height_pixels / (double)MYON_Font::Constants::AnnotationFontCellHeight;
  if (scale > 0.0)
  {
...
const MYON_FontGlyph* MYON_Font::Internal_ManagedCodePointGlyph(
  MYON__UINT32 unicode_code_point,
  bool bCreateIfMissing,
...
) const
{
  if ( false == MYON_IsValidUnicodeCodePoint(unicode_code_point) )
  {
    ...
    MYON_ERROR("invalid codepoint parameter.");
    return nullptr;
  }

  if (false == IsManagedFont())
  {
    MYON_ERROR("this->IsManagedFont() must be true.");
    return nullptr; 
  }
  
  // First see if we already have the glyph in the cache
  MYON_FontGlyphCache* font_cache = FontGlyphCache(true);
  MYON_GlyphMap* glyph_map
    = (nullptr == font_cache)
    ? nullptr
    : font_cache->m_glyphmap.get();
  if (nullptr == glyph_map)
  {
    MYON_ERROR("Unable to create MYON_FontGlyphCache.");
    return nullptr;
  }

  const MYON_FontGlyph* managed_glyph = glyph_map->FindGlyph(unicode_code_point);
  ...
  MYON_FontGlyph glyph;
  glyph.SetCodePoint(this, unicode_code_point);

  // Call external function to get glyph details
  MYON_TextBox font_unit_glyph_box = MYON_TextBox::Unset;
  const unsigned int font_glyph_index = MYON_ManagedFonts::GetGlyphMetricsInFontDesignUnits(this, unicode_code_point, font_unit_glyph_box);
  if (0 != font_glyph_index)
  {
    if (font_unit_glyph_box.IsSet())
    {
      ...
      MYON_TextBox normalized_glyph_box = font_unit_glyph_box;
      if (normalize_scale > 0.0)
        normalized_glyph_box = MYON_TextBox::Scale(font_unit_glyph_box, normalize_scale);
...
{
  return MYON_Internal_WindowsGetGlyphMetricsEx(font, 0, unicode_code_point, font_unit_glyph_box);
}

#endif

void MYON_Font::SetCustomMeasurementFunctions(
  MYON_GetGlyphMetricsFuncType measureGlyphFunc,
  MYON_GetFontMetricsFuncType metricsFunc
)
{
  MYON_Font::Internal_CustomGetGlyphMetricsFunc = measureGlyphFunc;
  MYON_Font::Internal_CustomGetFontMetricsFunc = metricsFunc;
}

unsigned int MYON_ManagedFonts::GetGlyphMetricsInFontDesignUnits(
...
)
{
  ...
  const MYON_FontGlyph g(font, unicode_code_point);

  if (nullptr != MYON_Font::Internal_CustomGetGlyphMetricsFunc)
  {
    glyph_index = MYON_Font::Internal_CustomGetGlyphMetricsFunc(&g, glyph_metrics_in_font_design_units);
    if (0 != glyph_index)
      return glyph_index;
  }

#if defined(MYON_OS_WINDOWS_GDI)
  glyph_index = MYON_WindowsDWriteGetGlyphMetrics(&g, glyph_metrics_in_font_design_units);
#elif defined(MYON_RUNTIME_APPLE_CORE_TEXT_AVAILABLE)
  glyph_index = MYON_AppleFontGetGlyphMetrics(&g, glyph_metrics_in_font_design_units);
#elif defined(OPENNURBS_FREETYPE_SUPPORT)
  // Look in opennurbs_system_rumtime.h for the correct place to define OPENNURBS_FREETYPE_SUPPORT.
  // Do NOT define OPENNURBS_FREETYPE_SUPPORT here or in your project setting ("makefile").
  glyph_index = MYON_FreeTypeGetGlyphMetrics(&g, glyph_metrics_in_font_design_units);
#endif

  if (0 != glyph_index)
    return glyph_index;

  glyph_metrics_in_font_design_units = MYON_TextBox::Unset;
  return 0;
}

bool MYON_ManagedFonts::GetFontMetricsInFontDesignUnits(
  const MYON_Font* font,
  MYON_FontMetrics& font_metrics_in_font_design_units
)
{
  if (nullptr == font)
  {
    font_metrics_in_font_design_units = MYON_FontMetrics::LastResortMetrics;
    return false;
  }

  const MYON_FontGlyphCache* font_glyph_cache = font->m_font_glyph_cache.get();
  ...
  MYON_FontMetrics fm(MYON_FontMetrics::Unset);
  for (;;)
  {
    if (nullptr != MYON_Font::Internal_CustomGetFontMetricsFunc)
    {
      MYON_Font::Internal_CustomGetFontMetricsFunc(font, fm);
      if (fm.AscentDescentAndUPMAreValid())
        break;
    }

#if defined(MYON_OS_WINDOWS_GDI)
    MYON_WindowsDWriteGetFontMetrics(font, fm);
#elif defined(MYON_RUNTIME_APPLE_CORE_TEXT_AVAILABLE)
    MYON_AppleFontGetFontMetrics(font, fm);
#elif defined(OPENNURBS_FREETYPE_SUPPORT)
    // Look in opennurbs_system_rumtime.h for the correct place to define OPENNURBS_FREETYPE_SUPPORT.
    // Do NOT define OPENNURBS_FREETYPE_SUPPORT here or in your project setting ("makefile").
    MYON_FreeTypeGetFontMetrics(font, fm);
#endif
  ...
  const int line_space =
    ...
    ? fm.LineSpace()
    : (int)ceil(MYON_FontMetrics::DefaultLineFeedRatio*(ascent - descent));
  ...
  font_metrics_in_font_design_units
    = (fm.HeightsAreValid())
    ? fm
    : MYON_FontMetrics::LastResortMetrics;

  return false;
}

void MYON_ManagedFonts::Internal_SetFakeWindowsLogfontName(
  const MYON_Font * font,
...
)
{
  if (nullptr == font)
    return;
  const_cast<MYON_Font * >(font)->m_loc_windows_logfont_name = fake_loc_logfont_name;
  const_cast<MYON_Font * >(font)->m_en_windows_logfont_name = fake_en_logfont_name;
}

void MYON_ManagedFonts::Internal_SetFakeWindowsLogfontNames(
...
)
{
  ...
  {
    const MYON_Font* f0 = device_list[i];
    if (nullptr == f0)
      continue;

    MYON_wString fake_loc_logfont_name = MYON_Font::FakeWindowsLogfontNameFromFamilyAndPostScriptNames(
      f0->FamilyName(MYON_Font::NameLocale::LocalizedFirst),
      f0->PostScriptName(MYON_Font::NameLocale::LocalizedFirst)
    );
    if (fake_loc_logfont_name.IsEmpty())
      continue;
    MYON_wString fake_en_logfont_name = MYON_Font::FakeWindowsLogfontNameFromFamilyAndPostScriptNames(
      f0->FamilyName(MYON_Font::NameLocale::English),
      f0->PostScriptName(MYON_Font::NameLocale::English)
    );
    ...
    {
      const bool bLocalFaceIsFamilyName = fake_loc_logfont_name.EqualOrdinal(f0->FamilyName(MYON_Font::NameLocale::LocalizedFirst), true);
      const bool bEnglishFaceIsFamilyName = fake_en_logfont_name.EqualOrdinal(f0->FamilyName(MYON_Font::NameLocale::English), true);
    ...
    MYON_ManagedFonts::Internal_SetFakeWindowsLogfontName(
      f0,
  ...
  device_list.QuickSort(MYON_FontList::CompareFamilyAndWindowsLogfontName);
  MYON_SimpleArray<const MYON_Font*> quartet_candidates(128);
  ...
  for (unsigned int i = 0; i < font_count; ++i)
  {
    ...
    {
      const MYON_Font* f = device_list[i + 1];
      if (nullptr == f || 0 != MYON_FontList::CompareFamilyAndWindowsLogfontName(&f0, &f))
        break;
    ...
    quartet_candidates.QuickSort(MYON_FontList::CompareStretch);

    // See which subset of family gets to use the family name as 
    // the fake logfont name.
    const int medium_stretch = (int)((unsigned)(MYON_Font::Stretch::Medium));
    MYON_Font::Stretch stretch0 = quartet_candidates[0]->FontStretch();
    ...
    {
      const MYON_Font::Stretch stretch = quartet_candidates[j]->FontStretch();
    ...
    for (unsigned int j = 0; j < quartet_candidate_count; ++j)
    {
      const MYON_Font* f = quartet_candidates[j];
      const MYON_Font::Stretch stretch = f->FontStretch();
      if (stretch == stretch0)
        continue;
      MYON_wString suffix = L" (";
      suffix += MYON_Font::StretchToWideString(stretch);
      if (suffix.Length() < 3)
        continue;
      suffix += L")";
      const MYON_wString loc_name = f->m_loc_windows_logfont_name+suffix;
      const MYON_wString en_name = f->m_en_windows_logfont_name+suffix;
      MYON_ManagedFonts::Internal_SetFakeWindowsLogfontName(f,loc_name,en_name);
      bSortAgain = true;
    }
  }
...
const MYON_FontList& MYON_ManagedFonts::InstalledFonts()
{
  if (0 == List.m_installed_fonts.Count())
  {
    ...
    MYON_SimpleArray<const MYON_Font*> device_list;
#if defined(MYON_OS_WINDOWS_GDI)
    MYON_ManagedFonts::Internal_GetWindowsInstalledFonts(device_list);
#elif defined (MYON_RUNTIME_APPLE_CORE_TEXT_AVAILABLE)
    MYON_ManagedFonts::Internal_GetAppleInstalledCTFonts(device_list);
    MYON_ManagedFonts::Internal_SetFakeWindowsLogfontNames(device_list);
#endif
    if (device_list.Count() > 0)
    {
      // Cull exact duplicates which occur in fonts like SLF..., Noto Emoji", ...
      const int count = device_list.Count();
      MYON_SimpleArray<int> index(count);
      for (int i = 0; i < count; ++i)
        index.Append(i);
      MYON_Sort(MYON::sort_algorithm::quick_sort, index.Array(), device_list.Array(), device_list.Count(), sizeof(MYON__UINT_PTR), (int (*)(const void*, const void*))MYON_FontList::CompareFontCharacteristicsHash);
      int j0 = index[0];
      ...
      for (int i = 1; i < count; ++i)
      {
        ...
        if (j0 == j1)
          continue;
        const MYON_Font* f0 = device_list[j0];
        const MYON_Font* f1 = device_list[j1];
        if (0 == MYON_FontList::CompareFontCharacteristicsHash(&f0, &f1))
        {
          // duplicate font - keep the first one the platform delivered.
          if (j0 < j1)
          {
            device_list[j1] = nullptr;
            delete const_cast<MYON_Font*>(f1);
            bCullNulls = true;
          }
          else if (j0 > j1)
          {
            device_list[j0] = nullptr;
            delete const_cast<MYON_Font*>(f0);
            j0 = j1;
            bCullNulls = true;
          }
        }
      ...
      if (bCullNulls)
      {
        ...
        for (int i = 0; i < count; ++i)
        {
          const MYON_Font* f = device_list[i];
          if (nullptr == f)
            continue;
  ...
  {
    // Calling List.m_installed_fonts.QuartetList() sets MYON_Font::m_quartet_member
    // for all installled font on Apple and for damaged installed fonts on Windows.
    // See comments near the end of MYON_FontList::QuartetList().
