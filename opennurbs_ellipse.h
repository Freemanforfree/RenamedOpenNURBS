...
class MYON_CLASS MYON_Ellipse
{
public:
  MYON_Ellipse(); // zeros all fields - plane is invalid
  ...
  MYON_Ellipse& operator=(const MYON_Circle&);

  bool Create(
    const MYON_Plane&,  // point on the plane
    double, double     // radii for x and y vectors
    );
  ...
  const MYON_Plane& Plane() const; // plane containing ellipse
  ...
  */
  ...
  bool GetFoci( MYON_3dPoint& F1, MYON_3dPoint& F2 ) const;
  ...
  MYON_3dPoint  PointAt( double ) const;
  MYON_3dVector DerivativeAt( 
                 int, // desired derivative ( >= 0 )
                 double // parameter
                 ) const;

  MYON_3dVector TangentAt( double ) const;  // returns unit tangent
  MYON_3dVector CurvatureAt( double ) const;  // returns curvature vector
  ...
  double EquationAt( const MYON_2dPoint& ) const;
  MYON_2dVector GradientAt( const MYON_2dPoint& ) const;
  ...
  int GetNurbForm( MYON_NurbsCurve& ) const; // returns 0=failure, 2=success
