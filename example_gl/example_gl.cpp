...
class CModel : public MYONX_Model
{
public:
  void GetObjectMaterial( int object_index, MYON_Material& material ) const;
  MYON_3dmView m_view;
...
{
  material.Default();
  //const MYON_Geometry* geo = 0;

  if ( object_index >= 0 && object_index <= m_object_table.Count() )
  {
    const MYONX_Model_Object& mo = m_object_table[object_index];
    if ( 0 != mo.m_object )
    {
      switch( mo.m_object->ObjectType() )
      {
      case MYON::surface_object:
      case MYON::brep_object:
      case MYON::mesh_object:
      case MYON::instance_reference:
        GetRenderMaterial( mo.m_attributes, material );
        break;
      default:
        {
          // use emmissive object color for curve objects
          MYON_Color c = WireframeColor( mo.m_attributes );
          MYON_Color black(0,0,0);
          material.Default();
...
void GetDefaultView( const MYON_BoundingBox& bbox, MYON_3dmView& view )
{
  ...
  camLoc = view.m_target + (dz + frus_near)*MYON_zaxis;
  camDir = -MYON_zaxis;
  camUp = MYON_yaxis;
...
MYON_BOOL32 myInitGL( const MYON_Viewport&, GLUnurbsObj*& );
...
#if defined(MYON_EXAMPLE_GL_USE_GLAUX)
...
#if defined(MYON_EXAMPLE_GL_USE_GLUT)
...
//
void SetGLModelViewMatrix( const MYON_Viewport& );
void SetGLProjectionMatrix( MYON_Viewport& );
///////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////
void SetGLModelViewMatrix( const MYON_Viewport& viewport )
{
  MYON_GL( viewport ); // updates GL model view matrix
}

void SetGLProjectionMatrix( MYON_Viewport& viewport )
{
  int pl, pr, pb, pt;
  viewport.GetScreenPort( &pl, &pr, &pb, &pt, NULL, NULL );
  MYON_GL( viewport, pl, pr, pb, pt ); // updates GL projection matrix
}

MYON_BOOL32 myInitGL( const MYON_Viewport& viewport, GLUnurbsObj*& nobj )
{
  ...
  // GLs.
  //MYON_Color background_color(0,128,128);
  MYON_Color background_color(0,63,127);
...
void MY_GL_CALLBACK myGLAUX_Reshape( GLsizei w, GLsizei h )
{
  ...
  if ( w != w0 || h != h0 ) {
    h0 = h;
    w0 = w;
    MYON_GL( glb_model->m_view.m_vp, 0, w-1, h-1, 0 ); // set projection transform
  }
  glViewport( 0, 0, w, h );
}
#endif
...
///////////////////////////////////////////////////////////////////////
static void myRotateView( MYON_Viewport& viewport,
                          ...
                          double angle )
{
  ...
  MYON_GL( viewport ); // update model view
}

static void myRotateLeftRight( MYON_Viewport& viewport, double angle )
{
  // MYON_3dVector axis = MYON_zaxis; // rotate camera about world z axis (z up feel)
  MYON_3dVector axis = MYON_zaxis; // rotate camera about world y axis (u up feel)
...
static void myRotateUpDown( MYON_Viewport& viewport, double angle )
{
...
{
  myRotateLeftRight( glb_model->m_view.m_vp, MYON_PI/12.0 );
}
...
{
  myRotateUpDown( glb_model->m_view.m_vp, MYON_PI/12.0 );
}
...
{
  double half_angle = 7.5*MYON_PI/180.0;
...
{
  static MYON_BOOL32 bMouseActive = false;
  static int mx0, my0;
  int mx, my;
...
void myDisplayObject( const MYON_Object& geometry, const MYON_Material& material, GLUnurbsObj* nobj )
{
  // Called from myDisplay() to show geometry.
  // Uses MYON_GL() functions found in rhinoio_gl.cpp.
  const MYON_Point* point=0;
  const MYON_PointCloud* cloud=0;
  const MYON_Brep* brep=0;
  const MYON_Mesh* mesh=0;
  const MYON_Curve* curve=0;
  const MYON_Surface* surface=0;

  // specify rendering material
  MYON_GL( material );

  brep = MYON_Brep::Cast(&geometry);
  if ( brep ) 
  {
    MYON_GL(*brep, nobj);
    return;
  ...
  curve = MYON_Curve::Cast(&geometry);
  if ( curve ) 
  {
    MYON_GL( *curve, nobj );
    return;
  ...
  surface = MYON_Surface::Cast(&geometry);
  if ( surface ) 
  {
    gluBeginSurface( nobj );
    MYON_GL( *surface, nobj );
    gluEndSurface( nobj );
    return;
  }

  point = MYON_Point::Cast(&geometry);
  if ( point ) 
  {
    MYON_GL(*point);
    return;
  ...
  cloud = MYON_PointCloud::Cast(&geometry);
  if ( cloud ) 
  {
    MYON_GL(*cloud);
    return;
...
void MY_GL_CALLBACK myDisplayLighting( const MYON_Viewport&, // viewport, // unreferenced
                                       const CModel& model
                                     )
{
  ...
  if ( light_count > 0 ) {
    ...
    for ( lighti = 0; lighti < maxlighti; lighti++ ) {
      MYON_GL( model.m_light_table[lighti].m_light, lighti+GL_LIGHT0 );
    }
  }
  else {
    // use default headlight
    // use basic bright white head light with a bit of ambient
    MYON_Light head_light;
    head_light.Default();
    MYON_GL( head_light, GL_LIGHT0 );
  }
}
...
static void myDrawAxesSprite( const MYON_Viewport& viewport, HDC hdc )
{
                         ...
                         )
{
  ...
  // display Rhino geometry using MYON_GL() functions found in rhinoio_gl.cpp
  int i;
  const int object_count = model.m_object_table.Count();
  for ( i = 0; i < object_count; i++ ) 
  {
    const MYONX_Model_Object& mo = model.m_object_table[i];
    if ( 0 != mo.m_object )
    {
