...
static bool MYON_ExtrusionPolyCurveProfileIsNotValid()
{
  return false; // good place for a breakpoint
}

bool MYON_Extrusion::IsValidPolyCurveProfile( const MYON_PolyCurve& polycurve, MYON_TextLog* text_log )
{
  const bool bAllowGaps = true;
  bool rc = polycurve.IsValid(bAllowGaps,text_log) ? true : MYON_ExtrusionPolyCurveProfileIsNotValid();
  if (!rc)
    return MYON_ExtrusionPolyCurveProfileIsNotValid();
  ...
  if ( 2 != polycurve.Dimension() )
  {
    ...
    MYON_BoundingBox bbox = polycurve.BoundingBox();
    if ( !bbox.IsValid() )
    {
      if ( text_log )
      {
  ...
  if ( profile_count > 1 )
  {
    ...
    {
      const MYON_Curve* segment = polycurve.SegmentCurve(i);
      if ( 0 == segment )
      {
        if ( text_log )
        {
...
bool MYON_Extrusion::CleanupPolyCurveProfile( MYON_PolyCurve& polycurve )
{
  if ( !MYON_Extrusion::IsValidPolyCurveProfile(polycurve) )
  {
    ...
    {
      MYON_Curve* old_segment = polycurve.SegmentCurve(i);
      if ( 0 == old_segment )
        return false;
    ...
    MYON_SimpleArray<MYON_PolyCurve*> new_polycurves(old_count);
    MYON_SimpleArray<MYON_Curve*> new_segments(old_count);
    MYON_PolyCurve* new_segment = 0;
    bool rc = true;
    for ( i = 0; i < old_count && rc; i++ )
    {
      ...
      {
        new_segment = new MYON_PolyCurve();
...
bool MYON_GetEndCapTransformation(MYON_3dPoint P, MYON_3dVector T, MYON_3dVector U, 
...
{
  if ( scale2d )
    *scale2d = MYON_Xform::IdentityTransformation;
  if ( rot3d )
    *rot3d = MYON_Xform::IdentityTransformation;
  ...
  p0.zaxis = T;
  p0.yaxis = U;
  p0.xaxis = MYON_CrossProduct(U,T);
  if ( !p0.xaxis.IsUnitVector() )
    p0.xaxis.Unitize();
  p0.UpdateEquation();
  xform.Rotation(MYON_xy_plane,p0);
  if ( rot3d )
    *rot3d = xform;
  if ( N.z > MYON_Extrusion::m_Nz_min && N.IsUnitVector() )
  {
    //double cosa = T.x*N.x + T.y*N.y + T.z*N.z; // N is relative to T
    double cosa = N.z; // N is relative to xy plane.
    for(;;)
    {
      MYON_3dVector A(-N.y,N.x,0.0); // N is relative to xy plane.
      if ( !A.IsValid() )
        break;
      double sina = A.Length();
      if ( !MYON_IsValid(sina) )
        break;
      if ( !A.Unitize() )
        break;
      ...
      MYON_Xform S(MYON_Xform::ZeroTransformation);
      ...
      MYON_Xform R;
      R.Rotation(sina,cosa,A,MYON_3dPoint::Origin);
      if ( rot3d )
        *rot3d = xform*R;
...
static void MYON_ExtrusionInitializeHelper(MYON_Extrusion& extrusion)
{
  extrusion.m_path.from = MYON_3dPoint::Origin;
  extrusion.m_path.to = MYON_3dPoint::Origin;
  extrusion.m_t.m_t[0] = 0.0;
  extrusion.m_t.m_t[1] = 1.0;
  extrusion.m_up = MYON_3dVector::ZeroVector;
  ...
  extrusion.m_N[0] = MYON_3dVector::ZeroVector;
  extrusion.m_N[1] = MYON_3dVector::ZeroVector;
  extrusion.m_path_domain.m_t[0] = 0.0;
  extrusion.m_path_domain.m_t[1] = 1.0;
  extrusion.m_bTransposed = false;
}

static void MYON_ExtrusionCopyHelper(const MYON_Extrusion& src,MYON_Extrusion& dst)
{
  if ( &src != &dst )
  {
    if ( dst.m_profile )
...
bool MYON_Extrusion::SetPath(MYON_3dPoint A, MYON_3dPoint B)
{
  double distAB = 0.0;
  bool rc =    A.IsValid() && B.IsValid() 
            && (distAB = A.DistanceTo(B)) > MYON_ZERO_TOLERANCE;
  if (rc)
  {
    m_path.from = A;
    m_path.to = B;
...
bool MYON_Extrusion::SetPathAndUp( MYON_3dPoint A, MYON_3dPoint B, MYON_3dVector up )
{
  double distAB = 0.0;

  bool rc =    up.IsValid() 
            && up.Length() > MYON_ZERO_TOLERANCE
            && A.IsValid() 
            && B.IsValid() 
            && (distAB = A.DistanceTo(B)) > MYON_ZERO_TOLERANCE;
  ...
  if (rc)
  {
    ...
    D.Unitize();
    double d = up*D;
    if ( !up.IsUnitVector() || fabs(d) > distAB*MYON_SQRT_EPSILON*0.015625 )
    {
      ...
      // validate up
      d = up*D;
      rc = ( up.IsUnitVector() && fabs(d) <= MYON_SQRT_EPSILON );
    }
...
int MYON_Extrusion::PathParameter() const
{
  return m_bTransposed ? 0 : 1;
}

int MYON_Extrusion::ProfileParameter() const
{
  return m_bTransposed ? 1 : 0;
}

MYON_3dPoint MYON_Extrusion::PathStart() const
{
  MYON_3dPoint P(MYON_3dPoint::UnsetPoint);
...
MYON_3dPoint MYON_Extrusion::PathEnd() const
{
  MYON_3dPoint P(MYON_3dPoint::UnsetPoint);
...
MYON_3dVector MYON_Extrusion::PathTangent() const
{
  MYON_3dVector T(MYON_3dVector::UnsetVector);
  if ( m_path.IsValid() )
    T = m_path.Tangent();
  return T;
}

void MYON_Extrusion::Destroy()
{
  if ( m_profile)
  {
    delete m_profile;
  ...
  MYON_ExtrusionInitializeHelper(*this);
  DestroyRuntimeCache();
  PurgeUserData();
}

bool MYON_Extrusion::SetMiterPlaneNormal(MYON_3dVector N, int end)
{
  bool rc = false;
  if ( end >= 0 && end <= 1 )
  {
    if (    N.IsValid() 
         && N.z > MYON_Extrusion::m_Nz_min
         && (N.IsUnitVector() || N.Unitize())
       )
    {
      if (fabs(N.x) <= MYON_SQRT_EPSILON && fabs(N.y) <= MYON_SQRT_EPSILON)
        N.Set(0.0,0.0,1.0);
      m_N[end] = N;
      m_bHaveN[end] = (N.z != 1.0);
      rc = true;
    }
    else if ( N.IsZero() || MYON_3dVector::UnsetVector == N )
    {
      m_bHaveN[end] = false;
      rc = true;
    }
...
void MYON_Extrusion::GetMiterPlaneNormal(int end, MYON_3dVector& N) const
{
...
int MYON_Extrusion::IsMitered() const
{
  int rc = 0;
...
int MYON_Extrusion::CapCount() const
{
...
int MYON_Extrusion::IsCapped() const
{
  ...
  {
    const MYON_PolyCurve* p = MYON_PolyCurve::Cast(m_profile);
    if ( 0 == p )
      return 0;
    const MYON_Curve* outer_profile = p->SegmentCurve(0);
    if ( 0 == outer_profile )
      return 0;
...
int MYON_Extrusion::FaceCount() const
{
  int face_count = 0;
  const MYON_Curve* profile0 = Profile(0);
...
bool MYON_Extrusion::IsSolid() const
{
  if ( !m_bCap[0] || !m_bCap[1] )
    return false;
  return 3 == IsCapped();
...
bool MYON_Extrusion::GetPathPlane( double s, MYON_Plane& plane ) const
{
  MYON_Plane p;
  p.origin = MYON_3dPoint::Origin;
  p.zaxis = PathTangent();
  p.yaxis = m_up;
  p.xaxis = MYON_CrossProduct(p.yaxis,p.zaxis);
  if ( !p.xaxis.Unitize() )
    return false;
  if ( !p.yaxis.Unitize() )
    return false;
  ...
  if ( !p.IsValid() )
  {
    p.yaxis = MYON_CrossProduct(p.zaxis,p.xaxis);
    p.yaxis.Unitize();
    if ( !p.IsValid() )
      return false;
  }
...
bool MYON_Extrusion::GetProfilePlane( double s, MYON_Plane& plane ) const
{
  MYON_Plane p;
  p.origin = MYON_3dPoint::Origin;
  p.zaxis = PathTangent();
...
bool MYON_Extrusion::GetProfileTransformation( double s, MYON_Xform& xform ) const
{
  //const MYON_3dVector* N = (end?(m_bHaveN[1]?&m_N[1]:0):(m_bHaveN[0]?&m_N[0]:0));
  const MYON_3dVector T = m_path.Tangent();
  if ( 0.0 == s )
  {
    return MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[0]),T,m_up,m_bHaveN[0]?&m_N[0]:0,xform,0,0);
  }
  if ( 1.0 == s )
  {
    return MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[1]),T,m_up,m_bHaveN[1]?&m_N[1]:0,xform,0,0);
  }
  MYON_Xform xform0, xform1;
  if ( !MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[0]),T,m_up,m_bHaveN[0]?&m_N[0]:0,xform0,0,0) )
    return false;
  if ( !MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[1]),T,m_up,m_bHaveN[1]?&m_N[1]:0,xform1,0,0) )
    return false;
...
static bool CleanProfileSegment( MYON_Curve* curve )
{
  MYON_NurbsCurve* nurbs_curve = MYON_NurbsCurve::Cast(curve);
  if ( nurbs_curve )
  {
...
static bool ProfileHelper( int desired_orientation, MYON_Curve* profile )
{
  ...
  if ( 0 == profile )
  {
    MYON_ERROR("MYON_Extrusion::Set/Add Profile - null input curve pointer.");
    return false;
  }
  MYON_BoundingBox bbox = profile->BoundingBox();
  if ( !bbox.IsValid() )
  {
    MYON_ERROR("MYON_Extrusion::Set/Add Profile - profile->BoundingBox() failed.");
    return false;
  }
  if ( fabs(bbox.m_min.z) > MYON_ZERO_TOLERANCE || fabs(bbox.m_max.z) > MYON_ZERO_TOLERANCE )
  {
    MYON_ERROR("MYON_Extrusion::Set/Add Profile - profile->BoundingBox() is not in the world xy plane.");
    return false;
  }
  if ( !profile->ChangeDimension(2) )
  {
    MYON_ERROR("MYON_Extrusion::Set/Add Profile - profile->ChangeDimension(2) failed.");
    return false;
  }

  if ( profile->IsClosed() )
  {
    int profile_orientation = MYON_ClosedCurveOrientation(*profile,0);
    ...
    case 0:
      if ( -1 == profile_orientation )
      {
        if ( !profile->Reverse() )
        {
          MYON_ERROR("MYON_Extrusion::SetOuterProfile() - profile->Reverse() failed.");
          return false;
        }
      ...
      {
        MYON_ERROR("MYON_Extrusion::SetOuterProfile() - profile has wrong orientation.");
        return false;
      }
      break;
    case -1:
      if ( 1 == profile_orientation )
      {
        if ( !profile->Reverse() )
        {
          MYON_ERROR("MYON_Extrusion::AddInnerProfile() - profile->Reverse() failed.");
          return false;
        }
        profile_orientation = -1;
      }
      if ( -1 != profile_orientation )
      {
        MYON_ERROR("MYON_Extrusion::AddInnerProfile() - profile has wrong orientation.");
        return false;
      }
      break;
    default:
      {
        MYON_ERROR("MYON_Extrusion::Set/Add Profile - invalid desired_orientation parameter.");
        return false;
      }
      break;
    }
  ...
  {
    MYON_ERROR("MYON_Extrusion::Set/Add Profile - profile is an open curve.");
    return false;
  }

  MYON_PolyCurve* polycurve = MYON_PolyCurve::Cast(profile);
  if ( 0 != polycurve )
  {
    ...
    {
      MYON_ERROR("MYON_Extrusion::Set/Add Profile - MYON_PolyCurve has no segments.");
      return false;
    }
    
    if ( polycurve->SegmentCurves().Count() + 1 != polycurve->SegmentParameters().Count() )
    {
      MYON_ERROR("MYON_Extrusion::Set/Add Profile - MYON_PolyCurve segment and parameter counts do not agree.");
      return false;
    }

    for ( int i = polycurve->Count()-1; i >= 0; i-- )
    {
      MYON_Curve* segment = polycurve->SegmentCurve(i);
    ...
    for ( int i = 0; i < polycurve->Count(); i++ )
    {
      ...
      if ( 0 == segment )
      {
        MYON_ERROR("MYON_Extrusion::Set/Add Profile - MYON_PolyCurve has null segment.");
        return false;
      }
      const MYON_Interval segment_domain = polycurve->SegmentDomain(i);
      if ( !segment_domain.IsIncreasing() )
      {
        MYON_ERROR("MYON_Extrusion::Set/Add Profile - segment has invalid domain.");
        return false;
      }
      if ( !segment->SetDomain( segment_domain ) )
      {
        MYON_ERROR("MYON_Extrusion::Set/Add Profile - segment->SetDomain() failed.");
        return false;
      }
    }
  }
...
bool MYON_Extrusion::SetOuterProfile( MYON_Curve* outer_profile, bool bCap )
{
  if ( 0 != m_profile )
  {
    MYON_ERROR("MYON_Extrusion::SetOuterProfile() called when m_profile is already not null.");
    return false;
  }
...
bool MYON_Extrusion::AddInnerProfile( MYON_Curve* inner_profile )
{
  if ( m_profile_count < 1 )
  {
    MYON_ERROR("MYON_Extrusion::AddInnerProfile() called when m_profile_count < 1.");
    return false;
  }
  if ( 0 == m_profile )
  {
    MYON_ERROR("MYON_Extrusion::AddInnerProfile() called when m_profile is null.");
    return false;
  }

  if ( 1 == m_profile_count && !m_profile->IsClosed() )
  {
    MYON_ERROR("MYON_Extrusion::AddInnerProfile() called when outer profile is not closed.");
    return false;
  }

  MYON_PolyCurve* polycurve = MYON_PolyCurve::Cast(m_profile);
  if (  m_profile_count > 1 && 0 == polycurve )
  {
    MYON_ERROR("MYON_Extrusion::AddInnerProfile() called when  m_profile_count > 1 but m_profile is not an MYON_PolyCurve.");
    return false;
  }
  if ( m_profile_count > 1 && m_profile_count != polycurve->Count() )
  {
    MYON_ERROR("MYON_Extrusion::AddInnerProfile() called when  m_profile_count > 1 but m_profile_count != m_profile->Count().");
    return false;
  }
  ...
  if ( polycurve->SegmentDomain(m_profile_count) != inner_profile->Domain() )
  {
    ...
    polycurve = MYON_PolyCurve::Cast(inner_profile);
    if ( 0 != polycurve )
    {
      polycurve->SynchronizeSegmentDomains();
    }
...
const MYON_PolyCurve* MYON_Extrusion::PolyProfile() const
{
  if ( m_profile_count <= 1 )
    return 0;
  const MYON_PolyCurve* poly_profile = MYON_PolyCurve::Cast(m_profile);
  return (0 != poly_profile && m_profile_count == poly_profile->Count() ) ? poly_profile : 0;
}

const MYON_Curve* MYON_Extrusion::Profile(int profile_index) const
{
  if ( 0 == profile_index && 1 == m_profile_count )
    return m_profile;
  if ( profile_index < 0 || profile_index > m_profile_count )
    return 0;
  const MYON_PolyCurve* poly_profile = PolyProfile();
  return ( 0 != poly_profile ? poly_profile->SegmentCurve(profile_index) : 0 );
}

MYON_Curve* MYON_Extrusion::Profile3d( MYON_COMPONENT_INDEX ci ) const
{
  double s = MYON_UNSET_VALUE;
  if ( MYON_COMPONENT_INDEX::extrusion_bottom_profile == ci.m_type )
    s = 0.0;
  else if ( MYON_COMPONENT_INDEX::extrusion_top_profile == ci.m_type )
    s = 1.0;
  else
    return 0;
...
MYON_LineCurve* MYON_Extrusion::PathLineCurve(MYON_LineCurve* line_curve) const
{
  if ( !m_path.IsValid() )
    return 0;

  MYON_Interval path_domain = Domain(PathParameter());
  if ( !path_domain.IsIncreasing() )
    return 0;

  if ( 0 == line_curve )
    line_curve = new MYON_LineCurve();
...
MYON_Curve* MYON_Extrusion::WallEdge( MYON_COMPONENT_INDEX ci ) const
{
  if ( MYON_COMPONENT_INDEX::extrusion_wall_edge != ci.m_type )
    return 0;
  if ( ci.m_index < 0 )
    return 0;
  ...
  const MYON_Curve* profile2d = Profile(profile_index);
  if ( 0 == profile2d )
    return 0;

  MYON_3dPoint profileP = profile_end
  ...
  MYON_LineCurve* line_curve = new MYON_LineCurve();
...
MYON_Surface* MYON_Extrusion::WallSurface( MYON_COMPONENT_INDEX ci ) const
{
  if ( MYON_COMPONENT_INDEX::extrusion_wall_surface != ci.m_type )
    return 0;

  const MYON_Curve* profile2d = Profile(ci.m_index);
  if ( 0 == profile2d )
    return 0;

  MYON_Interval wall_profile2d_domain = profile2d->Domain(); // 28 July, 2015 - Lowell - RH-29948
  if ( m_profile_count > 1 )
  {
    const MYON_PolyCurve* polyprofile2d = PolyProfile();
    if ( 0 == polyprofile2d )
      return 0;
  ...
  MYON_Curve* wall_profile2d = profile2d->DuplicateCurve();
  if ( 0 == wall_profile2d )
    return 0;
  wall_profile2d->SetDomain(wall_profile2d_domain);
  wall_profile2d->ChangeDimension(2);

  MYON_Extrusion* wall = new MYON_Extrusion();
...
MYON_Curve* MYON_Extrusion::Profile3d( int profile_index, double s ) const
{
  ...
  MYON_Curve* profile3d = profile2d->DuplicateCurve();
  if ( 0 == profile3d )
    return 0;
...
int MYON_Extrusion::ProfileIndex( double profile_parameter ) const
{
  if ( !m_profile )
    return -1;
  ...
  const MYON_PolyCurve* polycurve = PolyProfile();
  if ( 0 == polycurve )
    return -1;

  const MYON_SimpleArray<double>& polycurve_t = polycurve->SegmentParameters();
  if ( polycurve_t.Count() != m_profile_count+1 )
    return -1;

  int profile_index = MYON_SearchMonotoneArray( polycurve_t.Array(), polycurve_t.Count(), profile_parameter );
...
int MYON_Extrusion::ProfileCount() const
{
  if ( !m_profile )
    return 0;
  ...
  if ( m_profile_count > 1 )
  {
    const MYON_PolyCurve* polycurve = MYON_PolyCurve::Cast(m_profile);
    if ( 0 == polycurve )
      return 0;
...
int MYON_Extrusion::GetProfileCurves( MYON_SimpleArray< const MYON_Curve* >& profile_curves ) const
{
  if ( !m_profile )
    return 0;
...
const double MYON_Extrusion::m_Nz_min = 1.0/64.0;

const double MYON_Extrusion::m_path_length_min = MYON_ZERO_TOLERANCE;

MYON_OBJECT_IMPLEMENT(MYON_Extrusion,MYON_Surface,"36F53175-72B8-4d47-BF1F-B4E6FC24F4B9");

MYON_Extrusion::MYON_Extrusion()
{
  MYON_ExtrusionInitializeHelper(*this);
}

MYON_Extrusion::MYON_Extrusion(const MYON_Extrusion& src) : MYON_Surface(src), m_profile(0)
{
  MYON_ExtrusionCopyHelper(src,*this);
}
...
MYON_Extrusion& MYON_Extrusion::operator=(const MYON_Extrusion& src)
{
  if ( this != &src )
  {
    Destroy();
...
static
void MYON_Extrusion_IsNotValidMessage( MYON_TextLog* text_log, const char* msg )
{
...
static bool MYON_ExtrusionIsNotValid()
{
  return MYON_IsNotValid(); // good place for a breakpoint
}

void MYON_Extrusion::DestroyRuntimeCache( bool bDelete )
{
  if ( 0 != m_profile )
  {
...
bool MYON_Extrusion::IsValid( MYON_TextLog* text_log ) const
{
  // check m_profile
  if ( m_profile_count < 1 )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_profile_count < 1.");
    return MYON_ExtrusionIsNotValid();
  }
  if ( !m_profile )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_profile is nullptr.");
    return MYON_ExtrusionIsNotValid();
  }
  if ( m_profile_count > 1 )
  {
    const MYON_PolyCurve* c = MYON_PolyCurve::Cast(m_profile);
    if ( 0 == c )
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_profile_count > 1 but m_profile is not an MYON_PolyCurve.");
      return MYON_ExtrusionIsNotValid();
    }
    if ( m_profile_count != c->Count() )
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_profile_count > 1 but m_profile_count != m_profile->SegmentCount().");
      return MYON_ExtrusionIsNotValid();
    }

    if ( !MYON_Extrusion::IsValidPolyCurveProfile(*c,text_log) )
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_profile is not a valid MYON_PolyCurve.");
      return MYON_ExtrusionIsNotValid();
    }
    for ( int i = 0; i < m_profile_count; i++ )
    {
      const MYON_Curve* segment = c->SegmentCurve(i);
      if ( 0 == segment )
      {
        MYON_Extrusion_IsNotValidMessage(text_log,"m_profile_count > 1 but a m_profile_count->SegmentCurve() is null.");
        return MYON_ExtrusionIsNotValid();
      }
      if ( !segment->IsClosed() )
      {
        MYON_Extrusion_IsNotValidMessage(text_log,"m_profile_count > 1 but a m_profile_count->SegmentCurve() is not closed.");
        return MYON_ExtrusionIsNotValid();
      }
    }
  }
  ...
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_profile is not valid.");
    return MYON_ExtrusionIsNotValid();
  }
  ...
  if ( !m_path.IsValid() )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_path is not valid.");
    return MYON_ExtrusionIsNotValid();
  }
  MYON_3dVector D = m_path.to - m_path.from;
  double len = D.Length();
  if ( !MYON_IsValid(len) || len <= 0.0 )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_path has zero length.");
    return MYON_ExtrusionIsNotValid();
  }
  if ( !MYON_IsValid(len) || len <= MYON_Extrusion::m_path_length_min )
  {
    if ( text_log )
      text_log->Print("m_path has zero length <= MYON_Extrusion::m_path_length_min.");
    return MYON_ExtrusionIsNotValid();
  }
  if ( !D.Unitize() || !D.IsUnitVector() )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_path has zero direction.");
    return MYON_ExtrusionIsNotValid();
  }
  ...
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_t does not satisfy 0<=m_t[0]<m_t[1]<=1");
    return MYON_ExtrusionIsNotValid();
  }
  ...
  {
   MYON_Extrusion_IsNotValidMessage(text_log,"m_up is not a unit vector.");
    return MYON_ExtrusionIsNotValid();
  }
  len = m_up*D;
  if ( fabs(len) > MYON_SQRT_EPSILON )
  {
    MYON_Extrusion_IsNotValidMessage(text_log,"m_up is not perpendicular to m_path.");
    return MYON_ExtrusionIsNotValid();
  }
  ...
  if ( m_bHaveN[0] )
  {
    ...
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_N[0] is not a unit vector.");
      return MYON_ExtrusionIsNotValid();
    }
    if ( !(m_N[0].z > MYON_Extrusion::m_Nz_min) )
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_N[0].z is too small (<=MYON_Extrusion::m_Nz_min) or negative");
      return MYON_ExtrusionIsNotValid();
    }
  }
  if ( m_bHaveN[1] )
  {
    ...
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_N[1] is not a unit vector.");
      return MYON_ExtrusionIsNotValid();
    }
    if ( !(m_N[1].z > MYON_Extrusion::m_Nz_min) )
    {
      MYON_Extrusion_IsNotValidMessage(text_log,"m_N[1].z is too small (<=MYON_Extrusion::m_Nz_min) or negative");
      return MYON_ExtrusionIsNotValid();
    }
  }
...
void MYON_Extrusion::Dump( MYON_TextLog& text_log ) const
{
  text_log.Print("MYON_Extrusion: \n");
  {
    text_log.PushIndent();
    text_log.Print("Path: ");
...
unsigned int MYON_Extrusion::SizeOf() const
{
  unsigned int sz = sizeof(*this) - sizeof(MYON_Surface);
  if ( m_profile )
    sz += m_profile->SizeOf();
  return sz;
}

MYON__UINT32 MYON_Extrusion::DataCRC(MYON__UINT32 current_remainder) const
{
  if ( m_profile )
    current_remainder = m_profile->DataCRC(current_remainder);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_path),&m_path);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_t),&m_t);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_up),&m_up);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bHaveN[0]), &m_bHaveN[0]);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bHaveN[1]), &m_bHaveN[1]);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_N[0]), &m_N[0]);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_N[1]), &m_N[1]);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_path_domain), &m_path_domain);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bTransposed), &m_bTransposed);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_profile_count), &m_profile_count);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bCap[0]), &m_bCap[0]);
  current_remainder = MYON_CRC32(current_remainder,sizeof(m_bCap[1]), &m_bCap[1]);
  if ( m_profile )
    current_remainder = m_profile->DataCRC(current_remainder);
...
class MYON_V5ExtrusionDisplayMeshCache : public MYON_UserData
{
  MYON_OBJECT_DECLARE(MYON_V5ExtrusionDisplayMeshCache);
public:
  MYON_V5ExtrusionDisplayMeshCache();
  ~MYON_V5ExtrusionDisplayMeshCache();
  MYON_V5ExtrusionDisplayMeshCache(const MYON_V5ExtrusionDisplayMeshCache&);
  MYON_V5ExtrusionDisplayMeshCache& operator=(const MYON_V5ExtrusionDisplayMeshCache&);

  static MYON_V5ExtrusionDisplayMeshCache* CreateMeshCache(
    const MYON_Extrusion*
    );

  // virtual MYON_Object::Write function
  bool Write(MYON_BinaryArchive& binary_archive) const override;

  // virtual MYON_Object::Read function
  bool Read(MYON_BinaryArchive& binary_archive) override;

  // virtual MYON_UserData::GetDescription
  bool GetDescription( MYON_wString& description ) override;

  // virtual MYON_UserData::Archive
  bool Archive() const override; 

  // virtual MYON_UserData::DeleteAfterWrite
  bool DeleteAfterWrite(
    const class MYON_BinaryArchive& archive,
    const class MYON_Object* parent_object
    ) const override;

  // virtual MYON_UserData::DeleteAfterRead
...
private:
  void DestroyHelper();
  void CopyHelper(const MYON_V5ExtrusionDisplayMeshCache&);

  // Only extrusion objects use the obsolete MYON_V5ExtrusionDisplayMeshCache.
  //const MYON::object_type m_object_type = MYON::object_type::extrusion_object;
  std::shared_ptr<MYON_Mesh> m_render_mesh;
  std::shared_ptr<MYON_Mesh> m_analysis_mesh;
};


bool MYON_Extrusion::Write(
     ...
     ) const
{
  ...
  for (;;)
  {
    ...
    if (2 == minor_version && binary_archive.Save3dmRenderMesh(ObjectType()))
    {
      ...
      MYON_V5ExtrusionDisplayMeshCache::CreateMeshCache(this);
    }
    else if (minor_version >= 3)
    {
      if (binary_archive.Save3dmRenderMesh(ObjectType()))
        ...
        rc = MYON_MeshCache::Empty.Write(binary_archive);
      if (!rc) break;
    }
     ...
     )
{
  ...
  for (;;)
  {
    ...
    if ( minor_version < 2 )
    {
      const MYON_Curve* outer_profile = Profile(0);
...
MYON::object_type MYON_Extrusion::ObjectType() const
{
  return MYON::extrusion_object;
}


int MYON_Extrusion::Dimension() const
{
  return 3;
}

static 
bool GetBoundingBoxHelper(const MYON_Extrusion& extrusion, 
...
bool MYON_Extrusion::GetBBox(double* boxmin,double* boxmax,bool bGrowBox) const
{
  bool rc = false;
    ...
    if ( m_profile->GetTightBoundingBox(bbox) && GetBoundingBoxHelper(*this,bbox,0) )
    {
      ...
      if ( bGrowBox )
      {
        bGrowBox =  ( boxmin[0] <= boxmax[0] 
                      && boxmin[1] <= boxmax[1] 
                      && boxmin[2] <= boxmax[2]
                      && MYON_IsValid(boxmax[0]) 
                      && MYON_IsValid(boxmax[1]) 
                      && MYON_IsValid(boxmax[2]));
      }
      if ( bGrowBox )
      {
...
bool MYON_Extrusion::GetTightBoundingBox(MYON_BoundingBox& tight_bbox, bool bGrowBox, const MYON_Xform* xform ) const
{
  bool rc = false;
  if ( m_path.IsValid() && m_profile )
  {
...
static bool MYON_Extrusion_TransformFailed()
{
  return false; // good place for a breakpoint
}

static bool Profile2dTransform( MYON_Extrusion& e, const MYON_Xform& profile_xform, bool bNeedReverse )
{
  ...
  MYON_PolyCurve* polyprofile = const_cast<MYON_PolyCurve*>(e.PolyProfile());
  if ( 0 != polyprofile )
  {
    rc = true;
    if ( bNeedDeformable )
    ...
    {
      MYON_Curve* profile_segment = polyprofile->SegmentCurve(i);
      ...
      {
        rc = MYON_Extrusion_TransformFailed();
        continue;
      }
      if ( bNeedReverse )
      {
  ...
  else
  {
    ...
    {
      MYON_NurbsCurve* c = e.m_profile->NurbsCurve();
      if ( 0 != c )
      {
    ...
    {
      rc = e.m_profile->Transform(profile_xform)?true:MYON_Extrusion_TransformFailed();
    }
    if ( rc && bNeedReverse )
    {
      double t0, t1;
...
bool MYON_Extrusion::Transform( const MYON_Xform& xform )
{
  if ( !m_path.IsValid() || !m_up.IsUnitVector() || m_profile_count < 1 )
    return MYON_Extrusion_TransformFailed();

  MYON_3dVector T = m_path.Tangent();
  MYON_3dVector UxT = MYON_CrossProduct(m_up,T);
  if ( !UxT.IsUnitVector() && !UxT.Unitize() )
    return MYON_Extrusion_TransformFailed();

  const bool bUseVectorXform = (0.0 == xform[3][0] && 0.0 == xform[3][1] && 0.0 == xform[3][2]);

  MYON_3dPoint E[2], QE[2];
  E[0] = m_path.from;
  E[1] = m_path.to;
  QE[0] = xform*E[0];
  QE[1] = xform*E[1];
  ...
  if ( !QE[1].IsValid() )
    return MYON_Extrusion_TransformFailed();
  MYON_3dVector QT0 = bUseVectorXform ? (xform*(E[1] - E[0])) : (QE[1]-QE[0]);
  if ( !QT0.Unitize() )
    return MYON_Extrusion_TransformFailed();
  
  const int base_index = ( QE[1].DistanceTo(E[1]) < QE[0].DistanceTo(E[0]) ) ? 1 : 0;
  const MYON_3dPoint B(E[base_index]);
  const MYON_3dPoint QB(QE[base_index]);

  const double QT0oT = QT0*T;
  bool bSamePathDir = ( fabs(fabs(QT0oT) - 1.0) <= MYON_SQRT_EPSILON );
  MYON_3dVector QT = bSamePathDir ? ((QT0oT < 0.0) ? -T : T) : QT0;
  const double Qlen = QE[0].DistanceTo(QE[1]);
  if ( bSamePathDir )
  {
    MYON_3dPoint R =  QB + (base_index ? -Qlen : Qlen)*QT;
    if ( QE[1-base_index].DistanceTo( R ) <= MYON_SQRT_EPSILON*Qlen )
    {
      QE[1-base_index] = R;
    }
    else
  ...
  const MYON_3dPoint X0 = xform*(B + UxT);
  const MYON_3dPoint Y0 = xform*(B + m_up);
  const MYON_3dVector QDY = bUseVectorXform ? (xform*m_up) : (Y0-QB);
  MYON_3dVector QY = QDY;
  if ( !QY.Unitize() )
    return MYON_Extrusion_TransformFailed();
  MYON_3dVector QU0 = QDY - (QDY*QT)*QT;
  if ( !QU0.Unitize() )
    return MYON_Extrusion_TransformFailed();

  const double QU0oU = QU0*m_up;
  bool bSameUpDir = ( fabs(fabs(QU0oU) - 1.0) <= MYON_SQRT_EPSILON );
  MYON_3dVector QU = bSameUpDir ? ((QU0oU < 0.0) ? -m_up : m_up) : QU0;
  ...
  MYON_3dVector QUxQT = MYON_CrossProduct(QU,QT);
  if ( !QUxQT.Unitize() )
    return MYON_Extrusion_TransformFailed();

  const double QUxQToUxT = QUxQT*UxT;
  if ( (bSamePathDir && bSameUpDir) || fabs(fabs(QUxQToUxT) - 1.0) <= MYON_SQRT_EPSILON )
  {
    if ( QUxQToUxT < 0.0 )
      QUxQT = -UxT;
    else
  ...
  if ( fabs(QUoQY - 1.0) < MYON_SQRT_EPSILON )
    QY = QU;
  else if ( fabs(QUoQY + 1.0) < MYON_SQRT_EPSILON )
    QY = -QU;

  // profile_xform will be the transformation 
  // applied to the 2d profile curve.
  const MYON_3dVector QDX = bUseVectorXform ? (xform*UxT) : (X0 - QB);
  MYON_3dVector QX = QDX - (QDX*QY)*QY;
  if ( !QX.Unitize() )
    return MYON_Extrusion_TransformFailed();

  const double QUxQToQX = QUxQT*QX;
  if ( fabs(QUxQToQX - 1.0) < MYON_SQRT_EPSILON )
    QX = QUxQT;
  else if ( fabs(QUxQToQX + 1.0) < MYON_SQRT_EPSILON )
    QX = -QUxQT;


  MYON_3dVector QXxQY = MYON_CrossProduct(QX,QY);
  ...
  MYON_3dVector path_shear(0.0,QU*QXxQY,QT*QXxQY);
  bool bHavePathShear =    path_shear.z > MYON_Extrusion::m_Nz_min 
                        && path_shear.z < 1.0 - MYON_SQRT_EPSILON
                        && path_shear.y < 1.0 - MYON_SQRT_EPSILON;
  if ( bHavePathShear )
  {
    double x2 = path_shear.y*path_shear.y + path_shear.z*path_shear.z;
    if ( x2 > MYON_SQRT_EPSILON && x2 <= 1.0 )
    {
    ...
    if ( fabs(path_shear.y) <= MYON_SQRT_EPSILON )
      path_shear.y = 0.0;
  ...
  // miter normals
  MYON_3dVector QN[2] = {MYON_3dVector::ZeroVector,MYON_3dVector::ZeroVector};
  ...
  for ( int i = 0; i < 2; i++ )
  {
    if ( m_bHaveN[i] )
    {
      ...
      if ( bHavePathShear )
      {
        ...
        MYON_3dVector V1(m_N[i].y,-m_N[i].x,0.0);
        V1.Unitize();
        MYON_3dVector V2 = MYON_CrossProduct( m_N[i],V1);
        ...
        MYON_3dVector QV1 = xform*V1;
        MYON_3dVector QV2 = xform*V2;
        ...
        QN3d = MYON_CrossProduct(QV1,QV2);
      }
      else
      {
        const MYON_3dVector N3d = m_N[i].x*UxT + m_N[i].y*m_up + m_N[i].z*T;
        MYON_3dPoint QTip = xform*(E[i] + N3d);
      ...
      if ( fabs(QN[i].x) <= MYON_SQRT_EPSILON )
        QN[i].x = 0.0;
      if ( fabs(QN[i].y) <= MYON_SQRT_EPSILON )
        QN[i].y = 0.0;
      if (    QN[i].z < MYON_Extrusion::m_Nz_min
           || QN[i].z >= 1.0 - MYON_SQRT_EPSILON
           || (fabs(QN[i].x) <= MYON_SQRT_EPSILON && fabs(QN[i].y) <= MYON_SQRT_EPSILON )
         )
      {
        bHaveQN[i] = false;
      }
      ...
      if ( !bHaveQN[i] )
        QN[i] = MYON_3dVector::ZeroVector;
      else if ( fabs(QN[i]*m_N[i] - 1.0) <= 1.0e-6 ) // MYON_SQRT_EPSILON 
        QN[i] = m_N[i];
    }
  }

  // get 2d profile transformation
  MYON_Xform profile_xform(MYON_Xform::IdentityTransformation);
  ...
  if ( !bHavePathShear )
  {
    ...
    if ( !MYON_IsValid(profile_y_scale) || 0.0 == profile_y_scale )
    {
      // cannot flatten profile
      return MYON_Extrusion_TransformFailed(); 
    } 

    if ( !MYON_IsValid(profile_x_scale) || 0.0 == profile_x_scale )
    {
      // cannot flatten profile
      return MYON_Extrusion_TransformFailed(); 
    } 
    ...
    const double profile_y_scale_tol = MYON_SQRT_EPSILON;
    ...
    if ( !MYON_IsValid(profile_det) || 0.0 == profile_det )
      return MYON_Extrusion_TransformFailed();
  ...
  else
  {
    ...
    MYON_Xform xform1, xform3, xform4, xform5;

    // xform 1 maps original 2d profile to world 3d
    xform1.Rotation(MYON_Plane::World_xy,plane0);
    ...
    xform4.Rotation(plane1, MYON_Plane::World_xy);
    ...
    const double xform_tol = 10.0*MYON_SQRT_EPSILON;
    if ( profile_y_scale > xform_tol && fabs(profile_y_scale - 1.0) > xform_tol )
    {
      double profile_scale_tol = profile_y_scale*MYON_SQRT_EPSILON;
...
class CMyBrepIsSolidSetter : public MYON_Brep
{
public:
  void SetIsSolid(int is_solid) {m_is_solid = is_solid;}
  void SetBBox( const MYON_Extrusion& extrusion)
  {
    MYON_BoundingBox brep_bbox = BoundingBox();
    MYON_BoundingBox extr_bbox = extrusion.BoundingBox();
    MYON_BoundingBox bbox;
...
class MYON_Extrusion_BrepForm_FaceInfo
{
public:
  MYON_Extrusion_BrepForm_FaceInfo();
  ...
  */
  ...
  //   kinky faces, then the MYON_Extrusion_BrepForm_FaceInfo
  ...
  MYON_Curve* m_extrusion_profile;
  MYON_Extrusion* m_extrusion_srf;
  int m_face_index;
  int m_vid[4];
  int m_eid[4];
  bool m_bRev3d[4];
  ...
  MYON_NurbsCurve* m_cap_c2[2]; // 2d cap trim curves
};

MYON_Extrusion_BrepForm_FaceInfo::MYON_Extrusion_BrepForm_FaceInfo()
{
  Init();
}
...
bool MYON_Extrusion_BrepForm_FaceInfo::HaveBrepFaceFace() const
{
  if ( m_face_index < 0 )
    return false;
  return true;
...
void MYON_Extrusion_BrepForm_FaceInfo::Init()
{
  memset(this,0,sizeof(*this));
...
static 
MYON_PlaneSurface* MakeCapPlaneHelper(
    MYON_ClassArray< MYON_Extrusion_BrepForm_FaceInfo >& finfo,
    int cap_index, // 0 = bottom, 1 = top
    const MYON_Xform& rot 
    )
{
  double d;
  MYON_BoundingBox bbox;
  ...
  for ( int i = 0; i < finfo.Count(); i++ )
  {
    ...
    const MYON_NurbsCurve* c2 = finfo[i].m_cap_c2[cap_index];
    if ( 0 == c2 )
      return 0;
    MYON_BoundingBox c2bbox;
    ...
    if ( outer_loop_iso_trim_count ==  outer_loop_trim_count )
    {
      ...
      MYON_3dVector D = c2bbox.Diagonal();
      double zero_tol = MYON_ZERO_TOLERANCE;
      double nonzero_tol = 1000.0*MYON_ZERO_TOLERANCE;
  ...
  MYON_Interval u(bbox.m_min.x,bbox.m_max.x);
  MYON_Interval v(bbox.m_min.y,bbox.m_max.y);
  ...
  MYON_PlaneSurface* plane = new MYON_PlaneSurface(MYON_xy_plane);
          ...
          )
{
  ...
  MYON_BrepLoop& loop = brep->NewLoop(loop_type,*capface);
  ...
  for ( fi1 = fi0; fi1 < finfo.Count(); fi1++ )
  {
    ...
    MYON_NurbsCurve* c2 = finfo[fi1].m_cap_c2[cap_index];
    if ( 0 == c2 )
    {
      bCloseGaps = false;
      break;
  ...
  //    Fix bug 105311
  //    Sometimes there are gaps in MYON_PolyCurve profiles that
  //    need to be closed to make a valid MYON_Brep.  The gaps
  //    need to be closed after the call to SetTrimIsoFlags().
  if ( bCloseGaps ) for ( int lti = 0; lti < loop.m_ti.Count(); lti++ )
  {
    MYON_BrepTrim& trim0 = brep->m_T[loop.m_ti[lti]];
    MYON_BrepTrim& trim1 = brep->m_T[loop.m_ti[(lti+1)%loop.m_ti.Count()]];
...
bool MakeCap2dCurvesHelper(
        MYON_ClassArray< MYON_Extrusion_BrepForm_FaceInfo >& finfo,
        int is_capped,
        const MYON_Xform& scale0,
        const MYON_Xform& scale1
        )
{
  switch(is_capped)
  {
  ...
  for ( int i = 0; i < finfo.Count(); i++ )
  {
    MYON_NurbsCurve* c20 = 0;
    MYON_NurbsCurve* c21 = 0;
...
static bool GetNextProfileSegmentDiscontinuity(
                const MYON_Curve* profile_segment,
                double t0,
                double t1,
                double *t
                )
{
  // Do NOT change MYON_DEFAULT_ANGLE_TOLERANCE_COSINE to another value.
  ...
  return profile_segment->GetNextDiscontinuity(
                            ...
                            t0,t1,t,
                            0,0,
                            MYON_DEFAULT_ANGLE_TOLERANCE_COSINE,
                            MYON_SQRT_EPSILON
                            );
}


bool MYON_Extrusion::ProfileIsKinked( int profile_index ) const
{
  const MYON_Curve* profile = Profile(profile_index);
  if ( 0 == profile )
    return false;
  double t0 = MYON_UNSET_VALUE;
  double t1 = MYON_UNSET_VALUE;
  double t;
  if ( !profile->GetDomain(&t0,&t1) )
    return 0;
  if ( !MYON_IsValid(t0) || !(t0 < t1) )
    return 0;
  t  = t0;
  return (GetNextProfileSegmentDiscontinuity( profile, t0,t1, &t) && t0 < t && t < t1);
}

int MYON_Extrusion::ProfileSmoothSegmentCount( int profile_index ) const
{
...
int MYON_Extrusion::GetProfileKinkParameters( int profile_index, MYON_SimpleArray<double>& profile_kink_parameters ) const
{
  return GetProfileKinkParameters(profile_index, &profile_kink_parameters);
}

int MYON_Extrusion::GetProfileKinkParameters( int profile_index, MYON_SimpleArray<double>* profile_kink_parameters_ptr ) const
{
  const MYON_Curve* profile2d = Profile(profile_index);
  if ( 0 == profile2d )
    return 0;
...
MYON_Brep* MYON_Extrusion::BrepForm(MYON_Brep* brep) const
{
  return BrepForm(brep, true);
}

static int GetSmoothSideCount(const MYON_Extrusion& E)

{
  MYON_SimpleArray<const MYON_Curve*> profile_curves;
  ...
  for ( int profile_index = 0; profile_index < profile_count; profile_index++ ){
    const MYON_Curve* profile_segment = profile_curves[profile_index];
    double t0 = MYON_UNSET_VALUE;
    double t1 = MYON_UNSET_VALUE;
    if ( !profile_segment->GetDomain(&t0,&t1) 
          || !MYON_IsValid(t0)
...
MYON_Brep* MYON_Extrusion::BrepForm( MYON_Brep* brep, bool bSmoothFaces ) const
{
  if ( brep )
    brep->Destroy();
  ...
  if ( profile_count < 1 || profile_count != profile_curves.Count() )
  {
    MYON_ERROR("extrusion has no profile curve.");
    return 0;
  }
  ...
  if ( !T.IsUnitVector() )
  {
    MYON_ERROR("extrusion direction is zero.");
    return 0;
  }

  MYON_Xform 
    xform0(MYON_Xform::IdentityTransformation), 
    xform1(MYON_Xform::IdentityTransformation), 
    scale0(MYON_Xform::IdentityTransformation), 
    scale1(MYON_Xform::IdentityTransformation), 
    rot0(MYON_Xform::IdentityTransformation), 
    rot1(MYON_Xform::IdentityTransformation);
  if ( !MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[0]),T,m_up,m_bHaveN[0]?&m_N[0]:0,xform0,&scale0,&rot0) )
  {
    MYON_ERROR("Unable to get bottom cap location.");
    return 0;
  }

  if ( !MYON_GetEndCapTransformation(m_path.PointAt(m_t.m_t[1]),T,m_up,m_bHaveN[1]?&m_N[1]:0,xform1,&scale1,&rot1) )
  {
    MYON_ERROR("Unable to get top cap location.");
    return 0;
  }
  ...
  MYON_ClassArray< MYON_Extrusion_BrepForm_FaceInfo > finfo(2*profile_count);
  for ( int profile_index = 0; profile_index < profile_count; profile_index++ )
  {
    const MYON_Curve* profile_segment = profile_curves[profile_index];
    if ( 0 == profile_segment )
    {
      MYON_ERROR("null profile segment.");
      if (newbrep != brep )
        delete newbrep;
      return 0;
    }
    ...
    fi0 = finfo.Count();
    MYON_Extrusion_BrepForm_FaceInfo profile_fi;
    profile_fi.Init();
    {
      MYON_Curve* newprofile = profile_segment->DuplicateCurve();
      if ( 0 == newprofile )
      {
        MYON_ERROR("unable to duplicate profile segment.");
        if (newbrep != brep )
          delete newbrep;
        return 0;
      }
      ...
      profile_fi.m_profile_orientation = ( profile_fi.m_bClosedProfile )
                              ? MYON_ClosedCurveOrientation(*newprofile,0)
                              : 0;
      ...
      if ( bSmoothFaces )
      {
        ...
        {
          MYON_ERROR("Corrupt profile domain.");
          delete newprofile;
          if (newbrep != brep )
            delete newbrep;
          return 0;
        ...
        while ( GetNextProfileSegmentDiscontinuity(profile_segment,t0,t1,&t) )
        {
          if ( t0 < t && t < t1 )
          {
            MYON_Curve* left_side = 0;
            MYON_Curve* right_side = 0;
    ...
    if ( fi1 <= fi0 )
    {
      MYON_ERROR("Corrupt extrusion cannot produce brep form.");
      if (newbrep != brep )
        delete newbrep;
      return 0;
    }
    ...
    for ( int finfo_index = fi0; finfo_index < fi1; finfo_index++ )
    {
      ...
      MYON_Extrusion_BrepForm_FaceInfo& fi = finfo[finfo_index];
      fi.m_face_index = -1;

      fi.m_extrusion_srf = new MYON_Extrusion();
      ...
      MYON_NurbsSurface* face_srf = fi.m_extrusion_srf->NurbsSurface();
      if ( 0 == face_srf )
      {
        continue;
        ////if (newbrep != brep )
      ...
      const MYON_BrepFace* face = newbrep->NewFace(face_srf,fi.m_vid,fi.m_eid,fi.m_bRev3d);
      if ( 0 == face )
      {
      ...
      const MYON_BrepLoop* loop = (1 == face->LoopCount()) ? face->OuterLoop() : 0;
      ...
      const MYON_BrepTrim* bottom_trim = loop->Trim(eidmap[0]);
      if ( 0 == bottom_trim || MYON_BrepTrim::boundary != bottom_trim->m_type )
      {
        is_capped = 0;
        cap_count = 0;
        continue;
      ...
      const MYON_BrepTrim* top_trim = loop->Trim(eidmap[2]);
      if ( 0 == top_trim || MYON_BrepTrim::boundary != top_trim->m_type )
      {
        is_capped = 0;
        cap_count = 0;
        continue;
      ...
      const MYON_BrepEdge* edge0 = bottom_trim->Edge();
      const MYON_BrepEdge* edge1 = top_trim->Edge();
      if ( 0 == edge0 || 0 == edge1 )
      {
        is_capped = 0;
        cap_count = 0;
  ...
  while ( is_capped > 0 && cap_count == ((3 == is_capped) ? 2 : 1) )
  {
    ...
    MYON_BrepFace* capface0 = 0;
    MYON_BrepFace* capface1 = 0;
    {
      MYON_PlaneSurface* plane = 0;
      int si;
    ...
    for ( cap_index = 0; cap_index < 2; cap_index++ )
    {
      ...
      // it easier for the picking code to create MYON_Brep
      // component indices when picking the extrusion
      // and the brep index is not present.
      MYON_BrepFace* capface = ( 0 == cap_index ) ? capface0 : capface1;
      if ( 0 == capface )
        continue;
      ...
      for ( profile_index = 0; profile_index < profile_count && fi0 < finfo.Count(); profile_index++ )
      {
        if ( finfo[fi0].m_profile_index != profile_index )
          break;
        MYON_BrepLoop::TYPE loop_type = ( 0 == profile_index ) ? MYON_BrepLoop::outer : MYON_BrepLoop::inner;
      ...
      {
        MYON_ERROR("Failed to add caps to extrusion brep form.");
        if ( 0 != capface0 )
  ...
  if ( 0 != newbrep )
  {
    ...
    {
      MYON_ERROR("corrupt extrusion cannot produce a brep form.");
      if (newbrep != brep )
        delete newbrep;
      return 0;
    }
...
bool MYON_Extrusion::GetBrepFormComponentIndex(
  MYON_COMPONENT_INDEX extrusion_ci,
  MYON_COMPONENT_INDEX& brep_ci
  ) const
{
  return GetBrepFormComponentIndex(extrusion_ci,MYON_UNSET_VALUE,nullptr,brep_ci);
}

static bool GetBrepFormFaceIndex(
        const MYON_Extrusion& extrusion,
        int extrusion_profile_index,
        double extrusion_profile_parameter,
        bool bCountProfileDiscontinuities,
        int* brep_form_face_index,
        MYON_Interval* profile_segment_domain
        )
{
  ...
  double t0 = MYON_UNSET_VALUE;
  double t1 = MYON_UNSET_VALUE;
  const MYON_Curve* profile;
  if ( MYON_UNSET_VALUE == extrusion_profile_parameter )
  {
...
bool MYON_Extrusion::GetBrepFormComponentIndex(
  ...
  bool bSmoothFaces,
  MYON_COMPONENT_INDEX& brep_ci,
  MYON_Interval* profile_subdomain
) const
{
  brep_ci.UnSet();
  int face_index = -1;
  MYON_Interval face_profile_domain(MYON_UNSET_VALUE,MYON_UNSET_VALUE);
  ...
  {
    MYON_ERROR("brep_form parameter cannot be extrusion's BrepForm()");
    return false;
  }
  ...
  case MYON_COMPONENT_INDEX::extrusion_top_profile:
    ...
    if (MYON_COMPONENT_INDEX::extrusion_top_profile == extrusion_ci.m_type)
    {
    ...
    brep_ci.m_type = MYON_COMPONENT_INDEX::brep_edge;
  ...
  case MYON_COMPONENT_INDEX::extrusion_wall_surface:
    ...
    brep_ci.m_type = MYON_COMPONENT_INDEX::brep_face;
    if (nullptr != profile_subdomain)
      *profile_subdomain = face_profile_domain;
    break;
  ...
  default:
    // Other MYON_COMPONENT_INDEX::TYPE values intentionally ignored
    break;
  }
...
{
  bool rc = false;
  if ( MYON_IsValid(t0) && MYON_IsValid(t1) && t0 < t1 )
  {
...
MYON_Interval MYON_Extrusion::Domain(
  ...
  ) const 
{
  ...
  return (path_dir == dir ) 
         ? m_path_domain 
         : ((1-path_dir == dir && m_profile) ? m_profile->Domain() : MYON_Interval());
}

bool MYON_Extrusion::GetSurfaceSize( 
    double* width, 
    double* height 
    ) const
{
  ...
  if (height)
  {
    if ( m_profile )
    {
      ...
      MYON_NurbsCurve nurbs_profile_curve;
...
bool MYON_Extrusion::GetSpanVector( // span "knots" 
      int dir, // 0 gets first parameter's domain, 1 gets second parameter's domain
...
bool MYON_Extrusion::GetSpanVectorIndex(
      int dir , // 0 gets first parameter's domain, 1 gets second parameter's domain
      ...
      MYON_Interval* span_interval // [OUT] domain of the span containing "t"
      ) const
{
  const int path_dir = PathParameter();
  if ( path_dir == dir )
...
int MYON_Extrusion::Degree( // returns maximum algebraic degree of any span 
...
bool MYON_Extrusion::GetParameterTolerance( // returns tminus < tplus: parameters tminus <= s <= tplus
       ...
       ) const
{
  const int path_dir = PathParameter();
  if ( path_dir == dir )
    return MYON_Surface::GetParameterTolerance(dir,t,tminus,tplus);
  if ( 1-path_dir==dir && m_profile)
    return m_profile->GetParameterTolerance(t,tminus,tplus);
  return false;
}

MYON_Surface::ISO MYON_Extrusion::IsIsoparametric(
      const MYON_Curve& curve,
      const MYON_Interval* curve_domain
      ) const
{
  return MYON_Surface::IsIsoparametric(curve,curve_domain);
}

bool MYON_Extrusion::IsPlanar(
      MYON_Plane* plane,
      double tolerance
      ) const
{
  if ( m_profile && m_profile->IsLinear(tolerance) )
  {
    if ( plane )
    {
      MYON_3dPoint P0 = m_profile->PointAtStart();
      MYON_3dPoint P1 = m_profile->PointAtEnd();
      MYON_3dVector pathT = m_path.Tangent();
      MYON_3dVector Y = m_up;
      MYON_3dVector X = MYON_CrossProduct(Y,pathT);
      if ( !X.IsUnitVector() )
        X.Unitize();
      MYON_3dPoint Q0 = m_path.from + P0.x*X + P0.y*Y;
      MYON_3dPoint Q1 = m_path.from + P1.x*X + P1.y*Y;
      MYON_3dVector N = MYON_CrossProduct(pathT,Q1-Q0);
      N.Unitize();
      plane->origin = Q0;
...
bool MYON_Extrusion::IsClosed(int dir) const
{
  const int path_dir = PathParameter();
  if ( 1-path_dir == dir && m_profile )
...
bool MYON_Extrusion::IsPeriodic( int dir ) const
{
  const int path_dir = PathParameter();
  if ( 1-path_dir == dir && m_profile )
...
bool MYON_Extrusion::GetNextDiscontinuity( 
                int dir,
                MYON::continuity c,
                double t0,
                double t1,
...
bool MYON_Extrusion::IsContinuous(
  MYON::continuity c,
  double s, 
  double t, 
  int* hint,
...
{
  return MYON_Surface::IsIsoparametric(bbox);
}

bool MYON_Extrusion::Reverse( int dir )
{
  if ( 0 == m_profile )
    return false;
...
bool MYON_Extrusion::Transpose() // transpose surface parameterization (swap "s" and "t")
{
  m_bTransposed = m_bTransposed?false:true;
  return true;
}

bool MYON_Extrusion::Evaluate( // returns false if unable to evaluate
...
MYON_Curve* MYON_Extrusion::IsoCurve(
       int dir,
       double c
       ) const
{
  ...
  if ( 1 == dir )
  {
    MYON_3dPoint P = m_profile->PointAt(c);
    MYON_LineCurve* line_curve = new MYON_LineCurve();
  ...
  else if ( 0 == dir )
  {
    ...
    if ( isocurve )
    {
      ...
      if ( !isocurve->Transform(xform1) )
      {
        ...
        MYON_NurbsCurve* nurbs_curve = isocurve->NurbsCurve();
        delete isocurve;
  ...
  )
{
  ...
  if ( 1 == dir )
  {
    ...
    if ( rc )
    {
      ...
      double t1 = (1.0-s1)*m_t[0] + s1*m_t[1];
      bool bChanged = false;
      MYON_3dPoint P0 = m_path.from;
      MYON_3dPoint P1 = m_path.to;
      if ( t0 < m_t[0] )
      {
        bChanged = true;
       ...
       ) const
{
  ...
  MYON_Extrusion* left = 0;
  MYON_Extrusion* right = 0;
  if ( west_or_south_side )
  {
    left = MYON_Extrusion::Cast(west_or_south_side);
    if ( !left )
      return false;
  }
  if ( east_or_north_side )
  {
    right = MYON_Extrusion::Cast(east_or_north_side);
    if ( !right )
      return false;
  }
  ...
  {
    // split path
    MYON_Line left_path, right_path;
    MYON_Interval left_domain, right_domain;
    MYON_Interval left_t, right_t;

    const double t0 = m_t[0];
    const double t1 = m_t[1];
    const double t = (1.0-s)*t0 + s*t1;
    if ( !MYON_IsValid(t) || t <= t0 || t >= t1 )
      return false;

    MYON_3dPoint P = m_path.PointAt(s);
    ...
    // return result
    if ( !left )
      left = new MYON_Extrusion(*this);
    else if ( left != this )
      left->operator =(*this);
    else
      left->DestroyRuntimeCache();
    if ( !right )
      right = new MYON_Extrusion(*this);
    else if ( right != this )
      right->operator =(*this);
    else
  ...
  else
  {
    if ( 0 == m_profile )
      return false;
    MYON_Curve* left_profile = 0;
    MYON_Curve* right_profile = 0;
    ...
    const_cast<MYON_Extrusion*>(this)->m_profile = 0;
    ...
    const_cast<MYON_Extrusion*>(this)->m_profile = this_profile;
...
//bool MYON_Extrusion::GetLocalClosestPoint( const MYON_3dPoint&, // test_point
//        double,double,     // seed_parameters
//        double*,double*,   // parameters of local closest point returned here
//        const MYON_Interval* = nullptr, // first parameter sub_domain
//        const MYON_Interval* = nullptr  // second parameter sub_domain
//        ) const
//MYON_Surface* MYON_Extrusion::Offset(
//      double offset_distance, 
//      double tolerance, 
//      double* max_deviation = nullptr
//      ) const
int MYON_Extrusion::GetNurbForm(
      ...
      ) const
{
  ...
  {
    nurbs_surface.SetCV(i,0,MYON::intrinsic_point_style,nc0.CV(i));
    nurbs_surface.SetCV(i,1,MYON::intrinsic_point_style,nc1.CV(i));
  }
...
int MYON_Extrusion::HasNurbForm() const
{
  return m_profile ? m_profile->HasNurbForm() : 0;
}

bool MYON_Extrusion::GetSurfaceParameterFromNurbFormParameter(
...
bool MYON_Extrusion::GetNurbFormParameterFromSurfaceParameter(
      double surface_s, double surface_t,
      double* nurbs_s,  double* nurbs_t
      ) const
{
...
MYON_SumSurface* MYON_Extrusion::SumSurfaceForm( 
  MYON_SumSurface* sum_surface 
  ) const
{
  int i;
  if ( 0 != sum_surface )
  {
    for ( i = 0; i < 2; i++ )
    {
      ...
      sum_surface->m_basepoint = MYON_3dVector::ZeroVector;
  ...
  MYON_Curve* profile3d = 0;
  MYON_LineCurve* path = 0;
  MYON_Curve* curve0 = 0;
  MYON_Curve* curve1 = 0;
  for(;;)
  {
    if ( 1 == ProfileCount() )
    {
      ...
      if ( 0 != polycurve && 1 == polycurve->Count() )
      {
        const MYON_Curve* segment = polycurve->SegmentCurve(0);
        if ( 0 != segment )
        {
    ...
    if ( profile3d->IsLinear() && 0 == MYON_LineCurve::Cast(profile3d) )
    {
      MYON_LineCurve* line_curve = new MYON_LineCurve();
      line_curve->m_line.from = profile3d->PointAtStart();
      line_curve->m_line.to = profile3d->PointAtEnd();
      line_curve->MYON_Curve::SetDomain(profile3d->Domain());
      delete profile3d;
    ...
    path = new MYON_LineCurve();
    if ( 0 == path )
      break;
    path->m_line.from = MYON_3dPoint::Origin;
  ...
  MYON_SumSurface* sumsrf = ( 0 != sum_surface ) ? sum_surface : new MYON_SumSurface();
  if ( 0 == sumsrf )
  {
    delete curve0;
    delete curve1;
  ...
  sumsrf->m_basepoint = MYON_3dVector::ZeroVector;
...
MYON_Extrusion* MYON_Extrusion::Cylinder( 
  const MYON_Cylinder& cylinder, 
  bool bCapBottom,
  bool bCapTop,
  MYON_Extrusion* extrusion
  )
{
  ...
  if ( !path.IsValid() || !(path.Length() > MYON_ZERO_TOLERANCE) )
    return 0;

  MYON_3dVector up = cylinder.circle.plane.yaxis;
  if (    !up.IsValid()
       || !up.IsUnitVector()
       || fabs(up*path.Tangent()) > MYON_SQRT_EPSILON 
       )
    return 0;

  MYON_ArcCurve* circle_curve = new MYON_ArcCurve(cylinder.circle);
  circle_curve->m_arc.plane = MYON_Plane::World_xy;
  ...
  MYON_Extrusion* extrusion_cylinder = 0;
  if ( extrusion )
  {
    extrusion->Destroy();
  ...
  {
    extrusion_cylinder = new MYON_Extrusion();
  }
...
MYON_Extrusion* MYON_Extrusion::Pipe( 
  const MYON_Cylinder& cylinder, 
  double other_radius,
  bool bCapBottom,
  bool bCapTop,
...
{
  if (    !cylinder.IsValid() 
       || !MYON_IsValid(other_radius)
       || !(fabs(other_radius - cylinder.circle.Radius()) > MYON_ZERO_TOLERANCE)
       )
  {
    return 0;
  }
  ...
  double outer_radius = (other_radius < cylinder.circle.radius)
                      ? cylinder.circle.radius
                      : other_radius;
  if (    !MYON_IsValid(inner_radius) 
       || !MYON_IsValid(outer_radius)
       || !(outer_radius - inner_radius > MYON_ZERO_TOLERANCE)
     )
  {
    return 0;
  }

  MYON_Cylinder outer_cylinder = cylinder;
  outer_cylinder.circle.radius = outer_radius;

  MYON_Circle inner_circle(MYON_Plane::World_xy,inner_radius);
  MYON_ArcCurve* inner_profile = new MYON_ArcCurve(inner_circle);
  ...
  MYON_Extrusion* extrusion_pipe = MYON_Extrusion::Cylinder(outer_cylinder,bCapBottom,bCapTop,extrusion);
  if ( 0 == extrusion_pipe )
  {
    delete inner_profile;
    return 0;
...
MYON_Extrusion* MYON_Extrusion::CreateFrom3dCurve( 
    const MYON_Curve& curve,
    ...
    )
{
  ...
  if ( MYON_IsValid(height) && 0.0 == height )
    return 0;

  MYON_Interval z(0.0,height);
  if ( z.IsDecreasing() )
    z.Swap();
  if ( !z.IsIncreasing() )
    return 0;
  ...
  MYON_Xform xform2d;
  xform2d.ChangeBasis(MYON_Plane::World_xy,*plane);

  MYON_Curve* curve2d = curve.DuplicateCurve();
  if ( 0 == curve2d )
    return 0;

  MYON_Extrusion* result = 0;
...
class MYON_DisplayMesh
{
private:
  MYON_DisplayMesh();
  ~MYON_DisplayMesh();
  MYON_DisplayMesh(const MYON_DisplayMesh&);
  MYON_DisplayMesh& operator=(const MYON_DisplayMesh&);
  ...
  */
  int GetMeshDisplayVertices( MYON_Xform* model_view_xform, MYON_3fPoint const** mesh_V ) const;
...
private:
  ...
  MYON_Xform m_display_F_xform;
  MYON_SimpleArray< MYON_3fPoint > m_display_F;
...
MYON_OBJECT_IMPLEMENT(MYON_V5ExtrusionDisplayMeshCache,MYON_UserData,"A8130A3E-E4F3-4CB0-BB8A-F10A473912D0");

MYON_V5ExtrusionDisplayMeshCache::MYON_V5ExtrusionDisplayMeshCache()
  : MYON_UserData()
{
  m_userdata_uuid = MYON_CLASS_ID(MYON_V5ExtrusionDisplayMeshCache);
...
MYON_V5ExtrusionDisplayMeshCache::MYON_V5ExtrusionDisplayMeshCache(const MYON_V5ExtrusionDisplayMeshCache& src)
  : MYON_UserData(src)
{
  m_userdata_uuid = MYON_CLASS_ID(MYON_V5ExtrusionDisplayMeshCache);
  m_application_uuid = MYON_opennurbs5_id; // opennurbs.dll reads/writes this userdata
...
MYON_V5ExtrusionDisplayMeshCache& MYON_V5ExtrusionDisplayMeshCache::operator=(const MYON_V5ExtrusionDisplayMeshCache& src)
{
  if ( this != &src )
  {
    DestroyHelper();
...
void MYON_V5ExtrusionDisplayMeshCache::CopyHelper(const MYON_V5ExtrusionDisplayMeshCache& src)
{
  m_render_mesh = src.m_render_mesh;
  m_analysis_mesh = src.m_analysis_mesh;
}

void MYON_V5ExtrusionDisplayMeshCache::DestroyHelper()
{
  m_render_mesh.reset();
...
bool MYON_V5ExtrusionDisplayMeshCache::Write(MYON_BinaryArchive& binary_archive) const
{
  bool rc = true;
  bool bSaveMeshes = binary_archive.Save3dmRenderMesh(MYON::extrusion_object);
  const MYON_Mesh* meshes[3] = { m_render_mesh.get(), m_analysis_mesh.get(), nullptr };
  ...
  {
    const MYON_Mesh* mesh = bSaveMeshes ? meshes[i] : nullptr;
...
bool MYON_V5ExtrusionDisplayMeshCache::Read(MYON_BinaryArchive& binary_archive)
{
  DestroyHelper();
  bool rc = true;
  
  MYON_Mesh* meshes[3] = { 0 };
  for(size_t i = 0; i < sizeof(meshes)/sizeof(meshes[0]) && rc; i++ )
  {
    MYON_Object* p = 0;
  ...
  m_render_mesh = std::shared_ptr<MYON_Mesh>(meshes[0]);
  m_analysis_mesh = std::shared_ptr<MYON_Mesh>(meshes[1]);

  return rc;
}

bool MYON_V5ExtrusionDisplayMeshCache::DeleteAfterWrite(
  const class MYON_BinaryArchive& archive,
  const class MYON_Object* parent_object
  ) const
{
...
bool MYON_V5ExtrusionDisplayMeshCache::DeleteAfterRead(
  const class MYON_BinaryArchive& archive,
  class MYON_Object* parent_object
  ) const
{
  MYON_Extrusion* extrusion = MYON_Extrusion::Cast(parent_object);
  if (nullptr != extrusion)
  {
    if ( nullptr != m_analysis_mesh.get() )
      extrusion->m_mesh_cache.SetMesh(MYON_MeshCache::AnalysisMeshId, m_analysis_mesh);
    if ( nullptr != m_render_mesh.get() )
      extrusion->m_mesh_cache.SetMesh(MYON_MeshCache::RenderMeshId, m_render_mesh);
  }
  return true;
}

//virtual 
bool MYON_V5ExtrusionDisplayMeshCache::GetDescription( MYON_wString& description )
{
...
//virtual 
bool MYON_V5ExtrusionDisplayMeshCache::Archive() const
{
  return (nullptr == m_render_mesh.get() && m_analysis_mesh.get()) ? false : true;
}


MYON_V5ExtrusionDisplayMeshCache* MYON_V5ExtrusionDisplayMeshCache::CreateMeshCache(const MYON_Extrusion* p)
{
  if ( 0 == p )
    return nullptr;
  ...
  std::shared_ptr<MYON_Mesh> render_mesh = p->m_mesh_cache.MeshSharedPtr(MYON_MeshCache::RenderMeshId);
  std::shared_ptr<MYON_Mesh> analysis_mesh = p->m_mesh_cache.MeshSharedPtr(MYON_MeshCache::AnalysisMeshId);

  if (nullptr == render_mesh.get() && nullptr == analysis_mesh.get() )
    return nullptr;

  MYON_V5ExtrusionDisplayMeshCache* mc = MYON_V5ExtrusionDisplayMeshCache::Cast( p->GetUserData(MYON_CLASS_ID(MYON_V5ExtrusionDisplayMeshCache)) );
  if ( nullptr == mc )
  {
    mc = new MYON_V5ExtrusionDisplayMeshCache();
    if ( false == const_cast<MYON_Extrusion*>(p)->AttachUserData(mc) )
    {
      delete mc;
      mc = nullptr;
    }
...
bool MYON_Extrusion::SetMesh( MYON::mesh_type mt, MYON_Mesh* mesh )
{
  m_mesh_cache.SetMesh(mt,std::shared_ptr<MYON_Mesh>(mesh));
  return true;
}

const MYON_Mesh* MYON_Extrusion::Mesh( MYON::mesh_type mt ) const
{
  return m_mesh_cache.MeshSharedPtr(mt).get();
}

void MYON_Extrusion::DestroyMesh( MYON::mesh_type mt )
{
