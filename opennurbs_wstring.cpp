...
static int w2c_size( int w_count, const wchar_t* w )
{
  ...
  if ( w ) {
    ...
    rc = MYON_ConvertWideCharToUTF8(false,w,w_count,0,0,&error_status,0,0,0);
    if ( error_status )
    {
      MYON_ERROR("wchar_t string is not valid.");
    }
    if ( rc < 0 )
      rc = 0;
  }
                ...
                )
{
  ...
  if ( c_count > 0 && c )
  {
    ...
    if ( w ) 
    {
      ...
      const wchar_t* p1 = 0;
      rc = MYON_ConvertWideCharToUTF8(false,w,w_count,c, c_count, &error_status,error_mask,error_code_point,&p1);
      if ( error_status )
      {
        MYON_ERROR("wchar_t string is not valid.");
      }
                ...
                )
{
  ...
  if ( w_count > 0 && w && c_count > 0 && c && c[0] ) {
    ...
    if ( c ) 
    {
      ...
      MYON__UINT32 error_code_point = 0xFFFD;
      const char* p1 = 0;
      rc = MYON_ConvertUTF8ToWideChar(false,c,c_count,w,w_count,&error_status,error_mask,error_code_point,&p1);
      if ( rc > 0 && rc <= w_count )
        w[rc] = 0;
      else {
        w[w_count] = 0;
...
void MYON_String::CopyToArray( int w_count, const wchar_t* w )
{
...
class MYON_wStringHeader
{
private:
  MYON_wStringHeader() = delete;
public:
  ~MYON_wStringHeader() = default;
  MYON_wStringHeader(const MYON_wStringHeader&) = default;
  MYON_wStringHeader& operator=(const MYON_wStringHeader&) = default;
...
class MYON_Internal_Empty_wString
{
private:
  MYON_Internal_Empty_wString(const MYON_Internal_Empty_wString&) = delete;
  MYON_Internal_Empty_wString& operator=(const MYON_Internal_Empty_wString&) = delete;

public: 
  MYON_Internal_Empty_wString()
    : header(-1,0)
  {}
  ~MYON_Internal_Empty_wString() = default;

public:
  MYON_wStringHeader header;
  wchar_t  s = 0;    
};

static MYON_Internal_Empty_wString empty_wstring;
static const MYON_wStringHeader* pEmptyStringHeader = &empty_wstring.header;
static const wchar_t* pEmptywString = &empty_wstring.s;

static void MYON_wStringHeader_DecrementRefCountAndDeleteIfZero(class MYON_wStringHeader* hdr)
{
  //// sz must be = 12 or SDK breaks
  //size_t sz = sizeof(*hdr);
  //MYON_TextLog::Null.Print((const char*)nullptr, (int)sz);
...
) const
{
  ...
  for (;;)
  {
    ...
    if (0 != ((MYON__UINT_PTR)s) % 4)
      break;
#endif
    const MYON_wStringHeader* hdr = Header();
    if (nullptr == hdr)
      break;
  ...
  const_cast<MYON_wString*>(this)->m_s = (wchar_t*)pEmptywString;
  // Devs
  //  If you get this error, some earlier operation corrupted the string
  //  It is critical to track this bug down ASAP.
  MYON_ERROR("Corrupt MYON_wString - crash prevented.");
  return false;
}

MYON_wStringHeader* MYON_wString::IncrementedHeader() const
{
  MYON_wStringHeader* hdr = (MYON_wStringHeader*)m_s;
  if (nullptr == hdr)
    return nullptr;
...
MYON_wStringHeader* MYON_wString::Header() const
{
  MYON_wStringHeader* hdr = (MYON_wStringHeader*)m_s;
  if (hdr)
    hdr--;
...
wchar_t* MYON_wString::CreateArray( int capacity )
{
  Destroy();
  ...
  if ( capacity > 0 ) 
  {
    // This scope does not need atomic operations
		void* buffer = onmalloc( sizeof(MYON_wStringHeader) + (capacity+1)*sizeof(*m_s) );
    MYON_wStringHeader* hdr = new(buffer) MYON_wStringHeader(1,capacity);
...
void MYON_wString::Destroy()
{
  MYON_wStringHeader* hdr = Header();
  if ( hdr != pEmptyStringHeader && nullptr != hdr && (int)(hdr->ref_count) > 0 )
    MYON_wStringHeader_DecrementRefCountAndDeleteIfZero(hdr);
	Create();
}

void MYON_wString::Empty()
{
  Destroy();
  Create();  
}

void MYON_wString::EmergencyDestroy()
{
	Create();
}

void MYON_wString::EnableReferenceCounting( bool bEnable )
{
  // OBSOLETE - DELETE WHEN SDK CAN BE BROKEN
}

bool MYON_wString::IsReferenceCounted() const
{
  return true;
}


void MYON_wString::CopyArray()
{
  // If 2 or more string are using array, it is duplicated.
  // Call CopyArray() before modifying array contents.
  // hdr0 = original header
  MYON_wStringHeader* hdr0 = Header();
  if ( hdr0 != pEmptyStringHeader && nullptr != hdr0 && (int)(hdr0->ref_count) > 1 ) 
  {
    ...
    MYON_wStringHeader_DecrementRefCountAndDeleteIfZero(hdr0);
  }
}

wchar_t* MYON_wString::ReserveArray( size_t array_capacity )
{
  if (array_capacity <= 0)
    return nullptr;

  if (array_capacity > (size_t)MYON_wString::MaximumStringLength)
  {
    MYON_ERROR("Requested capacity > MYON_String::MaximumStringLength");
    return nullptr;
  }
  ...
  else if ( (int)(hdr0->ref_count) > 1 ) 
  {
    ...
    // Allocate a new array
		CreateArray(capacity);
    MYON_wStringHeader* hdr1 = Header();
  ...
  {
		hdr0 = (MYON_wStringHeader*)onrealloc( hdr0, sizeof(MYON_wStringHeader) + (capacity+1)*sizeof(*m_s) );
...
void MYON_wString::ShrinkArray()
{
  ...
  else if ( hdr0 != pEmptyStringHeader ) 
  {
    ...
    {
      // onrealloc string
		  hdr0 = (MYON_wStringHeader*)onrealloc( hdr0, sizeof(MYON_wStringHeader) + (hdr0->string_length+1)*sizeof(*m_s) );
...
void MYON_wString::CopyToArray( const MYON_wString& s )
{
  CopyToArray( s.Length(), s.Array() );
}

void MYON_wString::CopyToArray( int size, const char* s )
{
  while (size > 0 && s && s[0])
  {
...
void MYON_wString::CopyToArray( int size, const unsigned char* s )
{
  CopyToArray( size, ((char*)s) );
}

void MYON_wString::CopyToArray( int size, const wchar_t* s )
{
...
void MYON_wString::AppendToArray( const MYON_wString& s )
{
  AppendToArray( s.Length(), s.Array() );
}

void MYON_wString::AppendToArray( int size, const char* s )
{
  if ( size > 0 && s && s[0] ) 
  {
...
void MYON_wString::AppendToArray( int size, const unsigned char* s )
{
  AppendToArray( size, ((char*)s) );
}

void MYON_wString::AppendToArray( int size, const wchar_t* s )
{
  if ( size > 0 && s && s[0] ) 
  {
    if (nullptr == ReserveArray(size + Header()->string_length))
...
int MYON_wString::Length(const wchar_t* s)
{
  return MYON_wString::Length(s, 2147483645);
}

int MYON_wString::Length(
  const wchar_t* s,
...
unsigned int MYON_wString::UnsignedLength(const wchar_t* s)
{
...
MYON_wString::~MYON_wString()
{
  Destroy();
}

MYON_wString::MYON_wString(const MYON_wString& src)
{
  const MYON_wStringHeader* p = src.IncrementedHeader();
  if (nullptr != p)
  {
    m_s = src.m_s;
  }
...
// Clone constructor
MYON_wString::MYON_wString( MYON_wString&& src ) MYON_NOEXCEPT
{
  m_s = src.m_s;
...
MYON_wString& MYON_wString::operator=( MYON_wString&& src ) MYON_NOEXCEPT
{
  if ( this != &src )
  {
    this->Destroy();
...
MYON_wString::MYON_wString(const MYON_String& src)
{
	Create();
	*this = src;
}

MYON_wString::MYON_wString( const char* s )
{
	Create();
  if ( s && s[0] ) 
  {
...
MYON_wString::MYON_wString( const char* s, int length )
{
	Create();
  if ( s && length > 0 ) {
...
MYON_wString::MYON_wString( char c, int repeat_count )
{
  Create();
  if ( repeat_count > 0 ) {
...
MYON_wString::MYON_wString( const unsigned char* s )
{
	Create();
  if ( s && s[0] ) {
...
MYON_wString::MYON_wString( const unsigned char* s, int length )
{
	Create();
  if ( s && length > 0 ) {
    CopyToArray(length,s);
...
MYON_wString::MYON_wString( unsigned char c, int repeat_count )
{
  Create();
  if ( repeat_count > 0 ) {
    char* s = (char*)onmalloc((repeat_count+1)*sizeof(*s));
...
MYON_wString::MYON_wString( const wchar_t* s )
{
	Create();
  if ( s && s[0] ) {
...
MYON_wString::MYON_wString( const wchar_t* s, int length )
{
	Create();
  if ( s && length > 0 ) {
    CopyToArray( length, s );
...
MYON_wString::MYON_wString( wchar_t c, int repeat_count )
{
  Create();
...
bool MYON_wString::LoadResourceString(HINSTANCE instance, UINT id )
{
  bool rc = false;
...
MYON_String::MYON_String(CFStringRef appleString)
{
  Create();
  for (;;)
  {
    ...
    MYON_SimpleArray<char> local_buffer;
    if (nullptr == utf8_str)
    {
...
MYON_wString::MYON_wString(CFStringRef appleString)
{
  Create();
  for (;;)
  {
    ...
    MYON_SimpleArray<UniChar> local_buffer;
    ...
    else
    {
      ...
      for (CFIndex i = 0;i < utf16_count;i++)
      {
        code_point = (MYON__UINT32)(utf16_str[i]);
        if (
          0 == MYON_IsValidUTF16Singleton(code_point)
          && ( i+1 < utf16_count )
          && MYON_IsValidUTF16SurrogatePair(code_point,utf16_str[i + 1])
          )
        {
          code_point = MYON_DecodeUTF16SurrogatePair(code_point, utf16_str[i + 1], MYON_UnicodeCodePoint::MYON_InvalidCodePoint);
          if (MYON_UnicodeCodePoint::MYON_InvalidCodePoint != code_point)
            i++;
          else
            code_point = (MYON__UINT32)(utf16_str[i]);
        }
...
CFStringRef MYON_wString::ToAppleCFString() const
{
  if ( IsEmpty() || Length() <= 0)
    return nullptr;
  const MYON_String utf8_string(*this);
  return utf8_string.ToAppleCFString();
}

CFStringRef MYON_String::ToAppleCFString() const
{
  for(;;)
  {
    if ( IsEmpty() || Length() <= 0 )
...
bool MYON_String::IsHexDigit(char c)
{
  return (c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f');
}

bool MYON_String::IsDecimalDigit(char c)
{
  return (c >= '0' && c <= '9');
}


bool MYON_wString::IsHexDigit(wchar_t c)
{
  return (c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f');
}

bool MYON_wString::IsDecimalDigit(wchar_t c)
{
  return (c >= '0' && c <= '9');
}


bool MYON_wString::IsDecimalDigit(
  wchar_t c,
...
unsigned MYON_wString::DecimalDigitFromWideChar(
  wchar_t c,
...
int MYON_wString::PlusOrMinusSignFromWideChar(
  wchar_t c,
...
)
{
  switch (c)
  {
  case MYON_UnicodeCodePoint::MYON_Slash:
    return bOrdinarySlashResult ? true : false;
  case MYON_UnicodeCodePoint::MYON_FractionSlash:
    return bFractionSlashResult ? true : false;
  case MYON_UnicodeCodePoint::MYON_DivisionSlash:
    return bDivisionSlashResult ? true : false;
  case MYON_UnicodeCodePoint::MYON_MathimaticalSlash:
...
int MYON_wString::Length() const
{
  return Header()->string_length;
}

unsigned int MYON_wString::UnsignedLength() const
{
  return (unsigned int)Header()->string_length;
}

wchar_t& MYON_wString::operator[](int i)
{
  CopyArray();
  return m_s[i];
}

wchar_t MYON_wString::operator[](int i) const
{
  return m_s[i];
}

bool MYON_wString::IsEmpty() const
{
  return (Header()->string_length <= 0) ? true : false;
}

bool MYON_wString::IsNotEmpty() const
{
  return (Header()->string_length > 0) ? true : false;
}

const MYON_wString& MYON_wString::operator=(const MYON_wString& src)
{
  if (m_s != src.m_s)
  {
...
const MYON_wString& MYON_wString::operator=(const MYON_String& src)
{
  *this = src.Array();
  return *this;
}

const MYON_wString& MYON_wString::operator=( char c )
{
	CopyToArray( 1, &c );
	return *this;
}

const MYON_wString& MYON_wString::operator=( const char* s )
{
  if ( (void*)s != (void*)m_s )
...
const MYON_wString& MYON_wString::operator=( unsigned char c )
{
	CopyToArray( 1, &c );
	return *this;
}

const MYON_wString& MYON_wString::operator=( const unsigned char* s )
{
  if ( (void*)s != (void*)m_s )
...
const MYON_wString& MYON_wString::operator=( wchar_t c )
{
	CopyToArray( 1, &c );
	return *this;
}

const MYON_wString& MYON_wString::operator=( const wchar_t* s )
{
  if ( (void*)s != (void*)m_s )
...
MYON_wString MYON_wString::operator+(const MYON_wString& s2) const
{
	MYON_wString s(*this);
  s.AppendToArray( s2 );
	return s;
...
MYON_wString MYON_wString::operator+(const MYON_String& s2) const
{
	MYON_wString s(*this);
...
MYON_wString MYON_wString::operator+(char s2 ) const
{
	MYON_wString s(*this);
...
MYON_wString MYON_wString::operator+(unsigned char s2 ) const
{
	MYON_wString s(*this);
  s.AppendToArray( 1, &s2 );
	return s;
...
MYON_wString MYON_wString::operator+( wchar_t s2 ) const
{
	MYON_wString s(*this);
  s.AppendToArray( 1, &s2 );
	return s;
...
MYON_wString MYON_wString::operator+(const char* s2) const
{
	MYON_wString s(*this);
...
MYON_wString MYON_wString::operator+(const unsigned char* s2) const
{
	MYON_wString s(*this);
...
MYON_wString MYON_wString::operator+(const wchar_t* s2) const
{
	MYON_wString s(*this);
  s.AppendToArray( MYON_wString::Length(s2), s2 );
	return s;
}
...
void MYON_wString::Append( const char* s , int count )
{
...
void MYON_wString::Append( const unsigned char* s , int count )
{
  // append specified number of characters
  if ( s && count > 0 )
    AppendToArray(count,s);
...
void MYON_wString::Append( const wchar_t* s, int count )
{
  // append specified number of characters
  if ( s && count > 0 )
    AppendToArray(count,s);
...
const MYON_wString& MYON_wString::operator+=(const MYON_wString& s)
{
...
const MYON_wString& MYON_wString::operator+=(const MYON_String& s)
{
  AppendToArray( s.Length(), s.Array() );
	return *this;
}

const MYON_wString& MYON_wString::operator+=( char s )
{
  AppendToArray(1,&s);
	return *this;
}

const MYON_wString& MYON_wString::operator+=( unsigned char s )
{
  AppendToArray(1,&s);
	return *this;
}

const MYON_wString& MYON_wString::operator+=( wchar_t s )
{
  AppendToArray(1,&s);
	return *this;
}
...
const MYON_wString& MYON_wString::operator+=( const char* s )
{
  AppendToArray(MYON_String::Length(s),s);
	return *this;
}
...
const MYON_wString& MYON_wString::operator+=( const wchar_t* s )
{
  AppendToArray(MYON_wString::Length(s),s);
	return *this;
}

wchar_t* MYON_wString::SetLength(size_t string_length)
{
  if (string_length >= (size_t)MYON_String::MaximumStringLength)
  {
    MYON_ERROR("Requested size > MYON_String::MaximumStringLength");
    return nullptr;
  }
...
wchar_t* MYON_wString::Array()
{
  CopyArray();
  return ( Header()->string_capacity > 0 ) ? m_s : 0;
}

const wchar_t* MYON_wString::Array() const
{
  return ( Header()->string_capacity > 0 ) ? m_s : 0;
}

const MYON_wString MYON_wString::Duplicate() const
{
  if (Length() <= 0)
...
*/
unsigned int MYON_wString::SizeOf() const
{
  size_t sz = sizeof(*this);
  if ( ((const void*)m_s) != ((const void*)pEmptywString) )
    sz += (sizeof(MYON_wStringHeader) + sizeof(wchar_t)*(Header()->string_capacity+1));
  return ((unsigned int)sz);
}

MYON__UINT32 MYON_wString::DataCRC(MYON__UINT32 current_remainder) const
{
...
MYON__UINT32 MYON_wString::DataCRCLower(MYON__UINT32 current_remainder) const
{
  int string_length = Header()->string_length;
  if ( string_length > 0 )
  {
...
int MYON_wString::Compare( const wchar_t* s ) const
{
  return MYON_wString::CompareOrdinal(s,false);
}

int MYON_wString::CompareNoCase( const wchar_t* s) const
{
  return MYON_wString::CompareOrdinal(s,true);
}

bool MYON_WildCardMatch(const wchar_t* s, const wchar_t* pattern)
{
...
bool MYON_WildCardMatchNoCase(const wchar_t* s, const wchar_t* pattern)
{
  if ( !pattern || !pattern[0] ) {
    return ( !s || !s[0] ) ? true : false;
  }
...
bool MYON_wString::WildCardMatch( const wchar_t* pattern ) const
{
  return MYON_WildCardMatch(m_s,pattern);
}


bool MYON_wString::WildCardMatchNoCase( const wchar_t* pattern ) const
{
  return MYON_WildCardMatchNoCase(m_s,pattern);
}

/*
static TestReplace( MYON_TextLog* text_log )
{
...
int MYON_wString::Replace( const wchar_t* token1, const wchar_t* token2 )
{
  int count = 0;
...
int MYON_wString::Replace( wchar_t token1, wchar_t token2 )
{
  int count = 0;
  int i = Length();
  while (i--)
...
static bool IsValidUrlChar(wchar_t c)
{
  ...
  // MYON_wString::UrlEncode() encodes assumes the following
...
int MYON_wString::ReplaceWhiteSpace( wchar_t token, const wchar_t* whitespace )
{
  wchar_t* s0;
  wchar_t* s1;
  int n;
...
int MYON_wString::RemoveWhiteSpace( const wchar_t* whitespace )
{
  wchar_t* s0;
  wchar_t* s1;
  wchar_t* s;
...
const MYON_wString MYON_wString::RemovePrefix(
...
) const
{
  ...
  const int prefix_length = MYON_wString::Length(prefix);
  if (
    prefix_length > 0
...
const MYON_wString MYON_wString::RemoveSuffix(
...
{
  const wchar_t* str = static_cast<const wchar_t*>(*this);
  const int suffix_length = MYON_wString::Length(suffix);
  const int str_len = Length();
  if (
    suffix_length > 0
...
int MYON_wString::Find(char utf8_single_byte_c) const
{
  return (utf8_single_byte_c >= 0 && MYON_IsValidSingleElementUTF8Value(utf8_single_byte_c))
    ? Find((wchar_t)utf8_single_byte_c, 0)
    : -1;
}

int MYON_wString::Find(unsigned char utf8_single_byte_c) const
{
  return (MYON_IsValidSingleElementUTF8Value(utf8_single_byte_c))
    ? Find((wchar_t)utf8_single_byte_c, 0)
    : -1;
}

int MYON_wString::Find(wchar_t w) const
{
  return Find(w,0);
}

int MYON_wString::Find(const char* s) const
{
  return Find(s, 0);
}

int MYON_wString::Find(const unsigned char* s) const
{
  return Find(s, 0);
}

int MYON_wString::Find(const wchar_t* s) const
{
  return Find(s, 0);
}

int MYON_wString::Find(
...
{
  if (MYON_IsValidSingleElementWideCharValue(w))
  {
  ...
  ) const
{
  if ( start_index < 0x7FFFFFFF )
  {
    ...
    const int length = MYON_wString::Length(wcharString);
    if (length > 0)
    {
      ...
      if ( start_index_as_int < this_length && (this_length - start_index_as_int) >= length )
      {
        ...
        {
          if (w0 == p[0] && MYON_wString::EqualOrdinal(p, length, wcharString, length, false) )
            return ((int)(p - m_s));
        }
      }
    }
...
int MYON_wString::FindOneOf (const wchar_t* character_set) const
{
  ...
  MYON__UINT32 sUTF32[2] = { 0 };
  while (s < s1)
  {
    e.m_error_status = 0;
    int s_count = MYON_DecodeWideChar(s, (int)(s1 - s), &e, &sUTF32[0]);
    if (s_count <= 0 || 0 == sUTF32[0] || 0 != sUTF32[1])
      break;
    e.m_error_status = 0;
    int buffer_count = MYON_ConvertUTF32ToWideChar(
      false, 
      sUTF32, 1, 
...
int MYON_wString::ReverseFind(char utf8_single_byte_c) const
{
  return (utf8_single_byte_c >= 0 && MYON_IsValidSingleElementUTF8Value(utf8_single_byte_c))
...
int MYON_wString::ReverseFind(unsigned char utf8_single_byte_c) const
{
  return (MYON_IsValidSingleElementUTF8Value(utf8_single_byte_c))
    ? ReverseFind((wchar_t)utf8_single_byte_c)
    : -1;
...
int MYON_wString::ReverseFind( wchar_t c ) const
{
  if (MYON_IsValidSingleElementWideCharValue(c))
  {
    // find first single character
    int i = Length();
    while( i > 0 ) 
...
int MYON_wString::ReverseFind(const char* s) const
{
  const MYON_wString w(s);
  return ReverseFind(static_cast< const wchar_t* >(w));
}

int MYON_wString::ReverseFind(const wchar_t* s) const
{
  const int s_len = MYON_wString::Length(s);
  ...
  if (s_len > 0 && s_len <= this_len )
  {
    ...
    {
      p--;
      if ( w0 == p[0] && MYON_wString::EqualOrdinal(p,s_len,s,s_len,false) )
        return ((int)(p - p0));
    }
  }
  return -1;
...
void MYON_wString::MakeReverse()
{
  if ( IsNotEmpty() ) 
  {
  	CopyArray();
    MYON_wString::Reverse(m_s,Length());
  }
}

MYON_wString MYON_wString::Reverse() const
{
  MYON_wString reverse_string(*this);
  reverse_string.MakeReverse();
  return reverse_string;
}

static void MYON_String_ReverseUTF16(
  wchar_t* string,
  int element_count
  )
{
  ...
  MYON_wString buffer(string,element_count);
  ...
  )
{
  if (element_count < 0)
  {
    element_count = MYON_wString::Length(string);
    if (element_count < 0)
      return nullptr;
  }
  if ( 0 == element_count )
  ...
  for (i = 0, j = element_count - 1; i < j; i++, j--)
  {
    ...
    {
      MYON_String_ReverseUTF16(string + i, j - i + 1);
      return string;
    }
...
void MYON_wString::TrimLeft(const wchar_t* s)
{
  wchar_t c;
  const wchar_t* sc;
  wchar_t* dc;
...
void MYON_wString::TrimRight(const wchar_t* s)
{
  wchar_t c;
  const wchar_t* sc;
...
void MYON_wString::TrimLeftAndRight(const wchar_t* s)
{
  TrimRight(s);
  TrimLeft(s);
}
...
int MYON_wString::Remove(char c)
{
  if (c >= 0 && MYON_IsValidSingleElementUTF8Value((MYON__UINT32)c))
    return Remove((wchar_t)c);
  return 0;
}

int MYON_wString::Remove(unsigned char c)
{
  if (MYON_IsValidSingleElementUTF8Value((MYON__UINT32)c))
    return Remove((wchar_t)c);
  return 0;
}

int MYON_wString::Remove(wchar_t c)
{
  if (MYON_IsValidSingleElementWideCharValue(c))
  {
    wchar_t* s0;
...
wchar_t MYON_wString::GetAt(int i) const
{
  return m_s[i];
}


void MYON_wString::SetAt( int i, char c )
{
  if ( i >= 0 && i < Header()->string_length ) {
	  CopyArray();
    if (c < 0 || c > 127)
    {
      MYON_ERROR("c is not a valid single byte utf-8 value.");
    }
	  m_s[i] = (wchar_t)c;
  }
}

void MYON_wString::SetAt( int i, unsigned char c )
{
  SetAt( i, (char)c );
}

void MYON_wString::SetAt( int i, wchar_t c )
{
  if ( i >= 0 && i < Header()->string_length ) {
	  CopyArray();
	  m_s[i] = c;
...
MYON_wString MYON_wString::Mid(int i, int count) const
{
...
MYON_wString MYON_wString::Mid(int i) const
{
  return Mid( i, Length() - i );
}

const MYON_wString MYON_wString::SubString(
  int start_index
) const
{
...
MYON_wString MYON_wString::Left(int count) const
{
  MYON_wString s;
  if ( count > Length() )
    count = Length();
...
MYON_wString MYON_wString::Right(int count) const
{
  MYON_wString s;
  if ( count > Length() )
    count = Length();
...
const MYON_wString MYON_wString::EncodeXMLValue() const
{
...
const MYON_wString MYON_wString::EncodeXMLValue(
...
) const
{
  ...
  int length1 = 0;
  struct MYON_UnicodeErrorParameters e;
  for (const wchar_t* buffer1 = buffer0; buffer1 < buffer0_end; ++buffer1, ++length1)
  {
    const wchar_t c = *buffer1;
    switch (c)
    {
    case MYON_UnicodeCodePoint::MYON_QuotationMark:
      length1 += 5;
      break;
    case MYON_UnicodeCodePoint::MYON_Ampersand:
      length1 += 4;
      break;
    case MYON_UnicodeCodePoint::MYON_Apostrophe:
      length1 += 5;
      break;
    case MYON_UnicodeCodePoint::MYON_LessThanSign:
      length1 += 3;
      break;
    case MYON_UnicodeCodePoint::MYON_GreaterThanSign:
      length1 += 3;
      break;
    default:
      if (bEncodeCodePointsAboveBasicLatin && (c < 0 || c > 127))
      {
        e = MYON_UnicodeErrorParameters::MaskErrors;
        MYON__UINT32 u = MYON_UnicodeCodePoint::MYON_ReplacementCharacter;
        const int decoded_wchar_count = MYON_DecodeWideChar(buffer1, (int)(buffer0_end - buffer1), &e, &u);
        if (decoded_wchar_count > 0 && MYON_IsValidUnicodeCodePoint(u))
        {
  ...
  if (nullptr == encoded)
    return MYON_wString::EmptyString; // catastrophe
  ...
  for (const wchar_t* buffer1 = buffer0; buffer1 < buffer0_end; ++buffer1)
  {
    *encoded = *buffer1;
    ...
    {
    case MYON_UnicodeCodePoint::MYON_QuotationMark:
      *encoded++ = MYON_wString::Ampersand;
      *encoded++ = 'q';
      *encoded++ = 'u';
      *encoded++ = 'o';
      *encoded++ = 't';
      *encoded++ = MYON_wString::Semicolon;
      break;
    ...
    default:
      if (bEncodeCodePointsAboveBasicLatin && (*encoded < 0 || *encoded > 127))
      {
        ...
        if (decoded_wchar_count > 0 && MYON_IsValidUnicodeCodePoint(u))
        {
          ...
          {
            *encoded++ = MYON_wString::Ampersand;
            *encoded++ = MYON_wString::NumberSign;
            *encoded++ = 'x';
...
const MYON_wString MYON_wString::DecodeXMLValue() const
{
  const int length0 = this->Length();
  if (length0 <= 0)
    return MYON_wString::EmptyString;
  ...
  for (const wchar_t* buffer1 = buffer0; buffer1 < buffer0_end; ++buffer1)
  {
    if (MYON_wString::Ampersand != *buffer1)
      continue;
    if (nullptr == MYON_wString::ParseXMLCharacterEncoding(buffer1, (int)(buffer0_end - buffer1), 0, nullptr))
      continue;

    // need to copy and modify.
    MYON_wString s = this->Duplicate();
    if (s.Length() != length0)
      return MYON_wString::EmptyString; // catastrophe!
    wchar_t* b0 = s.Array();
    if ( b0 == buffer0)
    ...
    {
      c = *buffer1;
      if (MYON_wString::Ampersand == c)
      {
        unsigned u = MYON_UnicodeCodePoint::MYON_InvalidCodePoint;
        const wchar_t* buffer2 = MYON_wString::ParseXMLCharacterEncoding(buffer1, (int)(buffer0_end - buffer1), u, &u);
        if (buffer2 > buffer1)
        {
          buffer1 = buffer2;
          wchar_t w[8] = {};
          const int wcount = MYON_EncodeWideChar(u, sizeof(w) / sizeof(w[0]), w);
          if (wcount >= 1)
          {
...
bool MYON_wString::NeedsXMLEncode(void) const
{
  if (FindOneOf(L"&\"\'<>\n\r") >= 0)
    return true;
...
bool MYON_wString::IsXMLSpecialCharacter(wchar_t c)
{
  switch (c)
  {
  case MYON_UnicodeCodePoint::MYON_QuotationMark:
...
bool MYON_wString::IsXMLSpecialCharacterEncoding(void) const
{
...
const wchar_t* MYON_wString::ParseXMLUnicodeCodePointEncoding(
...
)
{
  ...
  */
  ...
  if (nullptr == buffer)
    return nullptr;
  if (-1 == buffer_length)
    buffer_length = MYON_wString::MaximumStringLength;
  else if (buffer_length < 4)
    return nullptr;

  if (MYON_wString::Ampersand != buffer[0] || MYON_wString::NumberSign != buffer[1])
    return nullptr;

  if (buffer_length >= 4 && MYON_wString::IsDecimalDigit(buffer[2]))
  {
    // decimal encoding
    unsigned n = 0U;
    int i;
    for (i = 2; i < buffer_length && n < MYON_MaximumCodePoint && MYON_wString::IsDecimalDigit(buffer[i]); ++i)
    {
      n = 10U * n + (unsigned)(buffer[i] - '0');
    }
    if (i <= buffer_length && MYON_wString::Semicolon == buffer[i] && MYON_IsValidUnicodeCodePoint(n))
    {
      if (nullptr != unicode_code_point)
  ...
  else if (buffer_length >= 5 && 'x' == buffer[2] && MYON_wString::IsHexDigit(buffer[3]))
  {
    // hexadecimal encoding
    unsigned n = 0U;
    int i;
    for (i = 3; i < buffer_length && n < MYON_MaximumCodePoint && MYON_wString::IsHexDigit(buffer[i]); ++i)
    {
...
const wchar_t* MYON_wString::ParseXMLCharacterEncoding(
  const wchar_t* buffer,
  int buffer_length,
  unsigned value_on_failure,
  unsigned* unicode_code_point
)
{
  ...
  if (MYON_wString::Ampersand != buffer[0])
    return nullptr;

  if (MYON_UnicodeCodePoint::MYON_NumberSign == buffer[1])
  ...
  case 'a':
    if (buffer_length >= 5
      && 'm' == buffer[2]
      && 'p' == buffer[3]
      && MYON_wString::Semicolon == buffer[4]
      )
    {
      buffer += 5;
      u = MYON_UnicodeCodePoint::MYON_Ampersand;
    }
    else if (buffer_length >= 6
      && 'p' == buffer[2]
      && 'o' == buffer[3]
      && 's' == buffer[4]
      && MYON_wString::Semicolon == buffer[5]
      )
    {
      buffer += 6;
      u = MYON_UnicodeCodePoint::MYON_Apostrophe;
    }
    break;
  ...
  case 'g':
    if (buffer_length >= 4
      && 't' == buffer[2] 
      && MYON_wString::Semicolon == buffer[3]
      )
    {
      buffer += 4;
      u = MYON_UnicodeCodePoint::MYON_GreaterThanSign;
    }
    break;
  ...
  case 'l':
    ...
    {
      buffer += 4;
      u = MYON_UnicodeCodePoint::MYON_LessThanSign;
    }
    break;
  ...
  case 'q':
    ...
    {
      buffer += 6;
      u = MYON_UnicodeCodePoint::MYON_QuotationMark;
    }
    break;
  }
...
const MYON_wString MYON_wString::RichTextExample(
...
)
{
  ...
  MYON_wString s = MYON_wString(L"{\\rtf1\\deff0{\\fonttbl{\\f0 ") + rich_text_font_name + MYON_wString(L";}}");

  // Specify a base font and size
  s += MYON_wString(L"\\f0 \\fs23");

  // Sample text
  s += MYON_wString(L"{\\f0 ") + rich_text_font_name + MYON_wString(L" rich text example:\\par}");

  s += MYON_wString(L"{\\f0 Regular"); 
  if (bUnderline)
    s += MYON_wString(L" }{\\f0\\ul underlined");
  s += MYON_wString(L"\\par}");

  if (bBold)
  {
    s += MYON_wString(L"{\\f0\\b Bold}");
    if (bUnderline) 
      s += MYON_wString(L" }{\\f0\\b\\ul underlined");
    s += MYON_wString(L"\\par}");
  }

  if (bItalic)
  {
    s += MYON_wString(L"{\\f0\\i Italic}");
    if (bUnderline) 
      s += MYON_wString(L" }{\\f0\\i\\ul underlined");
    s += MYON_wString(L"\\par}");
  }

  if (bBoldItalic)
  {
    s += MYON_wString(L"{\\f0\\b\\i Bold-Italic}");
    if (bUnderline)
      s += MYON_wString(L" }{\\f0\\b\\i\\ul underlined");
    s += MYON_wString(L"\\par}");
  }
...
{
  if (nullptr == quartet)
    return MYON_wString::Example(MYON_wString::ExampleType::RichText);
  return MYON_wString::RichTextExample(quartet->QuartetName(), quartet->HasBoldFace(), quartet->HasItalicFace(), quartet->HasBoldItalicFace(), true);
}
...
)
{
  ...
  const MYON_FontFaceQuartet q = font->FontQuartet();
  if (q.IsNotEmpty())
  {
    // restrict example to supported faces
    // Many fonts (Arial Black, Corsiva, ...) do not have all 4 rich text faces.
    return MYON_wString::RichTextExample(q.QuartetName(), q.HasBoldFace(), q.HasItalicFace(), q.HasBoldItalicFace(), true);
  }

  return MYON_wString::RichTextExample(font->RichTextFontName(), true, true, true, true);
}

const MYON_wString MYON_wString::Example(MYON_wString::ExampleType t)
{
  MYON_wString s;
  switch (t)
  {
  case MYON_wString::ExampleType::Empty:
    break;

  case MYON_wString::ExampleType::WideChar:
    s = MYON_wString(
      MYON_wString(L"The math teacher said, \"It isn't true that 2")
      + MYON_wString::Superscript3 + MYON_wString(L"=3") + MYON_wString::Superscript2
      + MYON_wString(L" & ")
      + MYON_wString::GreekCapitalSigma
      + MYON_wString(L" > 3")
      + MYON_wString::CentSign
      + MYON_wString(L" & ")
      + MYON_wString::GreekCapitalSigma
      + MYON_wString(L" < 2 ")
      + MYON_wString::RubleSign
      + MYON_wString(L" & ")
      + MYON_wString::GreekCapitalSigma
      + MYON_wString(L" > ")
      + MYON_wString::EuroSign
      + MYON_wString(L"99.\" ")
#if defined(MYON_SIZEOF_WCHAR_T) && MYON_SIZEOF_WCHAR_T >= 4
      + MYON_wString((wchar_t)0x1F5D1) // UTF-32 encoding for WASTEBASKET U+1F5D1
#else
      + MYON_wString((wchar_t)0xD83D) // (0xD83D, 0xDDD1) is the UTF-16 surrogate pair encoding for WASTEBASKET U+1F5D1
      + MYON_wString((wchar_t)0xDDD1)
#endif
      + MYON_wString(L"!")
    );
    break;

  case MYON_wString::ExampleType::UTF16:
    s = MYON_wString(
      MYON_wString(L"The math teacher said, \"It isn't true that 2")
      + MYON_wString::Superscript3 + MYON_wString(L"=3") + MYON_wString::Superscript2
      + MYON_wString(L" & ")
      ...
      + MYON_wString::EuroSign
      + MYON_wString(L"99.\" ")
      + MYON_wString((wchar_t)(wchar_t)0xD83D) // (0xD83D, 0xDDD1) is the UTF-16 surrogate pair encoding for WASTEBASKET U+1F5D1
      + MYON_wString((wchar_t)(wchar_t)0xDDD1)
      + MYON_wString(L"!")
    );
    break;

  case MYON_wString::ExampleType::RichText:
    s = MYON_wString::RichTextExample(&MYON_Font::Default);
    break;

  case MYON_wString::ExampleType::XML:
  ...
  case MYON_wString::ExampleType::XMLalternate1:
    /// The UTF string as an XML value with special characters encoded in the &amp;amp; format 
    /// and code points above basic latin encoded in the &#hhhh; format
    /// using  lower case hex digits (0123456789abcdef).
    s = MYON_wString(L"The math teacher said, &quot;It isn&apos;t true that 2&#xb3;=3&#xb2; &amp; &#x3a3; &gt; 3&#xa2; &amp; &#x3a3; &lt; 2 &#x20bd; &amp; &#x3a3; &gt; &#x20ac;99.&quot; &#x1f5d1;!");
    break;

  case MYON_wString::ExampleType::XMLalternate2:
    /// The UTF string as an XML value with special characters encoded in the &amp;amp; format 
  ...
  case MYON_wString::ExampleType::XMLalternate3:
  ...
  return s.IsNotEmpty() ? s : MYON_wString::EmptyString;
}

const MYON_wString MYON_wString::FormatToVulgarFraction(int numerator, int denominator)
{
  const bool bReduce = true;
  const bool bMix = true;
  const bool bUseVulgarFractionCodePoints = true;
  return MYON_wString::FormatToVulgarFraction(numerator, denominator,  bReduce, bMix, 0, bUseVulgarFractionCodePoints);
}

const MYON_wString MYON_wString::FormatToVulgarFraction(
  int numerator,
  int denominator,
  bool bReduced,
  bool bProper,
...
)
{
  ...
  {
    // ... Kids these days!
    return MYON_wString::FormatToVulgarFraction(MYON_wString::FormatToString(L"%d", numerator), L"0");
  }

  if (0 == numerator)
  {
    if (bReduced)
      return MYON_wString(L"0");
    if (bUseVulgarFractionCodePoints && 3 == numerator)
      return MYON_wString((wchar_t)0x2189); // Baseball zero for three 0/3 = U+2189

    return MYON_wString::FormatToVulgarFraction(L"0", MYON_wString::FormatToString(L"%d", denominator));
  }
  ...
  {
    const int gcd = (int)MYON_GreatestCommonDivisor((unsigned)(abs(numerator)), (unsigned)denominator);
    if (gcd > 0)
    {
      numerator /= gcd;
      denominator /= gcd;
  ...
  if (bProper && abs(numerator) >= denominator)
  {
    ...
    if (0 == numerator)
      return MYON_wString::FormatToString(L"%d", n);

    if (0 != proper_fraction_separator_cp && false == MYON_IsValidUnicodeCodePoint(proper_fraction_separator_cp))
  ...
  if (bUseVulgarFractionCodePoints && abs(numerator) < abs(denominator))
  {
    ...
    if (fraction_cp > 0 && MYON_IsValidUnicodeCodePoint(fraction_cp))
    {
      unsigned cp[3] = {};
      unsigned cp_count = 0;
      if (0 == n && numerator < 0)
        cp[cp_count++] = MYON_UnicodeCodePoint::MYON_HyphenMinus;
      cp[cp_count++] = fraction_cp;
      const MYON_wString fraction = MYON_wString::FromUnicodeCodePoints(cp, cp_count, MYON_UnicodeCodePoint::MYON_ReplacementCharacter);
      if (0 == n)
        return fraction;
      return MYON_wString::FormatToString(L"%d", n) 
        + MYON_wString::FromUnicodeCodePoint(proper_fraction_separator_cp)
        + fraction;
    }
  }

  const MYON_wString vulgar_fraction = MYON_wString::FormatToVulgarFraction(MYON_wString::FormatToString(L"%d", numerator), MYON_wString::FormatToString(L"%d", denominator));
  return
    (0 == n)
    ? vulgar_fraction
    : MYON_wString::FormatToString(L"%d", n) + MYON_wString::FromUnicodeCodePoint(proper_fraction_separator_cp) + vulgar_fraction;
}
...
{
  return MYON_wString::FormatToVulgarFractionNumerator(numerator) + MYON_wString::VulgarFractionSlash() + MYON_wString::FormatToVulgarFractionDenominator(denominator);
}

static const MYON_wString Internal_VulgarFractionXator(int updown, const MYON_wString X)
{
  if (0 == updown)
    return X;
  ...
  {
    e = MYON_UnicodeErrorParameters::MaskErrors;
    MYON__UINT32 cp0 = MYON_UnicodeCodePoint::MYON_InvalidCodePoint;
    delta = MYON_DecodeWideChar(s0 + i, len - i, &e, &cp0);
    MYON__UINT32 cp1 
      = (delta > 0 && MYON_IsValidUnicodeCodePoint(cp0))
      ? (updown > 0 ? MYON_UnicodeSuperscriptFromCodePoint(cp0,cp0) : MYON_UnicodeSubcriptFromCodePoint(cp0,cp0))
      : MYON_UnicodeCodePoint::MYON_ReplacementCharacter;
    if (cp1 != cp0 && cp1 != MYON_UnicodeCodePoint::MYON_ReplacementCharacter)
      bReturnAtor = true;
    ator += MYON_wString::FromUnicodeCodePoint(cp1);
  }
...
const MYON_wString MYON_wString::FormatToVulgarFractionNumerator(const MYON_wString numerator)
{
  return Internal_VulgarFractionXator(+1, numerator);
}

const MYON_wString MYON_wString::FormatToVulgarFractionDenominator(const MYON_wString denominator)
{
  return Internal_VulgarFractionXator(-1, denominator);
}

const MYON_wString MYON_wString::VulgarFractionSlash()
{
  return MYON_wString((wchar_t)0x2044);
}

bool MYON_wString::IsHorizontalSpace(wchar_t c, bool bTabResult, bool bNoBreakSpaceResult, bool bZeroWidthSpaceResult)
{
  ...
  else
  {
    switch (c)
    {
    case MYON_UnicodeCodePoint::MYON_OghamSpaceMark:
    case MYON_UnicodeCodePoint::MYON_EnQuad:
    case MYON_UnicodeCodePoint::MYON_EmQuad:
    case MYON_UnicodeCodePoint::MYON_EnSpace:
    case MYON_UnicodeCodePoint::MYON_EmSpace:
    case MYON_UnicodeCodePoint::MYON_ThreePerEmSpace:
    case MYON_UnicodeCodePoint::MYON_FourPerEmSpace:
    case MYON_UnicodeCodePoint::MYON_SixPerEmSpace:
    case MYON_UnicodeCodePoint::MYON_FigureSpace:
    case MYON_UnicodeCodePoint::MYON_PunctuationSpace:
    case MYON_UnicodeCodePoint::MYON_ThinSpace:
    case MYON_UnicodeCodePoint::MYON_HairSpace:
    case MYON_UnicodeCodePoint::MYON_MediumMathematicalSpace:
    case MYON_UnicodeCodePoint::MYON_IdeographicSpace:
      return true;
    ...
    case MYON_UnicodeCodePoint::MYON_ZeroWidthNonJoiner:
    case MYON_UnicodeCodePoint::MYON_ZeroWidthJoiner:
...
bool MYON_wString::IsHorizontalSpace(wchar_t c)
{
  return MYON_wString::IsHorizontalSpace(c, true, true, true);
}

const wchar_t* MYON_wString::ParseHorizontalSpace(const wchar_t* s, int len, bool bParseTab, bool bParseNoBreakSpace, bool bParseZeroWidthSpace)
{
  if (nullptr == s || len <= 0)
    return nullptr;

  int i = 0;
  for (wchar_t c = s[i]; i < len && MYON_wString::IsHorizontalSpace(c, bParseTab, bParseNoBreakSpace, bParseZeroWidthSpace); c = s[++i])
  {/*empty body*/
  }
...
const wchar_t* MYON_wString::ParseHorizontalSpace(const wchar_t* s, int len)
{
  return MYON_wString::ParseHorizontalSpace(s, len, true, true, true);
}

const wchar_t* MYON_wString::ParseVulgarFraction(const wchar_t* s, int len, int& numerator, int& denominator)
{
  numerator = 0;
  denominator = 0;
  ...
  const bool bOrdinary = MYON_wString::IsDecimalDigit(*s, true, false, false);
  const bool bSupSub = false == bOrdinary && MYON_wString::IsDecimalDigit(*s, false, true, false);
  if (false == bOrdinary || bSupSub)
    return nullptr;

  int x = 0;
  s = MYON_wString::ToNumber(s, 0, &x);
  if (nullptr == s)
    return nullptr;

  if (MYON_wString::IsSlash(*s,true,true,true,true))
    ++s;
  else
    return nullptr;

  if (false == MYON_wString::IsDecimalDigit(*s, bOrdinary, false, bSupSub))
    return nullptr;

  int y = 0;
  s = MYON_wString::ToNumber(s, 0, &y);
  if (nullptr == s)
    return nullptr;
...
const MYON_wString& MYON_wString::Set(const wchar_t* wsz, int numChars)
{
  CopyArray();
...
int MYON_wString::Count(wchar_t ch) const
{
  int count = 0;
...
bool MYON_wString::Contains(const wchar_t* wszSub) const
{
...
bool MYON_wString::ContainsNoCase(const wchar_t* wszSub) const
{
  MYON_wString s1 = *this;
  s1.MakeLowerOrdinal();
  MYON_wString s2 = wszSub;
  s2.MakeLowerOrdinal();
...
bool MYON_wString::TruncateMid(int pos)
{
  if (pos <= 0)
    return false;
...
bool MYON_wString::Insert(int index, wchar_t ch, int insert_count)
{
...
bool MYON_wString::Insert(int index, const wchar_t* wsz)
{
...
bool MYON_wString::IsValidIntegerNumber(void) const
{
  const auto length = Header()->string_length;
  return ::IsValidIntegerNumber(m_s, length);
}

bool MYON_wString::IsValidRealNumber(void) const
{
  const auto length = Header()->string_length;
...
bool MYON_wString::IsValid2dPoint() const
{
  const auto length = Header()->string_length;
  return IsCommaDelimitedDoubleArray(m_s, length, 2);
}

bool MYON_wString::IsValid3dPoint() const
{
  const auto length = Header()->string_length;
  return IsCommaDelimitedDoubleArray(m_s, length, 3);
}

bool MYON_wString::IsValid4dPoint() const
{
  const auto length = Header()->string_length;
  return IsCommaDelimitedDoubleArray(m_s, length, 4);
}

bool MYON_wString::IsValidMatrix() const
{
  const auto length = Header()->string_length;
  return IsCommaDelimitedDoubleArray(m_s, length, 16);
}

bool MYON_wString::StartsWith(const wchar_t* wszSub) const
{
...
bool MYON_wString::StartsWithNoCase(const wchar_t* wszSub) const
{
  const auto lenSub = wcslen(wszSub);
  if (0 == lenSub)
    return false;
