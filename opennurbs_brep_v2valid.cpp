...
bool MYON_Brep::IsValidForV2( const MYON_BrepTrim& trim ) const
{
  ...
  if ( trim.Domain() != trim.ProxyCurveDomain() )
    return false;
  const MYON_Curve * curve = trim.TrimCurveOf();
  ...
  if (    nurbs_curve->m_cv_count >= 4 
       && 0 == MYON_ComparePoint( nurbs_curve->m_dim, nurbs_curve->m_is_rat, nurbs_curve->m_cv, nurbs_curve->CV(nurbs_curve->m_cv_count-1) ) 
       )
  {
    ...
    {
      const MYON_BrepLoop* loop = Loop(trim.m_li);
...
bool MYON_Brep::IsValidForV2( const MYON_BrepEdge& edge ) const
{
  ...
  if ( edge.Domain() != edge.ProxyCurveDomain() )
    return false;
  const MYON_Curve * curve = edge.EdgeCurveOf();
...
bool MYON_Brep::IsValidForV2() const
{
  ...
  if ( rc )
  {
    ...
    for ( c2i = 0; c2i < c2_count; c2i++ )
    {
      ...
      if ( !MYON_NurbsCurve::Cast(m_C2[c2i]) )
        return false;
    }
    ...
    {
      // v2 3dm files expect NURBS curves
      if ( !MYON_NurbsCurve::Cast(m_C3[c3i]) )
        return false;
    }
    ...
    for ( si = 0; si < s_count; si++ )
    {
      ...
      if ( !MYON_NurbsSurface::Cast(m_S[si]) )
        return false;
    }
    ...
    for ( li = 0; li < loop_count; li++ )
    {
      ...
      for ( lti = 0; lti < loop_trim_count; lti++ )
      {
        ...
        if ( P0.DistanceTo(P1) > MYON_ZERO_TOLERANCE )
          return false;
      }
    }
  }
