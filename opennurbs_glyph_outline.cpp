...
unsigned int MYON_Outline::FigureCount() const
{
  return m_figures.UnsignedCount();
}

const MYON_ClassArray< MYON_OutlineFigure >& MYON_Outline::Figures() const
{
  return m_figures;
}

const MYON_OutlineFigure& MYON_Outline::Figure(
int i
) const
{
  return (i >= 0 && i < m_figures.Count()) ? m_figures[i] : MYON_OutlineFigure::Unset;
}

unsigned int MYON_OutlineAccumulator::CurrentFigurePointCount() const
{
  return m_point_accumulator.UnsignedCount();
}

unsigned int MYON_Outline::OutlinePointCount() const
{
...
unsigned int MYON_OutlineAccumulator::ErrorCount() const
{
  return m_error_count;
}
  
const MYON_BoundingBox MYON_Outline::OutlineBoundingBox() const
{
  while (0 == m_bbox_status)
  {
    m_bbox_status = 7;
    m_bbox = MYON_BoundingBox::NanBoundingBox;

    const unsigned int figure_count = m_figures.UnsignedCount();
    if (0 == figure_count)
      break;

    MYON_BoundingBox bbox = m_figures[0].BoundingBox();
...
double MYON_Outline::AreaEstimate() const
{
  if (
    MYON_OutlineFigure::Type::SingleStroke == m_figure_type
    || MYON_OutlineFigure::Type::DoubleStroke == m_figure_type
    )
  {
    return 0.0;
  }
  ...
  const MYON__UINT32 figure_count = m_figures.UnsignedCount();
  for (MYON__UINT32 i = 0; i < figure_count; i++)
  {
...
const bool Internal_FigureBoxesAreDisjoint(
  const MYON_BoundingBox& a,
  const MYON_BoundingBox& b
)
{
...
static int Internal_CompareAreaEstimate(MYON_OutlineFigure* const* lhs, MYON_OutlineFigure* const* rhs)
{
...
void MYON_Outline::SortFigures(
  MYON_OutlineFigure::Orientation outer_loop_orientation
)
{
  if (
    MYON_OutlineFigure::Orientation::Clockwise != outer_loop_orientation
    && MYON_OutlineFigure::Orientation::CounterClockwise != outer_loop_orientation
    )
    return;
  ...
  if (MYON_OutlineFigure::Orientation::Error == m_sorted_figure_outer_orientation)
    return; // not sortable
  ...
  const MYON_OutlineFigure::Orientation inner_loop_orientation
    = (MYON_OutlineFigure::Orientation::Clockwise == outer_loop_orientation)
    ? MYON_OutlineFigure::Orientation::CounterClockwise
    : MYON_OutlineFigure::Orientation::Clockwise;

  MYON_SimpleArray<MYON_OutlineFigure*> sorted_figures(figure_count);
  MYON_SimpleArray<MYON_OutlineFigure*> ignored_figures(figure_count);

  for (int i = 0; i < figure_count; i++)
  {
    MYON_OutlineFigure* ptr = &m_figures[i];
    if (
      (MYON_OutlineFigure::Type::Perimeter == ptr->FigureType()
        || MYON_OutlineFigure::Type::Unknown == ptr->FigureType()
        || MYON_OutlineFigure::Type::Unset == ptr->FigureType())
      )
    {
      const double figure_area = ptr->AreaEstimate();
      const MYON_BoundingBox figure_bbox = ptr->BoundingBox();
      const MYON_OutlineFigure::Orientation figure_orientation = ptr->FigureOrientation();

      if (
        0.0 != figure_area && MYON_IsValid(figure_area)
      ...
      // used in meshing.
      ptr->m_figure_type = MYON_OutlineFigure::Type::NotPerimeter;
    }
  ...
  MYON_SimpleArray<MYON_OutlineFigure*> outer_figures(sorted_figure_count);
  MYON_SimpleArray<MYON_OutlineFigure*> inner_figures(sorted_figure_count);
  MYON_SimpleArray<MYON_OutlineFigure*> inner_figures_parent(sorted_figure_count);

  for (  unsigned k = 0; k < sorted_figure_count; ++k)
  {
    MYON_OutlineFigure* f = sorted_figures[k];
  ...
  if ( 0 == outer_count )
  {
    m_sorted_figure_outer_orientation = MYON_OutlineFigure::Orientation::Error;
    return;
  }
  ...
  {
    MYON_OutlineFigure* outer_figure = outer_figures[i];
    ...
    for (unsigned j = 0; j < inner_count; ++j)
    {
      ...
      {
        MYON_OutlineFigure* inner_figure = inner_figures[j];
  ...
  {
    MYON_OutlineFigure* f = ignored_figures[k];
    if (nullptr != f)
  ...
  if (sorted_figures.Count() == figure_count)
  {
    for (int k = 0; k < figure_count; k++)
    {
      if (sorted_figures[k] != &m_figures[k])
      {
        ...
        MYON_ClassArray<MYON_OutlineFigure> tmp(figure_count);
        for (int i = 0; i < figure_count; i++)
        {
...
MYON_OutlineFigure::Orientation MYON_Outline::SortedFigureOuterOrientation() const
{
  return m_sorted_figure_outer_orientation;
}

MYON_OutlineFigure::Orientation MYON_Outline::SortedFigureInnerOrientation() const
{
  const MYON_OutlineFigure::Orientation outer_orientation = SortedFigureOuterOrientation();
  if (MYON_OutlineFigure::Orientation::CounterClockwise == outer_orientation)
    return MYON_OutlineFigure::Orientation::Clockwise;
  if (MYON_OutlineFigure::Orientation::Clockwise == outer_orientation)
    return MYON_OutlineFigure::Orientation::CounterClockwise;

  return outer_orientation;
}


const MYON_TextBox MYON_Outline::GlyphMetrics() const
{
  return m_glyph_metrics;
}

const MYON_OutlineFigurePoint MYON_OutlineAccumulator::CurrentFigureStartPoint() const
{
  return m_figure_start;
}
  
const MYON_OutlineFigurePoint MYON_OutlineAccumulator::CurrentFigurePreviousPoint() const
{
  return m_figure_prev;
}
  
const MYON_OutlineFigurePoint MYON_OutlineAccumulator::CurrentFigurePoint() const
{
  return m_figure_current;
}


const bool MYON_OutlineAccumulator::CurrentFigureAccumulating() const
{
  return Internal_InFigure();
}

MYON__UINT32 MYON_OutlineAccumulator::UnitsPerEM() const
{
  return m_units_per_em;
}

MYON__UINT32 MYON_Outline::UnitsPerEM() const
{
  return m_units_per_em;
}

void MYON_Outline::SetUnitsPerEM(
...
MYON_OutlineFigure::Type MYON_OutlineAccumulator::FigureType() const
{
  return m_figure_type;
}

bool MYON_OutlineAccumulator::IsInitialized() const
{
  return (1 == m_status);
}

bool MYON_OutlineAccumulator::IsFinalized() const
{
  return (2 == m_status);
}

bool MYON_OutlineAccumulator::IsInitializedOrFinalized() const
{
  return IsInitialized() || IsFinalized();
}

MYON_Outline* MYON_OutlineAccumulator::HarvestOutline()
{  
  MYON_Outline* outline = m_outline;
  m_outline = nullptr;
...
void MYON_OutlineAccumulator::Clear()
{
  ...
  if ( nullptr != m_outline)
    *m_outline = MYON_Outline::Unset;

  m_units_per_em = 0;
  m_status = 0;
  m_figure_type = MYON_OutlineFigure::Type::Unset;
  m_figure_depth = 0;
  m_error_count = 0;
...
bool MYON_OutlineAccumulator::BeginGlyphOutline(
...
)
{
  ...
  {
    m_managed_outline = new MYON_Outline();
    m_outline = m_managed_outline;
  }
  else
  {
    *destination_outline = MYON_Outline::Unset;
...
bool MYON_OutlineFigure::Internal_HasValidEnds(
  bool bLogErrors
) const
{
  ...
  if (bLogErrors)
  {
    MYON_ERROR("Invalid figure start or end information.");
  }
  return false;
}

static void Internal_GrowBBox(
  const MYON_2fPoint& pmin,
  const MYON_2fPoint& pmax,
  MYON_2fPoint& bbox_min,
  MYON_2fPoint& bbox_max
)
{
...
const MYON_BoundingBox MYON_OutlineFigure::BoundingBox() const
{
  while (0 == m_bbox_status)
  {
    m_bbox_status = 7;

    const MYON__UINT32 figure_end_dex = Internal_FigureEndDex(false);
    if (0 == figure_end_dex)
      break;

    const MYON_OutlineFigurePoint* a = m_points.Array();
    MYON_2fPoint bbox_min = a[0].m_point;
    MYON_2fPoint bbox_max = bbox_min;
    MYON_2fPoint bbox_cv_min = bbox_min;
    MYON_2fPoint bbox_cv_max = bbox_max;
    
    for (MYON__UINT32 i = 0U; i < figure_end_dex; ++i)
    {
      const MYON__UINT32 degree = MYON_OutlineFigure::Internal_SegmentDegree(i);
      if (degree < 1U || degree > 3U)
        continue;
      // a[i] = start of bezier (line,quadratic,or cubic)
      MYON_OutlineFigurePoint p = a[i+degree]; // p = a[i+degree] = end of bezier (line,quadratic,or cubic)
    ...
    for (MYON__UINT32 i = 0U; i < figure_end_dex; ++i)
    {
      ...
      MYON_BezierCurve bez(3, false, degree + 1);
      for (MYON__UINT32 j = 0; j <= degree; j++)
        bez.SetCV(j, MYON_3dPoint(cv[j].m_point.x, cv[j].m_point.y, 0.0));

      const MYON_BoundingBox bez_bbox = bez.BoundingBox();
      if (bez_bbox.IsNotEmpty())
      {
        const double minf = MYON_UNSET_FLOAT;
        const double maxf = MYON_UNSET_POSITIVE_FLOAT;
        if (
    ...
    {
      if (
        MYON_OutlineFigure::Type::Perimeter == m_figure_type
        || MYON_OutlineFigure::Type::Unset == m_figure_type
        || MYON_OutlineFigure::Type::Unknown == m_figure_type
        )
      {
        m_figure_type = MYON_OutlineFigure::Type::NotPerimeter;
      }
    }
    break;
  }
  ...
  if (1 == m_bbox_status)
    return MYON_BoundingBox(MYON_3dPoint(m_bbox_min), MYON_3dPoint(m_bbox_max));

  return MYON_BoundingBox::NanBoundingBox;
}

bool MYON_OutlineFigure::Internal_NegateY( MYON_2fPoint& p )
{
  if (0.0 != p.y && p.y > MYON_UNSET_FLOAT && p.y < MYON_UNSET_POSITIVE_FLOAT)
  {
    p.y = -p.y;
    return true;
  }
...
bool MYON_OutlineFigure::NegateY()
{
  bool rc = false;
  const MYON__UINT32 count = m_points.UnsignedCount();
  MYON_OutlineFigurePoint* a = m_points.Array();
  for (MYON__UINT32 i = 0; i < count; i++)
  {
    if ( MYON_OutlineFigure::Internal_NegateY(a[i].m_point) )
      rc = true;
  }

  if (rc)
  {
    if (MYON_OutlineFigure::Orientation::Clockwise == m_orientation)
      m_orientation = MYON_OutlineFigure::Orientation::CounterClockwise;
    else if (MYON_OutlineFigure::Orientation::CounterClockwise == m_orientation)
      m_orientation = MYON_OutlineFigure::Orientation::Clockwise;

    if (MYON_IsValid(m_area_estimate) && 0.0 != m_area_estimate)
...
static double Internal_DeltaArea(
  const MYON_2dPoint& p0,
  const MYON_2dPoint& p1
)
{
  return (p0.x - p1.x)*(p0.y + p1.y);
}

MYON_OutlineFigure::Type MYON_OutlineFigure::FigureType() const
{
  return m_figure_type;
}


MYON__UINT16 MYON_OutlineFigure::FigureIndex() const
{
  return m_figure_index;
}

double MYON_OutlineFigure::BoxArea() const
{
  const MYON_BoundingBox bbox = BoundingBox();
...
unsigned MYON_OutlineFigure::GetUpToFourPointsOnFigure(MYON_2fPoint p[4]) const
{
  if (nullptr == p)
    return 0;
  
  unsigned point_count = 0;
  const MYON__UINT32 end_dex = Internal_FigureEndDex(false);
  if (end_dex > 0)
  {
    // p[0] = point at the start
    const MYON_2fPoint p0 = this->m_points[0].m_point;
    p[point_count++] = p0;


    // p[1] = point between start and middle
    MYON_2fPoint prev_p = p0;
    unsigned i1 = end_dex / 2;
    if (i1 >= 2)
    {
      for (MYON__UINT32 i = i1 - 1; i > 0; --i)
      {
        if (false == this->m_points[i].IsOnFigure())
          continue;
        const MYON_2fPoint candidate_p = this->m_points[i].m_point;
    ...
    for (/*empty init*/; i1 <= end_dex; ++i1)
    {
      if (false == this->m_points[i1].IsOnFigure())
        continue;
      const MYON_2fPoint candidate_p = this->m_points[i1].m_point;
      if (prev_p != candidate_p)
      {
        prev_p = candidate_p;
        p[point_count++] = candidate_p;
    ...
    for (MYON__UINT32 i = end_dex; i > i1; --i)
    {
      if (false == this->m_points[i].IsOnFigure())
        continue;
      const MYON_2fPoint candidate_p = this->m_points[i].m_point;
...
)
{
  ...
  MYON_2dPoint Amin;
  MYON_2dPoint Amax;
  MYON_2dPoint A[2] = { MYON_2dPoint::NanPoint,a[0] };
  MYON_2dPoint B[2] = { MYON_2dPoint::NanPoint,MYON_2dPoint::NanPoint };
  double alpha[3] = { MYON_DBL_QNAN, MYON_DBL_QNAN, MYON_DBL_QNAN };
...
bool MYON_OutlineFigure::IsInsideOf(
  const MYON_OutlineFigure* outer_figure,
...
) const
{
  ...
  const MYON_OutlineFigure::Orientation outer_orientation = outer_figure->FigureOrientation();
  if (MYON_OutlineFigure::Orientation::Clockwise != outer_orientation && MYON_OutlineFigure::Orientation::CounterClockwise != outer_orientation)
    return false;


  const MYON_BoundingBox this_bbox = this->BoundingBox();
  const MYON_BoundingBox outer_bbox = outer_figure->BoundingBox();
  ...
  const int outer_orientation_sign = MYON_OutlineFigure::Orientation::CounterClockwise == outer_orientation ? 1 : -1;

  MYON_2fPoint this_p[4];
  ...
  const MYON_OutlineFigure::Orientation this_orientation = this->FigureOrientation();
...
MYON__UINT32 MYON_OutlineFigure::UnitsPerEM() const
{
  return m_units_per_em;
}

double MYON_OutlineFigure::AreaEstimate() const
{
  while (0 == m_area_status)
  {
    m_area_estimate = MYON_DBL_QNAN;
    m_area_status = 7;
    ...
    const MYON_OutlineFigurePoint figure_start = a[0];
    const MYON_OutlineFigurePoint figure_end = a[figure_end_dex];

    if (
      MYON_OutlineFigurePoint::Type::BeginFigureOpen == figure_start.m_point_type
      || MYON_OutlineFigurePoint::Type::EndFigureOpen == figure_end.m_point_type
      || figure_end_dex <= 2
      )
    {
      m_area_estimate = 0.0;
    ...
    MYON_2dPoint cv[4], p0, p1;

    double quad[3][3];
    double cubic[3][4];

    quad[0][0] = MYON_UNSET_VALUE;
    cubic[0][0] = MYON_UNSET_VALUE;
    ...
    {
      const MYON_OutlineFigurePoint p = a[i];
      p0 = p1;
      p1 = p.m_point;

      if (MYON_OutlineFigurePoint::Type::LineTo == p.m_point_type)
      {
        twice_area += Internal_DeltaArea(p0, p1);
        continue;
      }

      if (MYON_OutlineFigurePoint::Type::QuadraticBezierPoint == p.m_point_type)
      {
        if (
          p.IsOffFigure()
          && i + 1 < figure_end_dex
          ...
          )
        {
          ...
          if (MYON_UNSET_VALUE == quad[0][0])
          {
            for (int tdex = 1; tdex <= 3; tdex++)
            {
              ...
              {
                quad[tdex - 1][cvdex] = MYON_EvaluateBernsteinBasis(2, cvdex, t);
              }
            }
          }
          cv[0] = p0;
      ...
      if (MYON_OutlineFigurePoint::Type::CubicBezierPoint == p.m_point_type)
      {
        if (
          ...
          )
        {
          if (MYON_UNSET_VALUE == cubic[0][0])
          {
            for (int tdex = 1; tdex <= 3; tdex++)
            {
              ...
              {
                cubic[tdex - 1][cvdex] = MYON_EvaluateBernsteinBasis(3, cvdex, t);
              }
            }
          }
          cv[0] = p0;
    ...
    if (i != figure_end_dex)
    {
      MYON_ERROR("Area calculation failed.");
      if (
        MYON_OutlineFigure::Type::Perimeter == m_figure_type
        || MYON_OutlineFigure::Type::Unset == m_figure_type
        || MYON_OutlineFigure::Type::Unknown == m_figure_type
...
bool MYON_OutlineFigure::ReverseFigure()
{
  const bool error_rc = false;

  const MYON__UINT32 figure_end_dex = Internal_FigureEndDex(true);
  if (0 == figure_end_dex)
    return error_rc;

  const MYON__UINT32 figure_start_dex = 0;

  MYON_OutlineFigurePoint* a = m_points.Array();

  // Original beginning point
  const MYON_OutlineFigurePoint B0 = a[figure_start_dex];

  // Original ending point
  const MYON_OutlineFigurePoint E0 = a[figure_end_dex];

  for (MYON__UINT32 i = figure_start_dex+1; i < figure_end_dex; i++)
  {
  ...
  // New beginning point
  MYON_OutlineFigurePoint B1 = a[figure_end_dex-1];
  B1.m_point_type = B0.m_point_type;


  // New ending point
  MYON_OutlineFigurePoint E1 = B1;
  ...
  // reverse a[] order
  MYON__UINT32 j = figure_end_dex-1;
  for (MYON__UINT32 i = figure_start_dex; i < j; i++, j--)
  {
    MYON_OutlineFigurePoint x = a[i];
    a[i] = a[j];
    a[j] = x;
  }
...
unsigned int MYON_OutlineFigure::Internal_SegmentDegree(
...
) const
{
  ...
  const MYON_OutlineFigurePoint p0 = a[segment_start_dex];
  if (false == p0.IsOnFigure())
    return 0;

  const MYON_OutlineFigurePoint p1 = a[segment_start_dex+1];

  switch (p1.m_point_proximity)
  {
  case MYON_OutlineFigurePoint::Proximity::OnFigure:
    if (MYON_OutlineFigurePoint::Type::LineTo == p1.m_point_type || p1.IsEndFigurePoint())
      return 1;
    break;

  case MYON_OutlineFigurePoint::Proximity::OffFigure:
    if (segment_start_dex + 2 < count)
    {
      const MYON_OutlineFigurePoint p2 = a[segment_start_dex + 2];
      if (p2.m_point_type != p1.m_point_type || p2.m_figure_index != p1.m_figure_index)
        break;

      if (MYON_OutlineFigurePoint::Type::QuadraticBezierPoint == p1.m_point_type)
      {
        if (p2.IsOnFigure())
          return 2;
      }
      else if (MYON_OutlineFigurePoint::Type::CubicBezierPoint == p1.m_point_type)
      {
        if (false == p2.IsOffFigure())
          break;
        if (segment_start_dex + 3 >= count)
          break;
        const MYON_OutlineFigurePoint p3 = a[segment_start_dex + 3];
...
MYON_OutlineFigure::Orientation MYON_OutlineFigure::FigureOrientation() const
{
  if (MYON_OutlineFigure::Orientation::Unset == m_orientation)
  {
    ...
    if (fabs(area_estimate) <= area_tolerance)
      m_orientation = MYON_OutlineFigure::Orientation::NotOriented;
    else if (area_estimate > 0.0)
      m_orientation = MYON_OutlineFigure::Orientation::CounterClockwise;
    else if (area_estimate < 0.0)
      m_orientation = MYON_OutlineFigure::Orientation::Clockwise;
    else
      m_orientation = MYON_OutlineFigure::Orientation::Error;
  }
  return m_orientation;
}

unsigned int MYON_OutlineFigure::GetFigureCurves(
  double scale,
  bool b3d,
  MYON_SimpleArray< MYON_Curve* >& figure_curves
  ) const
{
  MYON_SimpleArray< MYON_NurbsCurve* >& figure_nurbs_curves = *((MYON_SimpleArray< MYON_NurbsCurve* > *)&figure_curves);
...
) const
{
  ...
  const bool bApplyScale = MYON_IsValid(scale) && scale > 0.0 && fabs(scale - 1.0) > 1.0e-5;
  if (false == bApplyScale)
    scale = 1.0;

  const MYON__UINT32 figure_start_dex = 0;
  const MYON__UINT32 figure_end_dex = m_points.UnsignedCount() - 1;

  //const MYON_OutlineFigurePoint figure_start = m_points[figure_start_dex];
  //const MYON_OutlineFigurePoint figure_end = m_points[figure_end_dex];
  ...
  MYON__UINT32 segment_start_dex = figure_start_dex;
  while (segment_start_dex < figure_end_dex)
  {
    MYON__UINT32 i = 0;
    MYON_NurbsCurve* figures_segment = Internal_GetFigureCurve(
      figure_end_dex,
      segment_start_dex,
      &i,
      b3d,
    ...
    {
      MYON_ERROR("Failed to parse a figure segment.");
      //bClosedFigure = false;
      break;
    }
...
void MYON_Outline::Internal_ClearCachedValues() const
{
  m_bbox_status = 0;
  m_bbox = MYON_BoundingBox::NanBoundingBox;
}

MYON__UINT16 MYON_Outline::AppendFigure(
...
void MYON_Outline::SetGlyphMetrics(
...
MYON__UINT16 MYON_Outline::Internal_AppendFigure(
...
)
{
  ...
  {
    MYON_ERROR("Invalid point_count or points[] array.");
    return 0;
  }

  const MYON_OutlineFigurePoint figure_start = points[0];
  if (
  ...
  {
    MYON_ERROR("points[0] is not a valid BeginFigure point.");
    return 0;
  }

  const MYON_OutlineFigurePoint figure_end = points[point_count-1];
  if (
  ...
  {
    MYON_ERROR("points[point_count-1] is not a valid EndFigure point.");
    return 0;
  }
  ...
  const bool bDoubleStrokeReductionTest
    = MYON_OutlineFigure::Type::DoubleStroke == m_figure_type
    && point_count >= 3
    && MYON_OutlineFigurePoint::Type::BeginFigureClosed == figure_start.PointType()
    && MYON_OutlineFigurePoint::Type::EndFigureClosed == figure_end.PointType()
    && figure_start.m_point == figure_end.m_point;
  ...
  {
    MYON_OutlineFigure& prev_figure = m_figures[m_figures.UnsignedCount() - 1];
    if (MYON_OutlineFigure::Type::DoubleStroke != prev_figure.m_figure_type)
      break;
    if (point_count != (prev_figure.m_points.UnsignedCount()))
      break;
    const MYON_OutlineFigurePoint prev_figure_start = prev_figure.m_points[0];
    const MYON_OutlineFigurePoint prev_figure_end = prev_figure.m_points[point_count-1];
    if (MYON_OutlineFigurePoint::Type::BeginFigureClosed != prev_figure_start.PointType())
      break;
    if (MYON_OutlineFigurePoint::Type::EndFigureClosed != prev_figure_end.PointType())
      break;
    ...
    for (int dup_check_pass = 0; dup_check_pass < 2; dup_check_pass++)
    {
      ...
      for (unsigned int i = 0; i < point_count; i++)
      {
        ...
        const MYON_OutlineFigurePoint& prev_point = prev_figure.m_points[j];
        bDuplicateLoop
      ...
      prev_figure.m_points[0].m_point_type = MYON_OutlineFigurePoint::Type::BeginFigureOpen;
      if (
        prev_figure.m_points[point_count - 2].IsOnFigure()
        && false == (prev_figure_end.m_point == prev_figure.m_points[point_count - 2].m_point)
        )
      {
        MYON_OutlineFigurePoint& new_figure_end = prev_figure.m_points.AppendNew();
        MYON_OutlineFigurePoint& p = prev_figure.m_points[prev_figure.m_points.UnsignedCount() - 2];
        p.m_point_type = MYON_OutlineFigurePoint::Type::LineTo;
        new_figure_end = p;
        new_figure_end.m_point_type = MYON_OutlineFigurePoint::Type::EndFigureOpen;
      }
      prev_figure.m_figure_type = MYON_OutlineFigure::Type::SingleStroke;
      return prev_figure.FigureIndex();
    }
    break;
  }
  
  MYON_OutlineFigure& figure = m_figures.AppendNew();
  figure.m_figure_index = (MYON__UINT16)m_figures.UnsignedCount();
  ...
  while (bDoubleStrokeReductionTest)
  {
    ...
    MYON_2fPoint Qpoint(MYON_2fPoint::NanPoint);
    ...
    if (i1 - 2 > 0)
    {
        ...
        )
      {
        ...
        && figure.m_points[i1 - 1].IsOnFigure()
        && MYON_OutlineFigurePoint::Type::LineTo == figure.m_points[i1 - 1].PointType()
        )
      {
    ...
    if ( bPrependQseg )
    {
      ...
      figure.m_points[1].m_point_type = MYON_OutlineFigurePoint::Type::LineTo;
      i1++;
    }
    figure.m_points[0].m_point_type = MYON_OutlineFigurePoint::Type::BeginFigureOpen;
    figure.m_points[i1 + 1].m_point = figure.m_points[i1].m_point;
    figure.m_points[i1 + 1].m_point_type = MYON_OutlineFigurePoint::Type::EndFigureOpen;
    figure.m_points[i1 + 1].m_point_proximity = MYON_OutlineFigurePoint::Proximity::OnFigure;
    figure.m_figure_type = MYON_OutlineFigure::Type::SingleStroke;
    figure.m_points.SetCount(i1 + 2);

    break;
  }

  m_sorted_figure_outer_orientation = MYON_OutlineFigure::Orientation::Unset;
  return figure.m_figure_index;
}


MYON_OutlineFigure::Type MYON_Outline::FigureType() const
{
  return m_figure_type;
}

unsigned int MYON_Outline::GetOutlineCurves(
    double scale,
    bool b3d,
  ...
  const
{
  const MYON__UINT32 outline_curves_count0 = outline_curves.UnsignedCount();
  ...
  for ( MYON__UINT32 i = 0; i <figure_count; i++ )
  {
    MYON_SimpleArray< MYON_Curve* >& figure_curves = outline_curves.AppendNew();
...
MYON__UINT32 MYON_OutlineFigure::Internal_FigureEndDex(
  bool bLogErrors
  ) const
{
  const MYON__UINT32 count = m_points.UnsignedCount();
  ...
  if (bLogErrors)
  {
    MYON_ERROR("Figure is missing start or end point.");    
  }
  return 0;
}

MYON__UINT32 MYON_OutlineFigure::Internal_EstimateFigureSegmentCount() const
{
  MYON__UINT32 figure_end_dex = Internal_FigureEndDex(false);
  const MYON__UINT32 count = m_points.UnsignedCount();
  if (0 == figure_end_dex || figure_end_dex >= count)
    return 0;

  MYON_OutlineFigurePoint::Type point_type = MYON_OutlineFigurePoint::Type::Unset;
  const MYON_OutlineFigurePoint* a = m_points.Array();
  const MYON_OutlineFigurePoint figure_end = a[figure_end_dex];
  if (
    figure_end.IsOnFigure()
    && MYON_OutlineFigurePoint::Type::EndFigureClosed == figure_end.m_point_type
  ...
  int segment_count = 0;
  for (MYON__UINT32 i = 1; i <= figure_end_dex; i++)
  {
...
class MYON_NurbsCurve* MYON_OutlineFigure::Internal_GetFigureCurve(
...
{
  MYON_NurbsCurve* nurbs_curve = destination_curve;
  ...
  *segment_end_dex = MYON_UNSET_UINT_INDEX;
  ...
  {
    MYON_ERROR("Invalid start and end indices.");
    return nullptr;
  }
  ...
  const MYON_OutlineFigurePoint segment_start = a[segment_start_dex];

  if (false == segment_start.IsOnFigure())
  {
    MYON_ERROR("m_outline_points[segment_start_dex].m_bToPoint is false.");
    return nullptr;
  }

  if (0 == segment_start.m_figure_index)
  {
    MYON_ERROR("m_outline_points[segment_start_dex].m_contour_index is zero.");
    return nullptr;
  }
  ...
  if (0 == degree)
  {
    MYON_ERROR("m_outline_points[segment_start_dex] or nearby subsequent points are non valid.");
    return nullptr;
  }

  MYON__UINT32 i1 = segment_start_dex + degree; 
  ...
  *segment_end_dex = i1;
  ...
  if (cv_count < order)
  {
    MYON_ERROR("Bug in this function.");
    return nullptr;
  }
  ...
  // Create NURBS curve
  if (nullptr == nurbs_curve)
    nurbs_curve = new MYON_NurbsCurve(dim, false, order, cv_count);
  else
  {
  ...
  nurbs_curve->m_cv_count = 0;
  MYON_3dPoint prev_cv = MYON_3dPoint::NanPoint;
  int cv_dex = 0;
  MYON__UINT32 i;
  for( i = segment_start_dex; i <= i1 && cv_dex < cv_count; i++)
  {
    const MYON_3dPoint cv(a[i].m_point);
...
public:
  ...
  MYON_2fPoint m_prev_point = MYON_2fPoint::NanPoint;
  unsigned int m_point_count = 0;

  void AddPoint(MYON_2fPoint pt)
  {
  ...
  void AddQuadraticBezier(MYON_2fPoint bez_cv[3])
  {
    Internal_OutlineFigureToPolyline::Internal_AddBezier(0, 3, bez_cv);
  }

  void AddCubicBezier(MYON_2fPoint bez_cv[4])
  {
  ...
  )
{
  ...
  float f14[14];
  MYON_2fPoint* cv  = (MYON_2fPoint*)f14;
...
double MYON_OutlineFigure::DefaultPolylineTolerance(
  double units_per_em
)
{
  const double default_polyline_tolerance 
    = (units_per_em > 0.0 && units_per_em < MYON_UNSET_POSITIVE_VALUE && units_per_em != MYON_UNSET_FLOAT)
    ? (units_per_em  / 1024.0) 
    : 1.0;
  return default_polyline_tolerance;
}

double MYON_OutlineFigure::DefaultPolylineTolerance() const
{
  return MYON_OutlineFigure::DefaultPolylineTolerance(m_units_per_em);
}

unsigned int MYON_OutlineFigure::GetPolyline(
  double tolerance, 
...
) const
{
  ...
  for (unsigned int i = 0; i < c; i++)
  {
    ...
    {
      if (
        MYON_OutlineFigurePoint::Type::QuadraticBezierPoint == a[i].m_point_type
        && i + 1 < c
        && MYON_OutlineFigurePoint::Type::QuadraticBezierPoint == a[i + 1].m_point_type
        )
      {
      ...
      if (
        MYON_OutlineFigurePoint::Type::CubicBezierPoint == a[i].m_point_type
        && i + 2 < c
        && MYON_OutlineFigurePoint::Type::CubicBezierPoint == a[i + 1].m_point_type
        && MYON_OutlineFigurePoint::Type::CubicBezierPoint == a[i + 2].m_point_type
        )
      {
        bez_cv[0] = tp.m_prev_point;
        bez_cv[1] = a[i++].m_point;
...
static void MYON_Internal_FigureWindingNumberCallback(float x, float y, void *context)
{
  ((MYON_WindingNumber*)context)->AddBoundary(MYON_2dPoint(x, y));
}

int MYON_OutlineFigure::WindingNumber(
...
) const
{
  ...
  MYON_WindingNumber wn;
  wn.SetWindingPoint(winding_point.x,winding_point.y);

  Internal_OutlineFigureToPolyline tp;
  tp.m_PointCallbackFunc = MYON_Internal_FigureWindingNumberCallback;
  tp.m_context = &wn;
  tp.m_2x_tolerance = 2.0;

  MYON_2fPoint bez_cv[4];

  MYON_2fPoint bbox_min, bbox_max;
        ...
        )
      {
        ...
        if (bbox.m_min.y <= winding_point.y && winding_point.y <= bbox.m_max.y)
        {
          if (bbox.m_min.x <= winding_point.x && winding_point.x <= bbox.m_max.x)
            ...
            wn.AddBoundary(MYON_2dPoint(tp.m_prev_point), MYON_2dPoint(a[i].m_point));
        }
...
bool  MYON_Outline::IsValidOutline(
  bool bLogErrors
) const
{
  const MYON__UINT32 figure_count = m_figures.UnsignedCount();
  if (figure_count <= 0)
  {
    if (bLogErrors) MYON_ERROR("No figures in the outline");
    return false;
  }
...
bool MYON_OutlineAccumulator::EndOutline()
{
  return EndOutline(false, MYON_Outline::DefaultOuterOrientation);
}
...
)
{
  ...
  MYON_Outline& outline = Internal_Outline();

  if (bNegatePointY)
  {
    const MYON__UINT32 figure_count = outline.m_figures.UnsignedCount();
    for (MYON__UINT32 i = 0; i < figure_count; i++)
    ...
    )
  {
    const double desired_area_sign =
      MYON_OutlineFigure::Orientation::CounterClockwise == orientation
      ? 1.0
      : (MYON_OutlineFigure::Orientation::Clockwise == orientation ? -1.0 : 0.0);
...
const MYON_OutlineFigurePoint MYON_OutlineAccumulator::ActiveFigureStartPoint() const
{
  return (m_figure_depth > 0) ? m_figure_start : MYON_OutlineFigurePoint::Unset;
}

const MYON_OutlineFigurePoint MYON_OutlineAccumulator::ActiveFigureCurrentPoint() const
{
  return (m_figure_depth > 0) ? m_figure_current : MYON_OutlineFigurePoint::Unset;
}


bool MYON_OutlineAccumulator::BeginFigure(
...
)
{
  if (1 != m_status)
  {
    MYON_ERROR("MYON_OutlineAccumulator is not initialized.");
    Internal_AccumulateError(true);
    return false;
  }

  if ( false == MYON_OutlineFigurePoint::IsBeginFigurePointType(point_type) )
  {
    MYON_ERROR("Invalid point_type for BeginFigure2f.");
    Internal_AccumulateError(true);
    return false;
  }
...
bool MYON_OutlineAccumulator::AppendLine(
  MYON_2fPoint point_location
)
{
  if (false == Internal_InFigure())
  {
    MYON_ERROR("Not continuing a figure.");
    Internal_AccumulateError(true);
    return false;
  }
  ...
  {
    MYON_ERROR("Invalid start of line segment.");
    Internal_AccumulateError(true);
    return false;
  }
...
bool MYON_OutlineAccumulator::AppendQuadraticBezier(
  MYON_2fPoint cv1,
  MYON_2fPoint cv2
)
{
  ...
  if (false == m_figure_current.IsOnFigure())
  {
    MYON_ERROR("Invalid start of quadratic bezier segment.");
    Internal_AccumulateError(true);
    return false;
  }

  const MYON_2fPoint cv0 = m_figure_current.m_point;
  ...
  if (cv0 == cv2)
  {
    ...
    //MYON_ERROR("Degenerate quadratic bezier segment.");
    return false;
  }

  const MYON_OutlineFigurePoint::Type point_type = MYON_OutlineFigurePoint::Type::QuadraticBezierPoint;

  const MYON__UINT32 count0 = m_point_accumulator.UnsignedCount();
  const bool rc
  ...
  if (false == rc)
  {
    m_figure_current = MYON_OutlineFigurePoint::Error;
...
bool MYON_OutlineAccumulator::AppendCubicBezier(
  MYON_2fPoint cv1,
  MYON_2fPoint cv2,
  MYON_2fPoint cv3
)
{
  ...
  if (false == m_figure_current.IsOnFigure())
  {
    MYON_ERROR("Invalid beginning of a quadratic bezeir segment.");
    Internal_AccumulateError(true);
    return false;
  }
  ...
  if (cv0 == cv3)
  {
    ...
    {
      // silently skip degenerate segments
      //MYON_ERROR("Degenerate cubic bezier segment.");
      return false;
    }
  }
  ...
  const MYON_OutlineFigurePoint::Type point_type = MYON_OutlineFigurePoint::Type::CubicBezierPoint;
...
bool MYON_OutlineAccumulator::EndFigure(
  MYON_OutlineFigurePoint::Type point_type
)
{
  if ( false == MYON_OutlineFigurePoint::IsEndFigurePointType(point_type) )
  {
    MYON_ERROR("Invalid point_type for EndFigure.");
    Internal_AccumulateError(true);
    return false;
  }
  ...
  const bool rc = Internal_AccumulatePoint(point_type, MYON_2fPoint::NanPoint, false);

  return rc;
}

bool MYON_OutlineFigure::IsValidFigure(
  bool bLogErrors
) const
{
  const MYON__UINT32 figure_end_dex = Internal_FigureEndDex(bLogErrors);
  if (0 == figure_end_dex)
  {
    return false;
  }
  ...
  {
    if ( bLogErrors ) MYON_ERROR("m_points[0] is not a BeginFigure point.");
    return false;
  }
  if ( MYON_OutlineFigurePoint::Type::BeginFigureUnknown == figure_start.m_point_type)
  {
    // Finished figures never have BeginFigureUnknown point type.
    if ( bLogErrors ) MYON_ERROR("m_outline_points[figure_start_dex] is MYON_OutlineFigurePoint::Type::BeginFigureUnknown.");
    return false;
  }

  const MYON__UINT16 figure_id = figure_start.m_figure_index;
  if (0 == figure_id)
  {
    if ( bLogErrors ) MYON_ERROR("BeginFigure point has 0 = m_contour_index.");
    return false;
  }
  ...
  {
    if ( bLogErrors ) MYON_ERROR("BeginFigure and EndFigure points have diffent m_contour_index values.");
    return false;
  }

  if ( false == figure_start.IsOnFigure())
  {
    if ( bLogErrors ) MYON_ERROR("BeginFigure point IsOnFigure() is false.");
    return false;
  }

  if (false == figure_end.IsOnFigure())
  {
    if ( bLogErrors ) MYON_ERROR("EndFigure point IsOnFigure() is false.");
    return false;
  }

  const bool bClosedFigure = (MYON_OutlineFigurePoint::Type::BeginFigureOpen != figure_start.m_point_type);
  if (bClosedFigure)
  {
    if (MYON_OutlineFigurePoint::Type::EndFigureClosed != figure_end.m_point_type)
    {
      if ( bLogErrors ) MYON_ERROR("Closed figure is not terminated with an EndFigureClosed point.");
      return false;
    }
    if (false == (figure_start.m_point == figure_end.m_point))
    {
      if ( bLogErrors ) MYON_ERROR("Closed figure has different start and end points.");
      return false;
    }
  }
  else
  {
    if (MYON_OutlineFigurePoint::Type::EndFigureOpen != figure_end.m_point_type)
    {
      if ( bLogErrors ) MYON_ERROR("Open figure is not terminated with an EndFigureOpen point.");
      return false;
    }
  }
  ...
  for (MYON__UINT32 i = 1; i < figure_end_dex; i++)
  {
    ...
    {
      if (bLogErrors) MYON_ERROR("BeginFigure and interior point have different m_contour_index values.");
      return false;
    }
    
    if (MYON_OutlineFigurePoint::Type::LineTo == p1.m_point_type)
    {
      if (false == p1.IsOnFigure())
      {
        if (bLogErrors) MYON_ERROR("LineTo point IsOnFigure() is false.");
        return false;
      }
      continue;
    }
    ...
    if (MYON_OutlineFigurePoint::Type::QuadraticBezierPoint == p1.m_point_type)
    {
      ...
      {
        if (bLogErrors) MYON_ERROR("Interior QuadraticBezier point IsOffFigure() is false.");
        return false;
      }

      if (i + 1 >= figure_end_dex)
      {
        if (bLogErrors) MYON_ERROR("Invalid QuadraticBezier segment.");
        return false;
      }
      ...
      if (MYON_OutlineFigurePoint::Type::QuadraticBezierPoint != p1.m_point_type)
      {
        if (bLogErrors) MYON_ERROR("Invalid end of QuadraticBezier segment point type.");
        return false;
      }

      if (false == p1.IsOnFigure())
      {
        if (bLogErrors) MYON_ERROR("End of QuadraticBezier point IsOnFigure() is false.");
        return false;
      }
      continue;
    }

    if (MYON_OutlineFigurePoint::Type::CubicBezierPoint == p1.m_point_type)
    {
      if (false == p1.IsOffFigure())
      {
        if (bLogErrors) MYON_ERROR("Interior CubicBezierPoint point IsOffFigure() is false.");
        return false;
      }
      ...
      for (int cv_dex = 2; cv_dex <= 3; cv_dex++)
      {
        if (i + 1 >= figure_end_dex)
        {
          if (bLogErrors) MYON_ERROR("Invalid CubicBezierPoint segment.");
          return false;
        }
    ...
    if (bLogErrors) MYON_ERROR("Invalid point type for figure interior.");
    return false;
  }
...
const MYON_Outline& MYON_OutlineAccumulator::Outline() const
{
  return (nullptr == m_outline) ? *m_outline : MYON_Outline::Unset;
}

MYON_Outline& MYON_OutlineAccumulator::Internal_Outline()
{
  if (nullptr == m_outline)
  {
    m_outline = new MYON_Outline();
    m_outline->SetUnitsPerEM(m_units_per_em);
  }
  return *m_outline;
}

bool MYON_OutlineAccumulator::Internal_InFigure() const
{
  if (
    1 == m_status
    && 1 == m_figure_depth
...
void MYON_OutlineAccumulator::Internal_AccumulateError(
...
void MYON_OutlineAccumulator::MYON_OutlineAccumulator::AbandonCurrentFigure()
{
  m_figure_depth = 0;
  m_point_accumulator.SetCount(0);
  m_figure_start = MYON_OutlineFigurePoint::Unset;
  m_figure_prev = MYON_OutlineFigurePoint::Unset;
  m_figure_current = MYON_OutlineFigurePoint::Unset;
}

bool MYON_OutlineAccumulator::Internal_AccumulatePoint(
  MYON_OutlineFigurePoint::Type point_type,
  MYON_2fPoint point_location,
...
)
{
  ...
  switch (point_type)
  {
  case MYON_OutlineFigurePoint::Type::Unset:
    MYON_ERROR("point_type is Unset");
    break;

  case MYON_OutlineFigurePoint::Type::QuadraticBezierPoint:
  case MYON_OutlineFigurePoint::Type::CubicBezierPoint:
  case MYON_OutlineFigurePoint::Type::LineTo:
    state = 2; // continue a figure
    break;

  case MYON_OutlineFigurePoint::Type::BeginFigureUnknown:
  case MYON_OutlineFigurePoint::Type::BeginFigureOpen:
  case MYON_OutlineFigurePoint::Type::BeginFigureClosed:
    if ( MYON_OutlineFigure::Type::SingleStroke == m_figure_type )
      point_type = MYON_OutlineFigurePoint::Type::BeginFigureOpen;
    bPointInBoundingBox = true;
    state = 1; // begin a figure
    break;

  //case MYON_OutlineFigurePoint::Type::LineToCloseContour:
  case MYON_OutlineFigurePoint::Type::EndFigureClosed:
    if ( MYON_OutlineFigure::Type::SingleStroke == m_figure_type )
      point_type = MYON_OutlineFigurePoint::Type::EndFigureOpen;
    state = 3; // end a figure - point_location input parameter is ignored.
    break;

  case MYON_OutlineFigurePoint::Type::EndFigureOpen:
    state = 3; // end a figure - point_location input parameter is ignored.
    break;

  case MYON_OutlineFigurePoint::Type::Error:
    break;

  default:
    MYON_ERROR("point_type is invalid.");
    break;
  }
  ...
  case 1: // begin a figure
    if (0 != m_figure_depth)
    {
      MYON_ERROR("Beginning a figure in an unexpected state.");
      Internal_AccumulateError(true);
    }
    bPointInBoundingBox = true;
    m_point_accumulator.SetCount(0);
  ...
  case 2: // continue a figure
    if ( false == Internal_InFigure() )
    {
      MYON_ERROR("Continuing a figure in an unexpected state.");
      state = 0;
    }
    else
    {
      if (MYON_OutlineFigurePoint::Type::LineTo == point_type)
        bPointInBoundingBox = true;
    }
    break;
  ...
  case 3: // end a figure
    if ( false == Internal_InFigure() )
    {
      MYON_ERROR("Ending a figure in an unexpected state.");
      state = 0;
    }
    else
    {
      if ( MYON_OutlineFigurePoint::Type::BeginFigureUnknown == m_figure_start.m_point_type )
      {
        if ( MYON_OutlineFigurePoint::Type::EndFigureClosed == point_type )
          m_figure_start.m_point_type = MYON_OutlineFigurePoint::Type::BeginFigureClosed;
        else if ( MYON_OutlineFigurePoint::Type::EndFigureOpen == point_type )
          m_figure_start.m_point_type = MYON_OutlineFigurePoint::Type::BeginFigureOpen;
        MYON_OutlineFigurePoint q = m_point_accumulator[0];
        if (
          MYON_OutlineFigurePoint::Type::BeginFigureUnknown == q.m_point_type
      ...
      if (MYON_OutlineFigurePoint::Type::BeginFigureOpen == m_figure_start.m_point_type)
      {
        if (MYON_OutlineFigurePoint::Type::EndFigureOpen != point_type)
        {
          MYON_ERROR("EndFigureClosed used to finish an open figure");
          point_type = MYON_OutlineFigurePoint::Type::EndFigureOpen;
        }
      ...
      else
      {
        if (MYON_OutlineFigurePoint::Type::EndFigureClosed != point_type)
        {
          MYON_ERROR("EndFigureOpen used to finish an closed figure");
  ...
  const MYON__UINT16 figure_index = (MYON__UINT16)(1+outline.FigureCount());


  MYON_OutlineFigurePoint gp;
  gp.m_point_type = point_type;
  gp.m_point_proximity = bPointInBoundingBox ? MYON_OutlineFigurePoint::Proximity::OnFigure : MYON_OutlineFigurePoint::Proximity::OffFigure;
...
bool MYON_FontGlyph::GetOutline(
...
) const
{
  ...
  if (MYON_OutlineFigure::Type::SingleStroke == font_figure_type)
    ...
    font_figure_type = MYON_OutlineFigure::Type::SingleStroke;

  bool rc = false;
  if (nullptr != MYON_Font::Internal_CustomGetGlyphOutlineFunc)
  {
    rc = MYON_Font::Internal_CustomGetGlyphOutlineFunc(
      this,
      bSingleStrokeFont,
      outline
    );
  ...
  else
  {
    ...
    rc = MYON_WindowsDWriteGetGlyphOutline(
      this,
      font_figure_type,
      outline
    );
    ...
    rc = MYON_AppleFontGetGlyphOutline(
      this,
      font_figure_type,
      outline
    );
    ...
    rc = MYON_FreeTypeGetGlyphOutline(
      this,
      font_figure_type,
      outline
    );
...
bool MYON_FontGlyph::GetGlyphContours(
  bool bSingleStrokeFont,
...
) const
{
  ...
  MYON_Outline outline;
  GetOutline(bSingleStrokeFont, outline);

  const MYON_FontMetrics fm = font->FontUnitFontMetrics();
  double scale = 1.0;
  if (height_of_capital > 0.0 && height_of_capital < MYON_UNSET_POSITIVE_FLOAT)
  {
    scale = fm.GlyphScale(height_of_capital);
  }
  else if ( 
    MYON_UNSET_VALUE == height_of_capital
    || MYON_UNSET_POSITIVE_VALUE == height_of_capital
    || MYON_UNSET_FLOAT == height_of_capital
    || MYON_UNSET_POSITIVE_FLOAT == height_of_capital
    )
  {
  ...
  else
  {
    ...
    if ( font_UPM > 0.0 &&  font_UPM < MYON_UNSET_POSITIVE_FLOAT )
    scale = ((double)MYON_Font::AnnotationFontCellHeight) / font_UPM;
  }
  ...
  const MYON_TextBox glyph_metrics = outline.GlyphMetrics();

  if (nullptr != glyph_advance)
    *glyph_advance = scale * MYON_3dVector(glyph_metrics.m_advance.i, glyph_metrics.m_advance.j, 0.0);

  if (nullptr != glyph_bbox)
    *glyph_bbox = MYON_BoundingBox(
      scale*MYON_3dPoint(glyph_metrics.m_bbmin.i, glyph_metrics.m_bbmin.j, 0.0),
      scale*MYON_3dPoint(glyph_metrics.m_bbmax.i, glyph_metrics.m_bbmax.j, 0.0)
    );
