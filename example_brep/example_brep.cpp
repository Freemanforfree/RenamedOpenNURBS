...
//
// 1) How to dig through a MYON_Brep face  and get at the surface
       ...
       )
{
  ...
  const MYON_BrepFace& face = brep.m_F[fi];

  // pSrf = underlying untrimmed surface
  const MYON_Surface* pSrf = NULL;
  ...
  for ( fli = 0; fli < loop_count; fli++ ) {
    ...
    const MYON_BrepLoop& loop = brep.m_L[li];
    ...
    for ( lti = 0; lti < loop_trim_count; lti++ ) {
      ...
      const MYON_BrepTrim& trim = brep.m_T[ti];

      //////////////////////////////////////////////////////
      // 2d trimming information
      //
      // Each trim has a 2d parameter space curve.
      const MYON_Curve* p2dCurve = NULL;
      ...
      //const MYON_BrepVertex& v0 = brep.m_V[v0i];
      //const MYON_BrepVertex& v1 = brep.m_V[v1i];
      ...
      if ( ei == -1 ) {
        ...
        // and the 3d curve obtained by composing the surface and 2d
        // curve are opposite.
        const MYON_BrepEdge& edge = brep.m_E[ei];
        const int c3i = edge.m_c3i;
        const MYON_Curve* p3dCurve = NULL;
...
static MYON_Curve* TwistedCubeTrimmingCurve(
              const MYON_Surface& s,
              int side // 0 = SW to SE
                       // 1 = SE to NE
                       // 2 = NE to NW
                       // 3 = NW to SW
              )
{
  ...
  MYON_Curve* c2d = new MYON_LineCurve( from, to );
...
static MYON_Curve* TwistedCubeEdgeCurve( const MYON_3dPoint& from, const MYON_3dPoint& to )
{
  // creates a 3d line segment to be used as a 3d curve in a MYON_Brep
  MYON_Curve* c3d = new MYON_LineCurve( from, to );
  c3d->SetDomain( 0.0, 1.0 );
  return c3d;
}

static MYON_Surface* TwistedCubeSideSurface( 
                             const MYON_3dPoint& SW, const MYON_3dPoint& SE,
                             const MYON_3dPoint& NE, const MYON_3dPoint& NW
                             )
{
  MYON_NurbsSurface* pNurbsSurface = new MYON_NurbsSurface(
                                        3,     // dimension
                                        false, // not rational
                                        2,     // "u" order
                                        2,     // "v" order
...
static void MakeTwistedCubeEdge( MYON_Brep& brep,
                                 int vi0, // index of start vertex
                                 int vi1, // index of end vertex
                                 int c3i  // index of 3d curve
                                 )
{
  MYON_BrepVertex& v0 = brep.m_V[vi0];
  MYON_BrepVertex& v1 = brep.m_V[vi1];
  ...
  edge.m_tolerance = 0.0;  // this simple example is exact - for models with
                           ...
                           // definition of MYON_BrepEdge.
}

static void MakeTwistedCubeEdges( MYON_Brep& brep )
{
...
static int MakeTwistedCubeTrimmingLoop(  MYON_Brep& brep, // returns index of loop
     MYON_BrepFace& face,  // face loop is on
...
{
  const MYON_Surface& srf = *brep.m_S[face.m_si];
  ...
  MYON_2dPoint q;
  MYON_Surface::ISO iso = MYON_Surface::not_iso;
  ...
  for ( int side = 0; side < 4; side++ ) {
    ...
    case 0: // south
      ei = eSi;
      bRev3d = (eS_dir == -1);
      iso = MYON_Surface::S_iso;
      break;
    case 1: // east
      ei = eEi;
      bRev3d = (eE_dir == -1);
      iso = MYON_Surface::E_iso;
      break;
    case 2: // north
      ei = eNi;
      bRev3d = (eN_dir == -1);
      iso = MYON_Surface::N_iso;
      break;
    case 3: // west
      ei = eWi;
      bRev3d = (eW_dir == -1);
      iso = MYON_Surface::W_iso;
      break;
    }

    MYON_BrepTrim& trim = brep.NewTrim( brep.m_E[ei], bRev3d, loop, c2i );
    q = c2->PointAtStart();
    //trim.m_P[0] = srf.PointAt( q.x, q.y );
    q = c2->PointAtEnd();
    //trim.m_P[1] = srf.PointAt( q.x, q.y );
    trim.m_iso = iso;
    trim.m_type = MYON_BrepTrim::mated; // This b-rep is closed, so all trims
                                         // have mates.
    ...
    trim.m_tolerance[1] = 0.0; // non-exact data, set tolerance as explained in
                               // definition of MYON_BrepTrim.
  }

  return loop.m_loop_index;
}

static void MakeTwistedCubeFace( MYON_Brep& brep,
...
static void MakeTwistedCubeFaces( MYON_Brep& brep )
{
...
static MYON_Brep* MakeTwistedCube( MYON_TextLog& error_log )
{
  // This example demonstrates how to construct a MYON_Brep
  ...
  MYON_3dPoint point[8] = {
    MYON_3dPoint(  0.0,  0.0,  0.0 ),  // point A = geometry for vertex 0
    MYON_3dPoint( 10.0,  0.0,  0.0 ),  // point B = geometry for vertex 1
    MYON_3dPoint( 10.0,  8.0, -1.0 ),  // point C = geometry for vertex 2
    MYON_3dPoint(  0.0,  6.0,  0.0 ),  // point D = geometry for vertex 3
    MYON_3dPoint(  1.0,  2.0, 11.0 ),  // point E = geometry for vertex 4
    MYON_3dPoint( 10.0,  0.0, 12.0 ),  // point F = geometry for vertex 5
    MYON_3dPoint( 10.0,  7.0, 13.0 ),  // point G = geometry for vertex 6
    MYON_3dPoint(  0.0,  6.0, 12.0 )   // point H = geometry for vertex 7
  };
  ...
  int vi;
  for ( vi = 0; vi < 8; vi++ ) {
    MYON_BrepVertex& v = brep->NewVertex(point[vi]);
    v.m_tolerance = 0.0; // this simple example is exact - for models with
                         // non-exact data, set tolerance as explained in
                         // definition of MYON_BrepVertex.
  }
  ...
  //MYON_BOOL32 bIsManifold;
  //MYON_BOOL32 bHasBoundary;
  //MYON_BOOL32 b = brep->IsManifold( &bIsManifold,&bHasBoundary );
...
int main()
{
  ...
  MYON_Brep* brep = MakeTwistedCube(error_log);
  if ( !brep )
    return 1;
  ...
  model.AddDefaultLayer(L"brep", MYON_Color::UnsetColor);
  ...
  const int version = 0; // version will be MYON_BinaryArchive::CurrentArchiveVersion()
