...
MYON_OBJECT_IMPLEMENT( MYON_SumSurface, MYON_Surface, "C4CD5359-446D-4690-9FF5-29059732472B" );

void MYON_SumSurface::DestroyRuntimeCache( bool bDelete )
{
...
MYON_SumSurface* MYON_SumSurface::New()
{
  return new MYON_SumSurface();
}

MYON_SumSurface* MYON_SumSurface::New( const MYON_SumSurface& rev_surface )
{
  return new MYON_SumSurface(rev_surface);
}

MYON_SumSurface::MYON_SumSurface()
{
  MYON__SET__THIS__PTR(m_s_MYON_SumSurface_ptr);
  m_curve[0] = nullptr;
  m_curve[1] = nullptr;
}
...
void MYON_SumSurface::Destroy()
{
  ...
  m_basepoint = MYON_3dPoint::Origin;
}

void MYON_SumSurface::EmergencyDestroy()
{
  m_curve[0] = nullptr;
  m_curve[1] = nullptr;
  m_bbox = MYON_BoundingBox::EmptyBoundingBox;
...
void MYON_SumSurface::Internal_CopyFrom(const MYON_SumSurface& src)
{
  m_curve[0] = nullptr;
  m_curve[1] = nullptr;
  for ( int i = 0; i < 2; i++ )
...
MYON_SumSurface::MYON_SumSurface( const MYON_SumSurface& src ) 
  : MYON_Surface(src)
{
  MYON__SET__THIS__PTR(m_s_MYON_SumSurface_ptr);
  m_curve[0] = nullptr;
...
unsigned int MYON_SumSurface::SizeOf() const
{
...
MYON__UINT32 MYON_SumSurface::DataCRC(MYON__UINT32 current_remainder) const
{
  if ( m_curve[0] )
...
MYON_SumSurface& MYON_SumSurface::operator=(const MYON_SumSurface& src )
{
  if ( this != &src ) 
  {
    Destroy();
...
bool MYON_SumSurface::Create( const MYON_Curve& curve, MYON_3dVector vector )
{
  Destroy();
  bool rc = false;
  if ( !vector.IsZero() )
  {
    MYON_Curve* pCurve = curve.DuplicateCurve();
    rc = Create( pCurve, vector );
  }
  return rc;
}

bool MYON_SumSurface::Create( MYON_Curve* pCurve, MYON_3dVector vector )
{
  Destroy();
  bool rc = false;
  if ( !vector.IsZero() )
  {
    MYON_LineCurve* pLineCurve = new MYON_LineCurve( MYON_Line( MYON_3dPoint::Origin, vector ) );
    pLineCurve->SetDomain( 0.0, vector.Length() );
    m_curve[0] = pCurve;
    m_curve[1] = pLineCurve;
    m_basepoint.Set(0.0,0.0,0.0);
    MYON_BoundingBox bbox0 = pCurve->BoundingBox();
    MYON_BoundingBox bbox1 = bbox0;
    bbox1.m_min += vector;
    bbox1.m_max += vector;
...
bool MYON_SumSurface::Create( const MYON_Curve& curve, const MYON_Curve& path_curve )
{
  Destroy();
  MYON_Curve* pCurve = curve.DuplicateCurve();
  MYON_Curve* pPathCurve = path_curve.DuplicateCurve();
  bool rc = Create( pCurve, pPathCurve );
  return rc;
}

bool MYON_SumSurface::Create( MYON_Curve* pCurve, MYON_Curve* pPathCurve )
{
  Destroy();
  bool rc = false;
  if ( pCurve && pPathCurve )
  {
    ...
    m_basepoint = MYON_3dPoint::Origin - pPathCurve->PointAtStart();
    m_bbox.Destroy();
    BoundingBox();
    rc = true;
  }
...
//
bool MYON_SumSurface::IsValid( MYON_TextLog* text_log ) const
{
  for ( int i = 0; i < 2; i++ )
  {
    if ( !m_curve[i] )
    {
      if ( text_log )
        text_log->Print("MYON_SumSurface.m_curve[%d] is nullptr.\n",i);
      return false;
    }
    if ( m_curve[i]->Dimension() != 3 )
    {
      if ( text_log )
        text_log->Print("MYON_SumSurface.m_curve[%d]->m_dim = %d (should be 3).\n",i,m_curve[i]->Dimension());
      return false;
    }
    if ( !m_curve[i]->IsValid(text_log) )
    {
      if ( text_log )
        text_log->Print("MYON_SumSurface.m_curve[%d] is not valid.\n",i);
      return false;
    }
  }
  ...
  {
    if ( text_log )
      text_log->Print("MYON_SumSurface.m_basepoint is not valid.\n");
    return false;
  }
  return true;
}

void MYON_SumSurface::Dump( MYON_TextLog& dump ) const
{
  MYON_Object::Dump(dump);
  dump.PushIndent();
...
bool MYON_SumSurface::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_SumSurface::Read( MYON_BinaryArchive& file )
{
  Destroy();
  int major_version = 0;
  int minor_version = 0;
...
//
int MYON_SumSurface::Dimension() const
{
  int dim  = 0;
...
void MYON_SumSurface::ClearBoundingBox()
{
  m_bbox.Destroy();
}

bool MYON_SumSurface::GetBBox( // returns true if successful
       ...
       ) const
{
  ...
  if (!rc )
  {
    ...
    {
      MYON_SumSurface* pS = const_cast<MYON_SumSurface*>(this);
...
bool MYON_SumSurface::IsDeformable() const
{
  bool rc = true;
  if ( m_curve[0] )
...
bool MYON_SumSurface::MakeDeformable()
{
  bool rc = true;
...
bool MYON_SumSurface::Transform( const MYON_Xform& xform )
{
  DestroyRuntimeCache();
  TransformUserData(xform);
  bool rc = false;

  MYON_3dPoint A0, A1, A2;
  if ( m_curve[0] ) 
  {
...
bool MYON_SumSurface::SetDomain( 
...
MYON_Interval MYON_SumSurface::Domain( int dir ) const
{
...
bool MYON_SumSurface::GetSurfaceSize( 
    double* width, 
    double* height 
    ) const
{
  ...
  for ( j = 0; j < 2; j++ )
  {
    ...
    *ptr[j] = 0.0;
    ...
    MYON_Interval cdom = m_curve[j]->Domain();
...
int MYON_SumSurface::SpanCount( int dir ) const
{
  int span_count = 0;
  if ( dir == 0 && m_curve[0] )
...
bool MYON_SumSurface::GetSpanVector( int dir, double* s ) const
{
  bool rc = false;
  if ( dir == 0 && m_curve[0] )
...
int MYON_SumSurface::Degree( int dir ) const
{
  int degree = 0;
  if ( dir == 0 && m_curve[0] )
...
bool MYON_SumSurface::GetParameterTolerance( // returns tminus < tplus: parameters tminus <= s <= tplus
...
bool MYON_SumSurface::IsClosed( int dir ) const
{
  bool rc = false;
  if ( dir == 0 && m_curve[0] )
...
bool MYON_SumSurface::IsPeriodic( int dir ) const
{
  bool rc = false;
  if ( dir == 0 && m_curve[0] )
...
bool MYON_SumSurface::IsSingular( int side ) const
{
  return false;
}


bool MYON_SumSurface::GetNextDiscontinuity( 
                int dir,
                MYON::continuity c,
                double t0,
                double t1,
...
bool MYON_SumSurface::IsContinuous(
...
bool MYON_SumSurface::Reverse( int dir )
{
  bool rc = false;
  if ( dir == 0 && m_curve[0] )
...
bool MYON_SumSurface::Transpose()
{
  MYON_Curve* c = m_curve[0];
  m_curve[0] = m_curve[1];
  m_curve[1] = c;
	DestroySurfaceTree();
  return true;
...
bool MYON_SumSurface::Evaluate( // returns false if unable to evaluate
...
MYON_Curve* MYON_SumSurface::IsoCurve(int dir, double c ) const
{
  MYON_Curve* iso_curve = 0;
  if ( dir >= 0 && dir <= 1 && m_curve[0] && m_curve[1] )
  {
    iso_curve = m_curve[dir]->Duplicate();
    MYON_3dPoint p = m_curve[1-dir]->PointAt(c);
    MYON_3dVector v = p + m_basepoint;
    if ( !v.IsZero() )
    {
...
class MYON_SumTensor : public MYON_TensorProduct
{
public:
  int dim;
...
int MYON_SumTensor::DimensionA() const
{
  return dim;
}

int MYON_SumTensor::DimensionB() const
{
  return dim;
}

int MYON_SumTensor::DimensionC() const
{
  return dim;
}

bool MYON_SumTensor::Evaluate( double a, const double* CurveA, double b, const double* CurveB, double* SrfPoint )
{
...
int MYON_SumSurface::GetNurbForm(
      ...
      ) const
{
  ...
  if ( dim > 0 )
  {
    MYON_NurbsCurve tmpA, tmpB;
    int rcA = 0;
    int rcB = 0;
    const MYON_NurbsCurve* nurbs_curveA=0;
    const MYON_NurbsCurve* nurbs_curveB=0;
    nurbs_curveA = MYON_NurbsCurve::Cast(m_curve[0]);
    if ( !nurbs_curveA ) 
    {
      rcA = 1;
    ...
    {
      rcB = 1;
      nurbs_curveB = MYON_NurbsCurve::Cast(m_curve[1]);
      if ( !nurbs_curveB ) 
      {
...
int MYON_SumSurface::HasNurbForm() const
...
bool MYON_SumSurface::GetSurfaceParameterFromNurbFormParameter(
...
bool MYON_SumSurface::GetNurbFormParameterFromSurfaceParameter(
      double surface_s, double surface_t,
      double* nurbs_s,  double* nurbs_t
      ) const
{
...
bool MYON_SumSurface::Trim(int dir,
...
bool MYON_SumSurface::Split(int dir,
                          double c,
...

{
  ...
  if ( !Domain(dir).Includes( c, true ) )
    return false;
  MYON_SumSurface* left_srf = 0;
  MYON_SumSurface* right_srf = 0;


  if ( west_or_south_side )
  {
    left_srf = MYON_SumSurface::Cast( west_or_south_side );
    if ( !left_srf )
      return false;
  ...
  {
    right_srf = MYON_SumSurface::Cast( east_or_north_side );
    if ( !right_srf )
      return false;
  ...
  if (!left_srf) left_srf = MYON_SumSurface::New(*this);
  else if (left_srf != this) *left_srf = *this;

  if (!right_srf) right_srf = MYON_SumSurface::New(*this);
