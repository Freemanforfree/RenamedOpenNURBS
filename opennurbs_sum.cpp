...
int MYON_Sum::SummandCount() const
{
  return m_pos_count + m_neg_count + m_zero_count;
}

void MYON_Sum::Begin( double starting_value )
{
  m_sum_err = 0.0;
  m_pos_sum = 0.0;
  m_neg_sum = 0.0;
...
double MYON_Sum::SortAndSum( int count, double* a )
{
  // note that the arrays passed to MYON_Sum::SortAndSum() are all 
  // homogeneous in sign
  double s = 0.0;
  if ( count > 0 )
  {
    if ( count >= 2 )
    {
      MYON_SortDoubleArray( MYON::sort_algorithm::quick_sort, a, count );
      //double a0 = fabs(a[0]);
      //double a1 = fabs(a[count-1]);
      m_sum_err += MYON_EPSILON*( fabs(a[count-1]) + count*fabs(a[0]) );
    }
    if ( a[count] < 0.0 )
    {
      a += count-1;
...
void MYON_Sum::Plus( double x, double dx )
{
  Plus(x);
  if ( MYON_IsValid(dx) )
    m_sum_err += fabs(dx);
    ...
    if ( m_pos_sum1_count == sum1_max_count )
    {
      ...
      if ( m_pos_sum2_count == sum2_max_count )
      {
        ...
        if ( m_pos_sum3_count == sum3_max_count )
        {
          ...
          m_sum_err += MYON_EPSILON*( fabs(x) + fabs(m_pos_sum) );
          m_pos_sum += x;
          m_pos_sum3_count = 0;
        }
      }
    ...
    if ( m_neg_sum1_count == sum1_max_count )
    {
      ...
      if ( m_neg_sum2_count == sum2_max_count )
      {
        ...
        if ( m_neg_sum3_count == sum3_max_count )
        {
          ...
          m_sum_err += MYON_EPSILON*( fabs(x) + fabs(m_neg_sum) );
          m_neg_sum += x;
          m_neg_sum3_count = 0;
        }
      }
...
void MYON_Sum::operator=(double x)
{
  Begin(x);
}
...
double MYON_Sum::Total( double* error_estimate )
{
  double x;
  ...
  if ( error_estimate )
  {
    *error_estimate = m_sum_err + MYON_EPSILON*(fabs(m_pos_sum) + fabs(m_neg_sum));
  }
