...
static
const MYON_BrepEdge* FindLinearEdge( const MYON_Brep& brep, int vi0, int vi1 )
{
  ...
  if ( vi0 == vi1 )
    return nullptr;
  const MYON_BrepVertex& v0 = brep.m_V[vi0];
  //const MYON_BrepVertex& v1 = brep.m_V[vi1];
  int vei;
  for ( vei = 0; vei < v0.m_ei.Count(); vei++ )
  {
    const MYON_BrepEdge* edge = brep.Edge( v0.m_ei[vei] );
    if ( !edge )
      continue;
...
static
void SynchFaceOrientation( MYON_Brep& brep, int fi )
{
  const MYON_BrepFace* face = brep.Face(fi);
  if ( face )
  {
    int flip = -1, fli, lti;
    for ( fli = 0; fli < face->m_li.Count(); fli++ )
    {
      const MYON_BrepLoop* loop = brep.Loop( face->m_li[fli] );
      if ( !loop )
        continue;
      for ( lti = 0; lti < loop->m_ti.Count(); lti++ )
      {
        const MYON_BrepTrim* trim = brep.Trim( loop->m_ti[lti] );
        if ( !trim )
          continue;
        const MYON_BrepEdge* edge = brep.Edge( trim->m_ei );
        if ( !edge )
          continue;
        if ( edge->m_ti.Count() != 2 )
          continue;
        const MYON_BrepTrim* trim0 = brep.Trim( edge->m_ti[0] );
        const MYON_BrepTrim* trim1 = brep.Trim( edge->m_ti[1] );
        if ( !trim0 || !trim1 )
          continue;
...
MYON_BrepFace* MYON_Brep::NewRuledFace(
      ...
      )
{
  ...
  if ( bRevEdgeB )
    cB.Reverse();
  MYON_NurbsSurface* srf = MYON_NurbsSurface::New();
  ...
  // east side
  const MYON_BrepEdge* east_edge = FindLinearEdge( *this, vid[1], vid[2] );
  if ( east_edge )
  {
  ...
  // west side
  const MYON_BrepEdge* west_edge = FindLinearEdge( *this, vid[3], vid[0] );
  if ( west_edge )
  {
  ...
  MYON_BrepFace* face = NewFace( srf, vid, eid, bRev3d );
  if ( face )
...
MYON_BrepFace* MYON_Brep::NewConeFace(
...
bool MYON_Brep::SetTrimBoundingBoxes( bool bLazy )
{
  bool rc = true;
...
bool MYON_Brep::SetTrimBoundingBoxes( MYON_BrepFace& face, bool bLazy )
{
  bool rc = true;
...
bool MYON_Brep::SetTrimBoundingBoxes( MYON_BrepLoop& loop, bool bLazy )
{
...
bool MYON_Brep::SetTrimBoundingBox( MYON_BrepTrim& trim, bool bLazy )
{
...
void MYON_Brep::SetTolerancesBoxesAndFlags(
     bool bLazy,
     ...
     )
{
  ...
  if ( bSetLoopTypeFlags )
  {
    for ( li = 0; li < loop_count; li++ )
    {
      ...
      if ( loop.m_type == MYON_BrepLoop::unknown || !bLazy )
      {
...
MYON_BrepFace* MYON_Brep::NewFace(
...
MYON_BrepLoop* MYON_Brep::NewOuterLoop(
       int face_index,
       int vid[4],
       int eid[4],
       bool boolRev3d[4]
       )
{
  ...
  const MYON_Surface* pSurface = face.SurfaceOf();
  if (!pSurface) 
    return nullptr;
  ...
  int i, eti;
  MYON_Curve* c3[4] = {nullptr,nullptr,nullptr,nullptr};
  ...
  for ( i = 0; i < 4; i++ )
  {
    if ( eid[i] != -1 )
    {
      ...
      {
        MYON_ERROR("Bad edge index passed to MYON_BrepNewFace.");
        return 0;
      }
      ...
      if ( vi0 < 0 || vi1 < 0 )
      {
        MYON_ERROR("MYON_Brep::NewFace(MYON_Surface*,...) error: Bad edge vertex informtion.");
        return 0;
      }
      if ( vid[i] == -1 )
        vid[i] = vi0;
      ...
      {
        MYON_ERROR("MYON_Brep::NewFace(MYON_Surface*,...) error: Edge and vertex informtion do not match.");
        return 0;
      }
      if ( vid[(i+1)%4] == -1 )
        vid[(i+1)%4] = vi1;
  ...
  for ( i = 0; i < 4; i++ )
  {
    if ( vid[i] != -1 )
    {
      ...
      {
        MYON_ERROR("Bad vertex index passed to MYON_Brep::NewFace.");
        return 0;
      }
    }
  }
  ...
  for (i = 0; i < 2; i++ )
  {
    if ( bIsClosed[i] )
    {
      ...
      else if ( eid[k] != -1 || eid[j] != -1)
      {
        ...
        {
          MYON_ERROR("Bad edge information passed to MYON_Brep::NewFace.");
          return 0;
        }
      }
    }
  ...
  for ( i = 0; i < 4; i++ )
  {
    ...
    if ( bIsSingular[i] || bIsClosed[i%2] )
    {
      ...
      {
        MYON_ERROR("Bad vertex indices passed to MYON_Brep::NewFace.");
        return 0;
      }
    }
  }
  ...
  for ( i = 0; i < 4; i++ )
  {
    ...
    if ( eid[i] != -1 )
    {
      const MYON_BrepEdge& edge = m_E[eid[i]];
    ...
    if ( !c3[i] )
    {
     MYON_ERROR("MYON_Brep::NewLoop unable to make 3d edge curve.");
      return 0;
    }
  ...
  for ( i = 0; i < 4; i++ )
  {
    if ( c3[i] )
    {
      ...
      MYON_BrepEdge& edge = NewEdge( m_V[vid[i0]], m_V[vid[i1]], AddEdgeCurve( c3[i] ) );
  ...
  MYON_Surface::ISO srf_iso[4] = {MYON_Surface::S_iso,MYON_Surface::E_iso,MYON_Surface::N_iso,MYON_Surface::W_iso};

  for ( i = 0; i < 4; i++ )
  {
    MYON_NurbsCurve* c2 = new MYON_NurbsCurve( 2, 0, 2, 2 );
    ...
    {
      MYON_BrepTrim& trim = NewTrim( m_E[eid[i]], bRev3d[i], loop, c2i);
      ...
      if ( bIsClosed[(i+1)%2] ) 
        ...
        if ( edge.m_ti.Count() > 1 )
        {
          ...
          {
            m_T[edge.m_ti[eti]].m_type = MYON_BrepTrim::mated;
          }
        }
      }
...
MYON_Brep* MYON_BrepBox( const MYON_3dPoint* box_corners, MYON_Brep* pBrep )
{
  MYON_Brep* brep = 0;
  int vi, ei, fi, si, c2i;
  if (box_corners)
  {
    ...
    for ( ei = 0; ei < 4; ei++ )
    {
      MYON_BrepVertex& v0 = brep->m_V[ei];
      MYON_BrepVertex& v1 = brep->m_V[(ei+1)%4];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    for ( ei = 4; ei < 8; ei++ )
    {
      MYON_BrepVertex& v0 = brep->m_V[ei];
      MYON_BrepVertex& v1 = brep->m_V[ei==7?4:(ei+1)];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    for ( ei = 8; ei < 12; ei++ )
    {
      MYON_BrepVertex& v0 = brep->m_V[ei-8];
      MYON_BrepVertex& v1 = brep->m_V[ei-4];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    ...
    */
    ...
    for ( fi = 0; fi < 6; fi++ )
    {
      MYON_BrepEdge& e0 = brep->m_E[f[fi].e[0]];
      MYON_BrepEdge& e1 = brep->m_E[f[fi].e[1]];
      MYON_BrepEdge& e2 = brep->m_E[f[fi].e[2]];
      MYON_BrepEdge& e3 = brep->m_E[f[fi].e[3]];
      MYON_BrepVertex& v0 = brep->m_V[e0.m_vi[f[fi].bRev[0]?1:0]];
      MYON_BrepVertex& v1 = brep->m_V[e1.m_vi[f[fi].bRev[1]?1:0]];
      MYON_BrepVertex& v2 = brep->m_V[e2.m_vi[f[fi].bRev[2]?1:0]];
      MYON_BrepVertex& v3 = brep->m_V[e3.m_vi[f[fi].bRev[3]?1:0]];

      si = brep->AddSurface( MYON_NurbsSurfaceQuadrilateral( v0.point, v1.point, v2.point, v3.point ) );
      MYON_Interval s = brep->m_S[si]->Domain(0);
      MYON_Interval t = brep->m_S[si]->Domain(1);
      MYON_2dPoint p0(s[0],t[0]);
      MYON_2dPoint p1(s[1],t[0]);
      MYON_2dPoint p2(s[1],t[1]);
      MYON_2dPoint p3(s[0],t[1]);
      ...
      c2i = brep->AddTrimCurve( new MYON_LineCurve( p0, p1 ) );
      MYON_BrepTrim& trim0 = brep->NewTrim( e0, f[fi].bRev[0], loop, c2i );
      trim0.m_tolerance[0] = 0.0;
      trim0.m_tolerance[1] = 0.0;
      trim0.m_type = (trim0.m_vi[0] != trim0.m_vi[1]) ? MYON_BrepTrim::mated : MYON_BrepTrim::singular;
      trim0.m_iso = MYON_Surface::S_iso;

      // east side of surface
      c2i = brep->AddTrimCurve( new MYON_LineCurve( p1, p2 ) );
      MYON_BrepTrim& trim1 = brep->NewTrim( e1, f[fi].bRev[1], loop, c2i );
      trim1.m_tolerance[0] = 0.0;
      trim1.m_tolerance[1] = 0.0;
      trim1.m_type = (trim1.m_vi[0] != trim1.m_vi[1]) ? MYON_BrepTrim::mated : MYON_BrepTrim::singular;
      trim1.m_iso = MYON_Surface::E_iso;

      // north side of surface
      c2i = brep->AddTrimCurve( new MYON_LineCurve( p2, p3 ) );
      MYON_BrepTrim& trim2 = brep->NewTrim( e2, f[fi].bRev[2], loop, c2i );
      trim2.m_tolerance[0] = 0.0;
      trim2.m_tolerance[1] = 0.0;
      trim2.m_type = (trim2.m_vi[0] != trim2.m_vi[1]) ? MYON_BrepTrim::mated : MYON_BrepTrim::singular;
      trim2.m_iso = MYON_Surface::N_iso;

      // west side of surface
      c2i = brep->AddTrimCurve( new MYON_LineCurve( p3, p0 ) );
      MYON_BrepTrim& trim3 = brep->NewTrim( e3, f[fi].bRev[3], loop, c2i );
      trim3.m_tolerance[0] = 0.0;
      trim3.m_tolerance[1] = 0.0;
      trim3.m_type = (trim3.m_vi[0] != trim3.m_vi[1]) ? MYON_BrepTrim::mated : MYON_BrepTrim::singular;
      trim3.m_iso = MYON_Surface::W_iso;
    }
    if ( !brep->IsValid() ) {
      if ( pBrep )
        pBrep->Destroy();
...
MYON_Brep* MYON_BrepWedge( const MYON_3dPoint* corners, MYON_Brep* pBrep )
{
  MYON_Brep* brep = 0;
  ...
  if(corners)
  {
    ...
    {
      MYON_BrepVertex& v0 = brep->m_V[ei];
      MYON_BrepVertex& v1 = brep->m_V[(ei+1)%3];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    ...
    {
      MYON_BrepVertex& v0 = brep->m_V[ei];
      MYON_BrepVertex& v1 = brep->m_V[ei==5?3:(ei+1)];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    ...
    for ( ei = 6; ei < 9; ei++ )
    {
      MYON_BrepVertex& v0 = brep->m_V[ei-6];
      MYON_BrepVertex& v1 = brep->m_V[ei-3];
      brep->m_C3.Append( new MYON_LineCurve( v0.point, v1.point ) );
      brep->NewEdge( v0, v1, ei, nullptr, 0.0 );
    }
    ...
    */
    ...
    for ( fi = 0; fi < 5; fi++ )
    {
      ...
      if( f[fi].e[3] >= 0)
        // 4 sided face
        srf = MYON_NurbsSurfaceQuadrilateral( v0->point, v1->point, v2->point, v3->point);
      else
        // 3 sided face
        srf = MYON_NurbsSurfaceQuadrilateral( v0->point, v1->point, v1->point + (v2->point - v0->point), v2->point);

      si = brep->AddSurface( srf);

      MYON_Interval s = brep->m_S[si]->Domain(0);
      MYON_Interval t = brep->m_S[si]->Domain(1);
      MYON_2dPoint p0, p1, p2, p3;
      p0.Set(s[0],t[0]);
      p1.Set(s[1],t[0]);
      p2.Set(s[1],t[1]);
      p3.Set(s[0],t[1]);
      ...
      if( f[fi].e[3] >= 0)
      {
        // south side of surface
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p0, p1 ) );
        brep->NewTrim( *e0, f[fi].bRev[0], loop, c2i ).m_iso = MYON_Surface::S_iso;
        
        // east side of surface
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p1, p2 ) );
        brep->NewTrim( *e1, f[fi].bRev[1], loop, c2i ).m_iso = MYON_Surface::E_iso;
        
        // north side of surface
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p2, p3 ) );
        brep->NewTrim( *e2, f[fi].bRev[2], loop, c2i ).m_iso = MYON_Surface::N_iso;

        // west side of surface
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p3, p0 ) );
        brep->NewTrim( *e3, f[fi].bRev[3], loop, c2i ).m_iso = MYON_Surface::W_iso;
      }
      else
      {
        // south side of surface
        ...
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p1, p3 ) );
        brep->NewTrim( *e1, f[fi].bRev[1], loop, c2i ).m_iso = MYON_Surface::not_iso;

        // west side of surface
        c2i = brep->AddTrimCurve( new MYON_LineCurve( p3, p0 ) );
        brep->NewTrim( *e2, f[fi].bRev[2], loop, c2i ).m_iso = MYON_Surface::W_iso;
      }
    }
    ...
    {
      MYON_BrepTrim& trim = brep->m_T[ti];
      trim.m_type =  ( trim.m_vi[0] != trim.m_vi[1] && trim.m_ei >= 0 )
                  ? MYON_BrepTrim::mated
                  : MYON_BrepTrim::singular;
      trim.m_tolerance[0] = 0.0;
      trim.m_tolerance[1] = 0.0;
    }
...
MYON_Brep* MYON_BrepSphere( const MYON_Sphere& sphere, MYON_Brep* pBrep )
{
  bool bArcLengthParameterization = true;
  MYON_Brep* brep = nullptr;
  if ( pBrep )
    pBrep->Destroy();
  MYON_RevSurface* pRevSurface = sphere.RevSurfaceForm(bArcLengthParameterization);
  if ( pRevSurface )
  {
    brep = MYON_BrepRevSurface( pRevSurface, false, false, pBrep );
    if ( !brep )
      delete pRevSurface;
  }
  return brep;
...
MYON_Brep* MYON_BrepQuadSphere( const MYON_3dPoint& Center, double radius, MYON_Brep* pBrep)

{
  ...
  MYON_NurbsSurface* pZp = MYON_NurbsSurface::New(3, true, 3, 3, 3, 3);
  for (int i=0; i<2; i++){
  ...
  MYON_NurbsSurface* pXp = MYON_NurbsSurface::New(*pZp);
  pXp->Transform(roty);

  MYON_NurbsSurface* pZn = MYON_NurbsSurface::New(*pXp);
  pZn->Transform(roty);

  MYON_NurbsSurface* pXn = MYON_NurbsSurface::New(*pZn);
  pXn->Transform(roty);
  ...
  MYON_NurbsSurface* pYp = MYON_NurbsSurface::New(*pZp);
  pYp->Transform(rotx);

  MYON_NurbsSurface* pYn = MYON_NurbsSurface::New(*pZn);
  pYn->Transform(rotx);
  ...
  MYON_Xform trans;
  trans = MYON_Xform::TranslationTransformation(Center.x, Center.y, Center.z);
...
MYON_Brep* MYON_BrepTorus( const MYON_Torus& torus, MYON_Brep* pBrep )
{
  bool bArcLengthParameterization = true;
  MYON_Brep* brep = nullptr;
  if ( pBrep )
    pBrep->Destroy();
  MYON_RevSurface* pRevSurface = torus.RevSurfaceForm();
  if ( pRevSurface )
  {
                          ...
                          MYON_Brep* pBrep )
{
  ...
  if ( pBrep )
    pBrep->Destroy();
  MYON_RevSurface* pRevSurface = cylinder.RevSurfaceForm();
  if ( pRevSurface )
  {
    if ( bArcLengthParameterization )
    {
      ...
      if ( r <= MYON_SQRT_EPSILON )
        r = 1.0;
      pRevSurface->SetDomain(0,0.0,2.0*MYON_PI*r);
    }
    brep = MYON_BrepRevSurface( pRevSurface, bCapBottom, bCapTop, pBrep );
    if ( !brep )
      delete pRevSurface;
  }
  return brep;
...
MYON_Brep* MYON_BrepCone( const MYON_Cone& cone, bool bCapBase, MYON_Brep* pBrep )
{
  bool bArcLengthParameterization = true;
  MYON_Brep* brep = nullptr;
  if ( pBrep )
    pBrep->Destroy();
  MYON_RevSurface* pRevSurface = cone.RevSurfaceForm();
  if ( pRevSurface )
  {
    if ( bArcLengthParameterization )
    {
    ...
    brep = MYON_BrepRevSurface( pRevSurface, bCapBase, bCapBase, pBrep );
    if ( !brep )
      delete pRevSurface;
  }
  return brep;
          ...
          )
{
  ...
  if ( pRevSurface && pRevSurface->m_curve )
  {
    ...
    MYON_Line axis = pRevSurface->m_axis;
    MYON_3dPoint R[2];
    ...
    else if ( bCapStart || bCapEnd )
    {
      ...
      for ( int capcount = 0; capcount < 2; capcount++ )
      {
        ...
        if ( brep->m_T[srf_trim_ti].m_type != MYON_BrepTrim::boundary )
          continue;
        if ( brep->m_T[srf_trim_ti].m_ei < 0 )
          continue;
        MYON_BrepEdge& edge = brep->m_E[brep->m_T[srf_trim_ti].m_ei];
        if ( !edge.IsClosed() )
          continue;
        ...
        {
          MYON_Arc arc;
          const MYON_Curve* edge_curve = edge.EdgeCurveOf();
          if ( 0 == edge_curve )
            continue;
        ...
        MYON_BrepFace& cap = brep->NewFace( si );
        MYON_BrepLoop& loop = brep->NewLoop( MYON_BrepLoop::outer, cap );
        MYON_BrepTrim& trim = brep->NewTrim( edge, true, loop, c2i );
        for ( int eti = 0; eti < edge.m_ti.Count(); eti++ )
          brep->m_T[ edge.m_ti[eti] ].m_type = MYON_BrepTrim::mated;
        trim.m_tolerance[0] = 0.0;
        trim.m_tolerance[1] = 0.0;
...
static bool AddC3Curve( const MYON_Curve* c3, MYON_SimpleArray<MYON_Curve*>& C3 )
{
  int j;
  if ( !c3 )
    return false;
  ...
  if ( c3dim != 3 && c3dim != 2 )
    return false;
  if ( MYON_PolyCurve::Cast(c3) )
  {
    const MYON_PolyCurve* polycrv = static_cast<const MYON_PolyCurve*>(c3);
  ...
  else if ( MYON_PolylineCurve::Cast(c3) )
  {
    MYON_Line line;
    //MYON_LineCurve* linecrv = 0;
    const MYON_PolylineCurve* pline = static_cast<const MYON_PolylineCurve*>(c3);
    ...
    for ( j = 1; j < pline->m_pline.Count(); j++ )
    {
      ...
      if ( 2 == c3dim )
        line.to.z = 0.0;
      if ( line.Length() > 0 )
        C3.Append( new MYON_LineCurve(line) );
    }
  }
  else
  {
    MYON_Curve* c3dup = c3->DuplicateCurve();
    if ( 0 == c3dup )
      return false;
...
bool MYON_Brep::NewPlanarFaceLoop(
      int face_index,
      ...
      )
{
  ...
  if ( face_index < 0 || face_index >= m_F.Count() || boundary.Count() < 1 )
    return false;
  MYON_BrepFace& face = m_F[face_index];
  const MYON_PlaneSurface* pPlaneSurface = MYON_PlaneSurface::Cast(face.SurfaceOf());
  if ( !pPlaneSurface )
    return false;
  const MYON_Plane plane(pPlaneSurface->m_plane);

  // get 3d edge curves
  MYON_SimpleArray<MYON_Curve*> C3( 2*boundary.Count() );
  MYON_Curve* c3;
  ...
  if ( i < count )
  {
    MYON_ERROR("MYON_Brep::NewPlanarFaceLoop - null 3d curve in boundary[] array");
    if ( bDuplicateCurves )
    {
  ...
  count = C3.Count();
  MYON_BoundingBox loop_pbox, cbox;
  MYON_SimpleArray<double> edge_tol(count);
  MYON_SimpleArray<MYON_NurbsCurve*> C2( count );
  MYON_NurbsCurve* c2;
  for ( i = 0; i < count; i++ )
  {
    c3 = C3[i];
    c2 = new MYON_NurbsCurve();
  ...
  if ( i < count )
  {
    MYON_ERROR("MYON_Brep::NewPlanarFaceLoop - unable to create 2d trim curve");
  ...
  for ( i = 0; i < count; i++ ) 
  {
    ...
    MYON_BrepEdge& edge = NewEdge( m_V[vi0+i], m_V[vi0+((i+1)%count)], c3i );
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if ( trim_tol[0] <= MYON_ZERO_TOLERANCE )
      trim_tol[0] = 0.0;
    if ( trim_tol[1] <= MYON_ZERO_TOLERANCE )
      trim_tol[1] = 0.0;
    MYON_BrepEdge& edge = m_E[ei0+i];
  ...
  case MYON_BrepLoop::unknown:
    if ( loop_dir > 0 )
      loop.m_type = MYON_BrepLoop::outer;
    else if ( loop_dir < 0 )
      loop.m_type = MYON_BrepLoop::inner;
    break;
  default:
    // intentionally ignoring other MYON_Brep::TYPE enum values
    break;
  }
            ...
            MYON_Brep* pBrep )
{
  ...
  MYON_PlaneSurface* s = new MYON_PlaneSurface();
  s->m_plane = plane;
  ...
  if ( brep->NewPlanarFaceLoop( face.m_face_index, MYON_BrepLoop::outer, boundary, bDuplicateCurves ) )
  {
    // set face domain
    const MYON_BrepLoop* loop = brep->m_L.Last();
...
MYON_Brep* MYON_BrepTrimmedPlane( 
            ...
            MYON_Brep* pBrep )
{
  MYON_SimpleArray<MYON_Curve*> c;
  c.Append(const_cast<MYON_Curve*>(&boundary));
  return MYON_BrepTrimmedPlane( plane, c, true, pBrep );
}
...
{
  double tol = .01;
  return MYON_BrepFromMeshWithNgons(mesh_topology, bTrimmedTriangles, false, tol, pBrep);
}

static MYON_Mesh* NgonulateMesh(const MYON_Mesh* mesh_input, double tol)
{
  if (nullptr == mesh_input)
    return nullptr;

  MYON_Mesh* mesh_output = new MYON_Mesh(*mesh_input);
  if (nullptr == mesh_output)
    return nullptr;

  if (0 < mesh_output->QuadCount())
    mesh_output->ConvertNonPlanarQuadsToTriangles(tol, MYON_UNSET_VALUE, 0);

  mesh_output->SetNgonCount(0);

  MYON_MeshVertexFaceMap vfmap;
  vfmap.SetFromMesh(mesh_output, true);

  mesh_output->AddPlanarNgons(vfmap.VertexFaceMap(), tol, 3, 0, true );
  //MYON_DebugWriteObject(mesh_output);
...
static bool FixNonPlanarNgons(MYON_Mesh& mesh, double tol)
{
  mesh.RemoveAllNgons();
  MYON_Mesh* pMesh = NgonulateMesh(&mesh, tol);
  if (nullptr != pMesh)
  {
    mesh = *pMesh;
    delete pMesh;
...
static bool AllNgonsArePlanar(const MYON_Mesh* pMesh, double tol)
{
  bool rc = true;
  int i, ict = pMesh->NgonCount();
  for (i = 0; ict > i && true == rc; i++)
  {
    const MYON_MeshNgon* pNgon = pMesh->Ngon(i);
    // establish a comparison plane with the first face.
    int fi, fct = pNgon->m_Fcount;
    if (0 == fct)
      continue;

    MYON_MeshFace face = pMesh->m_F[pNgon->m_fi[0]];
    MYON_Plane plane;
...
MYON_Brep* MYON_BrepFromMeshWithNgons(
  const MYON_MeshTopology& input_topology, 
  bool bTrimmedTriangles,
...
{
  MYON_BezierCurve edge_line(3,false,2);
  MYON_BezierCurve trim_line(2,false,2);
  MYON_Brep* brep = nullptr;
  if ( pBrep )
    pBrep->Destroy();

  const MYON_MeshTopology* mesh_topology = &input_topology;
  const MYON_Mesh* pMesh = mesh_topology->m_mesh;
  ...
  brep = (pBrep != nullptr) ? pBrep : new MYON_Brep();
  ...
  MYON_Interval srf_dom[2];
  MYON_3dPoint srf_2d_corner[4];
  MYON_3dPoint srf_3d_corner[4];
  MYON_Surface::ISO quad_iso[4] = {MYON_Surface::S_iso,MYON_Surface::E_iso,MYON_Surface::N_iso,MYON_Surface::W_iso};
  MYON_Surface::ISO tri_iso[3] = {MYON_Surface::S_iso,MYON_Surface::E_iso,MYON_Surface::not_iso};

  // May 1, 2012 Tim Fix for RR 104209
  // Use double precision vertexes from the mesh if they exist
  const MYON_3dPointArray* pDPV = 0;
  ...
  for ( int vi = 0; vi < vertex_count; vi++ )
  {
    ...
    // May 1, 2012 Tim Fix for RR 104209
    // Use double precision vertexes from the mesh if they exist
    const MYON_MeshTopologyVertex& topvert = mesh_topology->m_topv[vi];
    if (0 != pDPV)
      pt = *pDPV->At(topvert.m_vi[0]);
    else
      pt = pMesh->m_V[topvert.m_vi[0]];

    MYON_BrepVertex& vertex = brep->NewVertex( pt, 0.0 );
  ...
  {
    onmalloc(0);//for canceling.  
    const MYON_MeshTopologyEdge& mesh_edge = mesh_topology->m_tope[ei];
    MYON_BrepVertex& v0 = brep->m_V[mesh_edge.m_topvi[0]];
    MYON_BrepVertex& v1 = brep->m_V[mesh_edge.m_topvi[1]];
    edge_line.SetCV(0, v0.point);
    edge_line.SetCV(1, v1.point);
    MYON_Curve* pEdgeCurve = new MYON_NurbsCurve( edge_line );
    int c3i = brep->AddEdgeCurve( pEdgeCurve );
    MYON_BrepEdge& edge = brep->NewEdge( v0, v1, c3i );    
    memset(&edge.m_edge_user, 0, sizeof(edge.m_edge_user));
    edge.m_tolerance = 0.0;
  }

  MYON_SimpleArray<bool> usedfaces(face_count);
  usedfaces.SetCount(face_count);
  usedfaces.MemSet(0);

  if (true == bUseNgonsIfTheyExist)
  {
    const MYON_3dPointListRef mesh_vertex_list(pMesh);
    const MYON_MeshFaceList& mesh_face_list(pMesh);
    MYON_MeshVertexFaceMap vf_map;
    vf_map.SetFromMesh(pMesh, true);

    for (int fi = 0; ngon_count > fi; fi++)
    {
      const MYON_MeshNgon* pNgon = pMesh->Ngon(fi);
      if (nullptr == pNgon || 1 == pNgon->m_Fcount)
        continue; // handle single face ngons with original tri/quad code

      int vcount = pNgon->m_Vcount;
      MYON_SimpleArray<MYON_3dPoint> pts(vcount);
      for (int lti = 0; vcount > lti; lti++)
        pts.Append(pMesh->Vertex(pNgon->m_vi[lti]));

      MYON_Plane plane = MYON_Plane::FromPointList(pts);
      if (MYON_Plane::UnsetPlane == plane)
        continue;

      MYON_3dVector A(pMesh->Vertex(pMesh->m_F[pNgon->m_fi[0]].vi[1]) - pMesh->Vertex(pMesh->m_F[pNgon->m_fi[0]].vi[0]));
      MYON_3dVector B(pMesh->Vertex(pMesh->m_F[pNgon->m_fi[0]].vi[3]) - pMesh->Vertex(pMesh->m_F[pNgon->m_fi[0]].vi[0]));
      A.Unitize(); B.Unitize();
      MYON_3dVector C = MYON_CrossProduct(A, B);
      double dot = MYON_DotProduct(C, plane.Normal());
      if (0 > dot)
        plane.Flip();

      MYON_PlaneSurface* pSurface = new MYON_PlaneSurface(plane);
      if (nullptr == pSurface)
        continue;

      MYON_BoundingBox bbox = MYON_PointListBoundingBox(3, false, pts.Count(), 3, (double*)pts.Array());
      ...
      pSurface->SetExtents(0, MYON_Interval(u0 - 10, u1 + 10), true);
      pSurface->SetExtents(1, MYON_Interval(v0 - 10, v1 + 10), true);

      MYON_BrepFace& face = brep->NewFace(brep->AddSurface(pSurface));
      ...
      MYON_SimpleArray<MYON_3dPoint> boundary_pts(ngon_vi.Count());
      ...
      {
        MYON_BrepLoop* pLoop = nullptr;
        if (0 == li)
          pLoop = &brep->NewLoop(MYON_BrepLoop::outer, face);
        else
          pLoop = &brep->NewLoop(MYON_BrepLoop::inner, face);
        if (nullptr == pLoop)
          continue;

        bbox = MYON_PointListBoundingBox(3, false, ngon_vi_markers[li], 3, (double*)boundary_pts.At(ct));
        ...
        for (unsigned int lti = 0; ngon_vi_markers[li] > lti && false == bFoundBadEdge; lti++)
        {
          ...
          MYON_BrepEdge& brep_edge = brep->m_E[edge_index];
          ...
          trim_line.SetCV(0, MYON_3dPoint(u0, v0, 0));
          plane.ClosestPointTo(pMesh->m_V[mesh_topology->m_topv[topv1].m_vi[0]], &u1, &v1);
          trim_line.SetCV(1, MYON_3dPoint(u1, v1, 0));
          MYON_Curve* pTrimCurve = new MYON_NurbsCurve(trim_line);
          int c2i = brep->AddTrimCurve(pTrimCurve);

          const MYON_MeshTopologyEdge& edge = mesh_topology->m_tope[edge_index];
          ...
          for (int ff = 0; edge.m_topf_count > ff && false == b_foundmatch; ff++)
          {
            ...
            const MYON_MeshTopologyFace& mesh_face = mesh_topology->m_topf[face_idx];
          ...
          MYON_BrepTrim& trim = brep->NewTrim(brep_edge, b_reve ? true : false, *pLoop, c2i);
          trim.m__legacy_2d_tol = 0.0;
          trim.m__legacy_3d_tol = 0.0;
          trim.m_tolerance[0] = 0.0;
          trim.m_tolerance[1] = 0.0;
          trim.m_iso = MYON_Surface::not_iso;
          trim.m_type = (edge.m_topf_count > 1) ? MYON_BrepTrim::mated : MYON_BrepTrim::boundary;
        }
      }
  ...
  for ( int fi = 0; fi < face_count; fi++ )
  {
    ...
    onmalloc(0);//for canceling.  
    const MYON_MeshTopologyFace& mesh_face = mesh_topology->m_topf[fi];
    ...
    MYON_Surface* pSurface = MYON_NurbsSurfaceQuadrilateral(
    ...
    if ( bTriangle && bTrimmedTriangles )
    {
      ...
      for ( int lti = 0; lti < 3; lti++ )
      {
        ...
        const MYON_MeshTopologyEdge& mesh_edge = mesh_topology->m_tope[edge_index];
        trim_line.SetCV(0,srf_2d_corner[lti]);
        trim_line.SetCV(1,srf_2d_corner[(lti+1)%3]);
        MYON_Curve* pTrimCurve = new MYON_NurbsCurve( trim_line );
        int c2i = brep->AddTrimCurve( pTrimCurve );
        MYON_BrepTrim& trim = brep->NewTrim( brep_edge, 
        ...
        trim.m_type = (mesh_edge.m_topf_count > 1) ? MYON_BrepTrim::mated : MYON_BrepTrim::boundary;
      }
    }
    else
    {
      for ( int lti = 0; lti < 4; lti++ )
      {
        ...
        MYON_Curve* c2 = new MYON_NurbsCurve( trim_line );
        int c2i = brep->AddTrimCurve( c2 );
        if ( bTriangle && lti == 3 )
        {
...
static bool FoundSlitPair(const MYON_BrepLoop& L, int* t0, int* t1)

{
  MYON_Brep* B = L.Brep();
  if (!B) return false;
  const MYON_Surface* Srf = L.SurfaceOf();
  ...
  for (i=0; i<count; i++){
    int s0 = L.m_ti[i];
    MYON_BrepTrim& T0 = B->m_T[s0];
    if (T0.m_type != MYON_BrepTrim::seam) continue;
    int s1 = (L.m_ti[(i+1)%count]);
    MYON_BrepTrim& T1 = B->m_T[s1];
    if (T1.m_type != MYON_BrepTrim::seam) continue;
    if (T0.m_vi[0] != T1.m_vi[1]) continue;
    if (T0.m_ei != T1.m_ei) continue;
    const MYON_BrepEdge& E = B->m_E[T0.m_ei];
...
bool MYON_Brep::HasSlits(const MYON_BrepLoop& L) const

{
  if (L.m_loop_index < 0)
    return false;
  if (L.m_type == MYON_BrepLoop::slit)
    return true;
  int t0, t1;
  return FoundSlitPair(L, &t0, &t1);
}

bool MYON_Brep::HasSlits(const MYON_BrepFace& F) const

{
  ...
  for (i=0; i<F.m_li.Count(); i++){
    const MYON_BrepLoop* pL = F.Loop(i);
    if (!pL)
      continue;
    if (HasSlits(*pL))
      return true;
...
bool MYON_Brep::HasSlits() const

{
  int i;
  for (i=0; i<m_F.Count(); i++){
    const MYON_BrepFace& F = m_F[i];
    if (HasSlits(F))
      return true;
  }
  return false;
...
static bool MakeNurbClosed(MYON_NurbsCurve& NC)

{
  ...
  NC.ClampEnd(2);
  MYON_3dPoint P = 0.5*(NC.PointAtStart()+NC.PointAtEnd());
...
bool MYON_Brep::MatchTrimEnds(MYON_BrepTrim& T0,
                           MYON_BrepTrim& T1 
                           )

{
  ...
  //////const MYON_Surface* pSrf = (T0.m_li >= 0) ? T0.SurfaceOf() : T1.SurfaceOf();
  //////if (!pSrf) 
  //////  return false;
  ...
  MYON_NurbsCurve* nc0 = MakeTrimCurveNurb(T0);
  if (0 == nc0) 
    return false;
  MYON_NurbsCurve* nc1 = (&T0 == &T1) 
                     ? nc0
  ...
  if(T0.m_iso > MYON_Surface::not_iso)
  {
    if(T0.m_iso % 2 == 1) xiso0 = true;
    else                  yiso0 = true;
  }
  if(T1.m_iso > MYON_Surface::not_iso)
  {
  ...
  if (!nc0->SetEndPoint(MYON_3dPoint(p)))
    return false;

  // clean up T1 flags and cached information
  if(xiso0 && p0.x != p.x) T0.m_iso = MYON_Surface::not_iso;
  if(yiso0 && p0.y != p.y) T0.m_iso = MYON_Surface::not_iso;
  T0.m_pline.Destroy();
  if ( T0.m_pbox.IsValid() )
    T0.m_pbox.Set( MYON_3dPoint(p), true );
  T0.DestroyCurveTree();

  if (!nc1->SetStartPoint(MYON_3dPoint(p)))
    return false;

  // clean up T1 flags and cached information
  if(xiso1 && p1.x != p.x) T1.m_iso = MYON_Surface::not_iso;
  if(yiso1 && p1.y != p.y) T1.m_iso = MYON_Surface::not_iso;
  T1.m_pline.Destroy();
  if ( T1.m_pbox.IsValid() )
    T1.m_pbox.Set( MYON_3dPoint(p), true );
  T1.DestroyCurveTree();
...
bool MYON_Brep::MatchTrimEnds(int trim_index)

{
  if (trim_index < 0) return false;
  MYON_BrepTrim& T = m_T[trim_index];
  if (T.m_li < 0) return false;
  bool rc = true;
  int pt = PrevTrim(trim_index);
  if (pt >= 0) {
    MYON_BrepTrim& TP = m_T[pt];
    if (!MatchTrimEnds(TP, T)) rc = false;
  }
  int nt = NextTrim(trim_index);
  if (nt >= 0) {
    MYON_BrepTrim& TN = m_T[nt];
    if (!MatchTrimEnds(T, TN)) rc = false;
  }
  return rc;
}

bool MYON_Brep::MatchTrimEnds(MYON_BrepLoop& Loop)

{
  ...
  for (i=0; i<count; i++)
  {
    MYON_BrepTrim& trim = m_T[Loop.m_ti[i]];
    trim.m_pbox.m_min.z = 0.0;
    trim.m_pbox.m_max.z = 0.0;
...

{
  ...
  {
    nc = MYON_NurbsCurve::Cast(m_C2[T.m_c2i]);
    if ( 0 != nc 
...
static 
bool RemoveSlitPair(MYON_Brep& B, int LI, int lti0, int lti1, MYON_BrepLoop*& newloop)
{
  bool rc = false;
  MYON_BrepLoop* L = B.Loop(LI);
  newloop = nullptr;
  int trim_count = L->TrimCount();
  MYON_ASSERT_OR_RETURN( lti0>=0 && lti0<trim_count, false );
  MYON_ASSERT_OR_RETURN( lti1>=0 && lti1<trim_count, false );
  MYON_ASSERT_OR_RETURN( lti0!=lti1, false );
  MYON_BrepFace* F = L->Face();
  if( !F)
    return rc;
  ...
  if( delta == 1 || delta == trim_count-1){
    // trims are consecutive. Just remove them
      ...
      if( s0>=0 && s0<= B.m_T.Count() && s1>=0 && s1<= B.m_T.Count() ){ 
        ...
        if (NextPrev && pti >= 0 && nti >= 0 && B.NextTrim(pti) == nti){
          MYON_BrepTrim& Tprev = B.m_T[pti];
          MYON_BrepTrim& Tnext = B.m_T[nti];
    ...
    MYON_BrepLoop::TYPE original_type = L->m_type;
    if( original_type!=MYON_BrepLoop::inner && original_type!=MYON_BrepLoop::outer)
      return false;
    ...
    MYON_BrepTrim& T0 = *L->Trim(lti0);
    MYON_BrepTrim& T1 = *L->Trim(lti1);
    T0.m_li = -1;
    T1.m_li = -1;
    L->m_ti[lti0] = -1;
    L->m_ti[lti1] = -1;
...
//  Tune up newly adjacent trims to be sure they meet spot on.
    ...
    MYON_BrepTrim& Tstart = *newloop->Trim(0);
    MYON_BrepTrim& Tend = *newloop->Trim( newloop->TrimCount()-1);
    B.MatchTrimEnds( Tend , Tstart );  

    int tc = L->TrimCount();
    MYON_BrepTrim& Tprev = *L->Trim( ( lti0 -1 + tc)%tc);
    MYON_BrepTrim& Tnext = *L->Trim( ( lti1+1)%tc) ;
    B.MatchTrimEnds( Tprev, Tnext );  
    ...
    if( L->m_type==original_type && newloop->m_type==MYON_BrepLoop::inner){
      // Add new inner loop to original face
      newloop->m_fi = L->m_fi;
      F->m_li.Append(newloop->m_loop_index);
      rc = true;
    }
    else if( L->m_type==original_type && newloop->m_type==MYON_BrepLoop::outer){
      // New loop is outer loop on a new face
      MYON_BrepFace& newface = B.NewFace( F->m_si );
    ...
    else if( original_type==MYON_BrepLoop::inner && 
              L->m_type== MYON_BrepLoop::outer && 
              newloop->m_type == MYON_BrepLoop::inner)
    { 
      // Add new inner loop to original face
      newloop->m_fi = L->m_fi;
      F->m_li.Append( newloop->m_loop_index );
    ...
    else if( original_type==MYON_BrepLoop::outer && 
              L->m_type== MYON_BrepLoop::inner && 
              newloop->m_type == MYON_BrepLoop::outer)
    {  
...
bool MYON_Brep::RemoveSlits(MYON_BrepLoop& L)
...
bool MYON_Brep::RemoveSlits(MYON_BrepFace& F)

{
  int i;
  bool rc = false;
  MYON_SimpleArray<int> li = F.m_li;
  for (i=0; i<li.Count(); i++){
    MYON_BrepLoop& L = m_L[li[i]];
...
bool MYON_Brep::ChangeVertex( int old_vi, int new_vi, bool bClearTolerances )
{
  if ( old_vi == new_vi )
    return true;

  MYON_BrepVertex* old_v = Vertex(old_vi);
  MYON_BrepVertex* new_v = Vertex(new_vi);
  ...
  for ( vei = 0; vei < old_v->m_ei.Count(); vei++ )
  {
    ...
    if ( bClearTolerances )
    {
      ...
      new_v->m_tolerance = MYON_UNSET_VALUE;
    }

    for ( eti = 0; eti < edge->m_ti.Count(); eti++ )
    {
      MYON_BrepTrim* trim = Trim( edge->m_ti[eti]);
      if ( 0 == trim )
        continue;
...
/* Obsolete.  MYON_Curve::SetStartPoint and SetEndPoint do the right thing.
bool MYON_BrepEdge::SetStartPoint(MYON_3dPoint start_point)
{
  return false;
}

bool MYON_BrepEdge::SetEndPoint(MYON_3dPoint end_point)
{
  return false;
}


bool MYON_BrepTrim::SetStartPoint(MYON_3dPoint point)
{
  if ( 0 == m_brep )
    return false;
  if ( point.x == MYON_UNSET_VALUE || point.y == MYON_UNSET_VALUE )
    return false;
  if ( m_c2i < 0 || m_c2i >= m_brep->m_C2.Count() )
    return false;
  const MYON_Curve* c2 = m_brep->m_C2[m_c2i];
  if ( 0 == c2 )
    return false;

  point.z = 0.0;
  MYON_Interval domain = Domain();
  MYON_3dPoint q = PointAtStart();
  q.z = 0;
  if ( point != q )
  {
   
    MYON_NurbsCurve* nc2 = 0;
    if ( ProxyCurveDomain() == c2->Domain() && !ProxyCurveIsReversed() )
      nc2 = MYON_NurbsCurve::Cast(c2);
    ...
    {
      nc2->Trim( MYON_Interval(t,domain[1] );
    }
...
bool MYON_BrepTrim::SetEndPoint(MYON_3dPoint end_point)
{
  return false;
}
*/
...
bool MYON_Brep::CloseTrimGap( MYON_BrepTrim& trim0, MYON_BrepTrim& trim1 )
{
  ...
  if ( trim0.m_li < 0 || trim0.m_li >= m_L.Count() )
    return false;
  MYON_BrepLoop& loop = m_L[trim0.m_li];
  int lti;
  ...
  MYON_Interval domain0 = trim0.Domain();
  MYON_Interval domain1 = trim1.Domain();
  double t0 = domain0[1];
  double t1 = domain1[0];
  MYON_3dPoint p0, p1;
  trim0.EvPoint( t0, p0 );
  ...
  p0.z = 0.0;
  p1.z = 0.0;
  MYON_3dPoint p = MYON_Line(p0,p1).PointAt(0.5);
  if ( p0.x == p1.x )
    p.x = p0.x;
  if ( p0.y == p1.y )
    p.y = p0.y;
  ...
  case MYON_Surface::E_iso:
    ...
    {
    case 0:
      if ( MYON_Surface::x_iso == trim0.m_iso && MYON_Surface::x_iso != trim1.m_iso )
        p.x = p1.x; // trim1 is on surface edge
      else if ( MYON_Surface::x_iso != trim0.m_iso && MYON_Surface::x_iso == trim1.m_iso )
  ...
  if (MYON_ComparePoint(3,0,&p.x,&p0.x))
  {
    trim0.SetEndPoint(p);
  }
  if (MYON_ComparePoint(3,0,&p.x,&p1.x))
  {
...
bool MYON_Brep::CollapseEdge( int edge_index, bool bCloseTrimGap, int vertex_index  )
{
  ...
  MYON_BrepVertex* vertex = Vertex(vertex_index);
  if ( 0 == vertex )
    return false;
  ...
  if ( trim_count > 0 )
  {
    MYON_SimpleArray<int> ti(trim_count);
    MYON_SimpleArray<int> li(trim_count);
    MYON_SimpleArray<int> prev_ti(trim_count);
    MYON_SimpleArray<int> next_ti(trim_count);
    int i, eti;
    ...
    for ( eti = 0; eti < trim_count; eti++ )
    {
      ...
      //MYON_BrepLoop& loop = m_L[li[eti]];
      MYON_BrepTrim& prev_trim = m_T[prev_ti[eti]];
      MYON_BrepTrim& next_trim = m_T[next_ti[eti]];
  ...
  MYON_BrepVertex* V0 = Vertex(orig_vid0);
  if (V0 && V0->EdgeCount() == 0)
    DeleteVertex(*V0);
  MYON_BrepVertex* V1 = Vertex(orig_vid1);
  if (V1 && V1->EdgeCount() == 0)
    DeleteVertex(*V1);
  return true;
}

int MYON_Brep::RemoveWireEdges( bool bDeleteVertices )
{
  int rc = 0;
...
int MYON_Brep::RemoveWireVertices()
{
  int rc = 0;
        ...
        )
{
  ...
  if ( bEdges )
  {
    ...
    for ( i = 0; i < count; i++ )
    {
      polycurve = MYON_PolyCurve::Cast(m_C3[i]);
      if ( 0 != polycurve )
      {
  ...
  if ( bTrimCurves )
  {
    ...
    for ( i = 0; i < count; i++ )
    {
      polycurve = MYON_PolyCurve::Cast(m_C2[i]);
      if ( 0 != polycurve )
      {
        if ( polycurve->RemoveNesting() )
          rc = true;
...
static bool IsSlitTrim(const MYON_BrepTrim& T)

{
  ...
  const MYON_BrepLoop* pL = T.Loop();
  if (!pL)
    return false;

  const MYON_Brep* pB = T.Brep();
  if (!pB)
    return false;

  const MYON_BrepEdge* pE = T.Edge();
  ...
  const MYON_BrepTrim& AT = pB->m_T[atid];
  ...
  const MYON_Surface* pSrf = T.SurfaceOf();
  if (!pSrf)
    return false;
  ...
  MYON_2dPoint P(T.PointAtStart());
  MYON_2dPoint AP((bRev) ? AT.PointAtEnd() : AT.PointAtStart());
...
static bool MYON_BrepRemoveSlits(MYON_BrepLoop& L)

{
  if (L.m_loop_index < 0)
    return false;
  MYON_BrepFace* pF = L.Face();
  if (!pF)
    return false;
  MYON_Brep* pB = L.Brep();
  if (!pB)
  ...
  MYON_SimpleArray<bool> bIsSlit(L.m_ti.Count());
  MYON_SimpleArray<int> slits(L.m_ti.Count());
  bool all_slits = true;
  bool some_slits = false;
  int tcount = L.m_ti.Count();
  int i;
  for (i=0; i<tcount; i++){
    MYON_BrepTrim& T = pB->m_T[L.m_ti[i]];
    if (IsSlitTrim(T)){
      bIsSlit.Append(true);
  ...
  MYON_SimpleArray<bool> bUsed = bIsSlit;

  MYON_ClassArray<MYON_SimpleArray<int> > NewLoops;
  ...
  while (b < tcount){
    ...
    if (start_trim < 0)
      break;
    MYON_SimpleArray<int>& nl = NewLoops.AppendNew();
    b++;
    ...
    while (c < tcount){
      ...
      if (bIsSlit[next_trim]){
        ...
        MYON_BrepTrim& T = pB->m_T[L.m_ti[next_trim]];
        MYON_BrepEdge* pE = T.Edge();
  ...
  for (i=0; i<NewLoops.Count(); i++){
    MYON_SimpleArray<int>& nl = NewLoops[i];
    int j;
    for (j=0; j<nl.Count(); j++)
      nl[j] = L.m_ti[nl[j]];
  }

  bool bOuter = (L.m_type == MYON_BrepLoop::outer) ? true : false;

  for (i=0; i<slits.Count(); i++){
    MYON_BrepTrim& T = pB->m_T[slits[i]];
    T.m_li = -1;
  ...
  for (i=0; i<NewLoops.Count(); i++){
    MYON_BrepLoop& nL = pB->NewLoop(MYON_BrepLoop::unknown, *pF);
    MYON_SimpleArray<int>& nl = NewLoops[i];
    nL.m_ti = nl;
    int j;
    for (j=0; j<nl.Count(); j++){
      MYON_BrepTrim& T = pB->m_T[nl[j]];
      T.m_li = nL.m_loop_index;
    }
    nL.m_type = pB->ComputeLoopType(nL);
    if (bOuter && nL.m_type == MYON_BrepLoop::outer){
      int a = pF->m_li[0];
...

{
  ...
  //After all loops have been split, call MYON_Brep::RemoveSlits() on the result.
  ...
  for (i=0; i<loop_count; i++){
    MYON_BrepLoop& L = pB->m_L[F.m_li[i]];
    if (L.m_loop_index < 0)
      continue;
    if (MYON_BrepRemoveSlits(L))
      rc = true;
  }
  return rc;
}
...
static void CreateNewTrimList(const MYON_BrepLoop& L0, int tid0,//into L0.m_ti
                               const MYON_BrepLoop& L1, int tid1,//into L1.m_ti
                               MYON_SimpleArray<int>& new_tids //into brep.m_T
                               )
...

{
  ...
  if (fid0 < 0 || fid0 >= B.m_F.Count())
    return -1;
  MYON_BrepFace& F0 = B.m_F[fid0];
  if (F0.m_face_index < 0)
    return -1;

  if (fid1 < 0 || fid1 >= B.m_F.Count())
    return -1;
  MYON_BrepFace& F1 = B.m_F[fid1];
  ...
  for (li=0; li<F0.m_li.Count() && !pE; li++){
    MYON_BrepLoop& L = B.m_L[F0.m_li[li]];
    int ti;
    for (ti=0; ti<L.m_ti.Count() && !pE; ti++){
      MYON_BrepTrim& T0 = B.m_T[L.m_ti[ti]];
      MYON_BrepEdge* pEE = T0.Edge();
  ...
  MYON_BrepTrim& T0 = B.m_T[tid0];
  MYON_BrepTrim& T1 = B.m_T[tid1];

  int lid0 = T0.m_li;
  if (lid0 < 0)
    return -1;
  MYON_BrepLoop& L0 = B.m_L[lid0];
  ...
  for (i=0; i<L0.m_ti.Count(); i++){
    const MYON_BrepTrim& T = B.m_T[L0.m_ti[i]];
  ...
  for (i=0; i<L1.m_ti.Count(); i++){
    const MYON_BrepTrim& T = B.m_T[L1.m_ti[i]];
  ...
  MYON_BrepFace* pDF;
  int rc;
  if (L1.m_type != MYON_BrepLoop::inner){
    pL = &L0;
    pD = &L1;
    rc = fid0;
    pF = &F0;
...
{
  bool rc = false;
  MYON_SimpleArray<srf_face> SF(B.m_F.Count());
  int i;
  for (i=0; i<B.m_F.Count(); i++){
    const MYON_BrepFace& F = B.m_F[i];
    if (F.m_face_index < 0)
      continue;
    if (F.m_si < 0)
      continue;
  ...
  while (start_i<SF.Count()){
    ...
    for (i=start_i; i<next_i-1; i++){
      ...
      for (j=i+1; j<next_i; j++){
        int new_id = MYON_BrepMergeFaces(B, SF[i][1], SF[j][1]);
        if (new_id < 0)
          continue;
        SF[j][1] = new_id;
        rc = true;
  ...
  MYON_BrepMergeAllEdges(B);
  return rc;
}


static int MergeAdjacentEdge(MYON_Brep& B, int eid)

{
  MYON_BrepEdge& E = B.m_E[eid];
  if (!E.IsValid())
    return -1;
  ...
  for (i=0; i<2; i++){
    ...
    if (neid >= 0){
      MYON_BrepEdge* pE = B.CombineContiguousEdges(eid, neid);
      if (pE)
...
static bool OrderEdgesAroundClosedVertex(const MYON_Brep& B, int vid, MYON_2dex* trim_ends)

{
  if (vid<0)
    return false;
  const MYON_BrepVertex& V = B.m_V[vid];
  ...
  MYON_2dex& first_te = trim_ends[0];
  const MYON_BrepEdge& FirstEdge = B.m_E[V.m_ei[0]];
  if (FirstEdge.m_ti.Count() != 2)
    return false;
  first_te.i = FirstEdge.m_ti[0];
  const MYON_BrepTrim& FirstTrim = B.m_T[FirstEdge.m_ti[0]];
  ...
  while (count < V.m_ei.Count()){
    const MYON_2dex& lte = trim_ends[count];
    count++;
    int next_ti =  (lte.j) ? B.NextNonsingularTrim(lte.i) : B.PrevNonsingularTrim(lte.i);
    if (next_ti<0)
      return false;
    const MYON_BrepTrim& NT = B.m_T[next_ti];
    const MYON_BrepEdge* pNE = NT.Edge();
    if (!pNE)
      return false;
    if (pNE->m_ti.Count() != 2)
      return false;
    ...
    const MYON_BrepTrim& NTi = B.m_T[nte.i];
...
bool OrderEdgesAroundOpenVertex(const MYON_Brep& B, int vid, MYON_2dex* trim_ends)

{
  ...
  for (i=0; i<V.m_ei.Count(); i++){
    const MYON_BrepEdge& E = B.m_E[V.m_ei[i]];
    if (E.m_edge_index < 0)
      return false;
    if (E.m_ti.Count() == 1){
      if (start < 0)
  ...
  if (start < 0 || end < 0)
    return false;
  const MYON_BrepEdge& StartE = B.m_E[V.m_ei[start]];
  bool bAtStart = (StartE.m_vi[0] == vid) ? true : false;
  const MYON_BrepTrim& StartT = B.m_T[StartE.m_ti[0]];
  MYON_2dex& start_te = trim_ends[0];
   ...
   if (pNE->m_ti.Count() == 1){
     if (NT.m_ei == V.m_ei[end]){
       if (count != V.m_ei.Count()-1)
         return false;
       MYON_2dex& end_te = trim_ends[count];
       end_te.i = next_ti;
       end_te.j = 1-lte.j;
       return true;
     }
...

{
  ...
  return MYON_OrderEdgesAroundVertex(B, vid, trim_ends.Array(), bClosed);
}
