...
#define WIDE_STRING_COMPARE_PREAMBLE(S1,CT1,S2,CT2) STRING_COMPARE_PREAMBLE(wchar_t,MYON_wString::Length,S1,CT1,S2,CT2,0,-1,1)
#define CHAR_STRING_COMPARE_PREAMBLE(S1,CT1,S2,CT2) STRING_COMPARE_PREAMBLE(char,MYON_String::Length,S1,CT1,S2,CT2,0,-1,1)

#define WIDE_STRING_EQUAL_PREAMBLE(S1,CT1,S2,CT2) STRING_COMPARE_PREAMBLE(wchar_t,MYON_wString::Length,S1,CT1,S2,CT2,true,false,false)
#define CHAR_STRING_EQUAL_PREAMBLE(S1,CT1,S2,CT2) STRING_COMPARE_PREAMBLE(char,MYON_String::Length,S1,CT1,S2,CT2,true,false,false)
  ...
  )
{
  switch (map_type)
  {
  case MYON_StringMapType::UpperCase:
    return MYON_StringMapOrdinalType::UpperOrdinal;
  case MYON_StringMapType::LowerCase:
    return MYON_StringMapOrdinalType::LowerOrdinal;
  case MYON_StringMapType::Identity:
    return MYON_StringMapOrdinalType::Identity;
  }
  return MYON_StringMapOrdinalType::Identity;
}

static MYON__UINT32 MapCodePointOrdinal(
  ...
  )
{
  ...
  if (unicode_code_point <= 0x005AU)
  {
    ...
    if (MYON_StringMapOrdinalType::LowerOrdinal == map_type)
    {
  ...
  if (unicode_code_point <= 0x007AU)
  {
    ...
    if (MYON_StringMapOrdinalType::UpperOrdinal == map_type)
    {
      return unicode_code_point - 0x20;
    }

    if (MYON_StringMapOrdinalType::MinimumOrdinal == map_type)
    {
      return unicode_code_point - 0x20;
    }
  ...
  if (unicode_code_point <= 0x00FEU)
  {
    ...
    if ( MYON_StringMapOrdinalType::MinimumOrdinal == map_type || MYON_StringMapOrdinalType::UpperOrdinal == map_type )
      return unicode_code_point - 0x20;
    return unicode_code_point;
  }
  ...
  if (unicode_code_point <= 0x00FFU)
  {
    ...
    if ( MYON_StringMapOrdinalType::UpperOrdinal == map_type && 0x0178U <= maximum_singleton_value )
  ...
  if (unicode_code_point <= 0x017FU)
  {
    ...
    case 0x0130U: // LATIN CAPITAL LETTER I WITH DOT ABOVE
      if (MYON_StringMapOrdinalType::MinimumOrdinal == map_type || MYON_StringMapOrdinalType::LowerOrdinal == map_type)
  ...
  if (unicode_code_point <= 0x03FF)
  {
    ...
    {
      if (MYON_StringMapOrdinalType::UpperOrdinal == map_type && unicode_code_point >= 0x037BU )
    ...
    {
      if ( unicode_code_point == 0x037FU && MYON_StringMapOrdinalType::LowerOrdinal == map_type )
        return 0x03F3U;
      if ( unicode_code_point == 0x03F3U && ( MYON_StringMapOrdinalType::MinimumOrdinal == map_type || MYON_StringMapOrdinalType::UpperOrdinal == map_type ) )
        return 0x03F3U;
      return unicode_code_point;
    }
  ...
  )
{
  if (locale.IsOrdinalOrInvariantCulture())
    return MYON_UnicodeMapCodePointOrdinal(MYON_StringMapOrdinalTypeFromStringMapType(map_type), unicode_code_point);

  wchar_t w[7] = { 0 };
  wchar_t mapped_w[7] = { 0 };
  int w_count = MYON_EncodeWideChar(unicode_code_point, 6,  w);
  if (w_count > 0)
  {
    int mapped_count = MYON_wString::MapString(locale, map_type, w, w_count, mapped_w, sizeof(mapped_w) / sizeof(mapped_w[0]) - 1);
    if (mapped_count > 0)
    {
      MYON__UINT32 mapped_unicode_code_point = unicode_code_point;
      MYON_UnicodeErrorParameters e;
      if ( mapped_count == MYON_DecodeWideChar(w,w_count,&e,&mapped_unicode_code_point) )
  ...
  )
{
  ...
  const MYON_StringMapOrdinalType map_type
    = ( c <= 0x7A && c >= 0x41 && maximum_singleton_value >= 0x7A )
    ? MYON_StringMapOrdinalType::LowerOrdinal
...
{
  return OrdinalUTF16ToIgnoreCase((MYON__UINT16)c);
}
...
{
  return OrdinalUTF32ToIgnoreCase((MYON__UINT32)c);
}
#endif
...
{
  STRING_COMPARE_PREAMBLE(char,MYON_String::Length,string1,element_count1,string2,element_count2,0,-1,1);
...
{
  STRING_COMPARE_PREAMBLE(MYON__UINT16,MYON_StringLengthUTF16,string1,element_count1,string2,element_count2,0,-1,1);
...
{
  STRING_COMPARE_PREAMBLE(MYON__UINT32,MYON_StringLengthUTF32,string1,element_count1,string2,element_count2,0,-1,1);
  ...
  )
{
    ...
    return MYON_StringCompareOrdinalUTF8((const char*)string1, element_count1, (const char*)string2, element_count2, bOrdinalIgnoreCase);
#elif (2 == MYON_SIZEOF_WCHAR_T)
  return MYON_StringCompareOrdinalUTF16((const MYON__UINT16*)string1, element_count1, (const MYON__UINT16*)string2, element_count2, bOrdinalIgnoreCase);
#elif (4 == MYON_SIZEOF_WCHAR_T)
  return MYON_StringCompareOrdinalUTF32((const MYON__UINT32*)string1, element_count1, (const MYON__UINT32*)string2, element_count2, bOrdinalIgnoreCase);
#else
#error MYON_SIZEOF_WCHAR_T is not defined or has an unexpected value
#endif
}

#define MYON_UTF8_PATH_SEPARATOR '/'
static char MYON_NormalizeUTF8PathSepartor(
  char c
  )
{
  return ((c == '/') || (c == '\\')) ? MYON_UTF8_PATH_SEPARATOR : c;
}

#define MYON_WCHAR_PATH_SEPARATOR ((wchar_t)MYON_UTF8_PATH_SEPARATOR)
static wchar_t MYON_NormalizeWideCharPathSepartor(
  wchar_t c
  )
{
  return ((c == ((wchar_t)'/')) || (c == ((wchar_t)'\\'))) ? MYON_WCHAR_PATH_SEPARATOR : c;
}

bool MYON_FileSystemPath::PlatformPathIgnoreCase()
{
...
int MYON_String::ComparePath(
  const char* other_path
  ) const
{
  return MYON_String::ComparePath(m_s,-1,other_path,-1);
}
...
{
  return MYON_String::ComparePath(path1,-1,path2,-1);
}
  ...
  )
{
  ...
  const bool bOrdinalIgnoreCase = MYON_FileSystemPath::PlatformPathIgnoreCase();
  ...
  for(;;)
  {
    ...
    for (;;)
    {
      ...
      {
        c1 = MYON_NormalizeUTF8PathSepartor(*b1++);
        if (null_terminator == c1 || MYON_UTF8_PATH_SEPARATOR == c1)
          break;
      }
      else
      {
    ...
    for (;;)
    {
      ...
      {
        c2 = MYON_NormalizeUTF8PathSepartor(*b2++);
        if (null_terminator == c2 || MYON_UTF8_PATH_SEPARATOR == c2)
          break;
      }
      else
      {
    ...
    if (count1 > 0 || count2 > 0)
    {
      ...
      int rc = MYON_StringCompareOrdinalUTF8(a1, count1, a2, count2, bOrdinalIgnoreCase);
      if (0 != rc)
        return rc;
    }
...
bool MYON_String::EqualPath(
  const char* other_path
  ) const
{
  return MYON_String::EqualPath(m_s,-1,other_path,-1);
}
...
{
  return MYON_String::EqualPath(path1,-1,path2,-1);
}
  ...
  )
{
  ...
  if (bOrdinalIgnoreCase)
  {
    for(i=0; i < element_count; i++)
    {
      c1 = MYON_NormalizeUTF8PathSepartor((char)OrdinalUTF8ToIgnoreCase(*path1++));
      c2 = MYON_NormalizeUTF8PathSepartor((char)OrdinalUTF8ToIgnoreCase(*path2++));
      if ( c1 != c2 )
        return false;
    }
  }
  else
  {
    for(i=0; i < element_count; i++)
    {
      c1 = MYON_NormalizeUTF8PathSepartor(*path1++);
      c2 = MYON_NormalizeUTF8PathSepartor(*path2++);
      if ( c1 != c2 )
        return false;
    }
  }
...
int MYON_wString::ComparePath(
  const wchar_t* other_path
  ) const
{
  return MYON_wString::ComparePath(m_s,-1,other_path,-1);
}
...
{
  return MYON_wString::ComparePath(sPath1,-1,sPath2,-1);
}
  ...
  )
{
  ...
  for(;;)
  {
    ...
    for (;;)
    {
      if ((int)(b1 - path1) < element_count1)
      {
        c1 = MYON_NormalizeWideCharPathSepartor(*b1++);
        if (null_terminator == c1 || MYON_WCHAR_PATH_SEPARATOR == c1)
          break;
      }
      else
      {
    ...
    for (;;)
    {
      if ((int)(b2 - path2) < element_count2)
      {
        c2 = MYON_NormalizeWideCharPathSepartor(*b2++);
        if (null_terminator == c2 || MYON_WCHAR_PATH_SEPARATOR == c2)
          break;
      }
      else
      {
    ...
    if (count1 > 0 || count2 > 0)
    {
      ...
      //  At this point, we should apply a compare that uses the same UTF-8 string normalization
      //  as the file system on the current operating system.
      int rc = MYON_StringCompareOrdinalWideChar(a1, count1, a2, count2, bOrdinalIgnoreCase);
      if (0 != rc)
        return rc;
      if (count1 < count2)
        return -1;
...
bool MYON_wString::EqualPath(
  const wchar_t* other_path
  ) const
{
  return MYON_wString::EqualPath(m_s,-1,other_path,-1);
}
...
{
  return MYON_wString::EqualPath(path1,-1,path2,-1);
}
  ...
  )
{
  ...
  if (bOrdinalIgnoreCase)
  {
    for(i=0; i < element_count; i++)
    {
      c1 = MYON_NormalizeWideCharPathSepartor((wchar_t)OrdinalWideCharToIgnoreCase(*path1++));
      c2 = MYON_NormalizeWideCharPathSepartor((wchar_t)OrdinalWideCharToIgnoreCase(*path2++));
      if ( c1 != c2 )
        return false;
    }
  }
  else
  {
    for(i=0; i < element_count; i++)
    {
      c1 = MYON_NormalizeWideCharPathSepartor(*path1++);
      c2 = MYON_NormalizeWideCharPathSepartor(*path2++);
      if ( c1 != c2 )
        return false;
    }
  }
...
int MYON_String::CompareAttributeName(
  const char* other_name
  ) const
{
  const char* name1 = static_cast< const char* >(*this);
  return MYON_String::CompareAttributeName(name1, other_name );
}
  ...
  )
{
  ...
  return MYON_String::CompareOrdinal(name1, -1, name2, -1, true);
}

int MYON_wString::CompareAttributeName(
  const wchar_t* other_name
  ) const
{
  const wchar_t* name1 = static_cast< const wchar_t* >(*this);
  return MYON_wString::CompareAttributeName(name1, other_name );
}
...
)
{
  ...
  if (code_point < 0x20 || code_point >= 0x7F)
    return MYON_UNSET_UINT_INDEX; // no rank - sort some other way
  ...
  )
{
  ...
  if ( nullptr != name1 && nullptr != name2 )
  {
    ...
    while (rank1 == rank2 && MYON_UNSET_UINT_INDEX != rank1 && 0 != rank1)
    {
      name1++;
      name2++;
    ...
    if (MYON_UNSET_UINT_INDEX != rank1 || MYON_UNSET_UINT_INDEX != rank2)
    {
      if (rank1 < rank2)
        return -1;
      if (rank1 > rank2)
  ...
  return MYON_wString::CompareOrdinal(name1, -1, name2, -1, bIgnoreCase);
}

bool MYON_String::EqualAttributeName(
  const char* other_name
  ) const
{
  const char* name1 = static_cast< const char* >(*this);
  return MYON_String::EqualAttributeName(name1, other_name );
}
...
{
  // TODO - normalize and then use a culture aware ignore case test
  return MYON_String::EqualOrdinal(name1, -1, name2, -1, true);
}

bool MYON_wString::EqualAttributeName(
  const wchar_t* other_name
  ) const
{
  const wchar_t* name1 = static_cast< const wchar_t* >(*this);
  return MYON_wString::EqualAttributeName(name1, other_name );
}
...
{
  // TODO - normalize and then use a culture aware ignore case test
  return MYON_wString::EqualOrdinal(name1, -1, name2, -1, true);
}

int MYON_String::Compare( const char* s ) const
{
  return CompareOrdinal(s,false);
}

int MYON_String::Compare( const unsigned char* s) const
{
  return CompareOrdinal((const char*)s,false);
}

int MYON_String::CompareNoCase( const char* s) const
{
  return CompareOrdinal(s,true);
}

int MYON_String::CompareNoCase( const unsigned char* s) const
{
  return CompareOrdinal((const char*)s,true);
}

bool MYON_String::Equal(
  const MYON_String& other_string,
  const class MYON_Locale& locale,
  bool bIgnoreCase
  ) const
{
  return MYON_String::Equal(
...
{
  return MYON_String::Equal( string1, -1, string2, -1, locale, bIgnoreCase );
}
...
{
  if (locale.IsOrdinal())
    return MYON_String::EqualOrdinal(string1,element_count1,string2,element_count2,bIgnoreCase);
...
int MYON_String::Compare(
  const MYON_String& other_string,
  const class MYON_Locale& locale,
  bool bIgnoreCase
  ) const
{
  return MYON_String::Compare(
    static_cast< const char* >(*this),
    this->Length(),
    static_cast< const char* >(other_string),
    other_string.Length(),
...
{
  return MYON_String::Compare( string1, -1, string2, -1, locale, bIgnoreCase );
}
  ...
  )
{
  ...
  //
  return MYON_String::CompareOrdinal( string1, -1, string2, -1, bIgnoreCase );
  ...
  //{
  //  if ( n < element_count1 )
  //    rc = MYON_String::CompareOrdinal(string1+n,element_count1-n,"",0,bIgnoreCase);
  //  else if ( n < element_count2 )
  //    rc = MYON_String::CompareOrdinal("", 0, string2+n, element_count2-n, bIgnoreCase);
  //}
  //return rc;
#elif defined(MYON_RUNTIME_APPLE)
  // TODO
...
bool MYON_wString::Equal(
  const MYON_wString& other_string,
  const class MYON_Locale& locale,
  bool bIgnoreCase
  ) const
{
  return MYON_wString::Equal(
...
{
  return MYON_wString::Equal(string1,-1,string2,-1,locale,bIgnoreCase);
}
...
{
  if ( locale.IsOrdinal() )
    return MYON_wString::EqualOrdinal(string1, element_count1, string2, element_count2, bIgnoreCase );

#if defined(MYON_RUNTIME_WIN)
  return (0 == MYON_wString::Compare(string1, element_count1, string2, element_count2, locale, bIgnoreCase ));
#elif defined(MYON_RUNTIME_APPLE)
  return (0 == MYON_wString::Compare(string1, element_count1, string2, element_count2, locale, bIgnoreCase ));
#else
  return MYON_wString::EqualOrdinal(string1, element_count1, string2, element_count2, bIgnoreCase );
...
{
  return MYON_wString::Compare( string1, -1, string2, -1, locale, bIgnoreCase );
}
  ...
  )
{
  ...
  if ( locale.IsOrdinal() )
    return MYON_wString::CompareOrdinal(string1,element_count1,string2,element_count2,bIgnoreCase);

#if defined(MYON_RUNTIME_WIN)
  const bool bIsIsInvariantCulture = locale.IsInvariantCulture();
  wchar_t buffer[MYON_Locale::BUFFER_MAXIMUM_CAPACITY];
  ...
  //{
  //  if ( n < element_count1 )
  //    rc = MYON_wString::CompareOrdinal(string1+n,element_count1-n,L"",0,bIgnoreCase);
  //  else if ( n < element_count2 )
  //    rc = MYON_wString::CompareOrdinal(L"", 0, string2+n, element_count2-n, bIgnoreCase);
  //}
  //return rc;
...
bool operator==( const MYON_String& lhs, const char* rhs )
{
  ...
  return (length == MYON_String::Length(rhs))
    && MYON_String::EqualOrdinal(
    static_cast<const char*>(lhs),
    length,
    rhs,
...
bool operator==( const char* lhs, const MYON_String& rhs )
{
  ...
  return (length == MYON_String::Length(lhs))
    && MYON_String::EqualOrdinal(
    lhs,
    length,
    static_cast<const char*>(rhs),
...
bool operator<( const MYON_String& lhs, const MYON_String& rhs )
{
  const int rc = MYON_String::CompareOrdinal(
    static_cast<const char*>(lhs),
    lhs.Length(),
    static_cast<const char*>(rhs),
    rhs.Length(),
...
{
  const int length = lhs.Length();
  return (length == MYON_wString::Length(rhs))
    && MYON_wString::EqualOrdinal(
    static_cast<const wchar_t*>(lhs),
    length,
    rhs,
...
{
  const int length = rhs.Length();
  return (length == MYON_wString::Length(lhs))
    && MYON_wString::EqualOrdinal(
    lhs,
    length,
    static_cast<const wchar_t*>(rhs),
...
bool operator<( const MYON_wString& lhs, const MYON_wString& rhs )
{
  const int rc = MYON_wString::CompareOrdinal(
    static_cast<const wchar_t*>(lhs),
    lhs.Length(),
    static_cast<const wchar_t*>(rhs),
    rhs.Length(),
...
{
  const int length = Length();
  return (length == MYON_String::Length(other_string))
    && MYON_String::EqualOrdinal(
    static_cast< const char* >(*this),
    length,
    other_string,
...
{
  return MYON_String::EqualOrdinal(string1,-1,string2,-1,bOrdinalIgnoreCase);
}
...
bool MYON_wString::EqualOrdinal(
  const MYON_wString& other_string,
  bool bOrdinalIgnoreCase
  ) const
{
...
{
  const int length = Length();
  return (length == MYON_wString::Length(other_string))
    && MYON_wString::EqualOrdinal(
    static_cast< const wchar_t* >(*this),
    length,
    other_string,
...
{
  return MYON_wString::EqualOrdinal(string1,-1,string2,-1,bOrdinalIgnoreCase);
}
...
///////////////////////////////////////////////////////////////////////////////
//
// MYON_String::CompareOrdinal
//

int MYON_String::CompareOrdinal(
  const MYON_String& other_string,
  bool bOrdinalIgnoreCase
  ) const
{
  return MYON_String::CompareOrdinal(
    static_cast< const char* >(*this),
    this->Length(),
    static_cast< const char* >(other_string),
    other_string.Length(),
...
{
  return MYON_String::CompareOrdinal(string1, -1, string2, -1, bOrdinalIgnoreCase);
}
...
{
  return MYON_StringCompareOrdinalUTF8(string1,element_count1,string2,element_count2,bOrdinalIgnoreCase);
}

///////////////////////////////////////////////////////////////////////////////
//
// MYON_wString::CompareOrdinal
//

int MYON_wString::CompareOrdinal(
  const MYON_wString& other_string,
  bool bOrdinalIgnoreCase
  ) const
{
...
{
  return MYON_wString::CompareOrdinal(string1, -1, string2, -1, bOrdinalIgnoreCase);
}
...
{
  return MYON_StringCompareOrdinalWideChar(string1,element_count1,string2,element_count2,bOrdinalIgnoreCase);
}

char MYON_String::MapCharacterOrdinal(
  MYON_StringMapOrdinalType map_type,
  char c
  )
{
  switch (map_type)
  {
  case MYON_StringMapOrdinalType::UpperOrdinal:
  case MYON_StringMapOrdinalType::MinimumOrdinal:
    if (c >= 'a' && c <= 'z')
      return c - 0x20;
    break;
  case MYON_StringMapOrdinalType::LowerOrdinal:
    if (c >= 'A' && c <= 'Z')
      return c + 0x20;
    break;
  default:
...
MYON_String MYON_String::MapStringOrdinal(
  MYON_StringMapOrdinalType map_type
  ) const
{
  MYON_String mapped_string(*this);
  if (mapped_string.IsNotEmpty())
  {
    mapped_string.CopyArray();
    const int length = mapped_string.Length();
    MYON_String::MapStringOrdinal(map_type, mapped_string.m_s, length, mapped_string.m_s, length);
  }
  return mapped_string;
}

int MYON_String::MapStringOrdinal(
  MYON_StringMapOrdinalType map_type,
  const char* string,
  int element_count,
  char* mapped_string,
...
wchar_t MYON_wString::MapCharacterOrdinal(
  MYON_StringMapOrdinalType map_type,
  wchar_t c
  )
{
...
MYON_wString MYON_wString::MapStringOrdinal(
  MYON_StringMapOrdinalType map_type
  ) const
{
  MYON_wString mapped_string(*this);
  if (MYON_StringMapOrdinalType::Identity != map_type && mapped_string.IsNotEmpty())
  {
    mapped_string.CopyArray();
    const int length = mapped_string.Length();
    MYON_wString::MapStringOrdinal(map_type, mapped_string.m_s, length, mapped_string.m_s, length);
  }
  return mapped_string;
}

int MYON_wString::MapStringOrdinal(
  MYON_StringMapOrdinalType map_type,
  ...
  )
{
  ...
  while (string < s1)
    *mapped_string++ = MYON_wString::MapCharacterOrdinal(map_type,*string++);
...
MYON_String MYON_String::MapString(
  const class MYON_Locale& locale,
  MYON_StringMapType map_type
  ) const
{
  return MYON_String::MapString(
    locale,
    map_type,
    static_cast< const char* >(*this),
    Length()
...
MYON_wString MYON_wString::MapString(
  const class MYON_Locale& locale,
  MYON_StringMapType map_type
  ) const
{
  return MYON_wString::MapString(
    locale,
    map_type,
    static_cast< const wchar_t* >(*this),
    Length()
  ...
  )
{
  ...
  int mapped_string_capacity = MYON_String::MapString(locale,map_type,string,element_count,nullptr,0);

  if (mapped_string_capacity > 0)
  {
    MYON_String mapped_string;
    ...
    // Set mapped_string MYON_wString header length value to mapped_length.
    ...
    int rc = MYON_String::MapString(locale,map_type,string,element_count,mapped_string.m_s,mapped_string_capacity);
  ...
  )
{
  ...
  int mapped_string_capacity = MYON_wString::MapString(locale,map_type,string,element_count,nullptr,0);

  if (mapped_string_capacity > 0)
  {
    MYON_wString mapped_string;
    ...
    int rc = MYON_wString::MapString(locale,map_type,string,element_count,mapped_string.m_s,mapped_string_capacity);
    if ( rc == mapped_string_capacity )
      return mapped_string;
  }
  ...
  )
{
  if (mapped_string_capacity < 0)
  {
    MYON_ERROR("mapped_string_capacity is < 0.");
    return 0;
  }
  ...
  else if (mapped_string_capacity > 0)
  {
    ...
    {
      MYON_ERROR("mapped_string is nullptr.");
      return 0;
    }
    if ( string != mapped_string )
      mapped_string[0] = 0;
  ...
  if (false == locale.IsOrdinal())
  {
    ...
    while (s < s1)
    {
      ...
      const MYON_wString wide_string(string, element_count);
      const MYON_wString wide_mapped_string = wide_string.MapString(locale, map_type);
      int mapped_element_count = MYON_ConvertWideCharToUTF8(
      ...
      {
        MYON_ERROR("mapped_string_capacity too small.");
        return 0;
      }
  ...
  {
  case MYON_StringMapType::UpperCase:
    return MYON_String::MapStringOrdinal(MYON_StringMapOrdinalType::UpperOrdinal,string,element_count,mapped_string,mapped_string_capacity);
    break;
  case MYON_StringMapType::LowerCase:
    return MYON_String::MapStringOrdinal(MYON_StringMapOrdinalType::LowerOrdinal,string,element_count,mapped_string,mapped_string_capacity);
    break;
  default:
    MYON_ERROR("invalid map_type.");
    break;
  }
  ...
  )
{
  ...
  while (false == locale.IsOrdinal())
  {
    ...
    MYON_CRT_locale_t loc = locale.StringCollateAndMapLocalePtr();
  ...
  {
  case MYON_StringMapType::UpperCase:
    return MYON_wString::MapStringOrdinal(MYON_StringMapOrdinalType::UpperOrdinal,string,element_count,mapped_string,mapped_string_capacity);
    break;
  case MYON_StringMapType::LowerCase:
    return MYON_wString::MapStringOrdinal(MYON_StringMapOrdinalType::LowerOrdinal,string,element_count,mapped_string,mapped_string_capacity);
    break;
  default:
    MYON_ERROR("invalid map_type.");
    break;
...
void MYON_wString::MakeUpper()
{
  MakeUpperOrdinal();
}

void MYON_wString::MakeLower()
{
  MakeLowerOrdinal();
}

void MYON_String::MakeUpper()
{
  MakeUpperOrdinal();
}

void MYON_String::MakeLower()
{
  MakeLowerOrdinal();
}


void MYON_wString::MakeUpperOrdinal()
{
 	CopyArray();
  const int length = Length();
  if (length > 0)
    MYON_wString::MapStringOrdinal(
    MYON_StringMapOrdinalType::UpperOrdinal,
    m_s,
    length,
    m_s,
    length
...
void MYON_wString::MakeLowerOrdinal()
{
 	CopyArray();
  const int length = Length();
  if (length > 0)
...
void MYON_String::MakeUpperOrdinal()
{
 	CopyArray();
  const int length = Length();
  if (length > 0)
...
void MYON_String::MakeLowerOrdinal()
{
 	CopyArray();
  const int length = Length();
  if (length > 0)
