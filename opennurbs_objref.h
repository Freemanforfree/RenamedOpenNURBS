...
#if !defined(MYON_OBJREF_INC_)
#define MYON_OBJREF_INC_

class MYON_CLASS MYON_ObjRefEvaluationParameter
{
public:
  MYON_ObjRefEvaluationParameter();
  ~MYON_ObjRefEvaluationParameter();
  ...
  // If m_point != MYON_3dPoint::UnsetPoint and m_t_type != 0, then
  ...
  //     m_t_ci is the MYON_BrepTrim component index and m_t[1]
  //     is the MYON_BrepTrim parameter that corresponds to the
...
public:
  double m_t[4];
  MYON_Interval m_s[3]; // curve/surface/cage domains
  MYON_COMPONENT_INDEX m_t_ci; // Not necessarily the same as m_component_index
                             // See comment above for details.
};

class MYON_CLASS MYON_ObjRef_IRefID
{
public:
  MYON_ObjRef_IRefID() = default;
  ~MYON_ObjRef_IRefID() = default;
  MYON_ObjRef_IRefID(const MYON_ObjRef_IRefID&) = default;
  MYON_ObjRef_IRefID& operator=(const MYON_ObjRef_IRefID&) = default;
  ...
  // in its MYON_3dmObjectAttributes.m_uuid.
  MYON_UUID  m_iref_uuid = MYON_nil_uuid;

  // m_iref_xform is the value stored in MYON_InstanceRef.m_xform.
  MYON_Xform m_iref_xform = MYON_Xform::ZeroTransformation;

  // m_idef_uuid is the instance definition id stored in
  // MYON_InstanceRef.m_instance_definition_uuid and
  // MYON_InstanceDefinition.m_uuid.
  MYON_UUID  m_idef_uuid = MYON_nil_uuid;

  // m_geometry_index is the index of the uuid of the pertinent
  // piece of geometry in the MYON_InstanceRef.m_object_uuid[] 
  ...
  MYON_Xform m_geometry_xform = MYON_Xform::ZeroTransformation;

  // If this MYON_ObjRef_IRefID is the first entry in the 
  // MYON_ObjRef.m__iref[] array, then it references a "real"
  ...
  MYON_ObjRefEvaluationParameter m_evp;
};

#if defined(MYON_DLL_TEMPLATE)
MYON_DLL_TEMPLATE template class MYON_CLASS MYON_SimpleArray<MYON_ObjRef_IRefID>;
#endif

class MYON_CLASS MYON_ObjRef
{
public:
  MYON_ObjRef();
  MYON_ObjRef(const MYON_ObjRef& src);
  MYON_ObjRef& operator=(const MYON_ObjRef& src);
  ~MYON_ObjRef();
  ...
  // CRhinoObject id is stored on MYON_3dmObjectAttributes.m_uuid.
  MYON_UUID m_uuid;

  // The m_geometry and m_parent_geometry pointers are runtime values
  // that point to the object being referenced.  The destructor 
  // ~MYON_ObjRef  does not delete the objects these pointers reference.
  //
  ...
  //   geometry the MYON_ObjRef refers to.
  //
  //   For example if the MYON_ObjRef is to an edge of a brep in
  //   and instance reference, m_uuid would be the Rhino id of
  //   the CRhinoInstanceObject, m_parent_geometry would point
  //   to a, possibly proxy, MYON_Brep object, m_geometry would point
  //   to the MYON_BrepEdge in the MYON_Brep, m_component_index would
  //   record the edge's index in the MYON_Brep.m_E[] array and 
  //   m_geometry_type would be MYON::curve_object or MYON::brep_edge.
  ...
  const MYON_Geometry* m_geometry;
  const MYON_Geometry* m_parent_geometry;
  MYON_COMPONENT_INDEX m_component_index;
  int m_geometry_type;
  ...
  // but an MYON_ObjRef continues to reference the Rhino object.
  ...
  // and instance reference (MYON_InstanceRef/CRhinoInstanceObject)
  ...
  MYON_SimpleArray<MYON_ObjRef_IRefID> m__iref;

  /*
  Description:
    Expert user tool to decrement reference counts.  Most
    users will never need to call this tool.  It is called
    by ~MYON_ObjRef and used in rare cases when a
    MYON_ObjRef needs to reference an object only by uuid
    and component index.
  */
  ...
  /*
  Description:
    Expert user tool to initialize the MYON_ObjRef 
  ...
  */
  ...
  bool SetParentIRef( const MYON_InstanceRef& iref,
                      MYON_UUID iref_id,
  ...
  /*
  Returns:
     0:  This MYON_ObjRef is not counting references.
  ...
  */
  const MYON_Object* ProxyObject(int proxy_object_index) const;
  ...
  */
  void RemapObjectId( const MYON_SimpleArray<MYON_UuidPair>& uuid_remap );

private:
  // In simple (and the most common) cases where m_geometry
  // is managed by something outside of the MYON_ObjRef class,
  ...
  // counts the number of MYON_ObjRef classes that refer to m__proxy1/2.
  // When the last MYON_ObjRef is destroyed, m__proxy1/2 is deleted.
  // When the MYON_ObjRef is using reference counting and managing
  ...
  // MYON_MeshVertex/Edge/FaceRefs and CRhinoPolyEdges
  // and m__proxy2 to store transformed copies if instance
  // definition geometry.  
  MYON_Object* m__proxy1;
  MYON_Object* m__proxy2;
  int* m__proxy_ref_count;
  //MYON__INT_PTR m_reserved;
};

#if defined(MYON_DLL_TEMPLATE)
MYON_DLL_TEMPLATE template class MYON_CLASS MYON_ClassArray<MYON_ObjRef>;
#endif
