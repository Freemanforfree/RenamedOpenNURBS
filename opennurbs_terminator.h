...
class MYON_CLASS MYON_Terminator
{
public:
  MYON_Terminator();
  ~MYON_Terminator();
  ...
  Example:
            ...
            bool StopCalculation( MYON__UINT_PTR context )
            {
              if ( 0 != context )
              {
            ...
            terminator.SetTerminationQueryCallbackFunction(StopCalculation,(MYON__UINT_PTR)(&bStopCalculation));
            ...
  ...
  */
  ...
  MYON__UINT_PTR CallbackFunction() const;
  MYON__UINT_PTR CallbackContext() const;

  void SetThreadId(
    MYON__UINT_PTR thread_id
    );

  MYON__UINT_PTR ThreadId() const;
  ...
  */
  ...
  /*
  Description:
    A calculation calls MYON_Terminator::TerminationRequested(terminator) 
    to determine if it should continue or quit.
  Parameters:
    terminator - [in]
      A pointer to an MYON_Terminator or null pointer.
  Returns:
    True if the calculation should terminate as soon as possible.
    False if the calculation should continue.
  Example:    
            void MyLongCalculation( ..., MYON_Terminator* terminator, ...)
            {
               for ( i = 0; i < count; i++ )
               {
                 if ( MYON_Terminator::TerminationRequested(terminator) )
                   break;
                 ...
               }
            }
  */
  static bool TerminationRequested( MYON_Terminator* terminator);
  ...
  Parameters:
    ...
    callback_delta - [in]
      ...
      MYON_Terminator::TerminationRequested( terminator ) uses a value of 
  ...
  Returns:
    ...
    False if the calculation should continue.
  Remarks:
    In general, call the MYON_Terminator::TerminationRequested( terminator ).
  */
...
private:
  bool (*m_callback_function)(MYON__UINT_PTR) = nullptr;
  MYON__UINT_PTR m_callback_context = 0;
  MYON__UINT64 m_previous_query_clock = 0;
  bool m_bTerminationRequested = false;
  bool m_bAllowDefaultCancellationHandling = true;
  bool m_reserved1[6];
  MYON__UINT_PTR m_thread_id = 0;
  MYON__UINT64 m_reserved2[2];
};
