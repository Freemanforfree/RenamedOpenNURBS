...
/////////////////////////////////////////////////////////////
//
// Calculation use MYON_ProgressReporter to 
// report progress to the calling code.
//
class MYON_CLASS MYON_ProgressReporter
{
public:
  MYON_ProgressReporter();
  ~MYON_ProgressReporter();
  ...
  */
  ...
  /*
  Description:
    The calculation calls MYON_ProgressReporter::ReportProgress to report
  ...
  Parameters:
    progress_reporter - [in]
      A pointer to an MYON_ProgressReporter or null pointer.
    fraction_complete - [in]
  ...
  Example:    
            void MyLongCalculation( ..., MYON_ProgressReporter* pr, ...)
            {
               ...
               for ( i = 0; i < count; i++ )
               {
                 MYON_ProgressReporter::ReportProgress(pr,i/((double)count));   
                 ...
               }
               MYON_ProgressReporter::ReportProgress(pr,1.0); // finished
               ...
            }
  */
  ...
  Example:    
            void MyLongCalculation( ..., MYON_ProgressReporter* pr, ...)
            {
               ...
               for ( i = 0; i < count; i++ )
               {
                 MYON_ProgressReporter::ReportProgress(pr,i,count);   
                 ...
               }
               MYON_ProgressReporter::ReportProgress(pr,1.0); // finished
               ...
  ...
  */
  ...
  /*
  Description:
    The calculation may call MYON_ProgressReporter::FractionComplete to get
    the current fraction completed.
  Parameters:
    progress_reporter - [in]
      A pointer to an MYON_ProgressReporter or null pointer.
  Returns:
    MYON_UNSET_VALUE is returned when progress_reporter is nullptr.
...
private:
  void (*m_callback_function)(MYON__UINT_PTR,double);
  MYON__UINT_PTR m_callback_context;
  ...
  MYON__UINT64 m_reserved[4];
};

class MYON_CLASS MYON_ProgressStepCounter
{
public:

  // This class makes it easy for functions to use a progress reporter.
  MYON_ProgressStepCounter() = default;
  ~MYON_ProgressStepCounter() = default;
  MYON_ProgressStepCounter(const MYON_ProgressStepCounter&) = default;
  MYON_ProgressStepCounter& operator=(const MYON_ProgressStepCounter&) = default;

  /*
  Description:
    When you have a process that needs to call a MYON_ProgressReporter::ReportProgress()
    a reasonable number of times and you can calculate the number of steps your process 
    takes, then MYON_ProgressStepCounter will handle all the details.
  Parameters:
    progress_reporter - [in]
      Parameter passed to MYON_ProgressReporter::ReportProgress().
    step_count - [in]
      Number of steps.
  Remarks:
    This will create a MYON_ProgressStepCounter that will call 
    MYON_ProgressReporter::ReportProgress(progress_reporter) at most 101 times.
    The first call reports progress 0.0.
    The last call reports progress 1.0.
  */
  static MYON_ProgressStepCounter Create(
    MYON_ProgressReporter* progress_reporter,
  ...
  Parameters:
    ...
    maximum_progress_reports - [in]
      Maximum number of times to call MYON_ProgressReporter::ReportProgress()
  ...
  */
  ...
  static const MYON_ProgressStepCounter Empty;
