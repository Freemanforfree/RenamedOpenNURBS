...
MYON_OBJECT_IMPLEMENT(MYON_DetailView,MYON_Geometry,"C8C66EFA-B3CB-4e00-9440-2AD66203379E");

MYON_DetailView::MYON_DetailView()
{
...
void MYON_DetailView::MemoryRelocate()
{
  m_boundary.MemoryRelocate();
}

bool MYON_DetailView::IsValid( MYON_TextLog* text_log ) const
{
...
void MYON_DetailView::Dump( MYON_TextLog& text_log ) const
{
  m_view.Dump(text_log);
  m_boundary.Dump(text_log);
}

unsigned int MYON_DetailView::SizeOf() const
{
...
bool MYON_DetailView::Write( MYON_BinaryArchive& archive ) const
{
  ...
  for(;;)
  {
    // m_view is wrapped in a subchunk so MYON_3dmView can be expanded 
    ...
    // m_boundary is wrapped in a subchunk so MYON_NurbsCurve can be expanded 
    // without breaking the file format.
    rc = archive.BeginWrite3dmChunk( TCODE_ANONYMOUS_CHUNK, 1, 0 );
    if (rc) 
    {
...
bool MYON_DetailView::Read(MYON_BinaryArchive& archive)
{
  m_page_per_model_ratio = 0.0;
  m_view.Default();
  m_boundary.Destroy();
...
MYON::object_type MYON_DetailView::ObjectType() const
{
  return MYON::detail_object;
}

int MYON_DetailView::Dimension() const
{
  return m_boundary.Dimension();
}

bool MYON_DetailView::GetBBox(
    double* boxmin,
    double* boxmax,
    bool bGrowBox
    ) const
...
bool MYON_DetailView::GetTightBoundingBox( 
      MYON_BoundingBox& tight_bbox, bool bGrowBox, const MYON_Xform* xform
      ) const
{
  return m_boundary.GetTightBoundingBox(tight_bbox,bGrowBox,xform);
}

bool MYON_DetailView::Transform( const MYON_Xform& xform )
{
  return m_boundary.Transform(xform);
}

bool MYON_DetailView::UpdateFrustum(
...
)
{
  ...
  double detail_width_on_paper_mm = detail_width_on_paper * MYON::UnitScale(paper_units, MYON::LengthUnitSystem::Millimeters);
  if (!(detail_width_on_paper_mm > 0.0))
    return false;

  double frustum_width_mm = detail_width_on_paper_mm / m_page_per_model_ratio;
  double frustum_width = frustum_width_mm * MYON::UnitScale(MYON_UnitSystem(MYON::LengthUnitSystem::Millimeters), model_units);
