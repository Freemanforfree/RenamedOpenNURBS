...
#if !defined(MYON_SUN_INC_)
#define MYON_SUN_INC_

class MYON_CLASS MYON_Sun final
{
public:
  MYON_Sun();
  MYON_Sun(MYON_XMLNode& model_node);
  MYON_Sun(const MYON_Sun& sun);
  ~MYON_Sun();
  ...
  const MYON_Sun& operator = (const MYON_Sun& sun);

  bool operator == (const MYON_Sun& sun);
  bool operator != (const MYON_Sun& sun);
  ...
  MYON_3dVector Vector(void) const;
  ...
  void SetVector(const MYON_3dVector& v);
