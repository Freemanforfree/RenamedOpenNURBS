...
Adding several fields to MYON_Dimstyle
...
// Userdata class being used to extend MYON_V5x_DimStyle so the v4 sdk still works
// Presumably, this will be moved into MYON_V5x_DimStyle when the SDK is changed again
// Don't put this extension class in a header file or export it.

// April 11, 2014 - Lowell
// Moving these fields into MYON_Dimstyle
// MYON_DimStyleExtra class is still here for writing V5 files
class MYON_DimStyleExtra : public MYON_UserData
{
  MYON_OBJECT_DECLARE(MYON_DimStyleExtra);
public:
//  static MYON_DimStyleExtra* DimStyleExtensionGet( MYON_V5x_DimStyle* pDimStyle, bool bCreateIfNoneExists );
//  static const MYON_DimStyleExtra* DimStyleExtensionGet( const MYON_V5x_DimStyle* pDimStyle);
  ...
  /*
  Returns:
    True if this MYON_DimStyleExtra has default settings.
  */
  ...
  // 1: Use the MYON_Color returned by MaskColor()
  ...
  bool CompareFields(const MYON_DimStyleExtra* pOther) const;

  // Data storage

  MYON_UUID m_parent_dimstyle;  // MYON_nil_uuid if there is no parent dimstyle
  MYON_SimpleArray<bool> m_valid_fields;
  enum { eFieldCount = 66 };
  ...
  MYON_UUID  m_source_dimstyle;
};



// Added for v5 - 5/01/07 LW
MYON_OBJECT_IMPLEMENT(MYON_DimStyleExtra,MYON_UserData,"513FDE53-7284-4065-8601-06CEA8B28D6F");

//MYON_DimStyleExtra* MYON_DimStyleExtra::DimStyleExtensionGet( MYON_V5x_DimStyle* pDimStyle, bool bCreateIfNoneExists )
//{
//  MYON_DimStyleExtra* pExtra = 0;
//  if( pDimStyle)
//  {
//    pExtra = MYON_DimStyleExtra::Cast( pDimStyle->GetUserData( MYON_CLASS_ID(MYON_DimStyleExtra)));
...
//    //   MYON_DimStyleExtra unless it is actually being used
...
//
//const 
//MYON_DimStyleExtra* MYON_DimStyleExtra::DimStyleExtensionGet( const MYON_V5x_DimStyle* pDimStyle)
//{
//  // Please do not changes the "false" to a "true" in the second argument.
//  return MYON_DimStyleExtra::DimStyleExtensionGet( (MYON_V5x_DimStyle*)pDimStyle, false );
//}

MYON_DimStyleExtra::MYON_DimStyleExtra()
{
  m_userdata_uuid = MYON_CLASS_ID(MYON_DimStyleExtra);
  ...
  m_valid_fields.Reserve( MYON_DimStyleExtra::eFieldCount);
  m_valid_fields.SetCount( MYON_DimStyleExtra::eFieldCount);
  m_parent_dimstyle = MYON_nil_uuid;
  m_source_dimstyle = MYON_nil_uuid;
  SetDefaults();
}
...
void MYON_DimStyleExtra::SetDefaults()
{
  m_tolerance_style = MYON_V5x_DimStyle::DefaultToleranceStyle();
  m_tolerance_resolution = MYON_V5x_DimStyle::DefaultToleranceResolution();
  m_tolerance_upper_value = MYON_V5x_DimStyle::DefaultToleranceUpperValue();
  m_tolerance_lower_value = MYON_V5x_DimStyle::DefaultToleranceLowerValue();
  m_tolerance_height_scale = MYON_V5x_DimStyle::DefaultToleranceHeightScale();
  m_baseline_spacing = MYON_V5x_DimStyle::DefaultBaselineSpacing();
  m_bDrawMask = MYON_V5x_DimStyle::DefaultDrawTextMask(); // false;
  m_mask_color_source = MYON_V5x_DimStyle::DefaultMaskColorSource(); // 0;
  m_mask_color = MYON_V5x_DimStyle::DefaultMaskColor(); // .SetRGB(255,255,255);
  m_dimscale = MYON_V5x_DimStyle::DefaultDimScale(); // 1.0;
  m_dimscale_source = MYON_V5x_DimStyle::DefaultDimScaleSource(); // 0;

  for( int i = 0; i < m_valid_fields.Count(); i++)
    m_valid_fields[i] = false;
}

bool MYON_DimStyleExtra::IsDefault() const
{
  if ( m_tolerance_style != MYON_V5x_DimStyle::DefaultToleranceStyle() ) return false;
  if ( m_tolerance_resolution != MYON_V5x_DimStyle::DefaultToleranceResolution() ) return false;
  if ( m_tolerance_upper_value != MYON_V5x_DimStyle::DefaultToleranceUpperValue() ) return false;
  if ( m_tolerance_lower_value != MYON_V5x_DimStyle::DefaultToleranceLowerValue() ) return false;
  if ( m_tolerance_height_scale != MYON_V5x_DimStyle::DefaultToleranceHeightScale() ) return false;
  if ( m_baseline_spacing != MYON_V5x_DimStyle::DefaultBaselineSpacing() ) return false;
  if ( m_bDrawMask != MYON_V5x_DimStyle::DefaultDrawTextMask() ) return false;
  if ( m_mask_color_source != MYON_V5x_DimStyle::DefaultMaskColorSource() ) return false;
  if ( m_mask_color != MYON_V5x_DimStyle::DefaultMaskColor() ) return false;
  if ( m_dimscale != MYON_V5x_DimStyle::DefaultDimScale() ) return false;
  if ( m_dimscale_source != MYON_V5x_DimStyle::DefaultDimScaleSource() ) return false;

  // The m_valid_fields[] settings only matter when
  // m_parent_dimstyle is not zero.
  if ( !(m_parent_dimstyle == MYON_nil_uuid) )
  {
    for( int i = 0; i < m_valid_fields.Count() && i < MYON_DimStyleExtra::eFieldCount; i++)
    {
      if ( !m_valid_fields[i] )
        return false;
    }
...
void MYON_DimStyleExtra::Dump( MYON_TextLog& text_log ) const
{
  // do nothing
}

unsigned int MYON_DimStyleExtra::SizeOf() const
{
...
bool MYON_DimStyleExtra::Write(MYON_BinaryArchive& archive) const
{
...
bool MYON_DimStyleExtra::Read(MYON_BinaryArchive& archive)
{
...
bool MYON_DimStyleExtra::DeleteAfterWrite(
  const class MYON_BinaryArchive& archive,
  const class MYON_Object* parent_object
) const
{
...
bool MYON_DimStyleExtra::DeleteAfterRead(
  const class MYON_BinaryArchive& archive,
  class MYON_Object* parent_object
) const
{
  // Move settings to MYON_V5x_DimStyle
  for (;;)
  {
    MYON_V5x_DimStyle* dimstyle = MYON_V5x_DimStyle::Cast(parent_object);
    if (nullptr == dimstyle)
      break;

    if (MYON_nil_uuid == dimstyle->ParentId())
    ...
    if (0 == dimstyle->m_field_override_count)
    {
      ...
      if ( count > static_cast<unsigned int>(MYON_V5x_DimStyle::Field::FieldCount) )
        count = static_cast<unsigned int>(MYON_V5x_DimStyle::Field::FieldCount);
      for (unsigned int i = 0; i < count && i < capacity; i++)
      {
        bool bOverrideParentSetting 
          = (i > static_cast<unsigned int>(MYON_V5x_DimStyle::Field::fn_index) && i <= static_cast<unsigned int>(MYON_V5x_DimStyle::Field::fn_dim_arrow_blockname2) && this->m_valid_fields[i])
          ? true 
          : false;
...
bool MYON_DimStyleExtra::GetDescription( MYON_wString& description)
{
  description = L"Userdata extension of MYON_V5x_DimStyle";
  return true;
}

bool MYON_DimStyleExtra::Archive() const
{
  // true to write to file
  return true;
}

void MYON_DimStyleExtra::Scale( double scale)
{
...
// Tolerance style
void MYON_DimStyleExtra::SetToleranceStyle( int style)
{
  if( style >= 0 && style <= 4)
    m_tolerance_style = style;
}

int MYON_DimStyleExtra::ToleranceStyle() const
{
  return m_tolerance_style;
}

void MYON_DimStyleExtra::SetToleranceResolution( int resolution)
{
  if( resolution >= 0 && resolution < 16)
    m_tolerance_resolution = resolution;
}

int MYON_DimStyleExtra::ToleranceResolution() const
{
  return m_tolerance_resolution;
}

void MYON_DimStyleExtra::SetToleranceUpperValue( double upper_value)
{
  if( MYON_IsValid(upper_value))
    m_tolerance_upper_value = upper_value;
}

double MYON_DimStyleExtra::ToleranceUpperValue() const
{
  return m_tolerance_upper_value;
}

void MYON_DimStyleExtra::SetToleranceLowerValue( double lower_value)
{
  if( MYON_IsValid(lower_value))
    m_tolerance_lower_value = lower_value;
}

double MYON_DimStyleExtra::ToleranceLowerValue() const
{
  return m_tolerance_lower_value;
}

void MYON_DimStyleExtra::SetToleranceHeightScale( double scale)
{
  if( MYON_IsValid( scale) && scale > MYON_SQRT_EPSILON)
    m_tolerance_height_scale = scale;
}

double MYON_DimStyleExtra::ToleranceHeightScale() const
{
  return m_tolerance_height_scale;
}

void MYON_DimStyleExtra::SetBaselineSpacing( double spacing)
{
  if( MYON_IsValid( spacing) && spacing > MYON_SQRT_EPSILON)
    m_baseline_spacing = spacing;
}

double MYON_DimStyleExtra::BaselineSpacing() const
{
  return m_baseline_spacing;
}

bool MYON_DimStyleExtra::DrawTextMask() const
{
  return m_bDrawMask;
}

void MYON_DimStyleExtra::SetDrawTextMask(bool bDraw)
{
  m_bDrawMask = bDraw ? true : false;
}

int MYON_DimStyleExtra::MaskColorSource() const
{
  return m_mask_color_source;
}

void MYON_DimStyleExtra::SetMaskColorSource(int source)
{
  if(source == 1)
...
MYON_Color MYON_DimStyleExtra::MaskColor() const
{
  return m_mask_color;
}

void MYON_DimStyleExtra::SetMaskColor(MYON_Color color)
{
  m_mask_color = color;
}

void MYON_DimStyleExtra::SetDimScale(double scale)
{
  m_dimscale = scale;
}

double MYON_DimStyleExtra::DimScale() const
{
  return m_dimscale;
}

void MYON_DimStyleExtra::SetDimScaleSource(int source)
{
  m_dimscale_source = source;
}

int MYON_DimStyleExtra::DimScaleSource() const
{
  return m_dimscale_source;
}

void MYON_DimStyleExtra::SetSourceDimstyle(MYON_UUID source_uuid)
{
  m_source_dimstyle = source_uuid;
}

MYON_UUID MYON_DimStyleExtra::SourceDimstyle() const
{
  return m_source_dimstyle;
}

// returns true if they are the same
bool MYON_DimStyleExtra::CompareFields(const MYON_DimStyleExtra* pOther) const
{
  if(pOther == 0)
    return false;
...
MYON_OBJECT_IMPLEMENT( MYON_V5x_DimStyle, MYON_Object, "81BD83D5-7120-41c4-9A57-C449336FF12C" );
...
// copy from MYON_3dmAnnotationSettings and add a couple of fields
MYON_V5x_DimStyle::MYON_V5x_DimStyle( const MYON_3dmAnnotationSettings& src)
  : MYON_ModelComponent(MYON_ModelComponent::Type::DimStyle)
{
  m_extextension       = src.m_dimexe;
  m_extoffset          = src.m_dimexo;
  m_arrowsize          = src.m_arrowlength;
  m_dimstyle_textalign = MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode::kAboveLine;
...
bool MYON_V5x_DimStyle::CompareValidFields(const MYON_V5x_DimStyle& style) const
{
...
bool MYON_V5x_DimStyle::CompareDimstyle(const MYON_V5x_DimStyle& style) const
{
  if (
    ...
    //MYON_TextStyle::EqualTextStyleFontAndName(m_text_style, style.m_text_style) &&
...
bool MYON_V5x_DimStyle::IsValid( MYON_TextLog* text_log ) const
{
  return ( Name().Length() > 0 && Index() >= 0);
}

void MYON_V5x_DimStyle::Dump( MYON_TextLog& dump ) const
{
...
bool MYON_V5x_DimStyle::AttachDimstyleExtra()
{
  MYON_DimStyleExtra* pExtra = 0;
  // Get or make userdata
  // Look first so it only gets put on once.
  pExtra = MYON_DimStyleExtra::Cast(GetUserData(MYON_CLASS_ID(MYON_DimStyleExtra)));
  if (pExtra == 0)
  {
    pExtra = new MYON_DimStyleExtra;
    if (pExtra)
  ...
  if (0 != pExtra)
  {
    ...
    pExtra->m_valid_fields.Reserve(MYON_DimStyleExtra::eFieldCount);
    pExtra->m_valid_fields.SetCount(MYON_DimStyleExtra::eFieldCount);
    pExtra->m_valid_fields.Zero();
    for (int i = 0; i < MYON_DimStyleExtra::eFieldCount; i++)
    {
...
//void MYON_V5x_DimStyle::ConsolidateDimstyleExtra()
//{
//  MYON_DimStyleExtra* extra = MYON_DimStyleExtra::Cast(this->GetUserData(MYON_CLASS_ID(MYON_DimStyleExtra)));
//  if (extra)
//  {
  ...
  ) const
{
  ...
  if (file.Archive3dmVersion() == 5 || file.Archive3dmVersion() == 50)
  {
    ...
    const_cast<MYON_V5x_DimStyle*>(this)->AttachDimstyleExtra();
  }
...
bool MYON_V5x_DimStyle::Read(
  MYON_BinaryArchive& file // restore definition from binary archive
  )
{
  // MYON_V5x_DimStyle::Default cannot be a static member
  ...
  // before the obsolete MYON_V5x_DimStyle can be initialzed.
  // The MYON_V5x_DimStyle::Default() function is called only when files are 
  // read and by that time MYON_DimStyle::Default exists.
  const MYON_V5x_DimStyle unset_dimstyle(MYON::LengthUnitSystem::None, MYON_DimStyle::Unset);
  *this = unset_dimstyle;
...
bool MYON_V5x_DimStyle::Internal_Read_v6(
  MYON_BinaryArchive& file // restore definition from binary archive
  )
{
  ...
  if (major_version >= 1)
  {
    ...
    if (rc) m_dimstyle_textalign = (MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode)ui;
    ...
    if (minor_version >= 2)
    {
      ...
      if (minor_version >= 3)
      {
        MYON_UUID dimstyle_id = Id();
        if (rc) rc = file.ReadUuid(dimstyle_id);
        if (rc)
        {
          if (MYON_nil_uuid == dimstyle_id)
            dimstyle_id = MYON_CreateId(); // non nil id is required
          SetId(dimstyle_id);
        }
      }
    }
    ...
    if (minor_version >= 6)
    {
      ...
      MYON_SimpleArray<bool> valid_fields;
      ...
      if (rc)
      {
        ...
        {
          MYON__UINT64 len = c.LengthRemaining(file.CurrentPosition());
          if ( 16 == len )
...
bool MYON_V5x_DimStyle::Write_v5(
       MYON_BinaryArchive& file // serialize definition to binary archive
     ) const
{
  // March 22, 2010 - Global DimStyle was obsoleted and moved into DimStyles
...
void MYON_Internal_FixBogusDimStyleLengthFactor(
...
)
{
  if (1.0 == dimstyle_length_factor)
    return;
  if (false == MYON_IsValid(dimstyle_length_factor))
  {
  ...
  if (MYON_VersionNumberParse(opennurbs_version,nullptr,nullptr,&version_year,&version_month,&version_day_of_month,nullptr))
  {
...
bool MYON_V5x_DimStyle::Internal_Read_v5(
  MYON_BinaryArchive& file // restore definition from binary archive
  )
{
  ClearModelComponentAttributes(MYON_ModelComponent::Attributes::AllAttributes);
...
void MYON_V5x_DimStyle::EmergencyDestroy()
{
...
double MYON_V5x_DimStyle::ExtExtension() const
{
  return m_extextension;
}

void MYON_V5x_DimStyle::SetExtExtension( const double e)
{
  // Allow negative?
  m_extextension = e;
}

double MYON_V5x_DimStyle::ExtOffset() const
{
  return m_extoffset;
}

void MYON_V5x_DimStyle::SetExtOffset( const double e)
{
  m_extoffset = e;
}

double MYON_V5x_DimStyle::ArrowSize() const
{
  return m_arrowsize;
}

void MYON_V5x_DimStyle::SetArrowSize( const double e)
{
  m_arrowsize = e;
}

double MYON_V5x_DimStyle::LeaderArrowSize() const
{
  return m_leaderarrowsize;
}

void MYON_V5x_DimStyle::SetLeaderArrowSize( const double e)
{
  m_leaderarrowsize = e;
}

double MYON_V5x_DimStyle::CenterMark() const
{
  return m_centermark;
}

void MYON_V5x_DimStyle::SetCenterMark( const double e)
{
  m_centermark = e;
}

MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode MYON_V5x_DimStyle::TextAlignment() const
{
  return m_dimstyle_textalign;
}

void MYON_V5x_DimStyle::SetTextAlignment( MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode a)
{
  m_dimstyle_textalign = a;
}

int MYON_V5x_DimStyle::ArrowType() const
{
  return m_arrowtype;
}

void MYON_V5x_DimStyle::SetArrowType( eArrowType a)
{
  m_arrowtype = a;
}

int MYON_V5x_DimStyle::LeaderArrowType() const
{
  return m_leaderarrowtype;
}

void MYON_V5x_DimStyle::SetLeaderArrowType( eArrowType a)
{
  m_leaderarrowtype = a;
}

int MYON_V5x_DimStyle::AngularUnits() const
{
  return m_angularunits;
}

void MYON_V5x_DimStyle::SetAngularUnits( int u)
{
  m_angularunits = u;
}

int MYON_V5x_DimStyle::LengthFormat() const
{
  return m_lengthformat;
}

void MYON_V5x_DimStyle::SetLengthFormat( int f)
{
  m_lengthformat = f;
}

int MYON_V5x_DimStyle::AngleFormat() const
{
  return m_angleformat;
}

void MYON_V5x_DimStyle::SetAngleFormat( int f)
{
  m_angleformat = f;
}

int MYON_V5x_DimStyle::LengthResolution() const
{
  return m_lengthresolution;
}

void MYON_V5x_DimStyle::SetLengthResolution( int r)
{
  if( r >= 0 && r < 16)
  {
...
int MYON_V5x_DimStyle::AngleResolution() const
{
  return m_angleresolution;
}

void MYON_V5x_DimStyle::SetAngleResolution( int r)
{
  if( r >= 0 && r < 16)
  {
    m_angleresolution = r;
...
const MYON_TextStyle& MYON_V5x_DimStyle::V5TextStyle() const
{
  return m_v5_text_style;
}

void MYON_V5x_DimStyle::SetV5TextStyle(
  const MYON_TextStyle& text_style
  )
{
  m_v5_text_style = text_style;
}

double MYON_V5x_DimStyle::TextGap() const
{
  return m_textgap;
}

void MYON_V5x_DimStyle::SetTextGap( double gap)
{
  if( gap >= 0.0)
  {
    m_textgap = gap;
...
double MYON_V5x_DimStyle::TextHeight() const
{
  return m_textheight;
}

void MYON_V5x_DimStyle::SetTextHeight( double height)
{
...
double MYON_V5x_DimStyle::LengthFactor() const
{
  return m_lengthfactor;
}

void MYON_V5x_DimStyle::SetLengthFactor( double factor)
{
  m_lengthfactor = factor;
}

bool MYON_V5x_DimStyle::Alternate() const
{
  return m_bAlternate;
}
void MYON_V5x_DimStyle::SetAlternate( bool bAlternate)
{
  m_bAlternate = bAlternate;
}

double MYON_V5x_DimStyle::AlternateLengthFactor() const
{
  return m_alternate_lengthfactor;
}

void MYON_V5x_DimStyle::SetAlternateLengthFactor( double factor)
{
  m_alternate_lengthfactor = factor;
}

int MYON_V5x_DimStyle::AlternateLengthFormat() const
{
  return m_alternate_lengthformat;
}
void MYON_V5x_DimStyle::SetAlternateLengthFormat( int format)
{
  m_alternate_lengthformat = format;
}

int MYON_V5x_DimStyle::AlternateLengthResolution() const
{
  return m_alternate_lengthresolution;
}
void MYON_V5x_DimStyle::SetAlternateLengthResolution( int resolution)
{
  m_alternate_lengthresolution = resolution;
}

int MYON_V5x_DimStyle::AlternateAngleFormat() const
{
  return m_alternate_angleformat;
}
void MYON_V5x_DimStyle::SetAlternateAngleFormat( int format)
{
  m_alternate_angleformat = format;
}

int MYON_V5x_DimStyle::AlternateAngleResolution() const
{
  return m_alternate_angleresolution;
}
void MYON_V5x_DimStyle::SetAlternateAngleResolution( int resolution)
{
  m_alternate_angleresolution = resolution;
}

void MYON_V5x_DimStyle::GetPrefix( MYON_wString& prefix) const
{
  prefix = m_prefix;
}
const wchar_t* MYON_V5x_DimStyle::Prefix() const
{
  return static_cast< const wchar_t* >(m_prefix);
}
void MYON_V5x_DimStyle::SetPrefix( wchar_t* prefix)
{
  m_prefix = prefix;
}
void MYON_V5x_DimStyle::SetPrefix( const wchar_t* prefix)
{
  m_prefix = prefix;
}

void MYON_V5x_DimStyle::GetSuffix( MYON_wString& suffix) const
{
  suffix = m_suffix;
}
const wchar_t* MYON_V5x_DimStyle::Suffix() const
{
  return static_cast< const wchar_t* >(m_suffix);
}
void MYON_V5x_DimStyle::SetSuffix( wchar_t* suffix)
{
  m_suffix = suffix;
}
void MYON_V5x_DimStyle::SetSuffix( const wchar_t* suffix)
{
  m_suffix = suffix;
}

void MYON_V5x_DimStyle::GetAlternatePrefix( MYON_wString& prefix) const
{
  prefix = m_alternate_prefix;
}
const wchar_t* MYON_V5x_DimStyle::AlternatePrefix() const
{
  return static_cast< const wchar_t* >(m_alternate_prefix);
}
void MYON_V5x_DimStyle::SetAlternatePrefix( wchar_t* prefix)
{
  m_alternate_prefix = prefix;
}
void MYON_V5x_DimStyle::SetAlternatePrefix( const wchar_t* prefix)
{
  m_alternate_prefix = prefix;
}

void MYON_V5x_DimStyle::GetAlternateSuffix( MYON_wString& suffix) const
{
  suffix = m_alternate_suffix;
}
const wchar_t* MYON_V5x_DimStyle::AlternateSuffix() const
{
  return static_cast< const wchar_t* >(m_alternate_suffix);
}
void MYON_V5x_DimStyle::SetAlternateSuffix( wchar_t* suffix)
{
  m_alternate_suffix = suffix;
}
void MYON_V5x_DimStyle::SetAlternateSuffix( const wchar_t* suffix)
{
  m_alternate_suffix = suffix;
}

bool MYON_V5x_DimStyle::SuppressExtension1() const
{
  return m_bSuppressExtension1;
}
void MYON_V5x_DimStyle::SetSuppressExtension1( bool suppress)
{
  m_bSuppressExtension1 = suppress;
}
bool MYON_V5x_DimStyle::SuppressExtension2() const
{
  return m_bSuppressExtension2;
}
void MYON_V5x_DimStyle::SetSuppressExtension2( bool suppress)
{
  m_bSuppressExtension2 = suppress;
}

// ver 1.4, Dec 28, 05
double MYON_V5x_DimStyle::DimExtension() const
{
  return m_dimextension;
}

void MYON_V5x_DimStyle::SetDimExtension( const double e)
{
  // Allow negative?
  m_dimextension = e;
}
...
void MYON_V5x_DimStyle::SetExtensionLineColorSource(const MYON::object_color_source src)
{
  m_ext_line_color_source = (unsigned char)src;
}
MYON::object_color_source MYON_V5x_DimStyle::ExtensionLineColorSource() const
{
  return (MYON::object_color_source)m_ext_line_color_source;
}
void MYON_V5x_DimStyle::SetDimensionLineColorSource(const MYON::object_color_source src)
{
  m_dim_line_color_source = (unsigned char)src;
}
MYON::object_color_source MYON_V5x_DimStyle::DimensionLineColorSource() const
{
  return (MYON::object_color_source)m_dim_line_color_source;
}
void MYON_V5x_DimStyle::SetArrowColorSource(const MYON::object_color_source src)
{
  m_arrow_color_source = (unsigned char)src;
}
MYON::object_color_source MYON_V5x_DimStyle::ArrowColorSource() const
{
  return (MYON::object_color_source)m_arrow_color_source;
}
void MYON_V5x_DimStyle::SetExtensionLineColor(MYON_Color c)
{
  m_ext_line_color = c;
}
MYON_Color MYON_V5x_DimStyle::ExtensionLineColor() const
{
  return m_ext_line_color;
}
void MYON_V5x_DimStyle::SetDimensionLineColor(MYON_Color c)
{
  m_dim_line_color = c;
}
MYON_Color MYON_V5x_DimStyle::DimensionLineColor() const
{
  return m_dim_line_color;
}
void MYON_V5x_DimStyle::SetArrowColor(MYON_Color c)
{
  m_arrow_color = c;
}
MYON_Color MYON_V5x_DimStyle::ArrowColor() const
{
  return m_arrow_color;
}
void MYON_V5x_DimStyle::SetTextColor(MYON_Color c)
{
  m_text_color = c;
}
MYON_Color MYON_V5x_DimStyle::TextColor() const
{
  return m_text_color;
}

void MYON_V5x_DimStyle::SetExtensionLinePlotColorSource(const MYON::plot_color_source src)
{
  m_ext_line_plot_color_source = (unsigned char)src;
}
MYON::plot_color_source MYON_V5x_DimStyle::ExtensionLinePlotColorSource() const
{
  return (MYON::plot_color_source)m_ext_line_plot_color_source;
}
void MYON_V5x_DimStyle::SetDimensionLinePlotColorSource(const MYON::plot_color_source src)
{
  m_dim_line_plot_color_source = (unsigned char)src;
}
MYON::plot_color_source MYON_V5x_DimStyle::DimensionLinePlotColorSource() const
{
  return (MYON::plot_color_source)m_dim_line_plot_color_source;
}
void MYON_V5x_DimStyle::SetArrowPlotColorSource(const MYON::plot_color_source src)
{
  m_arrow_plot_color_source = (unsigned char)src;
}
MYON::plot_color_source MYON_V5x_DimStyle::ArrowPlotColorSource() const
{
  return (MYON::plot_color_source)m_arrow_plot_color_source;
}
void MYON_V5x_DimStyle::SetExtensionLinePlotColor(MYON_Color c)
{
  m_ext_line_plot_color = c;
}
MYON_Color MYON_V5x_DimStyle::ExtensionLinePlotColor() const
{
  return m_ext_line_plot_color;
}
void MYON_V5x_DimStyle::SetDimensionLinePlotColor(MYON_Color c)
{
  m_dim_line_plot_color = c;
}
MYON_Color MYON_V5x_DimStyle::DimensionLinePlotColor() const
{
  return m_dim_line_plot_color;
}
void MYON_V5x_DimStyle::SetArrowPlotColor(MYON_Color c)
{
  m_arrow_plot_color = c;
}
MYON_Color MYON_V5x_DimStyle::ArrowPlotColor() const
{
  return m_arrow_plot_color;
}
void MYON_V5x_DimStyle::SetTextPlotColor(MYON_Color c)
{
  m_text_plot_color = c;
}
MYON_Color MYON_V5x_DimStyle::TextPlotColor() const
{
  return m_text_plot_color;
}

void MYON_V5x_DimStyle::SetExtensionLinePlotWeightSource(const MYON::plot_weight_source src)
{
  m_ext_line_plot_weight_source = (unsigned char)src;
}
MYON::plot_weight_source MYON_V5x_DimStyle::ExtensionLinePlotWeightSource() const
{
  return (MYON::plot_weight_source)m_ext_line_plot_weight_source;
}
void MYON_V5x_DimStyle::SetDimensionLinePlotWeightSource(const MYON::plot_weight_source src)
{
  m_dim_line_plot_weight_source = (unsigned char)src;
}
MYON::plot_weight_source MYON_V5x_DimStyle::DimensionLinePlotWeightSource() const
{
  return (MYON::plot_weight_source)m_dim_line_plot_weight_source;
}
void MYON_V5x_DimStyle::SetExtensionLinePlotWeight(double w)
{
  m_ext_line_plot_weight_mm = w;
}
double MYON_V5x_DimStyle::ExtensionLinePlotWeight() const
{
  return m_ext_line_plot_weight_mm;
}
void MYON_V5x_DimStyle::SetDimensionLinePlotWeight(double w)
{
  m_dim_line_plot_weight_mm = w;
}
double MYON_V5x_DimStyle::DimensionLinePlotWeight() const
{
  return m_dim_line_plot_weight_mm;
}

void MYON_V5x_DimStyle::SetFixedExtensionLen(double l)
{
  m_fixed_extension_len = l;
}

double MYON_V5x_DimStyle::FixedExtensionLen() const
{
  return m_fixed_extension_len;
}

void MYON_V5x_DimStyle::SetFixedExtensionLenOn(bool on)
{
  m_fixed_extension_len_on = on;
}

bool MYON_V5x_DimStyle::FixedExtensionLenOn() const
{
  return m_fixed_extension_len_on;
}

void MYON_V5x_DimStyle::SetTextRotation(double r)
{
  m_text_rotation = r;
}
double MYON_V5x_DimStyle::TextRotation() const
{
  return m_text_rotation;
}

void MYON_V5x_DimStyle::SetAlternateToleranceResolution(int r)
{
  m_alt_tol_resolution = r;
}
int MYON_V5x_DimStyle::AlternateToleranceResolution() const
{
  return m_alt_tol_resolution;
}

void MYON_V5x_DimStyle::SetSuppressArrow1(bool s)
{
  m_suppress_arrow1 = s;
}
bool MYON_V5x_DimStyle::SuppressArrow1() const
{
  return m_suppress_arrow1;
}
void MYON_V5x_DimStyle::SetSuppressArrow2(bool s)
{
  m_suppress_arrow2 = s;
}
bool MYON_V5x_DimStyle::SuppressArrow2() const
{
  return m_suppress_arrow2;
}
void MYON_V5x_DimStyle::SetTextMoveLeader(int m)
{
  m_textmove_leader = m;
}
int MYON_V5x_DimStyle::TextMoveLeader() const
{
  return m_textmove_leader;
}
void MYON_V5x_DimStyle::SetArcLengthSymbol(int m)
{
  m_arclength_sym = m;
}
int MYON_V5x_DimStyle::ArcLengthSymbol() const
{
  return m_arclength_sym;
}
void MYON_V5x_DimStyle::SetStackFractionFormat(int f)
{
  if (0 <= f && 2 >= f)
    m_stack_format = f;
}
int MYON_V5x_DimStyle::StackFractionFormat() const
{
  return m_stack_format;
}
void MYON_V5x_DimStyle::SetStackHeightFraction(double f)
{
  m_stack_textheight_fraction = f;
}
double MYON_V5x_DimStyle::StackHeightFraction() const
{
  return m_stack_textheight_fraction;
}
void MYON_V5x_DimStyle::SetRoundOff(double r)
{
  m_round = r;
}
double MYON_V5x_DimStyle::RoundOff() const
{
  return m_round;
}
void MYON_V5x_DimStyle::SetAlternateRoundOff(double r)
{
  m_alt_round = r;
}
double MYON_V5x_DimStyle::AlternateRoundOff() const
{
  return m_alt_round;
}
void MYON_V5x_DimStyle::SetZeroSuppress(int s)
{
  m_zero_suppress = s;
}
int MYON_V5x_DimStyle::ZeroSuppress() const
{
  return m_zero_suppress;
}
void MYON_V5x_DimStyle::SetAlternateZeroSuppress(int s)
{
  m_alt_zero_suppress = s;
}
int MYON_V5x_DimStyle::AlternateZeroSuppress() const
{
  return m_alt_zero_suppress;
}
void MYON_V5x_DimStyle::SetToleranceZeroSuppress(int s)
{
  m_tol_zero_suppress = s;
}
int MYON_V5x_DimStyle::ToleranceZeroSuppress() const
{
  return m_tol_zero_suppress;
}
void MYON_V5x_DimStyle::SetAngleZeroSuppress(int s)
{
  m_ang_zero_suppress = s;
}
int MYON_V5x_DimStyle::AngleZeroSuppress() const
{
  return m_ang_zero_suppress;
}
void MYON_V5x_DimStyle::SetAlternateBelow(bool below)
{
  m_alt_below = below;
}
bool MYON_V5x_DimStyle::AlternateBelow() const
{
  return m_alt_below;
}
void MYON_V5x_DimStyle::SetArrowType1(MYON_Arrowhead::arrow_type type)
{
  m_arrow_1.SetArrowheadType(type);
}
MYON_Arrowhead::arrow_type MYON_V5x_DimStyle::ArrowType1() const
{
  return m_arrow_1.ArrowheadType();
}
void MYON_V5x_DimStyle::SetArrowType2(MYON_Arrowhead::arrow_type type)
{
  m_arrow_2.SetArrowheadType(type);
}
MYON_Arrowhead::arrow_type MYON_V5x_DimStyle::ArrowType2() const
{
  return m_arrow_2.ArrowheadType();
}
void MYON_V5x_DimStyle::SetArrowBlockId1(MYON_UUID id)
{
  m_arrow_1.SetArrowBlockId(id);
}
MYON_UUID MYON_V5x_DimStyle::ArrowBlockId1() const
{
  return m_arrow_1.ArrowBlockId();
}
void MYON_V5x_DimStyle::SetArrowBlockId2(MYON_UUID id)
{
  m_arrow_2.SetArrowBlockId(id);
}
MYON_UUID MYON_V5x_DimStyle::ArrowBlockId2() const
{
  return m_arrow_2.ArrowBlockId();
}
const MYON_Arrowhead& MYON_V5x_DimStyle::Arrowhead1() const
{
  return m_arrow_1;
}
const MYON_Arrowhead& MYON_V5x_DimStyle::Arrowhead2() const
{
  return m_arrow_2;
}

//--------------------------------------
// MYON_DimStyleExtra access functions
// Added for v5 5/01/07 LW


bool MYON_V5x_DimStyle::IsFieldOverride(MYON_V5x_DimStyle::Field field_id) const
{
...
void MYON_V5x_DimStyle::SetFieldOverride(MYON_V5x_DimStyle::Field field_id, bool bOverride)
{
...
void MYON_V5x_DimStyle::ClearAllFieldOverrides()
{
...
bool MYON_V5x_DimStyle::HasOverrides() const
{
  return (m_field_override_count > 0);
}

bool MYON_V5x_DimStyle::OverrideFields(const MYON_V5x_DimStyle& src, const MYON_V5x_DimStyle& parent)
{
  // NOTE WELL:
  ...
  for(unsigned int i = 0; i < count; i++)
  {
    ...
    const MYON_V5x_DimStyle* copyfrom = bFromSrc ? (&src) : (&parent);
...
bool MYON_V5x_DimStyle::InheritFields(const MYON_V5x_DimStyle& parent)
{
  OverrideFields(*this,parent);
  return (m_field_override_count < MYON_DimStyle::FieldCount);
}

bool MYON_V5x_DimStyle::IsChildDimstyle() const
{
  if(ParentId() != MYON_nil_uuid)
    return true;
  else
    return false;
}

bool MYON_V5x_DimStyle::IsChildOf(const MYON_UUID& parent_uuid) const
{
  if(parent_uuid != MYON_nil_uuid && ParentId() == parent_uuid)
    return true;
  else
    return false;
}

//void MYON_V5x_DimStyle::SetParentId(MYON_UUID parent_id)
//{
//  m_parent_dimstyle = parent_id;
//}
//
//MYON_UUID MYON_V5x_DimStyle::ParentId() const
//{
...
//  3: Limits
//  4: Basic
int MYON_V5x_DimStyle::ToleranceStyle() const
{
  return m_tolerance_style;
}

int MYON_V5x_DimStyle::ToleranceResolution() const
{
  return m_tolerance_resolution;
}

double MYON_V5x_DimStyle::ToleranceUpperValue() const
{
  return m_tolerance_upper_value;
}

double MYON_V5x_DimStyle::ToleranceLowerValue() const
{
  return m_tolerance_lower_value;
}

double MYON_V5x_DimStyle::ToleranceHeightScale() const
{
  return m_tolerance_height_scale;
}

double MYON_V5x_DimStyle::BaselineSpacing() const
{
  return m_baseline_spacing;
}

//static 
int MYON_V5x_DimStyle::DefaultToleranceStyle()
{
  return 0;
}

//static 
int MYON_V5x_DimStyle::DefaultToleranceResolution()
{
  return 4;
}

//static 
double MYON_V5x_DimStyle::DefaultToleranceUpperValue()
{
  return 0.0;
}

//static 
double MYON_V5x_DimStyle::DefaultToleranceLowerValue()
{
  return 0.0;
}

//static 
double MYON_V5x_DimStyle::DefaultToleranceHeightScale()
{
  return 1.0;
}

//static 
double MYON_V5x_DimStyle::DefaultBaselineSpacing()
{
  return 1.0;
}

// static
bool MYON_V5x_DimStyle::DefaultDrawTextMask()
{
  return false;
}

// static
int MYON_V5x_DimStyle::DefaultMaskColorSource()
{
  return 0;
}

// static
MYON_Color MYON_V5x_DimStyle::DefaultMaskColor()
{
  return MYON_Color(255, 255, 255);
}

// static
double MYON_V5x_DimStyle::DefaultDimScale()
{
  return 1.0;
}

// static
int MYON_V5x_DimStyle::DefaultDimScaleSource()
{
  return 0;
}
...
//-------------------
void MYON_V5x_DimStyle::Scale(double scale)
{
  if (MYON_IsValid(scale) && scale > MYON_SQRT_EPSILON && 1.0 != scale)
  {
...
void MYON_V5x_DimStyle::SetToleranceStyle(int style)
{
  if (style >= tsMin && style <= tsMax)
    m_tolerance_style = style;
}

void MYON_V5x_DimStyle::SetToleranceResolution(int resolution)
{
  if (resolution >= 0 && resolution < 16)
    m_tolerance_resolution = resolution;
}

void MYON_V5x_DimStyle::SetToleranceUpperValue(double upper_value)
{
  if (MYON_IsValid(upper_value))
    m_tolerance_upper_value = upper_value;
}

void MYON_V5x_DimStyle::SetToleranceLowerValue(double lower_value)
{
  if (MYON_IsValid(lower_value))
    m_tolerance_lower_value = lower_value;
}

void MYON_V5x_DimStyle::SetToleranceHeightScale(double scale)
{
  if (MYON_IsValid(scale) && scale > MYON_SQRT_EPSILON)
    m_tolerance_height_scale = scale;
}

void MYON_V5x_DimStyle::SetBaselineSpacing(double spacing)
{
  if (MYON_IsValid(spacing) && spacing > MYON_SQRT_EPSILON)
    m_baseline_spacing = spacing;
}

bool MYON_V5x_DimStyle::DrawTextMask() const
{
  return m_bDrawMask;
}

void MYON_V5x_DimStyle::SetDrawTextMask(bool bDraw)
{
  m_bDrawMask = bDraw ? true : false;
}

int MYON_V5x_DimStyle::MaskColorSource() const
{
  return m_mask_color_source;
}

void MYON_V5x_DimStyle::SetMaskColorSource(int source)
{
  if (source == 1)
    m_mask_color_source = 1;
  else
...
MYON_Color MYON_V5x_DimStyle::MaskColor() const
{
  return m_mask_color;
}

void MYON_V5x_DimStyle::SetMaskColor(MYON_Color color)
{
  m_mask_color = color;
}

double MYON_V5x_DimStyle::MaskOffsetFactor() const
{
  if (m_textheight != 0.0)
...
void MYON_V5x_DimStyle::SetDimScale(double scale)
{
  if (MYON_IsValid(scale))
    m_dimscale = scale;
}

double MYON_V5x_DimStyle::DimScale() const
{
  return m_dimscale;
}

void MYON_V5x_DimStyle::SetDimScaleSource(int source)
{
  m_dimscale_source = source;
}

int MYON_V5x_DimStyle::DimScaleSource() const
{
  return m_dimscale_source;
}

void MYON_V5x_DimStyle::SetSourceDimstyle(MYON_UUID source_uuid)
{
  // Can be MYON_nil_uuid
  m_source_dimstyle = source_uuid;
}

MYON_UUID MYON_V5x_DimStyle::SourceDimstyle() const
{
  return m_source_dimstyle;
}

// returns true if they are the same
bool MYON_V5x_DimStyle::CompareFields(const MYON_V5x_DimStyle& other) const
{
  if(
