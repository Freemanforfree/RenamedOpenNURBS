...
bool MYON_BezierCurve::GetTightBoundingBox( 
    ...
    ) const
{
  ...
  // The result from MYON_GetPointListBoundingBox() is good enough
...
MYON_PolynomialCurve::MYON_PolynomialCurve()
                   : m_dim(0), m_is_rat(0), m_order(0), m_domain(0.0,1.0)
{}

MYON_PolynomialCurve::MYON_PolynomialCurve( int dim, bool is_rat, int order )
                   : m_dim(0), m_is_rat(0), m_order(0), m_domain(0.0,1.0)
{
...
MYON_PolynomialCurve::MYON_PolynomialCurve(const MYON_PolynomialCurve& src)
                   : m_dim(0), m_is_rat(0), m_order(0), m_domain(0.0,1.0)
{
  *this = src;
}

MYON_PolynomialCurve::MYON_PolynomialCurve(const MYON_BezierCurve& src)
                   : m_dim(0), m_is_rat(0), m_order(0), m_domain(0.0,1.0)
{
  *this = src;
}

bool MYON_PolynomialCurve::Create( int dim, bool is_rat, int order )
{
  bool rc = true;
...
void MYON_PolynomialCurve::Destroy()
{
  m_dim = 0;
  m_is_rat = 0;
  m_order = 0;
...
MYON_PolynomialCurve& MYON_PolynomialCurve::operator=(const MYON_PolynomialCurve& src)
{
  if ( this != &src ) {
    m_dim = src.m_dim;
...
MYON_PolynomialCurve& MYON_PolynomialCurve::operator=(const MYON_BezierCurve& src)
{
  int i;
  double d;
  m_dim = src.m_dim;
  ...
  if ( m_order >= 2 && src.CVSize() <= 4 ) {
    MYON_BezierCurve s; // scratch surface for homogeneous evaluation
...
bool MYON_PolynomialCurve::Evaluate( // returns false if unable to evaluate
       ...
       ) const
{
  ...
  if ( m_order >= 1 && m_cv.Count() == m_order )
  {
    ...
    MYON_4dPointArray p(der_count+1);
    MYON_4dPoint c;
    double s;
    int i, j, der;
    p.Zero();
    ...
    if ( der_count >= 1 ) {
      ...
      if ( m_is_rat ) {
        MYON_EvaluateQuotientRule( 3, der_count, 4, &p[0].x );
      }
    }
...
MYON_PolynomialSurface::MYON_PolynomialSurface() : m_dim(0), m_is_rat(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
...
MYON_PolynomialSurface::MYON_PolynomialSurface( int dim, bool is_rat, int order0, int order1 )
{
  Create( dim, is_rat, order0, order1 );
}

MYON_PolynomialSurface::~MYON_PolynomialSurface()
{
  Destroy();
}

MYON_PolynomialSurface::MYON_PolynomialSurface(const MYON_PolynomialSurface& src)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_domain[0].m_t[0] = 0.0;
...
MYON_PolynomialSurface::MYON_PolynomialSurface(const MYON_BezierSurface& src)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_domain[0].m_t[0] = 0.0;
...
MYON_PolynomialSurface& MYON_PolynomialSurface::operator=(const MYON_PolynomialSurface& src)
{
  if ( this != &src ) {
...
MYON_PolynomialSurface& MYON_PolynomialSurface::operator=(const MYON_BezierSurface& src)
{
  if ( Create( src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1] ) ) {
    //m_domain[0] = src.m_domain[0];
    //m_domain[1] = src.m_domain[1];
...
bool MYON_PolynomialSurface::Create( int dim, bool is_rat, int order0, int order1 )
{
  bool rc = true;
  if ( dim > 0 ) m_dim = dim; else {m_dim = 0; rc = false;};
  m_is_rat = is_rat?1:0;
...
void MYON_PolynomialSurface::Destroy()
{
  m_dim = 0;
  m_is_rat = 0;
  m_order[0] = 0;
...
bool MYON_PolynomialSurface::Evaluate( // returns false if unable to evaluate
       double s, 
       double t,           // evaluation parameter
       int der_count,      // number of derivatives (>=0)
       int v_stride,       // array stride (>=Dimension())
...
{
  MYON_ERROR("TODO: - finish MYON_PolynomialSurface::Evaluate()\n");
  return false;
}

MYON_BezierCurve::MYON_BezierCurve()
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
                 ...
                 m_cv_capacity(0)
{
  ...
  m_reserved_MYON_BezierCurve = 0;
#endif
}

MYON_BezierCurve::MYON_BezierCurve( int dim, bool is_rat, int order )
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve::MYON_BezierCurve(const MYON_BezierCurve& src)
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve::MYON_BezierCurve(const MYON_PolynomialCurve& src)
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve::MYON_BezierCurve(const MYON_2dPointArray& cv)
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve::MYON_BezierCurve(const MYON_3dPointArray& cv)
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve::MYON_BezierCurve(const MYON_4dPointArray& cv)
               : m_dim(0), 
                 m_is_rat(0), 
                 m_order(0), 
                 m_cv_stride(0), 
...
MYON_BezierCurve& MYON_BezierCurve::operator=(const MYON_2dPointArray& cv)
{
  if ( Create( 2, false, cv.Count() ) )
  {
    int i;
    for ( i = 0; i < m_order; i++ )
      SetCV( i, MYON::intrinsic_point_style, cv[i] );
  }
  return *this;
}

MYON_BezierCurve& MYON_BezierCurve::operator=(const MYON_3dPointArray& cv)
{
...
MYON_BezierCurve& MYON_BezierCurve::operator=(const MYON_4dPointArray& cv)
{
...
MYON_BezierCurve& MYON_BezierCurve::operator=(const MYON_BezierCurve& src)
{
  int i;
  if ( this != &src ) {
    //m_domain = src.m_domain;
...
MYON_BezierCurve& MYON_BezierCurve::operator=(const MYON_PolynomialCurve& src)
{
  //m_domain = src.m_domain;
  if ( src.m_dim > 0 && src.m_cv.Count() == src.m_order && src.m_order >= 2 ) 
  {
    ...
    MYON_PolynomialCurve s; // scratch poly for homogeneous evaluation
    ...
    double t;
    MYON_4dPointArray pt(src.m_order);
...
bool MYON_BezierCurve::Loft( const MYON_3dPointArray& pt )
{
  const MYON_3dPoint* p0 = pt.Array();
  return Loft( 3, pt.Count(), 3, p0?&p0->x:0, 0, nullptr );
}


bool MYON_BezierCurve::Loft( int pt_dim, int pt_count, int pt_stride, const double* pt,
                           int t_stride, const double* t )
{
  bool rc = false;
  if ( pt_dim >= 1 && pt_count >= 2 && pt_stride >= pt_dim && pt != nullptr && (t_stride >= 1 || t == 0) ) {
    int i, j;
    MYON_SimpleArray<double> uniform_t;
    double s;
    if ( !t ) {
    ...
    for ( i = 0; i < m_order; i++ ) {
      ...
      for ( j = 0; j < m_order; j++ ) {
        M.m[i][j] = MYON_EvaluateBernsteinBasis( degree, j, s );
      }
      memcpy( m_cv + i*m_cv_stride, pt + i*pt_stride, sizeof_cv );
    }  

    int rank = M.RowReduce( MYON_EPSILON, m_dim, m_cv_stride, m_cv );
    M.BackSolve( MYON_EPSILON, m_dim, m_order, 
                 m_cv_stride, m_cv, 
                 m_cv_stride, m_cv );
    if ( rank == m_order )
      rc = true;
...
bool MYON_BezierCurve::IsValid() const
{
  if ( m_dim <= 0 )
    return false;
...
void MYON_BezierCurve::Dump( MYON_TextLog& dump ) const
{
  dump.Print( "MYON_BezierCurve dim = %d is_rat = %d\n"
               "        order = %d \n",
...
int MYON_BezierCurve::Dimension() const
{
  return m_dim;
}

bool MYON_BezierCurve::Create( int dim, bool is_rat, int order )
{
  m_dim = (dim>0) ? dim : 0;
  m_is_rat = is_rat ? 1 : 0;
...
void MYON_BezierCurve::EmergencyDestroy()
{
  m_cv = 0;
}

bool MYON_BezierCurve::GetBBox( // returns true if successful
       double* boxmin,    // minimum
       double* boxmax,    // maximum
       bool bGrowBox
       ) const
{
  return MYON_GetPointListBoundingBox( m_dim, m_is_rat, m_order, m_cv_stride, m_cv, boxmin, boxmax, bGrowBox );
}

bool MYON_BezierCurve::GetBoundingBox( // returns true if successful
...
MYON_BoundingBox MYON_BezierCurve::BoundingBox() const
{
  MYON_BoundingBox bbox;
  GetBoundingBox( bbox );
  return bbox;
...
bool MYON_Line::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_Arc::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
{
  ...
  {
    return MYON_Circle::GetTightBoundingBox(tight_bbox,bGrowBox,nullptr);
  }
  ...
  MYON_4dPoint nurbs_cv[9];
  MYON_NurbsCurve nurbs_arc;
...
bool MYON_Circle::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
{
  ...
  {
    // The MYON_Arc version handles all transformations including
    // ones that are not in rotations.
    MYON_Arc arc(*this,2.0*MYON_PI);
  ...
  const double rx = radius*MYON_Length2d(plane.zaxis.y, plane.zaxis.z);
  const double ry = radius*MYON_Length2d(plane.zaxis.z, plane.zaxis.x);
  const double rz = radius*MYON_Length2d(plane.zaxis.x, plane.zaxis.y);
  if ( bGrowBox )
  {    
...
bool MYON_ArcCurve::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_LineCurve::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_PolylineCurve::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_PolyCurve::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_CurveArray::GetTightBoundingBox( 
		MYON_BoundingBox& tight_bbox, 
    bool bGrowBox,
		const MYON_Xform* xform
    ) const
...
bool MYON_3dPointArray::GetTightBoundingBox( 
			MYON_BoundingBox& tight_bbox, 
      bool bGrowBox,
			const MYON_Xform* xform
      ) const
...
bool MYON_PointCloud::GetTightBoundingBox( 
			MYON_BoundingBox& tight_bbox, 
      bool bGrowBox,
			const MYON_Xform* xform
      ) const
{
  ...
  if ( m_P.Count() > 0 )
  {
    MYON_BoundingBox pc_bbox = BoundingBox();
    if ( bGrowBox )
    {
      if ( MYON_WorldBBoxIsInTightBBox( tight_bbox, pc_bbox, xform ) )
        return true;
    }
...
bool MYON_Mesh::GetTightBoundingBox(
	MYON_BoundingBox& tight_bbox,
	bool bGrowBox,
	const MYON_Xform* xform
	) const
{
	MYON_SimpleArray<MYON_PlaneEquation> NoCP;
...
bool MYON_Mesh::GetTightBoundingBox(
	MYON_BoundingBox& Xtight_bbox,
	bool bGrowBox,
	const MYON_SimpleArray<MYON_PlaneEquation>& Clip,
	const MYON_Xform* xform
) const
{
	...
	if (!bGrowBox)
		Xtight_bbox = MYON_BoundingBox::EmptyBoundingBox;
  ...
  // boxes are removed by MYON_Mesh.InvalidateBoundingBoxes() so this hash
		...
		for (int fi = 0; fi < FaceCount(); fi++)
		{
			...
			for (int fvi = 0; fvi < 4; fvi++)
			{
				...
				if ((LastOut || NextOut) && (LastData & NextData) == 0)
				{
					...
					MYON_3dPoint Last = m_V[F.vi[fvi]];
					MYON_3dPoint Next = m_V[F.vi[(fvi + 1) % 4]];
					if (xform)
					{
						Last = (*xform)*Last;
						Next = (*xform)*Next;
					...
					MYON_Interval Dom(0.0, 1.0);  // Dom parameterizes the line segment from Last to Next
					for (int j = 0; j < n && Dom != MYON_Interval::EmptyInterval; j++)
					{
						...
						if (mask & LastData)
						{
							...
							MYON_Interval D(cplast / (cplast - cpnext), 1.0);
							Dom.Intersection(D);
						}
						else if (mask & NextData)
						{
							...
							MYON_Interval D(0.0, -cplast / (cpnext - cplast));
							Dom.Intersection(D);
						}
					}
					if (Dom != MYON_Interval::EmptyInterval)
					{
						if (Dom[0] > 0.0)
						{
							MYON_3dPoint Pt = (1 - Dom[0])*Last + Dom[0] * Next;
							bbox.Set(Pt, true);
						}
						if (Dom[1] < 1.0)
						{
							MYON_3dPoint Pt = (1 - Dom[1])*Last + Dom[1] * Next;
							bbox.Set(Pt, true);
						}
					}
				}
...
bool MYON_BezierCurve::Transform( 
       const MYON_Xform& xform
       )
{
  if ( 0 == m_is_rat )
  ...
  return MYON_TransformPointList( m_dim, m_is_rat, m_order, m_cv_stride, m_cv, xform );
}

bool MYON_BezierCurve::Rotate(
...
bool MYON_BezierCurve::Translate( const MYON_3dVector& delta )
{
  MYON_Xform tr(MYON_Xform::TranslationTransformation( delta ));
  return Transform( tr );
}

bool MYON_BezierCurve::Scale( double x )
{
...
MYON_Interval MYON_BezierCurve::Domain() const
{
  //return m_domain;
  return MYON_Interval(0.0,1.0);
}

bool MYON_BezierCurve::Reverse()
{
  bool rc = MYON_ReversePointList( m_dim, m_is_rat, m_order, m_cv_stride, m_cv )?true:false;
  //if ( rc )
  //  m_domain.Reverse();
  return rc;
}

bool MYON_BezierCurve::Evaluate( // returns false if unable to evaluate
       double t,      // evaluation parameter
       int der_count, // number of derivatives (>=0)
       int v_stride,  // array stride (>=Dimension())
       double* v      // array of length stride*(ndir+1)
...
{
  return MYON_EvaluateBezier( m_dim, m_is_rat, m_order, m_cv_stride, m_cv, 
                            0.0, 1.0, //m_domain[0], m_domain[1], 
                            der_count, t, v_stride, v )?true:false;
}

int MYON_BezierCurve::GetNurbForm( MYON_NurbsCurve& n ) const
{
  int rc = 0;
  if ( n.Create( m_dim, m_is_rat, m_order, m_order ) ) {
    ...
    rc = MYON_ClampKnotVector(n.m_order, n.m_cv_count, n.m_knot, 2) ? 1 : 0;
  }
  return rc;
}

bool MYON_BezierCurve::IsRational() const
{
  return m_is_rat ? true : false;
}
  
int MYON_BezierCurve::CVSize() const
{
  return (m_dim>0&&m_is_rat) ? m_dim+1 : m_dim;
}

int MYON_BezierCurve::Order() const
{
  return m_order;
}

int MYON_BezierCurve::Degree() const
{
  return m_order>=2 ? m_order-1 : 0;
}

double* MYON_BezierCurve::CV( int i ) const
{
  return m_cv ? m_cv + (i*m_cv_stride) : 0;
}

const MYON_4dPoint MYON_BezierCurve::ControlPoint(
  int i
) const
{
  MYON_4dPoint cv;
...
MYON::point_style MYON_BezierCurve::CVStyle() const
{
  return m_is_rat ? MYON::homogeneous_rational : MYON::not_rational;
}

double MYON_BezierCurve::Weight( int i ) const 
{
  return ( m_is_rat && m_cv ) ? CV(i)[m_dim] : 1.0;
}

bool MYON_BezierCurve::SetWeight( int i, double w )
{
  bool rc = false;
  if ( m_is_rat ) {
    double* cv = CV(i);
...
bool MYON_BezierCurve::SetCV( int i, MYON::point_style style, const double* Point )
{
  bool rc = true;
  int k;
  double w;
...
bool MYON_BezierCurve::SetCV( int i, const MYON_3dPoint& point )
{
  bool rc = false;
  double* cv = CV(i);
  if ( cv ) {
...
bool MYON_BezierCurve::SetCV( int i, const MYON_4dPoint& point )
{
  bool rc = false;
  double* cv = CV(i);
  if ( cv ) {
...
bool MYON_BezierCurve::GetCV( int i, MYON::point_style style, double* Point ) const
{
...
bool MYON_BezierCurve::GetCV( int i, MYON_3dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i);
  if ( cv ) {
...
bool MYON_BezierCurve::GetCV( int i, MYON_4dPoint& point ) const
{
  bool rc = false;
...
int MYON_BezierCurve::CVCount() const
{
  return Order();
}

bool MYON_BezierCurve::MakeRational()
{
  if ( !IsRational() ) {
...
bool MYON_BezierCurve::MakeNonRational()
{
  if ( IsRational() ) {
    const int dim = Dimension();
    const int cv_count = CVCount();
...
bool MYON_BezierCurve::IncreaseDegree( int desired_degree )
{
  bool rc = false;
  if ( desired_degree > 0 ) {
    if ( desired_degree == m_order-1 )
      ...
      while ( m_order <= desired_degree ) {
        rc = MYON_IncreaseBezierDegree( m_dim, m_is_rat, m_order, m_cv_stride, m_cv )?true:false;
        if ( !rc )
          break;
        m_order++;
      }
...
bool MYON_BezierCurve::ReserveCVCapacity( int desired_capacity )
{
	bool rc = false;
...
bool MYON_BezierCurve::ChangeDimension( int dim )
{
  MYON_NurbsCurve c;
  c.m_dim = m_dim;
  c.m_is_rat = m_is_rat;
...
double MYON_BezierCurve::ControlPolygonLength() const
{
  double length = 0.0;
  MYON_GetPolylineLength( m_dim, m_is_rat, m_order, m_cv_stride, m_cv, &length );
  return length;
}

bool MYON_BezierCurve::Trim( const MYON_Interval& n )
{
  ...
  if ( rc ) {
    ...
    if ( t0 != 1.0 ) {
      ...
      MYON_EvaluatedeCasteljau( cvdim, m_order, +1, m_cv_stride, m_cv, t0 );
      MYON_EvaluatedeCasteljau( cvdim, m_order, -1, m_cv_stride, m_cv, s1 );
    }
    else {
      MYON_EvaluatedeCasteljau( cvdim, m_order, -1, m_cv_stride, m_cv, t1 );
      if ( t0 != 0.0 )
      {
        // 9/9/03 fix add != 0 test to avoid divide by zero bug
        double s0 = t1/t0;
        MYON_EvaluatedeCasteljau( cvdim, m_order, +1, m_cv_stride, m_cv, s0 );
      }
    }
  }
  return rc;
...
bool MYON_BezierCurve::Split( 
       double t, // t = splitting parameter must 0 < t < 1
       MYON_BezierCurve& left_bez, // left side returned here (can pass *this)
       MYON_BezierCurve& right_bez // right side returned here (can pass *this)
       ) const
{
...
MYON_BezierSurface::MYON_BezierSurface()
                 : m_dim(0),
                   m_is_rat(0),
                   m_cv(0),
                   m_cv_capacity(0)
{
  ...
  m_cv_stride[1] = 0;
#if 8 == MYON_SIZEOF_POINTER
  m_reserved_MYON_BezierSurface = 0;
#endif
}

MYON_BezierSurface::MYON_BezierSurface( int dim, bool is_rat, int order0, int order1 )
                 : m_dim(0),
                   m_is_rat(0),
                   m_cv(0),
                   m_cv_capacity(0)
...
MYON_BezierSurface::MYON_BezierSurface(const MYON_BezierSurface& src)
                 : m_dim(0),
                   m_is_rat(0),
                   m_cv(0),
                   m_cv_capacity(0)
...
MYON_BezierSurface::MYON_BezierSurface(const MYON_PolynomialSurface& src)
                 : m_dim(0),
                   m_is_rat(0),
                   m_cv(0),
                   m_cv_capacity(0)
...
MYON_BezierSurface& MYON_BezierSurface::operator=(const MYON_BezierSurface& src)
{
  if ( this != &src ) {
    if ( Create( src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1] ) ) {
...
MYON_BezierSurface& MYON_BezierSurface::operator=(const MYON_PolynomialSurface& src)
{
  if ( Create( src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1] ) ) {
...
bool MYON_BezierSurface::IsValid() const
{
  if ( m_dim <= 0 )
    return false;
  if ( m_is_rat != 0 && m_is_rat != 1 )
...
void MYON_BezierSurface::Dump( MYON_TextLog& dump ) const
{
  dump.Print( "MYON_BezierSurface dim = %d is_rat = %d\n"
               "        order = (%d, %d) \n",
  ...
  if ( !m_cv ) {
    ...
    for ( i = 0; i < m_order[0]; i++ )
    {
      if ( i > 0 )
        dump.Print("\n");
      sPreamble[0] = 0;
      MYON_String::FormatIntoBuffer(sPreamble, sPreamble_capacity, "  CV[%2d]", i);
      dump.PrintPointList( m_dim, m_is_rat, 
...
int MYON_BezierSurface::Dimension() const
{
  return m_dim;
}

bool MYON_BezierSurface::Create( int dim, bool is_rat, int order0, int order1 )
{
  if ( m_cv_capacity < 1 )
    m_cv = 0;
  m_dim = (dim>0) ? dim : 0;
...
void MYON_BezierSurface::Destroy()
{
  if ( m_cv && m_cv_capacity > 0 )
    onfree(m_cv);
  m_cv_capacity = 0;
...
void MYON_BezierSurface::EmergencyDestroy()
{
  m_cv = 0;
}

bool MYON_BezierSurface::Loft( const MYON_ClassArray<MYON_BezierCurve>& curve_list )
{
  int i;
  int count = curve_list.Count();
  MYON_SimpleArray<const MYON_BezierCurve*> ptr_list(count);
  for (i = 0; i < count; i++ )
  {
        ...
        )
{
  ...
  if (curve_count >= 2 && 0 != curve_list && 0 != curve_list[0] )
  {
    ...
    MYON_SimpleArray<double> meta_point(curve_count*shape_cv_stride*shape_order);
    MYON_BezierCurve* temp_shape = 0;
    for ( i = 0; i < curve_count; i++ )
    {
      const MYON_BezierCurve* shape = curve_list[i];
      if (    shape->m_order != shape_order 
           || shape->m_is_rat != shape_is_rat
           || shape->m_cv_stride != shape_cv_stride )
      {
        if ( 0 == temp_shape )
          temp_shape = new MYON_BezierCurve();
    ...
    if ( meta_point.Count() == curve_count*shape_cv_stride*shape_order )
    {
      ...
      MYON_SimpleArray<double>t(curve_count);
...
bool MYON_BezierSurface::GetBBox( // returns true if successful
       double* boxmin,    // minimum
       double* boxmax,    // maximum
       ...
       ) const
{
  ...
  for ( i = 0; rc && i < m_order[0]; i++ ) {
    rc = MYON_GetPointListBoundingBox( m_dim, m_is_rat, m_order[1], m_cv_stride[1],
...
bool MYON_BezierSurface::GetBoundingBox( // returns true if successful
       MYON_BoundingBox& bbox,
       int bGrowBox             // true means grow box
       ) const
{
...
MYON_BoundingBox MYON_BezierSurface::BoundingBox() const
{
  MYON_BoundingBox bbox;
...
bool MYON_BezierSurface::Transform( const MYON_Xform& xform )
{
  int i;
  bool rc = (m_order[0] > 0 && m_order[1] > 0) ? true : false;
  if (rc)
...
bool MYON_BezierSurface::Rotate(
      double sin_angle,          // sin(angle)
      double cos_angle,          // cos(angle)
      const MYON_3dVector& axis, // axis of rotation
      const MYON_3dPoint& center // center of rotation
...
bool MYON_BezierSurface::Translate( const MYON_3dVector& delta )
{
  MYON_Xform tr(MYON_Xform::TranslationTransformation( delta ));
  return Transform( tr );
}

bool MYON_BezierSurface::Scale( double x )
{
  MYON_Xform s(MYON_Xform::DiagonalTransformation(x));
  return Transform( s );
}

MYON_Interval MYON_BezierSurface::Domain( 
      int // dir - formal parameter intentionally ignored in this virtual function
      ) const
{
  // 0 = "u" domain, 1 = "v" domain
  return MYON_Interval(0.0,1.0); //m_domain[(dir>0)?1:0];
}

bool MYON_BezierSurface::Reverse( int  dir )
{
  int i;
  bool rc = (m_order[0] > 0 && m_order[1] > 0) ? true : false;
  if ( dir > 0 ) {
    for ( i = 0; rc && i < m_order[0]; i++ ) {
      rc = MYON_ReversePointList( m_dim, m_is_rat, m_order[1], m_cv_stride[1], CV(i,0) );
    }
    //m_domain[1].Reverse();
  }
  else {
    for ( i = 0; rc && i < m_order[1]; i++ ) {
      rc = MYON_ReversePointList( m_dim, m_is_rat, m_order[0], m_cv_stride[0], CV(0,i) );
    }
    //m_domain[0].Reverse();
  }
  return rc;
...
bool MYON_BezierSurface::Transpose()
{
  ...
  //MYON_Interval d = m_domain[0]; m_domain[0] = m_domain[1]; m_domain[1] = d;
  return true;
}

bool MYON_BezierSurface::Evaluate( // returns false if unable to evaluate
       ...
       ) const
{
  ...
  bool rc = MYON_EvaluateNurbsSurfaceSpan(
        m_dim,                          // dimension
...
MYON_3dPoint MYON_BezierSurface::PointAt(double s, double t) const
{
  MYON_3dPoint P;
...
MYON_BezierCurve* MYON_BezierSurface::IsoCurve(int dir, double t, MYON_BezierCurve* pCrv) const
{
	if( pCrv == nullptr )
  {
		pCrv = new MYON_BezierCurve(m_dim, m_is_rat, m_order[dir]);
	}
	...
	else
  {
	...
	MYON_EvaluateBezier( bigdim, 0, m_order[1-dir], stride,  cv, 0.0, 1.0, 0, t, bigdim, pCrv->m_cv );
...
int MYON_BezierSurface::GetNurbForm( MYON_NurbsSurface& n ) const
{
  int rc = 0;
  if ( n.Create( m_dim, m_is_rat, m_order[0], m_order[1], m_order[0], m_order[1] ) ) 
  {
    ...
    bool b0 = MYON_ClampKnotVector( n.m_order[0], n.m_cv_count[0], n.m_knot[0], 2 );
    bool b1 = MYON_ClampKnotVector( n.m_order[1], n.m_cv_count[1], n.m_knot[1], 2 );
    rc = (b0 && b1) ? 1 : 0;
  }
  return rc;
}

bool MYON_BezierSurface::IsRational() const
{
  return m_is_rat ? true : false;
}

int MYON_BezierSurface::CVSize() const
{
  return ( m_is_rat && m_dim>0 ) ? m_dim+1 : m_dim;
}

int MYON_BezierSurface::Order( int dir ) const
{
  return (dir>=0&&dir<=1) ? m_order[dir] : 0;
}

int MYON_BezierSurface::Degree(int dir) const
{
  int order = Order(dir);
  return (order>=2) ? order-1 : 0;
}

double* MYON_BezierSurface::CV( int i, int j ) const
{
  return (m_cv) ? (m_cv + i*m_cv_stride[0] + j*m_cv_stride[1]) : nullptr;
}

MYON::point_style MYON_BezierSurface::CVStyle() const
{
  return m_is_rat ? MYON::homogeneous_rational : MYON::not_rational;
}

double MYON_BezierSurface::Weight( int i, int j ) const
{
  return (m_cv && m_is_rat) ? m_cv[i*m_cv_stride[0] + j*m_cv_stride[1] + m_dim] : 1.0;
}


bool MYON_BezierSurface::SetWeight( int i, int j, double w )
{
  bool rc = false;
  if ( m_is_rat ) {
    double* cv = CV(i,j);
...
bool MYON_BezierSurface::SetCV( int i, int j, MYON::point_style style, const double* Point )
{
  bool rc = true;
  int k;
  double w;
...
bool MYON_BezierSurface::SetCV( int i, int j, const MYON_3dPoint& point )
{
  bool rc = false;
  double* cv = CV(i,j);
  if ( cv ) {
...
bool MYON_BezierSurface::SetCV( int i, int j, const MYON_4dPoint& point )
{
  bool rc = false;
  double* cv = CV(i,j);
  if ( cv ) {
...
bool MYON_BezierSurface::GetCV( int i, int j, MYON::point_style style, double* Point ) const
{
...
bool MYON_BezierSurface::GetCV( int i, int j, MYON_3dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i,j);
  if ( cv ) {
...
bool MYON_BezierSurface::GetCV( int i, int j, MYON_4dPoint& point ) const
{
  bool rc = false;
  const double* cv = CV(i,j);
  if ( cv ) {
...
bool MYON_BezierSurface::ZeroCVs()
{
...
bool MYON_BezierSurface::MakeRational()
{
  if ( !IsRational() ) {
    const int dim = Dimension();
...
bool MYON_BezierSurface::MakeNonRational()
{
  if ( IsRational() ) {
    const int dim = Dimension();
    if ( m_order[0] > 0 && m_order[1] > 0 && dim > 0 ) {
...
/////////////////////////////////////////////////////////////////
// Tools for managing CV and knot memory
bool MYON_BezierSurface::ReserveCVCapacity(
...
bool MYON_BezierSurface::Split( 
       int dir, // 0 split at "u"=t, 1= split at "v"=t
       double t, // t = splitting parameter must 0 < t < 1
       MYON_BezierSurface& left_bez, // west/south side returned here (can pass *this)
       MYON_BezierSurface& right_bez // east/north side returned here (can pass *this)
       ) const
{
  bool rc = false;
  if ( 0.0 < t && t < 1.0 ) {
    ...
    MYON_BezierCurve leftcrv, rightcrv;
    MYON_BezierCurve crv(crvdim,0,m_order[dir?1:0]);
    if (dir) {
      for ( j = 0; j < m_order[1]; j++ ) {
        crvcv = crv.CV(j);
        for ( i = 0; i < m_order[0]; i++) {
...
bool MYON_BezierSurface::IsSingular(		 // true if surface side is collapsed to a point
				...
				) const
{
  ...
  return MYON_PointsAreCoincident(m_dim,m_is_rat,point_count,point_stride,points);
}

static bool BezierControlPolygonLength(const MYON_BezierSurface& Bez, int dir, double* length)
{
	bool rc = false;
	if (length && dir >= 0 && dir <= 1 && Bez.m_order[0] >= 2 && Bez.m_order[1] >= 2 && Bez.m_cv != nullptr)
	{
		...
		*length = 0.0;
		...
		for (int i = 0; i < Bez.m_order[1 - dir]; i++)
		{
			...
			MYON_GetPolylineLength(Bez.m_dim, Bez.m_is_rat, Bez.m_order[dir], Bez.m_cv_stride[dir], p, &len);
			if (len > *length)
				*length = len;
		}
	}
...
bool MYON_BezierSurface::GetSurfaceSize(
	double* width,
	double* height
) const
{
...
bool MYON_ReparameterizeRationalBezierCurve(
          double c,
          int dim,
          int order,
          int cvstride,
...
bool MYON_BezierCurve::Reparametrize( double c )
{
  return Reparameterize(c);
}

bool MYON_BezierCurve::Reparameterize( double c )
{
  if ( !MYON_IsValid(c) || 0.0 == c )
    return false;
  ...
  return MYON_ReparameterizeRationalBezierCurve(c,m_dim,m_order,m_cv_stride,m_cv);

  //return true;
}

bool MYON_BezierCurve::ScaleConrolPoints( int i, double w )
{
  if ( i < 0 || i >= m_order || w == 0.0 || w == MYON_UNSET_VALUE )
    return false;
  if ( w == Weight(i) )
    return true;
...
bool MYON_ChangeRationalBezierCurveWeights(
          ...
          )
{
  ...
  if ( !MYON_IsValid(r) || r <= 0.0 ) 
    return false;
  if ( !MYON_IsValid(s) || 0.0 == s ) 
    return false;
  ...
  if (r != 1.0)
    MYON_ReparameterizeRationalBezierCurve(r,dim,order,cvstride,cv);
...
bool MYON_BezierCurve::ChangeWeights( int i0, double w0, int i1, double w1 )
{
  //double r, s, v0, v1;
  double v0, v1;
  int i;
  ...
  if ( i0 < 0 || i0 >= m_order || i1 < 0 || i1 >= m_order )
    return false;
  if ( 0.0 == w0 || !MYON_IsValid(w0) || 0.0 == w1 || !MYON_IsValid(w1) )
    return false;
  if ( w0 < 0.0 && w1 > 0.0 )
    return false;
  if ( w0 > 0.0 && w1 < 0.0 )
  ...
  MakeRational();
  return MYON_ChangeRationalBezierCurveWeights(m_dim,m_order,m_cv_stride,m_cv,i0,w0,i1,w1);

  /*
  if ( 0.0 == v0 || !MYON_IsValid(v0) || 0.0 == v1 || !MYON_IsValid(v1) )
    return false;
  if ( v0 < 0.0 && v1 > 0.0 )
    return false;
  if ( v0 > 0.0 && v1 < 0.0 )
...
MYON_3dPoint MYON_BezierCurve::PointAt( double t ) const
{
...
MYON_3dVector MYON_BezierCurve::DerivativeAt( double t ) const
{
  MYON_3dPoint  p(0.0,0.0,0.0);
...
MYON_3dVector MYON_BezierCurve::TangentAt( double t ) const
{
  MYON_3dPoint point;
...
MYON_3dVector MYON_BezierCurve::CurvatureAt( double t ) const
{
  MYON_3dPoint point;
...
bool MYON_BezierCurve::EvTangent(
       double t,
       MYON_3dPoint& point,
       MYON_3dVector& tangent
       ) const
  ...
  if ( rc && !tangent.Unitize() ) 
  {
    if ( Ev2Der( t, point, D1, D2 ) )
    {
      ...
      if ( rc )
      {
        ...
        double tminus = 0.0;
        double tplus = 0.0;
        if ( domain.IsIncreasing() && MYON_GetParameterTolerance( domain[0], domain[1], t, &tminus, &tplus ) )
        {
          MYON_3dPoint p;
          MYON_3dVector d1, d2;
          double eps = 0.0;
...
bool MYON_BezierCurve::EvCurvature(
       double t,
       MYON_3dPoint& point,
       MYON_3dVector& tangent,
       MYON_3dVector& kappa
       ) const
{
  ...
  if ( rc )
  {
    rc = MYON_EvCurvature( d1, d2, tangent, kappa )?true:false;
  }
  return rc;
}


bool MYON_BezierCurve::EvPoint( // returns false if unable to evaluate
       double t,         // evaluation parameter
...
bool MYON_BezierCurve::Ev1Der( // returns false if unable to evaluate
       double t,         // evaluation parameter
       MYON_3dPoint& point,
...
bool MYON_BezierCurve::Ev2Der( // returns false if unable to evaluate
       double t,         // evaluation parameter
       MYON_3dPoint& point,
