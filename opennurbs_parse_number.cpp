...
static int GetExplicitFormulaEndIndex(
  ...
  int str_index,
  int str_count,
  const MYON_ParseSettings& input_parse_settings,
  int& formula_index0,
  int& formula_index1
  )
{
...
static int ParseExplicitFormulaHelper(
  ...
  int str_index,
  int str_count,
  MYON_ParseSettings& input_parse_settings,
  ...
  )
{
  ...
  for(;;)
  {
    ...
    MYON_ParseSettings pr(MYON_ParseSettings::FalseSettings);
    int formula_count = formula_index1-formula_index0;
    int parsed_formula_count = MYON_ParseNumberExpression(
      str+formula_index0,
      formula_count,
...
static int MYON_ParsePiHelper( 
  const wchar_t* str,
  int str_index,
  int str_count,
  const MYON_ParseSettings& input_parse_settings,
  MYON_ParseSettings& pr,
  double* value
  )
{
...
static int MYON_ParseDoubleExponentHelper(
  const wchar_t* str,
  int str_index,
  int str_count,
  const MYON_ParseSettings& input_parse_settings,
...
static int MYON_ParseDoubleHelper( 
  const wchar_t* str,
  int str_index,
  int str_count,
  const MYON_ParseSettings& input_parse_settings,
  ...
  )
{
  ...
  int end_of_exponent_index = MYON_ParseDoubleExponentHelper(str,str_index,str_count,input_parse_settings,
  ...
  double x = MYON_UNSET_VALUE;
  if ( nullptr == MYON_String::ToNumber(buffer,MYON_DBL_QNAN,&x) )
    return 0;
  if ( 0 != value )
    *value = x;
...
{
  const MYON_ParseSettings input_parse_settings(parse_settings);
  double x = MYON_UNSET_VALUE;
  MYON_ParseSettings pr = MYON_ParseSettings::FalseSettings;
  if ( 0 != value )
    *value = MYON_UNSET_VALUE;
  if ( 0 != parse_results )
    *parse_results = pr;
  ...
  if ( -1 == str_count )
  {
    ...
    str_count = MYON_ParseSettings::max_number_str_count; 
  }
  ...
  for(;;)
  {
    ...
    str_index = MYON_ParseDoubleHelper(
      str, str_index0, str_count,
      input_parse_settings, pr, &x
      );
    ...
    MYON_ParseSettings parse_int = MYON_ParseSettings::IntegerNumberSettings;
    ...
    double x1 = MYON_UNSET_VALUE;
    MYON_ParseSettings pr1(MYON_ParseSettings::FalseSettings);
    int str_index1 = MYON_ParseDoubleHelper(
    ...
    if ( !MYON_IsValid(x1) || x1 != floor(x1) )
    {
      break;
    }
    ...
    double x2 = MYON_UNSET_VALUE;
    MYON_ParseSettings pr2(MYON_ParseSettings::FalseSettings);
    int str_index2 = MYON_ParseDoubleHelper(
    ...
    if ( !MYON_IsValid(x2) || x2 != floor(x2) || !(x2 > x1) )
    {
      break;
    }
...
static void SetParseExpressionError(
  const MYON_ArithmeticCalculator& calculator,
  MYON_ParseSettings* parse_results
  )
{
  if ( 0 != parse_results )
...
#define MYON_PARSE_FUNCTIONS
#if defined(MYON_PARSE_FUNCTIONS)
...
static int MYON_ParseFunctionHelper( 
  const wchar_t* str,
  int str_count,
  const MYON_ParseSettings& input_parse_settings,
  MYON_ParseSettings& pr,
  ...
  )
{
  ...
  tagFUNCTION* f = 0;
  double f_value = MYON_UNSET_VALUE;
  ...
  for(;;)
  {
    ...
    // with Dale Lear.
    MYON_ParseSettings psRadiansParameter(input_parse_settings);
    psRadiansParameter.SetDefaultAngleUnitSystem(MYON::AngleUnitSystem::Radians);
    ...
    {
      MYON_ParseSettings pr1(MYON_ParseSettings::FalseSettings);
      f_parameters[f_parameter_count] = MYON_UNSET_VALUE;
      int str_index2 = f->m_bRadiansParameter
        ? MYON_ParseAngleExpression(str + str_index1, str_count - str_index1 - 1, psRadiansParameter, MYON::AngleUnitSystem::Radians, &f_parameters[f_parameter_count], &pr1, 0)
        : MYON_ParseNumberExpression(str+str_index1,str_count-str_index1-1,input_parse_settings,&pr1,&f_parameters[f_parameter_count]);
      pr |= pr1;
      if ( str_index2 <= 0 )
        break;
      if ( pr1.ParseError() )
        break;
      if ( !MYON_IsValid(f_parameters[f_parameter_count]) )
        break;
  ...
  )
{
  ...
  MYON_ParseSettings input_parse_settings(parse_settings);
  ...
  MYON_ArithmeticCalculator calculator;
  ...
  while( str_index < str_count && 0 != str[str_index] )
  {
    ...
    double x1 = MYON_UNSET_VALUE;
    MYON_ParseSettings pr1(MYON_ParseSettings::FalseSettings);
    int str_index1 = MYON_ParseNumber(str+str_index,str_count-str_index,input_parse_settings,&pr1,&x1);

    #if defined(MYON_PARSE_FUNCTIONS)
    if ( str_index1 == 0 )
    {
      MYON_ParseSettings pr2(MYON_ParseSettings::FalseSettings);
      str_index1 = MYON_ParseFunctionHelper(
  ...
  if ( !calculator.Evaluate(&x) )
  {
    x = MYON_UNSET_VALUE;
    str_index = 0;
    pr = MYON_ParseSettings::FalseSettings;
...
{
  double x = MYON_UNSET_VALUE;
  MYON::LengthUnitSystem length_us = MYON::LengthUnitSystem::None;
  const MYON_ParseSettings input_parse_settings(parse_settings);
  MYON_ParseSettings pr = MYON_ParseSettings::FalseSettings;
  if ( 0 != length_value )
  ...
  int str_index = MYON_ParseNumberExpression(str,str_count,input_parse_settings,&pr,&x);
  if ( str_index <= 0 )
    return 0;
  ...
  MYON_ParseSettings parse_unit_name_settings(input_parse_settings);
  ...
  const int end_of_unit_index = MYON_ParseLengthUnitName(
    str+str_index,
    str_count,
  ...
  if ( end_of_unit_index > 0 )
  {
    ...
    {
      if ( MYON::LengthUnitSystem::Feet == length_us 
           && floor(x) == x
         ...
         )
      {
        ...
        MYON_ParseSettings parse_inches = input_parse_settings;
        DisableStartAndExpressionParsingStuff(parse_inches);
        parse_inches.SetParsePi(false);
        parse_inches.SetParseFeetInches(false);

        double inches_value = MYON_UNSET_VALUE;
        MYON::LengthUnitSystem inches_us = MYON::LengthUnitSystem::None;
        MYON_ParseSettings inches_pr = MYON_ParseSettings::FalseSettings;
        ...
        int end_of_inches_index = MYON_ParseLengthExpression(
        ...
        if (
          ...
          && MYON::LengthUnitSystem::None == inches_us
          && inches_value >= 0.0
          && inches_value < 12.0
          ...
          )
        {
          ...
          inches_us = MYON::LengthUnitSystem::Inches;
        }

        if ( end_of_inches_index > 0 
             && MYON::LengthUnitSystem::Inches == inches_us
           ...
           )
        {
          ...
          if ( x < 0.0 && inches_value > 0.0 )
            ...
            x = x*12.0 + inches_value;
          length_us = MYON::LengthUnitSystem::Inches;
  ...
  {
    length_us = MYON::LengthUnitSystem::None;
  }
...
static bool Internal_IsNegativeInteger(
  const MYON_ParseSettings& parse_settings,
  const wchar_t* str,
  int str_count
)
{
...
{
  double x = MYON_UNSET_VALUE;
  MYON::AngleUnitSystem angle_us = MYON::AngleUnitSystem::None;
  MYON_ParseSettings input_parse_settings(parse_settings);
  MYON_ParseSettings pr = MYON_ParseSettings::FalseSettings;
  if ( 0 != angle_value )
    *angle_value = MYON_UNSET_VALUE;
  ...
  int str_index = 0;
  MYON__UINT32 cSurveyorsNotationNS = 0;
  MYON__UINT32 cSurveyorsNotationEW = 0;
  if ( input_parse_settings.ParseSurveyorsNotation() )
  {
    ...
    MYON_ParseSettings pr1(MYON_ParseSettings::FalseSettings);
    int str_number_index = MYON_ParseNumberExpression(str+str_index,str_number_count,input_parse_settings,&pr1,&x);
    ...
    else
    {
      if ( input_parse_settings.ParseScientificENotation() )
      {
        ...
        str_number_index = MYON_ParseNumberExpression(str+str_index,str_number_count,input_parse_settings,&pr,&x);
        if ( str_number_index <= 0 )
          return 0;
  ...
  {
    str_index = MYON_ParseNumberExpression(str,str_count,input_parse_settings,&pr,&x);
    if ( str_index <= 0 )
      return 0;
  ...
  int end_of_unit_index = MYON_ParseAngleUnitName(str+str_index,str_count,parse_settings.PreferedLocaleId(),&angle_us);

  if (bNegativeDegreesMinutesSecondsIsPossible && MYON::AngleUnitSystem::Degrees != angle_us)
  ...
  if ( end_of_unit_index > 0 )
  {
    ...
    if ( -1 == str_count || str_index+1 < str_count )
    {
      if ( MYON::AngleUnitSystem::Degrees == angle_us 
           && floor(x) == x
      ...
      {
        // parse arc minutes
        MYON_ParseSettings next_parse_settings = input_parse_settings;
        ...
        {
          double next_value = MYON_UNSET_VALUE;
          MYON::AngleUnitSystem next_us(MYON::AngleUnitSystem::None);
          MYON_ParseSettings next_pr(MYON_ParseSettings::FalseSettings);
          int next_str_index = MYON_ParseAngleExpression(
            str+str_index,
          ...
          {
            if ( MYON::AngleUnitSystem::Minutes != next_us )
            {
              if ( MYON::AngleUnitSystem::Seconds == next_us && 0 == next_value_pass )
                next_value_pass = 1;
              else
              {
                bNegativeDegreesMinutesSecondsIsPossible = false;
          ...
          {
            if (MYON::AngleUnitSystem::Seconds != next_us)
            {
              bNegativeDegreesMinutesSecondsIsPossible = false;              
              break;
            }
          ...
          else if (MYON::AngleUnitSystem::Seconds == next_us)
        ...
        if ( 0 != arc_seconds_value )
        {
          ...
          angle_us = MYON::AngleUnitSystem::Seconds;
        }
        else if ( 0 != arc_minutes_value )
        {
          x = 60.0*x + arc_minutes_value;
          angle_us = MYON::AngleUnitSystem::Minutes;
        }
      }
    }
  }
  ...
  if ( str_index > 0 && 0 != cSurveyorsNotationNS )
  {
    ...
    if ( MYON::AngleUnitSystem::None == angle_us )
    {
      ...
      angle_us = MYON::AngleUnitSystem::Degrees;
    }
    ...
    // returned by MYON::AngleUnitScale(MYON::turns,angle_us) is exact 
    // for all supported angle units except radians where pi rounded
    // because the IEEE double must approximate the value of pi.
    const double right_angle = 0.25*MYON::AngleUnitScale(MYON::AngleUnitSystem::Turns,angle_us);
