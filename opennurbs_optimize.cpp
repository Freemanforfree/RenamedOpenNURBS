 ...
 */
{
  ...
  *t_addr = bx;
  ...
  if ( max_it < 2 )
  {
    MYON_ERROR("max_it must be >= 2");
    return 0;
  }
  if ( !MYON_IsValid(rel_stepsize_tol) || rel_stepsize_tol <= 0.0 || rel_stepsize_tol >= 1.0 )
  {
    MYON_ERROR("rel_stepsize_tol must be strictly between 0.0 and 1.0");
    return 0;
  }
  if ( !MYON_IsValid(abs_stepsize_tol) || abs_stepsize_tol <= 0.0 )
  {
    MYON_ERROR("abs_stepsize_tol must be > 0");
    return 0;
  }
  ...
  if (rc) {
    ...
    if ( rc < 0 ) {
      MYON_ERROR("MYON_FindLocalMinimum() f() failed to evaluate.");
    }
    *t_addr = x;
  ...
  *t_addr = x; // best known answer
  MYON_ERROR("MYON_FindLocalMinimum() failed to converge");
  return 2; // 2 means we failed to converge
}


MYON_LocalZero1::MYON_LocalZero1() 
: m_t0(MYON_UNSET_VALUE)
, m_t1(MYON_UNSET_VALUE)
, m_f_tolerance(0.0)
, m_t_tolerance(0.0)
, m_k(nullptr)
, m_k_count(0)
, m_s0(MYON_UNSET_VALUE)
, m_f0(MYON_UNSET_VALUE)
, m_s1(MYON_UNSET_VALUE)
, m_f1(MYON_UNSET_VALUE)
{}
...
bool
MYON_LocalZero1::BracketZero( double s0, double f0, 
                             double s1, double f1,
                             int level )
{
  double s, f, d;
...
bool
MYON_LocalZero1::BracketSpan( double s0, double f0, double s1, double f1 )
{
  int i0, i1, i;
  double fm, fp;
  bool rc = true;
  if ( m_k && m_k_count >= 3 ) {
    i0 = MYON_SearchMonotoneArray(m_k,m_k_count,s0);
    if ( i0 < 0 )
      i0 = 0;
    i1 = MYON_SearchMonotoneArray(m_k,m_k_count,s1);
    if ( i1 >= m_k_count )
      i1 = m_k_count-1;
...
bool MYON_LocalZero1::FindZero( double* t )
{
  // Find values of m_s0 and m_s1 between m_t0 and m_t1 such that
  // f(m_t0) and f(m_t1) have different signs

  if ( !MYON_IsValid(m_t0) )
  {
    if ( !MYON_IsValid(m_t1) )
    {
      MYON_ERROR("Illegal input - m_t0 and m_t1 are not valid.");
      return false;
    }
    m_s0 = m_s1 = m_t1;
  }
  else if ( !MYON_IsValid(m_t1) )
  {
    m_s0 = m_s1 = m_t0;
  }
  else if ( m_t0 <= m_t1 )
  ...
  if ( m_s0 == m_s1 )
  {
    if ( Evaluate( m_s0, &m_f0, nullptr, 1 ) ) 
    {
      ...
      MYON_ERROR("Illegal input - m_t0 = m_t1 and the function value is not zero at m_t0.");
      return false;
    }
    MYON_ERROR("Evaluation failed.");
    return false;
  }

  if ( !Evaluate( m_s0, &m_f0, nullptr, 1 ) )
  {
    MYON_ERROR("Evaluation failed at m_s0.");
    return false;
  }

  if ( !Evaluate( m_s1, &m_f1, nullptr, -1 ) )
  {
    MYON_ERROR("Evaluation failed at m_s1.");
    return false;
  }

  if ( !BracketZero( m_s0, m_f0, m_s1, m_f1 ) )
  {
    MYON_ERROR("Unable to bracket a zero of the function.");
    return false;
  }
  ...
  {
    MYON_ERROR("Unable to bracket the function in a span of m_k[].  m_k[] may be invalid.");
    return false;
  }

  if ( !NewtonRaphson( m_s0, m_f0, m_s1, m_f1, 128, t ) )
  {
    MYON_ERROR("Newton-Raphson failed to converge.  Is your function C2?");
    return false;
  }
...
bool MYON_LocalZero1::NewtonRaphson( double s0, double f0,
                                    double s1, double f1,
                                    int maxit, double* t )
{
  ...
  //
  //    When in doubt, ds_tolerance = (fabs(s0) + fabs(s1))*MYON_EPSILON
  //    works well.
