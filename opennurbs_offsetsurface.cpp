...
MYON_OBJECT_IMPLEMENT(MYON_OffsetSurface,MYON_SurfaceProxy,"00C61749-D430-4ecc-83A8-29130A20CF9C");

MYON_OffsetSurface::MYON_OffsetSurface()
                 : m__pSrf(0)
{
}
...
MYON_OffsetSurface::MYON_OffsetSurface( const MYON_OffsetSurface& src)
                 : MYON_SurfaceProxy(src),
                   m__pSrf(0),
                   m_offset_function(src.m_offset_function)
{
  if ( 0 != src.m__pSrf )
  {
    m__pSrf = src.MYON_SurfaceProxy::DuplicateSurface();
    SetProxySurface(m__pSrf);
  }
  m_offset_function.SetBaseSurface( BaseSurface() );
}

MYON_OffsetSurface& MYON_OffsetSurface::operator=(const MYON_OffsetSurface& src)
{
  if ( this != &src )
  {
    if ( 0 != m__pSrf && this != m__pSrf )
...
MYON_OffsetSurfaceFunction& MYON_OffsetSurface::OffsetFunction()
{
  return m_offset_function;
}

const MYON_OffsetSurfaceFunction& MYON_OffsetSurface::OffsetFunction() const
{
  return m_offset_function;
}


bool MYON_OffsetSurface::SetBaseSurface(
  ...
  if ( this != base_surface )
  {
    ...
    if ( 0 == base_surface )
    {
      if ( 0 != m__pSrf && this != m__pSrf )
        delete m__pSrf;
      m__pSrf = 0;
      MYON_SurfaceProxy::SetProxySurface(0);
      m_offset_function.SetBaseSurface(0);
    }
    else if ( BaseSurface() != base_surface )
    {
      if ( 0 != m__pSrf && this != m__pSrf )
        delete m__pSrf;
      m__pSrf = 0;
      MYON_SurfaceProxy::SetProxySurface(base_surface);
    }
    m_offset_function.SetBaseSurface( BaseSurface() );
  }
  return rc;
...
const MYON_Surface* MYON_OffsetSurface::BaseSurface() const
{
  return ProxySurface();
}

bool MYON_OffsetSurface::GetBBox(
       double* bbox_min,
       double* bbox_max,
       bool bGrowBox
       ) const
{
  bool rc = MYON_SurfaceProxy::GetBBox(bbox_min,bbox_max);
  if ( rc )
  {
...
bool MYON_OffsetSurface::Evaluate(
       double s, double t,
       int der_count,
       int v_stride,
       double* v,
       ...
       ) const
{
  ...
  double* vv = v;
  MYON_3dVector srf_value[6];//, normal_value[3];
  if ( der_count < 2 )
  {
    vv = &srf_value[0].x;
    vv_stride = 3;
  ...
  bool rc = MYON_SurfaceProxy::Evaluate(s,t,(der_count>2?der_count:2),vv_stride,vv,side,hint);
  ...
  if (rc)
  {
    ...
    if (rc)
    {
      ...
      {
        MYON_3dVector Ns, Nt;
        MYON_EvNormalPartials(srf_value[1], srf_value[2],
                  srf_value[3], srf_value[4], srf_value[5],
                  Ns, Nt);
...
void MYON_BumpFunction::Internal_EvaluateLinearBump(double t, double dt, int der_count, double* value) const
{
  value[0] = t;
  if (der_count>0)
  {
...
void MYON_BumpFunction::Internal_EvaluateQuinticBump(double t, double dt, int der_count, double* value) const
{
  // c(t) = (1-t)^3 * (1 + 3t + 6t^2)
  //bool neg = (t<0.0);
  //t = fabs(t);
...
void MYON_BumpFunction::Evaluate(double s, double t, int der_count, double* value) const
{
  double tmp[20];
  double* xvalue;
  double* yvalue;
...
bool MYON_OffsetSurfaceFunction::SetBaseSurface( const MYON_Surface* srf )
{
  bool rc = false;
  Destroy();
  m_srf = srf;
...
bool MYON_OffsetSurfaceFunction::SetSideTangency(
  int side,
  bool bEnable
  )
{
...
bool MYON_OffsetSurfaceFunction::SideTangency(int side) const
{
...
int MYON_OffsetSurfaceFunction::OffsetPointCount() const
{
  return (0 != m_srf) ? m_offset_value.Count() : 0;
}


double MYON_OffsetSurfaceFunction::DistanceAt(
  double s,
  double t
  ) const
{
...
bool MYON_OffsetSurfaceFunction::EvaluateDistance(
      double s,
      double t,
      int num_der,
      double* value
      ) const
{
  ...
  bool rc = const_cast<MYON_OffsetSurfaceFunction*>(this)->Initialize();
...
MYON_3dPoint MYON_OffsetSurfaceFunction::PointAt(
  double s,
  double t
  ) const
{
  ...
  MYON_3dVector N(MYON_3dVector::NanVector);
  if ( 0 != m_srf )
  {
...
MYON_2dPoint MYON_OffsetSurfaceFunction::OffsetSurfaceParameter(int i) const
{
  MYON_2dPoint p(MYON_UNSET_VALUE,MYON_UNSET_VALUE);
...
const MYON_Surface* MYON_OffsetSurfaceFunction::BaseSurface() const
{
  return m_srf;
}

double MYON_OffsetSurfaceFunction::OffsetDistance(int i) const
{
...
bool MYON_OffsetSurfaceFunction::SetOffsetPoint(
  double s,
  double t,
  double distance,
  double radius
...
{
  bool rc = false;
  if ( MYON_IsValid(s) && MYON_IsValid(t) && MYON_IsValid(distance) && MYON_IsValid(radius) )
  {
    ...
    const double dTol = MYON_SQRT_EPSILON; // tiny border around untrimmed edges
    ...
    {
      MYON_OffsetSurfaceValue offset_value;
      offset_value.m_s = s;
      offset_value.m_t = t;
...
bool MYON_OffsetSurfaceFunction::SetDistance( int index, double distance)
{
...
bool MYON_OffsetSurfaceFunction::SetPoint( int index, double s, double t)
{
  int count = m_offset_value.Count();
  if( index < 0 || index > count-1)
    return false;
...
void MYON_OffsetSurfaceFunction::Destroy()
{
  m_srf = 0;
  ...
  m_domain[0] = MYON_Interval::EmptyInterval;
  m_domain[1] = MYON_Interval::EmptyInterval;
  m_bumps.SetCount(0);
  m_bValid = false;
}
...
MYON_OffsetSurfaceFunction::MYON_OffsetSurfaceFunction()
{
  Destroy();
}
...
bool MYON_OffsetSurfaceFunction::Initialize()
{
  ...
  if ( !m_bValid && 0 != m_srf && count > 0)
  {
    ...
    for (i = 0; i < count; i++ )
    {
      MYON_BumpFunction& bump = m_bumps.AppendNew();
      MYON_OffsetSurfaceValue offset_value = m_offset_value[i];
    ...
    MYON_Matrix M(count,count);
    double* B = (double*)onmalloc(2*count*sizeof(*B));
    double* X = B + count;
    int j;
    for ( i = 0; i < count; i++ ) 
    {
      MYON_2dPoint p = m_bumps[i].m_point;
    ...
    int rank = M.RowReduce(MYON_ZERO_TOLERANCE,B);
    if ( count == rank )
    {
      if ( M.BackSolve(MYON_ZERO_TOLERANCE,count,B,X) )
      {
        m_bValid = true;
      }
    }
...
MYON_BumpFunction::MYON_BumpFunction()
{
  m_type[0] = 0;
  m_type[1] = 0;
  m_sx[0] = 0.0;
...
double MYON_BumpFunction::ValueAt(
  double s,
  double t
  ) const
{
