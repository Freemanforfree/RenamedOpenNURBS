...
static
unsigned int MYON_VersionNumberTime(
  unsigned int year,
  unsigned int month,
  ...
  )
{
  ...
  day_of_year = (year < 2100 && year >= 2000)
    ? MYON_DayOfGregorianYear(year,month,day_of_month)
    : 0;
...
static
bool MYON_VersionNumberTimeDeconstruct(
  unsigned int time,
  ...
  )
{
  ...
  if (u > 0)
  {
    ...
    {
      year = u + 2000;
      if ( !MYON_GetGregorianMonthAndDayOfMonth(year,day_of_year,&month,&day_of_month) )
      {
        year = 0;
        month = 0;
        day_of_month = 0;
  ...
  )
{
  ...
  unsigned int version_time = MYON_VersionNumberTime(year,month,day_of_month);
  ...
  if ( minor_version > MYON::VersionMinorMaximum())
    minor_version = MYON::VersionMinorMaximum();
  if ( major_version > MYON::VersionMajorMaximum())
    major_version = MYON::VersionMajorMaximum();
  version_number = ((major_version*(MYON::VersionMinorMaximum()+1) + minor_version)*(TIME_MAX+1) + version_time)*(BRANCH_MAX+1) + platform;
...
{
  unsigned int version_number = 0;
  if ( MYON_VersionNumberIsYearMonthDateFormat(version_number_major,YYYYMMDDn_version_number) )
  {
...
unsigned int MYON_VersionNumberMinimum(unsigned int major_version)
{
  unsigned int version_number_minimum = MYON_VersionNumberConstruct(
    major_version,0,
    2000,1,1,
    0
    );
  ...
  )
{
     ...
     )
  {
    ...
    if (    year >= min_old_format_year 
         ...
         && month >= 1 && month <= 12
         && day_of_month >= 1 && day_of_month <= MYON_DaysInMonthOfGregorianYear(year,month)
       )
    {
...
{
  bool rc = ( version_number >= MYON_VersionNumberMinimum(1) );
  if (rc)
  {
    ...
    if (rc)
    {
      const unsigned int v = MYON_VersionNumberConstruct(version_major,version_minor,version_year,version_month,version_day_of_month,version_branch);
      if ( v != version_number )
        rc = false;
    }
  }
  ...
  )
{
  ...
  if ( MYON_VersionNumberIsYearMonthDateFormat(0,u) )
  {
    unsigned int n = u%10;
    u /= 10;
    ...
    if (0 == n && 200612060 == version_number)
      ...
      n = 5; // debug maps to 5
    u = MYON_VersionNumberConstruct(n,0,u,mm,dd,0);
  }

  if ( 0 == (0x80000000 & u) )
  {
    // This number was not created by MYON_VersionNumberConstruct()
    u = 0;
  }
  else
  {
    // Clear the bit that makes new version numbers
    // created MYON_VersionNumberConstruct()
  ...
  rc = MYON_VersionNumberTimeDeconstruct(
    version_time,
    version_year,
    version_month,
    version_day_of_month);

  max = MYON::VersionMinorMaximum()+1;
  if ( 0 != version_minor )
    *version_minor = u % max;
  u /= max;
...
)
{
  ...
  if (false == MYON_VersionNumberIsValid(version_number))
  {
  ...
  {
    bDateFormat = MYON_VersionNumberParse(
      version_number,
      &version_major,
      &version_minor,
      &version_year,
  ...
  MYON_String str_version;
  if (bUnsignedFormat)
  {
    str_version = MYON_String::FormatToString("%u", version_number);
    if (bDateFormat)
      str_version += " (";
  }
  ...
  if (bDateFormat)
  {
    str_version += MYON_String::FormatToString(
      "%u.%u %04u-%02u-%02u:%u", 
    version_major,
    version_minor, 
    version_year, 
  ...
  if (str_version.IsEmpty())
  {
    str_version =
      (0 != version_number)
      ? MYON_String::FormatToString("0x%08X", version_number)
      : "0";
  }
...
{
  return MYON_wString(MYON_VersionNumberToString(version_number, bUnsignedFormat, bDateFormat));
}
...
{
  const bool bValidVersionNumber = MYON_VersionNumberIsValid(sdk_version_number);
  const bool bValidServiceReleaseNumber = MYON_VersionNumberIsValid(sdk_service_release_number);
  MYON_String str_version = MYON_VersionNumberToString(sdk_version_number, true, false);
  str_version += ".";
  str_version += MYON_VersionNumberToString(sdk_service_release_number, true, false);
  if (bValidVersionNumber || bValidServiceReleaseNumber)
  {
    ...
    {
      str_version += MYON_VersionNumberToString(bValidVersionNumber ? sdk_version_number : sdk_service_release_number, false, true);
    }
    else
    {
      str_version += MYON_VersionNumberToString(sdk_version_number, false, true);
      str_version += ", ";
      str_version += MYON_VersionNumberToString(sdk_service_release_number, false, true);
    }
    str_version += ")";
  }
  return str_version;
...
{
  return MYON_wString(MYON_SdkVersionNumberToString(sdk_version_number, sdk_service_release_number));
}
  ...
  )
{
  ...
  unsigned int version_number = MYON_VersionNumberConstruct(
                                      major_version,
                                      minor_version,
                                      year,
                                      month,
  ...
  for(i = 0; i < 4; i++ )
  {
    ...
    case 1:
      ...
      break;
    case 2:
      item_value = (year-2000)*1000 + MYON_DayOfGregorianYear(year,month,day_of_month);
      break;
    case 3:
  ...
  )
{
  ...
  unsigned int version_number = MYON_GetVersionQuartet(
                                      major_version,
                                      minor_version,
                                      year,
                                      month,
  ...
  )
{
  ...
  string_length = MYON_GetVersionString(
                    major_version,
                    minor_version,
                    year,
                    month,
