...
void MYON_FreeTypeGetFontMetrics(
  const class MYON_Font* font,
  class MYON_FontMetrics& font_unit_font_metrics
  );
...
Parameters:
  ...
  figure_type - [in]
    If known for certain, pass in figure_type. Otherwise, pass in MYON_OutlineFigure::Type::Unset.
  outline - [out]
    outline and metrics in font design units
*/
MYON_DECL
...
Parameters:
  ft_face - [in]
    A pointer to and FT_Face. One way to get this value is to call MYON_Font::FreeTypeFace()
  font_glyph_id - [in]
    font glyph id
Returns:
