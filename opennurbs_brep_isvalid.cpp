...
bool
MYON_Brep::IsValidVertexTopology( int vertex_index, MYON_TextLog* text_log ) const
{
...
bool
MYON_Brep::IsValidFaceTopology( int face_index, MYON_TextLog* text_log  ) const
{
  ...
  for ( fli = 0; fli < face_loop_count; fli++ ) 
  {
    ...
    if ( fli == 0 ) {
      ...
      if ( loop.m_type != MYON_BrepLoop::inner )
      {
        if ( text_log )
        {
          text_log->Print("brep.m_F[%d] face is not valid.\n",face_index);
...
bool
MYON_Brep::IsValidEdgeTopology( int edge_index, MYON_TextLog* text_log ) const
{
  ...
  {
    if ( text_log )
      text_log->Print("MYON_Brep.m_E[%d].m_c3i = %d, but m_C3[%d] is nullptr.\n",edge_index,c3i,c3i);
    return false;
  }
  ...
  MYON_Interval proxy_sub_dom = edge.ProxyCurveDomain();
  ...
  MYON_Interval c3_dom = m_C3[c3i]->Domain();
  ...
  MYON_Interval edge_dom = edge.Domain();
...
bool
MYON_Brep::IsValidLoopTopology( int loop_index, MYON_TextLog* text_log ) const
{
  int lti, ti;
  ...
  {
      if ( text_log )
        text_log->Print("MYON_Brep.m_L[%d].m_ti.Count() = %d  (should be > 0 )\n",loop_index,loop.m_ti.Count());
      return false;
  }
...
bool
MYON_Brep::IsValidTrimTopology( int trim_index, MYON_TextLog* text_log ) const
{
  ...
  MYON_Interval proxy_sub_dom = trim.ProxyCurveDomain();
  if ( !proxy_sub_dom.IsIncreasing() )
  {
    if ( text_log )
    {
  ...
  MYON_Interval c2_dom = m_C2[trim.m_c2i]->Domain();
  ...
  MYON_Interval trim_dom = trim.Domain();
  ...
  if ( trim.m_type == MYON_BrepTrim::singular )
  {
    ...
    {
      if ( text_log )
        text_log->Print("MYON_Brep.m_T[%d].m_type = singular, but m_bRev3d = %d (should be 0).\n",trim_index,trim.m_bRev3d);
      return false;
    }
    if ( trim.m_vi[0] != trim.m_vi[1] )
    {
      if ( text_log )
        text_log->Print("MYON_Brep.m_T[%d].m_type = singular, but m_vi = (%d,%d) (should be same vertex index).\n",
...
bool
MYON_Brep::IsValidTopology( MYON_TextLog* text_log ) const
{
  ...
  for ( ei = 0; ei < edge_count; ei++ ) 
  {
    if ( m_E[ei].m_edge_index == -1 )
    {
      ...
      {
        if ( text_log )
          text_log->Print( "MYON_Brep.m_E[%d] is deleted (m_edge_index = -1) but edge.m_ei.Count() = %d.\n",
                           ei, edge.m_ti.Count() );
        return false;
      }
      if ( edge.m_c3i != -1 )
  ...
  for ( ti = 0; ti < trim_count; ti++ )
  {
    ...
    if ( !IsValidTrimTopology( ti, text_log ) ) {
      if ( text_log )
        text_log->Print("MYON_Brep.m_T[%d] is invalid.\n",ti);
      return false;
    }
  }
  ...
  for ( li = 0; li < loop_count; li++ )
  {
    ...
    if ( !IsValidLoopTopology( li, text_log ) ) {
      if ( text_log )
        text_log->Print("MYON_Brep.m_L[%d] is invalid.\n",li);
      return false;
    }
  }
...
bool
MYON_Brep::IsValidVertexGeometry( int vertex_index, MYON_TextLog* text_log ) const
{
  if ( vertex_index < 0 || vertex_index >= m_V.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidEdgeGeometry( int edge_index, MYON_TextLog* text_log ) const
{
  if ( edge_index < 0 || edge_index >= m_E.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidFaceGeometry( int face_index, MYON_TextLog* text_log ) const
{
  if ( face_index < 0 || face_index >= m_F.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidLoopGeometry( int loop_index, MYON_TextLog* text_log ) const
{
  if ( loop_index < 0 || loop_index >= m_L.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidTrimGeometry( int trim_index, MYON_TextLog* text_log ) const
{
  if ( trim_index < 0 || trim_index >= m_T.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidGeometry( MYON_TextLog* text_log ) const
{
  const int curve2d_count = m_C2.Count();
  const int curve3d_count = m_C3.Count();
  const int surface_count = m_S.Count();
...
bool
MYON_Brep::IsValidVertexTolerancesAndFlags( int vertex_index, MYON_TextLog* text_log ) const
{
  if ( vertex_index < 0 || vertex_index >= m_V.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidEdgeTolerancesAndFlags( int edge_index, MYON_TextLog* text_log ) const
{
  if ( edge_index < 0 || edge_index >= m_E.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidFaceTolerancesAndFlags( int face_index, MYON_TextLog* text_log ) const
{
  if ( face_index < 0 || face_index >= m_F.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidLoopTolerancesAndFlags( int loop_index, MYON_TextLog* text_log ) const
{
  if ( loop_index < 0 || loop_index >= m_L.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidTrimTolerancesAndFlags( int trim_index, MYON_TextLog* text_log ) const
{
  if ( trim_index < 0 || trim_index >= m_T.Count() )
  {
    if ( text_log )
...
bool
MYON_Brep::IsValidTolerancesAndFlags( MYON_TextLog* text_log ) const
{
  const int vertex_count  = m_V.Count();
  const int edge_count    = m_E.Count();
  const int trim_count    = m_T.Count();
