...
MYON_BoundingBox::MYON_BoundingBox() MYON_NOEXCEPT
  : m_min(1.0,0.0,0.0)
  , m_max(-1.0,0.0,0.0)
{}

MYON_BoundingBox::MYON_BoundingBox(
  const MYON_3dPoint& min_pt,
  const MYON_3dPoint& max_pt 
  )
  : m_min( min_pt )
  , m_max( max_pt )
{}

void MYON_BoundingBox::Destroy()
{
  *this = MYON_BoundingBox::EmptyBoundingBox;
}
...
// bounding box.
bool MYON_BoundingBox::Transform( const MYON_Xform& xform )
{
...
double MYON_BoundingBox::Tolerance() const
{
  // rough guess at a tolerance to use for comparing
  return MYON_BoundingBoxTolerance( 3, m_min, m_max );
}

MYON_3dPoint& MYON_BoundingBox::operator[](int i)
{
  return (i>0) ? m_max : m_min;
}

const MYON_3dPoint& MYON_BoundingBox::operator[](int i) const
{
  return (i>0) ? m_max : m_min;
}

MYON_3dPoint MYON_BoundingBox::Min() const 
{
	return m_min;
}

MYON_3dPoint MYON_BoundingBox::Max() const 
{
	return m_max;
}

MYON_3dVector MYON_BoundingBox::Diagonal() const
{
  return m_max - m_min;
}

MYON_3dPoint MYON_BoundingBox::Center() const
{
  return 0.5*(m_max+m_min);
}

MYON_3dPoint MYON_BoundingBox::Corner( int x_index, int y_index, int z_index ) const
{
  // 8 corners of box
...
bool
MYON_BoundingBox::GetCorners( 
  MYON_3dPoint corners[8]// returns list of 8 corner points
  ) const
{
  int n = 0;
  if ( IsValid() ) 
...
MYON_Line MYON_BoundingBox::Edge(unsigned int index) const
{
  switch (index)
  {
  case 0u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_min.y, m_min.z), MYON_3dPoint(m_max.x, m_min.y, m_min.z));
  case 1u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_min.y, m_min.z), MYON_3dPoint(m_min.x, m_max.y, m_min.z));
  case 2u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_min.y, m_min.z), MYON_3dPoint(m_min.x, m_min.y, m_max.z));
  case 3u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_min.y, m_max.z), MYON_3dPoint(m_max.x, m_min.y, m_max.z));
  case 4u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_min.y, m_max.z), MYON_3dPoint(m_min.x, m_max.y, m_max.z));
  case 5u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_max.y, m_min.z), MYON_3dPoint(m_max.x, m_max.y, m_min.z));
  case 6u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_max.y, m_min.z), MYON_3dPoint(m_min.x, m_max.y, m_max.z));
  case 7u:
    return MYON_Line(MYON_3dPoint(m_max.x, m_min.y, m_min.z), MYON_3dPoint(m_max.x, m_min.y, m_max.z));
  case 8u:
    return MYON_Line(MYON_3dPoint(m_max.x, m_min.y, m_min.z), MYON_3dPoint(m_max.x, m_max.y, m_min.z));
  case 9u:
    return MYON_Line(MYON_3dPoint(m_min.x, m_max.y, m_max.z), MYON_3dPoint(m_max.x, m_max.y, m_max.z));
  case 10u:
    return MYON_Line(MYON_3dPoint(m_max.x, m_min.y, m_max.z), MYON_3dPoint(m_max.x, m_max.y, m_max.z));
  case 11u:
    return MYON_Line(MYON_3dPoint(m_max.x, m_max.y, m_min.z), MYON_3dPoint(m_max.x, m_max.y, m_max.z));
  default:
    index %= 12; return Edge(index);
  }
}

bool MYON_BoundingBox::GetEdges( 
  MYON_Line edges[12] 
  ) const
{
  MYON_Line line;
  ...
  {
    edges[0].from = MYON_3dPoint::UnsetPoint;
    edges[0].to   = MYON_3dPoint::UnsetPoint;
...
bool
MYON_BoundingBox::GetCorners( 
  MYON_3dPointArray& corners// returns list of 8 corner points
  ) const
{
  MYON_3dPoint c[8];
  corners.Empty();
...
MYON_ClippingRegion::MYON_ClippingRegion()
{
  memset(this,0,sizeof(*this));
}


bool MYON_ClippingRegion::SetObjectToClipTransformation(
  const MYON_Xform object_to_clip
  )
{
  ...
  if (false == rc)
    m_inverse_xform = MYON_Xform::ZeroTransformation;
  return rc;
}

bool MYON_ClippingRegion::SetObjectToClipTransformation(
  const MYON_Viewport& viewport
  )
{
  if (false == viewport.GetXform(
    MYON::coordinate_system::world_cs,
    MYON::coordinate_system::clip_cs,
    m_xform
    ))
  {
    m_xform = MYON_Xform::IdentityTransformation;
    m_inverse_xform = MYON_Xform::IdentityTransformation;
    return false;
  }
...
MYON_Xform MYON_ClippingRegion::ObjectToClipTransformation() const
{
  return m_xform;
}

MYON_Xform MYON_ClippingRegion::InverseObjectToClipTransformation() const
{
  return m_inverse_xform;
}

void MYON_ClippingRegion::SetClipPlaneTolerance( double clip_plane_tolerance )
{
  if ( clip_plane_tolerance > 0.0 && MYON_IsValid(clip_plane_tolerance) )
    m_clip_plane_tolerance = clip_plane_tolerance;
  else
    m_clip_plane_tolerance = 0.0;
}

double MYON_ClippingRegion::ClipPlaneTolerance() const
{
  return m_clip_plane_tolerance;
}

int MYON_ClippingRegion::InViewFrustum( 
  MYON_3dPoint P
  ) const
{
...
{
  if (    !MYON_IsValid(bbox.m_min.x) 
       || !MYON_IsValid(bbox.m_max.x) 
...
int MYON_ClippingRegion::InClipPlaneRegion( 
  MYON_3dPoint P
  ) const
{
...
{
  const MYON_PlaneEquation* cpeqn;
  const float* cv;
  double x;
...
int MYON_ClippingRegion::IsVisible( MYON_3dPoint P ) const
{
  return IsVisible(1,&P);
}

int MYON_ClippingRegion::IsVisible( const MYON_BoundingBox& bbox ) const
{
  if (    !MYON_IsValid(bbox.m_min.x) 
       || !MYON_IsValid(bbox.m_max.x) 
       || bbox.m_min.x > bbox.m_max.x
...
int MYON_ClippingRegion::IsVisible( int count, const MYON_3fPoint* p ) const
{
  const double* xform;
  const MYON_PlaneEquation* cpeqn;
  const float* cv;
...
int MYON_ClippingRegion::IsVisible( int count, const MYON_3dPoint* p ) const
{
  const double* xform;
  const MYON_PlaneEquation* cpeqn;
  const double* cv;
...
int MYON_ClippingRegion::IsVisible( int count, const MYON_4dPoint* p ) const
{
  const double* xform;
  const MYON_PlaneEquation* cpeqn;
  const double* cv;
...
unsigned int MYON_ClippingRegion::TransformPoint(
                     const MYON_4dPoint& P, 
                     MYON_4dPoint& Q
                     ) const
{
...
{
  MYON_3dPoint PP(P.x,P.y,P.z);
  return TransformPoint(PP,Q);
}

int MYON_ClippingRegion::TransformPoints( int count, MYON_4dPoint* p, unsigned int* pflags ) const
{
...
int MYON_ClippingRegion::TransformPoints( int count, MYON_4dPoint* p ) const
{
  // transforms cv's to pick coordinates
  const double* xform;
  const MYON_PlaneEquation* cpeqn;
...
bool MYON_ClippingRegion::GetLineClipPlaneParamters( 
       MYON_4dPoint P0, 
       MYON_4dPoint P1, 
       double* t0, 
       double* t1 
...
MYON_ClippingRegionPoints::~MYON_ClippingRegionPoints()
{
  Destroy();
}

MYON_ClippingRegionPoints::MYON_ClippingRegionPoints(const MYON_ClippingRegionPoints& src)
{
  // All "this" members are initialized in the class definition.
  *this = src;
}

MYON_ClippingRegionPoints& MYON_ClippingRegionPoints::operator=(const MYON_ClippingRegionPoints& src)
{
  if (this != &src)
  {
    Clear();
    if (src.m_point_count > 0 && nullptr != src.m_clip_points && nullptr != src.m_clip_flags)
    {
      ...
      const MYON_3dPoint* src_p = src.m_clip_points;
      MYON_3dPoint* p = m_clip_points;
      MYON_3dPoint* p1 = p + src.m_point_count;
      while (p < p1)
        *p++ = *src_p++;
...
MYON_ClippingRegionPoints::MYON_ClippingRegionPoints( MYON_ClippingRegionPoints&& src) MYON_NOEXCEPT
{
  m_point_count = src.m_point_count;
  m_point_capacity = src.m_point_capacity;
  m_clip_points = src.m_clip_points;
...
MYON_ClippingRegionPoints& MYON_ClippingRegionPoints::operator=(MYON_ClippingRegionPoints&& src)
{
  if (this != &src)
  {
    Destroy();
...
unsigned int MYON_ClippingRegionPoints::PointCapacity() const
{
  return m_point_capacity;
}

unsigned int MYON_ClippingRegionPoints::PointCout() const
{
...
Description:
  ...
  to the heap.  When an MYON_ClippingRegionPoints will be used
  multiple times, it is ore efficient to call Clear() between
  uses than calling Destroy().
*/
void MYON_ClippingRegionPoints::Clear()
{
  m_point_count = 0;
  m_and_clip_flags = 0;
  m_or_clip_flags = 0;
...
*/
void MYON_ClippingRegionPoints::Destroy()
{
...
MYON_3dPoint MYON_ClippingRegionPoints::ClipPoint(
...
unsigned int MYON_ClippingRegionPoints::ClipFlag(
  unsigned int point_index
  ) const
{
  return
...
bool MYON_ClippingRegionPoints::AppendClipPoint(
  const class MYON_ClippingRegion& clipping_region,
...
bool MYON_ClippingRegionPoints::AppendClipPoints(
  const class MYON_ClippingRegion& clipping_region,
  const MYON_SimpleArray<MYON_3dPoint>& world_points
  )
{
...
bool MYON_ClippingRegionPoints::AppendClipPoints(
  ...
  const MYON_3dPoint* world_points
  )
{
  ...
  )
{
  ...
  MYON_3dPoint clip_point;
  for (const double* P1 = world_points + world_point_stride*world_point_count; P < P1; P += world_point_stride)
  {
    clip_flags = clipping_region.TransformPoint(*((const MYON_3dPoint*)P),clip_point);
...
bool MYON_ClippingRegionPoints::ReserveBufferPointCapacity(
  ...
  )
{
  ...
  MYON_3dPoint* p0 = (MYON_3dPoint*)b;
  ...
  if (m_point_count > 0 && m_point_count <= m_point_capacity && nullptr != m_clip_points && nullptr != m_clip_flags)
  {
    if (p0 != m_clip_points)
    {
      const MYON_3dPoint* src_p = m_clip_points;
      MYON_3dPoint* p = p0;
      MYON_3dPoint* p1 = p0 + m_point_count;
      while (p < p1)
        *p++ = *src_p++;
    }
    if (f0 != m_clip_flags)
...
bool MYON_PickPoint::IsSet() const
{
  return (MYON_UNSET_VALUE != m_point.x && m_depth > MYON_UNSET_VALUE && m_distance < 1.0e300);
}

bool MYON_PickPoint::IsNotSet() const
{
  return (false == IsSet());
}


int MYON_PickPoint::Compare(
  const MYON_PickPoint& a,
  const MYON_PickPoint& b
  )
{
  if (false == a.IsSet())
    return b.IsSet() ? -1 : 0;
...
int MYON_BoundingBox::IsVisible( 
...
const double* p;
  ...
  if ( !MYON_IsValid(m_min.x) || !MYON_IsValid(m_max.x) || m_min.x > m_max.x)
    return 0;
...
bool MYON_BoundingBox::IsPointIn( const MYON_3dPoint& p, int bStrictlyIn ) const
{
  bool bIn = false;
  if ( bStrictlyIn ) {
...
MYON_3dPoint MYON_BoundingBox::ClosestPoint( 
  const MYON_3dPoint& test_point
  ) const
{
  MYON_3dPoint near_point = test_point;
...
int MYON_BoundingBox::GetClosestPoint( 
  const MYON_Line& line, MYON_3dPoint& box_point, double* t0, double* t1
  ) const
{
  if(!IsValid() || !line.IsValid()) 
		return 0;			
	
	MYON_3dPoint closest;
	
	if(line.Direction().Length()<=MYON_SQRT_EPSILON){
		MYON_3dPoint center = line.PointAt(.5);
		if(t0) *t0 = 0.0;
		if(t1) *t1 = 1.0;
	...
	MYON_Interval over[3];			//parameter overlap for each direction

	for(int j=0; j<3; j++){
		MYON_Interval pl( line[0][j], line[1][j]);
		if( pl[0]!=pl[1])
			over[j]= MYON_Interval(	pl.NormalizedParameterAt(Min()[j]),
														pl.NormalizedParameterAt(Max()[j])  );
		else 
			if( Min()[j]<=pl[0] && pl[0]<=Max()[j] )
				over[j]=MYON_Interval(-MYON_DBL_MAX, MYON_DBL_MAX);
			else 
				over[j]=MYON_Interval(MYON_UNSET_VALUE, MYON_UNSET_VALUE);
	}



	// Step 1.  Check for an intersection of the infinte line with the box
	MYON_Interval overlap(-MYON_DBL_MAX, MYON_DBL_MAX);	
	bool nonempty=true;
  int i;
	...
	if(nonempty){	// infinte line intersects box
		if( overlap.Intersection( MYON_Interval(0,1) ) ){
	...
	for(i=0; i<3; i++)
  {
		...
		if(!overlap.Intersection( over[(i+1)%3], over[(i+2)%3] )){	
			...
			MYON_3dVector StdUnit(0,0,0);
			StdUnit[i]=1.0;
			MYON_3dVector n = MYON_CrossProduct(line.Direction(), StdUnit);
			if(n.Length()==0)
				continue;
			n.Unitize();
			int ilo[3]={0,0,0};
	...
	for( k[0]=0; k[0]<2; k[0]++)
		for( k[1]=0; k[1]<2; k[1]++)
			for( k[2]=0; k[2]<2; k[2]++){
				MYON_3dPoint corner = Corner(k[0],k[1],k[2]);
				double tstar;
				line.ClosestPointTo( corner, &tstar );
				MYON_3dPoint cp = line.PointAt( tstar);
				MYON_3dVector disp = cp - corner;
				bool InNCone=true;
  ...
  MYON_ERROR("Should never get here.");
	return 0;  
}				



MYON_3dPoint MYON_BoundingBox::FarPoint( 
  const MYON_3dPoint& test_point
  ) const
{
  MYON_3dPoint far_point = test_point;
 // if ( IsValid() ) {
...
//TODO:  Replace this static function with an MYON_Interval member function.
//  Add to the MYON_Interval class in MYON_Interval.h
//
// returns true if the intersection is non-empty and sets AB to the intersection
//  bool   GetIntersection(MYON_Interval B, MYON_Interval& AB);


static bool Intersect( MYON_Interval A, MYON_Interval B, MYON_Interval& AB);

bool Intersect( MYON_Interval A, MYON_Interval B, MYON_Interval& AB){
	...
	if( A.m_t[0] <= B.m_t[0] && B.m_t[0]<=A.m_t[1] &&  A.m_t[1]<= B.m_t[1]){
		...
		AB = MYON_Interval::EmptyInterval;
		NotEmpty = false;
	}
	return NotEmpty;
}

bool MYON_BoundingBox::GetClosestPoint( 
       const MYON_BoundingBox& other_box, // "other" bounding box
       MYON_3dPoint& this_point, // point on "this" box that is closest to "other" box
       MYON_3dPoint& other_point // point on "other" box that is closest to "this" box
       )  const
{
  MYON_BoundingBox b;
	...
	for (int i=0; i<3; i++ )
  {
		MYON_Interval It(m_min[i],m_max[i]);
		MYON_Interval Io(other_box.m_min[i],other_box.m_max[i]);
		MYON_Interval intersect;
...
bool MYON_BoundingBox::GetFarPoint( 
       const MYON_BoundingBox& other_box, // "other" bounding box
       MYON_3dPoint& this_point, // point on "this" box that is farthest from "other" box point
       MYON_3dPoint& other_point // point on "other" box that is farthest from "this" box point
       )  const
{
	if(!IsValid() || !other_box.IsValid())
		return false;
...
bool MYON_BoundingBox::SwapCoordinates( int i, int j )
{
  bool rc = false;
...
bool MYON_BoundingBox::Expand(MYON_3dVector delta)
{
  m_min -= delta;
  m_max += delta;
  return IsValid();
...
bool MYON_BoundingBox::Shrink(MYON_3dVector delta)
{
  m_min += delta;
  m_max -= delta;
...
bool MYON_BoundingBox::IsDisjoint( const MYON_BoundingBox& other_bbox ) const
{
...
bool MYON_BoundingBox::IsDisjoint(const MYON_Line& line) const
{
  return IsDisjoint(line, false);
}

bool MYON_BoundingBox::IsDisjoint(const MYON_Line& line, bool infinite) const
{
  MYON_3dPoint center = Center();
  MYON_3dPoint halfdiag = Diagonal() * 0.5;

  MYON_3dVector lc = line.PointAt(0.5) - center;
  MYON_3dVector halfdir = (line.to-line.from) * 0.5;
  MYON_3dVector absdir{ fabs(halfdir.x), fabs(halfdir.y), fabs(halfdir.z) };
  ...
  MYON_3dVector cross = MYON_3dVector::CrossProduct(halfdir, lc);
...
bool MYON_BoundingBox::Intersection(
...
bool MYON_BoundingBox::Intersection(				//Returns true when intersect is non-empty. 
				 const MYON_Line& line,		//Infinite Line segment to intersect with 
				 double* t0 ,			// t0  parameter of first intersection point
				 double* t1       // t1  parameter of last intersection point (t0<=t1)   
				 ) const
{		 
	MYON_Interval t(-MYON_DBL_MAX, MYON_DBL_MAX), ti, Li;
...
bool MYON_BoundingBox::Includes( 
    ...
    bool bProperSubSet) const
{
  ...
  {
		MYON_Interval thisI( m_min[i], m_max[i]);
		MYON_Interval otherI( other.m_min[i], other.m_max[i]);
...
bool MYON_BoundingBox::IsValid() const
{
  return IsNotEmpty();
}

bool MYON_BoundingBox::IsEmpty() const
{
  return (m_min.x > m_max.x || m_min.y > m_max.y || m_min.z > m_max.z) && IsSet();
}

bool MYON_BoundingBox::IsNotEmpty() const 
{
  return (m_min.x <= m_max.x && m_min.y <= m_max.y && m_min.z <= m_max.z) && IsSet();
};

bool MYON_BoundingBox::IsPoint() const
{
  return (m_min.x == m_max.x  && m_min.y == m_max.y && m_min.z == m_max.z) && IsSet();
}

bool MYON_BoundingBox::IsSet() const
{
	return ( MYON_IS_VALID(m_min.x)
          && MYON_IS_VALID(m_max.x)
          && MYON_IS_VALID(m_min.y)
          && MYON_IS_VALID(m_max.y)
          && MYON_IS_VALID(m_min.z)
          && MYON_IS_VALID(m_max.z)
         );
}

bool MYON_BoundingBox::IsNan() const
{
...
bool MYON_BoundingBox::IsUnset() const
{
	return ( MYON_UNSET_POSITIVE_VALUE == fabs(m_min.x)
          || MYON_UNSET_POSITIVE_VALUE == fabs(m_max.x)
          || MYON_UNSET_POSITIVE_VALUE == fabs(m_min.y)
          || MYON_UNSET_POSITIVE_VALUE == fabs(m_max.y)
          || MYON_UNSET_POSITIVE_VALUE == fabs(m_min.z)
          || MYON_UNSET_POSITIVE_VALUE == fabs(m_max.z)
         );
}

bool MYON_BoundingBox::IsUnsetOrNan() const
{
  return IsUnset() || IsNan();
}


void MYON_BoundingBox::Dump(MYON_TextLog& text_log) const
{
...
double MYON_BoundingBox::Volume() const
{
  if (IsNotEmpty())
  {
...
double MYON_BoundingBox::Area() const
{
  if (IsNotEmpty())
  {
    const double dx = m_max.x - m_min.x;
...
bool MYON_BoundingBox::Set(     
    int dim, bool is_rat, int count, int stride, 
    const double* points, 
    int bGrowBox
  )
{
  return MYON_GetPointListBoundingBox(dim, is_rat, count, stride, points, *this, bGrowBox!=0, 0 );
}
...
bool MYON_BoundingBox::Set ( const MYON_3dPoint& P, int bGrowBox )
{
  if ( !bGrowBox || !IsValid() )
  {
    m_min = P;
...
bool MYON_BoundingBox::Set ( const MYON_2dPoint& P, int bGrowBox )
{
  const MYON_3dPoint dP(P.x,P.y,0.0);
  return Set(dP,bGrowBox);
}

bool MYON_BoundingBox::Set ( const MYON_3fPoint& P, int bGrowBox )
{
  const MYON_3dPoint dP(P);
  return Set(dP,bGrowBox);
}

bool MYON_BoundingBox::Set ( const MYON_2fPoint& P, int bGrowBox )
{
  const MYON_3dPoint dP(P.x,P.y,0.0);
  return Set(dP,bGrowBox);
}
...
bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_4dPoint>& a, int bGrowBox )
{
  const int count = a.Count();
  const double* p = (count>0) ? &a.Array()->x : 0;
  return MYON_GetPointListBoundingBox(3, 1, count, 4, p, *this, bGrowBox!=0, 0 );
}

bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_3dPoint>& a, int bGrowBox )
{
  const int count = a.Count();
  const double* p = (count>0) ? &a.Array()->x : 0;
  return MYON_GetPointListBoundingBox(3, 0, count, 3, p, *this, bGrowBox!=0, 0 );
}

bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_2dPoint>& a, int bGrowBox )
{
  const int count = a.Count();
  const double* p = (count>0) ? &a.Array()->x : 0;
  return MYON_GetPointListBoundingBox(2, 0, count, 2, p, *this, bGrowBox!=0, 0 );
}

bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_4fPoint>& a, int bGrowBox )
{
  const int count = a.Count();
...
bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_3fPoint>& a, int bGrowBox )
{
  const int count = a.Count();
  const float* p = (count>0) ? &a.Array()->x : 0;
  return MYON_GetPointListBoundingBox(3, 0, count, 3, p, *this, bGrowBox!=0, 0 );
...
bool MYON_BoundingBox::Set( const MYON_SimpleArray<MYON_2fPoint>& a, int bGrowBox )
{
  const int count = a.Count();
  const float* p = (count>0) ? &a.Array()->x : 0;
  return MYON_GetPointListBoundingBox(2, 0, count, 2, p, *this, bGrowBox!=0, 0 );
    ...
    )
{
  ...
  MYON_GetPointListBoundingBox( dim, is_rat, count, stride, points, bbox, false, 0 );
  return bbox;
}
    ...
    if ( points && dim > 0 && (count == 1 || stride >= dim+is_rat) ) 
    {
      ...
      if ( is_rat ) 
      {
        ...
        if ( count > 0 ) 
        {
          if ( !bGrowBox  )
          {
            MYON_ArrayScale( dim, 1.0/points[dim], points, boxmin );
    ...
    if ( points && dim > 0 && (count == 1 || stride >= dim+is_rat) ) 
    {
      if ( is_rat ) {
        ...
        if ( count > 0 ) {
          if ( !bGrowBox ) 
          {
            MYON_ArrayScale( dim, 1.0f/points[dim], points, boxmin );
            memcpy( boxmax, boxmin, dim*sizeof(*boxmax) );
            points += stride;
            count--;
            bGrowBox = true;
    ...
    )
{
  ...
  MYON_GetPointGridBoundingBox( dim, is_rat, 
        ...
        )
{
  ...
  {
    if ( !MYON_GetPointListBoundingBox( dim, is_rat, point_count1, point_stride1, p + i*point_stride0, boxmin, boxmax, bGrowBox ) ) {
      rc = false;
      break;
    }
    else 
  ...
  if ( bbox.IsValid() )
  {
    ...
    if ( bTooFar || bTooBig )
    {
      ...
      if ( 0 != xform )
      {
        ...
        MYON_Xform T(MYON_Xform::TranslationTransformation(-C));
        ...
        MYON_Xform S(MYON_Xform::DiagonalTransformation(s));
...
// generated by the do {...} while(0) in the MYON_ASSERT_OR_RETURN macro.
#pragma MYON_PRAGMA_WARNING_DISABLE_MSC( 4127 )
#endif

  MYON_ASSERT_OR_RETURN( dim > 0 && bboxmin != nullptr && bboxmax != nullptr,tolerance);
  for ( i = 0; i < dim; i++ ) {
    MYON_ASSERT_OR_RETURN(bboxmin[i] <= bboxmax[i],tolerance);
  }
...
#pragma MYON_PRAGMA_WARNING_POP
  ...
  tolerance = MYON_ArrayDistance(dim,bboxmin,bboxmax)*MYON_EPSILON;
  for ( i = 0; i < dim; i++ ) {
    x = (bboxmax[i] - bboxmin[i])*MYON_SQRT_EPSILON;
    if ( x > tolerance )
      tolerance = x;
    x = (fabs(bboxmax[i]) - fabs(bboxmin[i]))*MYON_EPSILON;
    if ( x > tolerance )
      tolerance = x;
  }
  if ( tolerance > 0.0 && tolerance < MYON_ZERO_TOLERANCE )
    tolerance = MYON_ZERO_TOLERANCE;
  return tolerance;
}

int MYON_BoundingBox::IsDegenerate( double tolerance ) const
{
  MYON_3dVector diag = Diagonal();
  if ( tolerance < 0.0 )
  {
...
double MYON_BoundingBox::MinimumDistanceTo( const MYON_3dPoint& P ) const
{
...
double MYON_BoundingBox::MaximumDistanceTo( const MYON_3dPoint& P ) const
{
  // this function must be fast
  // If Q = any point in box, then
...
static double MYON_BBoxMinimumDistanceToHelper( const MYON_BoundingBox& bbox, MYON_Line line )
{
  // 8 Feb 2005 - new function - not tested yet
  ...
  //   returns MYON_UNSET_VALUE if the line misses the box but the minimum distance
...
double MYON_BoundingBox::MinimumDistanceTo( const MYON_Plane& plane ) const
{
...
double MYON_BoundingBox::MinimumDistanceTo( const MYON_PlaneEquation& e ) const
{
  double t, t0, t1;
  MYON_3dPoint P(m_min); // min, min, min
  t0 = t1 = e.ValueAt(P);
...
double MYON_BoundingBox::MaximumDistanceTo( const MYON_Plane& plane ) const
{
  MYON_PlaneEquation e;
  e.Create(plane.origin,plane.zaxis);
  return MinimumDistanceTo(e);
...
double MYON_BoundingBox::MaximumDistanceTo( const MYON_PlaneEquation& e ) const
{
  double t, t0;
  MYON_3dPoint P(m_min); // min, min, min
  t0 = fabs(e.ValueAt(P));
...
bool MYON_BoundingBox::IsFartherThan( double d, const MYON_Plane& plane ) const
{
  MYON_PlaneEquation e;
  e.Create(plane.origin,plane.zaxis);
...
bool MYON_BoundingBox::IsFartherThan( double d, const MYON_PlaneEquation& e ) const
{
  double t, t0, t1;
  MYON_3dPoint P(m_min); // min, min, min
  t0 = t1 = e.ValueAt(P);
...
double MYON_BoundingBox::MinimumDistanceTo( const MYON_Line& line ) const
{
  double d = MYON_BBoxMinimumDistanceToHelper( *this, line );
  if ( d < 0.0 )
  {
    ...
    const MYON_BoundingBox line_bbox(line.BoundingBox());
    MYON_Line edge;
    double e,t;
    int i,j;
    ...
    {    
      ...
      for ( j = 0; j < 2; j++ )
      {
        ...
        if ( MYON_Intersect(edge,line,&e,&t) )
        {
...
double MYON_BoundingBox::MaximumDistanceTo( const MYON_Line& line ) const
{
  // 8 Feb 2005 - new function - not tested yet
...
double MYON_BoundingBox::MinimumDistanceTo( const MYON_BoundingBox& other ) const
{
  // this must be fast
  MYON_3dVector V;
...
double MYON_BoundingBox::MaximumDistanceTo( const MYON_BoundingBox& other ) const
{
  // this must be fast
  MYON_3dVector V;
  double d;
...
bool MYON_BoundingBox::IsFartherThan( double d, const MYON_3dPoint& P ) const
{
  return (d < MinimumDistanceTo(P));
}

bool MYON_BoundingBox::IsFartherThan( double d, const MYON_Line& line ) const
{
  MYON_BoundingBox bbox = *this;
  bbox.m_min.x -= d;
  bbox.m_min.y -= d;
  bbox.m_min.z -= d;
  bbox.m_max.x += d;
  bbox.m_max.y += d;
  bbox.m_max.z += d;
  d = MYON_BBoxMinimumDistanceToHelper( bbox, line );
  // d != 0.0 if and only if line misses the enlarged box
  return (d != 0.0);
}

bool MYON_BoundingBox::IsFartherThan( double d, const MYON_BoundingBox& other ) const
{
  return (d < MinimumDistanceTo(other));
}

void MYON_BoundingBoxAndHash::Set(
  const MYON_BoundingBox& bbox,
...
const MYON_BoundingBox& MYON_BoundingBoxAndHash::BoundingBox() const
{
  return m_bbox;
}

const MYON_SHA1_Hash& MYON_BoundingBoxAndHash::Hash() const
{
  return m_hash;
}

bool MYON_BoundingBoxAndHash::IsSet() const
{
  return m_bbox.IsSet() && MYON_SHA1_Hash::EmptyContentHash != m_hash;
}


bool MYON_BoundingBoxAndHash::Write(
...
bool MYON_BoundingBoxAndHash::Read(
  class MYON_BinaryArchive& archive
)
{
  int version = 0;
...
unsigned int MYON_BoundingBoxCache::Internal_CacheIndex(const MYON_SHA1_Hash& hash) const
{
...
void MYON_BoundingBoxCache::AddBoundingBox(
...
bool MYON_BoundingBoxCache::GetBoundingBox(
  const MYON_SHA1_Hash& hash,
  MYON_BoundingBox& bbox
  ) const
{
  ...
  if (MYON_UNSET_UINT_INDEX == i)
  {
    bbox = MYON_BoundingBox::NanBoundingBox;
    return false;
  }

  if (i > 0)
  {
    const MYON_BoundingBoxAndHash t = m_cache[i];
    while (i > 0)
    {
      m_cache[i] = m_cache[i - 1];
      i--;
...
void MYON_BoundingBoxCache::RemoveAllBoundingBoxes()
{
  m_count = 0;
}

unsigned int MYON_BoundingBoxCache::BoundingBoxCount() const
{
  return m_count;
}


bool MYON_BoundingBoxCache::RemoveBoundingBox(
  const MYON_SHA1_Hash& hash
  )
{
  unsigned int i = Internal_CacheIndex(hash);
...
bool MYON_BoundingBoxCache::Write(
  class MYON_BinaryArchive& archive
) const
{
  const int version = 1;
...
bool MYON_BoundingBoxCache::Read(
  class MYON_BinaryArchive& archive
)
{
  m_count = 0;
