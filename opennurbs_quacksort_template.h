...
#error Define MYON_QSORT_FNAME macro before including opennurbs_quacksort_template.h
#endif
...
#if defined(MYON_SORT_TEMPLATE_USE_CONTEXT)
...
#define BASETYPE MYON_SORT_TEMPLATE_TYPE *
#define DATATYPE MYON_SORT_TEMPLATE_TYPE
#define DATAWIDTH 1

#if defined(MYON_SORT_TEMPLATE_USE_SWAP)
#define Swap(a,b) m_swapfunc(a,b,m_width)
#else
// use intrinsic assignment
#define Swap(a,b) MYON_SORT_TEMPLATE_TYPE tmp = *a; *a = *b; *b = tmp
#endif

#if defined(MYON_SORT_TEMPLATE_COMPARE)
// use a compare function like strcmp for char* strings
#define GreaterThan(A,B) MYON_SORT_TEMPLATE_COMPARE(A,B) > 0
#else
...
#if !defined(MYON_QUACKSORT_SWAP_FUNCS_DEFINED)
#if !defined(MYON_SORT_TEMPLATE_TYPE) || defined(MYON_SORT_TEMPLATE_USE_SWAP)

// In some files this template is used multiple times.
// The MYON_QUACKSORT_SWAP_FUNCS_DEFINED define prevents
// multiple definitions of the static Swap*() functions.
#define MYON_QUACKSORT_SWAP_FUNCS_DEFINED
...
{
  MYON__UINT32* ai = (MYON__UINT32*)a;
  MYON__UINT32* bi = (MYON__UINT32*)b;
  do
  {
    MYON__UINT32 x = *ai;
    *ai++ = *bi;
...
{
  MYON__UINT64* ai = (MYON__UINT64*)a;
  MYON__UINT64* bi = (MYON__UINT64*)b;
  do
  {
    MYON__UINT64 x = *ai;
    *ai++ = *bi;
...
//#endif
      ...
      , m_rnd(62538161)
    {
      ...
      if ( 0 == width%sizeof(MYON__UINT64))
        m_swapfunc = SwapBigInts;
      else if ( 0 == width%sizeof(MYON__UINT32))
        m_swapfunc = SwapInts;
      else
