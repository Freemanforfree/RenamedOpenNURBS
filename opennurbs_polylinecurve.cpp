...
MYON_OBJECT_IMPLEMENT(MYON_PolylineCurve, MYON_Curve, "4ED7D4E6-E947-11d3-BFE5-0010830122F0");

MYON_PolylineCurve::MYON_PolylineCurve() MYON_NOEXCEPT
  : m_dim(3)
{}

MYON_PolylineCurve::~MYON_PolylineCurve()
{}

MYON_PolylineCurve::MYON_PolylineCurve(const MYON_PolylineCurve& src)
  : MYON_Curve(src) // copies user data
  , m_pline(src.m_pline)
  , m_t(src.m_t)
  , m_dim(src.m_dim)
{}

MYON_PolylineCurve& MYON_PolylineCurve::operator=(const MYON_PolylineCurve& src)
{
  if (this != &src)
  {
...
MYON_PolylineCurve::MYON_PolylineCurve(MYON_PolylineCurve&& src) MYON_NOEXCEPT
...
MYON_PolylineCurve& MYON_PolylineCurve::operator=(MYON_PolylineCurve&& src)
{
  if (this != &src)
  {
    MYON_Curve::operator=(std::move(src)); // moves userdata
    m_pline = std::move(src.m_pline);
    m_t = std::move(src.m_t);
    m_dim = src.m_dim;
  }
...
MYON_PolylineCurve::MYON_PolylineCurve(const MYON_3dPointArray& points, const MYON_SimpleArray<double>& params)
{
  *this = points;
...
MYON_PolylineCurve::MYON_PolylineCurve( const MYON_3dPointArray& L )
{
  *this = L;
}


unsigned int MYON_PolylineCurve::SizeOf() const
{
...
MYON__UINT32 MYON_PolylineCurve::DataCRC(MYON__UINT32 current_remainder) const
{
...
void MYON_PolylineCurve::EmergencyDestroy()
{
...
MYON_PolylineCurve& MYON_PolylineCurve::operator=( const MYON_3dPointArray& src )
{
  m_pline = src;
  m_dim   = 3;
...
int MYON_PolylineCurve::Dimension() const
{
  return m_dim;
}

bool MYON_PolylineCurve::GetBBox( // returns true if successful
         double* boxmin,    // minimum
         double* boxmax,    // maximum
         bool bGrowBox
         ) const
{
  return MYON_GetPointListBoundingBox( m_dim, false, PointCount(), 3, m_pline[0], 
...
bool
MYON_PolylineCurve::Transform( const MYON_Xform& xform )
{
...
bool
MYON_PolylineCurve::SwapCoordinates( int i, int j )
{
	DestroyCurveTree();
  return m_pline.SwapCoordinates(i,j);
}

bool MYON_PolylineCurve::IsValid( MYON_TextLog* text_log ) const
{
...
void MYON_PolylineCurve::Dump( MYON_TextLog& dump ) const
{
  MYON_Interval d = Domain();
  dump.Print( "MYON_PolylineCurve:  domain = [%g,%g]\n",d[0],d[1]);
...
bool MYON_PolylineCurve::Write( MYON_BinaryArchive& file ) const
{
...
bool MYON_PolylineCurve::Read( MYON_BinaryArchive& file )
{
  int major_version = 0;
  int minor_version = 0;
...
MYON_Interval MYON_PolylineCurve::Domain() const
{
  MYON_Interval d;
  //bool rc = false;
  const int count = PointCount();
...
bool MYON_PolylineCurve::SetDomain( double t0, double t1 )
{
  bool rc = false;
  ...
  if ( count >= 1 )
  {
    ...
    {
      const MYON_Interval old_domain = Domain();
      const MYON_Interval new_domain(t0,t1);
      m_t[0] = t0;
      m_t[count] = t1;
...
bool MYON_PolylineCurve::ChangeDimension( int desired_dimension )
{
  ...
  if ( rc && m_dim != desired_dimension )
  {
    ...
    if ( 2 == desired_dimension )
    {
      if ( count > 0 )
      {
        ...
        //   zero all z coords.  
        if ( MYON_UNSET_VALUE != m_pline[0].x )
        {
          for ( i = 0; i < count; i++ )
            m_pline[i].z = 0.0;
        }
    ...
    else 
    {
      if ( count > 0 )
      {
        ...
        //   zero all z coords.  
        if ( MYON_UNSET_VALUE != m_pline[0].x && MYON_UNSET_VALUE == m_pline[0].z )
        {
          for ( i = 0; i < count; i++ )
            m_pline[i].z = 0.0;
        }
...
bool MYON_PolylineCurve::ChangeClosedCurveSeam( double t )
{
  ...
  if ( rc )
  {
    ...
    if ( old_dom.Includes(k,true) )
    {
      ...
      int i = MYON_NurbsSpanIndex(2,old_count,m_t.Array(),k,0,0);
      if ( k < m_t[i] )
        return false;
      if ( k >= m_t[i+1] )
        return false;
      ...
      // of one that already exists. MYON_EPSILON didn't quite work but 8 * MYON_EPSILON does
      // comparing t = 4149.8519999999990 to m_t[1] = 4149.8519999999980 in RH-24591
      MYON_Interval span_domain(m_t[i],m_t[i+1]);
      double s = span_domain.NormalizedParameterAt(k);
      double normalized_span_tol = 8.0*MYON_EPSILON;
      ...
      {
        k = span_domain[1];
        i = MYON_NurbsSpanIndex(2,old_count,m_t.Array(),k,0,0);
      }
      ...
      else
      {
        ...
        MYON_SimpleArray<MYON_3dPoint> new_pt(new_count);
        MYON_SimpleArray<double> new_t(new_count);
        MYON_3dPoint new_start = (k==m_t[i]) ? m_pline[i] : PointAt(k);
...
int MYON_PolylineCurve::SpanCount() const
{
  return m_pline.SegmentCount();
}

bool MYON_PolylineCurve::GetSpanVector( // span "knots" 
...
int MYON_PolylineCurve::Degree() const
{
  return 1;
}
...
bool
MYON_PolylineCurve::IsLinear( // true if curve locus is a line segment
      ...
      ) const
{
  ...
  if ( nurbs_curve.m_cv_count >= 2 )
  {
    ...
    const MYON_Curve* ptr = &nurbs_curve;
...
int MYON_PolylineCurve::IsPolyline(
...
bool
MYON_PolylineCurve::IsArc( // true if curve locus in an arc or circle
      ...
      ) const
{
  ...
  if ( nurbs_curve.m_cv_count >= 2 )
  {
    if (m_dim == 2 )
    {
      rc = MYON_Curve::IsPlanar(plane,tolerance);
    }
    else
    {
      nurbs_curve.m_cv = const_cast<double*>(&m_pline[0].x);
...
bool 
MYON_PolylineCurve::IsClosed() const
{
  return m_pline.IsClosed(0.0);
}

bool 
MYON_PolylineCurve::IsPeriodic() const
{
  return false;
}

bool MYON_PolylineCurve::GetNextDiscontinuity( 
                MYON::continuity c,
                double t0,
                double t1,
                double* t,
                ...
                ) const
{
  ...
  if ( segment_count > 0 && t0 != t1 )
  {
    ...
    {
      MYON_3dPoint Pm, Pp;
      MYON_3dVector D1m, D1p, Tm, Tp;

      if ( dtype )
        *dtype = 0;
      c = MYON::PolylineContinuity((int)c);
      MYON::continuity parametric_c = MYON::ParametricContinuity((int)c);
      if ( segment_count >= 2 && parametric_c != MYON::continuity::C0_continuous ) 
      {
        int i = 0;
        int delta_i = 1;
        double s0 = t0;
        double s1 = t1;
        i = MYON_NurbsSpanIndex(2,PointCount(),m_t,t0,0,(hint)?*hint:0);
        double segtol = (fabs(m_t[i]) + fabs(m_t[i+1]) + fabs(m_t[i+1]-m_t[i]))*MYON_SQRT_EPSILON;
        if ( t0 < t1 )
        {
        ...
        else if ( t0 > t1 )
        {
          ...
          // MYON_CurveProxy::GetNextDiscontinuity() works on
        ...
        for ( /*empty*/; !rc && 0 < i && i < segment_count && s0 < m_t[i] && m_t[i] < s1; i += delta_i )
        {
          ...
          if ( parametric_c == MYON::continuity::C1_continuous || parametric_c == MYON::continuity::C2_continuous )
          {
            if ( !(D1m-D1p).IsTiny(D1m.MaximumCoordinate()*MYON_SQRT_EPSILON) )
              rc = true;
          }
          else if ( parametric_c == MYON::continuity::G1_continuous || parametric_c == MYON::continuity::G2_continuous || parametric_c == MYON::continuity::Gsmooth_continuous )
          {
            Tm = D1m;
            Tp = D1p;
            Tm.Unitize();
      ...
      if ( !rc && segment_count > 0 && parametric_c != c )
      {
        ...
        rc = MYON_Curve::GetNextDiscontinuity( c, t0, t1, t, hint, dtype, cos_angle_tolerance, curvature_tolerance );
      }
    }
  }
...
bool MYON_PolylineCurve::IsContinuous(
    ...
    ) const
{
  ...
  if ( segment_count >= 1 )
  {
    ...
    {
      if ( segment_count >= 2 && desired_continuity != MYON::continuity::C0_continuous ) 
      {
        int i = MYON_NurbsSpanIndex(2,PointCount(),m_t,t,0,(hint)?*hint:0);
        
        {
          ...
          if ( m_t[i]+segtol < m_t[i+1]-segtol )
          {
            ...
            {
              t = m_t[i+1];
              i = MYON_NurbsSpanIndex(2,PointCount(),m_t,t,0,(hint)?*hint:0);
            }
          }
        }
...
bool MYON_PolylineCurve::SetStartPoint(
...
bool MYON_PolylineCurve::SetEndPoint(
...
bool 
MYON_PolylineCurve::Evaluate( // returns false if unable to evaluate
       ...
       ) const
{
  ...
  if ( count >= 2 ) 
  {
    int segment_index = MYON_NurbsSpanIndex(2,count,m_t,t,side,(hint)?*hint:0);
    ...
    if ( -2 == side || 2 == side )
    {
      ...
      if ( MYON_TuneupEvaluationParameter( side, m_t[segment_index], m_t[segment_index+1], &a) )
      {
        ...
        t = a;
        segment_index = MYON_NurbsSpanIndex(2,count,m_t,t,side,segment_index);
      }
    }
    ...
    const MYON_3dPoint p = (1.0-s)*m_pline[segment_index] + s*m_pline[segment_index+1];
    v[0] = p.x;
    v[1] = p.y;
    if ( m_dim == 3 )
      v[2] = p.z;
    if ( der_count >= 1 ) {
      v += v_stride;
      MYON_3dVector d = 1.0/(t1-t0)*(m_pline[segment_index+1] - m_pline[segment_index]);
      v[0] = d.x;
      v[1] = d.y;
      if ( m_dim == 3 )
        v[2] = d.z;
...
int MYON_PolylineCurve::PointCount() const
{
  return m_pline.PointCount();
}


bool MYON_PolylineCurve::Append( const MYON_PolylineCurve& c )
{
...
bool MYON_PolylineCurve::ParameterSearch(double t, int& index, bool bEnableSnap) const{
	return MYON_Curve::ParameterSearch( t, index,bEnableSnap, m_t, MYON_SQRT_EPSILON);
}


bool MYON_PolylineCurve::Trim( const MYON_Interval& domain )
{
  ...
  const MYON_Interval original_polyline_domain = Domain();
  ...
  if ( s0 > 0 )
  {
    ...
    MYON_3dPoint* tmp_P = m_pline.Array();
  ...
  {
    MYON_Interval seg_domain(m_t[0],m_t[1]);
    MYON_3dPoint Q0 = m_pline[0];
    MYON_3dPoint Q1 = m_pline[1];
    MYON_Line seg_chord(Q0,Q1);
    double np0 = 0.0;
    double np1 = 1.0;
    bool bSet0 = false;
    bool bSet1 = false;
    ...
    if ( bSet0 )
    {
      if ( np0 >= 1.0-MYON_SQRT_EPSILON && Q0.DistanceTo(Q1) <= MYON_ZERO_TOLERANCE && s1>0 && m_t[1] < actual_trim_domain[1] )
      {
  ...
  if ( bTrimLastSegment )
  {
    MYON_Interval seg_domain(m_t[s1],m_t[s1+1]);
    MYON_3dPoint Q0 = m_pline[s1];
    MYON_3dPoint Q1 = m_pline[s1+1];
    MYON_Line seg_chord(Q0,Q1);
    double np = seg_domain.NormalizedParameterAt(actual_trim_domain[1]);
    Q1 = seg_chord.PointAt(np);
    if ( np <= MYON_SQRT_EPSILON && Q1.DistanceTo(Q0) <= MYON_ZERO_TOLERANCE && s1 > 0 )
    {
...
bool MYON_PolylineCurve::Extend(
...

{
  ...
  if (domain[0] < m_t[0]){
    ...
    if ( len <= 0.0 )
      return false;
    MYON_3dVector V = m_pline[1] - m_pline[0];
    MYON_3dPoint Q0 = m_pline[0];
  ...
  if (domain[1] > m_t[last]){
    ...
    if ( len <= 0.0 )
      return false;
    MYON_3dVector V = m_pline[last] - m_pline[last-1];
    MYON_3dPoint Q1 = m_pline[last];
...
{
  bool rc = false;
  MYON_PolylineCurve* left_pl=0;
  MYON_PolylineCurve* right_pl=0;
  if ( left_side ) 
  {
    left_pl = MYON_PolylineCurve::Cast(left_side);
    if (!left_pl)
      return false;
  }
  if ( right_side )
  {
    right_pl = MYON_PolylineCurve::Cast(right_side);
    if (!right_pl)
      return false;
  }
  ...
  if ( count >= 1 && m_t[0] < t && t < m_t[count] )
       ...
       )
    {
      ...
      {
        if ( !left_pl )
          left_pl = new MYON_PolylineCurve();
        ...
        memcpy( left_pl->m_pline.Array(), m_pline.Array(), left_point_count*sizeof(MYON_3dPoint) );
				if(split_at_break)
        {
      ...
      {
        if ( !right_pl )
          right_pl = new MYON_PolylineCurve();
        ...
        memcpy( right_pl->m_pline.Array(), 
							  ...
							  right_point_count*sizeof(MYON_3dPoint) );
				if( split_at_break)
        {
      ...
      left_pl->Trim( MYON_Interval( left_pl->m_t[0], t ) );
      right_pl->Trim( MYON_Interval( t, *right_pl->m_t.Last() ) );  
		  rc = true;  
    }
...
int MYON_PolylineCurve::GetNurbForm( 
...
int MYON_PolylineCurve::HasNurbForm() const
{
  if (PointCount() < 2)
    return 0;
  if (!IsValid())
...
bool MYON_PolylineCurve::GetCurveParameterFromNurbFormParameter(
      double nurbs_t,
      double* curve_t
      ) const
{
...
bool MYON_PolylineCurve::GetNurbFormParameterFromCurveParameter(
      double curve_t,
      double* nurbs_t
      ) const
{
...
void MYON_PolylineCurve::SetArcLengthParameterization(double tolerance)
{
