...
// Needed to parse RTF in MYON_textContent and to use freetype
  ...
  )
{
  ...
  static const MYON__UINT32 code_page_1252_0x80_to_0xFF[128] =
  {
    ...
    /* cp1252 0x8d */ 0x8d, // MYON_UnicodeCodePoint::MYON_ReplacementCharacter,
  ...
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_10000_0x80_to_0xFF[128] =
  {
  ...
  static const MYON__UINT32 code_page_874_0x80_to_0xFF[128] =
  {
  ...
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1250_0x80_to_0xFF[128] =
  {
  ...
  // 1251: Cyrillic - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1251_0x80_to_0xFF[128] =
  {
  ...
  // 1253: Greek - ANSI
  // Verified using Windows 10 Dec 2017
    static const MYON__UINT32 code_page_1253_0x80_to_0xFF[128] =
  {
    /* 0x80 */ 0x20ac, // Euro Sign
    /* 0x81 */ 0x0081,
    /* 0x82 */ 0x201a, // Single Low-9 Quotation Mark
  ...
  // 1254: Turkish - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1254_0x80_to_0xFF[128] =
  {
    /* 0x80 */ 0x20ac, // Euro Sign
    /* 0x81 */ 0x0081,
    /* 0x82 */ 0x201a, // Single Low-9 Quotation Mark
  ...
  // 1255: Hebrew - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1255_0x80_to_0xFF[128] =
  {
    /* 0x80 */ 0x20ac, // Euro Sign
  ...
  // 1256: Arabic - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1256_0x80_to_0xFF[128] =
  {
    /* 0x80 */ 0x20ac	, // Euro Sign
  ...
  // 1257: Baltic - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1257_0x80_to_0xFF[128] =
  {
    /* 80 */ 0x20ac, // Euro Sign
    /* 81 */ 0x0081,
  ...
  // 1258: Viet Nam - ANSI
  // Verified using Windows 10 Dec 2017
  static const MYON__UINT32 code_page_1258_0x80_to_0xFF[128] =
  {
    /* 0x80 */ 0x20ac	, // Euro Sign
    /* 0x81 */ 0x0081	, // Undefined -> Control
    /* 0x82 */ 0x201a	, // Single Low-9 Quotation Mark
...
public:
  MYON__UINT32 m_cp_encoding = 0;
  MYON__UINT32 m_unicode_code_point = 0;
};
...
{
  const MYON__UINT32 lhs_code_point = (nullptr != lhs) ? ((const class CodePageEncodingUnicodeCodePoint*)lhs)->m_unicode_code_point : 0xFFFFFFFF;
  const MYON__UINT32 rhs_code_point = (nullptr != rhs) ? ((const class CodePageEncodingUnicodeCodePoint*)rhs)->m_unicode_code_point : 0xFFFFFFFF;
...
public:
  ...
  MYON__UINT32 CodePage() const
  {
    return m_code_page;
  }

  MYON__UINT32 MapUnicodeToWindowsSingleByteCodePage(
...
private:
  const MYON__UINT32 m_code_page = 0;
  const MYON__UINT32 m_unicode_to_cp_count = 0;
  CodePageEncodingUnicodeCodePoint* m_unicode_to_cp = nullptr;

private:
  InternalUnicodeToCodePage(
    MYON__UINT32 code_page,
    const MYON__UINT32 unicode_to_cp_count,
...
)
{
  for (;;)
  {
    ...
    MYON__UINT32 a_count = 0;
    for (MYON__UINT32 i = 0; i < 128; i++)
    {
      if (cp_to_unicode[i] < 0x80 )
        continue;
      if (MYON_UnicodeCodePoint::MYON_ReplacementCharacter == cp_to_unicode[i])
        continue;
      if (0 == MYON_IsValidUnicodeCodePoint(cp_to_unicode[i]))
        continue;
    ...
    MYON_qsort(a, a_count, sizeof(a[0]), CodePageEncodingUnicodeCodePoint::CompareUnicodeCodePoint);
...
)
{
  ...
  for (;;)
  {
    if (MYON_UnicodeCodePoint::MYON_ReplacementCharacter == unicode_code_point )
      break;
...
)
{
  ...
  else
  {
    const MYON__UINT32* a
      ...
      ? MYON_MSSBCP_0x80_0xFF_Unicode(code_page)
      : nullptr;
    code_point
      = (nullptr != a)
...
)
{
  ...
  if (char_encoding > 0xFF)
  {
    ...
    MYON_ERROR("Invalid char_encoding parameter.");
    return 0;
  }

  const MYON__UINT32 unicode_code_point = MYON_MapMSSBCPToUnicode(code_page, char_encoding);
  if (MYON_UnicodeCodePoint::MYON_ReplacementCharacter == unicode_code_point)
  {
    rc = 2;
  }
  ...
  {
    const MYON__UINT32 c = MYON_MapUnicodeToMSSBCP(code_page, unicode_code_point);
    if (c == char_encoding)
    {
      rc = 1;
    }
    ...
    {
      rc = 0;
      text_log.Print("Code page %u char %02x U+%04X MYON_MapUnicodeToMSSBCP() = %02x\n",
            code_page,
            char_encoding,
            unicode_code_point,
            c
  ...
  while (bWindowsAPITest)
  {
    ...
    wchar_t w[16] = {};
    MYON__UINT32 ucp = MYON_UnicodeCodePoint::MYON_InvalidCodePoint;
    ...
    struct MYON_UnicodeErrorParameters e = MYON_UnicodeErrorParameters::FailOnErrors;
    const int decode_count = MYON_DecodeWideChar(w, w_count, &e, &ucp);
...
static bool MYON_InternalIsDoubleByteCodePage(
  const MYON__UINT32 code_page
)
{
  bool rc;
...
)
{
  ...
  bWindowsAPITest = false;
#endif
  const MYON__UINT32 char_encoding_max
    = MYON_InternalIsDoubleByteCodePage(code_page)
    ? 0x7F  // veriy identity map for values 0x00 to 0x7E
    : 0xFF;
  for (MYON__UINT32 char_encoding = 0; char_encoding <= char_encoding_max; char_encoding++)
  {
    switch (MYON_Test_MSSBCP(code_page, char_encoding, bWindowsAPITest, text_log))
    {
    case 0:
      fail_count++;
      break;
...
)
{
  ...
  {
    const MYON__UINT32 code_page = code_pages[i];
    text_log.Print(
      "Testing code page %u:\n",
      code_page
    );
    text_log.PushIndent();
    if (MYON_Test_MSSBCP(code_page,bWindowsAPITest,text_log))
    {
      pass_count++;
...
)
{
  ...
  MYON__UINT32 error_count = 0;
  text_log.Print(
    "const MYON__UINT32 mssb_code_page_%u_0x%02x_0x%02x_unicode[%u] =\n",
    code_page,
    char_encoding0,
    char_encoding1, 
  ...
  text_log.Print("{\n");
  text_log.PushIndent();
  for (MYON__UINT32 char_encoding = char_encoding0; char_encoding <= char_encoding1; char_encoding++)
  {
    MYON__UINT32 unicode_code_point = MYON_UnicodeCodePoint::MYON_InvalidCodePoint;
    for (;;)
    {
      const char mbcs[2] = { (char)char_encoding,0 };
      wchar_t w[16] = {};
      ...
      struct MYON_UnicodeErrorParameters e = MYON_UnicodeErrorParameters::FailOnErrors;
      MYON__UINT32 u = MYON_UnicodeCodePoint::MYON_InvalidCodePoint;
      const int decode_count = MYON_DecodeWideChar(w, w_count, &e, &u);
      if (false == MYON_IsValidUnicodeCodePoint(u))
        break;
    ...
    if (MYON_UnicodeCodePoint::MYON_InvalidCodePoint == unicode_code_point)
    {
...
#else
  MYON_ERROR("Platform not supported.");
  return false;
#endif
}
