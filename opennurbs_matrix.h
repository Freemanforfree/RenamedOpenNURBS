...
#if !defined(MYON_MATRIX_INC_)
#define MYON_MATRIX_INC_

class MYON_Xform;

class MYON_CLASS MYON_Matrix
{
public:
  MYON_Matrix();
  MYON_Matrix( 
  ...
  MYON_Matrix( // see MYON_Matrix::Create(int,int,int,int) for details
     int, // first valid row index
     int, // last valid row index
     int, // first valid column index
     int  // last valid column index
     );
  MYON_Matrix( const MYON_Xform& );
  MYON_Matrix( const MYON_Matrix& );

#if defined(MYON_HAS_RVALUEREF)
  // rvalue copy constructor
  MYON_Matrix(MYON_Matrix&&) MYON_NOEXCEPT;
  ...
  MYON_Matrix& operator=(MYON_Matrix&&);
#endif

  /*
  Description:
    This constructor is for experts who have storage for a matrix
    and need to use it in MYON_Matrix form.
  Parameters:
    row_count - [in]
    col_count - [in]
    M - [in]
    bDestructorFreeM - [in]
      If true, ~MYON_Matrix will call onfree(M).
      If false, caller is managing M's memory.
  Remarks:
    MYON_Matrix functions that increase the value of row_count or col_count
  ...
  */
  ...
  Returns:
    ...
    deleted by calling MYON_Matrix::Deallocate().
  */
  ...
  // MYON_Matrix[i][j] = value at row i and column j
  //           0 <= i < RowCount()
  //           0 <= j < ColCount()
  double* operator[](int);
  const double* operator[](int) const;

  MYON_Matrix& operator=(const MYON_Matrix&);
  MYON_Matrix& operator=(const MYON_Xform&);
  ...
  */
  ...
  void SetDiagonal(const MYON_SimpleArray<double>&); // sets size to length X lengthdiagonal values and zeros off diagonal values
  ...
  */
  bool Multiply( const MYON_Matrix& A, const MYON_Matrix& B );
  ...
  */
  bool Add( const MYON_Matrix& A, const MYON_Matrix& B );
  ...
  */
  ...
  //   double B[n] = ...;
  //   MYON_Matrix M(m,n) = ...;
  //   M.RowReduce(MYON_ZERO_TOLERANCE,B); // modifies M and B
...
private:
  ...
  MYON_SimpleArray<double*> m_rowmem; 
