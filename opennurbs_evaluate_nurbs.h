...
#if !defined(MYON_EVALUATE_NURBS_INC_)
#define MYON_EVALUATE_NURBS_INC_
...
MYON_DECL
bool MYON_RemoveBezierSingAt0( // input bezier is rational with 0/0 at start
        int,    // dimension 
        int,    // order (>=2)
        int,    // cv_stride (>=dim+1)
...
MYON_DECL
bool MYON_RemoveBezierSingAt1( // input bezier is rational with 0/0 at end
        int,    // dimension 
        int,    // order (>=2)
        int,    // cv_stride (>=dim+1)
        double* // cv[order*cv_stride] array
...
MYON_DECL
double MYON_EvaluateBernsteinBasis( // returns (i choose d)*(1-t)^(d-i)*t^i
        int, // degree, 
        int, // 0 <= i <= degree
        double //  t
        );
...
COMMENTS:
  ...
  N_{i}(t) = 0 if i <= k-d
           ...
           = B[i-k+d-1] if k-d+1 <= i <= k+1, where B[] is computed by the call
             MYON_EvaluateNurbsBasis( d+1, knot+k-d+1, t, B );

  If 0 <= j < n-d, 0 <= m <= d, knot[j+d-1] <= t < knot[j+d], and B[] is 
  computed by the call
    MYON_EvaluateNurbsBasis( d+1, knot+j, t, B ),
  then 
    N_{j+m}(t) = B[m].
*/
MYON_DECL
...
Returns:
  True if successful.
See Also:
  MYON_NurbsCurve::Evaluate
  MYON_EvaluateNurbsSurfaceSpan
  MYON_EvaluateNurbsCageSpan
*/
MYON_DECL
...
Returns:
  True if successful.
See Also:
  MYON_NurbsSurface::Evaluate
  MYON_EvaluateNurbsSpan
  MYON_EvaluateNurbsCageSpan
*/
MYON_DECL
...
MYON_DECL
bool MYON_EvaluateNurbsDeBoor( // for expert users only - no support available
...
MYON_DECL
void MYON_ConvertNurbSpanToBezier(
