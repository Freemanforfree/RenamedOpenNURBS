...
// MYON_V5x_DimStyle is used to read and write version 5 and earlier archives.
// MYON_DimStyle is the class for runtime dimension style.
class MYON_V5x_DimStyle : public MYON_ModelComponent
{
  MYON_OBJECT_DECLARE(MYON_V5x_DimStyle);

private:
  friend class MYON_DimStyle;

public:
  enum eArrowType
  {
    // eArrowType is used for V5 and earlier dimensions
    // V6 dimensions (MYON_Dimension) use   MYON_Arrowhead::arrow_type
    solidtriangle = 0,    // 2:1
    dot = 1,
    tick = 2,
    shorttriangle = 3,    // 1:1
...
public:
  MYON_V5x_DimStyle();
  ~MYON_V5x_DimStyle();
  MYON_V5x_DimStyle(const MYON_V5x_DimStyle&) = default;
  MYON_V5x_DimStyle& operator=(const MYON_V5x_DimStyle&) = default;

public:
  MYON_V5x_DimStyle( const class MYON_3dmAnnotationSettings& src);
  MYON_V5x_DimStyle(
    MYON::LengthUnitSystem model_length_unit_system,
    const class MYON_DimStyle& src
  );

public:
  bool CompareDimstyle(const MYON_V5x_DimStyle& src) const;
  bool CompareValidFields(const MYON_V5x_DimStyle& src) const;
...
public:
  ...
  MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode TextAlignment() const;
  void SetTextAlignment( MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode);

  int ArrowType() const;           // For MYON_OBSOLETE_V2_Annotation & MYON_OBSOLETE_V5_Annotation derived dimensions
  void SetArrowType( eArrowType);  // MYON_Dimension derived dimensions use ArrowType1() and ArrowType2()
  ...
  const class MYON_TextStyle& V5TextStyle() const;
  void SetV5TextStyle( 
    const class MYON_TextStyle& v5_text_style
    );
  ...
  enum class Field : unsigned int
  {
    ...
    fn_textheight                     = 7,
    fn_textalign                      = 8,
    fn_arrowtype                      = 9,  // For v5 and previous MYON_OBSOLETE_V2_Annotation and MYON_OBSOLETE_V5_Annotation dimensions
    fn_angularunits                   = 10,
    fn_lengthformat                   = 11,
    fn_angleformat                    = 12,
  ...
  // version 2.0
    ...
    fn_dim_arrow_type1                = 84,  // For MYON_Dimension derived dimensions
  ...
  enum : unsigned int
  {
    // must be 1 + the maximum value of an MYON_V5x_DimStyle::Field enum value.
    FieldCount = 88  
  };
  ...
  public:
    DimstyleField()
      : m_next(nullptr)
      , m_field_id(MYON_V5x_DimStyle::Field::fn_unset)
    {
      m_val.s_val = nullptr;
    }
    ~DimstyleField()
    ...
    MYON_V5x_DimStyle::Field m_field_id;
    union
    {
      bool b_val;
      int  i_val;
  ...
  bool IsFieldOverride(MYON_V5x_DimStyle::Field field_id) const;
  // Set a field to be overridden or not
  // Fields that aren't overrides inherit from their parent dimstyle
  void SetFieldOverride(MYON_V5x_DimStyle::Field field_id, bool bOverride);
  ...
  */
  ...
  bool OverrideFields( const MYON_V5x_DimStyle& source, const MYON_V5x_DimStyle& parent);
  ...
  bool InheritFields( const MYON_V5x_DimStyle& parent);
  ...
  bool IsChildOf(const MYON_UUID& parent_uuid) const;

  // use MYON_ModelComponent parent id - // MYON_UUID ParentId() const;

  // Set the parent of this dimstyle
  // use MYON_ModelComponent parent id - //void SetParentId(MYON_UUID parent_uuid);
  ...
  void SetArrowType1(MYON_Arrowhead::arrow_type);  // MYON_Dimension derived dimensions
  ...
  const MYON_Arrowhead& Arrowhead1() const;
  const MYON_Arrowhead& Arrowhead2() const;
  ...
  static MYON_Color DefaultMaskColor(); // .SetRGB(255,255,255);
  static double   DefaultDimScale(); // 1.0;
  static int      DefaultDimScaleSource(); // 0;

  bool CompareFields(const MYON_V5x_DimStyle& other) const;
...
public:
  ...
  MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode m_dimstyle_textalign = MYON_INTERNAL_OBSOLETE::V5_TextDisplayMode::kAboveLine;   // text alignment relative to the dimension line
  int m_arrowtype = 0;     // 0: filled narrow triangular arrow  - For MYON_OBSOLETE_V2_Annotation & MYON_OBSOLETE_V5_Annotation derived dimensnions
  // m_arrowtype = ((MYON_Arrowhead::arrow_type enum value as int) - 2)
...
private:
  MYON_TextStyle m_v5_text_style = MYON_TextStyle::Default;
...
public:
  ...
  MYON_wString m_alternate_prefix;    // string preceding alternate value string
  MYON_wString m_alternate_suffix;    // string following alternate value string
...
private:
  friend class MYON_DimStyleExtra;
  // 8 Apr, 2014 - The next few fields were transferred from MYON_DimStyleExtra for V6
  /// Use MYON_ModelComponent.ParentId() /// MYON_UUID m_parent_dimstyle = MYON_nil_uuid;  // MYON_nil_uuid if there is no parent dimstyle
  unsigned int m_field_override_count = 0; // number of 
  bool m_field_override[MYON_V5x_DimStyle::FieldCount];
...
public:
  ...
  // End of fields that were in MYON_DimStyleExtra
  ...
  //MYON_Arrowhead::arrow_type m_arrow_type_1;      // Arrow types for MYON_Dimension derived dimensions
  //MYON_Arrowhead::arrow_type m_arrow_type_2;
  //MYON_wString      m_dim_arrow_block1;
  //MYON_wString      m_dim_arrow_block2;

  MYON_Arrowhead    m_arrow_1;
  MYON_Arrowhead    m_arrow_2;
};

void MYON_Internal_FixBogusDimStyleLengthFactor(
  const class MYON_BinaryArchive& file,
