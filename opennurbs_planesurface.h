...
#if !defined(MYON_GEOMETRY_SURFACE_PLANE_INC_)
#define MYON_GEOMETRY_SURFACE_PLANE_INC_

class MYON_PlaneSurface;

class MYON_CLASS MYON_PlaneSurface : public MYON_Surface
{
  MYON_OBJECT_DECLARE(MYON_PlaneSurface);

public:
  MYON_PlaneSurface();
  MYON_PlaneSurface(const MYON_PlaneSurface&);
  MYON_PlaneSurface& operator=(const MYON_PlaneSurface&);

  MYON_PlaneSurface(const MYON_Plane&);
  MYON_PlaneSurface& operator=(const MYON_Plane&);

  virtual ~MYON_PlaneSurface();

  // An MYON_PlaneSurface is really a finite rectangle.
  ...
  */
  ...
  Example:
          ...
          MYON_PlaneSurface srf = ...;
          int dir = 1;
          MYON_PlaneSurface* south_side = 0;
          MYON_PlaneSurface* north_side = 0;
  ...
  */
  bool CreatePseudoInfinitePlaneTight(
    const MYON_PlaneEquation& plane,
    const MYON_BoundingBox& bbox,
    double padding = 0.0625
  );
...
protected:
  ...
  MYON_Interval m_domain[2]; // always increasing

  // rectangle extents (in m_plane x,y coordinates)
  MYON_Interval m_extents[2];
};


class MYON_CLASS MYON_ClippingPlaneSurface : public MYON_PlaneSurface
{
  MYON_OBJECT_DECLARE(MYON_ClippingPlaneSurface);
public:
  MYON_ClippingPlaneSurface();
  MYON_ClippingPlaneSurface(const MYON_Plane& src);
  MYON_ClippingPlaneSurface(const MYON_PlaneSurface& src);
  ~MYON_ClippingPlaneSurface();

  MYON_ClippingPlaneSurface& operator=(const MYON_Plane& src);
  MYON_ClippingPlaneSurface& operator=(const MYON_PlaneSurface& src);

  void Default();

  // override MYON_Object::ObjectType() - returns MYON::clipplane_object
  ...
  MYON_ClippingPlane m_clipping_plane;
};
