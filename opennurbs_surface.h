...
class MYON_Mesh;
class MYON_MeshParameters;
class MYON_PolyCurve;
class MYON_CurveProxy;
class MYON_Surface;


/* Return codes to be used in operations that attempt to fit to a tolerance.  
   For example MYON_Surface::Pullback() and MYON_Surface::PushUp().
*/

enum class MYON_FitResult: unsigned int
...
class MYON_CLASS MYON_Surface : public MYON_Geometry
{
  MYON_OBJECT_DECLARE(MYON_Surface);
...
public:
  ...
  // MYON_BrepTrim::m_iso uses these flags
  enum ISO 
  {
...
public:
  MYON_Surface();
  MYON_Surface(const MYON_Surface&);
  MYON_Surface& operator=(const MYON_Surface&);
  virtual ~MYON_Surface();
  ...
  Remarks:
    ...
    For non-MYON_SurfaceProxy objects, this simply duplicates the surface using
    MYON_Object::Duplicate.
    For MYON_SurfaceProxy objects, this duplicates the actual proxy surface 
  ...
  */
  virtual
  MYON_Surface* DuplicateSurface() const;

  //////////
  // override MYON_Object::ObjectType() - returns MYON::surface_object
  MYON::object_type ObjectType() const override;


  /////////////////////////////
  //
  // virtual MYON_Geometry functions
  //

  /*
  Description:
    Overrides virtual MYON_Geometry::HasBrepForm and returns true.
  Result:
    Returns true.
  See Also:
  ...
  */
  ...
  /*
  Description:
    Overrides virtual MYON_Geometry::HasBrepForm.  
    Uses MYON_Brep::Create( MYON_Surface&* ) to create a brep
  ...
  */
  ...
  Example:
          ...
          {
            srf->SetDomain( 0, MYON_Interval( 0.0, width ) );
            srf->SetDomain( 1, MYON_Interval( 0.0, height ) );
          }
  ...
  */
  ...
  Remarks:
    Because it may transpose domains, MYON_SurfaceProxy overrides
  ...
  */
  ...
  MYON_3dPoint  PointAt( double, double ) const;
  MYON_3dVector NormalAt( double, double ) const;
  bool FrameAt( double u, double v, MYON_Plane& frame) const;
  ...
  bool Ev1Der( // returns false if unable to evaluate
         ...
         MYON_3dPoint& point,    // returns value of surface
         MYON_3dVector& du,      // first partial derivatives (Ds)
         MYON_3dVector& dv,      // (Dt)
         int quadrant = 0,     // optional - determines which side to evaluate from
                               //         0 = default
                               //         1 from NE quadrant
  ...
  bool Ev2Der( // returns false if unable to evaluate
         ...
         MYON_3dVector& du,      // first partial derivatives (Ds)
         MYON_3dVector& dv,      // (Dt)
         MYON_3dVector& duu,     // second partial derivatives (Dss)
         MYON_3dVector& duv,     // (Dst)
         MYON_3dVector& dvv,     // (Dtt)
         int quadrant= 0,      // optional - determines which side to evaluate from
                               //         0 = default
                               //         1 from NE quadrant
                               //         2 from NW quadrant
  ...
  Remarks:
    ...
    In the other MYON_Surface functions that take a "dir"
  ...
  */
  ...
  Remarks:
    ...
    then MYON_Surface::NurbsSurface may be easier to use.
  See Also:
    MYON_Surface::NurbsSurface
  */
  virtual
  int GetNurbForm(
  ...
  */
  ...
  // See Also:
  //   MYON_Surface::GetNurbForm
  MYON_NurbsSurface* NurbsSurface(
        MYON_NurbsSurface* pNurbsSurface = nullptr,
        double tolerance = 0.0,
        const MYON_Interval* s_subdomain = nullptr,
        const MYON_Interval* t_subdomain = nullptr
        ) const;
...
class MYON_CLASS MYON_SurfaceProperties
{
  // Surface properties
public:
  // The constructor sets all fields to zero.
  MYON_SurfaceProperties();
  ...
  */
  void Set( const MYON_Surface* surface );
  ...
  bool m_bIsSingular[4];   // m_bSingular[i] = MYON_Surface::IsSingular(i)

  bool m_bHasSeam;         // true if at least one m_bClosed[] setting is true.
  bool m_bIsClosed[2];     // m_bClosed[i] = MYON_Surface::IsClosed(i)

private:
  bool m_bReserved[7];

public:
  MYON_Interval m_domain[2]; // m_domain[i] = MYON_Surface.Domain(i)

private:
  unsigned char m_reserved[16];

public:
  // Last pointer passed to MYON_SurfaceProperties::Set().
...
MYON_DLL_TEMPLATE template class MYON_CLASS MYON_SimpleArray<MYON_Surface*>;
#endif

class MYON_CLASS MYON_SurfaceArray : public MYON_SimpleArray<MYON_Surface*>
{
public:
  MYON_SurfaceArray( int = 0 );
  ~MYON_SurfaceArray();
  ...
  bool Duplicate( MYON_SurfaceArray& ) const; // operator= copies the pointer values
...
void MYON_DECL MYON_IsG1Closed(const MYON_Surface& Srf, bool closed[2]);
